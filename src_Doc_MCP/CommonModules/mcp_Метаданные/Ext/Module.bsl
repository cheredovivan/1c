...
Процедура ЗаполнитьТаблицуИнструментов(ТаблицаИнструментов) Экспорт
	...
	Для Каждого МД Из СоставПодсистемыКонтейнеры Цикл
		...
		// Проверяем наличие метода ДобавитьИнструменты и обрабатываем ошибки
		Попытка
			...
			КонецЦикла;
		Исключение
			// Метод ДобавитьИнструменты не реализован или возникла ошибка в обработке - пропускаем эту обработку
			// Логируем ошибку, но не прерываем обработку остальных инструментов
			СообщениеОшибки = СтрШаблон("Ошибка при получении инструментов из обработки '%1': %2", 
				МД.Имя, 
				ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
			// Записываем в журнал регистрации для диагностики
			Попытка
				ЗаписьЖурналаРегистрации("MCP.Инструменты", УровеньЖурналаРегистрации.Ошибка, , , СообщениеОшибки);
			Исключение
				// Игнорируем ошибку записи в журнал
				ОшибкаЛогирования = ОписаниеОшибки();
			КонецПопытки;
			// Продолжаем обработку остальных обработок
		КонецПопытки;
