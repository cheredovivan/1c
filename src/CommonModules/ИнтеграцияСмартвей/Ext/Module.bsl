...
Процедура ЗаполнитьБронирование(ДокументОбъект, СтрокаДокумента)
	...
	Если Не ЗначениеЗаполнено(СтрокаДокумента.ФизЛицоСсылка) Тогда
		...
		Запрос.Текст =
		"ВЫБРАТЬ ПЕРВЫЕ 1
		|	Бронирование.ПодразделениеСписания КАК ПодразделениеСписания,
		|	Бронирование.НаправлениеДеятельностиДляСписания КАК НаправлениеДеятельностиДляСписания,
		|	Бронирование.СтатьяРасходов КАК СтатьяРасходов,
		|	Бронирование.АналитикаРасходов КАК АналитикаРасходов,
		|	Бронирование.АналитикаАктивовПассивов КАК АналитикаАктивовПассивов,
		|	Бронирование.НастройкаСчетовУчета КАК НастройкаСчетовУчета
		|ИЗ
		|	Документ.Бронирование КАК Бронирование
		|ГДЕ
		|	Бронирование.Организация = &Организация
		|	И Бронирование.СпособСписанияНапрямую
		|	И Бронирование.Проведен
		|
		|УПОРЯДОЧИТЬ ПО
		|	Бронирование.Дата УБЫВ";
