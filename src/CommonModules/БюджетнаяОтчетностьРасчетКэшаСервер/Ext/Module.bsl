...
Процедура ДобавитьПодчиненныйНаборПоБюджетномуРегистру(ПараметрыПриемника,
	...
	УсловияКомпоновкиВиртуальнойТаблицыИсточника = Новый Массив;
	...
	Если Не ХранимыеФактическиДанные Тогда
		УсловияКомпоновкиВиртуальнойТаблицыИсточника.Добавить("(Статус).* КАК Статус");
	КонецЕсли;
		...
		Если НайтиОтборПоИмени(Источник.ДополнительныйОтбор, КлючИЗначение.Ключ,, Истина) Тогда
			Если ХранимыеФактическиДанные Тогда
				Если КлючИЗначение.Ключ = "Сценарий" Тогда
					...
					Иначе
						УсловияКомпоновкиВиртуальнойТаблицыИсточника.Добавить(Выражение);
					КонецЕсли;
					Продолжить;
				КонецЕсли;
			КонецЕсли;
	...
	КонецЕсли;
	Если УсловияКомпоновкиВиртуальнойТаблицыИсточника.Количество() > 0 Тогда
		Выражение = СтрСоединить(УсловияКомпоновкиВиртуальнойТаблицыИсточника, ", ");
	...
	Если Не ПустаяСтрока(ЗапросПоПравиламРасчета.ТекстЗапроса) Тогда
		ЗапросПоПравиламРасчета.ТекстЗапроса = СтрЗаменить(ЗапросПоПравиламРасчета.ТекстЗапроса,
			"&УсловияПоТаблицеИсточникаИПоляКомпановки", УсловияПоТаблицеИсточникаИПоляКомпановки);
		Если Не ПустаяСтрока(ЗапросПоПравиламРасчета.ТекстЗапросаВТ) Тогда
