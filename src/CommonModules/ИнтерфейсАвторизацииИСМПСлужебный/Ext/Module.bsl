...
Функция ОбновитьКлючСессииНаСервере(ПараметрыЗапроса, ОбновлятьКлючСессииНаСервере = Истина) Экспорт
	...
	Если СтрокаСертификата <> Неопределено Тогда
		...
		Если РезультатЗапроса.ПараметрыАвторизации.UnitedToken Тогда
			СертификатМЧД = ОбщегоНазначенияИС.ТребуетсяДоверенностьДляИспользованияСертификата(
				СтрокаСертификата.Сертификат, ПараметрыЗапроса.Организация, РезультатЗапроса.ПараметрыАвторизации.ИНН);
			Если СертификатМЧД Тогда
				РезультатЗапроса.ПараметрыАвторизации.Данные = РезультатЗапроса.ПараметрыАвторизации.ИНН;
			КонецЕсли;
		КонецЕсли;
