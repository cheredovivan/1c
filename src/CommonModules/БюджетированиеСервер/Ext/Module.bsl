...
Процедура ПроверитьДоступностьПолейЗаполненияАналитики(ПараметрыЗаполненияАналитик, Правило, ОчищатьНедоступныеНастройки = Ложь, ИмяТабличнойЧасти = "") Экспорт
...
//
Процедура УстановитьНастройкиЗаполненияАналитикиАвтоматически(ПараметрыЗаполненияАналитик, ТаблицаНастроек, ВыраженияЗаполненияАналитики = Неопределено) Экспорт
	...
	Если ЭтоАдресВременногоХранилища(ПараметрыЗаполненияАналитик.АдресСхемыКомпоновкиДанных) Тогда
		...
		Если ВыраженияЗаполненияАналитики = Неопределено Тогда
			Если ТипЗнч(ТаблицаНастроек) = Тип("ТаблицаЗначений") Тогда
				ВидыАналитик = ТаблицаНастроек.Скопировать(, "НомерАналитики, ВидАналитики");
			Иначе
				ВидыАналитик = ТаблицаНастроек.Выгрузить(, "НомерАналитики, ВидАналитики");
			КонецЕсли;
			
			ВыраженияЗаполненияАналитики = ВыраженияЗаполненияАналитикиПоСхемеКомпоновкиДанных(СхемаКомпоновкиДанных, ВидыАналитик);
		КонецЕсли;
		
		Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(ПараметрыЗаполненияАналитик, "НастройкиЗаполненияАналитики") Тогда
			ПутьКТаблицеНастроек = "НастройкиЗаполненияАналитики";
		Иначе
			ПутьКТаблицеНастроек = "Объект.НастройкиЗаполненияАналитик";
		КонецЕсли;
		...
		ШаблонПоля = ПутьКТаблицеНастроек + "[%1].ПредставлениеВыраженияЗаполненияАналитики";
		КонтекстФормы = Не ТипЗнч(ТаблицаНастроек) = Тип("ТаблицаЗначений");
		...
		Для каждого СтрокаНастройки Из ТаблицаНастроек Цикл
			...
			Если ЗначениеЗаполнено(РезультатПоиска.Выражение) Тогда
				...
				КонецЕсли;
				Если ПутьКТаблицеНастроек = "НастройкиЗаполненияАналитики" Тогда
...
Процедура ЗаполнитьРеквизитыОбъектаНастроекЗаполненияАналитики(Объект) Экспорт
	...
	УстановитьНастройкиЗаполненияАналитикиАвтоматически(ПараметрыЗаполненияАналитик,
		ПараметрыЗаполненияАналитик.НастройкиЗаполненияАналитики);
...
Функция СформироватьЗапросКИсточникуДанныхПоПравиламРасчета(ПравилаРасчета, ИдентификаторИсточника) Экспорт
	...
	ЗапросПоПравиламРасчета = Новый Структура("ТекстЗапроса, ТекстЗапросаВТ, ПараметрыЗапроса", "", "", Новый Структура);
