...
Процедура ОбновитьПоставляемыеДрайвера(ДрайвераОборудования) Экспорт
	...
	Драйвер.Наименование = НСтр("ru = 'Дримкас:ККТ с передачей данных в ОФД (ФФД 1.2) УСН НДС';
								|en = 'Dreamkas:Cash register with data transfer to FDO (FDF 1.2) STS VAT'", ОбщегоНазначенияБПО.КодОсновногоЯзыка());
	...
	Драйвер.Наименование = НСтр("ru = 'Инитпро:ККТ с передачей данных в ОФД (ФФД 1.2) УСН НДС';
								|en = 'Initpro:Cash register with data transfer to FDO (FDF 1.2) STS VAT'", ОбщегоНазначенияБПО.КодОсновногоЯзыка());
