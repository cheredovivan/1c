...
Функция ТекстЗапросаРеквизитыСчетовФактурПостановление981КнигаПокупок() Экспорт
	...
	|	Операция.Организация,
	|	ВЫБОР
	|		КОГДА Операция.ХозяйственнаяОперация = Значение(Перечисление.ХозяйственныеОперации.БронированиеЧерезАгента)
	|			ТОГДА Операция.КонтрагентАгент
	|		ИНАЧЕ Операция.КонтрагентПеревозчик
	|	КОНЕЦ,
