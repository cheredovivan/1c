...
Функция ПараметрыОкруженияФормыИнтернетПоддержкаИСервисы(Форма) Экспорт
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ИнтернетПоддержкаПользователей.СистемаБыстрыхПлатежей.БазоваяФункциональностьСБП") Тогда
		...
		ЕстьЗависимыеПодсистемы = Истина;
	...
	Результат.Вставить("НастройкаИнтеграцияДоступна"        , НастройкаДоступна И ЕстьЗависимыеПодсистемы);
...
Функция ПереопределитьСообщениеПользователю(
	...
	ИначеЕсли КодОшибки = "ОтсутствуетДоступКСервису"
		...
		Возврат НСтр("ru = 'Настройка подключения не активна или находится в процессе регистрации. Статус подключения необходимо уточнить у банка или платежного агрегатора, с которым заключен договор.';
					|en = 'The connection setting is inactive or being registered. Check the connection status with the bank or payment aggregator, with which the contract is concluded.'");
	...
	ИначеЕсли КодОшибки = "ПревышеноКоличествоПопыток"
		...
		Возврат НСтр("ru = 'Превышено количество попыток обращения к сервису платежной системы с не верными данными авторизации.';
					|en = 'You exceeded the number of attempts to access the payment system service with incorrect authorization data.'")
			+ " "
			+ НСтр("ru = 'Проверьте правильность данных авторизации и повторите попытку через 30 минут.';
					|en = 'Check if the authorization data is correct and try again in 30 minutes.'");
	КонецЕсли;
...
Процедура ЗагрузкаСтатусовОперацийПлатежныхСистем() Экспорт
	...
	ШаблонСообщения = НСтр("ru = 'Загружены терминальные статусы %1 операций %2.';
							|en = 'Terminal statuses %1 of the %2 transactions are imported.'");
