...
Процедура ПолучитьИтоговыеДанныеРНПТ(СтруктураПараметров)
	...
	Если СтруктураПараметров.НеподтвержденныеРеализации0 Тогда
		...
		|	       ТОГДА  ВЫРАЗИТЬ(-ВТ_НеподтвержденныеРеализации0.СтоимТовПросл КАК ЧИСЛО(15, 0)) 
		|	   ИНАЧЕ ВЫРАЗИТЬ(ВТ_НеподтвержденныеРеализации0.СтоимТовПросл КАК ЧИСЛО(15, 0))	   
        |   КОНЕЦ,
		|	ВТ_НеподтвержденныеРеализации0.КодОперации
		|ИЗ
		|	ВТ_НеподтвержденныеРеализации0 КАК ВТ_НеподтвержденныеРеализации0
		|ГДЕ
		|	ВТ_НеподтвержденныеРеализации0.КолТовПросл <> 0 ИЛИ ВЫРАЗИТЬ(ВТ_НеподтвержденныеРеализации0.СтоимТовПросл КАК ЧИСЛО(15, 0)) <> 0";
	КонецЕсли;
	...
	Если СтруктураПараметров.ЕстьОтгрузки Тогда
		...
		|			ТОГДА  ВЫРАЗИТЬ(-ВТ_Отгрузки.СтоимТовПросл КАК ЧИСЛО(15, 0)) 
		|		ИНАЧЕ ВЫРАЗИТЬ(ВТ_Отгрузки.СтоимТовПросл КАК ЧИСЛО(15, 0))	   
		|	КОНЕЦ,
		|	ВТ_Отгрузки.КодОперации
		|ИЗ
		|	ВТ_Отгрузки КАК ВТ_Отгрузки
		|ГДЕ
		|	ВТ_Отгрузки.КолТовПросл <> 0 ИЛИ ВЫРАЗИТЬ(ВТ_Отгрузки.СтоимТовПросл КАК ЧИСЛО(15, 0)) <> 0";
	КонецЕсли;
	...
	Если СтруктураПараметров.ЕстьЗаписиКнигиПокупок Тогда
		...
		|	ВЫРАЗИТЬ(ВТ_ЗаписиКнигиПокупок.СтоимТовПросл КАК ЧИСЛО(15, 0)),
		|	NULL
		|ИЗ
