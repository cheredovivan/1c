...
Функция ТекстСвязиУзловРезервовПодОбесценениеЗапасов()
	...
	Возврат "
		...
		|				ИЛИ &РегламентноеЗадание)
		|		
		|		ЛЕВОЕ СОЕДИНЕНИЕ ВТВзаимосвязанныеОрганизации КАК ВзаимосвязанныеОрганизации
		|		ПО ВзаимосвязанныеОрганизации.Организация1 = Себестоимость.Организация
		|			И ВзаимосвязанныеОрганизации.Организация2 = Себестоимость.КорОрганизация
		|		
		|		ЛЕВОЕ СОЕДИНЕНИЕ ВтУзлыКорректировки КАК УзлыКорректировкиПриемникПередачи
		|		ПО
		|			УзлыКорректировкиПриемникПередачи.АналитикаУчетаНоменклатуры  = Себестоимость.КорАналитикаУчетаНоменклатуры
		...
		|			И Себестоимость.КорОрганизация <> Себестоимость.Организация
		|			И Себестоимость.КорОрганизация <> ЗНАЧЕНИЕ(Справочник.Организации.ПустаяСсылка)
		// Передача резерва под обесценение может происходить только между взаимосвязанными организациями.
		|			И ВзаимосвязанныеОрганизации.Организация2 ЕСТЬ НЕ NULL
		|		
...
Функция ТекстВтТаблицаСвязейПостатейныеРасходыСРегистратором()
	...
	ТекстЗапросаСписаниеМатериаловНаСтатьи = "
		...
		|			КОГДА ВТУчетныеПолитикиФинУчета.МетодОценкиСтоимостиТоваров = ЗНАЧЕНИЕ(Перечисление.МетодыОценкиСтоимостиТоваров.Среднескользящая)
		|			 И Себестоимость.РазделУчета <> ЗНАЧЕНИЕ(Перечисление.РазделыУчетаСебестоимостиТоваров.ВыпускПродукции)
		|			 И Себестоимость.ХозяйственнаяОперация <> ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.СторноПроизводственныхЗатрат)
		|				ТОГДА ИСТИНА
		|			ИНАЧЕ ЛОЖЬ КОНЕЦ) КАК ИсточникНаСреднескользящей
		|	ИЗ
		|		ВтСебестоимостьТоваровСписания КАК Себестоимость
...
Функция ТекстНачальнаяСтоимостьПостатейныеРасходы()
	Возврат "
		...
		// рассчитывать не требуется, кроме выпуска продукции со списанием на расходы.
		// Выпуск продукции со списанием на расходы отражается по разделу учета "Выпуск продукции".
		|		КОГДА ПеремещенияСписания.ИсточникНаСреднескользящей
		|		 И ЕСТЬNULL(СтоимостьПартийТоваров.РазделУчета, НЕОПРЕДЕЛЕНО) <> ЗНАЧЕНИЕ(Перечисление.РазделыУчетаСебестоимостиТоваров.ВыпускПродукции)
		|			ТОГДА 0
...
Процедура ПодготовкаДанныхДляРешенияСЛУРезервыПредстоящихРасходов(ПараметрыРасчета)
	...
	РасчетСебестоимостиПрикладныеАлгоритмы.ИнициализироватьПодготовкуДанныхДляСледующихЭтапов(
		ПараметрыРасчета,
		"РезервыПоУзлам, ДетальныеРезервыПоУзлам, УзлыСЛУ_РезервыПредстоящихРасходов, УзлыСЛУ_РезервыПредстоящихРасходов_НУ,
...
Функция ТекстЗапросаДляРешенияСЛУРезервыПредстоящихРасходов(ПараметрыРасчета = Неопределено) Экспорт
	...
	ТекстЗапроса = ""
		...
		//++ Локализация
		+ ОбщегоНазначения.РазделительПакетаЗапросов() + ТекстУзлыСЛУ_РезервыПредстоящихРасходов_НУ() // вт УзлыСЛУ_РезервыПредстоящихРасходов_НУ
...
Функция ТекстУзлыСЛУ_РезервыПредстоящихРасходов_НУ() // вт УзлыСЛУ_РезервыПредстоящихРасходов_НУ
	Возврат "
		// В таблице узлов отражается начальная стоимость до расчета.
		// Стоимость налогового учета формируется как сумма пяти составляющих себестоимости регл учета за минусом постоянной и 
		// временной разницы.
		...
		|
		|ПОМЕСТИТЬ УзлыСЛУ_РезервыПредстоящихРасходов_НУ
		|ИЗ
...
Функция ТекстСебестоимостьНалоговыйУчет() Экспорт // вт СебестоимостьНалоговыйУчет
	Возврат "
		...
		|			КОГДА Себестоимость.ТипЗаписи = ЗНАЧЕНИЕ(Перечисление.ТипыЗаписейПартий.ВключениеИсключениеНДСВСтоимости)
		|			 И НЕ Себестоимость.Сторно
		|			ТОГДА
...
Процедура РешитьСЛУПлатформой_РезервыПредстоящихРасходов(ПараметрыРасчета)
	...
	|ВЫБРАТЬ
	|	УзлыКорректировки.НомерУзла,
	|	УзлыКорректировки.РасходыНУ
	|ИЗ
	|	УзлыСЛУ_РезервыПредстоящихРасходов_НУ КАК УзлыКорректировки
	|";
	...
	РасчетСебестоимостиПрикладныеАлгоритмы.СкорректироватьСтоимостьУзловСЛУИзВременныхТаблиц(
		ПараметрыРасчета,
		"УзлыСЛУ_РезервыПредстоящихРасходов_НУ",
		"НомерУзла",
...
Функция ТекстВтТаблицаРешений_НалоговыйУчет(ПараметрыРасчета)
	...
	|		- ЕСТЬNULL(ТаблицаРешенийНУ.СуммаНУ, 0)
	...
	|		ПО СтоимостьДопРасходов.НомерУзла = УзлыКорректировки.НомерУзла
	|
	|	ЛЕВОЕ СОЕДИНЕНИЕ ТаблицаРешений_ПостояннаяРазница КАК ТаблицаРешенийПостояннаяРазница
	|		ПО ТаблицаРешенийПостояннаяРазница.НомерУзла = УзлыКорректировки.НомерУзла
	|;
