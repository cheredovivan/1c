...
Функция ТекстСебестоимостьОтгруженногоТовара()
	...
	|	ВЫБОР
	|		КОГДА Операция.ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ПереходПраваСобственности)
	|			ТОГДА ЕСТЬNULL(ТаблицаНалогообложенияПрибылиПриПереходеПраваСобственности.ВариантНалогообложенияПрибыли, НЕОПРЕДЕЛЕНО)
	|		ИНАЧЕ
	...
	|
	|	ЛЕВОЕ СОЕДИНЕНИЕ 
	|		РаздельныйУчет_НастройкиНалогообложенияПрибыли КАК ТаблицаНалогообложенияПрибылиПриПереходеПраваСобственности
	|	ПО 
	|		ТаблицаНалогообложенияПрибылиПриПереходеПраваСобственности.Период = Операция.Дата
	|		И ТаблицаНалогообложенияПрибылиПриПереходеПраваСобственности.Организация = Операция.Организация
	|		И ТаблицаНалогообложенияПрибылиПриПереходеПраваСобственности.Подразделение = Операция.Подразделение
	|		И ТаблицаНалогообложенияПрибылиПриПереходеПраваСобственности.ОбъектУчета = Строки.Номенклатура
	|		И ТаблицаНалогообложенияПрибылиПриПереходеПраваСобственности.НаправлениеДеятельности = Операция.НаправлениеДеятельности
	|		И Операция.ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ПереходПраваСобственности)
	|ГДЕ
...
Функция ТекстНДССРеализацииПриПереходеПраваСобственности()
	...
	|		РаздельныйУчет_НастройкиНалогообложенияПрибыли КАК ТаблицаНалогообложенияПрибыли
	|	ПО 
	|		ТаблицаНалогообложенияПрибыли.Период = ВЫБОР КОГДА Операция.ХозяйственнаяОперация В
	|			(ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.РеализацияБезПереходаПраваСобственности),
	|			 ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.РеализацияЧерезКомиссионераБезПереходаПраваСобственности))
	|			ТОГДА Операция.ДатаПереходаПраваСобственности ИНАЧЕ Операция.Дата КОНЕЦ
	|		И ТаблицаНалогообложенияПрибыли.Организация = Операция.Организация
	|		И ТаблицаНалогообложенияПрибыли.Подразделение = Операция.Подразделение
	|		И ТаблицаНалогообложенияПрибыли.ОбъектУчета = Строки.АналитикаУчетаНоменклатуры.Номенклатура
	|		И ТаблицаНалогообложенияПрибыли.НаправлениеДеятельности = Операция.НаправлениеДеятельности
