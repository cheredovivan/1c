...
// Возвращаемое значение:
// 	Булево - Необходимость обновления ключа сессии.
Функция ТребуетсяОбновлениеКлючаСессииРозница(ПараметрыЗапроса, НомерФН = "", Знач СрокДействия = Неопределено) Экспорт
	
	Если ИнтеграцияТСПИоТ.ПоНомеруФискальногоНакопителяНастроеноПодключениеТСПИоТ(НомерФН) Тогда
		Возврат Ложь;
	КонецЕсли;
	...
	Если РезультатОтправкиЗапроса.ОтветПолучен Тогда
		...
		Если РезультатОтправкиЗапроса.КодСостояния = 200 Тогда
			...
			Иначе
				...
				ТипТокенаАвторизации = ИнтерфейсИСМПОбщегоНазначенияКлиентСервер.ТипТокенаАвторизацииПоИмениДанныхКлючаСессии(ПараметрыЗапроса.ИмяПараметраСеанса);
				...
				ПараметрыАвторизации.UnitedToken   = ТипТокенаАвторизации = Перечисления.ТипыТокеновАвторизации.ИСМП
					И ОбщегоНазначенияИСМПКлиентСерверПовтИсп.ИспользоватьТокенФорматаUUID();
...
Процедура УстановитьКлючСессии(ПараметрыЗапроса, ПараметрыКлючаСессии, ЗаписатьВРегистр = Истина) Экспорт
	...
	Иначе
		...
		Иначе
			ТекущиеДанныеКлючаСессии = ДанныеКлючаСессии[ПараметрыЗапроса.Организация];
			Если ТекущиеДанныеКлючаСессии <> Неопределено
				И ТекущиеДанныеКлючаСессии.КлючСессии = ПараметрыКлючаСессии.КлючСессии Тогда
				Если ЗаписатьВРегистр Тогда
