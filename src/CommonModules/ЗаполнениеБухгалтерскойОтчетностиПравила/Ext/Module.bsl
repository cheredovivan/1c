...
Процедура ЗаполнитьНастройкиПоУмолчаниюОФР(Процессор)
	...
	Если Процессор.Упрощенная Тогда
	...
	Иначе
		...
		ДобавитьПоказатель(Процессор, НСтр("ru = 'Проценты к получению';
											|en = 'Проценты к получению'", КодЯзыка)); // Только доходы
		ДополнитьОтбор(Процессор, Перечисления.ВидыПрочихДоходовИРасходов.ПроцентыКПолучению);
		ДополнитьОтбор(Процессор, Перечисления.ВидыПрочихДоходовИРасходов.ПроцентыПоГосударственнымЦеннымБумагам);
		ДополнитьОтбор(Процессор, Перечисления.ВидыПрочихДоходовИРасходов.ПроцентыПоГосударственнымЦеннымБумагамПоСтавке0);
		...
		ДобавитьПоказательРасходы(Процессор, НСтр("ru = 'Проценты к уплате';
													|en = 'Проценты к уплате'", КодЯзыка)); // Только расходы
		ДополнитьОтбор(Процессор, Перечисления.ВидыПрочихДоходовИРасходов.ПроцентыКУплате);
		ДополнитьОтбор(Процессор, Перечисления.ВидыПрочихДоходовИРасходов.ПроцентыНачисленныеПоСт269);
...
Процедура ЗаполнитьНастройкиПоУмолчаниюОДДС(Процессор)
	...
	ДобавитьПоказатель(Процессор, НСтр("ru = 'Поступления от продаж по платежным картам';
										|en = 'Receipts from sales by payment cards'", КодЯзыка));
	ДополнитьОтбор(Процессор, ПланыСчетов.Хозрасчетный.ПродажиПоПлатежнымКартам);
...
Процедура ОписатьПорядокЗаполненияПоказателяДенежныеПотоки(Описание, Показатель, ДанныеАлгоритма, КодЯзыка)
	...
	ИначеЕсли Показатель.Заполнение.Отбор.Свойство("РаспределениеПриемник") Тогда
		Если Показатель.Заполнение.Отбор.РаспределениеПриемник.Ключ = "ЕНС" Тогда
			...
			ШаблонТекста = НСтр(
				...
				|en = 'Information on the amounts credited to the unified tax account and allocated by the tax authority to the following payment kinds: %1'",
				КодЯзыка);
			Если Показатель.Заполнение.Отбор.РаспределениеПриемник.Значение.Количество() = 1 Тогда
				ШаблонТекста = НСтр(
					...
					|en = 'Information on the amounts credited to the unified tax account and allocated by the tax authority to the ""%1"" payment kind'",
					КодЯзыка);
			КонецЕсли;
			Описание.Текст = СтрШаблон(
				ШаблонТекста,
			...
			Описание.Разъяснение = НСтр(
				...
				|en = 'The indicator is included in the report only if the positive balance of the unified tax account is absent or insignificant'",
				КодЯзыка);
		КонецЕсли;
	...
	Иначе
		...
		Если Показатель.Заполнение.Отбор.Свойство("Счет") Тогда
			...
			Предложения.Добавить(СтрШаблон(ШаблонТекста, ПредставлениеСчетовОтбора.Текст));
			Описание.Разъяснение = НСтр(
		...
		Иначе
			...
			ШаблонТекста = НСтр("ru = 'Оборот по счетам денежных средств и денежных эквивалентов %1.
	                             ...
	                             |en = 'Turnover on cash and cash equivalent accounts %1.
	                             |The list of accounts is given below.'", КодЯзыка);
			ОписаниеСтатей = "";
		...
		Иначе
			...
			Союз = НСтр("ru = ', а также ';
						|en = ', as well as'");// используем нестандартный союз, так как описания сворачиваемых потоков уже содержат союз "и"
