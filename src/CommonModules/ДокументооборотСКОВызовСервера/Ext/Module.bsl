...
Функция ПолнотекстовыйПоискВидовДокументов(СтрокаИлиМассив) Экспорт
	...
	Если НЕ ЗначениеЗаполнено(СтрокаИлиМассив)
		ИЛИ ПолнотекстовыйПоиск.ПолучитьРежимПолнотекстовогоПоиска() <> РежимПолнотекстовогоПоиска.Разрешить Тогда
	...
	Для каждого СтрокаПоиска Из СтрокиПоиска Цикл
		...
		СписокПоиска.ПорогНечеткости = ?(ЭтоПоискПоКоду, 0, 33);
		
		МассивОтбор = Новый Массив;
		МассивОтбор.Добавить(Метаданные.Справочники.ВидыОтправляемыхДокументов);
		ИмяТипаРегистраСведенийВидыДокументовГосОргановДляПоиска = "ВидыДокументовГосОргановДляПоиска";
		Если Метаданные.РегистрыСведений.Найти(ИмяТипаРегистраСведенийВидыДокументовГосОргановДляПоиска) <>
			Неопределено Тогда
			МассивОтбор.Добавить(Метаданные.РегистрыСведений[ИмяТипаРегистраСведенийВидыДокументовГосОргановДляПоиска]);
		КонецЕсли;
		...
		Пока Истина Цикл
			Для каждого СтрокаРезультата Из СписокПоиска Цикл
				...
				Если ТипЗнч(ЗначениеСтрокиРезультата) = Тип("СправочникСсылка.ВидыОтправляемыхДокументов") Тогда
				...
				ИначеЕсли ЗначениеСтрокиРезультата <> Неопределено Тогда
					...
					Если ЗначениеЗаполнено(ЗначениеСтрокиРезультата)
						...
						Если ЗначениеЗаполнено(ВидОтправляемогоДокумента) Тогда
							Результат.ВидыОтправляемыхДокументов.Добавить(ВидОтправляемогоДокумента);
						Иначе
