...
Процедура ПодписатьЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Результат <> Неопределено
		...
		Для Каждого ТекущийЭлементНабораДанных Из Результат.НаборДанных Цикл
			...
			Если Не ПроверитьДоступностьДляАвторизации(
				Сообщение, Результат.ВыбранныйСертификат.Ссылка, ДанныеДляПроверкиСертификата) Тогда
				Продолжить;
			КонецЕсли;
...
Процедура ДобавитьДанныеДляАвторизацииПоИНН(Сообщения, ПараметрыЗапроса, ПараметрыАвторизации) Экспорт
	
	Если ПараметрыЗапроса.РазрешенаПодписьДоверенногоЛицаСМЧД
		И ПараметрыАвторизации.UnitedToken
		И ЗначениеЗаполнено(ПараметрыАвторизации.ИНН) Тогда
		
		Описание = СтрШаблон(
			НСтр("ru = 'Авторизация по ИНН в %1 для %2';
				|en = 'Авторизация по ИНН в %1 для %2'"),
			ПараметрыЗапроса.ПредставлениеСервиса,
			ПараметрыЗапроса.Организация);
		
		ПараметрыАвторизацииПоИНН = ОбщегоНазначенияКлиент.СкопироватьРекурсивно(ПараметрыАвторизации);
		ПараметрыАвторизацииПоИНН.Данные = ПараметрыАвторизацииПоИНН.ИНН;
		
		Сообщения.Добавить(
			РезультатПодписания(
				ПараметрыЗапроса,
				Описание,
				ПараметрыАвторизацииПоИНН));
		
	КонецЕсли;
	
КонецПроцедуры

Функция ПроверитьДоступностьДляАвторизации(Сообщение, Сертификат, ДанныеДляПроверкиСертификата) Экспорт
	
	Если Сообщение.Свойство("ПараметрыАвторизации")
		И Сообщение.ПараметрыЗапроса.РазрешенаПодписьДоверенногоЛицаСМЧД
		И Сообщение.ПараметрыАвторизации.UnitedToken Тогда
		СертификатМЧД = ДанныеДляПроверкиСертификата[Сообщение.ПараметрыЗапроса.Организация];
		Если СертификатМЧД = Неопределено Тогда
			Если Сообщение.ПараметрыЗапроса.Свойство("ИНН") Тогда
				ИНН = Сообщение.ПараметрыЗапроса.ИНН;
			Иначе
				ИНН = "";
			КонецЕсли;
			СертификатМЧД = ОбщегоНазначенияИСВызовСервера.ТребуетсяДоверенностьДляИспользованияСертификата(
				Сертификат, Сообщение.ПараметрыЗапроса.Организация, ИНН);
			ДанныеДляПроверкиСертификата.Вставить(Сообщение.ПараметрыЗапроса.Организация, СертификатМЧД);
		КонецЕсли;
		Если СертификатМЧД
				И Сообщение.ПараметрыАвторизации.Данные <> Сообщение.ПараметрыАвторизации.ИНН
			Или Не СертификатМЧД
				И Сообщение.ПараметрыАвторизации.Данные = Сообщение.ПараметрыАвторизации.ИНН Тогда
			Возврат Ложь;
		КонецЕсли;
	КонецЕсли;
