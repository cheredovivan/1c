...
Функция УстановитьВариантДействияВыделеннымСтрокам(Форма, РезультатВыбораПользователя) Экспорт
	...
	Для Каждого ВыделеннаяСтрока Из Форма.Элементы.Товары.ВыделенныеСтроки Цикл
		...
		Иначе
			...
			ВернутьПерепоставленноеБезОформления = ТекущаяСтрока.Действие = ПредопределенноеЗначение("Перечисление.ВариантыДействийПоРасхождениямВАктеПослеПриемки.ВернутьПерепоставленноеБезОформления");
			...
			Если РасхожденияКлиентСервер.ИзменитьДействиеВСтроке(ТекущаяСтрока, РезультатВыбораПользователя) Тогда
				...
				Если ВернутьПерепоставленноеБезОформления
					Или ВариантДействия = ПредопределенноеЗначение("Перечисление.ВариантыДействийПоРасхождениямВАктеПослеПриемки.ВернутьПерепоставленноеБезОформления") Тогда
					СтруктураВозврата.СтрокиДляПересчетаСерий.Добавить(ВыделеннаяСтрока);
				КонецЕсли;
			КонецЕсли;
...
Процедура ТоварыДействиеНачалоВыбора(ТекущиеДанные, Объект, Форма, ТипАкта, ПоказыватьПояснение = Ложь) Экспорт
	...
	Если Не ОбщегоНазначенияУТКлиентСервер.АвторизованВнешнийПользователь() Тогда
		...
		Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(ТекущиеДанные, "ЗаказКлиента") Тогда
			
			ЕстьРаспоряжениеОснованияОтгрузкаТоваровКлиенту =
				ТипЗнч(ТекущиеДанные.ЗаказКлиента) = Тип("ДокументСсылка.ОтгрузкаТоваровКлиенту")
				Или ТипАкта = ПредопределенноеЗначение("Перечисление.ТипыОснованияАктаОРасхождении.ОтгрузкаТоваровКлиенту")
					И Не ЗначениеЗаполнено(ТекущиеДанные.ЗаказКлиента);
			
		Иначе
			ЕстьРаспоряжениеОснованияОтгрузкаТоваровКлиенту = Ложь;
		КонецЕсли;
		...
		ПараметрыФормы.Вставить("ЕстьРаспоряжениеОснованияОтгрузкаТоваровКлиенту",
			ЕстьРаспоряжениеОснованияОтгрузкаТоваровКлиенту);
...
Процедура ИзменитьДействиеВыделенныхСтрок(Объект, Форма, ТипАкта, ПоказыватьПояснение = Ложь) Экспорт
	...
	Для Каждого ВыделеннаяСтрока Из Форма.Элементы.Товары.ВыделенныеСтроки Цикл
		...
		Если ДанныеСтроки.Свойство("ЗаказКлиента") Тогда
			
			Если ТипЗнч(ДанныеСтроки.ЗаказКлиента) = Тип("ДокументСсылка.ОтгрузкаТоваровКлиенту")
				Или ТипАкта = ПредопределенноеЗначение("Перечисление.ТипыОснованияАктаОРасхождении.ОтгрузкаТоваровКлиенту")
					И Не ЗначениеЗаполнено(ДанныеСтроки.ЗаказКлиента) Тогда
				ЕстьРаспоряжениеОснованияОтгрузкаТоваровКлиенту = Истина;
			КонецЕсли;
...
Процедура ОбработкаКомандыОтчетаОформлениеДокументов(Форма, ОписаниеКоманды, АктОРасхождениях, ОснованиеАкта, ОформляемыйДокумент) Экспорт
	...
	ИначеЕсли ОписаниеКоманды = "Оформить" Тогда
		...
		ИначеЕсли ТипЗнч(ОформляемыйДокумент) = Тип("ДокументСсылка.ПоступлениеТоваровОтКлиента") Тогда
			
			ПараметрыФормы.Вставить("АктОРасхождениях", АктОРасхождениях);
			ПараметрыФормы.Вставить("ОснованиеАкта", ОснованиеАкта);
			ОткрытьФорму("Документ.ПоступлениеТоваровОтКлиента.Форма.ФормаДокумента",
		...
		ИначеЕсли ТипЗнч(ОформляемыйДокумент) = Тип("Структура")
			...
			Если ОформляемыйДокумент.Свойство("ЭтоВозвратВПуть") Тогда
				ЭтоВозвратВПуть = ОформляемыйДокумент.ЭтоВозвратВПуть;
			КонецЕсли;
