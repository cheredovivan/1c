...
Процедура ЗаполнитьКодыКлассификатора2026годаВТрудовыхФункциях(ПараметрыОбновления) Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		Если Лев(Выборка.ОКПДТРКод, 1) = "1" Тогда
			СтрокаКлассификатора = КлассификаторОКПДТРПрофессии.Найти(Выборка.ОКПДТРКод + Выборка.ОКПДТРКЧ, "Code");
		Иначе
			СтрокаКлассификатора = КлассификаторОКПДТРДолжности.Найти(Выборка.ОКПДТРКод + Выборка.ОКПДТРКЧ, "Code");
		КонецЕсли;
