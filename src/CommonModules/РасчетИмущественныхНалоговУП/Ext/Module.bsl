...
Процедура СформироватьТаблицуРегистрацияЗемельныхУчастковСрезПоследних(ПараметрыРасчета, Период, ДополнительныеПараметры, ИмяТаблицы = "", УсловияЗапроса = "") Экспорт
	...
	Если ВременнаяТаблицаЗаписейСУчетомИсправлений = Неопределено Тогда
		 ВызватьИсключение НСтр("ru = 'Для формирования среза последних по записям регистра параметров начисления земельного налога
		 							...
		 							|en = 'To generate the slice of the last records of the register of land tax accrual parameters,
	...
	Пока Выборка.Следующий() Цикл
		...
		Попытка
			...
			Если ЗначениеЗаполнено(Выборка.РегламентныйДокумент) Тогда
				...
				ПривестиСуммуНалогаНаИмуществоКЦеломуЦислу(Выборка.Организация, Выборка.ДатаОтражения, РасчетПоНалогу);
...
Процедура ПривестиСуммуНалогаНаИмуществоКЦеломуЦислу(Организация, ДатаОтражения, РасчетПоНалогу)
	...
	Если РасчетПоНалогу.Свойство("РасчетНалогаНаИмуществоПоСреднегодовойСтоимости") Тогда
		...
		ВыборкаПогрешностей.Следующий();
		
		// Распределим погрешность округления до целого между строками перерасчета пропорционально сумме налога.
		КоэффициентыРаспределения = РасчетПоСреднегодовойСтоимости.ВыгрузитьКолонку("СуммаНалога");
		...
		Индекс = 0;
		Для Каждого СтрокаПерерасчета Из РасчетПоСреднегодовойСтоимости Цикл
