...
Функция ТекстСторноВыручкиОтРеализацииСобственногоКомиссионногоТовара()
	...
	|	0 КАК ВалютнаяСуммаКт,
	|	-Строки.Количество КАК КоличествоКт,
	|	ЕСТЬNULL(СуммыДокументовВВалютахУчета.СуммаБезНДСРегл + Строки.СуммаАкциза, -(Строки.СуммаСНДС - Строки.СуммаНДС + Строки.СуммаАкциза)) КАК СуммаНУКт,
	|	0 КАК СуммаПРКт,
	|	0 КАК СуммаВРКт,
...
Функция ТекстСторноВыручкиОтРеализацииСобственногоКомиссионногоТовараПрошлыеПериоды()
	...
	|	0 КАК ВалютнаяСуммаКт,
	|	-Строки.Количество КАК КоличествоКт,
	|	ЕСТЬNULL(СуммыДокументовВВалютахУчета.СуммаБезНДСРегл, -(Строки.СуммаСНДС - Строки.СуммаНДС)) - Строки.СуммаАкциза КАК СуммаНУКт,
	|	0 КАК СуммаПРКт,
	|	0 КАК СуммаВРКт,
...
Функция ТекстКорректировкаНачисленияАкцизовПрошлыхПериодовНаДоходы()
	...
	|		И ТаблицаНалогообложенияПрибыли.Организация = СтрокиКт.АналитикаУчетаПоПартнерам.Организация
	|		И ТаблицаНалогообложенияПрибыли.Подразделение = СтрокиКт.Подразделение
	|		И ТаблицаНалогообложенияПрибыли.ОбъектУчета = СтрокиКт.АналитикаУчетаНоменклатуры.Номенклатура
	|		И ТаблицаНалогообложенияПрибыли.НаправлениеДеятельности = СтрокиКт.НаправлениеДеятельностиНоменклатуры
	|
	|	ЛЕВОЕ СОЕДИНЕНИЕ
	|		РегистрацииВНалоговомОргане КАК РегистрацииПоПодразделению
	|	ПО
