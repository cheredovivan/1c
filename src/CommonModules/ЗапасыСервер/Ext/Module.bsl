...
//		* ПериодНачисленияАкциза - Дата - используется для определения подакцизности товара и действующей ставки акциза.
...
Процедура ЗаполнитьВидыЗапасовПоУмолчанию(МенеджерВременныхТаблиц,
	...
	|	ТаблицаТоваров.ПодразделениеПроизводства КАК ПодразделениеПроизводства,
	//-- НЕ УТ
	...
	//++ НЕ УТ
	|	ВидыЗапасовРезерва.ОблагаетсяАкцизом КАК ОблагаетсяАкцизом,
	|	ВидыЗапасовРезерва.ПодразделениеПроизводства КАК ПодразделениеПроизводства,
	//-- НЕ УТ
	...
	//++ НЕ УТ
	|			И ТаблицаТоваров.ОблагаетсяАкцизом = ВидыЗапасовРезерва.ОблагаетсяАкцизом
	|			И ТаблицаТоваров.ПодразделениеПроизводства = ВидыЗапасовРезерва.ПодразделениеПроизводства
	//-- НЕ УТ
	|			И (ВЫБОР
	...
	Пока Выборка.Следующий() Цикл
		...
		Если Выборка.Организация <> КлючСтроки.Организация
			...
			//++ НЕ УТ
			Или Выборка.ОблагаетсяАкцизом <> КлючСтроки.ОблагаетсяАкцизом
			Или Выборка.ПодразделениеПроизводства <> КлючСтроки.ПодразделениеПроизводства
			//-- НЕ УТ
...
Процедура СформироватьВТДоступныеВидыЗапасов(ДокументОбъект,
	...
	//++ НЕ УТ
	|	И (&ПоВсемНалогообложениямАкцизов
	|			ИЛИ ВидыЗапасов.ОблагаетсяАкцизом В (&ОблагаетсяАкцизом))
	|	И (&ПоВсемПодразделениямПроизводства
	|			ИЛИ (НЕ ВидыЗапасов.ОблагаетсяАкцизом
	|				ИЛИ (ВидыЗапасов.ОблагаетсяАкцизом
	|					И ВидыЗапасов.ПодразделениеПроизводства В (&ПодразделениеПроизводства)
	|					)
	|				)
	|		)
	//-- НЕ УТ
	...
	//++ НЕ УТ
	|	И (НЕ ВидыЗапасов.ОблагаетсяАкцизом
	|			ИЛИ Настройка.СпособПередачиТоваров = ЗНАЧЕНИЕ(Перечисление.СпособыПередачиТоваров.Продажа))
	//-- НЕ УТ
	|	И (НЕ ВидыЗапасов.Устаревший
	|				И НЕ ВидыЗапасов.ЭтоДубль
	...
	//++ НЕ УТ
	Запрос.УстановитьПараметр("ОблагаетсяАкцизом", ОтборыВидовЗапасов.ОблагаетсяАкцизом);
	Запрос.УстановитьПараметр("ПоВсемНалогообложениямАкцизов", Не ЗначениеЗаполнено(ОтборыВидовЗапасов.ОблагаетсяАкцизом));
	
	Запрос.УстановитьПараметр("ПодразделениеПроизводства", ОтборыВидовЗапасов.ПодразделениеПроизводства);
	Запрос.УстановитьПараметр("ПоВсемПодразделениямПроизводства", Не ЗначениеЗаполнено(ОтборыВидовЗапасов.ПодразделениеПроизводства));
	//-- НЕ УТ
...
Процедура ДобавитьЭтап_ИсправлениеДублейВидовЗапасов(ТаблицаЭтапов,ТекущийРодитель) Экспорт
	...
	НоваяСтрока.МультиязычноеНаименование = "ru = 'Исправление дублей видов запасов';
											|en = 'Correct duplicates of inventory owner attributes'"; // @НСтр-1
...
Процедура ДобавитьЭтап_ИсправлениеОтрицательныхОстатковТоваровОрганизаций(ТаблицаЭтапов,ТекущийРодитель) Экспорт
	...
	НоваяСтрока.МультиязычноеНаименование = "ru = 'Исправление отрицательных остатков товаров организаций';
											|en = 'Correct negative stock balance of company goods'"; // @НСтр-1
	НоваяСтрока.ВыполняетсяВручную = Истина;
...
Процедура ДобавитьЭтап_ОформлениеПередачТоваров(ТаблицаЭтапов,ТекущийРодитель) Экспорт
	...
	НоваяСтрока.МультиязычноеНаименование = "ru = 'Оформление передач товаров';
											|en = 'Register goods transfers'"; // @НСтр-1
	НоваяСтрока.ВыполняетсяВручную = Истина;
...
Процедура ДобавитьЭтап_ОформлениеОстатковТоваровКПередаче(ТаблицаЭтапов,ТекущийРодитель) Экспорт
	...
	НоваяСтрока.МультиязычноеНаименование =
		"ru = 'Оформление передач и возвратов товаров (по данным, введенным до версии 2.4)';
		|en = 'Register goods transfers and returns (according to the data entered before version 2.4)'"; // @НСтр-1
	НоваяСтрока.ВыполняетсяВручную = Истина;
	НоваяСтрока.ТекстВыполнить = НСтр("ru = 'Оформить';
										|en = 'Register'");
	НоваяСтрока.ДействиеИспользование = ЗакрытиеМесяцаСервер.ОписаниеДействия_СервернаяПроцедура(
...
Процедура ДобавитьЭтап_ОформлениеВыкупаТоваров(ТаблицаЭтапов,ТекущийРодитель) Экспорт
	...
	НоваяСтрока.МультиязычноеНаименование = "ru = 'Оформление выкупа товаров';
											|en = 'Register goods redemption'"; // @НСтр-1
	НоваяСтрока.ВыполняетсяВручную = Истина;
	НоваяСтрока.ТекстВыполнить = НСтр("ru = 'Оформить';
										|en = 'Register'");
	НоваяСтрока.ДействиеИспользование = ЗакрытиеМесяцаСервер.ОписаниеДействия_СервернаяПроцедура(
...
Процедура ДобавитьЭтап_ОформлениеОтчетовПоКомиссииМеждуОрганизациями(ТаблицаЭтапов,ТекущийРодитель) Экспорт
	...
	НоваяСтрока.МультиязычноеНаименование = "ru = 'Оформление отчетов по комиссии между организациями';
											|en = 'Register ""Consignment notifications — Intercompany"" '"; // @НСтр-1
	НоваяСтрока.ВыполняетсяВручную = Истина;
	НоваяСтрока.ТекстВыполнить     = НСтр("ru = 'Оформить';
											|en = 'Register'");
	НоваяСтрока.ДействиеИспользование = ЗакрытиеМесяцаСервер.ОписаниеДействия_СервернаяПроцедура(
...
Процедура ДобавитьЭтап_ОформлениеОтчетовКомитентамОСписании(ТаблицаЭтапов,ТекущийРодитель) Экспорт
	...
	НоваяСтрока.МультиязычноеНаименование = "ru = 'Оформление отчетов комитентам о списании';
											|en = 'Register write-off reports to consignors'"; // @НСтр-1
	НоваяСтрока.ВыполняетсяВручную = Истина;
	НоваяСтрока.ТекстВыполнить     = НСтр("ru = 'Оформить';
											|en = 'Register'");
	НоваяСтрока.ДействиеИспользование = ЗакрытиеМесяцаСервер.ОписаниеДействия_СервернаяПроцедура(
...
Процедура ЗаполнитьВидыЗапасовПоТаблицеОстатковСформироватьВТНовыеРезервы(ДокументОбъект, МенеджерВременныхТаблиц, Отказ, ПараметрыЗаполненияВидовЗапасов)
	...
	Пока ВыборкаОстаткиПоСтрокам.Следующий() Цикл
		...
		Пока Выборка.Следующий() Цикл
			...
			//++ НЕ УТ
			СуммаАкцизаОстаток				= Выборка.СуммаАкцизаОстаток;
			//-- НЕ УТ
			...
			ИначеЕсли (КоличествоТовара > 0
				...
				СтруктураДанныхСтроки.СуммаНДСВознаграждения	= ?(КоличествоТовара <> 0,
																	...
																	0);
				//++ НЕ УТ
				СтруктураДанныхСтроки.СуммаАкциза				= ?(КоличествоОстаток <> 0,
																	?(КоличествоОстаток - Количество = 0,
																		СуммаАкцизаОстаток,
																		Окр(Количество * СуммаАкцизаОстаток
																		/ КоличествоОстаток, 2, 1)),
																	0);
				//-- НЕ УТ
		...
		Если КоличествоТовара <> 0
			...
			Если (ПриНехваткеТоваровОрганизацииЗаполнятьВидамиЗапасовПоУмолчанию
				...
				Если ВыборкаТаблицаТоваров.НайтиСледующий(ОтборСтрок) Тогда
					...
					//++ НЕ УТ
					СтруктураВидаЗапасовПоУмолчанию.ОблагаетсяАкцизом			= ВыборкаТаблицаТоваров.ОблагаетсяАкцизом;
					СтруктураВидаЗапасовПоУмолчанию.ПодразделениеПроизводства	= ВыборкаДанныеДокумента.ПодразделениеПроизводства;
					//-- НЕ УТ
...
Процедура ЗаполнитьВидыЗапасовПоТаблицеОстатков(ДокументОбъект,
	...
	//++ НЕ УТ
	СтруктураВидаЗапасовПоУмолчанию.ОблагаетсяАкцизом			= Ложь;
	СтруктураВидаЗапасовПоУмолчанию.ПодразделениеПроизводства	= ВыборкаДанныеДокумента.ПодразделениеПроизводства;
	//-- НЕ УТ
		...
		Пока Выборка.Следующий() Цикл
			...
			ИначеЕсли (КоличествоТовара > 0
				...
				СтруктураДанныхСтроки.СуммаНДСВознаграждения	= ?(КоличествоТовара <> 0,
																	...
																	0));
				//++ НЕ УТ
				СтруктураДанныхСтроки.СуммаАкциза				= ?(КоличествоОстатокВсего <> 0,
																	?(КоличествоОстатокВсего - Количество = 0,
																		СуммаАкцизаОстаток,
																		Окр(Количество * СуммаАкцизаОстаток / КоличествоОстатокВсего, 2, 1)),
																	0);
				//-- НЕ УТ
		...
		Если КоличествоТовара <> 0
			...
			Если (ПриНехваткеТоваровОрганизацииЗаполнятьВидамиЗапасовПоУмолчанию
				...
				Если ВыборкаТаблицаТоваров.НайтиСледующий(ОтборСтрок) Тогда
					...
					//++ НЕ УТ
					СтруктураДанныхСтроки.СуммаАкциза = 0;
					//-- НЕ УТ
...
Функция ЗапросРаспределенияБезУчетаПриоритетовТаблицыТоваровНаТаблицуОстатков(МенеджерВременныхТаблиц,
	...
	//++ НЕ УТ
	|	ТаблицаДанныхДокумента.ОблагаетсяАкцизом КАК ОблагаетсяАкцизом,
	//-- НЕ УТ
	...
	//++ НЕ УТ
	|	&ТекстПоляТаблицаТоваровСуммаАкциза_ КАК СуммаАкциза,
	//-- НЕ УТ
	...
	//++ НЕ УТ
	|	ТаблицаДанныхДокумента.ОблагаетсяАкцизом,
	//-- НЕ УТ
	...
	//++ НЕ УТ
	|	ТаблицаДанныхДокумента.ОблагаетсяАкцизом КАК ОблагаетсяАкцизом,
	|	ТаблицаДанныхДокумента.ПодразделениеПроизводства КАК ПодразделениеПроизводства,
	//-- НЕ УТ
	...
	//++ НЕ УТ
	|	ВЫБОР
	|		КОГДА ТаблицаДокумента.ОблагаетсяАкцизом
	|			ТОГДА ТаблицаДокумента.ПодакцизныйТовар
	|		ИНАЧЕ ЛОЖЬ
	...
	//++ НЕ УТ
	|	&ТекстПоляТаблицаВидыЗапасовСуммаАкциза_ КАК СуммаАкциза,
	//-- НЕ УТ
	|	ВЫБОР
	...
	//++ НЕ УТ
	|	МАКСИМУМ(ВтОстаткиБезГруппировки.СуммаАкцизаОстаток) КАК СуммаАкцизаОстаток,
	//-- НЕ УТ
	|
	...
	//++ НЕ УТ
	|		ВидыЗапасов.СуммаАкциза КАК СуммаАкцизаОстаток,
	//-- НЕ УТ
	...
	//++ НЕ УТ
	|		&ТекстПоляТаблицаОстатковСуммаАкцизаОстаток_ КАК СуммаАкцизаОстаток,
	//-- НЕ УТ
	|		ВЫБОР
	...
	//++ НЕ УТ
	|	ЕСТЬNULL(Остатки.СуммаАкцизаОстаток, 0) КАК СуммаАкцизаОстаток,
	//-- НЕ УТ
	...
	ОбщегоНазначенияУТ.ЗаменитьОтсутствующиеПоляВтВТекстеЗапроса(МенеджерВременныхТаблиц,
																Запрос.Текст,
																"&ТекстПоляТаблицаТоваровСуммаАкциза_",
																"ТаблицаТоваров",
																"СуммаАкциза",
																"СУММА(ТаблицаТоваров.СуммаАкциза)",
																"0");
	//-- НЕ УТ
	...
	ОбщегоНазначенияУТ.ЗаменитьОтсутствующиеПоляВтВТекстеЗапроса(МенеджерВременныхТаблиц,
																Запрос.Текст,
																"&ТекстПоляТаблицаВидыЗапасовСуммаАкциза_",
																"ТаблицаВидыЗапасов",
																"СуммаАкциза",
																"СУММА(ТаблицаВидыЗапасов.СуммаАкциза)",
																"0");
	//-- НЕ УТ
	...
	ОбщегоНазначенияУТ.ЗаменитьОтсутствующиеПоляВтВТекстеЗапроса(МенеджерВременныхТаблиц,
																Запрос.Текст,
																"&ТекстПоляТаблицаОстатковСуммаАкцизаОстаток_",
																"ТаблицаОстатков",
																"СуммаАкцизаОстаток",
																"Остатки.СуммаАкцизаОстаток",
																"0");
	//-- НЕ УТ
...
Функция ЗапросРаспределенияСУчетомПриоритетовТаблицыТоваровНаТаблицуОстатков(МенеджерВременныхТаблиц,
	...
	//++ НЕ УТ
	|	ОстаткиСгруппированные.ОблагаетсяАкцизом					КАК ОблагаетсяАкцизом,
	|	МАКСИМУМ(ОстаткиСгруппированные.СуммаАкцизаОстаток)			КАК СуммаАкцизаОстаток,
	//-- НЕ УТ
	...
	|		ВЫБОР
	|			КОГДА ТаблицаВидыЗапасовСводно.Количество <= Остатки.КоличествоОстаток
	|				ТОГДА ТаблицаВидыЗапасовСводно.СуммаАкциза
	|			КОГДА ТаблицаВидыЗапасовСводно.Количество = 0
	|				ТОГДА 0
	|			ИНАЧЕ Остатки.КоличествоОстаток * ТаблицаВидыЗапасовСводно.СуммаАкциза / ТаблицаВидыЗапасовСводно.Количество
	|		КОНЕЦ										КАК СуммаАкцизаОстаток,
	//-- НЕ УТ
	|		ВЫБОР
	|			КОГДА Остатки.КоличествоОстаток <> 0
	|				ТОГДА Остатки.СуммаОстаток
	...
	//++ НЕ УТ
	|		ВидыЗапасов.ОблагаетсяАкцизом                               КАК ОблагаетсяАкцизом,
	|		ТаблицаВидыЗапасовСводно.СуммаАкциза                        КАК СуммаАкцизаОстаток,
	//-- НЕ УТ
	|		0                                                           КАК СуммаОстаток
	|	ИЗ
	...
	//++ НЕ УТ
	|	ОстаткиСгруппированные.ОблагаетсяАкцизом,
	...
	|	НомерСтроки ВОЗР,
	//++ НЕ УТ
	|	&ПриоритетПодбораПодакцизныхЗапасов,
	//-- НЕ УТ
	|	Приоритет ВОЗР,
...
#Область ЗаменаИспользованияНеобязательныхПолей	
	...
	ТекстУсловиеОтбораПодакцизныхТоваров = ?(ПараметрыЗаполнения.ПодбиратьЗапасыРазныхПериодов,
											...
											"ИСТИНА");
	//++ НЕ УТ
	ТекстПриоритетПодбораПодакцизныхЗапасов = ?(ПараметрыЗаполнения.ПриоритетПодбораПодакцизныхЗапасов,
												"ЕСТЬNULL(Остатки.ОблагаетсяАкцизом, ЛОЖЬ) УБЫВ,",
												"");
	
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса,
								"&ПриоритетПодбораПодакцизныхЗапасов,",
								ТекстПриоритетПодбораПодакцизныхЗапасов);
	//-- НЕ УТ
...
Процедура ЗаполнитьСтруктуруПоСтрокеТоваров(Структура, СтрокаТоваров, ДанныеШапки)
	...
	//++ НЕ УТ
	Структура.ОблагаетсяАкцизом			= Ложь;
	Структура.ПодразделениеПроизводства	= Неопределено;
	//-- НЕ УТ
...
Функция КлючКэшаОтбораВидовЗапасов(ВладелецТовараВШапке, ОтборВидовЗапасов)
	...
	ЧастиКлючаКэша.Добавить(КлючКэша);
	//++ НЕ УТ
	КлючКэша = ?(ЗначениеЗаполнено(ОтборВидовЗапасов.ОблагаетсяАкцизом),
				Строка(ОтборВидовЗапасов.ОблагаетсяАкцизом),
				ПустоеЗначение);
	ЧастиКлючаКэша.Добавить(КлючКэша);
	
	КлючКэша = ?(ЗначениеЗаполнено(ОтборВидовЗапасов.ПодразделениеПроизводства),
				Строка(ОтборВидовЗапасов.ПодразделениеПроизводства.УникальныйИдентификатор()),
				ПустоеЗначение);
	ЧастиКлючаКэша.Добавить(КлючКэша);
	//-- НЕ УТ
...
Процедура ТаблицаОстатковРеализованныхТоваров(ДокументОбъект, МенеджерВременныхТаблиц, ПараметрыЗаполненияВидовЗапасов)
	...
	|	КОНЕЦ КАК КоличествоПоРНПТ,
	//++ НЕ УТ
	|	ТЧВидыЗапасов.СуммаАкциза КАК СуммаАкциза,
	//-- НЕ УТ
	|	ВЫБОР
	...
	|		КОГДА НЕ ТЧВидыЗапасов.НаДоходыРасходы
	|				ИЛИ &ЭтоКорректировкаРеализации
	|			ТОГДА ТчВидыЗапасов.СуммаАкциза
	|		ИНАЧЕ 0
	...
	//++ НЕ УТ
	|	СУММА(РеализованныеТовары.СуммаАкциза) КАК СуммаАкцизаОстаток,
	//-- НЕ УТ
	|	0 КАК СуммаОстаток,
	...
	ОбщегоНазначенияУТ.ЗаменитьОтсутствующиеПоляВтВТекстеЗапроса(МенеджерВременныхТаблиц,
																...
																"ТаблицаВидыЗапасов",
																"СуммаАкциза",
																"ТаблицаВидыЗапасов.СуммаАкциза",
																"0");
	//-- НЕ УТ
...
Процедура ТаблицаОстатковТаблицаВидыЗапасов(ДокументОбъект, МенеджерВременныхТаблиц, ПараметрыЗаполненияВидовЗапасов)
	...
	//++ НЕ УТ
	|	&ТекстПоляТаблицаВидыЗапасовСуммаАкциза_ КАК СуммаАкцизаОстаток,
	//-- НЕ УТ
	|	0 КАК СуммаОстаток
	|ПОМЕСТИТЬ ТаблицаОстатков
	|ИЗ
...
Процедура ТаблицаОстатковПереданныхМеждуОрганизациямиТоваров(ДокументОбъект, МенеджерВременныхТаблиц, ПараметрыЗаполненияВидовЗапасов)
	...
	//++ НЕ УТ
	|	СУММА(ПереданныеТовары.СуммаАкциза) КАК СуммаАкцизаОстаток,
	//-- НЕ УТ
	|	0 КАК СуммаОстаток
	|ИЗ
	|	(ВЫБРАТЬ
	...
	|		КОНЕЦ КАК КоличествоПоРНПТ,
	//++ НЕ УТ
	|		ДанныеЗапасов.СуммаАкциза КАК СуммаАкциза,
	//-- НЕ УТ
	...
	//++ НЕ УТ
	|	ОстаткиПереданныхТоваров.СуммаАкцизаОстаток КАК СуммаАкцизаОстаток,
	//-- НЕ УТ
	...
	//++ НЕ УТ
	|	ОстаткиПереданныхТоваров.СуммаАкцизаОстаток,
	//-- НЕ УТ
	|	0
	|ИЗ
...
Функция ОписаниеТаблицСОбязательнымиКолонками()
	...
	//++ НЕ УТ
	ОписаниеТаблиц.ИсходнаяТаблицаТоваров.Вставить("ОблагаетсяАкцизом", "ЛОЖЬ");
	ОписаниеТаблиц.ИсходнаяТаблицаТоваров.Вставить("ПодразделениеПроизводства", "ЗНАЧЕНИЕ(Справочник.СтруктураПредприятия.ПустаяСсылка)");
	//-- НЕ УТ
	...
	//++ НЕ УТ
	ОписаниеТаблиц.ТаблицаДанныхДокумента.Вставить("ОблагаетсяАкцизом", "ЛОЖЬ");
	ОписаниеТаблиц.ТаблицаДанныхДокумента.Вставить("ПодразделениеПроизводства", "ЗНАЧЕНИЕ(Справочник.СтруктураПредприятия.ПустаяСсылка)");
	//-- НЕ УТ
