...
Процедура ДобавитьЭтап_ПереходНаУчетВнеоборотныхАктивовВерсии24(ТаблицаЭтапов,ТекущийРодитель) Экспорт
	НоваяСтрока = ЗакрытиеМесяцаСервер.ДобавитьЭтапВТаблицу(ТаблицаЭтапов, ТекущийРодитель,
		Справочники.ОперацииЗакрытияМесяца.ПереходНаУчетВнеоборотныхАктивовВерсии24,,,,
...
Процедура ДобавитьЭтап_ПризнаниеВНалоговомУчетеАрендныхПлатежей(ТаблицаЭтапов,ТекущийРодитель) Экспорт
	...
	НоваяСтрока.ПредшествующиеЭтапы.Добавить(Справочники.ОперацииЗакрытияМесяца.ПереходНаУчетВнеоборотныхАктивовВерсии24);
...
Функция ТекстЗапросаПрочиеРасходыПринятияКУчету(ЕстьПорядокУчетаУУ, ЕстьПорядокУчетаБУ, ЕстьПорядокУчетаНУ) Экспорт
...
Функция ДоступныеВариантыПримененияИнвестиционныхНалоговыйВычетов(Организация, Подразделение, Период, ПроверитьРИНВ = Истина) Экспорт
	...
	Если НастройкиНалоговУчетныхПолитикЛокализация.ПрименяетсяФедеральныйИнвестиционныйНалоговыйВычет(Организация, Период) Тогда
		СписокИНВ.Добавить(Перечисления.ВариантыПримененияИНВ.Федеральный);
	КонецЕсли;

	Если ПроверитьРИНВ 
		И НастройкиНалоговУчетныхПолитикЛокализация.ПрименяетсяРегиональныйИнвестиционныйНалоговыйВычет(Организация, Подразделение, Период) Тогда
		СписокИНВ.Добавить(Перечисления.ВариантыПримененияИНВ.Региональный);
	КонецЕсли;
	...
	Для Каждого Элемент Из СведенияОбУчете Цикл
		...
		Если СведенияОбУчетеОС.СостояниеНУ = Перечисления.СостоянияОС.ПринятоКУчету
			...
			Если СведенияОбУчетеОС.ИнвестиционныйНалоговыйВычет = Перечисления.ВариантыПримененияИНВ.Региональный Тогда
				...
				НастройкиНалогов = НастройкиНалоговУчетныхПолитикЛокализация.ПараметрыПримененияРегиональныхИНВ(
