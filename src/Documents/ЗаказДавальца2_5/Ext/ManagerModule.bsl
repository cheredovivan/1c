...
Процедура ДобавитьКомандыСозданияНаОсновании(КомандыСозданияНаОсновании, Параметры) Экспорт
	...
	Команда = Документы.ЗаказПереработчику2_5.ДобавитьКомандуСоздатьОбеспечивающийЗаказ(КомандыСозданияНаОсновании);
...
//
// Возвращаемое значение:
//	Массив из ПеречислениеСсылка.СтатусыЗаказовДавальцев - массив допустимых статусов.
//
Функция ДопустимыеСтатусыВводаНаОсновании(ИмяДокумента, ХозяйственнаяОперация = Неопределено) Экспорт
	...
	ИначеЕсли ИмяДокумента = "ОбеспечивающйЗаказ" Тогда
...
Процедура ТоварыДокументаДляСозданияОбеспечивающегоЗаказа(Ссылка, ПараметрыСоздания, МенеджерВременныхТаблиц) Экспорт
	...
	ПроверитьВозможностьВводаНаОсновании(Ссылка, Реквизиты.Статус, Не Реквизиты.Проведен, "ОбеспечивающйЗаказ");
	...
	Если ПараметрыСоздания.ИмяМенеджераДокумента = "ЗаказПереработчику2_5" Тогда
		...
		|		КОГДА
		|			НЕ СУММА(ВЫБОР
		|				КОГДА НЕ Товары.Спецификация = ЗНАЧЕНИЕ(Справочник.РесурсныеСпецификации.ПустаяСсылка)
		|					ТОГДА 1
		|				ИНАЧЕ 0
		|			КОНЕЦ) = 0
		|			И НЕ СУММА(ВЫБОР
		|				КОГДА Товары.Спецификация = ЗНАЧЕНИЕ(Справочник.РесурсныеСпецификации.ПустаяСсылка)
		|					ТОГДА 1
		|				ИНАЧЕ 0
		|			КОНЕЦ) = 0
		|			ТОГДА ИСТИНА
		|		ИНАЧЕ ЛОЖЬ
		|	КОНЕЦ         КАК ЕстьОшибкаСпецификация
		|ИЗ
		...
		Если Выборка.Следующий()
			И Выборка.ЕстьОшибкаСпецификация Тогда
			ВызватьИсключение НСтр("ru = 'Для ввода заказа переработчику необходимо чтобы спецификация продукции во всех строках была заполнена, либо не заполнена.
									|Ввод на основании заказа со смешанным заполнением спецификации не поддерживается.';
									|en = 'To generate a subcontracting purchase order, either fill bills of materials in all lines or clear bills of materials in all lines.
									|Generation based on orders with partially specified bills of materials is not supported.'");
		КонецЕсли;
...
Функция ТекстЗапросаТаблицаРаспоряженияНаОтгрузку(Запрос, ТекстыЗапроса, Регистры)
	...
	|		ИНАЧЕ ЗНАЧЕНИЕ(Справочник.Склады.ПустаяСсылка)
	|	КОНЕЦ КАК Склад,
	|	ЗНАЧЕНИЕ(Перечисление.ПоказателиРаспоряженийНаОтгрузкуИВозврат.КОформлению) КАК Показатель,
	|	ТаблицаТовары.Количество КАК Количество,
	...
	|	ЗНАЧЕНИЕ(Перечисление.ПоказателиРаспоряженийНаОтгрузкуИВозврат.КОтгрузке) КАК Показатель,
	|	ТаблицаТовары.Количество КАК Количество,
	|	0 КАК Сумма,
	|	ЗНАЧЕНИЕ(Справочник.ПричиныОтменыЗаказовКлиентов.ПустаяСсылка) КАК ПричинаОтмены
	|ИЗ
