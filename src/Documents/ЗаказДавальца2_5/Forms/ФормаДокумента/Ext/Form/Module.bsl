...
Процедура ГиперссылкаСостояниеОбеспеченияНажатиеОбщий()
	...
	Если Объект.Статус = ПредопределенноеЗначение("Перечисление.СтатусыЗаказовДавальцев.НеСогласован") Тогда
...
Процедура МатериалыУстановитьДействиеВСтрокахНаСервере(Действие, Знач ИдентификаторыСтрок, КэшированныеЗначения)
	...
	СтруктураДействий.Вставить(
		...
		Новый Структура("Склад, ПараметрыУказанияСерий, ТаблицаТоваров",
			Объект.СкладПоступления, ПараметрыУказанияСерий.Материалы, ОбрабатываемыеСтроки));
