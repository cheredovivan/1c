...
Процедура ОбработкаЗаполнения(ДанныеЗаполнения, ТекстЗаполнения, СтандартнаяОбработка)
	...
	Если Не ЗначениеЗаполнено(СпособРаспределенияЗатратНаВыходныеИзделия)
		И (Не ГруппировкаЗатрат = Перечисления.ГруппировкиЗатратВПроизводствеБезЗаказа.ПоПродукции
				И Не ГруппировкаЗатрат = Перечисления.ГруппировкиЗатратВПроизводствеБезЗаказа.ПоПродукцииИНазначениям
				И Не ГруппировкаЗатрат = Перечисления.ГруппировкиЗатратВПроизводствеБезЗаказа.ПоПродукцииБезСерий
				И Не ГруппировкаЗатрат = Перечисления.ГруппировкиЗатратВПроизводствеБезЗаказа.ПоПродукцииИНазначениямБезСерий
			Или Исправление
				И ИсправлениеСохраняетПартииПроизводства) Тогда
...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Если Не Исправление
		Или Не ИсправлениеСохраняетПартииПроизводства Тогда
	...
	ИначеЕсли ГруппировкаЗатрат = Перечисления.ГруппировкиЗатратВПроизводствеБезЗаказа.ПоПродукции
		Или ГруппировкаЗатрат = Перечисления.ГруппировкиЗатратВПроизводствеБезЗаказа.ПоПродукцииИНазначениям
		Или ГруппировкаЗатрат = Перечисления.ГруппировкиЗатратВПроизводствеБезЗаказа.ПоПродукцииБезСерий
		Или ГруппировкаЗатрат = Перечисления.ГруппировкиЗатратВПроизводствеБезЗаказа.ПоПродукцииИНазначениямБезСерий Тогда
		МассивНепроверяемыхРеквизитов.Добавить("ВыходныеИзделия.Спецификация");
	КонецЕсли;
	...
	ПараметрыПроверкиНазначения.ДавальческиеГруппировкиЗатрат.Добавить(Перечисления.ГруппировкиЗатратВПроизводствеБезЗаказа.ПоПродукцииИНазначениямБезСерий);
	...
	Если (ГруппировкаЗатрат = Перечисления.ГруппировкиЗатратВПроизводствеБезЗаказа.ПоПродукции
			Или ГруппировкаЗатрат = Перечисления.ГруппировкиЗатратВПроизводствеБезЗаказа.ПоПродукцииИНазначениям
			Или ГруппировкаЗатрат = Перечисления.ГруппировкиЗатратВПроизводствеБезЗаказа.ПоПродукцииБезСерий
			Или ГруппировкаЗатрат = Перечисления.ГруппировкиЗатратВПроизводствеБезЗаказа.ПоПродукцииИНазначениямБезСерий)
		И (Не Исправление
			Или Не ИсправлениеСохраняетПартииПроизводства) Тогда
...
Функция ВременныеТаблицыДанныхВыходныеИзделия()
	...
	|		ПО ТаблицаТоваров.Организация = ПроизводителиПодакцизныхТоваров.Организация
	|
	|ГДЕ
...
Функция ВременныеТаблицыДанныхПобочныеИзделия()
	...
	|	ВЫБОР
	//++ НЕ УТКА
	|		КОГДА ТаблицаТоваров.ВладелецИзделия <> ЗНАЧЕНИЕ(Справочник.Организации.ПустаяСсылка)
...
Функция ТаблицаДляЗаполненияВидыЗапасовВыходныеИзделия(МенеджерВременныхТаблиц)
	...
	//-- НЕ УТКА
	|		КОГДА ИСТИНА
	|			ТОГДА ЕСТЬNULL(ДанныеКатегорий.ПодакцизныйТовар, ЛОЖЬ)
...
Процедура ИнициализироватьДокумент(ДанныеЗаполнения = Неопределено)
	...
	// Если это исправление, то флаг ИсправлениеСохраняетПартииПроизводства наследуется из исправляемого документа.
	// Если нет, то для всех новых документов устанавливаем этот флаг в Истину.
	Если Не Исправление Тогда
		ИсправлениеСохраняетПартииПроизводства = Истина;
	КонецЕсли;
...
Процедура ПроверитьДолиСтоимости(Отказ)
	...
	Запрос.УстановитьПараметр("ИсправлениеСохраняетПартииПроизводства", ИсправлениеСохраняетПартииПроизводства);
...
Процедура ПроверитьПривязкуЗатратКГруппамЗатрат(Отказ)
	...
	Запрос.УстановитьПараметр("ГруппировкаЗатрат", ГруппировкаЗатрат);
	Запрос.УстановитьПараметр("ВыходныеИзделия",   ВыходныеИзделия.Выгрузить(,  "НомерСтроки, НомерГруппыЗатрат"));
	...
	Пока Выборка.Следующий() Цикл
		...
		Если Выборка.НетГруппыЗатрат Тогда
...
Процедура ПроверитьКорректностьУказанияПодакцизныхТоваров(Отказ)
	...
	|	ТаблицаТоваров.Получатель	КАК Получатель,
	|	ПроизводителиПодакцизныхТоваров.Страна КАК Страна,
	|	ЕСТЬNULL(ПроизводителиПодакцизныхТоваров.ПроизводитПодакцизныйТовар, ЛОЖЬ) КАК ПроизводитПодакцизныйТовар
	|ПОМЕСТИТЬ ТаблицаТоваров
	|ИЗ
	|	ВтВыходныеИзделия КАК ТаблицаТоваров
	...
	|	ЕСТЬNULL(ПроизводителиПодакцизныхТоваров.ПроизводитПодакцизныйТовар, ЛОЖЬ) КАК ПроизводитПодакцизныйТовар
	|ИЗ
	|	ВтПобочныеИзделия КАК ТаблицаТоваров
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.Номенклатура КАК СправочникНоменклатура
	|		ПО ТаблицаТоваров.Номенклатура = СправочникНоменклатура.Ссылка
	//++ НЕ УТКА
	|		
	...
	ЧастьТекстаЗапроса = 
	"ВЫБРАТЬ
	|	ТаблицаОшибок.ИмяТЧ						КАК ИмяТЧ,
	|	ТаблицаОшибок.НомерСтроки				КАК НомерСтроки,
	|	ТаблицаОшибок.Организация				КАК Организация,
	...
	|		ТаблицаТоваров.Организация	КАК Организация,
	|		ВЫБОР
	|			КОГДА НЕ ТаблицаТоваров.ПроизводитПодакцизныйТовар
	|					И ЕСТЬNULL(ДанныеКатегорий.ПодакцизныйТовар, ЛОЖЬ)
	|				ТОГДА ИСТИНА
	|			ИНАЧЕ ЛОЖЬ
	|		КОНЕЦ						КАК УказанПодакцизныйТовар,
	|		ВЫБОР
	|			КОГДА ТаблицаТоваров.ПроизводитПодакцизныйТовар
	|					И ЕСТЬNULL(ДанныеКатегорий.ПодакцизныйТовар, ЛОЖЬ)
	...
	|			ПО ТаблицаТоваров.Получатель = СправочникСклады.Ссылка
	|	
	|	) КАК ТаблицаОшибок
	|
	|ГДЕ
	...
	ОписаниеТабличныхЧастей = Метаданные().ТабличныеЧасти;
	...
	КолонкиВыходныеИзделия = ВыгружаемыеКолонки;
	КолонкиПобочныеИзделия = ВыгружаемыеКолонки;
	//++ НЕ УТКА
	КолонкиВыходныеИзделия = ВыгружаемыеКолонки + ", ВнутренняяПереработка, ОрганизацияДавалец";
	КолонкиПобочныеИзделия = ВыгружаемыеКолонки + ", ВладелецИзделия";
	//-- НЕ УТКА
	...
	Запрос.УстановитьПараметр("Дата",					Дата);
	Запрос.УстановитьПараметр("Организация",			Организация);
	Запрос.УстановитьПараметр("ИмяТЧВыходныеИзделия",	ОписаниеТабличныхЧастей.ВыходныеИзделия.Имя);
	Запрос.УстановитьПараметр("ИмяТЧПобочныеИзделия",	ОписаниеТабличныхЧастей.ПобочныеИзделия.Имя);
	Запрос.УстановитьПараметр("ВыходныеИзделия",		ВыходныеИзделия.Выгрузить(, КолонкиВыходныеИзделия));
	Запрос.УстановитьПараметр("ПобочныеИзделия",		ПобочныеИзделия.Выгрузить(, КолонкиПобочныеИзделия));
	...
	ШаблонСообщенияУказанПодакцизныйТовар = НСтр("ru = 'В строке %1 списка ""%2"" указана подакцизная продукция.
		|Выбор такой номенклатуры запрещен, так как учетная политика финансового учета организации ""%3"" не предусматривает производство подакцизной продукции.';
		|en = 'Excisable products are specified in line %1 of the ""%2"" list.
		|Cannot select this item, as the financial accounting policy of the ""%3"" company does not allow producing excisable products.'");
	
	ШаблонСообщенияУказанРозничныйСклад = НСтр("ru = 'В строке %1 списка ""%2"" в качестве получателя указан розничный склад.
	...
	Пока Выборка.Следующий() Цикл
		...
		Если Выборка.УказанПодакцизныйТовар Тогда
			...
			ТекстСообщения	= СтрШаблон(ШаблонСообщенияУказанПодакцизныйТовар,
										Выборка.НомерСтроки,
										ОписаниеТабличныхЧастей[Выборка.ИмяТЧ].Синоним,
										Выборка.Организация);
		ИначеЕсли Выборка.УказанРозничныйСклад Тогда
			ИмяПоля			= "Получатель";
			ТекстСообщения	= СтрШаблон(ШаблонСообщенияУказанРозничныйСклад,
										Выборка.НомерСтроки,
										ОписаниеТабличныхЧастей[Выборка.ИмяТЧ].Синоним);
		КонецЕсли;
		
		Если Не ПустаяСтрока(ТекстСообщения) Тогда
			Поле = ОбщегоНазначенияКлиентСервер.ПутьКТабличнойЧасти(Выборка.ИмяТЧ, Выборка.НомерСтроки, ИмяПоля);
