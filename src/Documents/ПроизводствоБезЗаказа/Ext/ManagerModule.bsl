...
Функция ПараметрыРаспределенияЗатрат(Объект) Экспорт
	...
	Если Объект.ГруппировкаЗатрат = Перечисления.ГруппировкиЗатратВПроизводствеБезЗаказа.ПоДокументу
		...
		Или Объект.Исправление
			И Объект.ИсправлениеСохраняетПартииПроизводства Тогда
...
Функция ПереченьПолейГруппыЗатрат(ГруппировкаЗатрат) Экспорт
	...
	ИначеЕсли ГруппировкаЗатрат = Перечисления.ГруппировкиЗатратВПроизводствеБезЗаказа.ПоПродукцииИНазначениям Тогда
		...
		СписокПолей.Добавить("Серия",              НСтр("ru = 'Серия';
														|en = 'Batch'", КодЯзыка));
	ИначеЕсли ГруппировкаЗатрат = Перечисления.ГруппировкиЗатратВПроизводствеБезЗаказа.ПоПродукцииБезСерий Тогда
		СписокПолей.Добавить("Номенклатура",       НСтр("ru = 'Номенклатура';
														|en = 'Items'", КодЯзыка));
		СписокПолей.Добавить("Характеристика",     НСтр("ru = 'Характеристика';
														|en = 'Variant'", КодЯзыка));
	ИначеЕсли ГруппировкаЗатрат = Перечисления.ГруппировкиЗатратВПроизводствеБезЗаказа.ПоПродукцииИНазначениямБезСерий Тогда
		СписокПолей.Добавить("Номенклатура",       НСтр("ru = 'Номенклатура';
														|en = 'Items'", КодЯзыка));
		СписокПолей.Добавить("Характеристика",     НСтр("ru = 'Характеристика';
														|en = 'Variant'", КодЯзыка));
...
Процедура ЗаполнитьГруппыЗатратВКоллекции(Коллекция)
	...
	Для Каждого ТекСтрока Из Коллекция Цикл
		Если ЗначениеЗаполнено(ТекСтрока.Назначение) Тогда
			...
			ИначеЕсли ТекСтрока.ГруппировкаЗатрат = Перечисления.ГруппировкиЗатратВПроизводствеБезЗаказа.ПоПродукцииБезСерий Тогда
				ТекСтрока.ГруппировкаЗатрат = Перечисления.ГруппировкиЗатратВПроизводствеБезЗаказа.ПоПродукцииИНазначениямБезСерий;
			КонецЕсли;
		КонецЕсли;
	КонецЦикла;
...
Процедура ЗаполнитьНазначениеМатериаловПоОбособленнымОстаткам(МенеджерВременныхТаблиц, Параметры)
	...
	Запрос.Текст =
		...
		|		ЗНАЧЕНИЕ(Перечисление.ГруппировкиЗатратВПроизводствеБезЗаказа.ПоПродукцииИНазначениямБезСерий),
...
Процедура ЗаполнитьПараметрыИнициализации(Запрос, ДокументСсылка)
	...
	Запрос.Текст =
		...
		|	Реквизиты.ИсправлениеСохраняетПартииПроизводства,
		|	ВЫБОР
		|		КОГДА Реквизиты.Исправление
		|			И Реквизиты.ИсправлениеСохраняетПартииПроизводства
		|			ТОГДА Реквизиты.ИсправляемыйДокумент
	...
	Запрос.УстановитьПараметр("ИспользоватьИсправлениеДокументов", ПолучитьФункциональнуюОпцию("ИспользоватьИсправлениеДокументов"));
...
Функция ТекстЗапросаВтОснований(Запрос, ТекстыЗапроса)
	...
	|			КОГДА ДанныеГруппЗатрат.ОрганизацияДавалец ЕСТЬ НЕ NULL
	|				ТОГДА ДанныеГруппЗатрат.ОрганизацияДавалец
	//-- НЕ УТКА
	|			КОГДА ИСТИНА
	...
	|		
	//++ НЕ УТКА
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.ПроизводствоБезЗаказа.ВыходныеИзделия КАК ДанныеГруппЗатрат
	|		ПО ДанныеГруппЗатрат.Ссылка           = ДанныеДокумента.Ссылка
	|		И ДанныеГруппЗатрат.НомерГруппыЗатрат = ДанныеДокумента.НомерГруппыЗатрат
	|		И ДанныеГруппЗатрат.ВнутренняяПереработка
	|	
	//-- НЕ УТКА
	|	ГДЕ
	|		ДанныеДокумента.Ссылка = &Ссылка
...
Функция ТекстЗапросаТаблицаТрудозатратыСИсправлением(Запрос, ТекстыЗапроса) Экспорт
	
	ИмяРегистра = "ТрудозатратыСИсправлением";
	...
	|	ЛОЖЬ                              КАК Сторно
	|ПОМЕСТИТЬ ТрудозатратыСИсправлением
	|ИЗ
	...
	|	
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ПроизводствоБезЗаказа.Трудозатраты КАК ТаблицаТрудозатраты
	|	ПО ТаблицаТрудозатраты.Ссылка = РеквизитыДокумента.СторнируемыйДокумент
	|	
	|	ЛЕВОЕ СОЕДИНЕНИЕ Справочник.ПартииПроизводства КАК СпрПартииПроизводства
	|	ПО СпрПартииПроизводства.Документ = &ДокументПартии
	|	И СпрПартииПроизводства.Код       = ТаблицаТрудозатраты.НомерГруппыЗатрат
	...
	|	И РеквизитыДокумента.Ссылка = &Ссылка
	|	И &РаспоряжениеДляТрудозатрат
	|	И &Исправление
	|	И &ИсправлениеСохраняетПартииПроизводства
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	ПартияПроизводства,
	|	ВидРабот,
...
Функция ТекстЗапросаТаблицаТрудозатратыИзменения(Запрос, ТекстыЗапроса) Экспорт
	
	ИмяРегистра = "ТрудозатратыИзменения";
	
	Если НЕ ПроведениеДокументов.ЕстьТаблицаЗапроса("ТрудозатратыСИсправлением", ТекстыЗапроса) Тогда
		ТекстЗапросаТаблицаТрудозатратыСИсправлением(Запрос, ТекстыЗапроса)
	КонецЕсли;
	
	ТекстЗапроса =
	"ВЫБРАТЬ
	|	ТаблицаТрудозатраты.ПартияПроизводства КАК ПартияПроизводства,
	|	ТаблицаТрудозатраты.ВидРабот           КАК ВидРабот,
	|	ТаблицаТрудозатраты.Исполнитель        КАК Исполнитель,
	|	ТаблицаТрудозатраты.Подразделение      КАК Подразделение,
	|	СУММА(ТаблицаТрудозатраты.Количество)  КАК Количество
	|ПОМЕСТИТЬ ТрудозатратыИзменения
	|ИЗ
	|	ТрудозатратыСИсправлением КАК ТаблицаТрудозатраты
	|
	|СГРУППИРОВАТЬ ПО
	|	ТаблицаТрудозатраты.ПартияПроизводства,
	|	ТаблицаТрудозатраты.ВидРабот,
	|	ТаблицаТрудозатраты.Исполнитель,
	|	ТаблицаТрудозатраты.Подразделение
	|
	|ИМЕЮЩИЕ
	|	НЕ СУММА(ТаблицаТрудозатраты.Количество) = 0
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	ПартияПроизводства,
	|	ВидРабот,
...
Функция ТекстЗапросаТаблицаТрудозатратыКОформлению(Запрос, ТекстыЗапроса, Регистры)
	...
	Если НЕ ПроведениеДокументов.ЕстьТаблицаЗапроса("ТрудозатратыИзменения", ТекстыЗапроса) Тогда
		ТекстЗапросаТаблицаТрудозатратыИзменения(Запрос, ТекстыЗапроса)
	КонецЕсли;
	...
	|	ТаблицаТрудозатраты.Количество            КАК Количество,
	|	ТаблицаТрудозатраты.Сторно                КАК Сторно
	|ИЗ
	|	ТрудозатратыСИсправлением КАК ТаблицаТрудозатраты
	|ГДЕ
	|	ИСТИНА В(
	...
	|			ИСТИНА
	|		ИЗ
	|			ТрудозатратыИзменения КАК ТрудозатратыИзменения
	|		ГДЕ
	|			ТрудозатратыИзменения.ПартияПроизводства = ТаблицаТрудозатраты.ПартияПроизводства
	|			И ТрудозатратыИзменения.ВидРабот         = ТаблицаТрудозатраты.ВидРабот
	|			И ТрудозатратыИзменения.Исполнитель      = ТаблицаТрудозатраты.Исполнитель
	|			И ТрудозатратыИзменения.Подразделение    = ТаблицаТрудозатраты.Подразделение
	|		)
	|";
...
// Параметры:
//  Объект - ДокументОбъект.ПроизводствоБезЗаказа - объект, для которого необходимо сформировать партии производства.
//
Процедура СформироватьПартииПроизводства(Объект) Экспорт
	...
	ТекстыЗапроса.Добавить(ТекстЗапросаТаблицаПродукции());
	ТекстыЗапроса.Добавить(ТекстЗапросаПоляГруппЗатрат());
	...
	ТекстВыборкаПолейПартий = ТекстЗапросаВыборкаПолейПартий();
...
Функция ТекстЗапросаПоляГруппЗатрат()
	
	Возврат
	"ВЫБРАТЬ РАЗРЕШЕННЫЕ
	|	ТаблицаПродукция.Ссылка                КАК Ссылка,
	|	ТаблицаПродукция.НомерГруппыЗатрат     КАК НомерГруппыЗатрат,
	|	ТаблицаПродукция.ГруппировкаЗатрат     КАК ГруппировкаЗатрат,
	|	ВЫБОР
	//++ НЕ УТКА
	...
	//-- НЕ УТКА
	|		КОГДА ИСТИНА
	|			ТОГДА ТаблицаПродукция.Организация
	|	КОНЕЦ                                  КАК Организация,
	//++ НЕ УТКА
	|	ТаблицаПродукция.ВнутренняяПереработка КАК ВнутренняяПереработка,
	//-- НЕ УТКА
	|	ТаблицаПродукция.ВыпускПодДеятельность КАК ВыпускПодДеятельность,
	|	МИНИМУМ(ВЫБОР
	|		КОГДА ТаблицаПродукция.ГруппировкаЗатрат В
	...
	|				ЗНАЧЕНИЕ(Перечисление.ГруппировкиЗатратВПроизводствеБезЗаказа.ПоПродукцииБезСерий),
	|				ЗНАЧЕНИЕ(Перечисление.ГруппировкиЗатратВПроизводствеБезЗаказа.ПоСпецификациям))
	|			ТОГДА ТаблицаПродукция.НаправлениеДеятельности
	...
	|	ТаблицаПродукция.ГруппировкаЗатрат,
	|	ВЫБОР
	//++ НЕ УТКА
	|		КОГДА ТаблицаПродукция.ВнутренняяПереработка
	|			ТОГДА ТаблицаПродукция.ОрганизацияДавалец
	...
	|	КОНЕЦ,
	//++ НЕ УТКА
	|	ТаблицаПродукция.ВнутренняяПереработка,
	//-- НЕ УТКА
	|	ТаблицаПродукция.ВыпускПодДеятельность
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	Организация,
	//++ НЕ УТКА
...
Функция ТекстЗапросаВыборкаПолейПартий()
	
	Возврат
	"ВЫБРАТЬ РАЗРЕШЕННЫЕ
	|	ПоляГруппЗатрат.Ссылка                       КАК Документ,
	|	ПоляГруппЗатрат.НомерГруппыЗатрат            КАК НомерГруппыЗатрат,
	|	ПоляГруппЗатрат.ГруппировкаЗатрат            КАК ГруппировкаЗатрат,
	|	ПоляГруппЗатрат.Организация                  КАК Организация,
	//++ НЕ УТКА
	|	ПоляГруппЗатрат.ВнутренняяПереработка        КАК ВнутренняяПереработка,
	//-- НЕ УТКА
	|	ПоляГруппЗатрат.ВыпускПодДеятельность        КАК ВидДеятельностиНДС,
	...
	|	ПоляГруппЗатрат.НаправлениеДеятельности      КАК НаправлениеДеятельности,
	|	ПоляГруппЗатрат.Номенклатура                 КАК Номенклатура,
	|	ПоляГруппЗатрат.Характеристика               КАК Характеристика,
	|	ПоляГруппЗатрат.Спецификация                 КАК Спецификация,
	|	ПоляГруппЗатрат.Назначение                   КАК Назначение,
	|	ПоляГруппЗатрат.Серия                        КАК Серия
	|ПОМЕСТИТЬ ВТПоляПартийПроизводства
	|ИЗ
	|	ПоляГруппЗатрат КАК ПоляГруппЗатрат
	|
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ НомераСтрокОсновныхИзделий КАК НомераСтрокОсновныхИзделий
	|	ПО НомераСтрокОсновныхИзделий.Ссылка = ПоляГруппЗатрат.Ссылка
	|	И НомераСтрокОсновныхИзделий.НомерГруппыЗатрат = ПоляГруппЗатрат.НомерГруппыЗатрат
	|
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ ТаблицаПродукция КАК ОсновныеИзделия
	|	ПО ОсновныеИзделия.Ссылка = ПоляГруппЗатрат.Ссылка
	|	И ОсновныеИзделия.НомерСтроки = НомераСтрокОсновныхИзделий.НомерСтроки
	|
...
Функция ТекстЗапросаЗаполненияСтатусовУказанияСерийВыходныеИзделия()
	...
	|				ИЛИ &Дата < Склады.ДатаНачалаОрдернойСхемыПриПоступлении)
	|			И ПолитикиУчетаСерийСклад.ПолитикаУчетаСерий.УказыватьПриПриемкеПродукцииИзПроизводства
	|			ТОГДА ВЫБОР
	...
	|						КОНЕЦ
	|				КОНЕЦ
	|		КОГДА ПолитикиУчетаСерийОбщие.ПолитикаУчетаСерий.УказыватьПриПроизводствеПродукции
	|			ТОГДА ВЫБОР
	|					КОГДА ТоварыДляЗапроса.Количество = ЕСТЬNULL(СерииДляЗапроса.Количество, 0)
	|							И ТоварыДляЗапроса.Количество > 0
	|						ТОГДА 2
	...
	|		ПО ПолитикиУчетаСерийСклад.Склад = ТоварыДляЗапроса.Получатель
	|		И ТоварыДляЗапроса.ВидНоменклатуры = ПолитикиУчетаСерийСклад.Ссылка
	|
	|	ЛЕВОЕ СОЕДИНЕНИЕ Справочник.Склады КАК Склады
	|		ПО ПолитикиУчетаСерийСклад.Склад = Склады.Ссылка
	|	
	|	ЛЕВОЕ СОЕДИНЕНИЕ Справочник.ВидыНоменклатуры КАК ПолитикиУчетаСерийОбщие
	|	ПО ПолитикиУчетаСерийОбщие.Ссылка = ТоварыДляЗапроса.ВидНоменклатуры
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
...
Функция ТекстЗапросаЗаполненияСтатусовУказанияСерийМатериалыИРаботы()
	...
	|						КОНЕЦ
	|				КОНЕЦ
	|		КОГДА ПолитикиУчетаСерийОбщие.ПолитикаУчетаСерий.УказыватьПриРасходеМатериалов
	|			ТОГДА ВЫБОР
	|					КОГДА Товары.Серия <> ЗНАЧЕНИЕ(Справочник.СерииНоменклатуры.ПустаяСсылка)
	|						ТОГДА 2
	|					ИНАЧЕ 1
	...
	|	
	|	ЛЕВОЕ СОЕДИНЕНИЕ Справочник.ВидыНоменклатуры.ПолитикиУчетаСерий КАК ПолитикиУчетаСерийСклад
	|	ПО ПолитикиУчетаСерийСклад.Склад = Товары.Склад
	|	И ПолитикиУчетаСерийСклад.Ссылка = СпрНоменклатура.ВидНоменклатуры
	|	
	|	ЛЕВОЕ СОЕДИНЕНИЕ Справочник.ВидыНоменклатуры КАК ПолитикиУчетаСерийОбщие
	|	ПО ПолитикиУчетаСерийОбщие.Ссылка = СпрНоменклатура.ВидНоменклатуры
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик.Версия = "2.5.25.26";
	Обработчик.РежимВыполнения = "Отложенно";
	Обработчик.Идентификатор = Новый УникальныйИдентификатор("22e5af9c-ec3d-4253-83a1-3a0812b5dd49");
	...
	СписокОписаний.Добавить(НСтр("ru = '- заполняет служебный реквизит ""Исправление не сохраняет партии производства"".';
								|en = '- Fills in the internal ""Correction does not save production lots"" attribute.'"));
...
Процедура ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	|	Документ.ПроизводствоБезЗаказа КАК РеквизитыДокумента
	|ГДЕ
	// Взводим флаг для всех исходных документов, по которым не введены исправляющие/сторнирующие документы.
	|	НЕ РеквизитыДокумента.Исправление
	|	И НЕ РеквизитыДокумента.ИсправлениеСохраняетПартииПроизводства
	|	И НЕ ИСТИНА В(
	|		ВЫБРАТЬ ПЕРВЫЕ 1
	|			ИСТИНА
	|		ИЗ
	...
	|			
	|			ЛЕВОЕ СОЕДИНЕНИЕ Документ.ПроизводствоБезЗаказа КАК РеквизитыИсправления
	|			ПО РеквизитыИсправления.Ссылка = СторнирующиеДокументы.Ссылка
	|			
	|		ГДЕ
	|			СторнирующиеДокументы.СторнируемыйДокумент = РеквизитыДокумента.Ссылка
	|			И СторнирующиеДокументы.Проведен
	|			И НЕ СторнирующиеДокументы.ДополнительнаяЗапись
	|			И НЕ ЕСТЬNULL(РеквизитыИсправления.ИсправлениеСохраняетПартииПроизводства, ЛОЖЬ)
	|		)
	|";
	...
	Пока Выборка.Следующий() Цикл
		...
		Попытка
			...
			Если ДокументОбъект <> Неопределено Тогда
				ДокументОбъект.ИсправлениеСохраняетПартииПроизводства = Истина;
		...
		Исключение
			...
			Если ПричинаИсключения = 2 Тогда
				ВызватьИсключение НСтр("ru = 'Не удалось обработать обновление документов ""Производство без заказа"".';
										|en = 'Cannot process the update of the ""Backflush production"" documents.'");
			КонецЕсли;
