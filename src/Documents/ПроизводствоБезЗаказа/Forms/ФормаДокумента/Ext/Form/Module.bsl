	...
	ИначеЕсли НоменклатураКлиент.ЭтоУказаниеСерий(ИсточникВыбора) Тогда
		...
		Если Не ТекущиеДанные = Неопределено Тогда
			...
			Если ВыбранноеЗначение.ИмяТЧ = "ВыходныеИзделия"
				И (Объект.ГруппировкаЗатрат = ПредопределенноеЗначение("Перечисление.ГруппировкиЗатратВПроизводствеБезЗаказа.ПоПродукции")
					Или Объект.ГруппировкаЗатрат = ПредопределенноеЗначение("Перечисление.ГруппировкиЗатратВПроизводствеБезЗаказа.ПоПродукцииИНазначениям")) Тогда
				
				ПоляСтроки = "НомерГруппыЗатрат, ГруппаЗатрат, Номенклатура, Характеристика, Спецификация, Назначение, Серия";
				//++ НЕ УТКА
				ПоляСтроки = "НомерГруппыЗатрат, ГруппаЗатрат, Номенклатура, Характеристика, Спецификация, Назначение, Серия, ВнутренняяПереработка, ОрганизацияДавалец";
				//-- НЕ УТКА
				ТекущиеДанныеСтруктура = Новый Структура(ПоляСтроки);
...
Процедура ГруппировкаЗатратПриИзмененииНаСервере()
	...
	Если Объект.ГруппировкаЗатрат = Перечисления.ГруппировкиЗатратВПроизводствеБезЗаказа.ПоПродукции
		Или Объект.ГруппировкаЗатрат = Перечисления.ГруппировкиЗатратВПроизводствеБезЗаказа.ПоПродукцииИНазначениям
		Или Объект.ГруппировкаЗатрат = Перечисления.ГруппировкиЗатратВПроизводствеБезЗаказа.ПоПродукцииБезСерий
		Или Объект.ГруппировкаЗатрат = Перечисления.ГруппировкиЗатратВПроизводствеБезЗаказа.ПоПродукцииИНазначениямБезСерий Тогда
...
Процедура ВыходныеИзделияПриИзменении(Элемент)
	...
	Если (Не Объект.Исправление
			Или Не Объект.ИсправлениеСохраняетПартииПроизводства)
	...
	ИначеЕсли Объект.Исправление
		И Объект.ИсправлениеСохраняетПартииПроизводства Тогда
		УстановитьНомерГруппыЗатратПоУмолчанию("ВыходныеИзделия");
	КонецЕсли;
	
	УстановитьЗаголовкиГруппПродукции(ЭтотОбъект, "ВыходныеИзделия");
...
&НаКлиенте
Процедура ВыходныеИзделияГруппаЗатратНачалоВыбора(Элемент, ДанныеВыбора, ВыборДобавлением, СтандартнаяОбработка)
	
	ТаблицаГруппаЗатратНачалоВыбора("ВыходныеИзделия", Ложь);
...
Процедура ПобочныеИзделияПриИзменении(Элемент)
	...
	УстановитьЗаголовкиГруппПродукции(ЭтотОбъект, "ПобочныеИзделия");
...
Процедура ТаблицаГруппаЗатратВыборНаСервере(РезультатЗакрытия, ПараметрыОповещения)
	...
	Для Каждого ИдентификаторСтроки Из ВыделенныеСтроки Цикл
		...
		//++ НЕ УТКА
		Если ПараметрыОповещения.ИмяТЧ = "ВыходныеИзделия"
			Или ПараметрыОповещения.ИмяТЧ = "ПобочныеИзделия" Тогда
...
Процедура ВыходныеИзделияЗаполнитьГруппуЗатрат(Команда)
	...
	ТаблицаГруппаЗатратНачалоВыбора("ВыходныеИзделия", Истина);
...
Процедура ВыходныеИзделияЗаполнитьЦеныПоКалькуляции(Команда)
	...
	Если ЗначениеЗаполнено(ТекущиеДанные.Спецификация) Тогда
		ОбъектыКалькуляции.Добавить(ТекущиеДанные.Спецификация);
	КонецЕсли;
	СтруктураОтбора.Вставить("ОбъектыКалькуляции", ОбъектыКалькуляции);
	СтруктураОтбора.Вставить("ОтключитьОтборПоОбъектамКалькуляции", Истина);
...
Процедура УстановитьУсловноеОформление()
	...
	// Установка видимости колонки группы затрат таблицы изделий
	...
	ОтборЭлемента = ГруппаИли.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
	ОтборЭлемента.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("Объект.ИсправлениеСохраняетПартииПроизводства");
	ОтборЭлемента.ВидСравнения = ВидСравненияКомпоновкиДанных.Равно;
	ОтборЭлемента.ПравоеЗначение = Ложь;
	
	ЭлементУсловногоОформления.Оформление.УстановитьЗначениеПараметра("Видимость", Ложь);
	
	// Установка видимости колонки группы затрат затратных частей
	ЭлементУсловногоОформления = УсловноеОформление.Элементы.Добавить();
	...
	СписокЗначений.Добавить(Перечисления.ГруппировкиЗатратВПроизводствеБезЗаказа.ПоДокументу);
	СписокЗначений.Добавить(Перечисления.ГруппировкиЗатратВПроизводствеБезЗаказа.ЗатратыНеУказываются);
	ОтборЭлемента.ПравоеЗначение = СписокЗначений;
	
	ЭлементУсловногоОформления.Оформление.УстановитьЗначениеПараметра("Видимость", Ложь);
	
	// Спецификация обязательна только для группировки "По спецификациям"
	ЭлементУсловногоОформления = УсловноеОформление.Элементы.Добавить();
	...
	СписокЗначений.Добавить(Перечисления.ГруппировкиЗатратВПроизводствеБезЗаказа.ПоПродукцииИНазначениямБезСерий);
	СписокЗначений.Добавить(Перечисления.ГруппировкиЗатратВПроизводствеБезЗаказа.ПоСпецификациямИНазначениям);
	...
	СписокЗначений.Добавить(Перечисления.ГруппировкиЗатратВПроизводствеБезЗаказа.ПоПродукцииИНазначениям);
	СписокЗначений.Добавить(Перечисления.ГруппировкиЗатратВПроизводствеБезЗаказа.ПоПродукцииБезСерий);
	СписокЗначений.Добавить(Перечисления.ГруппировкиЗатратВПроизводствеБезЗаказа.ПоПродукцииИНазначениямБезСерий);
	ОтборЭлемента.ПравоеЗначение = СписокЗначений;
	...
	// "Внутренняя переработка" - недоступно в исправлении.
	Элемент = УсловноеОформление.Элементы.Добавить();
...
Процедура НастроитьЭлементыГруппировкиЗатрат(Инициализация)
	...
	// Настройка редактирования полей, влияющих на партии производства
	
	ТолькоПросмотрПолейПартий =
		Объект.Исправление
		И Объект.ИсправлениеСохраняетПартииПроизводства;
	
	Элементы.ГруппировкаЗатрат.ТолькоПросмотр       = ТолькоПросмотрПолейПартий;
	Элементы.Организация.ТолькоПросмотр             = ТолькоПросмотрПолейПартий;
	Элементы.НаправлениеДеятельности.ТолькоПросмотр = ТолькоПросмотрПолейПартий;
	Элементы.ВыпускПодДеятельность.ТолькоПросмотр   = ТолькоПросмотрПолейПартий;
	//++ НЕ УТКА
	Элементы.ВнутренняяПереработка.ТолькоПросмотр   = ТолькоПросмотрПолейПартий;
	//-- НЕ УТКА
	...
	Если Не ИспользоватьНазначения Тогда
		...
		ДоступныеЗначения.Добавить(Перечисления.ГруппировкиЗатратВПроизводствеБезЗаказа.ПоПродукцииБезСерий);
	...
	ПодсказкаГруппыВыходныеИзделия	= НСтр("ru = '%1, к которой относится изделие';
											|en = '%1 to which the product belongs'");
	...
	Элементы.ВыходныеИзделияГруппаЗатрат.Заголовок = ЗаголовокГруппыЗатрат;
	Элементы.ВыходныеИзделияЗаполнитьГруппуЗатрат.Видимость =
		Объект.Исправление
		И Объект.ИсправлениеСохраняетПартииПроизводства;
	Элементы.ВыходныеИзделияЗаполнитьГруппуЗатратРасширеннаяПодсказка.Заголовок = ПодсказкаГруппыВыходныеИзделия;
	
	ВидимостьКомандыЗаполнить =
		Не Объект.ГруппировкаЗатрат = Перечисления.ГруппировкиЗатратВПроизводствеБезЗаказа.ЗатратыНеУказываются
		И Не Объект.ГруппировкаЗатрат = Перечисления.ГруппировкиЗатратВПроизводствеБезЗаказа.ПоДокументу;
	
	Элементы.ПобочныеИзделияГруппаЗатрат.Заголовок = ЗаголовокГруппыЗатрат;
	Элементы.ПобочныеИзделияЗаполнитьГруппуЗатрат.Видимость = ВидимостьКомандыЗаполнить;
	Элементы.ПобочныеИзделияЗаполнитьГруппуЗатратРасширеннаяПодсказка.Заголовок = ПодсказкаГруппыПобочныеИзделия;
	
	Элементы.МатериалыИРаботыГруппаЗатрат.Заголовок = ЗаголовокГруппыЗатрат;
	Элементы.МатериалыИРаботыЗаполнитьГруппуЗатрат.Видимость = ВидимостьКомандыЗаполнить;
	Элементы.МатериалыИРаботыЗаполнитьГруппуЗатратРасширеннаяПодсказка.Заголовок = ПодсказкаГруппыМатериалыИРаботы;
	
	Элементы.ТрудозатратыГруппаЗатрат.Заголовок = ЗаголовокГруппыЗатрат;
	Элементы.ТрудозатратыЗаполнитьГруппуЗатрат.Видимость = ВидимостьКомандыЗаполнить;
	Элементы.ТрудозатратыЗаполнитьГруппуЗатратРасширеннаяПодсказка.Заголовок = ПодсказкаГруппыТрудозатраты;
...
Процедура НастройкаЗависимыхЭлементовФормы(Форма, СписокРеквизитов = "")
	...
	Если Инициализация
		...
		//-- НЕ УТКА
		Элементы.ГруппаВнутренняяПереработка.Видимость = ВидимостьЭлемента;
		...
		//-- НЕ УТКА
		Элементы.ОтображатьВнутреннююПереработку.Видимость = ВидимостьЭлемента;
	...
	Если Инициализация
		...
		Элементы.ОрганизацияДавалец.ТолькоПросмотр =
			Не Объект.ВнутренняяПереработка
			Или Объект.Исправление
				И Объект.ИсправлениеСохраняетПартииПроизводства;
	КонецЕсли;
...
Процедура ПослеИзмененияСтрокТЧПрограммноНаСервере(ИмяТЧ,
	...
	Если ИмяТЧ = "ВыходныеИзделия"
		И (Не Объект.Исправление
			Или Не Объект.ИсправлениеСохраняетПартииПроизводства) Тогда
		ОбновитьСписокГруппЗатрат();
	КонецЕсли;
	...
	Если ИмяТЧ = "ВыходныеИзделия"
		Или ИмяТЧ = "ПобочныеИзделия" Тогда
		НастройкаСчетовУчетаСервер.ОбновитьПредставлениеНастройки(ЭтотОбъект, "Объект." + ИмяТЧ, МассивИдентификаторовСтрок);
	КонецЕсли;
...
Процедура ДобавитьИзменитьГруппуЗатрат(Строка)
	...
	Если НомерГруппы = 0 Тогда
		...
		Если Объект.ГруппировкаЗатрат = Перечисления.ГруппировкиЗатратВПроизводствеБезЗаказа.ПоПродукцииИНазначениям
			ИЛИ Объект.ГруппировкаЗатрат = Перечисления.ГруппировкиЗатратВПроизводствеБезЗаказа.ПоПродукцииИНазначениямБезСерий
	...
	Если Не НомерГруппыДоИзменения = НомерГруппы Тогда
		...
		Если КоличествоИдентифицирующихСтрок(НомерГруппыДоИзменения) = 1 Тогда
			...
			Если Не НомерГруппыДоИзменения = 0
				Или СписокГрупп.Количество() = 1 Тогда
...
Процедура ОбновитьСписокГруппЗатрат()
	...
	Если Объект.Исправление
		И Объект.ИсправлениеСохраняетПартииПроизводства Тогда
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "&ТаблицаДанных", "Документ.ПроизводствоБезЗаказа.ВыходныеИзделия");
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "&Отбор", "ТаблицаДанных.Ссылка = &Ссылка");
		Запрос.УстановитьПараметр("Ссылка", Объект.ИсправляемыйДокумент);
	Иначе
...
Процедура ЗаполнитьПлановыеКалькуляцииИЦеныНаСервере(ИдентификаторыСтрок = Неопределено, Калькуляция = Неопределено)
	...
	НомераГруппЗатрат = Новый Массив();
	Для Каждого Идентификатор Из ИдентификаторыСтрок Цикл
		...
		НомераГруппЗатрат.Добавить(СтрокаТовара.НомерГруппыЗатрат);
	КонецЦикла;
	
	НомераГруппЗатрат = ОбщегоНазначенияКлиентСервер.СвернутьМассив(НомераГруппЗатрат);
	МассивСтрок = Новый Массив;
	Для Каждого ЗначНомерГруппыЗатрат Из НомераГруппЗатрат Цикл
		СтрокиТоваров = Объект.ВыходныеИзделия.НайтиСтроки(Новый Структура("НомерГруппыЗатрат", ЗначНомерГруппыЗатрат));
		ОбщегоНазначенияКлиентСервер.ДополнитьМассив(МассивСтрок, СтрокиТоваров);
	КонецЦикла;
...
Процедура АктуализироватьГруппыЗатратПослеИзмененияСпецификаций(Строки)
	...
	Если Не ЗначениеЗаполнено(Строки)
		Или Объект.Исправление
			И Объект.ИсправлениеСохраняетПартииПроизводства Тогда
		Возврат;
	КонецЕсли;
		...
		Если ИмяТЧ = "ВыходныеИзделия" Тогда
			...
			Если Не Объект.Исправление
				...
				НоваяСтрокаСтруктура =
					Новый Структура("НомерГруппыЗатрат, ГруппаЗатрат, Номенклатура, Характеристика, Спецификация, Назначение, Серия");
				//++ НЕ УТКА
				НоваяСтрокаСтруктура =
					Новый Структура("НомерГруппыЗатрат, ГруппаЗатрат, Номенклатура, Характеристика, Спецификация, Назначение, Серия, ВнутренняяПереработка, ОрганизацияДавалец");
				//-- НЕ УТКА
...
Процедура ОбработкаПодбораТоваров(АдресВХранилище)
	...
	ПродажиСерверЛокализация.ПеренестиШтрихкодыУпаковок(ЭтотОбъект, РезультатПодбора.ШтрихкодыУпаковок, "ШтрихкодыУпаковокМатериалы");
	ПослеИзмененияСтрокТЧПрограммноНаСервере(ИмяТЧ, МассивСтрок, СтруктураДействий);
