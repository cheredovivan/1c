...
//
// Параметры:
//	ДокументОбъект - ДокументОбъект.ПередачаТоваровМеждуОрганизациями, ДанныеФормыСтруктура - данные документа.
//
// Возвращаемое значение:
...
Функция ИспользуютсяПодакцизныеТовары(ДокументОбъект) Экспорт
	...
	Возврат ДокументОбъект.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.РеализацияТоваровВДругуюОрганизацию;
...
Функция ПолучитьДанныеПоАкцизам(ДокументСсылка) Экспорт
	...
	|		ИЗ
	|			Документ.ПередачаТоваровМеждуОрганизациями.Товары КАК Т
	|		ГДЕ
	...
	|	ЕСТЬNULL(ТаблицаПодакцизныеКатегории.КатегорияПодакцизногоТовара,
	|		ЗНАЧЕНИЕ(Справочник.КатегорииПодакцизныхТоваров.ПустаяСсылка)) КАК КатегорияПодакцизногоТовара,
	|	ВЫБОР КОГДА ТаблицаВидыЗапасов.ВидЗапасов.ОблагаетсяАкцизом
...
Функция ТекстЗапросаТаблицаВыручкаИСебестоимостьПродаж(Запрос, ТекстыЗапроса, Регистры)
	...
	//++ НЕ УТ
	|	СУММА(ТаблицаВидыЗапасов.СуммаАкцизаРегл) КАК СуммаАкцизаРегл,
	|	СУММА(ТаблицаВидыЗапасов.СуммаАкцизаУпр)  КАК СуммаАкцизаУпр,
	//-- НЕ УТ
	|
	|	ТаблицаВидыЗапасов.НалогообложениеНДС  КАК НалогообложениеНДС,
