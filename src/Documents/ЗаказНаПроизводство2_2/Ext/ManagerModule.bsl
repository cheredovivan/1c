...
Процедура ДобавитьКомандыОтчетов(КомандыОтчетов, Параметры) Экспорт
	...
	КомандаОтчета = Неопределено;
	
	Если ПолучитьФункциональнуюОпцию("ИспользоватьПланированиеПоМатериальнымРесурсам")
		ИЛИ ПолучитьФункциональнуюОпцию("ИспользоватьПланированиеПоПроизводственнымРесурсам") Тогда
		КомандаОтчета = Отчеты.ГрафикПроизводстваПродукции2_2.ДобавитьКомандуОтчета(КомандыОтчетов);
	ИначеЕсли ПолучитьФункциональнуюОпцию("ИспользуетсяНормативныйГрафикСтруктурыЗаказа") Тогда
		КомандаОтчета = Отчеты.НормативныйГрафикПроизводстваПоЗаказам.ДобавитьКомандуОтчета(КомандыОтчетов);
	КонецЕсли;
	...
	Отчеты.СравнениеПлановыхКалькуляций.ДобавитьКомандуОтчета(КомандыОтчетов);
	...
	Отчеты.КонтрольЗаказаНаПроизводство.ДобавитьКомандуОтчета(КомандыОтчетов);
...
// Возвращает текст запроса формирующий временную таблицу ВтТребуетсяСформироватьЭтапы.
//
// Параметры:
//   ВыборкаТипыЗаказов - ВыборкаИзРезультатаЗапроса
//
// Возвращаемое значение:
//  Строка - ТекстЗапроса
//
Функция ТекстЗапросаВТТребуетсяСформироватьЭтапы(ВыборкаТипыЗаказов) Экспорт
...
Процедура ОбновитьПланируемоеПоступлениеТоваров(
	...
	Запрос.Текст = 
		...
		|
		|ИМЕЮЩИЕ
		|	СУММА(СтруктураЗаказа.Требуется) > 0 ИЛИ СУММА(СтруктураЗаказа.Запланировано) > 0
		|;
		|
		|ВЫБРАТЬ
		|
		...
		|
		|	СУММА(ПродукцияЗаказа.Запланировано) КАК Количество
		|
		|ИЗ
		...
		|		ПО ГрафикПроизводства.ЗаказНаПроизводство = ПродукцияЗаказа.ЗаказНаПроизводство
		|		 И ГрафикПроизводства.КлючПартия          = ПродукцияЗаказа.КлючПартия
		...
		|	НАЧАЛОПЕРИОДА(ГрафикПроизводства.Окончание, ДЕНЬ)
		|
		|ИМЕЮЩИЕ
		|	СУММА(ПродукцияЗаказа.Запланировано) > 0 
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|ВЫБРАТЬ
		...
		|	СУММА(ПродукцияЗаказа.Требуется - ПродукцияЗаказа.Запланировано) КАК Количество
		|
		|ИЗ
		|	ПродукцияЗаказа КАК ПродукцияЗаказа
		|
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ЗаказНаПроизводство2_2.Продукция КАК ТабЧасть
		|		ПО ПродукцияЗаказа.ЗаказНаПроизводство = ТабЧасть.Ссылка
		|		 И ПродукцияЗаказа.Номенклатура        = ТабЧасть.Номенклатура
		|		 И ПродукцияЗаказа.КлючНоменклатура    = ТабЧасть.КлючНоменклатура
		|
		|СГРУППИРОВАТЬ ПО
		|	ПродукцияЗаказа.Номенклатура,
		|	ПродукцияЗаказа.Характеристика,
		...
		|
		|ИМЕЮЩИЕ
		|	СУММА(ПродукцияЗаказа.Требуется - ПродукцияЗаказа.Запланировано) > 0
		|
		|ИТОГИ ПО
		|	ЗаказНаПроизводство
		|";
...
Процедура СопоставитьЗагружаемыеДанные(АдресЗагружаемыхДанных, АдресТаблицыСопоставления, СписокНеоднозначностей, ПолноеИмяТабличнойЧасти, ДополнительныеПараметры) Экспорт
	...
	ТекстыЗапроса = Новый Массив;
	ТекстыЗапроса.Добавить(ЗагружаемыеДанныеТекстЗапросаВнешниеДанные());
	ТекстыЗапроса.Добавить(ЗагружаемыеДанныеТекстЗапросаВтДанныеШтрихкодов());
	ТекстыЗапроса.Добавить(ЗагружаемыеДанныеТекстЗапросаВтДанныеНоменклатуры());
	ТекстыЗапроса.Добавить(ЗагружаемыеДанныеТекстЗапросаВтДанныеХарактеристик());
	ТекстыЗапроса.Добавить(ЗагружаемыеДанныеТекстЗапросаВтДанныеУпаковок());
	ТекстыЗапроса.Добавить(ЗагружаемыеДанныеТекстЗапросаВтДанныеСкладов());
	ТекстыЗапроса.Добавить(ЗагружаемыеДанныеТекстЗапросаВтДанныеПодразделений());
	Если ДополнительныеПараметры.ИспользуютсяНазначения Тогда
		ТекстыЗапроса.Добавить(ЗагружаемыеДанныеТекстЗапросаВтДанныеНазначений());
	КонецЕсли;
	Если ДополнительныеПараметры.ИспользуютсяСпецификации Тогда
		ТекстыЗапроса.Добавить(ЗагружаемыеДанныеТекстЗапросаВтДанныеСпецификаций());
	КонецЕсли;
	ТекстыЗапроса.Добавить(ЗагружаемыеДанныеТекстЗапросаИтоговаяВыборка());
	
	Запрос = ЗагружаемыеДанныеСоздатьЗапрос(ТекстыЗапроса, ЗагружаемыеДанные, ДополнительныеПараметры);
	...
	Для Каждого Строка Из Результат Цикл
		...
		КонецЕсли;
		ЗагружаемыеДанныеСоздатьЗаписиОНеоднозначности(СписокНеоднозначностей, Строка, ДополнительныеПараметры);
	КонецЦикла;
...
Процедура ЗагружаемыеДанныеСоздатьЗаписиОНеоднозначности(СписокНеоднозначностей, Строка, ДополнительныеПараметры)
		...
		Если Строка.КоличествоСкладов > 1 Тогда
			ЗаписьОНеоднозначности               = СписокНеоднозначностей.Добавить();
			ЗаписьОНеоднозначности.Идентификатор = Строка.Идентификатор;
			ЗаписьОНеоднозначности.Колонка       = "Склад";
		КонецЕсли;
		
		Если Строка.КоличествоПодразделений > 1 Тогда
			ЗаписьОНеоднозначности               = СписокНеоднозначностей.Добавить();
			ЗаписьОНеоднозначности.Идентификатор = Строка.Идентификатор;
...
//  ДополнительныеПараметры - см. ПланированиеКлиент.ДополнительныеПараметрыЗагрузкиИзФайла
Процедура ЗаполнитьСписокНеоднозначностей(ИмяТабличнойЧасти, СписокНеоднозначностей, ИмяКолонки, ЗагружаемыеЗначенияСтрока, ДополнительныеПараметры) Экспорт
	...
	Если ИмяКолонки = "Подразделение" Тогда
		...
		|ГДЕ
		|	НЕ &Партнер = """"
		|	И СтруктураПредприятия.Наименование = &Подразделение
		|";
...
// * ДатаПотребностиИспользуется - Булево -
// * ДоступныеТипыНоменклатуры - Массив из ПеречислениеСсылка.ТипыНоменклатуры -
// 
// Возвращаемое значение:
//  Запрос
Функция ЗагружаемыеДанныеСоздатьЗапрос(ТекстыЗапроса, ЗагружаемыеДанные, ДополнительныеПараметры)
	
	ТекстЗапроса = СтрСоединить(ТекстыЗапроса, ОбщегоНазначения.РазделительПакетаЗапросов());
	
	ПолеКоличество = "ЗагружаемыеДанные.Количество КАК Количество,
					 |ЗагружаемыеДанные.Количество КАК КоличествоУпаковок,";
	
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&ЗагружаемыеДанныеКоличествоУпаковок,", ПолеКоличество);

	Если ДополнительныеПараметры.ИспользуютсяНазначения Тогда
		
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&ЗагружаемыеДанныеНазначение,", "ЗагружаемыеДанные.Назначение КАК Назначение,");
		...
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&КоличествоНазначений,", "ЕСТЬNULL(ДанныеНазначений.КоличествоНазначений, 0) КАК КоличествоНазначений,");
	...
	Если ДополнительныеПараметры.ИспользуютсяСпецификации Тогда
		...
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&ЗагружаемыеДанныеСпецификация,", "ЗагружаемыеДанные.Спецификация КАК Спецификация,");
		
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&Спецификация,", "
			|	ВЫБОР
		...
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&КоличествоСпецификаций,", "ЕСТЬNULL(ДанныеСпецификаций.КоличествоСпецификаций, 0) КАК КоличествоСпецификаций,");
	...
	Если ДополнительныеПараметры.ИспользуетсяДатаПотребности Тогда
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&ЗагружаемыеДанныеДаты,", "ЗагружаемыеДанные.ДатаПотребности КАК ДатаПотребности,");
	Иначе
...
// Возвращаемое значение:
//  Строка
Функция ЗагружаемыеДанныеТекстЗапросаВнешниеДанные()
	...
	|	ЗагружаемыеДанные.Получатель          КАК Получатель,
...
Функция ЗагружаемыеДанныеТекстЗапросаВтДанныеШтрихкодов()
...
Функция ЗагружаемыеДанныеТекстЗапросаВтДанныеНоменклатуры()
...
Функция ЗагружаемыеДанныеТекстЗапросаВтДанныеХарактеристик()
...
Функция ЗагружаемыеДанныеТекстЗапросаВтДанныеНазначений()
	ТекстЗапроса = "
	|ВЫБРАТЬ
...
Функция ЗагружаемыеДанныеТекстЗапросаВтДанныеУпаковок()
...
Функция ЗагружаемыеДанныеТекстЗапросаВтДанныеСпецификаций()
	ТекстЗапроса = "
	|ВЫБРАТЬ
...
Функция ЗагружаемыеДанныеТекстЗапросаВтДанныеСкладов()
	ТекстЗапроса = "
	|ВЫБРАТЬ
	...
	|		ПО Склады.Наименование = ЗагружаемыеДанные.Получатель
	|
	|	ГДЕ
	|		НЕ ЗагружаемыеДанные.Получатель = """"
	|	) КАК Склады
	|
	|СГРУППИРОВАТЬ ПО
...
// Возвращаемое значение:
//  Строка - Текст запроса вт данные подразделений
Функция ЗагружаемыеДанныеТекстЗапросаВтДанныеПодразделений()
	ТекстЗапроса = "
	|ВЫБРАТЬ
	...
	|		ПО СтруктураПредприятия.Наименование = ЗагружаемыеДанные.Получатель
	|
	|	ГДЕ
	|		НЕ ЗагружаемыеДанные.Получатель = """"
	|	) КАК Подразделения
...
Функция ЗагружаемыеДанныеТекстЗапросаИтоговаяВыборка()
	...
	|	ВЫБОР
	|		КОГДА ЕСТЬNULL(ДанныеНоменклатуры.КоличествоНоменклатуры, 0) = 1
	|			ТОГДА ДанныеНоменклатуры.Номенклатура.ТипНоменклатуры = ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.Работа)
	|		ИНАЧЕ ЛОЖЬ
	|	КОНЕЦ                                                    КАК ЭтоРабота,
	|	ВЫБОР
	...
	|	ЕСТЬNULL(ДанныеСкладов.КоличествоСкладов, 0)             КАК КоличествоСкладов,
	|	ЕСТЬNULL(ДанныеПодразделений.КоличествоПодразделений, 0)       КАК КоличествоПодразделений,
