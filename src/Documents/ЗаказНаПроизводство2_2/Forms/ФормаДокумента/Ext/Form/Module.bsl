...
Процедура ЗагрузитьИзВнешнегоФайла(Команда)
	...
	ДополнительныеПараметры = ДополнительныеПараметрыЗагрузкиИзФайла();
...
Процедура ДиспетчированиеЭтапов(Команда)
	...
	ПараметрыФормы = Новый Структура;
	ПараметрыФормы.Вставить("СтруктураОтборов", Новый Структура("Распоряжение", Объект.Ссылка));
	
	ОткрытьФорму("Документ.ЭтапПроизводства2_2.Форма.Диспетчирование", ПараметрыФормы, ЭтотОбъект, Истина);
...
&НаКлиенте
Функция ДополнительныеПараметрыЗагрузкиИзФайла()
	
	ДополнительныеПараметры = Новый Структура;
	
	ИспользуютсяСпецификации = Не Объект.ТипПроизводственногоПроцесса = ПредопределенноеЗначение("Перечисление.ТипыПроизводственныхПроцессов.БезСпецификаций");
	ДополнительныеПараметры.Вставить("ИспользуютсяСпецификации", ИспользуютсяСпецификации);
	
	ДополнительныеПараметры.Вставить("ИспользуютсяНазначения", Не НазначениеУказываетсяВШапкеДокумента(ЭтотОбъект));
	
	ДополнительныеПараметры.Вставить("ИспользуетсяДатаПотребности", Объект.ДатаПотребностиУказываетсяВСтроках);
	
	ДоступныеТипыНоменклатуры = Новый Массив;
	ДоступныеТипыНоменклатуры.Добавить(ПредопределенноеЗначение("Перечисление.ТипыНоменклатуры.Товар"));
	Если Не Объект.ТипПроизводственногоПроцесса = ПредопределенноеЗначение("Перечисление.ТипыПроизводственныхПроцессов.Разборка") Тогда
		ДоступныеТипыНоменклатуры.Добавить(ПредопределенноеЗначение("Перечисление.ТипыНоменклатуры.МногооборотнаяТара"));
...
Процедура ПоказатьФормуЗагрузкиИзФайла(ДополнительныеПараметры)
	...
	ПараметрыЗагрузки.ПолноеИмяТабличнойЧасти = "ЗаказНаПроизводство2_2.Продукция";
	...
	ПараметрыЗагрузки.КолонкиМакета = ОписаниеКолонокМакетаДляЗагрузкиНоменклатуры(ДополнительныеПараметры);
...
&НаСервереБезКонтекста
// Создает массив колонок макета загрузки данных
// Параметры:
//	ДополнительныеПараметры - Структура - регулирует видимость колонок:
//                         * ИспользуютсяНазначения      - Булево
//                         * ИспользуютсяСпецификации    - Булево
//                         * ИспользуетсяДатаПотребности - Булево 
// 
// Возвращаемое значение:
//  Массив из Структура - Описание колонок макета для загрузки номенклатуры
Функция ОписаниеКолонокМакетаДляЗагрузкиНоменклатуры(ДополнительныеПараметры) Экспорт
	
	КолонкиМакета = Новый Массив;
	
	ДобавитьКолонкуЗагрузкиДанных(КолонкиМакета, "Штрихкод", ОбщегоНазначения.ОписаниеТипаСтрока(200), НСтр("ru = 'Штрихкод';
																											|en = 'Barcode'"), 10, "Номенклатура");
	ДобавитьКолонкуЗагрузкиДанных(КолонкиМакета, "Артикул", Справочники.Номенклатура.ПустаяСсылка().Метаданные().Реквизиты["Артикул"].Тип, НСтр("ru = 'Артикул';
																																				|en = 'Item ID'"), 10, "Номенклатура");
	ДлинаКода = Справочники.Номенклатура.ПустаяСсылка().Метаданные().ДлинаКода;
	ДобавитьКолонкуЗагрузкиДанных(КолонкиМакета, "КодНоменклатуры", ОбщегоНазначения.ОписаниеТипаСтрока(ДлинаКода), НСтр("ru = 'Код номенклатуры';
																														|en = 'Items code'"), 15, "Номенклатура");
	ДобавитьКолонкуЗагрузкиДанных(КолонкиМакета, "Номенклатура", ТипНаименованияСправочника("Номенклатура"), НСтр("ru = 'Номенклатура';
																													|en = 'Items'"), 15, "Номенклатура");
	ДобавитьКолонкуЗагрузкиДанных(КолонкиМакета, "Характеристика", ТипНаименованияСправочника("ХарактеристикиНоменклатуры"), НСтр("ru = 'Характеристика';
																																	|en = 'Variant'"), 15);
	Если ДополнительныеПараметры.ИспользуютсяНазначения Тогда
		ДобавитьКолонкуЗагрузкиДанных(КолонкиМакета, "Назначение", ТипНаименованияСправочника("Назначения"), НСтр("ru = 'Назначение';
																													|en = 'Assignment'"), 15);
	КонецЕсли;
	ДлинаКода = Справочники.УпаковкиЕдиницыИзмерения.ПустаяСсылка().Метаданные().ДлинаКода;
	ДобавитьКолонкуЗагрузкиДанных(КолонкиМакета, "КодУпаковки", ОбщегоНазначения.ОписаниеТипаСтрока(ДлинаКода), НСтр("ru = 'Код упаковки';
																													|en = 'Packaging unit code'"), 12);
	ДобавитьКолонкуЗагрузкиДанных(КолонкиМакета, "Упаковка", ТипНаименованияСправочника("УпаковкиЕдиницыИзмерения"), НСтр("ru = 'Упаковка';
																															|en = 'Packaging unit'"), 10);
	ДобавитьКолонкуЗагрузкиДанных(КолонкиМакета, "Количество", ОбщегоНазначения.ОписаниеТипаЧисло(10, 2), НСтр("ru = 'Количество';
																												|en = 'Quantity'"), 10);
	Если ДополнительныеПараметры.ИспользуютсяСпецификации Тогда
		ДобавитьКолонкуЗагрузкиДанных(КолонкиМакета, "Спецификация", ТипНаименованияСправочника("РесурсныеСпецификации"), НСтр("ru = 'Спецификация';
																																|en = 'Bill of materials'"), 15);
	КонецЕсли;
	ДобавитьКолонкуЗагрузкиДанных(КолонкиМакета, "Получатель", ТипНаименованияСправочника("СтруктураПредприятия"), НСтр("ru = 'Получатель';
																														|en = 'Recipient'"), 15);
	Если ДополнительныеПараметры.ИспользуетсяДатаПотребности Тогда
		ДобавитьКолонкуЗагрузкиДанных(КолонкиМакета, "ДатаПотребности", Новый ОписаниеТипов("Дата",, Новый КвалификаторыДаты()), НСтр("ru = 'Желаемая дата выпуска';
																																		|en = 'Requested completion date'"), 15);
	КонецЕсли;
	ДобавитьКолонкуЗагрузкиДанных(КолонкиМакета, "Комментарий", ОбщегоНазначения.ОписаниеТипаСтрока(0), НСтр("ru = 'Комментарий';
																											|en = 'Comment'"), 20);
...
&НаСервереБезКонтекста
Функция ТипНаименованияСправочника(ИмяСправочника)
	ДлинаНаименования = Справочники[ИмяСправочника].ПустаяСсылка().Метаданные().ДлинаНаименования;
	Возврат Новый ОписаниеТипов("Строка",, Новый КвалификаторыСтроки(ДлинаНаименования));
КонецФункции

&НаСервереБезКонтекста
Процедура ДобавитьКолонкуЗагрузкиДанных(КолонкиМакета, Наименование, ТипКолонки, Синоним, ШиринаКолонки, Родитель = "")
	Колонка = ЗагрузкаДанныхИзФайлаКлиентСервер.ОписаниеКолонкиМакета(Наименование, ТипКолонки, Синоним, ШиринаКолонки);
	Колонка.Позиция = КолонкиМакета.ВГраница() + 1;
	Если ЗначениеЗаполнено(Родитель) Тогда
		Колонка.Родитель = Родитель;
	КонецЕсли;
...
Процедура ЗагрузитьИзФайлаНаСервере(АдресЗагруженныхДанных, ДополнительныеПараметры)
	...
	КэшированныеЗначения = ПакетнаяОбработкаТабличнойЧастиКлиентСервер.ПолучитьСтруктуруКэшируемыеЗначения();
	ЗагружаемыеДанныеДобавитьТовары(ЗагруженныеДанные, КэшированныеЗначения);
	
КонецПроцедуры

&НаСервере
Процедура ЗагружаемыеДанныеДобавитьТовары(ТаблицаТоваров, КэшированныеЗначения = Неопределено)
