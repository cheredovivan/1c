...
Процедура ОбработкаЗаполнения(ДанныеЗаполнения, СтандартнаяОбработка)
	...
	Если ТипЗнч(ДанныеЗаполнения) = Тип("ДокументСсылка.РеализацияТоваровУслуг")
		...
		ТекстСообщения = НСтр("ru = 'Для документа реализации с операцией ""Переход права собственности"" не требуется оформление счета-фактуры';
								|en = 'For the sales document with the ""Transfer of title to goods"" transaction, registering a tax invoice is not required'");
...
Процедура ПроверитьСуммуЗачтенныхАвансов(Отказ)
	...
	Если СуммаЗачтенныхАвансовВалютная > 0
		...
		ТекстОшибки = СтрШаблон(НСтр("ru = 'Сумма зачтенных авансов %1 %2 превышает сумму по счету-фактуре %3 %2.';
									|en = 'The offset advance amount %1 %2 exceeds the invoice amount %3 %2.'", ОбщегоНазначения.КодОсновногоЯзыка()), СуммаЗачтенныхАвансовВалютная, ВалютаВзаиморасчетов, СуммаПоСчетуФактуреВалютная);
