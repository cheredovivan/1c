...
Процедура ОблагаетсяАкцизомПриИзменении(Элемент)
	//++ НЕ УТ
	Элементы.ПодразделениеПроизводства.ТолькоПросмотр = Не Объект.ОблагаетсяАкцизом;
...
Процедура ОрганизацияПриИзмененииСервер()
	...
	АктуализироватьСборкуПодДеятельность();
	//++ НЕ УТ
	ОбновитьЭлементыУчетаПодакцизныхТоваров();
	//-- НЕ УТ
...
Процедура УправлениеЭлементамиФормы(ЕстьИзменениеНоменклатуры = Ложь)
	...
	//++ НЕ УТ
	ОбновитьЭлементыУчетаПодакцизныхТоваров(ЕстьИзменениеНоменклатуры);
	
	Если ЭтоРазборка Тогда
		Элементы.ОблагаетсяАкцизом.Заголовок =
			НСтр("ru = 'Подакцизный разбираемый комплект произведен организацией и облагается акцизом';
				|en = 'Excisable disassembled kit is produced by the company and is subject to excise'");
	Иначе
		Элементы.ОблагаетсяАкцизом.Заголовок =
			НСтр("ru = 'Подакцизный комплект произведен организацией и облагается акцизом';
				|en = 'Excisable kit is produced by the company and is subject to excise'");
	КонецЕсли;
	//-- НЕ УТ
...
Функция ПоместитьТоварыИВидыЗапасовВХранилище()
	...
	//++ НЕ УТ
	Если Документы.СборкаТоваров.ИспользуютсяПодакцизныеТовары(Объект) Тогда
...
&НаСервере
Процедура ОбновитьЭлементыУчетаПодакцизныхТоваров(ОбновлятьДанные = Истина)
	
	ДоступноОприходованиеПодакцизногоЗапаса =
		УчетПодакцизныхТоваров.ЭтоПодакцизныйТовар(Объект.Номенклатура, Объект.Организация, Объект.Дата)
		И УчетПодакцизныхТоваров.ИспользуетсяПроизводствоПодакцизныхТоваров(Объект.Организация, Объект.Дата);
	Элементы.ОблагаетсяАкцизом.ТолькоПросмотр = Не ДоступноОприходованиеПодакцизногоЗапаса;
	Элементы.ПодразделениеПроизводства.ТолькоПросмотр = Не ДоступноОприходованиеПодакцизногоЗапаса
		Или Не Объект.ОблагаетсяАкцизом;
	
	Если ОбновлятьДанные И Не ДоступноОприходованиеПодакцизногоЗапаса Тогда
