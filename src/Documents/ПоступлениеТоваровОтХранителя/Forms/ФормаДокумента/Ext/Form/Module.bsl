...
Процедура ПослеЗаписиНаСервере(ТекущийОбъект, ПараметрыЗаписи)
	...
	//++ НЕ УТ
	Обработчик.ЗаполнитьПризнакЦеховаяКладоваяВТаблице(Объект);
	//-- НЕ УТ
...
Процедура ДатаПриИзменении(Элемент)
	...
	Если Объект.ХозяйственнаяОперация = ПредопределенноеЗначение("Перечисление.ХозяйственныеОперации.ПоступлениеОтПереработчика2_5") Тогда
		ДатаПриИзмененииЗавершение(Ложь, Неопределено);
	Иначе
	//-- НЕ УТ
			...
			Если Объект.ХозяйственнаяОперация = ПредопределенноеЗначение("Перечисление.ХозяйственныеОперации.ПоступлениеОтПереработчика2_5") Тогда
				...
				ТекстВопроса =
					НСтр("ru = 'При выполнении операции будет очищена информация о качестве поступающих товаров. Продолжить?';
						|en = 'Item quality will be cleared. Continue?'");
...
Процедура ПолучитьСтрокиИзБуфераОбмена()
	...
	Если Объект.ХозяйственнаяОперация = ПредопределенноеЗначение("Перечисление.ХозяйственныеОперации.ПоступлениеОтПереработчика2_5") Тогда
		Колонки =
			"Номенклатура,Характеристика,Упаковка,КоличествоУпаковок,НомерГТД,
			|НоменклатураНабора,ХарактеристикаНабора,Порча,НоменклатураОприходование,ХарактеристикаОприходование";
	КонецЕсли;
	//-- НЕ УТ
...
Процедура ОбработкаВыбораПодборНаСервере(ВыбранноеЗначение)
	...
	Если Объект.ХозяйственнаяОперация = ПредопределенноеЗначение("Перечисление.ХозяйственныеОперации.ПоступлениеОтПереработчика2_5") Тогда
		СписокСвойств =
			"НоменклатураНабора, Номенклатура, Характеристика, ХарактеристикаНабора, Упаковка, Серия, КоличествоУпаковок";
	КонецЕсли;
	//-- НЕ УТ
...
Процедура НастроитьФормуПоПорче()
	...
	КомандаВозвратПорчиЗаголовок = НСтр("ru = 'Возврат товаров другого качества';
										|en = 'Return nonconforming products'");
	ОтгруженныйТоварЗаголовок = НСтр("ru = 'Ранее был отгружен товар';
									|en = 'Goods were unloaded earlier'");
	ВозвращаемыйТоварЗаголовок = НСтр("ru = 'Возвращается товар другого качества';
										|en = 'Goods of other quality are returned'");
	
	//++ НЕ УТ
	Если Объект.ХозяйственнаяОперация =
		Перечисления.ХозяйственныеОперации.ПоступлениеОтПереработчика2_5 Тогда
		
		КомандаВозвратПорчиЗаголовок = НСтр("ru = 'Поступление товаров другого качества';
											|en = 'Nonconforming product receipt'");
		ОтгруженныйТоварЗаголовок = НСтр("ru = 'Ожидаемый товар';
										|en = 'Expected item'");
		ВозвращаемыйТоварЗаголовок = НСтр("ru = 'Поступает товар другого качества';
											|en = 'Nonconforming product is received'");
...
Процедура УстановитьСвойстваЭлементов()
	...
	//++ НЕ УТ
	Если Не Объект.ХозяйственнаяОперация = ПредопределенноеЗначение("Перечисление.ХозяйственныеОперации.ПоступлениеОтПереработчика2_5")
		И Не Объект.ХозяйственнаяОперация = ПредопределенноеЗначение("Перечисление.ХозяйственныеОперации.ВозвратОтПереработчика2_5") Тогда
	//-- НЕ УТ
