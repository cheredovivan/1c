...
Функция РаспределитьНачальныеОстаткиПоПартиямПроизводства(Ссылка, АдресРезультата = Неопределено) Экспорт
	...
	Запрос.Текст = СтрСоединить(ТекстыЗапросаРаспределенияЗатрат(РеквизитыДокумента.Дата), ОбщегоНазначения.РазделительПакетаЗапросов());
...
Функция ОшибкиВДанныхРаспределенияОстатковНЗП(Ссылка) Экспорт
	...
	Запрос = Новый Запрос;
	Запрос.Текст = СтрСоединить(ТекстыЗапросаОшибкиВДанныхРаспределенияОстатковНЗП(РеквизитыДокумента.Дата), ОбщегоНазначения.РазделительПакетаЗапросов());
...
Функция ТекстыЗапросаРаспределенияЗатрат(ДатаЦен)
	
	ИспользуетсяЦенообразование25 = ЦенообразованиеВызовСервера.ИспользуетсяЦенообразование25(ДатаЦен);
	...
	ТекстыЗапроса.Добавить(ТекстЗапросаТаблицаМатериалыЭтаповПодготовительная(ИспользуетсяЦенообразование25));
	Если ИспользуетсяЦенообразование25 Тогда
		ТекстыЗапроса.Добавить(ТекстЗапросаТаблицаМатериалыЭтаповПодготовительнаяЦО25());
	КонецЕсли;
	ТекстыЗапроса.Добавить(ТекстЗапросаТаблицаВтМатериалыЭтапов(ИспользуетсяЦенообразование25));
...
Функция ТекстыЗапросаОшибкиВДанныхРаспределенияОстатковНЗП(ДатаЦен)
...
Функция ТекстЗапросаТаблицаМатериалыЭтаповПодготовительная(ИспользуетсяЦенообразование25 = Ложь)
	...
	Если ИспользуетсяЦенообразование25 Тогда
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "МатериалыЭтаповПодготовительная", "МатериалыЭтаповБазовая");
	КонецЕсли;
...
Функция ТекстЗапросаТаблицаМатериалыЭтаповПодготовительнаяЦО25()
	...
	|			ТОГДА ЗНАЧЕНИЕ(Справочник.ХарактеристикиНоменклатурыДляЦенообразования.ПустаяСсылка)
	|		ИНАЧЕ ЕСТЬNULL(ХарактеристикиНоменклатуры.ХарактеристикаНоменклатурыДляЦенообразования, ЗНАЧЕНИЕ(Справочник.ХарактеристикиНоменклатурыДляЦенообразования.ПустаяСсылка))
	...
	|		КОГДА ТаблицаМатериалы.Серия = ЗНАЧЕНИЕ(Справочник.СерииНоменклатуры.ПустаяСсылка)
	|			ТОГДА ЗНАЧЕНИЕ(Справочник.СерииНоменклатурыДляЦенообразования.ПустаяСсылка)
	|		ИНАЧЕ ЕСТЬNULL(ТаблицаМатериалы.Серия.СерияНоменклатурыДляЦенообразования, ЗНАЧЕНИЕ(Справочник.СерииНоменклатурыДляЦенообразования.ПустаяСсылка))
	|	КОНЕЦ КАК СерияЦО,
	|	ТаблицаМатериалы.СтатьяКалькуляции КАК СтатьяКалькуляции,
	|	ТаблицаМатериалы.Назначение КАК Назначение,
	...
	|ПОМЕСТИТЬ МатериалыЭтаповПодготовительная
	|ИЗ
	|	МатериалыЭтаповБазовая КАК ТаблицаМатериалы
	|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.Номенклатура КАК СправочникНоменклатура
	|		ПО ТаблицаМатериалы.Номенклатура = СправочникНоменклатура.Ссылка
	...
	|			И (СправочникНоменклатура.ИспользованиеХарактеристик <> ЗНАЧЕНИЕ(Перечисление.ВариантыИспользованияХарактеристикНоменклатуры.НеИспользовать))
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	Номенклатура,
	|	ХарактеристикаЦО,
...
Функция ТекстЗапросаТаблицаВтМатериалыЭтапов(ИспользуетсяЦенообразование25 = Ложь)
	...
	|
	|	ЛЕВОЕ СОЕДИНЕНИЕ ЦеныНоменклатуры КАК ЦеныНоменклатуры ПО ИСТИНА
	|
	...
	Если ИспользуетсяЦенообразование25 Тогда
		ТекстСоединенияСЦенами =
		"ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ЦеныНоменклатуры25.СрезПоследних(КОНЕЦПЕРИОДА(&ДатаОстатков, МЕСЯЦ), (Номенклатура, ХарактеристикаЦО,
		...
		|			&ВидЦены
		|		ИЗ
		|			МатериалыЭтаповПодготовительная КАК Т)) КАК ЦеныНоменклатуры
		|	ПО ЦеныНоменклатуры.Номенклатура = ТаблицаМатериалы.Номенклатура
		|	И ЦеныНоменклатуры.ХарактеристикаЦО = ТаблицаМатериалы.ХарактеристикаЦО
		|	И ЦеныНоменклатуры.СерияЦО = ТаблицаМатериалы.СерияЦО
		|	И ЦеныНоменклатуры.УпаковкаЦО = ТаблицаМатериалы.Упаковка";
	Иначе
		ТекстСоединенияСЦенами =
	...
	КонецЕсли;
	ПодстрокаЗамены = "ЛЕВОЕ СОЕДИНЕНИЕ ЦеныНоменклатуры КАК ЦеныНоменклатуры ПО ИСТИНА";
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса, ПодстрокаЗамены, ТекстСоединенияСЦенами);
...
Функция ТекстЗапросаТаблицаВтТрудозатратыЭтапов()
	...
	|	МАКСИМУМ(ВЫБОР
	|				КОГДА НЕ ЦеныВидовРабот.Расценка ЕСТЬ NULL
	|					ТОГДА ЦеныВидовРабот.Расценка
	//++ НЕ УТКА
	
	//++ Локализация
	|				КОГДА НЕ ТарифныеСтавки.Тариф ЕСТЬ NULL
	|					ТОГДА ТарифныеСтавки.Тариф
	|				КОГДА НЕ ТарифныеСтавкиОбщие.Тариф ЕСТЬ NULL
	|					ТОГДА ТарифныеСтавкиОбщие.Тариф
	//-- Локализация
	...
	|	ПО ТаблицаТрудозатраты.ВидРабот = ВидыРабот.Ссылка
	|	
	...
	//++ Локализация
	|	ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ТарифныеСтавки.СрезПоследних(КОНЕЦПЕРИОДА(&ДатаОстатков, МЕСЯЦ),
	...
	|	ПО &НормированиеИРасценкаРаботПоТарифнымСеткам
	|	И ВидыРабот.РасценкаПоТарифнойСетке
	|	И ВидыРабот.Должность = ТарифныеСтавки.Должность
	|	И ВидыРабот.КвалификационныйРазряд = ТарифныеСтавки.Разряд
	|	И (ТаблицаТрудозатраты.Подразделение = ТарифныеСтавки.Подразделение И ТарифныеСтавки.Тариф <> 0
	|	ИЛИ &Организация = ТарифныеСтавки.Организация И ТарифныеСтавки.Тариф <> 0)
	|	
	|	ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ТарифныеСтавки.СрезПоследних(КОНЕЦПЕРИОДА(&ДатаОстатков, МЕСЯЦ), Подразделение = ЗНАЧЕНИЕ(Справочник.СтруктураПредприятия.ПустаяСсылка)
	|		И Организация = ЗНАЧЕНИЕ(Справочник.Организации.ПустаяСсылка)) КАК ТарифныеСтавкиОбщие
	|	ПО &НормированиеИРасценкаРаботПоТарифнымСеткам
	|	И ВидыРабот.РасценкаПоТарифнойСетке
	|	И ВидыРабот.Должность = ТарифныеСтавкиОбщие.Должность
	|	И ВидыРабот.КвалификационныйРазряд = ТарифныеСтавкиОбщие.Разряд
