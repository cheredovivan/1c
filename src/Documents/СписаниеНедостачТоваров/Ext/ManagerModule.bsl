...
Функция ПолучитьДанныеПоАкцизам(ДокументСсылка) Экспорт
	...
	|	ВЫБОР
	|		КОГДА ВтОснований.КоличествоНаправленийДеятельности = 1
	|			ТОГДА ВтОснований.НаправлениеДеятельности
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВтОснований КАК ВтОснований
	|		ПО (ВтОснований.Ссылка = &Ссылка)
	|ГДЕ
...
//
// Возвращаемое значение:
//	РезультатЗапроса - данные по прочим активам и пассивам.
//
...
Функция ТекстЗапросаВтВидыЗапасов(Запрос, ТекстыЗапроса)
	...
	//++ НЕ УТ
	|	Строки.СуммаАкциза                             КАК СуммаАкцизаРегл,
	|	ВЫРАЗИТЬ(Строки.СуммаАкциза * &КоэффициентПересчетаИзРеглВУпр КАК ЧИСЛО(31, 2)) КАК СуммаАкцизаУпр,
	//-- НЕ УТ
...
Функция ТекстЗапросаТаблицаДвиженияПоПрочимАктивамПассивам(Запрос, ТекстыЗапроса, Регистры)
	...
	|
	|ИМЕЮЩИЕ
	|	СУММА(ТаблицаТовары.СуммаАкцизаРегл) <> 0
	|		ИЛИ СУММА(ТаблицаТовары.СуммаАкцизаУпр) <> 0
	|";
...
Функция ТекстЗапросаТаблицаВтИсходныеПрочиеРасходы(Запрос, ТекстыЗапроса)
	...
	|	0 КАК ВременнаяРазница,
	|	&ХозяйственнаяОперация КАК ХозяйственнаяОперация,
	|	ТаблицаРасходы.АналитикаУчетаНоменклатуры КАК АналитикаУчетаНоменклатуры,
	|	
	|	&ИдентификаторДокумента         КАК ИдентификаторФинЗаписи,
	|	&НастройкаХозяйственнойОперации КАК НастройкаХозяйственнойОперации
	|	
	...
	|	ТаблицаРасходы.АналитикаУчетаНоменклатуры
	//-- НЕ УТ
	|";
...
Функция ОписаниеОперацийУчетаСебестоимости(Документ) Экспорт
	...
	//++ НЕ УТ
	|		И ПрочиеРасходы.АналитикаУчетаНоменклатуры = ТаблицаВидыЗапасов.АналитикаУчетаНоменклатуры
	//-- НЕ УТ
	|ГДЕ
	|	ТаблицаДокумента.Ссылка В (&Ссылка)
	|	И ТаблицаВидыЗапасов.ТипЗапасов <> ЗНАЧЕНИЕ(Перечисление.ТипыЗапасов.ТоварНаХраненииСПравомПродажи)
...
Функция ЗаполнитьИсходныеПараметры(ДокументСсылка, ТекстыЗапроса)
	...
	|				ИЗ
	|					Документ.СписаниеНедостачТоваров.Товары КАК Т
	|				ГДЕ
	...
	|	ВЫБОР
	|		КОГДА Строки.ВидЗапасов.ОблагаетсяАкцизом
	|			ТОГДА ЕСТЬNULL(ТаблицаПодакцизныеКатегории.КоэффициентПересчета, 0) * Строки.Количество
	|		ИНАЧЕ 0
	|	КОНЕЦ КАК КоличествоАкциза,
	|	Строки.ВидЗапасов.ПодразделениеПроизводства КАК ПодразделениеПроизводства,
	|	Строки.ИдентификаторСтроки КАК ИдентификаторСтроки,
	|	Строки.АналитикаУчетаНоменклатуры КАК АналитикаУчетаНоменклатуры
	|ПОМЕСТИТЬ ВтВидыЗапасов
	|ИЗ
	...
	|			ПО Аналитика.Номенклатура = ТаблицаПодакцизныеКатегории.Номенклатура
	|ГДЕ
	|	Строки.Ссылка = &Ссылка
