...
Процедура ОперацииРегистрВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	Если Поле.Имя = "ОперацииРегистрОперацияНаименование"
		...
		Если ЗначениеЗаполнено(Элемент.ТекущиеДанные.Операция) Тогда
			ПоказатьЗначение(, Элемент.ТекущиеДанные.Операция);
		ИначеЕсли ЗначениеЗаполнено(Элемент.ТекущиеДанные.ПроизводственнаяОперация) Тогда
			ПоказатьЗначение(, Элемент.ТекущиеДанные.ПроизводственнаяОперация);
		КонецЕсли;
...
Процедура ОперацииДокумент_УстановитьСтатусНеВыполнена(Команда)
	...
	Для каждого Ссылка Из ВыделенныеСтроки Цикл
		...
		ПараметрыПодтверждения = ОперативныйУчетПроизводстваКлиент.ПодтверждениеПриУстановкеСтатусаОперации(
			ДанныеСтроки, НовыйСтатус, ВыделенныеСтроки.Количество() > 0);
		Если ПараметрыПодтверждения <> Неопределено Тогда
			ПараметрыПодтверждения.Вставить("КлючиОпераций", ВыделенныеСтроки);
...
Процедура УстановитьСтатус(Статус)
	...
	Иначе
		...
		ОповеститьОбИзмененииСменногоЗадания();
...
&НаКлиенте
Процедура ОповеститьОбИзмененииСменногоЗадания()
	...
	Оповестить("Запись_СменноеЗадание", ПараметрОповещения, ИсточникОповещения);
...
Процедура ПодтверждениеПриУстановкеСтатусаОперацийЗавершение(РезультатВопроса, ДополнительныеПараметры) Экспорт
	...
	Если ДополнительныеПараметры.ПодтверждаемоеСобытие = ОперативныйУчетПроизводстваКлиентСервер.Подтверждение_АннулированиеПоказателей()
		...
		ОперативныйУчетПроизводстваКлиент.УстановитьСтатусПроизводственнойОперации(
			ДополнительныеПараметры.КлючиОпераций, ДополнительныеПараметры.НовыйСтатус, Объект.Подразделение);
	...
	ИначеЕсли ДополнительныеПараметры.ПодтверждаемоеСобытие = ОперативныйУчетПроизводстваКлиентСервер.Подтверждение_СозданиеИндивидуальнойОперации() Тогда
		...
		Если РезультатВопроса = КодВозвратаДиалога.Да Тогда
			Для каждого КлючОперации Из ДополнительныеПараметры.КлючиОпераций Цикл
				ДанныеСтроки = Элементы.ОперацииДокумент.ДанныеСтроки(КлючОперации);
				Если РедакторПроизводственногоПроцессаКлиентСервер.ЭтоОперацияИндивидуальная(ДанныеСтроки) Тогда
					ОткрытьФорму("Документ.ПроизводственнаяОперация2_2.Форма.ФормаДокумента",
						Новый Структура("ЗначениеКопирования", ДанныеСтроки.Ссылка),
						ЭтотОбъект, ДанныеСтроки.Ссылка);
				КонецЕсли;
			КонецЦикла;
		КонецЕсли;
		ОперативныйУчетПроизводстваКлиент.УстановитьСтатусПроизводственнойОперации(
