...
// Возвращает признак использования подакцизных товаров собственного производства в документе.
//
// Параметры:
//	ДокументОбъект - ДокументОбъект.ВозвратТоваровОтКлиента, ДанныеФормыСтруктура - данные документа.
//
// Возвращаемое значение:
//	Булево - используются подакцизные товары собственного производства.
//
Функция ИспользуютсяПодакцизныеТовары(ДокументОбъект) Экспорт
	
	Если Не ПолучитьФункциональнуюОпцию("ИспользоватьПодакцизныеТовары") Тогда
		Возврат Ложь;
	КонецЕсли;
	
	Возврат ДокументОбъект.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ВозвратТоваровОтКлиента;
...
//
// Параметры:
//	ДокументСсылка - ДокументСсылка.ВозвратТоваровОтКлиента - ссылка на документ.
//
// Возвращаемое значение:
...
Функция ПолучитьДанныеПоАкцизам(ДокументСсылка) Экспорт
	...
	|	ЕСТЬNULL(ТаблицаВидыЗапасов.ОблагаетсяАкцизом, ЛОЖЬ) КАК ОблагаетсяАкцизом,
	|	ТаблицаВидыЗапасов.ПодразделениеПроизводства КАК ПодразделениеПроизводства,
	|	ТаблицаТовары.СпособОпределенияСебестоимости КАК СпособОпределенияСебестоимости,
	|	ДанныеОснования.Дата КАК ДатаДокументаРеализации,
	|	ЕСТЬNULL(ДанныеОснования.Дата, &Период) КАК Дата
	|ПОМЕСТИТЬ ВтТаблицаТоваров
	|ИЗ
	|	Документ.ВозвратТоваровОтКлиента.ВидыЗапасов КАК ТаблицаТовары
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.РеализацияТоваровУслуг КАК ДанныеОснования
	|		ПО ТаблицаТовары.ДокументРеализации = ДанныеОснования.Ссылка
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.ВидыЗапасов КАК ТаблицаВидыЗапасов
	|		ПО ТаблицаТовары.ВидЗапасов = ТаблицаВидыЗапасов.Ссылка
	|ГДЕ
	...
	ПараметрыТекстаЗапроса.ИмяТаблицыТоваров = "ВтТаблицаТоваров";
	ПараметрыТекстаЗапроса.ИмяПоляПериод = "Дата";
	
	ТекстЧастиЗапроса = УчетПодакцизныхТоваров.ТекстЗапросаВТПодакцизныеКатегорииТоваров(, ПараметрыТекстаЗапроса);
	ЧастиТекстаЗапроса.Добавить(ТекстЧастиЗапроса);
	...
	|	ВидыЗапасов.СуммаАкциза                                             КАК СуммаАкцизаРегл,
	|	ВЫРАЗИТЬ(ВидыЗапасов.СуммаАкциза * &КоэффициентПересчетаИзРеглВУпр  КАК ЧИСЛО(31, 2)) КАК СуммаАкцизаУпр,
	|	ТаблицаПодакцизныеКатегории.КатегорияТовара                         КАК КатегорияПодакцизногоТовара,
	|	ВЫБОР КОГДА ЕСТЬNULL(ВидыЗапасов.ОблагаетсяАкцизом, ЛОЖЬ)
	|		ТОГДА ЕСТЬNULL(ТаблицаПодакцизныеКатегории.КоэффициентПересчета, 0) * ВидыЗапасов.Количество
	|		ИНАЧЕ 0
	|	КОНЕЦ                                                               КАК КоличествоАкциза,
	...
	|			И ВидыЗапасов.ДатаДокументаРеализации < &НачалоМесяцаПериода
	...
	|ИЗ
	|	ВтТаблицаТоваров КАК ВидыЗапасов
	|
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТПодакцизныеКатегорииТоваров КАК ТаблицаПодакцизныеКатегории
	|		ПО ВидыЗапасов.Номенклатура = ТаблицаПодакцизныеКатегории.Номенклатура
	|		И ВидыЗапасов.Дата = ТаблицаПодакцизныеКатегории.Период
	|
	|ГДЕ
	|	ТаблицаПодакцизныеКатегории.ПодакцизныйТовар
	|	И ТаблицаПодакцизныеКатегории.КатегорияТовара <> ЗНАЧЕНИЕ(Справочник.КатегорииПодакцизныхТоваров.ПустаяСсылка)
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|	ТаблицаВидыЗапасов.ПодразделениеПроизводства КАК ПодразделениеУплатыАкциза,
	|	&НаправлениеДеятельности КАК НаправлениеДеятельности,
	|	ТаблицаВидыЗапасов.Номенклатура КАК Номенклатура,
	|	ТаблицаВидыЗапасов.КатегорияПодакцизногоТовара КАК КатегорияПодакцизногоТовара,
	|	ЗНАЧЕНИЕ(Перечисление.ВидыОперацийСПодакцизнымТоваром.Возврат) КАК ВидОперацииСПодакцизнымТоваром,
	|	-ТаблицаВидыЗапасов.Количество КАК Количество,
	|	-ТаблицаВидыЗапасов.КоличествоАкциза КАК КоличествоАкциза,
	|	-ТаблицаВидыЗапасов.СуммаАкцизаРегл КАК СуммаАкцизаРегл,
	|	-ТаблицаВидыЗапасов.СуммаАкцизаУпр КАК СуммаАкцизаУпр,
	|	ТаблицаВидыЗапасов.ИдентификаторСтроки КАК ИдентификаторФинЗаписи,
	|	ТаблицаВидыЗапасов.НастройкаХозяйственнойОперации
	|ИЗ
	|	ВтТаблицаВидыЗапасов КАК ТаблицаВидыЗапасов
	|ГДЕ
	|	ТаблицаВидыЗапасов.СуммаАкцизаРегл <> 0
	|		ИЛИ ТаблицаВидыЗапасов.СуммаАкцизаУпр <> 0
	|		ИЛИ ТаблицаВидыЗапасов.КоличествоАкциза <> 0
	|";
	ЧастиТекстаЗапроса.Добавить(ТекстЧастиЗапроса);
	...
	РеквизитыДокумента = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ДокументСсылка, "Организация, ХозяйственнаяОперация,
		|Дата , НаправлениеДеятельности"); 
	
	СтранаРегистрации = ЗначениеНастроекКлиентСерверПовтИсп.СтранаРегистрацииОрганизации(РеквизитыДокумента.Организация);
	...
	Запрос.УстановитьПараметр("ХозяйственнаяОперация", РеквизитыДокумента.ХозяйственнаяОперация);
	Запрос.УстановитьПараметр("Страна", СтранаРегистрации);
	Запрос.УстановитьПараметр("НачалоМесяцаПериода", НачалоМесяца(РеквизитыДокумента.Дата));
	Запрос.УстановитьПараметр("ИспользоватьПодакцизныеТовары", ИспользуютсяПодакцизныеТовары(РеквизитыДокумента));
...
Процедура УстановитьПараметрыЗапросаКоэффициентыВалют(Запрос)
	...
	Если Запрос.Параметры.Свойство("КоэффициентПересчетаИзРеглВУпр") Тогда
		Возврат;
	КонецЕсли;
...
Функция ТекстЗапросаТаблицаВыручкаИСебестоимостьПродаж(Запрос, ТекстыЗапроса, Регистры)
	...
	//++ НЕ УТ
	|	-ВидыЗапасов.СуммаАкцизаРегл КАК СуммаАкцизаРегл,
	|	-ВидыЗапасов.СуммаАкцизаУпр КАК СуммаАкцизаУпр,
	//-- НЕ УТ
	|
...
Процедура ОтразитьВУчетеНДС(Запрос, ТекстыЗапроса, Регистры, ДокументСсылка)
	...
	//++ НЕ УТ
	|	ВидыЗапасов.СуммаАкциза КАК СуммаАкциза,
	//-- НЕ УТ
