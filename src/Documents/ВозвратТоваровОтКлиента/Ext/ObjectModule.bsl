...
Функция ВременныеТаблицыДанныхДокумента(ОтборПоСпособуОпределенияСебестоимости = "Все",
	...
	//++ НЕ УТ
	|	ЕСТЬNULL(РеестрДокументов.ДатаДокументаИБ, &Дата) КАК Дата,
	//-- НЕ УТ
	...
	|		ПО ТаблицаТоваров.ДокументРеализации = РеестрДокументов.Ссылка
	...
	ПараметрыТекстаЗапроса.ИмяПоляПериод = "Дата";
	...
	|		
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.РеестрДокументов КАК РеестрДокументов
	|		ПО ТаблицаВидыЗапасов.ДокументРеализации = РеестрДокументов.Ссылка
	|			И НЕ РеестрДокументов.ДополнительнаяЗапись
	|
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВТПодакцизныеКатегорииТоваров КАК ДанныеКатегорий
	|		ПО СпрНоменклатура.Ссылка = ДанныеКатегорий.Номенклатура
	|			И ЕСТЬNULL(РеестрДокументов.ДатаДокументаИБ, &Дата) = ДанныеКатегорий.Период
	|		
	...
	Запрос.УстановитьПараметр("ОблагаетсяАкцизом",
								?(ОтборПоСпособуОпределенияСебестоимости = "НеИзДокументаПродажи",
								ОблагаетсяАкцизом,
								Ложь));
	Запрос.УстановитьПараметр("ПодразделениеПроизводства",
								?(ОтборПоСпособуОпределенияСебестоимости = "НеИзДокументаПродажи",
								ПодразделениеПроизводства,
								Справочники.СтруктураПредприятия.ПустаяСсылка()));
	
	Если ОтборПоСпособуОпределенияСебестоимости = "НеИзДокументаПродажи" Тогда
		ТекстОтборПодакцизныхТоваров = "(НЕ ЕСТЬNULL(ДанныеКатегорий.ПодакцизныйТовар, ЛОЖЬ)
			|		ИЛИ ТаблицаВидыЗапасов.ВидЗапасов = ЗНАЧЕНИЕ(Справочник.ВидыЗапасов.ПустаяСсылка)
			|		ИЛИ (ЕСТЬNULL(ДанныеКатегорий.ПодакцизныйТовар, ЛОЖЬ)
			|			И ДанныеЗапасов.ОблагаетсяАкцизом = &ОблагаетсяАкцизом
			|			И ДанныеЗапасов.ПодразделениеПроизводства = &ПодразделениеПроизводства)
			|	)";
	Иначе
		ТекстОтборПодакцизныхТоваров = "ИСТИНА";
	КонецЕсли;
	
	Запрос.Текст = СтрЗаменить(Запрос.Текст, "&ОтборПодакцизныхТоваров", ТекстОтборПодакцизныхТоваров);
	//-- НЕ УТ
...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Иначе
		...
		ПараметрыКонтроля.ШаблонТекстаОшибки =
			НСтр("ru = 'Запрещен возврат подакцизного товара ""%Номенклатура%"", указанного в строке %НомерСтроки% списка ""%ПредставлениеТабличнойЧасти%"" на розничный склад ""%Склад%""';
				|en = 'Cannot return the ""%Номенклатура%"" excisable goods specified in line %НомерСтроки% of the ""%ПредставлениеТабличнойЧасти%"" list  in the ""%Склад%"" retail warehouse'");
		ПараметрыКонтроля.ПроверятьСклад = Истина;
	...
	Иначе
		...
		Если Не Проведен
			...
			Если ДокументСодержитТовары Тогда
				...
				//++ НЕ УТ
				Если Документы.ВозвратТоваровОтКлиента.ИспользуютсяПодакцизныеТовары(ЭтотОбъект) Тогда
					ДополнительныеСвойства.Вставить("ЗаписатьСуммуАкциза", Истина);
		...
		//++ НЕ УТ
		ИначеЕсли Документы.ВозвратТоваровОтКлиента.ИспользуютсяПодакцизныеТовары(ЭтотОбъект)
			И НаправлениеДеятельности <> ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Ссылка, "НаправлениеДеятельности") Тогда
			ДополнительныеСвойства.Вставить("ЗаписатьСуммуАкциза", Истина);
		//-- НЕ УТ
		КонецЕсли;
		...
		Если Не Проведен
			...
			//++ НЕ УТ
			Если ОблагаетсяАкцизом
				И Документы.ВозвратТоваровОтКлиента.ИспользуютсяПодакцизныеТовары(ЭтотОбъект) Тогда
				
				ДополнительныеСвойства.Вставить("ЗаписатьСуммуАкциза", Истина);
				ПараметрыЗаполнения.РассчитатьСуммуАкциза = Истина;
				Отборы.ОблагаетсяАкцизом.Очистить();
			...
			//++ НЕ УТ
			УчетПодакцизныхТоваров.РассчитатьСуммуАкциза(ЭтотОбъект, ПараметрыЗаполнения, , Отказ);
			//-- НЕ УТ
		...
		//++ НЕ УТ
		ИначеЕсли ОблагаетсяАкцизом
			И Документы.ВозвратТоваровОтКлиента.ИспользуютсяПодакцизныеТовары(ЭтотОбъект)
			И НаправлениеДеятельности <> ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Ссылка, "НаправлениеДеятельности") Тогда
			ДополнительныеСвойства.Вставить("ЗаписатьСуммуАкциза", Истина);
		//-- НЕ УТ
...
Процедура ЗаполнитьНоменклатуруОприходованиеВидовЗапасов(Отказ)
	...
	//++ НЕ УТ
	СписокВидовЗапасов = ВидыЗапасов.ВыгрузитьКолонку("ВидЗапасов");
	СписокВидовЗапасов = ОбщегоНазначенияКлиентСервер.СвернутьМассив(СписокВидовЗапасов);
	
	ДанныеЗапасовСписания	= ОбщегоНазначения.ЗначениеРеквизитаОбъектов(СписокВидовЗапасов, "ОблагаетсяАкцизом");
	ПодакцизныеСтроки		= Новый Массив;
	//-- НЕ УТ
		...
		Для Каждого СтрокаЗапасов Из ВидыЗапасов.НайтиСтроки(СтруктураПоиска) Цикл
			...
			НоваяСтрока.СуммаНДС					= Мин(СуммаНДС,
														?(СтрокаЗапасов.Количество <> 0,
															...
															0));
			//++ НЕ УТ
			НоваяСтрока.СуммаАкциза					= Количество * СтрокаЗапасов.СуммаАкциза / СтрокаЗапасов.Количество;
			//-- НЕ УТ
			...
			//++ НЕ УТ
			СтрокаЗапасов.СуммаАкциза	= СтрокаЗапасов.СуммаАкциза - НоваяСтрока.СуммаАкциза;
			
			Если ДанныеЗапасовСписания.Получить(СтрокаЗапасов.ВидЗапасов) Тогда
				ПодакцизныеСтроки.Добавить(СтрокаТоваров.НомерСтроки);
			КонецЕсли;
			//-- НЕ УТ
	...
	//++ НЕ УТ
	ПроверитьКорректностьУказанияПодакцизныхКатегорий(ПодакцизныеСтроки, Отказ);
	//-- НЕ УТ
...
Функция ПроверитьИзменениеРеквизитовДокумента(МенеджерВременныхТаблиц, ОтборПоСпособуОпределенияСебестоимости = "Все")
	...
	//++ НЕ УТ
	Если ОтборПоСпособуОпределенияСебестоимости = "НеИзДокументаПродажи" Тогда
		КоллекцияРеквизитов.Добавить("ОблагаетсяАкцизом");
		КоллекцияРеквизитов.Добавить("ПодразделениеПроизводства");
	КонецЕсли;
	//-- НЕ УТ
...
Процедура ЗаполнитьСебестоимостьВидовЗапасов(СоответствиеТиповНоменклатуры, ДокументСодержитОборотнуюТару, Отказ)
	...
	//++ НЕ УТ
	СкладыОприходования = Товары.ВыгрузитьКолонку("Склад");
	ЗапасыОприходования = ВидыЗапасов.ВыгрузитьКолонку("ВидЗапасов");
	
	СкладыОприходования = ОбщегоНазначенияКлиентСервер.СвернутьМассив(СкладыОприходования);
	ЗапасыОприходования = ОбщегоНазначенияКлиентСервер.СвернутьМассив(ЗапасыОприходования);
	
	ДанныеСкладов = ОбщегоНазначения.ЗначениеРеквизитаОбъектов(СкладыОприходования, "ТипСклада");
	ДанныеЗапасов = ОбщегоНазначения.ЗначениеРеквизитаОбъектов(ЗапасыОприходования, "ОблагаетсяАкцизом");
	//-- НЕ УТ
	...
	Для Каждого СтрокаТоваров Из Товары Цикл
		...
		Для Каждого СтрокаЗапасов Из ТаблицаЗапасов.НайтиСтроки(СтруктураПоиска) Цикл
			...
			Если ВыводитьСообщенияОбОшибках
				И ДанныеЗапасов.Получить(СтрокаЗапасов.ВидЗапасов)
				И ДанныеСкладов.Получить(СтрокаТоваров.Склад) = Перечисления.ТипыСкладов.РозничныйМагазин Тогда
				
				ИмяПоля = "Номенклатура";
				ВозвращаемыйТовар = СтрокаТоваров.Номенклатура;
				
				Если СтрокаТоваров.Порча Тогда
					ИмяПоля = "НоменклатураОприходование";
					ВозвращаемыйТовар = СтрокаТоваров.НоменклатураОприходование;
				КонецЕсли;
				
				ТекстСообщения = НСтр("ru = 'Запрещен возврат подакцизного товара ""%1"", указанного в строке %2 списка ""Товары"" на розничный склад ""%3"".';
										|en = 'Cannot return the ""%1"" excisable goods specified in line %2 of the ""Goods"" list in the ""%3"" retail warehouse.'");
				ТекстСообщения = СтрШаблон(ТекстСообщения,
											ВозвращаемыйТовар,
											СтрокаТоваров.НомерСтроки,
											СтрокаТоваров.Склад);
				
				Поле = ОбщегоНазначенияКлиентСервер.ПутьКТабличнойЧасти("Товары", СтрокаТоваров.НомерСтроки, ИмяПоля);
...
//++ НЕ УТ
Процедура ПроверитьКорректностьУказанияПодакцизныхКатегорий(ПодакцизныеСтроки, Отказ)
	
	Если Отказ
		Или ПодакцизныеСтроки.Количество() = 0
		Или Не ПолучитьФункциональнуюОпцию("ИспользоватьПодакцизныеТовары") Тогда
	...
	ПодакцизныеСтроки = ОбщегоНазначенияКлиентСервер.СвернутьМассив(ПодакцизныеСтроки);
	
	Для Каждого НомерСтроки Из ПодакцизныеСтроки Цикл
		СписокСтрок.Добавить(Товары[НомерСтроки - 1]);
	КонецЦикла;
	
	ПараметрыКонтроля = УчетПодакцизныхТоваров.ПараметрыКонтроляЗаполненияПодакцизныхТоваров();
	ПараметрыКонтроля.ИмяПоляСОшибкой		= "НоменклатураОприходование";
	ПараметрыКонтроля.ШаблонТекстаОшибки	=
		НСтр("ru = 'Различаются категории подакцизных товаров %Номенклатура% (отгруженный товар) и %НоменклатураОприходование% (испорченный товар) в строке %НомерСтроки% списка ""%ПредставлениеТабличнойЧасти%""';
			|en = 'The %Номенклатура% (shipped goods) and %НоменклатураОприходование% (spoiled goods) excisable goods categories differ in line %НомерСтроки% of the ""%ПредставлениеТабличнойЧасти%"" list'");
	ПараметрыКонтроля.СписокСтрок = СписокСтрок;
	
	УчетПодакцизныхТоваров.КонтрольСоответствияКатегорииПодакцизныхТоваров(ЭтотОбъект, ПараметрыКонтроля, Отказ);
