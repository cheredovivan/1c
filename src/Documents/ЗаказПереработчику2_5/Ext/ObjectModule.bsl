	...
	ИначеЕсли ТипДанныхЗаполнения = Тип("Структура") Тогда
		...
		Если ДанныеЗаполнения.Свойство("Основание") Тогда
			...
			ИначеЕсли ТипОснования = Тип("ДокументСсылка.ЗаказДавальца2_5") Тогда
				ЗаполнитьДокументНаОснованииЗаказаДавальца(ДанныеЗаполнения);
			//-- НЕ УТКА
			КонецЕсли;
...
Процедура ЗаполнитьДокументНаОснованииЗаказаНаПеремещение(ДанныеЗаполнения)
	...
	Запрос = Новый Запрос(
		...
		|ГДЕ
		|	Распоряжения.Ссылка = &ЗаказНаПеремещение");
...
Процедура ЗаполнитьДокументНаОснованииСделкиПоОбособленнымЗаказам(Знач СправочникОснование)
	...
	ПараметрыФормированияТаблицы.Индексы = "Распоряжение, Номенклатура, Характеристика, КодСтроки";
	...
	|			Распоряжения.ОформленоКоличествоОборот) КАК Заказано,
	|		ВЫБОР
	|			КОГДА НЕ ТоварыЗаказа.Ссылка ЕСТЬ NULL
	...
	|			Распоряжения.ОформленоКоличествоОборот) > 0) КАК Т
	|
	|СГРУППИРОВАТЬ ПО
	|	Т.Номенклатура,
	|	Т.Характеристика,
...
Процедура ЗаполнитьДокументНаОснованииЗаказаДавальца(ДанныеЗаполнения)
	
	#Область ДанныеОснования
	
	Запрос = Новый Запрос;
	Запрос.Текст =
	"ВЫБРАТЬ
	|	ТаблицаТовары.НомерСтрокиВДокументе КАК НомерСтрокиВДокументе,
	...
	|	ТаблицаТовары КАК ТаблицаТовары
	|	
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ЗаказДавальца2_5.Продукция КАК ТаблицаПродукция
	|	ПО ТаблицаПродукция.Ссылка = &ЗаказДавальца
	|	И ТаблицаПродукция.НомерСтроки = ТаблицаТовары.НомерСтрокиВДокументе
	|
	|СГРУППИРОВАТЬ ПО
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ РАЗРЕШЕННЫЕ
	|	ЗаказДавальца.Ссылка                          КАК ДокументОснование,
	...
	|	ЗаказДавальца.Склад                           КАК СкладПродукции,
	|	ЗаказДавальца.СкладПоступления                КАК СкладМатериалов,
	|	ЗаказДавальца.НалогообложениеНДС              КАК ЗакупкаПодДеятельность,
	|	ВЫБОР
	|		КОГДА ЗаказДавальца.НаправлениеДеятельности.УчетЗатрат
	|				ИЛИ ЗаказДавальца.НаправлениеДеятельности.УчетРасчетовСПоставщиками
	|			ТОГДА ЗаказДавальца.НаправлениеДеятельности
	|	КОНЕЦ                                         КАК НаправлениеДеятельности,
	|	ВЫБОР
	|		КОГДА
	|			НЕ СУММА(ВЫБОР
	|				КОГДА НЕ ТаблицаПродукция.Спецификация = ЗНАЧЕНИЕ(Справочник.РесурсныеСпецификации.ПустаяСсылка)
	|					ТОГДА 1
	|				ИНАЧЕ 0
	|			КОНЕЦ) = 0
	|			ТОГДА ЗНАЧЕНИЕ(Перечисление.ГруппировкиЗатратВЗаказеПереработчику.ПоСпецификациямИНазначениям)
	|		ИНАЧЕ ЗНАЧЕНИЕ(Перечисление.ГруппировкиЗатратВЗаказеПереработчику.ПоПродукцииИНазначениям)
	...
	|	ЗаказДавальца.Приоритет,
	|	ЗаказДавальца.Организация,
	|	ЗаказДавальца.Сделка,
	|	ЗаказДавальца.Подразделение,
	|	ЗаказДавальца.Склад,
	|	ЗаказДавальца.СкладПоступления,
	|	ЗаказДавальца.НалогообложениеНДС,
	...
	|	ТаблицаПродукция.ДатаОтгрузки       КАК ДатаПоступления,
	...
	|	ТаблицаМатериалы.Цена                      КАК Цена,
	|	СУММА(ТаблицаМатериалы.Сумма)              КАК Сумма,
	|	ТаблицаМатериалы.ДатаПоступления           КАК ДатаОтгрузки,
	|	ЗНАЧЕНИЕ(Перечисление.ВариантыОбеспечения.КОбеспечению) КАК ВариантОбеспечения,
	|	ИСТИНА                                     КАК Обособленно
	|ИЗ
	|	Документ.ЗаказДавальца2_5.Материалы КАК ТаблицаМатериалы
	|
	|ГДЕ
	|	ТаблицаМатериалы.Ссылка = &ЗаказДавальца
	|
	|СГРУППИРОВАТЬ ПО
	...
	|	ТаблицаМатериалы.Склад,
	|	ТаблицаМатериалы.Упаковка,
	|	ТаблицаМатериалы.ДатаПоступления,
	|	ТаблицаМатериалы.Цена
	|";
	
	Запрос.УстановитьПараметр("ЗаказДавальца", ДанныеЗаполнения.Основание);
	Запрос.УстановитьПараметр("ТаблицаТовары", ПолучитьИзВременногоХранилища(ДанныеЗаполнения.АдресТовары));
	УдалитьИзВременногоХранилища(ДанныеЗаполнения.АдресТовары);
	...
	// Заполнение шапки
	Реквизиты = Результат[Результат.ВГраница() - 2].Выбрать();
	Реквизиты.Следующий();
	ЗаполнитьЗначенияСвойств(ЭтотОбъект, Реквизиты);
	
	// Заполнение табличной части.
	ВыходныеИзделия.Загрузить(Результат[Результат.ВГраница() - 1].Выгрузить());
	
	СписокПолей = ПереработкаНаСторонеКлиентСервер.ПереченьПолейГруппыЗатратЗаказаПереработчику(ГруппировкаЗатрат);
	СписокПолей = СтрСоединить(СписокПолей.ВыгрузитьЗначения(), ",");
	
	ГруппыЗатрат = ВыходныеИзделия.Выгрузить(Неопределено, СписокПолей);
	ГруппыЗатрат.Свернуть(СписокПолей);
	ГруппыЗатрат.Колонки.Добавить("НомерГруппыЗатрат", ОбщегоНазначения.ОписаниеТипаЧисло(10, 0, ДопустимыйЗнак.Неотрицательный));
	Для Каждого ТекущаяГруппа Из ГруппыЗатрат Цикл
		МаксимальныйНомерГруппыЗатрат = МаксимальныйНомерГруппыЗатрат + 1;
		ТекущаяГруппа.НомерГруппыЗатрат = МаксимальныйНомерГруппыЗатрат;
	КонецЦикла;
	ГруппыЗатрат.Индексы.Добавить(СписокПолей);
	...
	Для Каждого ТекущиеДанные Из ВыходныеИзделия Цикл
		...
		НайденныеСтроки = ГруппыЗатрат.НайтиСтроки(Отбор);
		Если Не НайденныеСтроки = Неопределено Тогда
			ТекущиеДанные.НомерГруппыЗатрат = НайденныеСтроки[0].НомерГруппыЗатрат;
		КонецЕсли;
		
	КонецЦикла;
	
	Если ГруппировкаЗатрат = Перечисления.ГруппировкиЗатратВЗаказеПереработчику.ПоСпецификациямИНазначениям Тогда
		
		Документы.ЗаказПереработчику2_5.ЗаполнитьПобочныеИзделияМатериалыУслугиПоСпецификации(ВыходныеИзделия, ЭтотОбъект);
		
	ИначеЕсли Не ВыходныеИзделия.Количество() = 0 Тогда
		
		ОбеспечениеМатериаламиИРаботами.Загрузить(Результат[Результат.ВГраница()].Выгрузить());
		Если ГруппыЗатрат.Количество() = 1 Тогда
			Для Каждого ТекущиеДанные Из ОбеспечениеМатериаламиИРаботами Цикл
				ТекущиеДанные.НомерГруппыЗатрат = ГруппыЗатрат[0].НомерГруппыЗатрат;
			КонецЦикла;
		КонецЕсли;
		
		#Область ПакетнаяОбработкаТЧ
		...
		ПакетнаяОбработкаТабличнойЧастиСервер.ОбработатьТЧ(ОбеспечениеМатериаламиИРаботами, СтруктураДействийМатериалы);
		...
		Для Каждого ТекущаяГруппа Из ГруппыЗатрат Цикл
			...
			НоваяСтрока.НомерГруппыЗатрат = ТекущаяГруппа.НомерГруппыЗатрат;
		КонецЦикла;
