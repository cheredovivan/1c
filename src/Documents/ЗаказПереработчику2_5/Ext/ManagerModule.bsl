...
Процедура ЗаполнитьПобочныеИзделияМатериалыУслугиПоСпецификации(СписокПродукции, Объект, СписокТЧ = "") Экспорт
	...
	// Если интерактивный вызов заполнения на форме, то таблица выходных изделий не подлежит изменению.
	Если Не ТипЗнч(Объект) = Тип("ДанныеФормыСтруктура")
		И Не ПереченьДанных.Найти("ВозвратныеОтходы") = Неопределено Тогда
		ИсключитьПобочныеИзделияИзВыходных(Объект, СписокПродукции, ДанныеСпецификаций);
	КонецЕсли;
...
Функция СкорректироватьСтрокиЗаказа(Объект, СтруктураПараметров) Экспорт
	...
	Если КорректироватьМатериалы Тогда
		...
		ПараметрыЗаполнения.МенеджерРегистра  = РегистрыНакопления.РаспоряженияНаОтгрузкуИВозврат;
...
Функция ДобавитьКомандуСоздатьНаОсновании(КомандыСозданияНаОсновании) Экспорт
	...
	Если ПравоДоступа("Добавление", Метаданные.Документы.ЗаказПереработчику2_5) Тогда
		...
		КомандаСоздатьНаОсновании.ФункциональныеОпции = "ИспользоватьЗаказыПереработчикам2_5";
...
Функция ДобавитьКомандуСоздатьОбеспечивающийЗаказ(КомандыСозданияНаОсновании) Экспорт
	...
	Если ПравоДоступа("Добавление", Метаданные.Документы.ЗаказПереработчику2_5) Тогда
		...
		КомандаСоздатьНаОсновании.РежимЗаписи = "Проводить";
		КомандаСоздатьНаОсновании.ФункциональныеОпции = "ИспользоватьЗаказыПереработчикам2_5";
		ПараметрыСоздания = ОбеспечениеВДокументахСервер.ПараметрыСозданияОбеспечивающегоЗаказа(КомандаСоздатьНаОсновании.ДополнительныеПараметры);
...
// 
// Параметры:
//  ВсеЗаказы - Массив Из ДокументСсылка.ЗаказПереработчику2_5
...
Функция ПараметрыВыполненияКомандУстановкиДействий(ВсеЗаказы, КодДействия) Экспорт
	...
	Результат.ПараметрыВстраивания = ДоступныеОстаткиПараметрыВстраивания();
	...
	Результат.ПараметрыВстраивания = Документы.ЭтапПроизводства2_2.ДоступныеОстаткиПараметрыВстраивания(
...
Функция ТаблицаСырьеУПереработчика()
	...
	|	РаспоряженияОбороты.КОформлениюКоличествоОборот КАК КПередаче,
	|	(РаспоряженияОбороты.КОформлениюКоличествоОборот -
	|		РаспоряженияОбороты.ОформленоКоличествоОборот) КАК Передать,
	|	РаспоряженияОбороты.ОформленоКоличествоОборот КАК Передано
	|ИЗ
	|	#РаспоряженияНаОтгрузкуОборотыРазвернутая КАК РаспоряженияОбороты
	|ГДЕ
	|	РаспоряженияОбороты.КОформлениюКоличествоОборот <> 0
	|	ИЛИ (РаспоряженияОбороты.КОформлениюКоличествоОборот -
	|		РаспоряженияОбороты.ОформленоКоличествоОборот) <> 0
	|	ИЛИ РаспоряженияОбороты.ОформленоКоличествоОборот <> 0
	|ИТОГИ
	|ПО
	|	Документ
	|;
...
Процедура ИсключитьПобочныеИзделияИзВыходных(Объект, СписокПродукции, ДанныеСпецификаций)
	...
	Для Каждого ТекСтруктура Из ДанныеСпецификаций Цикл
		...
		Для Каждого ТекущийОтход Из ТекСтруктура.ВозвратныеОтходы Цикл
			
			ЗаполнитьЗначенияСвойств(Отборы, ТекущийОтход, "Номенклатура, Характеристика");
			
			СтрокиПродукции = СписокПродукции.НайтиСтроки(Отборы);
			Для Каждого ТекущаяПродукция Из СтрокиПродукции Цикл
				
				Если ТекущаяПродукция.Количество = 0 Тогда
					Продолжить;
				КонецЕсли;
				
				Количество = Мин(ТекущаяПродукция.Количество, ТекущийОтход.Количество);
				
				НоваяСтрока = ВозвратныеОтходы.Добавить();
				ЗаполнитьЗначенияСвойств(НоваяСтрока, ТекущийОтход);
				НоваяСтрока.Назначение         = ТекущаяПродукция.Назначение;
				НоваяСтрока.Количество         = Количество;
				НоваяСтрока.КоличествоУпаковок =
				...
				ТекущийОтход.Количество     = ТекущийОтход.Количество - Количество;
				
			КонецЦикла;
			
			Если Не ТекущийОтход.Количество = 0 Тогда
				
				НоваяСтрока = ВозвратныеОтходы.Добавить();
				ЗаполнитьЗначенияСвойств(НоваяСтрока, ТекущийОтход);
				
				НоваяСтрока.Назначение = СоответствиеНазначений.Получить(ТекСтруктура.НомерГруппыЗатрат);
				НоваяСтрока.КоличествоУпаковок =
					НоваяСтрока.Количество
					* НоваяСтрока.ДанныеУпаковки.Знаменатель
					/ НоваяСтрока.ДанныеУпаковки.Числитель;
	...
	Для Каждого СтрокаТаблицы Из СписокПродукции.НайтиСтроки(Новый Структура("Количество", 0)) Цикл
		СписокПродукции.Удалить(СтрокаТаблицы);
	КонецЦикла;
...
Функция ДопустимыеОтклоненияМерныхТоваров(СписокРаспоряжений, ТипДвиженияЗапасов) Экспорт
	...
	Иначе
		...
		Запрос.Текст =
			...
			|	СУММА(ТоварыКОтгрузке.КОформлениюКоличествоОборот * (&ДопустимоеОтклонениеОтгрузкиПриемкиМерныхТоваров / 100)) КАК
			|		ДопустимоеОтклонение
			|ИЗ
			|	#РаспоряженияНаОтгрузкуОборотыРазвернутая КАК ТоварыКОтгрузке
			|СГРУППИРОВАТЬ ПО
