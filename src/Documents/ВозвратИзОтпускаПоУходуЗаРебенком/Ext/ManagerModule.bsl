...
Процедура СформироватьДвиженияВозвратаВременноОсвобожденныхПозиции(Движения, Сотрудники, ДатаВозврата) Экспорт
	...
	Запрос.Текст =
		...
		|		ПО ЗанятостьПозиций.Сотрудник = ЗанятостьПозицийШтатногоРасписанияИнтервальный.Сотрудник
		|			И ЗанятостьПозиций.Период >= ЗанятостьПозицийШтатногоРасписанияИнтервальный.ДатаНачала
		|			И ЗанятостьПозиций.ГоловнаяОрганизация = ЗанятостьПозицийШтатногоРасписанияИнтервальный.ГоловнаяОрганизация
		|			И ЗанятостьПозиций.ФизическоеЛицо = ЗанятостьПозицийШтатногоРасписанияИнтервальный.ФизическоеЛицо
		|
		|УПОРЯДОЧИТЬ ПО
		|	ЗанятостьПозицийШтатногоРасписанияИнтервальный.ДатаНачала УБЫВ
		|ИТОГИ ПО
		|	Сотрудник,
		|	ПозицияШтатногоРасписания";
	Пакет = Запрос.ВыполнитьПакет();
	ТаблицаДвижении = Пакет[0].Выгрузить();
	ВыборкаПоСотрудникам = Пакет[1].Выбрать(ОбходРезультатаЗапроса.ПоГруппировкам);
	Пока ВыборкаПоСотрудникам.Следующий() Цикл
		Выборка = ВыборкаПоСотрудникам.Выбрать();
		Пока Выборка.Следующий() Цикл
			Если ТипЗнч(Выборка.РегистраторЗаписи) <> Тип("ДокументСсылка.ОтпускПоУходуЗаРебенком") Тогда
				Отбор = Новый Структура("Сотрудник, ПозицияШтатногоРасписания", Выборка.Сотрудник, Выборка.ПозицияШтатногоРасписания);
				НайденныеСтроки = ТаблицаДвижении.НайтиСтроки(Отбор);
				Для каждого НайденнаяСтрока Из НайденныеСтроки Цикл
					ЗаполнитьЗначенияСвойств(НайденнаяСтрока, Выборка,
						"ВидЗанятостиПозиции, ДействуетДо, ПланируемаяДатаЗавершения, ЗамещаемыйСотрудник");
				КонецЦикла;
			КонецЕсли;
		КонецЦикла;
	КонецЦикла;
	
	КадровыйУчетРасширенный.СформироватьДвиженияЗанятостьПозицийШтатногоРасписания(Движения, ТаблицаДвижении);
КонецПроцедуры
