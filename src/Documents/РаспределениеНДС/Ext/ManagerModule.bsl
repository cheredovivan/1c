...
Функция ДанныеДляРаспределения(Реквизиты, ДопПараметры)
	...
	Если Не ПроведениеДокументов.ЕстьТаблицаЗапроса("ВтВнеоборотныеАктивы", Неопределено, Запрос.МенеджерВременныхТаблиц) Тогда
		Запрос.Текст =
		"ВЫБРАТЬ
	...
	Если Не ПроведениеДокументов.ЕстьТаблицаЗапроса("ВтВнеоборотныеАктивыКвартал", Неопределено, Запрос.МенеджерВременныхТаблиц) Тогда
		Запрос.Текст =
		"ВЫБРАТЬ
		|	ДАТАВРЕМЯ(1,1,1) КАК ДатаПринятияКУчету,
		|	""ОС"" КАК ВнеоборотныйАктив,
		...
		|	НЕОПРЕДЕЛЕНО КАК СтатьяРасходовНУ,
		|	НЕОПРЕДЕЛЕНО КАК АналитикаРасходовНУ
		|ПОМЕСТИТЬ ВтВнеоборотныеАктивыКвартал
		|ГДЕ
		|	ЛОЖЬ
		|";
		Запрос.Выполнить();
	...
	// Получение таблиц для анализа условий распределения входящего НДС по ОС и НМА.
	
	НачалоМесяца = НачалоМесяца(Реквизиты.Период);
	КонецМесяца = КонецМесяца(Реквизиты.Период);
	КонецКвартала = КонецКвартала(Реквизиты.Период);
	
	Запрос.УстановитьПараметр("НачалоМесяца", НачалоМесяца);
	Запрос.УстановитьПараметр("КонецМесяца", КонецМесяца);
	Запрос.УстановитьПараметр("КонецМесяцаГраница", Новый Граница(КонецМесяца, ВидГраницы.Включая));
	Запрос.УстановитьПараметр("КонецКвартала", Новый Граница(КонецКвартала, ВидГраницы.Включая));
	Запрос.УстановитьПараметр("КонецПредыдущегоМесяцаГраница", Новый Граница(НачалоМесяца - 1, ВидГраницы.Включая));
	...
	|	Т.НДСРеглОстаток КАК НДСРегл,
	|	Т.НДСУпрОстаток КАК НДСУпр,
	|	ЛОЖЬ КАК ЭтоФинальноеРаспределение,
	|	ИСТИНА КАК ЭтоОстатки
	|ПОМЕСТИТЬ ВтПартииОстаткиИРасходы_ПредварительнаяКвартал
	|ИЗ
	|	РегистрНакопления.ПартииНДСКРаспределению.Остатки(
	|		&КонецКвартала,
	...
	|					Т.Объект
	|					ИЗ
	|						втВнеоборотныеАктивыКвартал КАК Т
	|					ГДЕ 
	|						Т.ЭтоВводВЭксплуатацию)
	|	) КАК Т
	|	
	|ОБЪЕДИНИТЬ ВСЕ
	|	
	...
	|	Т.НДСРегл,
	|	Т.НДСУпр,
	|	ВЫБОР КОГДА Т.НастройкаХозяйственнойОперации = ЗНАЧЕНИЕ(Справочник.НастройкиХозяйственныхОпераций.ПустаяСсылка)
	|		ТОГДА ЛОЖЬ
	|		ИНАЧЕ ИСТИНА
	|	КОНЕЦ,
	|	ЛОЖЬ КАК ЭтоОстатки
	|	ИЗ
	|		РегистрНакопления.ПартииНДСКРаспределению КАК Т
	|			ВНУТРЕННЕЕ СОЕДИНЕНИЕ втВнеоборотныеАктивыКвартал КАК ВнеобАктивы
	|			ПО Т.АналитикаРасходов = ВнеобАктивы.Объект
	|				И ВнеобАктивы.ЭтоВводВЭксплуатацию
	|	ГДЕ
	|		&РаспределятьНДСВМесяцеОсуществленияКапВложений
	|		И Т.Период < &НачалоМесяца
	|		И Т.Организация = &Организация
	|		И Т.Активность
	...
	|	Т.НДСУпрОстаток КАК НДСУпр,
	|	ИСТИНА КАК ЭтоОстатки
	|ПОМЕСТИТЬ ВтПартииОстаткиИРасходы_ПредварительнаяМесяц
	|ИЗ
	|	РегистрНакопления.ПартииНДСКРаспределению.Остатки(
	|		&КонецМесяцаГраница,
	|		Организация = &Организация
	...
	|	ИЗ
	|		РегистрНакопления.ПартииНДСКРаспределению КАК Т
	|			ВНУТРЕННЕЕ СОЕДИНЕНИЕ втВнеоборотныеАктивы КАК ВнеобАктивы
	|			ПО Т.АналитикаРасходов = ВнеобАктивы.Объект
	|				И ВнеобАктивы.ЭтоВводВЭксплуатацию
	|	ГДЕ
	|		&РаспределятьНДСВМесяцеОсуществленияКапВложений
	...
	|	Т.СтавкаНДС КАК СтавкаНДС,
	|	Т.ВидДеятельностиНДС КАК ВидДеятельностиНДС,
	|	Т.ВидДеятельностиНДС КАК НалогообложениеНДСПартии,
	|	Т.ЭтоОстатки КАК ЭтоОстатки,
	|	СтатьиРасходов.ВариантРаспределенияРасходовРегл КАК ВариантРаспределенияРасходовСтатьи,
	|	СУММА(Т.СтоимостьРегл) КАК СтоимостьРегл,
	|	СУММА(Т.НДСРегл) КАК НДСРегл,
	|	СУММА(Т.НДСУпр) КАК НДСУпр
	|ПОМЕСТИТЬ ВтОстаткиНДСПартииИРасходыПредыдущихМесяцевПоВНАМесяц
	|ИЗ
	|	ВтПартииОстаткиИРасходы_ПредварительнаяМесяц КАК Т
	|
	|	ЛЕВОЕ СОЕДИНЕНИЕ
	|		ПланВидовХарактеристик.СтатьиРасходов КАК СтатьиРасходов
	|	ПО
	|		Т.СтатьяРасходов = СтатьиРасходов.Ссылка
	|
	|СГРУППИРОВАТЬ ПО
	|	Т.АналитикаРасходов,
	|	Т.ДокументПоступленияРасходов,
	...
	|	СУММА(Т.НДСРегл) КАК НДСРегл,
	|	СУММА(Т.НДСУпр) КАК НДСУпр
	|ПОМЕСТИТЬ ВтОстаткиНДСПартииИРасходыПредыдущихМесяцевПоВНАКвартал
	|ИЗ
	|	ВтПартииОстаткиИРасходы_ПредварительнаяКвартал КАК Т
	|
	|	ЛЕВОЕ СОЕДИНЕНИЕ
	|		ПланВидовХарактеристик.СтатьиРасходов КАК СтатьиРасходов
	|	ПО
	...
	|;
	|
	|УНИЧТОЖИТЬ ВтПартииОстаткиИРасходы_ПредварительнаяМесяц
	|; 
	|
	|ВЫБРАТЬ
	|	ПартииНДСКРаспределению.Организация                 КАК Организация,
	...
	|	
	|	ПартииНДСКРаспределению.Поставщик                   КАК Поставщик,
	|	ПартииНДСКРаспределению.ВидДеятельностиНДС          КАК НалогообложениеНДСПартии,
	|	ПартииНДСКРаспределению.СтавкаНДС                   КАК СтавкаНДС,
	|	
	|	ВнеобАктивы.РаспределятьНДС                         КАК РаспределятьНДС,
	|	
	|	КОНЕЦПЕРИОДА(ПартииНДСКРаспределению.Период, МЕСЯЦ) КАК ПериодБазы,
	|	ЕСТЬNULL(ПартииНДСКРаспределениюКорректировка.ОснованиеКорректировки, НЕОПРЕДЕЛЕНО) КАК ОснованиеКорректировки,
	|	
	|	ВЫБОР КОГДА ВнеобАктивы.РаспределятьНДС
	|		ТОГДА НЕОПРЕДЕЛЕНО
	...
	|	
	|	ПартииНДСКРаспределению.ВидЦенности                 КАК ВидЦенности,
	|	ВЫБОР КОГДА ПартииНДСКРаспределению.Период < &НачалоМесяца
	|		ТОГДА ЛОЖЬ
	|		ИНАЧЕ ИСТИНА
	|	КОНЕЦ                                               КАК ЭтоОстатки,
	|	
	...
	|ПОМЕСТИТЬ ВтПартииНДСПоВНАВведеннымВЭксплуатациюВЭтомМесяце
	|ИЗ
	|	РегистрНакопления.ПартииНДСКРаспределению КАК ПартииНДСКРаспределению
	|	
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ 
	...
	|	ПО 
	|		ПартииНДСКРаспределению.АналитикаРасходов = ВнеобАктивы.Объект
	|
	|	ЛЕВОЕ СОЕДИНЕНИЕ
	|		ПартииНДСКРаспределениюКорректировка КАК ПартииНДСКРаспределениюКорректировка
	|	ПО
	|		ПартииНДСКРаспределению.АналитикаРасходов = ПартииНДСКРаспределениюКорректировка.АналитикаРасходов
	|		И ПартииНДСКРаспределению.ДокументПоступленияРасходов = ПартииНДСКРаспределениюКорректировка.ДокументПоступленияРасходов
	|		И ПартииНДСКРаспределению.Организация = ПартииНДСКРаспределениюКорректировка.Организация  
	|		И ПартииНДСКРаспределению.Подразделение = ПартииНДСКРаспределениюКорректировка.Подразделение
	|		И ПартииНДСКРаспределению.СтатьяРасходов = ПартииНДСКРаспределениюКорректировка.СтатьяРасходов
	|		И ПартииНДСКРаспределению.АналитикаАктивовПассивов = ПартииНДСКРаспределениюКорректировка.АналитикаАктивовПассивов
	|		И ПартииНДСКРаспределению.Поставщик = ПартииНДСКРаспределениюКорректировка.Поставщик
	|		И ПартииНДСКРаспределению.ВидЦенности = ПартииНДСКРаспределениюКорректировка.ВидЦенности
	|		И ПартииНДСКРаспределению.НаправлениеДеятельности = ПартииНДСКРаспределениюКорректировка.НаправлениеДеятельности
	|		И ПартииНДСКРаспределению.СтавкаНДС = ПартииНДСКРаспределениюКорректировка.СтавкаНДС
	|		И ПартииНДСКРаспределению.ВидДеятельностиНДС = ПартииНДСКРаспределениюКорректировка.ВидДеятельностиНДС
	|ГДЕ
	|	&РаспределятьНДСВМесяцеОсуществленияКапВложений
	|	И ПартииНДСКРаспределению.Организация = &Организация
	|	И ПартииНДСКРаспределению.Активность
	|	И ПартииНДСКРаспределению.Период <= &КонецМесяца
	|	И ПартииНДСКРаспределению.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Приход)
	|	И ПартииНДСКРаспределению.ВидДеятельностиНДС = ЗНАЧЕНИЕ(Перечисление.ТипыНалогообложенияНДС.ПродажаНеОблагаетсяНДС)
	|	И (ВнеобАктивы.РаспределятьНДС ИЛИ  ВнеобАктивы.НалогообложениеНДС = ЗНАЧЕНИЕ(Перечисление.ТипыНалогообложенияНДС.ПродажаОблагаетсяНДС))
	|;
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|	НДСПредъявленный.НДСОстаток КАК СуммаНДС
	|ПОМЕСТИТЬ ВтСуммаНДСПредъявленный
	|ИЗ
	|	РегистрНакопления.НДСПредъявленный.Остатки(
	|			&КонецПредыдущегоМесяцаГраница,
	|			(СчетФактура, ВидЦенности, СтавкаНДС, ВидДеятельностиНДС) В
	|				(ВЫБРАТЬ
	|					Т.ДокументПоступленияРасходов,
	|					Т.ВидЦенности,
	|					Т.СтавкаНДС,
	|					Т.ВидДеятельностиНДС
	|				ИЗ
	|					ВтОстаткиНДСПартииИРасходыПредыдущихМесяцевПоВНАКвартал  КАК Т
	|				ГДЕ
	|					&РаспределениеЗаПолныйПериод)) КАК НДСПредъявленный
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	...
	|					Т.ВидДеятельностиНДС
	|				ИЗ
	|					ВтОстаткиНДСПартииИРасходыПредыдущихМесяцевПоВНАМесяц КАК Т
	|				ГДЕ
	|					НЕ &РаспределениеЗаПолныйПериод)) КАК НДСПредъявленный
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	...
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВтОстаткиНДСПартииИРасходыПредыдущихМесяцевПоВНАКвартал КАК Партии
	|		ПО (Партии.ДокументПоступленияРасходов = НДСПредъявленный.СчетФактура)
	|			И (Партии.ВидЦенности = НДСПредъявленный.ВидЦенности)
	|			И (Партии.СтавкаНДС = НДСПредъявленный.СтавкаНДС)
	|			И (Партии.ВидДеятельностиНДС = НДСПредъявленный.ВидДеятельностиНДС)
	|ГДЕ
	|	&РаспределениеЗаПолныйПериод
	|	И НДСПредъявленный.Активность
	|	И НЕ НДСПредъявленный.РегламентнаяОперация
	|	И НДСПредъявленный.Период >= &НачалоМесяца
	...
	|ИЗ
	|	РегистрНакопления.НДСПредъявленный КАК НДСПредъявленный
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВтОстаткиНДСПартииИРасходыПредыдущихМесяцевПоВНАМесяц КАК Партии
	|		ПО (Партии.ДокументПоступленияРасходов = НДСПредъявленный.СчетФактура)
	|			И (Партии.ВидЦенности = НДСПредъявленный.ВидЦенности)
	|			И (Партии.СтавкаНДС = НДСПредъявленный.СтавкаНДС)
	|			И (Партии.ВидДеятельностиНДС = НДСПредъявленный.ВидДеятельностиНДС)
	|ГДЕ
	|	НЕ &РаспределениеЗаПолныйПериод
	|	И НДСПредъявленный.Активность
	|	И НЕ НДСПредъявленный.РегламентнаяОперация
	|	И НДСПредъявленный.Период >= &НачалоМесяца
	|	И НДСПредъявленный.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Приход)
	...
	|
	|ВЫБРАТЬ
	|	СписанияНДС.СчетФактура,
	|	ТЧЦенности.ВидЦенности,
	|	ТЧЦенности.СтавкаНДС,
	|	ТЧЦенности.ВидДеятельностиНДС,
	|	ТЧЦенности.НДС
	|ИЗ
	|	ВтОстаткиНДСПартииИРасходыПредыдущихМесяцевПоВНАКвартал КАК Партии
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.СписаниеНДСНаРасходы КАК СписанияНДС
	|			ПО (Партии.ДокументПоступленияРасходов = СписанияНДС.СчетФактура)
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.СписаниеНДСНаРасходы.Ценности КАК ТЧЦенности
	|		ПО (ТЧЦенности.Ссылка = СписанияНДС.Ссылка)
	|			И (Партии.ВидЦенности = ТЧЦенности.ВидЦенности)
	|			И (Партии.СтавкаНДС = ТЧЦенности.СтавкаНДС)
	|			И (Партии.ВидДеятельностиНДС = ТЧЦенности.ВидДеятельностиНДС)
	|ГДЕ
	|	&РаспределениеЗаПолныйПериод
	|	И СписанияНДС.Проведен
	|	И СписанияНДС.Дата < &НачалоМесяца
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	...
	|	ТЧЦенности.НДС
	|ИЗ
	|	ВтОстаткиНДСПартииИРасходыПредыдущихМесяцевПоВНАМесяц КАК Партии
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.СписаниеНДСНаРасходы КАК СписанияНДС
	|			ПО (Партии.ДокументПоступленияРасходов = СписанияНДС.СчетФактура)
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.СписаниеНДСНаРасходы.Ценности КАК ТЧЦенности
	|		ПО (ТЧЦенности.Ссылка = СписанияНДС.Ссылка)
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ВтСуммаНДСПредъявленный.СчетФактура КАК СчетФактура,
	|	СУММА(ВтСуммаНДСПредъявленный.СуммаНДС) КАК СуммаНДС
	|ПОМЕСТИТЬ ВтСуммаНДСПредъявленныйСФ
	|ИЗ
	|	ВтСуммаНДСПредъявленный КАК ВтСуммаНДСПредъявленный
	|
	|СГРУППИРОВАТЬ ПО
	|	ВтСуммаНДСПредъявленный.СчетФактура
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	СчетФактура
	|;
	|
	|УНИЧТОЖИТЬ ВтСуммаНДСПредъявленный
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ВтОстаткиНДСПартииИРасходыПредыдущихМесяцевПоВНАКвартал.ДокументПоступленияРасходов КАК СчетФактура,
	|	СУММА(ВтОстаткиНДСПартииИРасходыПредыдущихмесяцевПоВНАКвартал.НДСРегл) КАК НДСРегл
	|ПОМЕСТИТЬ ВтСуммаНДСПартииИРасходыСПредыдущихМесяцахПоВНА
	|ИЗ
	|	ВтОстаткиНДСПартииИРасходыПредыдущихМесяцевПоВНАКвартал КАК ВтОстаткиНДСПартииИРасходыПредыдущихМесяцевПоВНАКвартал
	|ГДЕ
	|	&РаспределениеЗаПолныйПериод
	|
	|СГРУППИРОВАТЬ ПО
	|	ВтОстаткиНДСПартииИРасходыПредыдущихМесяцевПоВНАКвартал.ДокументПоступленияРасходов
	|
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
    |ВЫБРАТЬ
	|	ВтОстаткиНДСПартииИРасходыПредыдущихМесяцевПоВНАМесяц.ДокументПоступленияРасходов КАК СчетФактура,
	|	СУММА(ВтОстаткиНДСПартииИРасходыПредыдущихмесяцевПоВНАМесяц.НДСРегл) КАК НДСРегл
	|ИЗ
	|	ВтОстаткиНДСПартииИРасходыПредыдущихМесяцевПоВНАМесяц КАК ВтОстаткиНДСПартииИРасходыПредыдущихМесяцевПоВНАМесяц
	|ГДЕ
	|	НЕ &РаспределениеЗаПолныйПериод
	|
	|СГРУППИРОВАТЬ ПО
	|	ВтОстаткиНДСПартииИРасходыПредыдущихМесяцевПоВНАМесяц.ДокументПоступленияРасходов
	|ИНДЕКСИРОВАТЬ ПО
	|	СчетФактура
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ЕСТЬNULL(НДСПредъявленныйСФ.СуммаНДС, 0) = Партии.НДСРегл КАК ВычетНеЗаявлялся,
	|	Партии.СчетФактура КАК СчетФактура
	|ПОМЕСТИТЬ ВтДанныеВычета
	|ИЗ
	|	ВтСуммаНДСПредъявленныйСФ КАК НДСПредъявленныйСФ
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВтСуммаНДСПартииИРасходыСПредыдущихМесяцахПоВНА КАК Партии
	|		ПО Партии.СчетФактура = НДСПредъявленныйСФ.СчетФактура
	|;
	|
	|УНИЧТОЖИТЬ ВтСуммаНДСПартииИРасходыСПредыдущихМесяцахПоВНА
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|	Т.СтавкаНДС КАК СтавкаНДС,
	|	Т.ВидДеятельностиНДС КАК ВидДеятельностиНДС,
	|	Т.НалогообложениеНДСПартии КАК НалогообложениеНДСПартии,
	|	Т.ВариантРаспределенияРасходовСтатьи КАК ВариантРаспределенияРасходовСтатьи,
	|	Т.ЭтоОстатки КАК ЭтоОстатки,
	|	Т.СтоимостьРегл КАК СтоимостьРегл,
	|	Т.НДСРегл КАК НДСРегл,
	|	Т.НДСУпр КАК НДСУпр
	|ПОМЕСТИТЬ ВтПартииСменаОблагаемойНаНеоблагаемуюВычетЗаявлялся
	|ИЗ
	|	ВтОстаткиНДСПартииИРасходыПредыдущихМесяцевПоВНАКвартал КАК Т
	|
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ втВнеоборотныеАктивыКвартал КАК ВнеобАктивы
	|	ПО Т.АналитикаРасходов = ВнеобАктивы.Объект
	|
	|	ЛЕВОЕ СОЕДИНЕНИЕ ВтДанныеВычета КАК ДанныеВычета
	|	ПО Т.ДокументПоступленияРасходов = ДанныеВычета.СчетФактура
	|		И ДанныеВычета.ВычетНеЗаявлялся
	|	
	|ГДЕ
	|	ДанныеВычета.СчетФактура ЕСТЬ NULL
	|	И Т.НалогообложениеНДСПартии = ЗНАЧЕНИЕ(Перечисление.ТипыНалогообложенияНДС.ПродажаОблагаетсяНДС)
	|	И (ВнеобАктивы.РаспределятьНДС
	|		ИЛИ ВнеобАктивы.НалогообложениеНДС = ЗНАЧЕНИЕ(Перечисление.ТипыНалогообложенияНДС.ПродажаНеОблагаетсяНДС))
	|	И НЕ ИСТИНА В
	|				(ВЫБРАТЬ ПЕРВЫЕ 1
	|					ИСТИНА
	|				ИЗ
	|					ВтПартииОстаткиИРасходы_ПредварительнаяКвартал КАК Партии
	|				ГДЕ
	|					Партии.АналитикаРасходов = Т.АналитикаРасходов
	|					И Партии.ДокументПоступленияРасходов = Т.ДокументПоступленияРасходов
	|					И Партии.ЭтоФинальноеРаспределение)
	|	
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	АналитикаРасходов,
	...
	|	Т.НДСРегл КАК НДСРегл,
	|	Т.НДСУпр КАК НДСУпр
	|ПОМЕСТИТЬ ВтПартииСменаОблагаемойНаНеоблагаемуюВычетНеЗаявлялся
	|ИЗ
	|	ВтОстаткиНДСПартииИРасходыПредыдущихМесяцевПоВНАМесяц КАК Т
	|
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ втВнеоборотныеАктивы КАК ВнеобАктивы
	|	ПО Т.АналитикаРасходов = ВнеобАктивы.Объект
	|
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВтДанныеВычета КАК ДанныеВычета
	|	ПО Т.ДокументПоступленияРасходов = ДанныеВычета.СчетФактура
	|		И ДанныеВычета.ВычетНеЗаявлялся
	|	
	|ГДЕ
	|	Т.НалогообложениеНДСПартии = ЗНАЧЕНИЕ(Перечисление.ТипыНалогообложенияНДС.ПродажаОблагаетсяНДС)
	|	И ВнеобАктивы.НалогообложениеНДС = ЗНАЧЕНИЕ(Перечисление.ТипыНалогообложенияНДС.ПродажаНеОблагаетсяНДС)
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	АналитикаРасходов,
	|	ДокументПоступленияРасходов
	...
	|	Т.НДСРегл КАК НДСРегл,
	|	Т.НДСУпр КАК НДСУпр
	|ПОМЕСТИТЬ ВтПартииСменаОблагаемойНаРаспределяемуюВычетНеЗаявлялся
	|ИЗ
	|	ВтОстаткиНДСПартииИРасходыПредыдущихМесяцевПоВНАКвартал КАК Т
	|
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ втВнеоборотныеАктивыКвартал КАК ВнеобАктивы
	...
	|;
	|
	|УНИЧТОЖИТЬ ВтПартииОстаткиИРасходы_ПредварительнаяКвартал
	|;
	|
	|УНИЧТОЖИТЬ ВтДанныеВычета
	|;
	|
	|УНИЧТОЖИТЬ ВтОстаткиНДСПартииИРасходыПредыдущихМесяцевПоВНАКвартал
	|;
	|
	|УНИЧТОЖИТЬ ВтОстаткиНДСПартииИРасходыПредыдущихМесяцевПоВНАМесяц
	|;
    |
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	...
	|	Партии.ЭтоОстатки                  КАК ЭтоОстатки,
	|
	|	ЛОЖЬ                               КАК РаспределятьНДС,
	|	Партии.ПериодБазы                  КАК ПериодБазы,
	|	НЕОПРЕДЕЛЕНО                       КАК ОснованиеКорректировки,
	|	Партии.ВидДеятельностиНДС          КАК НовоеНалогообложениеНДС,
	|	Партии.АналитикаРасходов           КАК ВнеоборотныйАктив,
	|	
	|	ИСТИНА                             КАК ВключаетсяВРасходыНУПриПринятииКУчету,
	|	НЕОПРЕДЕЛЕНО                       КАК ПодразделениеРасходовНУ,
	|	НЕОПРЕДЕЛЕНО                       КАК СтатьяРасходовНУ,
	|	НЕОПРЕДЕЛЕНО                       КАК АналитикаРасходовНУ,
	|
	...
	|ПОМЕСТИТЬ ВтОсобыеУсловияВводаВЭксплуатацию
	|ИЗ
	|	ВтПартииСменаОблагаемойНаНеоблагаемуюВычетЗаявлялся КАК Партии
	|ГДЕ
	|	(Партии.ВариантРаспределенияРасходовСтатьи = ЗНАЧЕНИЕ(Перечисление.ВариантыРаспределенияРасходов.НаВнеоборотныеАктивы)
	|		ИЛИ Партии.НалогообложениеНДСПартии = ЗНАЧЕНИЕ(Перечисление.ТипыНалогообложенияНДС.ВводОСВЭксплуатацию))
	...
	|ИЗ
	|	ВтПартииСменаОблагаемойНаНеоблагаемуюВычетНеЗаявлялся КАК Партии
	|ГДЕ
	|	(Партии.ВариантРаспределенияРасходовСтатьи = ЗНАЧЕНИЕ(Перечисление.ВариантыРаспределенияРасходов.НаВнеоборотныеАктивы)
	|		ИЛИ Партии.НалогообложениеНДСПартии = ЗНАЧЕНИЕ(Перечисление.ТипыНалогообложенияНДС.ВводОСВЭксплуатацию))
	|
	...
	|	Партии.НДСУпр                      КАК НДСУпр
	|ИЗ
	|	ВтПартииНДСПоВНАВведеннымВЭксплуатациюВЭтомМесяце КАК Партии
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	...
	|	Партии.НДСУпр                      КАК НДСУпр
	|ИЗ
	|	ВтПартииСменаОблагаемойНаРаспределяемуюВычетНеЗаявлялся КАК Партии
	|
	|ГДЕ
	|	(Партии.ВариантРаспределенияРасходовСтатьи = ЗНАЧЕНИЕ(Перечисление.ВариантыРаспределенияРасходов.НаВнеоборотныеАктивы)
	|		ИЛИ Партии.НалогообложениеНДСПартии = ЗНАЧЕНИЕ(Перечисление.ТипыНалогообложенияНДС.ВводОСВЭксплуатацию))
	...
	|ОБЪЕДИНИТЬ ВСЕ
	|
	// Общие условия распределения НДС при вводе в эксплуатацию ОС и НМА
	|ВЫБРАТЬ
	|	ТаблицаОстатков.Организация                      КАК Организация,
	|	ТаблицаОстатков.Подразделение                    КАК Подразделение,
	|	ТаблицаОстатков.НаправлениеДеятельности          КАК НаправлениеДеятельности,
	...
	|					ИСТИНА
	|				ИЗ
	|					ВтОсобыеУсловияВводаВЭксплуатацию КАК ВтОсобыеУсловияВводаВЭксплуатацию
	|				ГДЕ
	|					ТаблицаОстатков.АналитикаРасходов = ВтОсобыеУсловияВводаВЭксплуатацию.АналитикаРасходов
	|					И ТаблицаОстатков.ДокументПоступленияРасходов = ВтОсобыеУсловияВводаВЭксплуатацию.ДокументПоступленияРасходов
	|					И ТаблицаОстатков.Организация = ВтОсобыеУсловияВводаВЭксплуатацию.Организация
	|					И ТаблицаОстатков.Подразделение = ВтОсобыеУсловияВводаВЭксплуатацию.Подразделение
	|					И ТаблицаОстатков.СтатьяРасходов = ВтОсобыеУсловияВводаВЭксплуатацию.СтатьяРасходов
	|					И ТаблицаОстатков.АналитикаАктивовПассивов = ВтОсобыеУсловияВводаВЭксплуатацию.АналитикаАктивовПассивов
	|					И ТаблицаОстатков.Поставщик = ВтОсобыеУсловияВводаВЭксплуатацию.Поставщик
	|					И ТаблицаОстатков.ВидЦенности = ВтОсобыеУсловияВводаВЭксплуатацию.ВидЦенности
	|					И ТаблицаОстатков.НаправлениеДеятельности = ВтОсобыеУсловияВводаВЭксплуатацию.НаправлениеДеятельности
	|					И ТаблицаОстатков.СтавкаНДС = ВтОсобыеУсловияВводаВЭксплуатацию.СтавкаНДС
	|					И ТаблицаОстатков.ВидДеятельностиНДС = ВтОсобыеУсловияВводаВЭксплуатацию.ВидДеятельностиНДС)
	...
	|	И НЕ (НЕ &РаспределениеЗаПолныйПериод
	|			И ТаблицаОстатков.ВидДеятельностиНДС = ЗНАЧЕНИЕ(Перечисление.ТипыНалогообложенияНДС.ПродажаОблагаетсяНДС)
	|			И (ВнеобАктивы.РаспределятьНДС
	|				ИЛИ ВнеобАктивы.НалогообложениеНДС = ЗНАЧЕНИЕ(Перечисление.ТипыНалогообложенияНДС.ПродажаНеОблагаетсяНДС)))
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	// Восстановление НДС и списание на расходы при смене вида деятельности 
	// с «Облагаемой» на «Распределяемую»/«Необлагаемую» в «Принятии к учету», если вычет заявлялся
	|ВЫБРАТЬ
	|	ТаблицаОстатков.Организация                      КАК Организация,
	|	ТаблицаОстатков.Подразделение                    КАК Подразделение,
	|	ТаблицаОстатков.НаправлениеДеятельности          КАК НаправлениеДеятельности,
	...
	|	ТаблицаОстатков.АналитикаРасходов                КАК АналитикаРасходов,
	//++ Локализация
	|	&СтатьяОтраженияРасходов                         КАК СтатьяОтраженияРасходов,
	|	&АналитикаОтраженияРасходов                      КАК АналитикаОтраженияРасходов,
	//-- Локализация
	|	ТаблицаОстатков.АналитикаАктивовПассивов         КАК АналитикаАктивовПассивов,
	|	ТаблицаОстатков.ДокументПоступленияРасходов      КАК ДокументПоступленияРасходов,
	|	ТаблицаОстатков.ВидДеятельностиНДС               КАК ВидДеятельностиНДС,
	...
	|	ВнеобАктивы.РаспределятьНДС                      КАК РаспределятьНДС,
	|	
	|	КОНЕЦПЕРИОДА(ВнеобАктивы.ДатаПринятияКУчету, КВАРТАЛ) КАК ПериодБазы,
	|	ЕСТЬNULL(ПартииНДСКРаспределениюКорректировка.ОснованиеКорректировки, НЕОПРЕДЕЛЕНО) КАК ОснованиеКорректировки,
	|	ЗНАЧЕНИЕ(Справочник.НастройкиХозяйственныхОпераций.РаспределениеНДС) КАК НастройкаХозяйственнойОперации,
	|	
	|	ВЫБОР 
	...
	|	ТаблицаОстатков.НДСУпр                            КАК НДСУпр
	|ИЗ
	|	ВтПартииСменаОблагаемойНаНеоблагаемуюВычетЗаявлялся КАК ТаблицаОстатков
	|	
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ 
	|		ВтВнеоборотныеАктивыКвартал КАК ВнеобАктивы
	|	ПО 
	|		ТаблицаОстатков.АналитикаРасходов = ВнеобАктивы.Объект
	|	
	|	ЛЕВОЕ СОЕДИНЕНИЕ
	...
	|ОБЪЕДИНИТЬ ВСЕ
	|
	// Включение НДС в стоимость при смене вида деятельности 
	// с «Облагаемой» на «Необлагаемую» в «Принятии к учету», если вычет не заявлялся
	|ВЫБРАТЬ
	|	ТаблицаОстатков.Организация                      КАК Организация,
	|	ТаблицаОстатков.Подразделение                    КАК Подразделение,
	|	ТаблицаОстатков.НаправлениеДеятельности          КАК НаправлениеДеятельности,
	...
	|	ЗНАЧЕНИЕ(Справочник.НастройкиХозяйственныхОпераций.РаспределениеНДС) КАК НастройкаХозяйственнойОперации,
	|	
	|	ВнеобАктивы.НалогообложениеНДС                    КАК НовоеНалогообложениеНДС,
	|	ВнеобАктивы.ВнеоборотныйАктив                     КАК ВнеоборотныйАктив,
	|	
	|	ВнеобАктивы.ВключаетсяВРасходыНУПриПринятииКУчету КАК ВключаетсяВРасходыНУПриПринятииКУчету,
	|	ВнеобАктивы.ПодразделениеРасходовНУ               КАК ПодразделениеРасходовНУ,
	...
	|	ТаблицаОстатков.НДСУпр                            КАК НДСУпр
	|ИЗ
	|	ВтПартииСменаОблагаемойНаНеоблагаемуюВычетНеЗаявлялся КАК ТаблицаОстатков
	|	
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ 
	|		ВтВнеоборотныеАктивы КАК ВнеобАктивы
	|	ПО 
	...
	|ОБЪЕДИНИТЬ ВСЕ
	|
	// Включение в стоимость в пропорции месяца поступления на 08 счете при смене деятельности 
	// с «Облагаемой» на «Распределяемую» в «Принятии к учету», если вычет не заявлялся
	|ВЫБРАТЬ
	|	ТаблицаОстатков.Организация                      КАК Организация,
	|	ТаблицаОстатков.Подразделение                    КАК Подразделение,
	|	ТаблицаОстатков.НаправлениеДеятельности          КАК НаправлениеДеятельности,
	...
	|	ТаблицаОстатков.НДСУпр                            КАК НДСУпр
	|ИЗ
	|	ВтПартииСменаОблагаемойНаРаспределяемуюВычетНеЗаявлялся КАК ТаблицаОстатков
	|	
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ 
	|		ВтВнеоборотныеАктивыКвартал КАК ВнеобАктивы
	|	ПО 
	...
	|
	// Включение в стоимость в пропорции месяца поступления на 08 счете при смене деятельности 
	// с «Облагаемой» на «Распределяемую» в «Принятии к учету» в пропорции месяца, если вычет не заявлялся
	|ВЫБРАТЬ
	|	ТаблицаОстатков.Организация                      КАК Организация,
	|	ТаблицаОстатков.Подразделение                    КАК Подразделение,
	|	ТаблицаОстатков.НаправлениеДеятельности          КАК НаправлениеДеятельности,
	...
	|ОБЪЕДИНИТЬ ВСЕ
	|
	// Распределение в пропорции месяца поступления на 08 счете при смене деятельности 
	// с «Необлагаемой» на «Распределяемую»/«Облагаемую» в «Принятии к учету»
	|ВЫБРАТЬ
	|	ПартииНДСКРаспределению.Организация                 КАК Организация,
	|	ПартииНДСКРаспределению.Подразделение               КАК Подразделение,
	|	ПартииНДСКРаспределению.НаправлениеДеятельности     КАК НаправлениеДеятельности,
	...
	|	ПартииНДСКРаспределению.ВидДеятельностиНДС          КАК НалогообложениеНДСПартии,
	|	ПартииНДСКРаспределению.СтавкаНДС                   КАК СтавкаНДС,
	|	ПартииНДСКРаспределению.РаспределятьНДС             КАК РаспределятьНДС,
	|	ПартииНДСКРаспределению.ПериодБазы                  КАК ПериодБазы,
	|	ПартииНДСКРаспределению.ОснованиеКорректировки      КАК ОснованиеКорректировки,
	|	ЗНАЧЕНИЕ(Справочник.НастройкиХозяйственныхОпераций.РаспределениеНДС) КАК НастройкаХозяйственнойОперации,
	|	ПартииНДСКРаспределению.НовоеНалогообложениеНДС     КАК НовоеНалогообложениеНДС,
	|	ПартииНДСКРаспределению.ВнеоборотныйАктив           КАК ВнеоборотныйАктив,
	|	
	|	ПартииНДСКРаспределению.ВключаетсяВРасходыНУПриПринятииКУчету   КАК ВключаетсяВРасходыНУПриПринятииКУчету,
	|	ПартииНДСКРаспределению.ПодразделениеРасходовНУ                 КАК ПодразделениеРасходовНУ,
	|	ПартииНДСКРаспределению.СтатьяРасходовНУ                        КАК СтатьяРасходовНУ,
	|	ПартииНДСКРаспределению.АналитикаРасходовНУ                     КАК АналитикаРасходовНУ,
	|	
	|	ПартииНДСКРаспределению.ВидЦенности                 КАК ВидЦенности,
	|	ПартииНДСКРаспределению.СтоимостьРегл               КАК СтоимостьРегл,
	|	ПартииНДСКРаспределению.НДСРегл                     КАК НДСРегл,
	|	ПартииНДСКРаспределению.НДСУпр                      КАК НДСУпр
	|ИЗ
	|	ВтПартииНДСПоВНАВведеннымВЭксплуатациюВЭтомМесяце КАК ПартииНДСКРаспределению
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	// Восстановление партий под особые условия ввода в эксплуатацию
	|ВЫБРАТЬ
	|	ПартииНДСКРаспределению.Организация                 КАК Организация,
	|	ПартииНДСКРаспределению.Подразделение               КАК Подразделение,
	|	ПартииНДСКРаспределению.НаправлениеДеятельности     КАК НаправлениеДеятельности,
	...
	|	-ПартииНДСКРаспределению.НДСУпр                     КАК НДСУпр
	|ИЗ
	|	ВтОсобыеУсловияВводаВЭксплуатацию КАК ПартииНДСКРаспределению
	|ГДЕ
	|	НЕ ПартииНДСКРаспределению.ЭтоОстатки
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	...
	|	ТаблицаОстатков.СтавкаНДС                        КАК СтавкаНДС,
	|	
	|	ВЫБОР ТаблицаОстатков.НалогообложениеНДСПартии
	|		КОГДА ЗНАЧЕНИЕ(Перечисление.ТипыНалогообложенияНДС.ВводОСВЭксплуатацию)
	|			ТОГДА ИСТИНА
	|		КОГДА ЗНАЧЕНИЕ(Перечисление.ТипыНалогообложенияНДС.ОпределяетсяРаспределением)
	|			ТОГДА ИСТИНА
	|		ИНАЧЕ ЛОЖЬ
	...
	|		КОГДА ЗНАЧЕНИЕ(Перечисление.ТипыНалогообложенияНДС.ОпределяетсяРаспределением)
	|			ТОГДА НЕОПРЕДЕЛЕНО
	|		КОГДА ЗНАЧЕНИЕ(Перечисление.ТипыНалогообложенияНДС.ПродажаНеОблагаетсяНДС)
	|			ТОГДА ЗНАЧЕНИЕ(Перечисление.ТипыНалогообложенияНДС.ПродажаНеОблагаетсяНДС)
	|		КОГДА ЗНАЧЕНИЕ(Перечисление.ТипыНалогообложенияНДС.ЛьготноеНалогообложениеНДСНаУСН)
	|			ТОГДА ЗНАЧЕНИЕ(Перечисление.ТипыНалогообложенияНДС.ПродажаНеОблагаетсяНДС)
	|		ИНАЧЕ ЗНАЧЕНИЕ(Перечисление.ТипыНалогообложенияНДС.ПродажаОблагаетсяНДС)
	|	КОНЕЦ                                            КАК НовоеНалогообложениеНДС,
	|	ВЫБОР
	...
	|///////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ОстаткиНДСПредъявленный.СчетФактура,
	|	ОстаткиНДСПредъявленный.СуммаНДС
	|ИЗ
	|	ВтСуммаНДСПредъявленныйСФ КАК ОстаткиНДСПредъявленный
	//-- НЕ УТ
	|";
	...
	Результат.Вставить("ОстаткиНДСПредъявленный", РезультатыЗапроса[2].Выгрузить());
	//-- НЕ УТ
	...
	|;
	|
	|УНИЧТОЖИТЬ ВтОсобыеУсловияВводаВЭксплуатацию
	|;
	|
	|УНИЧТОЖИТЬ ВтВнеоборотныеАктивыКвартал
	|;
	|
	|УНИЧТОЖИТЬ ВтПартииНДСПоВНАВведеннымВЭксплуатациюВЭтомМесяце
	|;
	|
	|УНИЧТОЖИТЬ ВтПартииСменаОблагаемойНаНеоблагаемуюВычетЗаявлялся
	|;
	|
	|УНИЧТОЖИТЬ ВтПартииСменаОблагаемойНаРаспределяемуюВычетНеЗаявлялся
	|;
	|
	|УНИЧТОЖИТЬ ВтСуммаНДСПредъявленныйСФ
	|;
	//-- НЕ УТ
...
Функция СформироватьДвижения(Реквизиты, Регистры, ДопПараметры)
	...
	Если ВыполнятьРаспределение Тогда
		...
		РезультатРаспределения = РаспределитьПартииНДСПоБазе(ДанныеДляРаспределения, Реквизиты);
...
Функция РаспределитьПартииНДСПоБазе(ДанныеДляРаспределения, Реквизиты)
			...
			Если Таблица = Неопределено Тогда
				ТекстСообщения = СтрШаблон(
					НСтр("ru = 'Не определено налогообложение по внеоборотному активу %1.';
						|en = 'Taxation by fixed asset %1 is not defined.'"), 
...
Функция ТаблицаПрочиеРасходы(РезультатРаспределения, Реквизиты, ПартииНДСКРаспределению)
	...
	Если Реквизиты.ИспользоватьВНА2_4
		...
		Для Каждого Строка Из ПартииНДСКРаспределению Цикл
			...
			Если Строка.НДСРегл < 0
				И Строка.КорВидДеятельностиНДС = Перечисления.ТипыНалогообложенияНДС.ПустаяСсылка() Тогда
