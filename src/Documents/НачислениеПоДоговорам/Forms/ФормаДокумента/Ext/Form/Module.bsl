...
Процедура ДанныеДляЗаполненияВДанныеФормы(ДанныеЗаполнения, ПозицииВставки = Неопределено)
	...
	Документ.РасчетЗарплатыВДанныeОбъекта(ДанныеЗаполнения, ПозицииВставки);
	
	ЗначениеВРеквизитФормы(Документ, "Объект");
	ДанныеВРеквизиты(Документ);
	
	ЗаполненныеФизлица = ?(ЗаполнениеВыполнено = Неопределено, Новый Соответствие, Новый Соответствие(ЗаполнениеВыполнено));
	Для Каждого Строка Из ДанныеЗаполнения.НачисленияПоДоговорам Цикл
		ЗаполненныеФизлица.Вставить(Строка.ФизическоеЛицо, Истина);
	КонецЦикла;
	ЗаполнениеВыполнено = Новый ФиксированноеСоответствие(ЗаполненныеФизлица);
