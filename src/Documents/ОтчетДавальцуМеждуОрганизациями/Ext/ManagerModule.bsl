...
Функция ЗапросОстаткиОтчетовКОформлению(Параметры) Экспорт
	...
	|	
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.РеестрДокументов КАК РеквизитыДокументаПартии
	|	ПО РеквизитыДокументаПартии.Ссылка = СпрПартииПроизводства.Документ
	|	И РеквизитыДокументаПартии.Проведен
	|	
	|	ЛЕВОЕ СОЕДИНЕНИЕ Документ.ПроизводствоБезЗаказа КАК РеквизитыПартииПроизводстваБезЗаказа
	|	ПО РеквизитыПартииПроизводстваБезЗаказа.Ссылка = СпрПартииПроизводства.Документ
	|	
	...
	|	И НЕ ЕСТЬNULL(РеквизитыПартииПроизводстваБезЗаказа.ИсправлениеСохраняетПартииПроизводства, ЛОЖЬ)
	|	
	|	ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.РеестрДокументов КАК СторноСохраняемойПартии
	|	ПО СторноСохраняемойПартии.ИсправляемыйДокумент = СпрПартииПроизводства.Документ
	|	И СторноСохраняемойПартии.Проведен
	|	И ЕСТЬNULL(РеквизитыПартииПроизводстваБезЗаказа.ИсправлениеСохраняетПартииПроизводства, ЛОЖЬ)
	|	И ТИПЗНАЧЕНИЯ(СторноСохраняемойПартии.Ссылка) = ТИП(Документ.Сторно)
	|	
	...
	|		И СторноСохраняемойПартии.Ссылка ЕСТЬ NULL
	// Партия производства, без сохранения при исправлении, исправлена/отсторнирована в следующих месяцах
	|		ИЛИ СторноПартии.Ссылка ЕСТЬ НЕ NULL
	|			И НАЧАЛОПЕРИОДА(СторноПартии.ДатаДокументаИБ, МЕСЯЦ) > НАЧАЛОПЕРИОДА(РеквизитыДокументаПартии.ДатаДокументаИБ, МЕСЯЦ)
	// и есть не отсторнированный отчет
	|			И ТоварыОтчетовДавальцу.Ссылка ЕСТЬ НЕ NULL
	// Партия производства, с сохранением при исправлении, отсторнирована в следующих месяцах
	|		ИЛИ СторноСохраняемойПартии.Ссылка ЕСТЬ НЕ NULL
	|			И НАЧАЛОПЕРИОДА(СторноСохраняемойПартии.ДатаДокументаИБ, МЕСЯЦ) > НАЧАЛОПЕРИОДА(РеквизитыДокументаПартии.ДатаДокументаИБ, МЕСЯЦ)
	// и есть не отсторнированный отчет
	|			И ТоварыОтчетовДавальцу.Ссылка ЕСТЬ НЕ NULL)
...
Процедура ЗаполнитьПараметрыИнициализации(Запрос, ДокументСсылка)
	...
	ПараметрыУчетнойПолитикиДавальца = НастройкиНалоговУчетныхПолитик.ДействующиеПараметрыНалоговУчетныхПолитикНаДату("УчетнаяПолитикаФинансовогоУчета",
		Реквизиты.ОрганизацияПолучатель,
		Реквизиты.Период);
	СреднеСкользящаяУДавальца = (ПараметрыУчетнойПолитикиДавальца <> Неопределено
		И ПараметрыУчетнойПолитикиДавальца.МетодОценкиСтоимостиТоваров = Перечисления.МетодыОценкиСтоимостиТоваров.Среднескользящая);
	Запрос.УстановитьПараметр("СреднеСкользящаяУДавальца", СреднеСкользящаяУДавальца);
...
Функция ОписаниеОперацийУчетаСебестоимости(Документ) Экспорт
	...
	|	ВтТовары.Количество КАК Количество,
	|	(ВЫБОР
	|		КОГДА &СреднеСкользящаяУДавальца ТОГДА ВтТовары.Стоимость
	|		ИНАЧЕ 0 КОНЕЦ)		КАК Стоимость,
	|	(ВЫБОР
	|		КОГДА &СреднеСкользящаяУДавальца ТОГДА ВтТовары.СтоимостьБезНДС
	|		ИНАЧЕ 0 КОНЕЦ)		КАК СтоимостьБезНДС,
	|	(ВЫБОР
	|		КОГДА &СреднеСкользящаяУДавальца ТОГДА ВтТовары.СтоимостьРегл
	|		ИНАЧЕ 0 КОНЕЦ)		КАК СтоимостьРегл,
	|	(ВЫБОР
	|		КОГДА &СреднеСкользящаяУДавальца ТОГДА ВтТовары.СтоимостьУпр
	...
	|	(ВЫБОР
	|		КОГДА &СреднеСкользящаяУДавальца ТОГДА ЗНАЧЕНИЕ(Перечисление.СпособыОпределенияСебестоимостиТоваров.ИзДокумента)
	|		ИНАЧЕ ЗНАЧЕНИЕ(Перечисление.СпособыОпределенияСебестоимостиТоваров.ПустаяСсылка) КОНЕЦ)	КАК СпособОпределенияСебестоимости
	|
	|ИЗ
	|	Документ.ОтчетДавальцуМеждуОрганизациями КАК ТаблицаДокумента
