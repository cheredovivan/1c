...
Функция ПолучитьРезультатЗапросаПоОстаткамЗаказов(ДанныеОтбора, 
	...
	|			РаспоряженияОбороты.ОтгруженоКоличествоОборот) КАК Заказано
	|	ИЗ
	...
	|				ТОГДА -1
	|			ИНАЧЕ 1
	|		КОНЕЦ * РаспоряженияДвижения.Количество КАК Количество
	|	ИЗ
	...
	ПараметрыФормированияТаблицы.КонецПериода = "&ГраницаПериод";
