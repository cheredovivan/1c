...
Процедура УстановитьУсловноеОформление()
	...
	// обязательность заполнения сотрудника в зависимости от распоряжения
	
	Элемент = УсловноеОформление.Элементы.Добавить();
	
	ПолеЭлемента = Элемент.Поля.Элементы.Добавить();
	ПолеЭлемента.Поле = Новый ПолеКомпоновкиДанных(Элементы.ВидыРаботСотрудник.Имя);
	
	ОтборЭлемента = Элемент.Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
	ОтборЭлемента.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("Объект.ВидыРабот.ДатаРаспоряжения");
	ОтборЭлемента.ВидСравнения = ВидСравненияКомпоновкиДанных.Заполнено;
	
	ОтборЭлемента = Элемент.Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
	ОтборЭлемента.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("Объект.ВидыРабот.ДатаРаспоряжения");
	ОтборЭлемента.ВидСравнения = ВидСравненияКомпоновкиДанных.Меньше;
	ОтборЭлемента.ПравоеЗначение = ПолучитьФункциональнуюОпцию("УчетТрудозатратВРазрезеСотрудников");
	
	ОтборЭлемента = Элемент.Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
	ОтборЭлемента.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("Объект.ВидыРабот.Сотрудник");
	ОтборЭлемента.ВидСравнения = ВидСравненияКомпоновкиДанных.НеЗаполнено;
...
Процедура УстановитьВидимостьИДоступность()
	...
	Элементы.ФормаЗаполнитьДокумент.Доступность = Не ТолькоПросмотр;
	Элементы.ВидыРаботПодобратьВидыРабот.Доступность = Не ТолькоПросмотр;
	Элементы.ПоказатьОтклонения.Доступность = Не ТолькоПросмотр;
	Элементы.СотрудникиЗаполнитьКадровыеДанные.Доступность = Не ТолькоПросмотр;
	...
	Если Объект.УчетТрудозатратВРазрезеСотрудников И МассивРеквизитовОперации.Найти("ВидыРабот.Сотрудник") <> Неопределено
		И МассивРеквизитовОперации.Найти("ВидыРабот.Работник") <> Неопределено Тогда
		Элементы.ВидыРаботСотрудник.АвтоОтметкаНезаполненного = Ложь;
	КонецЕсли;
...
Процедура ЗаполнитьРасценкуНаСервере(Идентификатор)
	...
	ДанныеСтроки.Расценка = Справочники.ВидыРаботСотрудников.ДействующаяРасценкаВидаРабот(ДанныеСтроки.ВидРабот,
		Объект.КонецПериода,
		ДанныеСтроки.КвалификацияИсполнителя,
		Объект.Подразделение,
		Объект.Организация);
...
&НаСервере
Процедура ЗаполнитьДатыРаспоряжений()
	...
	Если Объект.УчетТрудозатратВРазрезеСотрудников Тогда
		...
		Для Каждого Строка Из Объект.ВидыРабот Цикл
			Если ТипЗнч(Строка.Распоряжение) = Тип("ДокументСсылка.ПроизводствоБезЗаказа") Тогда
				Распоряжения.Добавить(Строка.Распоряжение);
			КонецЕсли;
		КонецЦикла;
		
		ДатыРаспоряжений = ОбщегоНазначения.ЗначениеРеквизитаОбъектов(Распоряжения, "Дата");
		
		Для каждого Строка Из Объект.ВидыРабот Цикл
			Если ТипЗнч(Строка.Распоряжение) = Тип("ДокументСсылка.ПроизводствоБезЗаказа") Тогда
				Строка.ДатаРаспоряжения = ДатыРаспоряжений.Получить(Строка.Распоряжение);
			КонецЕсли;
		КонецЦикла;
