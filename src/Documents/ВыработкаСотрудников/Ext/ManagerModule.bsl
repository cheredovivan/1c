...
Процедура СформироватьДокументы(СтруктураОтбора, АдресРезультата, УдалятьДокументыСОшибками = Неопределено) Экспорт
	...
	Если УдалятьДокументыСОшибками = Истина Тогда
		...
		Для Каждого Строка Из СписокОшибок Цикл
			...
			Если Строка.ТипОшибки = "ДокументСуществует"
				Или Не ОбработанныеДокументы[Строка.Документ] = Неопределено Тогда
				Продолжить;
			КонецЕсли;
			
			ОбработанныеДокументы.Вставить(Строка.Документ, Истина);
...
Функция ТрудозатратыКОформлению(СтруктураОтбора, МенеджерВременныхТаблиц = Неопределено) Экспорт
	...
	Если СтруктураОтбора.Свойство("Организация") И ЗначениеЗаполнено(СтруктураОтбора.Организация) Тогда
		Если ТипЗнч(СтруктураОтбора.Организация) = Тип("Массив") Тогда
			Организации = СтруктураОтбора.Организация;
		Иначе
	...
	Запрос.УстановитьПараметр("УчетТрудозатратВРазрезеСотрудников", ПроизводствоСервер.ИспользуетсяУчетТрудозатратВРазрезеСотрудников(
		?(ЗначениеЗаполнено(НачалоПериода), НачалоПериода, ТекущаяДатаСеанса())));
	...
	//-- НЕ УТКА
	Запрос.УстановитьПараметр("ИспользоватьРазрядыРасценокРаботСотрудников", ИспользоватьРазрядыРасценокРаботСотрудников);
	
	//++ Локализация
	
	//++ НЕ УТКА
	Запрос.УстановитьПараметр("НормированиеИРасценкаРаботПоТарифнымСеткам", ПолучитьФункциональнуюОпцию("НормированиеИРасценкаРаботПоТарифнымСеткам"));
	Запрос.УстановитьПараметр("ДополнительныйРазрезТарифнойСетки", Константы.ДополнительныйРазрезТарифнойСетки.Получить());
	Запрос.УстановитьПараметр("ИспользоватьТарифныеСеткиПриРасчетеЗарплаты", ПолучитьФункциональнуюОпцию("ИспользоватьТарифныеСеткиПриРасчетеЗарплаты"));
	//-- НЕ УТКА
	...
	|					ИЛИ Исполнитель В (&Бригады)
	|					ИЛИ ТИПЗНАЧЕНИЯ(Исполнитель) В(
	//++ Локализация
	|							ТИП(Справочник.Сотрудники),
	//-- Локализация
	|							ТИП(Справочник.ФизическиеЛица)))
	|				И (&БезОтбораПоРаботникам
	|					ИЛИ Исполнитель В (&Работники)
	|					ИЛИ ТИПЗНАЧЕНИЯ(Исполнитель) = ТИП(Справочник.Бригады))
	|	) КАК ДД
	|
	|СГРУППИРОВАТЬ ПО
	|	ДД.Организация,
	...
	|
	|////////////////////////////////////////////////////////////////////////////////
	// Получим распоряжения, по которым были оформлены исправления с сохранением партии производства.
	...
	|	МАКСИМУМ(ТрудозатратыКОформлению.ЕстьИсправления) КАК ЕстьИсправления,
	|	СУММА(ТрудозатратыКОформлению.Количество) КАК Количество
	|ПОМЕСТИТЬ Остатки
	|ИЗ
	|	(ВЫБРАТЬ
	|		ВЫБОР
	|			КОГДА &Периодичность = ЗНАЧЕНИЕ(Перечисление.ПериодичностьОформленияВыработкиСотрудников.ПустаяСсылка)
	|				ТОГДА СпрПодразделения.ПериодичностьОформленияВыработки
	|			ИНАЧЕ &Периодичность
	...
	|				ТОГДА НЕОПРЕДЕЛЕНО
	|		КОНЕЦ КАК Статус,
	|		ЛОЖЬ КАК ЕстьИсправления
	|	ИЗ
	|		РегистрНакопления.ТрудозатратыКОформлению.Остатки(
	|			,
	|			(Организация, Распоряжение, ВидРабот, Подразделение, Исполнитель) В(
	|				ВЫБРАТЬ
	|					Обороты.Организация,
	|					Обороты.Распоряжение,
	|					Обороты.ВидРабот,
	...
	|					Обороты.Исполнитель
	|				ИЗ
	|					Обороты КАК Обороты)) КАК ТрудозатратыКОформлению
	|		
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.СтруктураПредприятия КАК СпрПодразделения
	|		ПО СпрПодразделения.Ссылка = ТрудозатратыКОформлению.Подразделение
	|		
	// Исключаем распоряжения с исправлениями.
	|		ЛЕВОЕ СОЕДИНЕНИЕ РаспоряженияСИсправлением КАК РаспоряженияСИсправлением
	|		ПО РаспоряженияСИсправлением.Организация = ТрудозатратыКОформлению.Организация
	|		И РаспоряженияСИсправлением.ПартияПроизводства = ТрудозатратыКОформлению.ПартияПроизводства
	|		И РаспоряженияСИсправлением.Распоряжение = ТрудозатратыКОформлению.Распоряжение
	|		И РаспоряженияСИсправлением.КодСтрокиРаспоряжения = ТрудозатратыКОформлению.КодСтрокиРаспоряжения
	|		
	|	ГДЕ
	// Исключаем распоряжения с исправлениями.
	...
	|		КОНЕЦ КАК Периодичность,
	|		ТрудозатратыКОформлению.ВидРабот КАК ВидРабот,
	|		ТрудозатратыКОформлению.КоличествоОборот КАК Количество,
	|		ТрудозатратыКОформлению.Организация КАК Организация,
	|		ТрудозатратыКОформлению.Распоряжение КАК Распоряжение,
	|		ТрудозатратыКОформлению.Исполнитель КАК Исполнитель,
	|		ТрудозатратыКОформлению.Подразделение КАК Подразделение,
	...
	|				ТОГДА НЕОПРЕДЕЛЕНО
	|		КОНЕЦ КАК Статус,
	|		ИСТИНА КАК ЕстьИсправления
	|	ИЗ
	|		Обороты КАК ТрудозатратыКОформлению
	|		
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.СтруктураПредприятия КАК СпрПодразделения
	|		ПО СпрПодразделения.Ссылка = ТрудозатратыКОформлению.Подразделение
	|		
	...
	|	
	|	ГДЕ
	|		НЕ ТрудозатратыКОформлению.КоличествоОборот = 0
	|	
	|	ОБЪЕДИНИТЬ ВСЕ
	|	
	// Восстанавливаем остатки движениями текущего документа
	|	ВЫБРАТЬ
	|		ВЫБОР
	|			КОГДА &Периодичность = ЗНАЧЕНИЕ(Перечисление.ПериодичностьОформленияВыработкиСотрудников.ПустаяСсылка)
	|				ТОГДА СпрПодразделения.ПериодичностьОформленияВыработки
	...
	|				ТОГДА НЕОПРЕДЕЛЕНО
	|		КОНЕЦ КАК Статус,
	|		РаспоряженияСИсправлением.Распоряжение ЕСТЬ НЕ NULL КАК ЕстьИсправления
	|	ИЗ
	...
	|	ГДЕ
	|		(&БезОтбораПоОрганизациям
	|			ИЛИ ТрудозатратыКОформлению.Организация В (&Организации))
	|		И (&БезОтбораПоРаспоряжениям
	|			ИЛИ ТрудозатратыКОформлению.Распоряжение В (&Распоряжения))
	|		И (&БезОтбораПоВидамРабот
	|			ИЛИ ТрудозатратыКОформлению.ВидРабот В (&ВидыРабот))
	|		И (&БезОтбораПоПодразделениям
	|			ИЛИ ТрудозатратыКОформлению.Подразделение В (&Подразделения))
	|		И (&БезОтбораПоБригадам
	|			ИЛИ ТрудозатратыКОформлению.Исполнитель В (&Бригады)
	|			ИЛИ ТИПЗНАЧЕНИЯ(Исполнитель) = ТИП(Справочник.Бригады))
	|		И (&БезОтбораПоРаботникам
	|			ИЛИ ТрудозатратыКОформлению.Исполнитель В (&Работники)
	|			ИЛИ ТИПЗНАЧЕНИЯ(Исполнитель) = ТИП(Справочник.Бригады))
	|		И ТрудозатратыКОформлению.Регистратор = &ДокументИсключение
	|	) КАК ТрудозатратыКОформлению
	|
	|СГРУППИРОВАТЬ ПО
	|	ТрудозатратыКОформлению.Периодичность,
	...
	ПараметрыТекстаЗапроса.Отборы.Добавить("Очередь.Этап В (ВЫБРАТЬ РАЗЛИЧНЫЕ ДД.Распоряжение.Этап ИЗ Остатки КАК ДД)");
	ПараметрыТекстаЗапроса.ПоляИндексирования.Добавить("Этап");
	
	ТекстыЗапросов.Добавить(РегистрыСведений.ОчередьПроизводственныхОпераций.ТекстЗапросаОчередьОпераций(ПараметрыТекстаЗапроса));
	...
	|	Очередь.Запланировано - Очередь.ТребуетПовторения          КАК Запланировано,
	|	Очередь.Создано                                            КАК Создано,
	|	Очередь.Выполнено                                          КАК Выполнено,
	|	Очередь.Брак                                               КАК Брак
	|ПОМЕСТИТЬ ПоказателиВыполненияОпераций
	|ИЗ
	|	ВТОчередь КАК Очередь
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	...
	|	МАКСИМУМ(Операции.ИдентификаторКонтрольнойОперации)         КАК ИдентификаторКонтрольнойОперации,
	|	СУММА(Операции.Количество - Операции.КоличествоОтменено)    КАК Запланировано,
	|	СУММА(Операции.Количество - Операции.КоличествоОтменено)    КАК Создано,
	|	СУММА(Операции.КоличествоФакт)                              КАК Выполнено,
	|	СУММА(Операции.КоличествоБрак)                              КАК Брак
	|	
	|ИЗ
	...
	"ВЫБРАТЬ
	|	ВЫБОР
	|		КОГДА НЕ ТрудозатратыКОформлению.ЕстьИсправления
	|			ТОГДА ТрудозатратыПроизводстваБезЗаказа.Ссылка.Дата
	|		КОГДА ТрудозатратыКОформлению.ЕстьИсправления
	|			И ТрудозатратыПроизводстваБезЗаказа.Ссылка.Дата МЕЖДУ &НачалоПериода И &КонецПериода
	|			ТОГДА ТрудозатратыПроизводстваБезЗаказа.Ссылка.Дата
	|		ИНАЧЕ &КонецПериода
	...
	|	ВЫБОР
	|		КОГДА ТрудозатратыПроизводстваБезЗаказа.Ссылка.Исправление
	|			И ТрудозатратыПроизводстваБезЗаказа.Ссылка.ИсправлениеСохраняетПартииПроизводства
	|			ТОГДА ТрудозатратыПроизводстваБезЗаказа.Ссылка.ИсправляемыйДокумент
	|		ИНАЧЕ ТрудозатратыПроизводстваБезЗаказа.Ссылка
	...
	|	ПО СпрПартииПроизводства.Документ = ТрудозатратыПроизводстваБезЗаказа.ДокументПартии
	...
	|
	|	ЕСТЬNULL(Операции.Операция.Владелец, Операции.Ссылка)  КАК ВладелецТехнологическойОперации,
	|	ЕСТЬNULL(Операции.Операция.Спецификация, ЗНАЧЕНИЕ(Справочник.РесурсныеСпецификации.ПустаяСсылка)) КАК СпецификацияТехнологическойОперации,
	...
	|
	|	ЕСТЬNULL(Очередь.ИдентификаторКонтрольнойОперации, -1) КАК ИдентификаторКонтрольнойОперации,
	|	ЕСТЬNULL(Очередь.Создано,   0)                         КАК Создано,
	|	ЕСТЬNULL(Очередь.Выполнено, 0)                         КАК Выполнено,
	|	ЕСТЬNULL(Очередь.Запланировано, 0)                     КАК Запланировано
	|
	...
	|
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ПоказателиВыполненияОпераций КАК Очередь
	...
	|
	|ВЫБРАТЬ
	|	ВложенныйЗапрос.ВладелецТехнологическойОперации КАК ВладелецТехнологическойОперации,
	|	ВложенныйЗапрос.ВидРабот                        КАК ВидРабот,
	|	МАКСИМУМ(ВложенныйЗапрос.СтатьяКалькуляции)     КАК СтатьяКалькуляции
	|ПОМЕСТИТЬ ВтСтатьиКалькуляцииПоНСИ
	|ИЗ
	|	(ВЫБРАТЬ
	...
	|
	|СГРУППИРОВАТЬ ПО
	|	ВложенныйЗапрос.ВладелецТехнологическойОперации,
	|	ВложенныйЗапрос.ВидРабот
	|
	|ИМЕЮЩИЕ
	|	КОЛИЧЕСТВО(РАЗЛИЧНЫЕ ВложенныйЗапрос.СтатьяКалькуляции) = 1
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	ВладелецТехнологическойОперации,
	|	ВидРабот
	|
	|;
	|
	...
	|ИЗ
	|	Документ.ПроизводственнаяОперация2_2.Трудозатраты КАК Трудозатраты
	|	
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ ПроизводственныеОперации КАК Операции
	|	ПО Операции.Ссылка = Трудозатраты.Ссылка
	|		И НЕ Трудозатраты.СтатьяКалькуляции = ЗНАЧЕНИЕ(Справочник.СтатьиКалькуляции.ПустаяСсылка)
	|		И Трудозатраты.Ссылка.НаОснованииНСИ
	|	
	|	ОБЪЕДИНИТЬ ВСЕ
	|
	|	ВЫБРАТЬ
	...
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ПроизводственныеОперации КАК Операции
	|		ПО Операции.ВладелецТехнологическойОперации = Трудозатраты.Ссылка
	|			И НЕ Трудозатраты.Ссылка.НаОснованииНСИ
	|
	|	) КАК ВложенныйЗапрос
	|
	|СГРУППИРОВАТЬ ПО
	...
	|	ЕСТЬNULL(СтатьиКалькуляцииОпераций.СтатьяКалькуляции, СтатьиКалькуляцииПоНСИ.СтатьяКалькуляции) КАК СтатьяКалькуляции
	//++ Локализация
	|	,ЗНАЧЕНИЕ(Справочник.СпособыОтраженияЗарплатыВБухУчете.ПустаяСсылка) КАК СпособОтраженияЗарплаты
	//-- Локализация
	|ИЗ
	...
	|	ПО Операции.Ссылка = СтатьиКалькуляцииОпераций.ПроизводственнаяОперация
	|	И ТрудозатратыКОформлению.ВидРабот = СтатьиКалькуляцииОпераций.ВидРабот
	|
	|	ЛЕВОЕ СОЕДИНЕНИЕ ВтСтатьиКалькуляцииПоНСИ КАК СтатьиКалькуляцииПоНСИ
	|	ПО Операции.ВладелецТехнологическойОперации = СтатьиКалькуляцииПоНСИ.ВладелецТехнологическойОперации
	|	И ТрудозатратыКОформлению.ВидРабот = СтатьиКалькуляцииПоНСИ.ВидРабот
	|
	|ГДЕ
	|	ЕСТЬNULL(КонтрольныеОперации.Выполнено + КонтрольныеОперации.Брак, Операции.КоличествоФакт + Операции.КоличествоБрак) > 0
	|
	//-- НЕ УТКА
	|;
	|
	...
	|	Расценки.КвалификационныйРазряд КАК КвалификационныйРазряд,
	|	Расценки.Расценка КАК Расценка
	|ПОМЕСТИТЬ ВТРасценки
	|ИЗ
	|	РегистрСведений.РасценкиРаботСотрудников.СрезПоследних(&КонецПериода, ВидРабот В (ВЫБРАТЬ ДД.ВидРабот ИЗ втТрудозатратыКОформлению КАК ДД)) КАК Расценки
	|ИНДЕКСИРОВАТЬ ПО
	|	ВидРабот
	|
	//++ Локализация
	...
	|
	|ВЫБРАТЬ
	|	РазрядыКатегорииСотрудников.Сотрудник КАК Сотрудник,
	|	РазрядыКатегорииСотрудников.РазрядКатегория КАК РазрядКатегория
	|ПОМЕСТИТЬ ВТРазрядыСотрудников
	|ИЗ
	|	РегистрСведений.РазрядыКатегорииСотрудников.СрезПоследних(&ТекущаяДата, Сотрудник В (ВЫБРАТЬ ДД.Исполнитель ИЗ втТрудозатратыКОформлению КАК ДД)) КАК РазрядыКатегорииСотрудников
	|ГДЕ
	|	НЕ РазрядыКатегорииСотрудников.РазрядКатегория = ЗНАЧЕНИЕ(Справочник.РазрядыКатегорииДолжностей.ПустаяСсылка)
	|ИНДЕКСИРОВАТЬ ПО
	|	Сотрудник
	//++ НЕ УТКА
	|;
	...
	|ИЗ
	|	РегистрСведений.ТарифныеСтавки.СрезПоследних(&КонецПериода,
	|		&ДополнительныйРазрезТарифнойСетки = ЗНАЧЕНИЕ(Перечисление.ДополнительныеРазрезыТарифнойСетки.Организация) И Подразделение = ЗНАЧЕНИЕ(Справочник.СтруктураПредприятия.ПустаяСсылка)
	|		ИЛИ &ДополнительныйРазрезТарифнойСетки = ЗНАЧЕНИЕ(Перечисление.ДополнительныеРазрезыТарифнойСетки.Подразделение) И Организация = ЗНАЧЕНИЕ(Справочник.Организации.ПустаяСсылка)
	|		ИЛИ &ДополнительныйРазрезТарифнойСетки = ЗНАЧЕНИЕ(Перечисление.ДополнительныеРазрезыТарифнойСетки.БезДетализации) И Подразделение = ЗНАЧЕНИЕ(Справочник.СтруктураПредприятия.ПустаяСсылка)
	|		И Организация = ЗНАЧЕНИЕ(Справочник.Организации.ПустаяСсылка)) КАК ТарифныеСтавки
	|ГДЕ
	|	&НормированиеИРасценкаРаботПоТарифнымСеткам
	...
	|	ЗначенияТарифов.Тариф КАК Тариф
	|ПОМЕСТИТЬ ВТЗначенияТарифов
	|ИЗ
	|	РегистрСведений.ЗначенияТарифов.СрезПоследних(&КонецПериода) КАК ЗначенияТарифов
	|ГДЕ
	|	&ИспользоватьТарифныеСеткиПриРасчетеЗарплаты
	|ИНДЕКСИРОВАТЬ ПО
	|	ТарифнаяСетка
	|
	|;
	...
	|	ДД.Исполнитель КАК Исполнитель,
	|	ДД.ВидРабот КАК ВидРабот,
	|	МАКСИМУМ(ЗначенияТарифов.Тариф) КАК Тариф
	|ПОМЕСТИТЬ ВТТарифыИсполнителей
	|ИЗ
	|	втТрудозатратыКОформлению КАК ДД
	|	
	|	ЛЕВОЕ СОЕДИНЕНИЕ ВТРазрядыСотрудников КАК РазрядыСотрудников
	|		ПО ДД.Исполнитель = РазрядыСотрудников.Сотрудник
	|	
	...
	|	ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.МестоПозицииШтатногоРасписанияВСтруктуреПредприятия КАК МестоПозицииШР
	|		ПО ДД.Подразделение = МестоПозицииШР.Подразделение
	|	ЛЕВОЕ СОЕДИНЕНИЕ Справочник.ШтатноеРасписание КАК ШтатноеРасписание
	|		ПО МестоПозицииШР.Позиция = ШтатноеРасписание.Ссылка
	|		И Должности.Ссылка = ШтатноеРасписание.Должность
	|	ЛЕВОЕ СОЕДИНЕНИЕ ВТЗначенияТарифов КАК ЗначенияТарифов
	|		ПО ЗначенияТарифов.ТарифнаяСетка = ШтатноеРасписание.ТарифнаяСетка
	|		И ЗначенияТарифов.РазрядКатегория = РазрядыСотрудников.РазрядКатегория
	|ГДЕ
	|	&ИспользоватьТарифныеСеткиПриРасчетеЗарплаты
	|	И ДД.Исполнитель Ссылка Справочник.Сотрудники
	|
	|СГРУППИРОВАТЬ ПО
	|	ДД.Исполнитель,
	|	ДД.ВидРабот
	...
	|ГДЕ
	|	&ИспользоватьТарифныеСеткиПриРасчетеЗарплаты
	|	И ДД.Исполнитель Ссылка Справочник.Бригады
	|
	|СГРУППИРОВАТЬ ПО
	|	ДД.Исполнитель,
	|	ДД.ВидРабот
	...
	//++ НЕ УТКА
	|		КОГДА ВидыРабот.РасценкаПоТарифнойСетке И &ИспользоватьТарифныеСеткиПриРасчетеЗарплаты И НЕ ТарифыИсполнителей.Тариф ЕСТЬ NULL
	|			ТОГДА ТарифыИсполнителей.Тариф * ВидыРабот.Трудоемкость / ВидыРабот.КратностьТрудоемкости
	|		КОГДА &НормированиеИРасценкаРаботПоТарифнымСеткам И НЕ ТарифныеСтавки.Тариф ЕСТЬ NULL
	|			 И НЕ &ИспользоватьТарифныеСеткиПриРасчетеЗарплаты
	|			ТОГДА ТарифныеСтавки.Тариф * ВидыРабот.Трудоемкость / ВидыРабот.КратностьТрудоемкости
	|		КОГДА &НормированиеИРасценкаРаботПоТарифнымСеткам И НЕ ТарифныеСтавкиОбщие.Тариф ЕСТЬ NULL
	|			 И НЕ &ИспользоватьТарифныеСеткиПриРасчетеЗарплаты
	|			ТОГДА ТарифныеСтавкиОбщие.Тариф * ВидыРабот.Трудоемкость / ВидыРабот.КратностьТрудоемкости
	//-- НЕ УТКА
	
	//-- Локализация
	|		КОГДА НЕ Расценки.Расценка ЕСТЬ NULL
	|			ТОГДА Расценки.Расценка * ВЫБОР
	|										КОГДА ВидыРабот.РасценкаЗаЧасРаботы
	|											ТОГДА ВидыРабот.Трудоемкость / ВидыРабот.КратностьТрудоемкости
	|										ИНАЧЕ 1
	|									КОНЕЦ
	|		ИНАЧЕ 0
	|	КОНЕЦ КАК Расценка,
	|	ДД.КоличествоКОплате * ВЫБОР
	//++ Локализация
	...
	|			ТОГДА ДД.Исполнитель
	|	КОНЕЦ КАК Сотрудник,
	|	РазрядыСотрудников.РазрядКатегория КАК КвалификацияИсполнителя,
	//-- Локализация
	|	ДД.Подразделение КАК Подразделение,
	...
	//++ НЕ УТКА
	|	ЛЕВОЕ СОЕДИНЕНИЕ ВТТарифыИсполнителей КАК ТарифыИсполнителей
	|	ПО ДД.Исполнитель = ТарифыИсполнителей.Исполнитель
	|	И ДД.ВидРабот = ТарифыИсполнителей.ВидРабот
	//-- НЕ УТКА
	|	ЛЕВОЕ СОЕДИНЕНИЕ ВТРазрядыСотрудников КАК РазрядыСотрудников
	|	ПО ДД.Исполнитель = РазрядыСотрудников.Сотрудник
	//-- Локализация
	|	ЛЕВОЕ СОЕДИНЕНИЕ Справочник.ВидыРаботСотрудников КАК ВидыРабот
	|	ПО ДД.ВидРабот = ВидыРабот.Ссылка
	|	ЛЕВОЕ СОЕДИНЕНИЕ ВТРасценки КАК Расценки
	|	ПО ДД.ВидРабот = Расценки.ВидРабот
	|	И НЕ ВидыРабот.РасценкаПоТарифнойСетке
	|	И ((НЕ &ИспользоватьРазрядыРасценокРаботСотрудников ИЛИ ВидыРабот.КвалификационныйРазряд = Расценки.КвалификационныйРазряд)
	//++ Локализация
	|	И РазрядыСотрудников.РазрядКатегория ЕСТЬ NULL
	|	ИЛИ НЕ РазрядыСотрудников.РазрядКатегория ЕСТЬ NULL
	|	И (НЕ &ИспользоватьРазрядыРасценокРаботСотрудников ИЛИ РазрядыСотрудников.РазрядКатегория = Расценки.КвалификационныйРазряд)
	//-- Локализация
	|	)
	//++ Локализация
	...
	|	И ((НЕ &УчетТрудозатратВРазрезеСотрудников ИЛИ ДД.ВидНаряда = ЗНАЧЕНИЕ(Перечисление.ВидыБригадныхНарядов.Бригадный)) И ТарифныеСтавки.Разряд = ВидыРабот.КвалификационныйРазряд
	|	ИЛИ &УчетТрудозатратВРазрезеСотрудников И (ТарифныеСтавки.Разряд = РазрядыСотрудников.РазрядКатегория
	|		ИЛИ РазрядыСотрудников.РазрядКатегория ЕСТЬ NULL И ТарифныеСтавки.Разряд = ВидыРабот.КвалификационныйРазряд)
	|	)
	|	И (ДД.Подразделение = ТарифныеСтавки.Подразделение И ТарифныеСтавки.Тариф <> 0
	|	ИЛИ ДД.Организация = ТарифныеСтавки.Организация И ТарифныеСтавки.Тариф <> 0)
	|
	|	ЛЕВОЕ СОЕДИНЕНИЕ ВТТарифныеСтавки КАК ТарифныеСтавкиОбщие
	|	ПО ВидыРабот.РасценкаПоТарифнойСетке
	|	И ВидыРабот.Должность = ТарифныеСтавкиОбщие.Должность
	|	И ((НЕ &УчетТрудозатратВРазрезеСотрудников ИЛИ ДД.ВидНаряда = ЗНАЧЕНИЕ(Перечисление.ВидыБригадныхНарядов.Бригадный)) И ТарифныеСтавкиОбщие.Разряд = ВидыРабот.КвалификационныйРазряд
	|	ИЛИ &УчетТрудозатратВРазрезеСотрудников И (ТарифныеСтавкиОбщие.Разряд = РазрядыСотрудников.РазрядКатегория
	|		ИЛИ РазрядыСотрудников.РазрядКатегория ЕСТЬ NULL И ТарифныеСтавкиОбщие.Разряд = ВидыРабот.КвалификационныйРазряд)
	|	)
	|	И (ТарифныеСтавкиОбщие.Подразделение = ЗНАЧЕНИЕ(Справочник.СтруктураПредприятия.ПустаяСсылка)
	|	И ТарифныеСтавкиОбщие.Организация = ЗНАЧЕНИЕ(Справочник.Организации.ПустаяСсылка) И ТарифныеСтавкиОбщие.Тариф <> 0)
	//-- НЕ УТКА
...
Процедура ИменаРеквизитовПоВидуНаряда(Объект, МассивВсехРеквизитов, МассивРеквизитовОперации) Экспорт
	...
	СмешанныйТипИсполнителя = СмешанныйТипИсполнителя(Объект);
	...
	Если Объект.ВидНаряда = Перечисления.ВидыБригадныхНарядов.Персональный Тогда
		...
		Если Объект.УчетТрудозатратВРазрезеСотрудников Тогда
			...
			Если СмешанныйТипИсполнителя Тогда
...
Процедура РаспределитьСуммуПоУчастникам(Объект) Экспорт
	...
	Результат = РаспределениеВидовРаботПоСотрудникам(Объект, Ложь, Ложь, Ложь);
...
Функция ИнициализироватьХозяйственныеОперацииИДокументы(ХозяйственныеОперацииИДокументы) Экспорт
	...
	Строка.КлючНазначенияИспользования  = "ВыработкаСотрудников";
...
Функция РаспределениеВидовРаботПоСотрудникам(Объект, ДетальныеЗаписи, ГруппировкаПоДатеВыполнения = Истина, РаспределятьКоличество = Истина, РаспределятьСумму = Истина)
	...
	Если ДетальныеЗаписи Тогда
		...
		"ВЫБРАТЬ
		|	ВидыРабот.Распоряжение          КАК Распоряжение,
		|	&ДатаВыполнения                 КАК ДатаВыполненияРабот,
		|	ВидыРабот.КодСтрокиРаспоряжения КАК КодСтрокиРаспоряжения,
		...
		|	ВтВидыРабот КАК ВидыРабот
		|
		|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВтДокументыПартийПроизводстваБезЗаказа КАК ВтДокументыПартийПроизводстваБезЗаказа
		|	ПО ВтДокументыПартийПроизводстваБезЗаказа.Распоряжение = ВидыРабот.Распоряжение
		|	И ВтДокументыПартийПроизводстваБезЗаказа.КодСтрокиРаспоряжения = ВидыРабот.КодСтрокиРаспоряжения
		|
		|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.ПартииПроизводства КАК СпрПартииПроизводства
		|	ПО СпрПартииПроизводства.Документ = ВтДокументыПартийПроизводстваБезЗаказа.ДокументПартии
		|	И СпрПартииПроизводства.Код = ВтДокументыПартийПроизводстваБезЗаказа.НомерГруппыЗатрат
		|	И НЕ СпрПартииПроизводства.ПометкаУдаления
		|
		|СГРУППИРОВАТЬ ПО
		|	ВидыРабот.Распоряжение,
	...
	Если ГруппировкаПоДатеВыполнения Тогда
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "&ДатаВыполнения", "ВидыРабот.ДатаВыполненияРабот");
	Иначе
		Запрос.УстановитьПараметр("ДатаВыполнения", Дата(1, 1, 1));
	КонецЕсли;
...
Функция ТекстЗапросаВтДокументыПартийПроизводстваБезЗаказа(Запрос, ТекстыЗапроса)
	
	ИмяРегистра = "ВтДокументыПартийПроизводстваБезЗаказа";
...
Функция ТекстЗапросаТаблицаЗначенияОперативныхПоказателейРасчетаЗарплатыСотрудников(Запрос, ТекстыЗапроса, Регистры)
	...
	|		КОГДА &Исправление
	|			ТОГДА &Дата
	|		ИНАЧЕ Работники.ДатаВыполненияРабот
	|	КОНЕЦ КАК Дата,
	...
	|	&Организация КАК Организация,
	|	ВЫБОР
	|		КОГДА Выработка.Исправление
	|			ТОГДА Выработка.Дата
	|		ИНАЧЕ ВыработкаСотрудниковВидыРабот.ДатаВыполненияРабот
	|	КОНЕЦ,
...
Функция ТекстЗапросаТаблицаТрудозатратыНезавершенногоПроизводства(Запрос, ТекстыЗапроса, Регистры)
	...
	Если НЕ ПроведениеДокументов.ЕстьТаблицаЗапроса("ВтДокументыПартийПроизводстваБезЗаказа", ТекстыЗапроса) Тогда
		ТекстЗапросаВтДокументыПартийПроизводстваБезЗаказа(Запрос, ТекстыЗапроса);
	КонецЕсли;
	...
	|			ТОГДА СпрПартииПроизводстваПроизводстваБезЗаказа.Ссылка
	//++ НЕ УТКА
	|		КОГДА ТИПЗНАЧЕНИЯ(ВидыРабот.Распоряжение) = ТИП(Документ.ЭтапПроизводства2_2)
	|			ТОГДА ВЫРАЗИТЬ(ВидыРабот.Распоряжение КАК Документ.ЭтапПроизводства2_2).ПартияПроизводства
	|		КОГДА ТИПЗНАЧЕНИЯ(ВидыРабот.Распоряжение) = ТИП(Документ.ПроизводственнаяОперация2_2)
	...
	//-- Устарело_Производство21
	|
	|	ЛЕВОЕ СОЕДИНЕНИЕ ВтДокументыПартийПроизводстваБезЗаказа КАК ДокументыПартийПроизводстваБезЗаказа
	|	ПО ДокументыПартийПроизводстваБезЗаказа.Распоряжение = ВидыРабот.Распоряжение
	|	И ДокументыПартийПроизводстваБезЗаказа.КодСтрокиРаспоряжения = ВидыРабот.КодСтрокиРаспоряжения
	|
	|	ЛЕВОЕ СОЕДИНЕНИЕ Справочник.ПартииПроизводства КАК СпрПартииПроизводстваПроизводстваБезЗаказа
	|	ПО СпрПартииПроизводстваПроизводстваБезЗаказа.Документ = ДокументыПартийПроизводстваБезЗаказа.ДокументПартии
	|	И СпрПартииПроизводстваПроизводстваБезЗаказа.Код = ДокументыПартийПроизводстваБезЗаказа.НомерГруппыЗатрат
	|	И НЕ СпрПартииПроизводстваПроизводстваБезЗаказа.ПометкаУдаления
	|
	|ГДЕ
	...
	|	И НЕ ВидыРабот.КоличествоКОплате = 0
	|	И НЕ ВидыРабот.Сумма * &КоэффициентПересчетаВВалютуУПР = 0
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	...
	|
	|ГДЕ
	|	НЕ ВидыРабот.Количество = 0
	|	И НЕ ВидыРабот.НормативнаяСтоимость * &КоэффициентПересчетаВВалютуУПР = 0
	|";
...
Функция ТекстЗапросаТаблицаТрудозатратыКОформлению(Запрос, ТекстыЗапроса, Регистры)
	...
	|
	|	ЛЕВОЕ СОЕДИНЕНИЕ ВтДокументыПартийПроизводстваБезЗаказа КАК ДокументыПартийПроизводстваБезЗаказа
	|	ПО ДокументыПартийПроизводстваБезЗаказа.Распоряжение = ТаблицаТрудозатраты.Распоряжение
	|	И ДокументыПартийПроизводстваБезЗаказа.КодСтрокиРаспоряжения = ТаблицаТрудозатраты.КодСтрокиРаспоряжения
	|
	|	ЛЕВОЕ СОЕДИНЕНИЕ Справочник.ПартииПроизводства КАК СпрПартииПроизводстваПроизводстваБезЗаказа
	|	ПО СпрПартииПроизводстваПроизводстваБезЗаказа.Документ = ДокументыПартийПроизводстваБезЗаказа.ДокументПартии
	|	И СпрПартииПроизводстваПроизводстваБезЗаказа.Код = ДокументыПартийПроизводстваБезЗаказа.НомерГруппыЗатрат
	...
	|			И ТаблицаТрудозатраты.Сотрудник <> ЗНАЧЕНИЕ(Справочник.Сотрудники.ПустаяСсылка)
	|			ТОГДА ТаблицаТрудозатраты.Сотрудник
	//-- Локализация
	|		КОГДА ИСТИНА
	|			ТОГДА ТаблицаТрудозатраты.Работник
	|	КОНЕЦ КАК Исполнитель,
	|	ТаблицаТрудозатраты.КоличествоКОформлению КАК Количество,
	|	ТаблицаТрудозатраты.КодСтрокиРаспоряжения КАК КодСтрокиРаспоряжения
	|ИЗ
...
Функция ТекстЗапросаТаблицаРеестрДокументов(Запрос, ТекстыЗапроса, Регистры)
	...
	|	ДД.Исправление							КАК СторноИсправление,
	|	ДД.СторнируемыйДокумент					КАК СторнируемыйДокумент,
	|	ДД.ИсправляемыйДокумент					КАК ИсправляемыйДокумент,
...
Процедура ЗаписатьНовыйДокумент(ТекущийДокумент, СписокДокументов, СписокОшибок)
	...
	Для Каждого ТекущееСообщение Из ПолучитьСообщенияПользователю(Истина) Цикл
		...
		СтруктураОшибки.Вставить("Организация", ТекущийДокумент.Организация);
		СтруктураОшибки.Вставить("Документ", ТекущийДокумент.Ссылка);
		СтруктураОшибки.Вставить("ТекстОшибки", ТекущееСообщение.Текст);
		СтруктураОшибки.Вставить("ТипОшибки", "СообщенияПользователю");
...
Функция СмешанныйТипИсполнителя(Объект)
	
	Если НЕ Объект.УчетТрудозатратВРазрезеСотрудников Тогда
		Возврат Ложь;
	КонецЕсли;
	...
	|	ИСТИНА
	|ИЗ
	|	Документ.ПроизводствоБезЗаказа КАК РеквизитыДокумента
	|	
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ Константы КАК Константы
	|	ПО ИСТИНА
	|
	|ГДЕ
	|	РеквизитыДокумента.Ссылка В(&Распоряжения)
	//++ Локализация
	|	И РеквизитыДокумента.Дата < Константы.ДатаНачалаВеденияУчетаТрудозатратПоСотрудникам
	//-- Локализация
	|";
	Запрос.УстановитьПараметр("Распоряжения", Объект.ВидыРабот.Выгрузить(, "Распоряжение").ВыгрузитьКолонку("Распоряжение"));
