...
Функция ДанныеКорректировкиДоВводаОстатков(Объект, ИспользуетсяКоличествоУпаковок = Ложь) Экспорт
	...
	//++ НЕ УТ
	Запрос.УстановитьПараметр("Дата", ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Объект.ДокументОснование, "Дата"));
	Запрос.УстановитьПараметр("Страна", ЗначениеНастроекКлиентСерверПовтИсп.СтранаРегистрацииОрганизации(Объект.Организация));
	Запрос.УстановитьПараметр("ОблагаетсяАкцизом", Объект.ОблагаетсяАкцизом);
	
	ТекстЧастиЗапроса = УчетПодакцизныхТоваров.ТекстЗапросаВтПодакцизныеТовары("ВТТовары");
	ЧастиТекстаЗапроса.Добавить(ТекстЧастиЗапроса);
	//-- НЕ УТ
	...
	//++ НЕ УТ
	|		Т.ОблагаетсяАкцизом КАК ОблагаетсяАкцизом,
	//-- НЕ УТ
	...
	|			ВТТовары КАК Товары
	//++ НЕ УТ
	|				ЛЕВОЕ СОЕДИНЕНИЕ ВтПодакцизныеТовары
	...
	|				ПО Расхождения.Номенклатура = ВтПодакцизныеТовары.Номенклатура
	//-- НЕ УТ
	|		) КАК Т
	...
	//++ НЕ УТ
	|			ЛЕВОЕ СОЕДИНЕНИЕ ВтПодакцизныеТовары КАК ВтПодакцизныеТовары
	|			ПО ВтПодакцизныеТовары.Номенклатура = Товары.Номенклатура
	//-- НЕ УТ
...
//
// Параметры:
//	ДокументСсылка - ДокументСсылка.КорректировкаРеализации - ссылка на документ.
//
// Возвращаемое значение:
...
Функция ПолучитьДанныеПоАкцизам(ДокументСсылка) Экспорт
	...
	|ИЗ
	|	РегистрСведений.КатегорииПодакцизныхТоваров.СрезПоследних(&ПериодОтгрузки, (Номенклатура, Страна) В
	|		(ВЫБРАТЬ
	|			Т.Номенклатура КАК Номенклатура,
	...
	|	ВидыЗапасов.ВидЗапасов.ПодразделениеПроизводства КАК ПодразделениеПроизводства,
	...
	|	ВЫБОР КОГДА ВидыЗапасов.ВидЗапасов.ОблагаетсяАкцизом
	|		ТОГДА ТаблицаПодакцизныеКатегории.КоэффициентПересчета * ВидыЗапасов.Количество
	|		ИНАЧЕ 0
	...
	|		ПО ВидыЗапасов.АналитикаУчетаНоменклатуры.Номенклатура = ТаблицаПодакцизныеКатегории.Номенклатура
	|
	|ГДЕ
	...
	|	ЗНАЧЕНИЕ(Перечисление.ВидыОперацийСПодакцизнымТоваром.Реализация) КАК ВидОперацииСПодакцизнымТоваром,
	...
	ПериодКурса = Запрос.Параметры.ПериодПродажи;
...
Функция ТекстЗапросаВтВидыЗапасов() Экспорт
	...
	//++ НЕ УТ
	|	СУММА(ВсеВидыЗапасов.СуммаАкциза)                            КАК СуммаАкциза,
	//-- НЕ УТ
	|
...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	СписокОписаний.Добавить(НСтр("ru = '- заполнение реквизитов ""Статус указания серий на складах"" и ""Статус указания серий, переданных товаров"" в табличной части ""Расхождения"".';
								|en = '- Fill the ""Batch specification status in warehouses"" and ""Batch indication status of goods transferred"" attributes in the ""Discrepancies"" table.'"));
	СписокОписаний.Добавить(НСтр("ru = '- заполнение реквизита ""Кор. аналитика учета номенклатуры"" в табличных частях ""Расхождения"", ""Виды запасов оприходование"" для документов с основанием ""Реализация товаров и услуг"" с операцией ""Реализация"".';
								|en = '- Fill in the ""Cor. inventory location attributes"" attribute in the ""Discrepancies"" and ""Inventory owner attributes goods receipt"" tables for documents generated based on the ""Customer invoice"" document with the ""Sales"" transaction.'"));
	СписокОписаний.Добавить(НСтр("ru = '- заполнение реквизита ""Кор. аналитика учета номенклатуры"" в табличной части ""Виды запасов списание"" для документов с основанием ""Реализация товаров и услуг"" с операциями ""Реализация"" и ""Переход права собственности"".';
								|en = '- Fill in the ""Cor. inventory location attributes"" attribute in the ""Inventory owner attributes write-off"" table for documents generated based on the ""Customer invoice"" document with the ""Sales"" and ""Transfer of title to goods"" transactions.'"));
