				...
				Если ВариантыОтраженийСОбязательнымСкладом.НайтиПоЗначению(СтрокаТЧ.ВариантОтражения) <> Неопределено Тогда
					...
					ИначеЕсли Не ЗначениеЗаполнено(СтрокаТЧ.Склад) Тогда
						...
						ТекстОшибки = НСтр("ru = 'Не заполнена колонка ""Склад"" в строке %НомерСтроки% списка ""Товары""';
											|en = 'The ""Warehouse"" column in line %НомерСтроки% of the ""Goods"" list is not filled in'");
			...
			Для Каждого СтрокаТЧ Из Расхождения Цикл
				...
				Если ВариантыОтраженийСОбязательнымСкладом.НайтиПоЗначению(СтрокаТЧ.ВариантОтражения) <> Неопределено Тогда
					...
					ИначеЕсли Не ЗначениеЗаполнено(СтрокаТЧ.Склад) Тогда
						...
						ТекстОшибки = НСтр("ru = 'Не заполнена колонка ""Склад"" в строке %НомерСтроки% списка ""Расхождения""';
											|en = 'The ""Warehouse"" column in line %НомерСтроки% of the ""Discrepancies"" list is not filled in'");
						ТекстОшибки =  СтрЗаменить(ТекстОшибки, "%НомерСтроки%", СтрокаТЧ.НомерСтроки);
...
Процедура ЗаполнитьПоАктуОРасхожденияхПослеОтгрузки(Знач СтруктураПараметров, ДанныеЗаполнения)
	...
	ДанныеЗаполнения.Вставить("ЭтоВозвратВПуть", ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(СтруктураПараметров, "ЭтоВозвратВПуть", Ложь));
	...
	ЭтоВозвратВПуть = Ложь;
	Если СтруктураПараметров.Свойство("ЭтоВозвратВПуть") Тогда
		ЭтоВозвратВПуть = СтруктураПараметров.ЭтоВозвратВПуть;
	КонецЕсли;
...
Процедура ЗаполнитьВидыЗапасов(Отказ)
	...
	ПериодНачисленияАкциза = Дата;
	//++ НЕ УТ
	ПодбиратьПодакцизныеТовары = ПолучитьФункциональнуюОпцию("ИспользоватьПодакцизныеТовары")
									И ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.РеализацияКлиенту;
	Если ПодбиратьПодакцизныеТовары Тогда
		ПериодНачисленияАкциза = ПериодНачисленияАкциза();
	КонецЕсли;
	ДополнительныеСвойства.Вставить("ЗаписатьСуммуАкциза", Ложь);
	//-- НЕ УТ
	...
	Иначе
		...
		Если ПодбиратьПодакцизныеТовары Тогда
			ОтборСтрок = Новый Структура("ВариантОтражения",
										Перечисления.ВариантыОтраженияКорректировокРеализаций.СписатьНаРасходы);
			ЕстьСтрокиСписания = Расхождения.НайтиСтроки(ОтборСтрок).Количество() > 0;
			...
			МенеджерВременныхТаблиц = ВременныеТаблицыДанныхДокументаОприходование(МассивВариантовОтражения, Ложь);
			Если Не Проведен
				...
				Иначе
					...
					ИменаКолонокСуммирования	= "Количество, КоличествоПоРНПТ, СуммаСНДС, СуммаНДС,
													|Себестоимость, СебестоимостьБезНДС, СебестоимостьРегл, СуммаАкциза,
		...
		Если Не Проведен
			...
			Если ТипКорректировки = Перечисления.ТипыКорректировки.КорректировкаДоВводаОстатковТоваровИУслуг Тогда
				Если ЕстьСтрокиСписания Тогда
					ВидыЗапасовСтрокСписания = ВидыЗапасовОприходование.Выгрузить();
				КонецЕсли;
				ВидыЗапасовОприходование.Очистить();
				...
				Если ТаблицаВидыЗапасовОприходование.Количество() > 0 Тогда
					...
					ПараметрыЗаполнения.ИмяТЧВидыЗапасов = "ВидыЗапасовОприходование";
					Если ПодбиратьПодакцизныеТовары Тогда
						ПараметрыЗаполнения.ПериодНачисленияАкциза = ПериодНачисленияАкциза;
	...
	Иначе
		...
		Если Не Проведен
			...
			Если ПодбиратьПодакцизныеТовары Тогда
				...
				ПараметрыЗаполнения.РассчитатьСуммуАкциза = Истина;
				ПараметрыЗаполнения.ОтборыВидовЗапасов.ОблагаетсяАкцизом.Очистить();
				ПараметрыЗаполнения.ЗаполнятьПодакцизныеТоварыВидамиЗапасовПоУмолчанию = Истина;
				ПараметрыЗаполнения.ПериодНачисленияАкциза = ПериодНачисленияАкциза;
				Если ТипКорректировки = Перечисления.ТипыКорректировки.КорректировкаДоВводаОстатковТоваровИУслуг
					И Не ОблагаетсяАкцизом Тогда
					ПараметрыЗаполнения.ОтборыВидовЗапасов.ОблагаетсяАкцизом.Добавить(Ложь);
				КонецЕсли;
			...
			ИменаКолонокСуммирования = "Количество, КоличествоПоРНПТ, СуммаСНДС, СуммаНДС";
			//++ НЕ УТ
			ИменаКолонокСуммирования = ИменаКолонокСуммирования + ", СуммаАкциза";
			УчетПодакцизныхТоваров.РассчитатьСуммуАкциза(ЭтотОбъект, ПараметрыЗаполнения, , Отказ);
			//-- НЕ УТ
	...
	//++ НЕ УТ
	Если ПодбиратьПодакцизныеТовары
		И Не ДополнительныеСвойства.ЗаписатьСуммуАкциза
...
Функция ПроверитьИзменениеРеквизитовДокумента(МенеджерВременныхТаблиц)
	...
	//++ НЕ УТ
	ИменаРеквизитов.Добавить("ОблагаетсяАкцизом");
	ИменаРеквизитов.Добавить("ПодразделениеПроизводства");
	//-- НЕ УТ
...
Процедура ЗаполнитьДопКолонкиВидовЗапасов(ВидыЗапасов, МассивВариантовОтражения, ЗаполнятьВидЗапасовОтгрузки = Ложь, ЗаполнятьСебестоимость = Ложь) Экспорт
	...
	Для Каждого СтрокаТоваров Из ТаблицаТовары Цикл
		...
		Для Каждого СтрокаЗапасов Из ВидыЗапасов.НайтиСтроки(СтруктураПоиска) Цикл
			...
			ИначеЕсли ЗаполнятьСебестоимость Тогда
...
Функция ВременныеТаблицыДанныхДокументаОприходование(МассивВариантовОтражения, ИспользоватьОтгрузкуБезПереходаПраваСобственности2_5)
	...
	//++ НЕ УТ
	ЕстьВариантСписания =
		МассивВариантовОтражения.Найти(Перечисления.ВариантыОтраженияКорректировокРеализаций.СписатьНаРасходы) <> Неопределено;
	
	Запрос.УстановитьПараметр("ОблагаетсяАкцизом",
								?(ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.РеализацияКлиенту
										И Не ЕстьВариантСписания,
									ОблагаетсяАкцизом,
									Ложь));
	Запрос.УстановитьПараметр("ПодразделениеПроизводства",
								?(ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.РеализацияКлиенту
...
Функция ТаблицаДляЗаполненияВидыЗапасовОприходование(МенеджерВременныхТаблиц)
	...
	ТекстыЗапроса = Новый Массив;
	//++ НЕ УТ
	ТекстыЗапроса.Добавить(УчетПодакцизныхТоваров.ТекстЗапросаВТПодакцизныеКатегорииТоваров(МенеджерВременныхТаблиц));
	//-- НЕ УТ
	...
	|			ТОГДА ТаблицаДанныхДокумента.ОблагаетсяАкцизом
	|		ИНАЧЕ ЛОЖЬ
	|	КОНЕЦ											КАК ОблагаетсяАкцизом,
	|	ВЫБОР
	|		КОГДА ЕСТЬNULL(ДанныеКатегорий.ПодакцизныйТовар, ЛОЖЬ)
	|			ТОГДА ТаблицаДанныхДокумента.ПодразделениеПроизводства
	|		ИНАЧЕ ЗНАЧЕНИЕ(Справочник.СтруктураПредприятия.ПустаяСсылка)
	...
	Запрос.УстановитьПараметр("Дата",	ПериодНачисленияАкциза());
...
Процедура ЗаполнитьСуммыНДСРеглУпр() Экспорт
	...
	Если ВидКорректировки = Перечисления.ХозяйственныеОперации.ИсправлениеОшибок
		И ЗначениеЗаполнено(ДокументОснование) Тогда
		ДатаКурса = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДокументОснование, "Дата");
...
Функция ПериодНачисленияАкциза()

	ПериодНачисленияАкциза = Дата;
	
	Если ЗначениеЗаполнено(ДокументОснование) Тогда
		ПериодНачисленияАкциза = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДокументОснование, "Дата");
	КонецЕсли;
	
	Возврат ПериодНачисленияАкциза;
