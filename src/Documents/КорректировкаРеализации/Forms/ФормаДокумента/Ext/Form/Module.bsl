...
Процедура НастроитьОтображениеЭлементовПоИтогамРасхождений()
	...
	//++ НЕ УТ
	ДанныеОснования = Новый Структура("ХозяйственнаяОперация", Операция);
	ИспользоватьПодакцизныеТовары = ТипЗнч(Объект.ДокументОснование) = Тип("ДокументСсылка.РеализацияТоваровУслуг")
									И Документы.РеализацияТоваровУслуг.ИспользуютсяПодакцизныеТовары(ДанныеОснования);
	//-- НЕ УТ
		...
		Если ТекущиеДанные <> Неопределено Тогда
			...
			Если Не (ТекущиеДанные[ИмяПоляКорСтатус] > 0
				...
				ТекстПредупреждения = НСтр("ru = 'Невозможно разбить строку.
					...
					|en = 'Cannot split the line.
					|You can only do it for lines where batches can be specified.'");
