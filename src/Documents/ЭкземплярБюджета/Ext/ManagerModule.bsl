...
Процедура ЗаполнитьБюджетПоДаннымИнформационнойБазы(Объект) Экспорт
	...
	Для Каждого СтрокаДанных Из СтрокиДанных Цикл
		...
		Если ПараметрыЯчейки.РасчетЗаполнение <> "заполнение"
			И ПараметрыЯчейки.РасчетЗаполнение <> "расчет" Тогда
			Продолжить;
		КонецЕсли;
...
//	*ТекстЗапросаТаблицыДокумента - Строка -
...
Функция ПараметрыВыводаВидаБюджетаВТабличныйДокумент(Объект,
	...
	ТекстыЗапросов.Добавить(ОбщегоНазначенияУТ.ТекстЗапросаДанныеТЧОбъекта(Объект.Ссылка, "АналитикаСтатейБюджетов",
		Ложь, "ИдентификаторСтроки"));
	ТекстыЗапросов.Добавить(ОбщегоНазначенияУТ.ТекстЗапросаДанныеТЧОбъекта(Объект.Ссылка, "ОборотыПоСтатьямБюджетов",
		Ложь, "ИдентификаторСтроки"));
	ТекстыЗапросов.Добавить(ОбщегоНазначенияУТ.ТекстЗапросаДанныеТЧОбъекта(Объект.Ссылка, "АналитикаЗначений",
		Ложь, "ИдентификаторСтроки"));
	ПараметрыДанных.Вставить("ТекстЗапросаТаблицыДокумента", СтрСоединить(ТекстыЗапросов,
...
// Возвращает значения измерений Организация, Подразделение по умолчанию для модели бюджетирования и пользователя
//
// Параметры:
//  МодельБюджетирования - СправочникСсылка.МоделиБюджетирования - модель бюджетирования
//  Ответственный - СправочникСсылка.Пользователи - пользователь.
//
// Возвращаемое значение:
//  Структура - структура параметров:
//   *Организация - СправочникСсылка.Организации - организация по умолчанию
//   *Подразделение - СправочникСсылка.СтруктураПредприятия - подразделение по умолчанию
//
