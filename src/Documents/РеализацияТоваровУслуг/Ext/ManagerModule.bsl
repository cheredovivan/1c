		...
		Если БонусныеПрограммыЛояльности.Количество() > 1
			...
			Если ДанныеОтбора.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ПереходПраваСобственности Тогда
				ТекстОшибки = НСтр("ru = 'Обнаружены разные программы лояльности для начисления бонусных баллов. Распоряжения 
					...
					|en = 'Different loyalty programs are found for accrual of bonus points. References 
		...
		Если ВыборкаПроверки.РеализацияПоЗаказам Тогда
			...
			Если ВыборкаПроверки.ХозяйственнаяОперация =
				...
				ТекстОшибки = НСтр("ru = 'У документа %Документ% статус ""%Статус%"" не установлен, т.к. реализация оформлена по распоряжению';
									|en = 'Status ""%Статус%"" is not set for the ""%Документ%"" document as the sale is registered by a reference'");
			Иначе
...
//
// Параметры:
//	ДокументОбъект - ДокументОбъект.РеализацияТоваровУслуг, ДанныеФормыСтруктура - данные документа.
//
// Возвращаемое значение:
...
Функция ИспользуютсяПодакцизныеТовары(ДокументОбъект) Экспорт
	...
	Возврат ОперацииОблагаемыеАкцизом().Найти(ДокументОбъект.ХозяйственнаяОперация) <> Неопределено;
...
//
// Параметры:
//	ДокументСсылка - ДокументСсылка.РеализацияТоваровУслуг - ссылка на документ.
//
// Возвращаемое значение:
...
Функция ПолучитьДанныеПоАкцизам(ДокументСсылка) Экспорт
	...
	|	ВЫРАЗИТЬ(ТаблицаВидыЗапасов.СуммаАкциза * &КоэффициентПересчетаИзРеглВУпр
	|		КАК ЧИСЛО(31,2)) КАК СуммаАкцизаУпр
	...
	|		ИЗ
	|			ДанныеДокументаТаблицаВидыЗапасов КАК Т)) КАК ТаблицаКатегории
	|ГДЕ
	...
	|	ТаблицаВидыЗапасов.АналитикаУчетаНоменклатурыНоменклатура КАК Номенклатура,
	|	ТаблицаПодакцизныеКатегории.КатегорияПодакцизногоТовара КАК КатегорияПодакцизногоТовара,
	|	ВЫБОР
	|		КОГДА &НалогообложениеНДС = ЗНАЧЕНИЕ(Перечисление.ТипыНалогообложенияНДС.ПродажаНаЭкспорт)
	|				И &КонтрагентУчастникЕАЭС
	|			ТОГДА ЗНАЧЕНИЕ(Перечисление.ВидыОперацийСПодакцизнымТоваром.ЭкспортЕАЭС)
	|		КОГДА &НалогообложениеНДС = ЗНАЧЕНИЕ(Перечисление.ТипыНалогообложенияНДС.ПродажаНаЭкспорт)
	|			ТОГДА ЗНАЧЕНИЕ(Перечисление.ВидыОперацийСПодакцизнымТоваром.Экспорт)
	|		ИНАЧЕ ЗНАЧЕНИЕ(Перечисление.ВидыОперацийСПодакцизнымТоваром.Реализация)
	|	КОНЕЦ КАК ВидОперацииСПодакцизнымТоваром,
	|	ТаблицаВидыЗапасов.Количество КАК Количество,
	|	ВЫБОР КОГДА ТаблицаВидыЗапасов.ОблагаетсяАкцизом
	|		ТОГДА ЕСТЬNULL(ТаблицаПодакцизныеКатегории.КоэффициентПересчета, 0) * ТаблицаВидыЗапасов.Количество
	|		ИНАЧЕ 0
	|	КОНЕЦ КАК КоличествоАкциза,
	|	ТаблицаВидыЗапасов.СуммаАкциза     КАК СуммаАкцизаРегл,
	...
	|
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВтКатегорииПодакцизныхТоваров КАК ТаблицаПодакцизныеКатегории
	|		ПО ТаблицаВидыЗапасов.АналитикаУчетаНоменклатурыНоменклатура = ТаблицаПодакцизныеКатегории.Номенклатура
	|
	...
	|	И ТаблицаПодакцизныеКатегории.КатегорияПодакцизногоТовара <> ЗНАЧЕНИЕ(Справочник.КатегорииПодакцизныхТоваров.ПустаяСсылка)
	|	И (ТаблицаВидыЗапасов.СуммаАкциза <> 0
	|		ИЛИ ТаблицаВидыЗапасов.СуммаАкцизаУпр <> 0
	|		ИЛИ ВЫБОР КОГДА ТаблицаВидыЗапасов.ОблагаетсяАкцизом
	|				ТОГДА ЕСТЬNULL(ТаблицаПодакцизныеКатегории.КоэффициентПересчета, 0) * ТаблицаВидыЗапасов.Количество
	|			ИНАЧЕ 0
	|			КОНЕЦ <> 0)
	|";
	...
	Если (Реквизиты.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.РеализацияБезПереходаПраваСобственности
			Или Реквизиты.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.РеализацияЧерезКомиссионераБезПереходаПраваСобственности) 
		И Реквизиты.Статус = Перечисления.СтатусыРеализацийТоваровУслуг.Отгружено Тогда
		ПериодКурса = Реквизиты.ДатаПереходаПраваСобственности;
	Иначе
		ПериодКурса = Реквизиты.Период;
	КонецЕсли;
...
Функция ТекстЗапросаВтТаблицаВидыЗапасов(Запрос, ТекстыЗапроса, ДокументОбъект)
	...
	|	ВЫРАЗИТЬ(ВидыЗапасов.СуммаАкциза * &КоэффициентПересчетаИзРеглВУпр
	|		КАК ЧИСЛО(31,2)) КАК СуммаАкцизаУпр,
	//-- НЕ УТ
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьАктыРасхожденийПослеОтгрузки") Тогда
		...
		Если КомандаСоздания <> Неопределено Тогда
			...
			КомандаСоздания.Представление = НСтр("ru = 'Акт о расхождениях после отгрузки товаров клиенту и перехода права собственности';
												|en = 'Sales dispute after goods shipment to customer and transfer of title to goods'");
		КонецЕсли;
	...
	Для Каждого СтрокаТаблицы Из ТаблицаПроверки Цикл
		...
		Если СтрокаТаблицы.ЭтоПереходПраваСобственности Тогда
			...
			ТекстСообщения = НСтр("ru = 'В документе %1 хозяйственная операция %2. Печать расходной накладной не требуется.';
									|en = 'The ""%1"" document has the ""%2"" business transaction. Printing a sales invoice is not required.'");
...
Функция СтруктураОснования(Объект, ПорядокРасчетов)
	...
	ИначеЕсли (ПорядокРасчетов = Перечисления.ПорядокРасчетов.ПоЗаказам
		...
		Если МассивЗаказов <> Неопределено Тогда
			...
			Запрос.УстановитьПараметр("СинонимОтгрузкиТоваров", НСтр("ru = 'Отгрузка товаров клиенту';
...
Функция ОперацииОблагаемыеАкцизом()
	
	ОблагаемыеОперации = Новый Массив;
	ОблагаемыеОперации.Добавить(Перечисления.ХозяйственныеОперации.РеализацияКлиенту);
	ОблагаемыеОперации.Добавить(Перечисления.ХозяйственныеОперации.РеализацияКлиентуРеглУчет);
	
	Возврат ОблагаемыеОперации;
