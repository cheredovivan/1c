...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Если Не РеализацияПоЗаказам
		...
		ТекстОшибки = НСтр("ru = 'Переход права собственности должен формироваться только по распоряжению.';
							|en = 'Transfer of title to goods must be generated only based on the reference.'");
		...
		Если ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ПереходПраваСобственности Тогда
			...
			Если НайденнаяСтрока <> Неопределено Тогда
				...
				ТекстОшибки = НСтр("ru = 'В строке %НомерСтроки% списка ""Товары"" указана номенклатура с типом ""%ПредставлениеТипаНоменклатуры%"", Переход права собственности может делаться только по товарам.';
									|en = 'In line %НомерСтроки% of the ""Goods"" list, the item with the ""%ПредставлениеТипаНоменклатуры%"" type is specified. You can make transfer of title to goods only for goods.'");
				ТекстОшибки = СтрЗаменить(ТекстОшибки, "%НомерСтроки%", СтрокаТовары.НомерСтроки);
	...
	Если Не РеализацияПоЗаказам
		...
		Если ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ПереходПраваСобственности Тогда
			ТекстОшибки = НСтр("ru = 'Нет прав на создание реализации товаров и услуг без распоряжения';
								|en = 'Insufficient rights to create a customer invoice without a reference'");
		Иначе
	...
	Если ХозяйственнаяОперацияРеализации = Перечисления.ХозяйственныеОперации.ПереходПраваСобственности
		...
		Если Не РеквизитыСоглашения.ИспользуютсяДоговорыКонтрагентов Тогда
			...
			ТекстИсключения = НСтр("ru = 'В соглашении ""%1"" не используются договоры с контрагентами. Использование договоров является обязательным для реализации товаров с отложенным переходом права собственности.';
									|en = 'Contracts with counterparties are not used in the ""%1"" agreement. Using contracts is required for selling goods with deferred transfer of title to goods.'");
			ТекстИсключения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				ТекстИсключения,
				РеквизитыСоглашения.Представление);
...
Процедура ЗаполнитьСтрокуВидовЗапасов(СтрокаТоваров, СтруктураПоиска, Параметры, СоответствиеВидовЗапасов)
	...
	Для Каждого СтрокаЗапасов Из ВидыЗапасов.НайтиСтроки(СтруктураПоиска) Цикл
		...
		Если СтрокаЗапасов.Количество <> 0 Тогда
			...
			//++ НЕ УТ
			НоваяСтрока.СуммаАкциза	= КоличествоПоСтроке * СтрокаЗапасов.СуммаАкциза / СтрокаЗапасов.Количество;
			//-- НЕ УТ
...
Процедура ЗаполнитьВидыЗапасов(Отказ)
	...
	Если (Статус <> Перечисления.СтатусыРеализацийТоваровУслуг.КПредоплате
		...
		ДополнительныеСвойства.Вставить("ЗаписатьСуммуАкциза", ПараметрыЗаполнения.РассчитатьСуммуАкциза);
		//-- НЕ УТ
...
Функция ПараметрыЗаполненияВидовЗапасов(ЭтоПроведение)
	...
	//++ НЕ УТ
	Если Документы.РеализацияТоваровУслуг.ИспользуютсяПодакцизныеТовары(ЭтотОбъект) Тогда
		ПараметрыЗаполнения.РассчитатьСуммуАкциза = Истина;
		ОтборыВидовЗапасов.ОблагаетсяАкцизом.Очистить();
	КонецЕсли;
	//-- НЕ УТ
