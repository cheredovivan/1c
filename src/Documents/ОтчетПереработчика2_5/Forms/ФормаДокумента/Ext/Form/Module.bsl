...
Процедура ПодобратьПереданныеМатериалы(Команда)
	...
	ОценкаПроизводительностиКлиент.ЗамерВремени(Истина,
		"Документ.ОтчетПереработчика2_5.ФормаДокумента.Команда.ПодобратьПереданныеТовары");
...
Процедура НастроитьЭлементыГруппЗатрат(Форма)
	...
	Если Не Объект.ГруппировкаЗатрат = ПредопределенноеЗначение("Перечисление.ГруппировкиЗатратВЗаказеПереработчику.БезГруппировки") Тогда
		...
		ПереченьПолейГруппыЗатрат =
			?(Объект.ПоЗаказам,
				ПереработкаНаСторонеКлиентСервер.ПереченьПолейГруппыЗатратЗаказаПереработчику(Объект.ГруппировкаЗатрат),
				ПереработкаНаСторонеКлиентСервер.ПереченьПолейГруппыЗатратОтчетаПереработчика(Объект.ГруппировкаЗатрат));
		
		ЗаголовокГруппы = ПереработкаНаСторонеКлиентСервер.ЗаголовокПоляГруппыЗатрат2_5(ПереченьПолейГруппыЗатрат);
...
Процедура ОбработкаВыбораПодборПереданныхТоваров(ВыбранноеЗначение)
	...
	Для Каждого СтрокаТовара Из ТаблицаТоваров Цикл
		...
		СтрокиТоваров = Объект.МатериалыИРаботы.НайтиСтроки(ПоискТоваров);
		...
		ИначеЕсли СтрокиТоваров.Количество() = 1 Тогда
			...
			ТекущиеДанные = СтрокиТоваров[0];
			
		ИначеЕсли СтрокиТоваров.Количество() > 1 Тогда
			
			Для Каждого СтрокаКУдалению Из СтрокиТоваров Цикл
				Объект.МатериалыИРаботы.Удалить(СтрокаКУдалению);
			КонецЦикла;
...
Функция ПоместитьТоварыВХранилищеДляПодбораПереданногоТовара()
	...
	Адрес = ПоместитьВоВременноеХранилище(
		Объект.МатериалыИРаботы.Выгрузить(,"НомерСтроки, Номенклатура, Характеристика, Назначение, Серия, Количество"),
		УникальныйИдентификатор);
