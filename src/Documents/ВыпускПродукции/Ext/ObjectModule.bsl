...
Процедура ПроверитьКорректностьУказанияПодакцизныхТоваров(Отказ)
	...
	|ГДЕ
	|	ДанныеКатегорий.ПодакцизныйТовар
	|
	|УПОРЯДОЧИТЬ ПО
	|	НомерСтроки";
	ТекстыЗапроса.Добавить(ЧастьТекстаЗапроса);
	...
	ШаблонПодакцизныйТовар = НСтр("ru = 'В строке %1 списка ""Товары и работы"" указана подакцизная продукция.
		|Выпуск подакцизной продукции не поддерживается. Используйте документ %2.';
		|en = 'In line %1 of the ""Goods and works"" list, excisable product is specified.
		|Excisable product release is not supported. Use the %2 document.'");
	
	РазрешенныеДокументы = Новый Массив;
	ПредставлениеДокумента = НСтр("ru = '""%ПроизводствоБезЗаказа%""';
									|en = '""%ПроизводствоБезЗаказа%""'");
	ПредставлениеДокумента = СтрЗаменить(
		ПредставлениеДокумента, "%ПроизводствоБезЗаказа%", Метаданные.Документы.ПроизводствоБезЗаказа.Синоним);
	РазрешенныеДокументы.Добавить(ПредставлениеДокумента);
	//++ НЕ УТКА
	ПредставлениеДокумента = НСтр("ru = 'или ""%ЭтапПроизводства%""';
									|en = 'or ""%ЭтапПроизводства%""'");
	ПредставлениеДокумента = СтрЗаменить(
		ПредставлениеДокумента, "%ЭтапПроизводства%", Метаданные.Документы.ЭтапПроизводства2_2.Синоним);
	РазрешенныеДокументы.Добавить(ПредставлениеДокумента);
	//-- НЕ УТКА
	
	ТекстРазрешенныеДокументы = СтрСоединить(РазрешенныеДокументы, " ");
	
	Выборка = РезультатЗапроса.Выбрать();
	Пока Выборка.Следующий() Цикл
		ТекстСообщения = СтрШаблон(ШаблонПодакцизныйТовар, Выборка.НомерСтроки, ТекстРазрешенныеДокументы);
