...
Функция ТекстЗапросаТаблицаРаспоряженияНаОтгрузку(Запрос, ТекстыЗапроса, Регистры)
	...
	|	ТаблицаПродукции.КодСтроки КАК КодСтроки,
	|	ВЫБОР
	|		КОГДА ТаблицаПродукции.Номенклатура.ТипНоменклатуры В (ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.Товар),
...
Функция СформироватьПечатнуюФормуОтчетОПродукцииИМатериалах(ДанныеПечати, ОбъектыПечати, ТабличныйДокумент) Экспорт
	...
	Пока ДанныеПечати.Следующий() Цикл
		...
		Если Не ПараметрыУчетнойПолитики = Неопределено
			...
			"ВЫБРАТЬ
			// Продукция
			|	ВЫРАЗИТЬ(Продукция.АналитикаУчетаПродукции КАК Справочник.КлючиАналитикиУчетаНоменклатуры) КАК АналитикаУчетаПродукции,
			|	ВЫРАЗИТЬ(СУММА(Продукция.Количество) КАК ЧИСЛО(31, 2))                                     КАК КоличествоПродукция,
			|	ВЫРАЗИТЬ(Продукция.ЕдиницаИзмерения КАК Справочник.УпаковкиЕдиницыИзмерения)               КАК ЕдиницаИзмерения
			...
			// Продукция
			|	Продукция.АналитикаУчетаПродукции,
			|	Продукция.АналитикаУчетаПродукции.Номенклатура,
			|	Продукция.АналитикаУчетаПродукции.Номенклатура.НаименованиеПолное,
			|	Продукция.АналитикаУчетаПродукции.Номенклатура.Код,
			|	Продукция.АналитикаУчетаПродукции.Номенклатура.Артикул,
			...
			|	Затраты.Номенклатура.НаименованиеПолное,
			|	Затраты.Номенклатура.Код,
			|	Затраты.Номенклатура.Артикул,
...
Функция ПолучитьРезультатЗапросаПоОстаткамЗаказов(ДанныеОтбора, МассивЗаказов = Неопределено)
	...
	|		РаспоряженияНаОтгрузку.Количество,
	...
	|		И РаспоряженияНаОтгрузку.Показатель =
	...
	ПараметрыФормированияТаблицы.НачалоПериода = "&ДатаНачала";
