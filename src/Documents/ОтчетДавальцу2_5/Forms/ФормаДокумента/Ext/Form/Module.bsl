...
Процедура ПродукцияПриНачалеРедактирования(Элемент, НоваяСтрока, Копирование)
	...
	Если НоваяСтрока Тогда
		...
		Иначе
			...
			ПараметрыЗаполненияВидаЦен.ВидЦеныПоУмолчанию = ВидЦеныПоУмолчанию;
			ПараметрыЗаполненияВидаЦен.ПараметрыОтбора.ЦенаВключаетНДС = Объект.ЦенаВключаетНДС;
			ПараметрыЗаполненияВидаЦен.ПараметрыОтбора.ИспользоватьПриПередачеПродукцииДавальцу = Истина;
...
Процедура ПодобратьТовары(Команда)
	...
	ПараметрыВыбораВидаЦен.Добавить(Новый ПараметрВыбора("Отбор.ИспользоватьПриПередачеПродукцииДавальцу", Истина));
	ПараметрыФормы.Вставить("ПараметрыВыбораВидаЦен", ПараметрыВыбораВидаЦен);
...
Процедура ОбработкаВыбораПодборВыпущеннойПродукции(ВыбранноеЗначение)
	...
	Для Каждого СтрокаТовара Из ТаблицаПродукции Цикл
		...
		СтрокиТоваров = Объект.Продукция.НайтиСтроки(ПоискТоваров);
		...
		Если СтрокиТоваров.Количество() = 0 Тогда
			...
			ТекущаяСтрока.ЗаказДавальца = ТекущийЗаказДавальца;
			ТекущаяСтрока.Склад = СтрокаТовара.МестоХранения;
			ОбрабатываемыеНовыеСтроки.Добавить(ТекущаяСтрока);
	...
	СтруктураДействийДляНовыхСтрок = Новый Структура(Новый ФиксированнаяСтруктура(СтруктураДействий));
	СтруктураДействийДляНовыхСтрок.Вставить("ЗаполнитьВидЦены", Новый Структура("ВидЦены", ВидЦеныПоУмолчанию));
	СтруктураДействийДляНовыхСтрок.Вставить("ЗаполнитьЦенуПродажи", СтруктураЗаполненияЦены);
...
Функция ПоместитьТоварыВХранилищеДляПодбораПринятогоТовара()
	...
	Результат = Объект.Продукция.Выгрузить(, Колонки);
...
Процедура ОбработатьШтрихкоды(ДанныеШтрихкодов)
	...
	ПараметрыЗаполненияВидаЦен.ПараметрыОтбора.ЦенаВключаетНДС = Объект.ЦенаВключаетНДС;
	ПараметрыЗаполненияВидаЦен.ПараметрыОтбора.ИспользоватьПриПередачеПродукцииДавальцу = Истина;
	СтруктураДействийСДобавленнымиСтроками.Вставить("ПроверитьЗаполнитьВидЦены", ПараметрыЗаполненияВидаЦен);
...
Функция ПривязатьСтрокиКЗаказам(ОтобратьПоЗаказу)
	...
	|	СУММА(РаспоряженияОбороты.КОтгрузкеКоличествоОборот) КАК КПередачеПриход,
	|	СУММА(РаспоряженияОбороты.ОформленоКоличествоОборот) КАК КОформлениюРасход
	...
	|		РаспоряженияНаОтгрузку.Количество     КОформлению
	|	ИЗ
	...
	|			РаспоряженияНаОтгрузку.Склад          КАК Склад,
	|			РаспоряженияНаОтгрузку.Серия          КАК Серия,
	|			РаспоряженияНаОтгрузку.Количество     КАК КОформлению
	|		ИЗ
	|			РегистрНакопления.РаспоряженияНаОтгрузкуИВозврат КАК РаспоряженияНаОтгрузку
	|		ГДЕ
	|			РаспоряженияНаОтгрузку.Активность
	...
	ПараметрыФормированияТаблицы.Показатели = "КОтгрузке, Оформлено";
