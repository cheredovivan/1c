...
Процедура ОбновитьСписокРаспоряженияНаОформление()
	...
	Если РежимФормированияДоИзменения <> РежимФормирования Тогда
		...
		Элементы.СписокРаспоряженияНаОформлениеКОформлению.Видимость = РежимФормирования = 1;
	КонецЕсли;
...
Процедура ОбновитьСписокРаспоряженияНаОформлениеНаСервере()
	...
	ПараметрыОтбора = НакладныеСервер.ПараметрыОтбораРаспоряжений(Организация,,, ОтборХозяйственныеОперации, Менеджер);
...
Процедура СформироватьТекстыЗапросов()
	...
	ПараметрыОтбора = НакладныеСервер.ПараметрыОтбораРаспоряжений(Организация, Подразделение, Склад, ОтборХозяйственныеОперации, Менеджер);
	...
	Для каждого КЗ Из Состояния Цикл
		...
		ИначеЕсли КЗ.Ключ = "СостоянияПередачВПроизводство" Тогда
			...
			|	ИСТИНА КАК НакладнаяНаОтгрузку,
			|	ЛОЖЬ КАК НакладнаяНаПриемку,
			|	СостоянияПередачВПроизводство.КОформлению КАК КОформлению
			|ИЗ
		...
		ИначеЕсли КЗ.Ключ = "СостоянияПередачИзПроизводства" Тогда
			...
			|	ЛОЖЬ КАК НакладнаяНаОтгрузку,
			|	ИСТИНА КАК НакладнаяНаПриемку,
			|	СостоянияПередачИзПроизводства.КОформлению КАК КОформлению
			|ИЗ
	...
	ТекстЗапросаСпискаРаспоряжений =
		...
		|	ОстаткиСостояния.КОформлению                                                   КАК КОформлению,
