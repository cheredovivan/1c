...
Функция ВременныеТаблицыДанныхДокумента() Экспорт
	...
	|	ВЫБОР
	|		КОГДА &ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ПередачаПродукцииИзКладовой)
	|			ТОГДА &ИспользуетсяПроизводствоПодакцизныхТоваров
	|		ИНАЧЕ ЛОЖЬ
	|	КОНЕЦ КАК ОблагаетсяАкцизом,
	|	ВЫБОР
	|		КОГДА &ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ПередачаПродукцииИзКладовой)
	|			ТОГДА ЕСТЬNULL(СправочникСклады.Подразделение, ЗНАЧЕНИЕ(Справочник.СтруктураПредприятия.ПустаяСсылка))
	|		ИНАЧЕ ЗНАЧЕНИЕ(Справочник.СтруктураПредприятия.ПустаяСсылка)
	...
	|		ПО &Склад = СправочникСклады.Ссылка
	|
	|ГДЕ
	...
	ИспользуетсяПроизводствоПодакцизныхТоваров = УчетПодакцизныхТоваров.ИспользуетсяПроизводствоПодакцизныхТоваров(
													Организация, Дата);
...
Процедура ЗаполнитьВидыЗапасов(Отказ)
	...
	Если Не Проведен
		...
		КонтрольЗаполненияПодакцизныхВидовЗапасов(Отказ);
...
Процедура КонтрольЗаполненияПодакцизныхВидовЗапасов(Отказ)
	
	Если Отказ
		Или ТипЗнч(Получатель) = Тип("СправочникСсылка.СтруктураПредприятия")
	...
	ТипСклада = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Получатель, "ТипСклада");
	...
	Запрос.УстановитьПараметр("ТаблицаВидыЗапасов", ВидыЗапасов.Выгрузить(, ВыгружаемыеКолонки));
	...
	ТаблицаПроверки = РезультатЗапроса.Выгрузить();
	ТаблицаПроверки.Индексы.Добавить("АналитикаУчетаНоменклатуры");
	
	ШаблонСообщенияОшибки = НСтр("ru = 'В строке %1 списка ""Товары"" указан подакцизный товар.
		|Запрещено перемещение подакцизных товаров на розничные склады.';
		|en = 'Excisable goods are specified in line %1 of the ""Goods"" list.
		|Cannot transfer excisable goods to retail warehouses.'");
	
	Для Каждого СтрокаТЧ Из Товары Цикл
		НайденнаяСтрока = ТаблицаПроверки.Найти(СтрокаТЧ.АналитикаУчетаНоменклатуры, "АналитикаУчетаНоменклатуры");
		
		Если НайденнаяСтрока = Неопределено Тогда
			Продолжить;
		КонецЕсли;
		
		ТекстСообщения = СтрШаблон(ШаблонСообщенияОшибки, СтрокаТЧ.НомерСтроки);
...
Процедура ПроверитьКорректностьУказанияПодакцизныхТоваров(Отказ)
	...
	Если Не ИспользоватьПодакцизныеТовары
		Или ХозяйственнаяОперация <> Перечисления.ХозяйственныеОперации.ПередачаПродукцииИзКладовой Тогда
	...
	|	ТаблицаОшибок.УказанРозничныйСкладОтправитель	КАК УказанРозничныйСкладОтправитель,
	|	ТаблицаОшибок.УказанРозничныйСкладПолучатель	КАК УказанРозничныйСкладПолучатель
	|ИЗ
	|	(ВЫБРАТЬ
	|		ТаблицаТоваров.НомерСтроки	КАК НомерСтроки,
	|		ВЫБОР
	...
	|					И ЕСТЬNULL(ДанныеКатегорий.ПодакцизныйТовар, ЛОЖЬ)
	|					И ЕСТЬNULL(СправочникСкладыОтправитель.ТипСклада, НЕОПРЕДЕЛЕНО) = ЗНАЧЕНИЕ(Перечисление.ТипыСкладов.РозничныйМагазин)
	|				ТОГДА ИСТИНА
	|			ИНАЧЕ ЛОЖЬ
	|		КОНЕЦ						КАК УказанРозничныйСкладОтправитель,
	|		ВЫБОР
	|			КОГДА &ИспользуетсяПроизводствоПодакцизныхТоваров
	|					И ЕСТЬNULL(ДанныеКатегорий.ПодакцизныйТовар, ЛОЖЬ)
	|					И ЕСТЬNULL(СправочникСкладыПолучатель.ТипСклада, НЕОПРЕДЕЛЕНО) = ЗНАЧЕНИЕ(Перечисление.ТипыСкладов.РозничныйМагазин)
	|				ТОГДА ИСТИНА
	|			ИНАЧЕ ЛОЖЬ
	|		КОНЕЦ						КАК УказанРозничныйСкладПолучатель
	|	ИЗ
	|		ТаблицаТоваров КАК ТаблицаТоваров
	...
	|			
	|			ЛЕВОЕ СОЕДИНЕНИЕ Справочник.Склады КАК СправочникСкладыОтправитель
	|			ПО &Отправитель = СправочникСкладыОтправитель.Ссылка
	|			
	|			ЛЕВОЕ СОЕДИНЕНИЕ Справочник.Склады КАК СправочникСкладыПолучатель
	|			ПО &Получатель = СправочникСкладыПолучатель.Ссылка
	|	
	|	) КАК ТаблицаОшибок
	|
	|ГДЕ
	|	(ТаблицаОшибок.УказанПодакцизныйТовар
	|		ИЛИ ТаблицаОшибок.УказанРозничныйСкладОтправитель
	|		ИЛИ ТаблицаОшибок.УказанРозничныйСкладПолучатель
	|	)
	|
	|УПОРЯДОЧИТЬ ПО
	|	НомерСтроки";
	...
	ШаблонСообщенияУказанПодакцизныйТовар = НСтр("ru = 'В строке %1 списка ""Продукция"" указана подакцизная продукция.
		|Выбор такой номенклатуры запрещен, так как учетная политика финансового учета организации ""%2"" не предусматривает производство подакцизной продукции.';
		|en = 'Excisable products are specified in line %1 of the ""Manufactured products"" list.
		|Cannot select this item, as the financial accounting policy of the ""%2"" company does not allow producing excisable products.'");
	
	ШаблонСообщенияУказанаРозничнаяКладовая = НСтр("ru = 'В строке %1 списка ""Продукция"" указан подакцизный товар.
		|Выпуск подакцизной продукции на розничный склад запрещен.';
		|en = 'Excisable goods are specified in line %1 of the ""Manufactured products"" list.
		|Cannot release excisable goods to a retail warehouse.'");
	
	ШаблонСообщенияУказанРозничныйСклад = НСтр("ru = 'В строке %1 списка ""Продукция"" указан подакцизный товар.
		|Запрещена передача подакцизной продукции на розничные склады.';
		|en = 'Excisable goods are specified in line %1 of the ""Manufactured products"" list.
		|Cannot transfer excisable goods to retail warehouses.'");
	...
	Пока Выборка.Следующий() Цикл
		...
		Если Выборка.УказанПодакцизныйТовар Тогда
			ТекстСообщения	= СтрШаблон(ШаблонСообщенияУказанПодакцизныйТовар,
										Выборка.НомерСтроки,
										Организация);
		ИначеЕсли Выборка.УказанРозничныйСкладОтправитель Тогда
			ТекстСообщения	= СтрШаблон(ШаблонСообщенияУказанаРозничнаяКладовая,
										Выборка.НомерСтроки);
		ИначеЕсли Выборка.УказанРозничныйСкладПолучатель Тогда
			ТекстСообщения	= СтрШаблон(ШаблонСообщенияУказанРозничныйСклад,
										Выборка.НомерСтроки);
		КонецЕсли;
		
		Если Не ПустаяСтрока(ТекстСообщения) Тогда
			Поле = ОбщегоНазначенияКлиентСервер.ПутьКТабличнойЧасти("Товары", Выборка.НомерСтроки, ИмяПоля);
