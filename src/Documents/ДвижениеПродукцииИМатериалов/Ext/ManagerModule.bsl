...
//
// Параметры:
//	ДокументОбъект - ДокументОбъект.ДвижениеПродукцииИМатериалов, ДанныеФормыСтруктура - данные документа.
//
// Возвращаемое значение:
...
Функция ИспользуютсяПодакцизныеТовары(ДокументОбъект) Экспорт
	...
	Возврат ДокументОбъект.ХозяйственнаяОперация <> Перечисления.ХозяйственныеОперации.ПеремещениеПолуфабрикатовМеждуФилиалами;
...
Процедура ОформитьОтгрузкуТоваров(Запрос, ТекстыЗапроса, Регистры)
	...
	|	ДанныеИсточника.Характеристика					КАК Характеристика,
	|	ДанныеИсточника.НазначениеОтправителя			КАК Назначение,
	|	ДанныеИсточника.Назначение						КАК НазначениеДляСоединения,
	|	ДанныеИсточника.Серия							КАК Серия,
	|	ДанныеИсточника.СтатусУказанияСерийОтправитель	КАК СтатусУказанияСерий,
	|	ДанныеИсточника.Количество						КАК Количество,
	|	ВЫБОР
	...
	ПараметрыМодульногоПроведения.СинонимТаблицыДокумента = "ДанныеИсточника";
	ПараметрыМодульногоПроведения.ПереопределяемыеПоляСоединенияССериями = ПереопределяемыеПоляСоединенияССериями;
	ПараметрыМодульногоПроведения.ОбъектМетаданных = Метаданные.Документы.ДвижениеПродукцииИМатериалов;
...
Функция ДокументыПоПараметрам(Параметры) Экспорт
	...
	Если ПравоДоступа("Чтение", Метаданные.Документы.ПроизводствоБезЗаказа) Тогда
		...
		|
		|ГДЕ
		|	РеквизитыДокумента.Ссылка В(&ОбъектыОснований)
		|
		|СГРУППИРОВАТЬ ПО
		|	ТаблицаТовары.Ссылка,
