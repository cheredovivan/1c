...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	ИначеЕсли ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ВводОстатковОтгруженныхТоваров Тогда
		...
		Если Не ЗначениеЗаполнено(Партия) Тогда
			...
			ТекстОшибки = НСтр("ru = 'Поле ""Документ отгрузки"" не заполнено';
								|en = '""Shipping document"" is not filled in'");
	...
	Иначе
		...
		ПараметрыКонтроля.ШаблонТекстаОшибки =
			НСтр("ru = 'Запрещен ввод остатков подакцизного товара ""%Номенклатура%"", указанного в строке %НомерСтроки% списка ""%ПредставлениеТабличнойЧасти%"" на розничный склад ""%Склад%""';
				|en = 'Cannot enter the balance of the ""%Номенклатура%"" excisable goods specified in line %НомерСтроки% of the ""%ПредставлениеТабличнойЧасти%"" list in the ""%Склад%"" retail warehouse'");
...
Процедура ЗаполнитьВидыЗапасов()
	...
	ПараметрыТекстаЗапроса.ИмяТаблицыТоваров = "ТаблицаТоваровВрем";
	...
	//++ НЕ УТ
	|	ИЛИ НЕ &Проведен
	|	ИЛИ СохраненныеДанные.Дата <> &Дата
	|	ИЛИ СохраненныеДанные.ОблагаетсяАкцизом <> &ОблагаетсяАкцизом
	|	ИЛИ СохраненныеДанные.ПодразделениеПроизводства <> &ПодразделениеПроизводства
	//-- НЕ УТ
	|
	|	ОБЪЕДИНИТЬ ВСЕ
	|
	...
	|	НЕОПРЕДЕЛЕНО КАК ПодразделениеПроизводства,
	//-- НЕ УТ
	|	ЛОЖЬ КАК ДеятельностьОблагаетсяЕНВД
	|ИЗ
	|	ТаблицаТоваров КАК ТаблицаТоваров
