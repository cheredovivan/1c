...
Процедура ПослеЗаписиНаСервере(ТекущийОбъект, ПараметрыЗаписи)
	...
	УстановитьЗаголовокПоляОблагаетсяАкцизом(ЭтотОбъект);
	//-- НЕ УТ
...
Процедура ОблагаетсяАкцизомПриИзменении(Элемент)
	//++ НЕ УТ
	УчетПодакцизныхТоваровКлиентСервер.НастроитьПолеПодразделениеПроизводства(ЭтотОбъект);
	//-- НЕ УТ
...
Процедура ПереданныеПереработчикамТоварыПередНачаломДобавления(Элемент, Отказ, Копирование, Родитель, Группа, Параметр)
	...
	ОткрытьФорму(ФормаПодбораТоваровИзЗаказа, ПараметрыПодбора, ЭтотОбъект,,,, Неопределено, РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
	//-- Устарело_Переработка24
...
Процедура УстановитьУсловноеОформление()
	...
	//++ НЕ УТКА
	НоменклатураСервер.УстановитьУсловноеОформлениеХарактеристикНоменклатуры(ЭтотОбъект, "ПринятыеВПереработкуМатериалыХарактеристика");
	НоменклатураСервер.УстановитьУсловноеОформлениеХарактеристикНоменклатуры(ЭтотОбъект, "ПолуфабрикатыДавальцаХарактеристика");
	НоменклатураСервер.УстановитьУсловноеОформлениеХарактеристикНоменклатуры(ЭтотОбъект, "ПродукцияДавальцаХарактеристика");
	//-- НЕ УТКА
	...
	//++ НЕ УТКА
	НоменклатураСервер.УстановитьУсловноеОформлениеСерийНоменклатуры(ЭтотОбъект, Ложь, "ПринятыеВПереработкуМатериалыСерия");
	НоменклатураСервер.УстановитьУсловноеОформлениеСерийНоменклатуры(ЭтотОбъект, Ложь, "ПолуфабрикатыДавальцаСерия");
	НоменклатураСервер.УстановитьУсловноеОформлениеСерийНоменклатуры(ЭтотОбъект, Ложь, "ПродукцияДавальцаСерия");
	//-- НЕ УТКА
	...
	//++ НЕ УТКА
	НоменклатураСервер.УстановитьУсловноеОформлениеСтатусовУказанияСерий(ЭтотОбъект, Ложь, "ПринятыеВПереработкуМатериалыСтатусУказанияСерий");
	НоменклатураСервер.УстановитьУсловноеОформлениеСтатусовУказанияСерий(ЭтотОбъект, Ложь, "ПолуфабрикатыДавальцаСтатусУказанияСерий");
	НоменклатураСервер.УстановитьУсловноеОформлениеСтатусовУказанияСерий(ЭтотОбъект, Ложь, "ПродукцияДавальцаСтатусУказанияСерий");
	//-- НЕ УТКА
	...
	КомпоновкаДанныхКлиентСервер.ДобавитьОформляемоеПоле(Элемент.Поля, Элементы.ПереходПраваСобственностиПартнер.Имя);
...
Процедура НоменклатураПриИзменении(ИдентификаторТекущейСтроки = Неопределено)
	...
	Если ИспользоватьПодакцизныеТовары Тогда
		СтруктураДействий.Вставить("ЗаполнитьПризнакПодакцизныйТовар",
...
Процедура ПриЧтенииСозданииНаСервере(ДанныеОбъекта)
	...
	//++ НЕ УТ
	ИспользоватьПодакцизныеТовары = ЭтоОстаткиСобственныхТоваров
		И ПолучитьФункциональнуюОпцию("ИспользоватьПодакцизныеТовары");
	ЗаполнитьПризнакПодакцизныйТовар();
	УстановитьЗаголовокПоляОблагаетсяАкцизом(ЭтотОбъект);
	//-- НЕ УТ
...
Процедура ПолучитьЗагруженныеТоварыИзХранилища(АдресТоваровВХранилище, КэшированныеЗначения)
	...
	//++ НЕ УТ
	Если ИспользоватьПодакцизныеТовары Тогда
		СтруктураДействийПостОбработка.Вставить("ЗаполнитьПризнакПодакцизныйТовар",
			ПакетнаяОбработкаТабличнойЧастиКлиентСервер.ПараметрыЗаполненияПризнакаПодакцизныйТовар(Объект));
	КонецЕсли;
	//-- НЕ УТ
...
//++ НЕ УТ
&НаСервере
Процедура ЗаполнитьПризнакПодакцизныйТовар()
...
&НаКлиентеНаСервереБезКонтекста
Процедура УстановитьЗаголовокПоляОблагаетсяАкцизом(Форма)
	
	КоличествоПодакцизныхТоваров = Форма.Объект.Товары.Итог("ПодакцизныйТовар");
	Форма.Элементы.ОблагаетсяАкцизом.Заголовок =
		УчетПодакцизныхТоваровКлиентСервер.ЗаголовокПоляОблагаетсяАкцизом(КоличествоПодакцизныхТоваров);
