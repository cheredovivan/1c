...
Функция ДанныеДокументаДляПроведения(Документ, Регистры, ДопПараметры = Неопределено) Экспорт
	...
	Если Не ДопПараметры.ПолучитьТекстыЗапроса Тогда
		...
		// Запись в журналы документов, ОС и НМА раздельно.
...
Функция ТекстЗапросаТаблицаДокументыПоОС(Запрос, ТекстыЗапроса, Регистры)
	...
	|	&Ссылка КАК Ссылка,
	|	&Период КАК Дата,
	|	СписокОСДляЖурнала.Организация КАК Организация,
...
Процедура ИнициализироватьВтСписокОСДляЖурнала(Запрос)
	...
	|ПОМЕСТИТЬ СписокОС
	|ИЗ
	|	РегистрСведений.СоставЕГДС.СрезПоследних(&Период, ВнеоборотныйАктив ССЫЛКА Справочник.ОбъектыЭксплуатации) КАК СоставЕГДССрезПоследних
...
Функция ТекстЗапросаТаблицаДокументыПоНМА(Запрос, ТекстыЗапроса, Регистры)
	...
	|	СписокНМАДляЖурнала.Организация КАК Организация,
...
Процедура ИнициализироватьВтСписокНМАДляЖурнала(Запрос)
	...
	|ПОМЕСТИТЬ СписокНМА
	|ИЗ
	|	РегистрСведений.СоставЕГДС.СрезПоследних(&Период,  ВнеоборотныйАктив ССЫЛКА Справочник.НематериальныеАктивы) КАК СоставЕГДССрезПоследних
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ОбесценениеВНА.ЕдиницыГенерирующиеДенежныеСредства КАК
	|			ОбесценениеВНАЕдиницыГенерирующиеДенежныеСредства
	|		ПО СоставЕГДССрезПоследних.ЕГДС = ОбесценениеВНАЕдиницыГенерирующиеДенежныеСредства.ЕГДС
	|
