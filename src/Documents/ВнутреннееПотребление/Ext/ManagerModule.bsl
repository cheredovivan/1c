...
Функция ПолучитьДанныеПоАкцизам(ДокументСсылка) Экспорт
	...
	ТекстыЗапроса = Новый СписокЗначений;
	Запрос        = ЗаполнитьИсходныеПараметры(ДокументСсылка, ТекстыЗапроса);
	...
	|	ЗНАЧЕНИЕ(Перечисление.ВидыОперацийСПодакцизнымТоваром.ВнутреннееПотребление) КАК ВидОперацииСПодакцизнымТоваром,
	...
	|ГДЕ
	|	ТаблицаВидыЗапасов.КатегорияПодакцизногоТовара <> ЗНАЧЕНИЕ(Справочник.КатегорииПодакцизныхТоваров.ПустаяСсылка)
	|	И (ТаблицаВидыЗапасов.СуммаАкцизаРегл <> 0
	...
	Возврат Результаты[Результаты.ВГраница()];
	
КонецФункции

// Формируем запрос по получению данных для заполнения регистра "Движения по прочим активам и пассивам".
//
// Параметры:
//	ДокументСсылка - ДокументСсылка.ВнутреннееПотребление - ссылка на документ.
//
// Возвращаемое значение:
//	РезультатЗапроса - данные по движениям прочим активам и пассивам.
//
Функция ПолучитьДанныеПоПрочимАктивамПассивам(ДокументСсылка) Экспорт
	...
	Регистры.Вставить(ИмяРегистра, Неопределено);
...
Функция ТекстЗапросаТаблицаДвиженияПоПрочимАктивамПассивам(Запрос, ТекстыЗапроса, Регистры)
	...
	|	&Подразделение КАК Подразделение,
	|	&НаправлениеДеятельности КАК НаправлениеДеятельности,
	|	ТаблицаТовары.СтатьяРасходов КАК Статья,
	|	ТаблицаТовары.АналитикаРасходов КАК Аналитика,
	|	ТаблицаТовары.НастройкаСчетовУчета КАК НастройкаСчетовУчета,
	|	ЗНАЧЕНИЕ(Перечисление.ВидыДвиженийПрочихАктивовПассивов.Дебет) КАК ДебетКредит,
	|
	|	СУММА(ТаблицаТовары.СуммаАкцизаРегл) КАК СуммаРегл,
	|	СУММА(ТаблицаТовары.СуммаАкцизаУпр) КАК СуммаУпр,
	|	СУММА(ТаблицаТовары.СуммаАкцизаУпр) КАК СуммаСНДС,
	|	СУММА(ТаблицаТовары.СуммаАкцизаУпр) КАК СуммаБезНДС,
	|
	...
	|ГДЕ
	|	ТаблицаТовары.СтатьяРасходов ССЫЛКА ПланВидовХарактеристик.СтатьиАктивовПассивов
	|	И &ИспользоватьПодакцизныеТовары
	|
	|СГРУППИРОВАТЬ ПО
	|	ТаблицаТовары.СтатьяРасходов,
	|	ТаблицаТовары.АналитикаРасходов,
	|	ТаблицаТовары.НастройкаСчетовУчета,
...
Функция ТекстЗапросаТаблицаВтИсходныеПрочиеРасходы(Запрос, ТекстыЗапроса)
	...
	|	НЕОПРЕДЕЛЕНО КАК ВидДеятельностиНДС,
	|
	|	СУММА(ТаблицаРасходы.СуммаАкцизаУпр) КАК СуммаСНДС,
	|	СУММА(ТаблицаРасходы.СуммаАкцизаУпр) КАК СуммаБезНДС,
	|	СУММА(ТаблицаРасходы.СуммаАкцизаУпр) КАК СуммаБезНДСУпр,
	|	СУММА(ТаблицаРасходы.СуммаАкцизаРегл) КАК СуммаСНДСРегл,
	|	СУММА(ТаблицаРасходы.СуммаАкцизаРегл) КАК СуммаБезНДСРегл,
	|	0 КАК ПостояннаяРазница,
	|	0 КАК ВременнаяРазница,
	|	&ХозяйственнаяОперация КАК ХозяйственнаяОперация,
	|	НЕОПРЕДЕЛЕНО КАК АналитикаУчетаНоменклатуры,
...
Функция ЗаполнитьИсходныеПараметры(ДокументСсылка, ТекстыЗапроса)
	...
	ТекстыЗапроса.Добавить(ТекстЗапроса, "ВтКатегорииПодакцизныхТоваров");
	...
	|	ЕСТЬNULL(ТаблицаПодакцизныеКатегории.КатегорияПодакцизногоТовара, ЗНАЧЕНИЕ(Справочник.КатегорииПодакцизныхТоваров.ПустаяСсылка)) КАК КатегорияПодакцизногоТовара,
	|	ВЫБОР
	|		КОГДА ТаблицаВидыЗапасов.ВидЗапасов.ОблагаетсяАкцизом
	...
	|		ПО ТаблицаВидыЗапасов.АналитикаУчетаНоменклатуры.Номенклатура = ТаблицаПодакцизныеКатегории.Номенклатура
	|ГДЕ
	|	ТаблицаВидыЗапасов.Ссылка = &Ссылка
	|	И ТаблицаПодакцизныеКатегории.КатегорияПодакцизногоТовара <> ЗНАЧЕНИЕ(Справочник.КатегорииПодакцизныхТоваров.ПустаяСсылка)";
	
	ТекстыЗапроса.Добавить(ТекстЗапроса, "ВтВидыЗапасов");
	
	УстановитьПараметрыЗапросаКоэффициентыВалют(Запрос);
	
	СтранаРегистрации = ЗначениеНастроекКлиентСерверПовтИсп.СтранаРегистрацииОрганизации(Реквизиты.Организация);
	Запрос.УстановитьПараметр("СтранаРегистрации", СтранаРегистрации);
	Запрос.УстановитьПараметр("ИспользоватьПодакцизныеТовары", ПолучитьФункциональнуюОпцию("ИспользоватьПодакцизныеТовары"));
