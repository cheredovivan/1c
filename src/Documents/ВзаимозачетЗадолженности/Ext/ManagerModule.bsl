	...
	Для Каждого СтрокаДанных Из ОбновляемыеДанные Цикл
		...
		Исключение
			...
			ИначеЕсли ПричинаИсключения = "Запись" Тогда
				...
				ВызватьИсключение СтрСоединить(НСтр("ru = 'Не удалось обработать документ ""Взаимозачет задолженности"".';
													|en = 'Cannot process the ""AR/AP offset"" document.'"), Символы.ПС);
