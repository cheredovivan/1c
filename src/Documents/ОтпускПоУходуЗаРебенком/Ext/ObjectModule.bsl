	...
	Если ДанныеОДетях.Количество() <> КоличествоДетей Тогда
		Если ДанныеОДетях.Количество() < КоличествоДетей Тогда
			Текст = НСтр("ru = 'Необходимо добавить сведения о детях (%1) на вкладке ""Пилотный проект ФСС""';
						|en = 'Add information about children (%1) on the ""SSF pilot project"" tab'");
		...
		Иначе
			Текст = НСтр("ru = 'Количество строк на вкладке ""Пилотный проект ФСС"" (%1) не соответствует полю ""Количество детей"" = %2 на первой вкладке.';
						|en = 'The number of lines on the ""SSF pilot project"" tab (%1) does not match the ""Number of children"" = %2 field on the first tab.'");
