...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	СписокОписаний.Добавить(НСтр("ru = 'Обновление документов ""Приходный ордер на товары"":';
								|en = 'Update the ""Goods receipt"" documents:'"));
	//++ НЕ УТ
	СписокОписаний.Добавить(НСтр("ru = '- перезаполняет реквизит ""Отправитель"" в документах, распоряжениями для которых выступает ""Производство без заказа"".';
								|en = '- Refills the ""Sender"" attribute in the documents generated based on ""Backflush production"".'"));
	//-- НЕ УТ
	...
	Блокируемые.Добавить(Метаданные.Документы.ПриходныйОрдерНаТовары.ПолноеИмя());
...
Процедура ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	|	ПО РеквизитыПбЗ.Ссылка = РеквизитыДокумента.Распоряжение
	|
	|ГДЕ
	|	НЕ РеквизитыДокумента.Отправитель = РеквизитыПбЗ.Подразделение
	//-- НЕ УТ
	|";
...
Процедура ОбработатьДанныеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	|	ОбновляемыеДанные.Ссылка        КАК Ссылка,
	//++ НЕ УТ
	|	РеквизитыПбЗ.Подразделение      КАК Отправитель,
	//-- НЕ УТ
		...
		Исключение
			...
			Если ПричинаИсключения = 2 Тогда
				ВызватьИсключение НСтр("ru = 'Не удалось обработать обновление документов ""Приходный ордер на товары"".';
										|en = 'Cannot process the update of the ""Goods receipt"" documents.'");
			КонецЕсли;
