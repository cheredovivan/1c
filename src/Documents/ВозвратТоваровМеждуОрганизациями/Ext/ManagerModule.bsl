...
Функция ПолучитьДанныеПоАкцизам(ДокументСсылка) Экспорт
	...
	|	ЕСТЬNULL(ВидыЗапасовПолучателя.ОблагаетсяАкцизом, ЛОЖЬ) КАК ОблагаетсяАкцизом,
	|	ВидыЗапасовПолучателя.ПодразделениеПроизводства КАК ПодразделениеПроизводства,
	|	ТаблицаВидыЗапасов.СпособОпределенияСебестоимости КАК СпособОпределенияСебестоимости,
	|	ДанныеОснования.Дата КАК ДатаДокументаПередачи,
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.ПередачаТоваровМеждуОрганизациями КАК ДанныеОснования
	|		ПО ТаблицаВидыЗапасов.ДокументПередачи = ДанныеОснования.Ссылка
	...
	ПараметрыТекстаЗапроса.ИмяТаблицыТоваров = "ТаблицаВидыЗапасов";
	...
	|	ВЫРАЗИТЬ(ТаблицаВидыЗапасов.СуммаАкциза * &КоэффициентПересчетаИзРеглВУпр КАК ЧИСЛО(31, 2)) КАК СуммаАкцизаУпр,
	|	ЕСТЬNULL(ТаблицаПодакцизныеКатегории.КатегорияТовара,
	...
	|		И ТаблицаВидыЗапасов.ДатаДокументаПередачи < &НачалоМесяцаПериода
	|	ТОГДА
	...
	|		ПО ТаблицаВидыЗапасов.Номенклатура = ТаблицаПодакцизныеКатегории.Номенклатура
	|		И ТаблицаВидыЗапасов.Дата = ТаблицаПодакцизныеКатегории.Период
	|
	|ГДЕ
	...
	ВалютаРегламентированногоУчета =
		ЗначениеНастроекПовтИсп.ВалютаРегламентированногоУчетаОрганизации(Реквизиты.ОрганизацияПолучатель);
	...
	СтранаРегистрации = ЗначениеНастроекКлиентСерверПовтИсп.СтранаРегистрацииОрганизации(Реквизиты.ОрганизацияПолучатель);
	...
	Запрос.УстановитьПараметр("ИспользоватьПодакцизныеТовары", ИспользуютсяПодакцизныеТовары(Реквизиты));
...
//
// Параметры:
//	ДокументОбъект - ДокументОбъект.ВозвратТоваровМеждуОрганизациями, ДанныеФормыСтруктура - данные документа.
//
// Возвращаемое значение:
...
Функция ИспользуютсяПодакцизныеТовары(ДокументОбъект) Экспорт
	...
	Возврат ДокументОбъект.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ВозвратТоваровМеждуОрганизациями;
...
Процедура ЗаполнитьПараметрыИнициализации(Запрос, ДокументСсылка)
	...
	СтранаРегистрации =
		ЗначениеНастроекКлиентСерверПовтИсп.СтранаРегистрацииОрганизации(Реквизиты.ОрганизацияПолучатель);
	//-- НЕ УТ
...
Процедура ОтразитьВУчетеНДС(Запрос, ТекстыЗапроса, Регистры)
	...
	//++ НЕ УТ
	|	ТаблицаЗапасов.СуммаАкциза          КАК СуммаАкциза,
	//-- НЕ УТ
