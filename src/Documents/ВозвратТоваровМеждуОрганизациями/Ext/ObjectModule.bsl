...
Процедура ЗаполнитьВидыЗапасов(Отказ)
	...
	Если ВидыЗапасовПерезаполнены
		...
		И ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ВозвратТоваровМеждуОрганизациями Тогда
		
		ПараметрыЗаполнения = ПараметрыЗаполненияВидовЗапасов("ОрганизацияПолучатель", Ложь);
		ПараметрыЗаполнения.РассчитатьСуммуАкциза = Истина;
		ИмяКолонкиВидЗапасов = "ВидЗапасовПолучателя";
		УчетПодакцизныхТоваров.РассчитатьСуммуАкциза(ЭтотОбъект, ПараметрыЗаполнения, ИмяКолонкиВидЗапасов, Отказ);
...
Функция ВременныеТаблицыДанныхДокумента(ПоТоварамКОформлению = Ложь) Экспорт
	...
	//++ НЕ УТ
	Запрос.УстановитьПараметр("ОблагаетсяАкцизом", ?(Не ПоТоварамКОформлению, ОблагаетсяАкцизом, Ложь));
...
Процедура ЗаполнитьДопКолонкиВидовЗапасов(ПоТоварамКОформлению)
	...
	Если ВидыЗапасов.Найти(Справочники.ВидыЗапасов.ПустаяСсылка(), "ВидЗапасовПолучателя") <> Неопределено Тогда
		...
		|	ТаблицаВидыЗапасов.НомерСтроки КАК НомерСтроки,
		//++ НЕ УТ
		|	ЕСТЬNULL(ДанныеОснования.Дата, &Дата) КАК Дата,
		//-- НЕ УТ
		...
		ПараметрыТекстаЗапроса.ИмяТаблицыТоваров = "ВтИсходнаяТаблицаТоваров";
		...
		|			И ТаблицаТоваров.Дата = ДанныеКатегорий.Период
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		...
		//++ НЕ УТ
		ИменаВыгружаемыхКолонок = ИменаВыгружаемыхКолонок + ", ДокументПередачи";
		//-- НЕ УТ
...
Функция ПроверитьИзменениеРеквизитовДокумента(МенеджерВременныхТаблиц)
	...
	//++ НЕ УТ
	|ОблагаетсяАкцизом, ПодразделениеПроизводства,
	//-- НЕ УТ
	|Контрагент";
...
Функция ПараметрыЗаполненияВидовЗапасов(ИмяПоляОрганизация, ЗаполнениеВидовЗапасов)
	...
	Иначе
		...
		//++ НЕ УТ
		Если Документы.ВозвратТоваровМеждуОрганизациями.ИспользуютсяПодакцизныеТовары(ЭтотОбъект) Тогда
