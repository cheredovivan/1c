...
Процедура СтраницыПриСменеСтраницы(Элемент, ТекущаяСтраница)
	//++ НЕ УТ
	Если ТекущаяСтраница.Имя = "СтраницаДополнительно" И ТребуетсяОбновитьЗаголовокОблагаетсяАкцизом Тогда
		ВычислитьКоличествоПодакцизныхТоваровОтложенно();
		УчетПодакцизныхТоваровКлиент.ПослеРасчетаКоличестваПодакцизныхТоваров(ЭтотОбъект);
	КонецЕсли;
	//-- НЕ УТ
	Возврат; // Не используется в УТ
КонецПроцедуры
...
Процедура ТоварыНоменклатураПриИзменении(Элемент)
	...
	КонецЕсли;
	//++ НЕ УТ
	ТребуетсяОбновитьЗаголовокОблагаетсяАкцизом = ИспользоватьПодакцизныеТовары;
	//-- НЕ УТ
...
Функция ПоместитьТоварыИВидыЗапасовВХранилище()
	...
	//++ НЕ УТ
	Если Документы.ВозвратТоваровМеждуОрганизациями.ИспользуютсяПодакцизныеТовары(Объект) Тогда
...
Процедура ОбновитьИнформациюПоПередачам(ОбновлятьДанные = Истина)
	...
	//++ НЕ УТ
	ИспользоватьПодакцизныеТовары = ЭтоПродажа И ПолучитьФункциональнуюОпцию("ИспользоватьПодакцизныеТовары");
	//-- НЕ УТ
...
//++ НЕ УТ
&НаСервере
Процедура ВычислитьКоличествоПодакцизныхТоваровОтложенно()
	
	ПараметрыРасчета = УчетПодакцизныхТоваров.ПараметрыРасчетаКоличестваПодакцизныхТоваров();
	ПараметрыРасчета.ИмяПоляОрганизация = "ОрганизацияПолучатель";
	ПараметрыОтбора = Новый Структура("ДокументПередачи", Документы.ПередачаТоваровМеждуОрганизациями.ПустаяСсылка());
	ПараметрыРасчета.ПараметрыОтбора = ПараметрыОтбора;
	ДанныеРасчетаКоличестваПодакцизныхТоваров =
		УчетПодакцизныхТоваров.ЗапуститьРасчетКоличестваПодакцизныхТоваров(Объект, ПараметрыРасчета);
	
КонецПроцедуры

&НаКлиенте
Процедура Подключаемый_ОбновитьЗаголовокПоляОблагаетсяАкцизом()
