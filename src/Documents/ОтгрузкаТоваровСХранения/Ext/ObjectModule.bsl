...
Процедура ЗаполнитьНаОснованииЗаказДавальца2_5(ДанныеЗаполнения)
	...
	|	ТаблицаТовары.Характеристика               КАК Характеристика,
	|	ВЫБОР
	|		КОГДА ТаблицаНоменклатуры.ТипНоменклатуры = ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.МногооборотнаяТара)
	...
	|			ТОГДА ТаблицаТовары.КОтгрузкеКоличествоОборот
	|				- ТаблицаТовары.ОтгруженоКоличествоОборот
	|		ИНАЧЕ (ТаблицаТовары.КОтгрузкеКоличествоОборот
	|				- ТаблицаТовары.ОтгруженоКоличествоОборот) / &ТекстЗапросаКоэффициентУпаковки
	|	КОНЕЦ                                      КАК КоличествоУпаковок,
	|	(ТаблицаТовары.КОтгрузкеКоличествоОборот
	|		- ТаблицаТовары.ОтгруженоКоличествоОборот) КАК Количество,
	...
	|	ТаблицаТовары.КодСтроки                    КАК КодСтроки
	|ИЗ
	|	ВТ_РаспоряженияНаОтгрузкуОборотыРазвернутая КАК ТаблицаТовары
	...
	|	И (ТаблицаТовары.КОтгрузкеКоличествоОборот
	|		- ТаблицаТовары.ОтгруженоКоличествоОборот) <> 0
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
...
Процедура ЗаполнитьДокументНаОснованииОтчетДавальцу2_5(ДанныеЗаполнения)
	...
	ПараметрыФормированияТаблицы.ТекстУсловия =
		...
		|				ТаблицаТоваров КАК ТаблицаТоваров)";
	ПараметрыФормированияТаблицы.Группировка = "Распоряжение, Номенклатура, Характеристика, КодСтроки, Склад, Серия";
	ПараметрыФормированияТаблицы.Показатели = "Заказано, КОформлению";
	...
	|	ИЗ
	|		ВТ_РаспоряженияНаОтгрузкуОборотыРазвернутая КАК Обороты
	|
	...
	|		И НЕ ВЫРАЗИТЬ(Обороты.Склад КАК Справочник.Склады).Ссылка ЕСТЬ NULL
	|		И ИСТИНА В
	|			(ВЫБРАТЬ ПЕРВЫЕ 1
	|				ИСТИНА
	|			ИЗ
	...
	|				И ТаблицаТоваров.Серия = Обороты.Серия
	|				И ТаблицаТоваров.Склад = Обороты.Склад)) КАК ОстаткиБезЗаказа
	|
	|СГРУППИРОВАТЬ ПО
