...
Функция ПараметрыЗаполненияДокумента() Экспорт
	...
	ПараметрыЗаполнения.Вставить("ИмяРегистраЗаказ",         "РаспоряженияНаОтгрузкуИВозврат");
...
Процедура ИнициализироватьПараметрыЗаполнения(ПараметрыЗаполнения, РеквизитыШапки, МассивЗаказов) Экспорт
	...
	Если ПараметрыЗаполнения.ЗаполнятьПоОрдеру = Неопределено Тогда
		...
		|		РаспоряженияНаОтгрузку.ОтгруженоКоличествоОборот) КАК КПередачеОстаток
		|ИЗ
...
Процедура ЗаполнитьПоЗаказамОрдерам(Товары, Регистратор, ПараметрыЗаполнения, ШтрихкодыУпаковок = Неопределено) Экспорт
	...
	ТекстЗапросаРегистраЗаказы =
		РегистрыНакопления.РаспоряженияНаОтгрузкуИВозврат.ТекстЗапросаОстатки("ВтДанныеУчета", Отбор, "КПередаче <> 0");
...
Функция ТекстЗапросаТаблицаЗаказы(ИмяВременнойТаблицы = "") Экспорт
	...
	|				ТОГДА (РаспоряженияОбороты.КОтгрузкеКоличествоОборот
	...
	//++ НЕ УТКА
	|			КОГДА &ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ПередачаДавальцу2_5)
	|			И РаспоряженияДвижения.Показатель = ЗНАЧЕНИЕ(Перечисление.ПоказателиРаспоряженийНаОтгрузкуИВозврат.КОтгрузке)
...
Функция ПолучитьРезультатЗапросаПоОстаткамЗаказов(ДанныеОтбора, Склад = Неопределено, МассивЗаказов = Неопределено)
	...
	|		-ЗаказыОстатки.ОтгруженоКоличествоОборот КАК Количество
	|	ИЗ
	|		#РаспоряженияНаОтгрузкуОборотыРазвернутая2 КАК ЗаказыОстатки
	|
	...
	ПараметрыФормированияТаблицы.ТекстУсловия =
		...
		|			ИЗ
		|				Заказ)";
	ПараметрыФормированияТаблицы.Группировка = "Регистратор, Распоряжение, Номенклатура, Характеристика, Склад, Серия, КодСтроки";
