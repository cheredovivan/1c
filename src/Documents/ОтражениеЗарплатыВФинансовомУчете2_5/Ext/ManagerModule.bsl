...
Функция РаспределитьНачисленияПоЭтапамПроизводства(ПараметрыЗапроса)
	...
	Для Каждого РаспределяемаяСтрока Из РаспределяемыеРасходы Цикл
		...
		МассивРаспределенияРегл = ОбщегоНазначенияУТ.РаспределитьСуммуПропорциональноКоэффициентамРазныхЗнаков(
		...
		Иначе
			МассивРаспределенияУпр = ОбщегоНазначенияУТ.РаспределитьСуммуПропорциональноКоэффициентамРазныхЗнаков(
...
Процедура ТекстЗапросаТаблицаВыработкиСотрудников(ТекстыЗапроса)
	...
	|	ВыработкаПоПартиям.ПартияПроизводства                      КАК ПартияПроизводства,
	...
	|	
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ОтражениеЗарплатыВФинансовомУчете2_5 КАК СторнируемыеДокументы
	...
	|	ВыработкаПоПартиям.ПартияПроизводства,
	...
	|	ПартияПроизводства
	|";
	ТекстыЗапроса.Добавить(ТекстЗапроса, "ПериодыБазыРаспределенияПартийСоСторнированием");
	...
	|	Трудозатраты.СтатьяКалькуляцииВыработки КАК СтатьяКалькуляцииВыработки,
	...
	|		ВЫБОР
	|			КОГДА &ДетализацияДоФизическихЛиц
	...
	|		ТрудозатратыНЗП.НормативнаяСтоимость КАК НормативнаяСтоимость,
	...
	|		ПО ТрудозатратыНЗП.Организация = ПериодыБазыРаспределения.Организация
	|		И ТрудозатратыНЗП.ПартияПроизводства = ПериодыБазыРаспределения.ПартияПроизводства
	|		
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.ПартииПроизводства КАК СпрПартииПроизводства
	|		ПО СпрПартииПроизводства.Ссылка = ТрудозатратыНЗП.ПартияПроизводства
	|	
	|	ГДЕ
	|		ТрудозатратыНЗП.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Приход)
	|		И ТрудозатратыНЗП.Период МЕЖДУ ПериодыБазыРаспределения.ДатаНачала И ПериодыБазыРаспределения.ДатаОкончания
	|		И ТИПЗНАЧЕНИЯ(ТрудозатратыНЗП.Регистратор) В(
	|			ТИП(Документ.ВыработкаСотрудников),
	|			ТИП(Документ.Сторно))
	|		И НЕ ТрудозатратыНЗП.НормативнаяСтоимость = 0
	...
	|	Трудозатраты.СтатьяКалькуляцииВыработки,
