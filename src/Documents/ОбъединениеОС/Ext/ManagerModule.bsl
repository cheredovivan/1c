...
Процедура ТекстЗапросаВтОтражениеДоходовРасходов(ТекстыЗапроса)
	...
	|	ДанныеДокумента.Период КАК ДатаДокумента,
	|	МАКСИМУМ(СоставЕГДС.Период) КАК МаксимальныйПериод
	|ПОМЕСТИТЬ ВтВсеОСДляПоискаЕГДС
	|ИЗ
	|	ДанныеДокументаТаблицаОС КАК ТаблицаОС
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.СоставЕГДС КАК СоставЕГДС
	|			ВНУТРЕННЕЕ СОЕДИНЕНИЕ ДанныеДокументаРеквизиты КАК ДанныеДокумента
	|			ПО ДанныеДокумента.Организация = СоставЕГДС.Организация
	|			И СоставЕГДС.Период <= ДанныеДокумента.Период
	|		ПО ТаблицаОС.ОсновноеСредство = СоставЕГДС.ВнеоборотныйАктив
	|СГРУППИРОВАТЬ ПО
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ВтВсеОСДляПоискаЕГДС.ОсновноеСредство КАК ОсновноеСредство,
	|	ВтВсеОСДляПоискаЕГДС.Организация КАК Организация,
	|	ВтВсеОСДляПоискаЕГДС.ДатаДокумента КАК ДатаДокумента,
	|	ЕСТЬNULL(СоставЕГДС.ЕГДС, ЗНАЧЕНИЕ(Справочник.ЕдиницыГенерирующиеДенежныеСредства.ПустаяСсылка)) КАК ЕГДС,
	|	ЕСТЬNULL(ВтВсеОСДляПоискаЕГДС.МаксимальныйПериод, ВтВсеОСДляПоискаЕГДС.ДатаДокумента) КАК ДатаПоследнегоИзмененияЕГДС
	|ПОМЕСТИТЬ ВтВсеОСиЕГДС
	|ИЗ
	|	ВтВсеОСДляПоискаЕГДС КАК ВтВсеОСДляПоискаЕГДС
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.СоставЕГДС КАК СоставЕГДС
	|		ПО ВтВсеОСДляПоискаЕГДС.Организация = СоставЕГДС.Организация
	|		И ВтВсеОСДляПоискаЕГДС.ОсновноеСредство = СоставЕГДС.ВнеоборотныйАктив
	|		И ВтВсеОСДляПоискаЕГДС.МаксимальныйПериод = СоставЕГДС.Период
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	ЕГДС,
	|	ОсновноеСредство,
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ВтВсеОСиЕГДС.ОсновноеСредство КАК ОсновноеСредство,
	|	ВтВсеОСиЕГДС.Организация КАК Организация,
	|	ВтВсеОСиЕГДС.ЕГДС КАК ЕГДС,
	|	ВтВсеОСиЕГДС.ДатаПоследнегоИзмененияЕГДС КАК ДатаПоследнегоИзмененияЕГДС,
	|	ОбесценениеВНАОсновныеСредства.Ссылка.Дата КАК ДатаОбесценения,
	|	ОбесценениеВНАОсновныеСредства.Ссылка КАК ДокументОбесценения,
	|	1 КАК ИДПодзапроса
	|ПОМЕСТИТЬ ВтВсеДокументыОбесцененияПоОС
	|ИЗ
	|	ВтВсеОСиЕГДС КАК ВтВсеОСиЕГДС
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ОбесценениеВНА.ОсновныеСредства КАК ОбесценениеВНАОсновныеСредства
	|		ПО ВтВсеОСиЕГДС.ОсновноеСредство = ОбесценениеВНАОсновныеСредства.ОсновноеСредство
	|		И ОбесценениеВНАОсновныеСредства.Ссылка.Дата <= ВтВсеОСиЕГДС.ДатаДокумента
	|		И ОбесценениеВНАОсновныеСредства.Ссылка.Проведен
	|ГДЕ
	|	ВтВсеОСиЕГДС.ЕГДС = ЗНАЧЕНИЕ(Справочник.ЕдиницыГенерирующиеДенежныеСредства.ПустаяСсылка)
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	...
	|	ВтВсеОСиЕГДС.ЕГДС КАК ЕГДС,
	|	ВтВсеОСиЕГДС.ДатаПоследнегоИзмененияЕГДС КАК ДатаПоследнегоИзмененияЕГДС,
	|	ОбесценениеВНАЕГДС.Ссылка.Дата КАК Дата,
	|	ОбесценениеВНАЕГДС.Ссылка КАК Ссылка,
	|	2 КАК ИДПодзапроса
	|ИЗ
	|	ВтВсеОСиЕГДС КАК ВтВсеОСиЕГДС
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ОбесценениеВНА.ЕдиницыГенерирующиеДенежныеСредства КАК ОбесценениеВНАЕГДС
	|		ПО ВтВсеОСиЕГДС.ЕГДС = ОбесценениеВНАЕГДС.ЕГДС
	|		И ОбесценениеВНАЕГДС.Ссылка.Дата <= ВтВсеОСиЕГДС.ДатаДокумента
	|		И ОбесценениеВНАЕГДС.Ссылка.Проведен
	|ГДЕ
	|	ВтВсеОСиЕГДС.ЕГДС <> ЗНАЧЕНИЕ(Справочник.ЕдиницыГенерирующиеДенежныеСредства.ПустаяСсылка)
	|
	|ИНДЕКСИРОВАТЬ ПО
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ВтВсеДокументыОбесцененияПоОС.ОсновноеСредство КАК ОсновноеСредство,
	|	ВтВсеДокументыОбесцененияПоОС.Организация КАК Организация,
	|	ВтВсеДокументыОбесцененияПоОС.ЕГДС КАК ЕГДС,
	|	МАКСИМУМ(ВтВсеДокументыОбесцененияПоОС.ДатаОбесценения) КАК ДатаОбесценения
	|ПОМЕСТИТЬ ВтФильтрДокументовОбесценения
	|ИЗ
	|	ВтВсеДокументыОбесцененияПоОС КАК ВтВсеДокументыОбесцененияПоОС
	|СГРУППИРОВАТЬ ПО
	|	ВтВсеДокументыОбесцененияПоОС.ОсновноеСредство,
	|	ВтВсеДокументыОбесцененияПоОС.Организация,
	|	ВтВсеДокументыОбесцененияПоОС.ЕГДС
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	ОсновноеСредство,
	|	Организация,
	...
	|	ВтВсеДокументыОбесцененияПоОС.ОсновноеСредство КАК ОсновноеСредство,
	|	ВтВсеДокументыОбесцененияПоОС.Организация КАК Организация,
	|	ВтВсеДокументыОбесцененияПоОС.ДокументОбесценения КАК ДокументОбесценения
	|ПОМЕСТИТЬ ВтОтборДокументовОбесценения
	|ИЗ
	|	ВтВсеДокументыОбесцененияПоОС КАК ВтВсеДокументыОбесцененияПоОС
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВтФильтрДокументовОбесценения КАК ВтФильтрДокументовОбесценения
	|		ПО ВтФильтрДокументовОбесценения.ОсновноеСредство = ВтВсеДокументыОбесцененияПоОС.ОсновноеСредство
	|		И ВтФильтрДокументовОбесценения.Организация = ВтВсеДокументыОбесцененияПоОС.Организация
	|		И ВтФильтрДокументовОбесценения.ЕГДС = ВтВсеДокументыОбесцененияПоОС.ЕГДС
	|		И ВтФильтрДокументовОбесценения.ДатаОбесценения = ВтВсеДокументыОбесцененияПоОС.ДатаОбесценения
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	ДокументОбесценения
	|;
	...
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	ДанныеДокумента.Ссылка КАК Ссылка,
	|	ВтОтборДокументовОбесценения.ОсновноеСредство КАК ВнеоборотныйАктив,
	|	ОбесценениеВНА.СтатьяРасходов КАК СтатьяРасходов,
	|	ОбесценениеВНА.АналитикаРасходов КАК АналитикаРасходов,
	|	ОбесценениеВНА.СтатьяДоходов КАК СтатьяДоходов,
	|	ОбесценениеВНА.АналитикаДоходов КАК АналитикаДоходов
	|ПОМЕСТИТЬ ВтОтражениеДоходовРасходов
	|ИЗ
	|	ВтОтборДокументовОбесценения КАК ВтОтборДокументовОбесценения
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ОбесценениеВНА КАК ОбесценениеВНА
	|		ПО ВтОтборДокументовОбесценения.ДокументОбесценения = ОбесценениеВНА.Ссылка
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ДанныеДокументаРеквизиты КАК ДанныеДокумента
	|		ПО ИСТИНА
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|УНИЧТОЖИТЬ ВтВсеОСДляПоискаЕГДС;
	|УНИЧТОЖИТЬ ВтВсеОСиЕГДС;
	|УНИЧТОЖИТЬ ВтВсеДокументыОбесцененияПоОС;
	|УНИЧТОЖИТЬ ВтФильтрДокументовОбесценения;
	|УНИЧТОЖИТЬ ВтОтборДокументовОбесценения";
