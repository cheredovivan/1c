...
Функция СтруктураСообщения11010(ТекстXML) Экспорт
	...
	// Чтение реквизитов.
	ИменаУзлов = "noticeType, batchNo, socialAssistNum,
	|registerRecieve, paymentSend, paymentDone, paymentCanceling, noticeSend";
...
Процедура ЗаполнитьВложенныеДанные(Документ, ЭлементDOM, НеобработанныеУзлы = Неопределено)
	...
	Если УзлыДанных.infoType <> Неопределено Тогда
	...
	Если УзлыДанных.period <> Неопределено Тогда
		УзлыПериода = СериализацияБЗК.УзлыЭлементаDOM(УзлыДанных.period, "periodStart, periodEnd", НеобработанныеУзлы);
	...
	КонецЕсли;
	Если УзлыДанных.summa <> Неопределено Тогда
	...
	КонецЕсли;
	Если УзлыДанных.dutySum <> Неопределено Тогда
