...
Процедура ДекорацияНастройкиЗаполненияУведомленияНажатие(Элемент)
	...
	ОткрытьФорму("Документ.УведомлениеОКонтролируемыхСделках.Форма.НастройкиЗаполнения", ПараметрыОткрытияФормы, ЭтотОбъект, Объект.Ссылка);
...
// Формирует тескт оповещения пользователю о выгруженных файлах
//
// Параметры:
//  ПолученныеФайлы - Массив Из Структура - массив структур, содержащих параметры файла.
//  ДополнительныеПараметры - Неопределено - дополнительные параметры оповещения.
//
&НаКлиенте
