...
Процедура ПриОпределенииДоступныхВидовОповещенийПользователей(ОписаниеВидовОповещений) Экспорт
	...
	НовыйВидОповещений.Наименование = НСтр("ru = 'Изменение статуса заявки на расходование денежных средств';
											|en = 'Payment request status change'", ОбщегоНазначения.КодОсновногоЯзыка());
	...
	НовыйВидОповещений.ШаблонТекстаОповещения = "ru = 'Документу установлен статус [Статус]';
												|en = 'The document status is set to [Статус]'"; // @NStr-1
...
Функция ТекстЗапросаИсточникФактаПоТипамДанныхУчета(Расход, Приход, ИмяВременнойТаблицы = "") Экспорт
	...
	Если Приход = Перечисления.ТипыДанныхУчета.ДенежныеСредства Тогда
		...
		|	ТаблицаДанных.Подразделение КАК ПодразделениеПлатежа,
		|	НЕОПРЕДЕЛЕНО КАК НаправлениеДеятельности,
		|	ВЫБОР
