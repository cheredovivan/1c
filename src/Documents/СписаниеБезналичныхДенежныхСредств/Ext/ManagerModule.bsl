...
Процедура ЗаполнитьНастройкиПолейФормы(Настройки) Экспорт
	...
	//++ НЕ УТ
	Финансы.НовыйОтбор(ПодГруппаИли, "ХозяйственнаяОперация", Операции.ОплатаАрендодателю);
	//-- НЕ УТ
...
Функция ТекстЗапросаИсточникФактаПоТипамДанныхУчета(Расход, Приход, ИмяВременнойТаблицы = "") Экспорт
	...
	|	&Организация КАК Организация,
	|	&ПодразделениеПлатежа КАК Подразделение,
	|	&ПодразделениеПлатежа КАК ПодразделениеПлатежа,
	|	&НаправлениеДеятельности КАК НаправлениеДеятельности,
