...
Функция ГотовностьКОтправке(ПараметрыПроверки, ТолькоПроверка, Прогрессор) Экспорт
	...
	Если КоличествоСПустойОтчетностью > 0 Тогда
		...
		МнЧислоСлова = ?(КоличествоСПустойОтчетностью = 1, НСтр("ru = 'компании';
																|en = 'companies'"), НСтр("ru = 'компаниям:';
																						|en = 'companies:'"));
		ТекстШаблона = НСтр("ru = 'В пакете не отмечены отчеты к отправке по %1 %2. Необходимо заполнить хотя бы один в пакете перед отправкой';
							|en = 'There are no reports marked for sending by %1 %2 in the package. Select at least one in the package before sending'");
		ДобавитьРезультатПоОтчету(РезультатПроверки, Строка(ПараметрыПроверки.Предмет), ,
		...
		Если Отчет.ЕстьXML Тогда
			...
			Если ФайлПомеченНаУдаление Тогда
				...
				Отчет.ОписаниеОшибки = НСтр("ru = 'Присоединенный файл XML помечен на удаление.';
											|en = 'XML attachment is marked for deletion.'");
			КонецЕсли;
...
Процедура ГотовностьКОтправкеПрисоединенногоФайла(ДанныеДляПроверки, ПараметрыПроверки, РезультатПроверки)
	...
	Если ФайлПомеченНаУдаление Тогда
		...
		Отчет.ОписаниеОшибки = НСтр("ru = 'Присоединенный файл помечен на удаление.';
									|en = 'Attachment is marked for deletion.'");
		Возврат;
	КонецЕсли;
