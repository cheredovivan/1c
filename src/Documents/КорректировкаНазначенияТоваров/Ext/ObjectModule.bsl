...
Процедура ПроверитьЗаполнениеНовогоНазначения(Отказ)
	...
	ЭтоДавальческоеНазначение =
		...
		Или ТипНазначения = Перечисления.ТипыНазначений.Давальческое2_5;
		
	Если ЭтоДавальческоеНазначение
			И Не ВидОперации = Перечисления.ВидыОперацийКорректировкиНазначения.ПроизвольнаяКорректировкаНазначений Тогда
		...
		Для Каждого СтрокаТовары Из Товары Цикл
			...
			Если Не СтрокаТовары.НовоеНазначение.Пустая() И Не СтрокаТовары.ИсходноеНазначение.Пустая() Тогда
		...
		Если Таблица.Количество() > 0 Тогда
			...
			|ГДЕ
			|	НЕ ВЫБОР
			// Переброска под один договор допустима.
			
			//++ Устарело_Переработка24
			
			|		КОГДА Таблица.ИсходныйТип = ЗНАЧЕНИЕ(Перечисление.ТипыНазначений.Давальческое21)
			|			ТОГДА Таблица.НовыйТип = ЗНАЧЕНИЕ(Перечисление.ТипыНазначений.Давальческое21)
			|					И Таблица.ИсходныйПартнер = Таблица.НовыйПартнер
			|					И Таблица.ИсходныйДоговор = Таблица.НовыйДоговор
			|				ИЛИ Таблица.НовыйТип = ЗНАЧЕНИЕ(Перечисление.ТипыНазначений.Собственное)
			...
			|				ЗНАЧЕНИЕ(Перечисление.ТипыНазначений.ДавальческоеМатериалыПодЭтап22),
			|				ЗНАЧЕНИЕ(Перечисление.ТипыНазначений.Давальческое2_5))
			|			ТОГДА Таблица.ИсходныйТип = ЗНАЧЕНИЕ(Перечисление.ТипыНазначений.Собственное)
			|		ИНАЧЕ ИСТИНА
			|	КОНЕЦ";
			...
			Пока Выборка.Следующий() Цикл
				...
				ИсходныйТипСтарыеДавальцы = Выборка.ИсходныйТип = Перечисления.ТипыНазначений.Давальческое21;
				НовыйТипСтарыеДавальцы = Выборка.НовыйТип = Перечисления.ТипыНазначений.Давальческое21;
				
				//-- Устарело_Переработка24
				
				ИсходныйТипНовыеДавальцы = Выборка.ИсходныйТип = Перечисления.ТипыНазначений.ДавальческоеМатериалыПодЭтап22
						
						//++ Устарело_Переработка24
						
						Или Выборка.ИсходныйТип = Перечисления.ТипыНазначений.ДавальческоеМатериалы22
						
						//-- Устарело_Переработка24
						
						Или Выборка.ИсходныйТип = Перечисления.ТипыНазначений.Давальческое2_5;
						
				НовыйТипНовыеДавальцы = Выборка.НовыйТип = Перечисления.ТипыНазначений.ДавальческоеМатериалыПодЭтап22
						
						//++ Устарело_Переработка24
						
						Или Выборка.НовыйТип = Перечисления.ТипыНазначений.ДавальческоеМатериалы22
						
						//-- Устарело_Переработка24
						
						Или Выборка.НовыйТип = Перечисления.ТипыНазначений.Давальческое2_5;
				
				ТекстСообщения = "";
				
				//++ Устарело_Переработка24
				
				Если ИсходныйТипСтарыеДавальцы И НовыйТипНовыеДавальцы Тогда
					
					ТекстСообщения = СтрШаблон(
						НСтр("ru = 'Не допускается корректировка назначений давальческого сырья производства версии 2.1 на назначения производства версии 2.2. Строка %1.';
							|en = 'Cannot adjust customer-provided material assignments of Production 2.1 to assignments of Production 2.2. Line %1.'"),
						Выборка.НомерСтроки);
					
				ИначеЕсли ИсходныйТипНовыеДавальцы И НовыйТипСтарыеДавальцы Тогда
					
					ТекстСообщения = СтрШаблон(
						НСтр("ru = 'Не допускается корректировка назначений давальческого сырья производства версии 2.2 на назначения производства версии 2.1. Строка %1.';
							|en = 'Cannot adjust customer-provided material assignments of Production 2.2 to assignments of Production 2.1. Line %1.'"),
						Выборка.НомерСтроки);
				...
				ИначеЕсли ВидОперации = Перечисления.ВидыОперацийКорректировкиНазначения.СнятьРезерв
						Или ВидОперации = Перечисления.ВидыОперацийКорректировкиНазначения.СнятьРезервПоМногимНазначениям Тогда
					
					Если Выборка.ИсходныйТип = Перечисления.ТипыНазначений.ДавальческоеМатериалыПодЭтап22 Тогда
						
						ТекстСообщения = НСтр("ru = 'Для материалов, обособленных под производственный этап (%1) допускается обособление под производственные этапы этого же договора или под договор в целом. В строке %2 указано назначение по другому договору (%3)';
					...
					ИначеЕсли Выборка.ИсходныйТип = Перечисления.ТипыНазначений.Давальческое21 Тогда
						
						ТекстСообщения = НСтр("ru = 'Для товаров, обособленных под заказ давальца версии произодства 2.1, допускается обособление под другие заказы давальца по этому же договору (%1). В строке %2 указано назначение заказа по другому договору (%3)';
												|en = 'Items assigned for the material provider order of Production 2.1 can be assigned for other material provider orders under the same contract (%1). In line %2, the order assignment under another contract (%3) is specified'");
						
					ИначеЕсли Выборка.ИсходныйТип = Перечисления.ТипыНазначений.ДавальческоеМатериалы22 Тогда
						
						ТекстСообщения = НСтр("ru = 'Для материалов, обособленных под договор (%1) допускается обособление под производственные этапы этого же договора. В строке %2 указано назначение по другому договору (%3)';
					...
					ИначеЕсли Выборка.ИсходныйТип = Перечисления.ТипыНазначений.Давальческое2_5 Тогда
						
						ТекстСообщения = НСтр("ru = 'Для товаров, обособленных под договор (%1) допускается обособление под производственные этапы этого же договора. В строке %2 указано назначение по другому договору (%3)';
												|en = 'Items assigned for the contract (%1) can be assigned for production stages of the same contract. In line %2, the assignment for another contract (%3) is specified'");
						
					КонецЕсли;
					
					ТекстСообщения = СтрШаблон(ТекстСообщения, Выборка.ИсходныйДоговор, Выборка.НомерСтроки, Выборка.НовыйДоговор);
					
				Иначе
					
					Если Выборка.НовыйТип = Перечисления.ТипыНазначений.ДавальческоеМатериалыПодЭтап22 Тогда
						
						ТекстСообщения = НСтр("ru = 'Не допускается обособление под производственные этапы договора (%1) из остатков обособленных под другой договор (или производственные этапы). В строке %2 указано назначение по другому договору (%3)';
					...
					ИначеЕсли Выборка.НовыйТип = Перечисления.ТипыНазначений.Давальческое21 Тогда
						...
						ТекстСообщения =  НСтр("ru = 'По назначению заказа давальца версии производства 2.1 допускается обособление из остатков обособленных по заказам этого же договору (%1). В строке %2 указано назначение заказа по другому договору (%3)';
					...
					ИначеЕсли Выборка.НовыйТип = Перечисления.ТипыНазначений.ДавальческоеМатериалы22 Тогда
						...
						ТекстСообщения = НСтр("ru = 'Не допускается обособление под договор (%1) из остатков обособленных под другие договоры с давальцем (или производственные этапы). В строке %2 указано назначение по другому договору (%3)';
					...
					ТекстСообщения = СтрШаблон(ТекстСообщения, Выборка.НовыйДоговор, Выборка.НомерСтроки, Выборка.ИсходныйДоговор);
