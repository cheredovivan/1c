...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	//++ НЕ УТ
	ПараметрыНастройки = Документы.Бронирование.ПараметрыНастройкиСчетовУчетаДляСпособаСписанияНапрямую();
...
Процедура ОбработкаПроверкиЗаполненияНаСервере(Отказ, ПроверяемыеРеквизиты)
	...
	Если Объект.ПринятьНДСКВычету
		...
		ТекстОшибки = НСтр("ru = 'Указан некорректный код вида операции.';
							|en = 'An incorrect transaction type code is specified.'");
		ОбщегоНазначения.СообщитьПользователю(
	...
	ИначеЕсли НавигационнаяСсылкаФорматированнойСтроки = "ВключитьВАвансовыйОтчет" Тогда
		...
		Если ПокупкаПолностьюВозвращена() Тогда
			...
			ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru = 'Покупка полностью возвращена. Авансовый отчет не требуется.';
															|en = 'The purchase is fully refunded. No expense report is required.'"));
			Возврат;
		КонецЕсли;
		...
		ПараметрыФормы.Вставить("НеЗаписыватьРедактируемыйДокументЗакупки", Истина);
	...
	Если ЗначениеЗаполнено(Объект.ФИОПассажира) Тогда
		ПоказатьВопрос(ОписаниеОповещенияПриИзменении,
		               НСтр("ru = 'Перезаполнить ФИО пассажира?';
							|en = 'Refill the passenger''s full name?'"),
...
Процедура ВидимостьНесколькихОпераций()
	...
	Если НесколькоОпераций ИЛИ Объект.СпособСписанияНапрямую Тогда
...
Процедура АктуализироватьЗакупкуПодДеятельность(Заполнить = Ложь)
	...
	ПараметрыЗаполнения = Документы.Бронирование.ПараметрыЗаполненияВидаДеятельностиНДС(Объект);
