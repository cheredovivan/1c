...
Процедура РазделыТаможеннаяСтоимостьПриИзменении(Элемент)
	...
	ЗаполнитьЗначенияСвойств(ЭтаФорма, РазностиИтогов(Объект.Разделы, Объект.Товары));
	РаспределитьНаТоварыСервер(Истина);
КонецПроцедуры
...
Процедура РаспределитьНаТоварыСервер(РаспределятьТаможеннуюСтоимость = Истина)
	...
	Для Каждого Раздел Из Разделы Цикл
		...
		Для каждого Товар Из ТоварыРаздела Цикл
			...
			Если Раздел.СуммаНДС <> 0. Тогда
				СтрокаТовары = Объект.Товары[Товар.НомерСтроки-1];
				СтрокаТовары.СуммаНДС = ЦенообразованиеКлиентСервер.РассчитатьСуммуНДС(
					СтрокаТовары.ТаможеннаяСтоимость + СтрокаТовары.СуммаПошлины + СтрокаТовары.СуммаАкциза,
					СтрокаТовары.СтавкаНДС,
					Ложь);
			Иначе
				Объект.Товары[Товар.НомерСтроки-1].СуммаНДС = 0;
			КонецЕсли;
		КонецЦикла;
	КонецЦикла;
...
Процедура АктуализироватьСуммуНДС()
	...
	Если Объект.ИспользоватьРазделы Тогда
		...
		СуммыПоРазделам = Объект.Товары.Выгрузить(, "НомерРаздела, СуммаАкциза, СуммаНДС");
		СуммыПоРазделам.Свернуть("НомерРаздела","СуммаАкциза, СуммаНДС");
		...
		Для Каждого Раздел Из Объект.Разделы Цикл
			...
			Если СтрокаРаздела <> Неопределено Тогда
				...
				Раздел.СуммаНДС = СтрокаРаздела.СуммаНДС;
			КонецЕсли;
		КонецЦикла;
...
Процедура ПересчитатьСуммыВРазделе(ПоВсемРазделам = Ложь)
	...
	Для Каждого Раздел Из Разделы Цикл
		...
		Для Каждого СтрокаРаздела Из ТоварыРаздела Цикл
			...
			Раздел.СуммаНДС = Раздел.СуммаНДС + СтрокаРаздела.СуммаНДС;
		КонецЦикла;
...
Процедура ПересчитатьСуммыВРазделахНаСервере(ДанныеРаздела)
	...
	Для Каждого СтрокаРаздела Из Разделы Цикл
		ТоварыРаздела = Объект.Товары.Выгрузить(Отбор, "СуммаАкциза, СуммаНДС");
		ТоварыРаздела.Свернуть(,"СуммаАкциза, СуммаНДС");
		Раздел = Объект.Разделы[СтрокаРаздела.НомерСтроки-1];
		Раздел.СуммаАкциза = ТоварыРаздела[0].СуммаАкциза;
		Раздел.СуммаНДС = ТоварыРаздела[0].СуммаНДС;
	КонецЦикла;
...
Процедура НастроитьЭлементыФормы()
	...
	Если ВалютыДокументаИРеглУчетаОтличаются Тогда
		ТекстЗаголовкаСуммаАкциза = СтрШаблон(НСтр("ru = 'Сумма акциза, %1';
													|en = 'Excise amount, %1'"), ВалютаРегламентированногоУчета);
	Иначе
