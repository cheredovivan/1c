...
Процедура ДобавитьВыбранныеОС(ОСВыделенныеСтроки)
	...
	Для Каждого СтрокаОС Из ОСВыделенныеСтроки Цикл
		...
		Если КорзинаОС.НайтиСтроки(ФильтрПоискаОС).Количество() = 0 Тогда
			...
			СтрокаВыбранныхОС.Организация = ТекДанныеОС.Организация;
		КонецЕсли;
...
Процедура ДобавитьВыбранныеНМА(НМАВыделенныеСтроки)
	...
	Для Каждого СтрокаНМА Из НМАВыделенныеСтроки Цикл
		...
		Если КорзинаНМА.НайтиСтроки(ФильтрПоискаНМА).Количество() = 0 Тогда
			...
			СтрокаВыбранныхНМА.Организация = ТекДанныеНМА.Организация;
		КонецЕсли;
...
Процедура ИзменитьПривязку(ПараметрыПривязки)
	...
	Для Каждого СтрокаВыбранныхАктивов Из ПараметрыПривязки.ВыбранныеАктивы Цикл
		СписокОрганизаций.Добавить(СтрокаВыбранныхАктивов.Организация);
	КонецЦикла;

	Для Каждого Организация Из СписокОрганизаций Цикл
		
		СписокАктивовОрганизации = ПараметрыПривязки.ВыбранныеАктивы.НайтиСтроки(Новый Структура("Организация", Организация));
		
		ПервыйАктив = СписокАктивовОрганизации.Получить(0);
		ЭтоОС = ПервыйАктив.Свойство("ОсновноеСредство");
		Если ЭтоОС Тогда
			ИмяПоля = "ОсновноеСредство";
			ИмяТаблицыДокумента = "ОС";
			ИмяФормыДокумента = "Документ.ИзменениеПараметровОС2_4.Форма.ФормаДокумента";
		Иначе
			ИмяПоля = "НематериальныйАктив";
			ИмяТаблицыДокумента = "НМА";
			ИмяФормыДокумента = "Документ.ИзменениеПараметровНМА2_4.Форма.ФормаДокумента";
		КонецЕсли;
		
		СписокАктивов = Новый Массив;
		Для Каждого СтрокаАктивов из СписокАктивовОрганизации Цикл
			СписокАктивов.Добавить(Новый Структура( ИмяПоля, СтрокаАктивов[ИмяПоля]));
		...
		ЗначенияЗаполнения.Вставить(ИмяТаблицыДокумента, СписокАктивов);
		ПараметрыОткрытия = Новый Структура("ЗначенияЗаполнения", ЗначенияЗаполнения);
		
		ОткрытьФорму(ИмяФормыДокумента, ПараметрыОткрытия, ЭтотОбъект, Новый УникальныйИдентификатор());
...
Процедура УстановитьПараметрыДинамическихСписков()
	СписокОрганизаций = ОбесценениеВНАСервер.ПолучитьОрганизацииДляОбесценения(Организация);
	
	ОС.Параметры.УстановитьЗначениеПараметра("Организация", СписокОрганизаций);
	НМА.Параметры.УстановитьЗначениеПараметра("Организация", СписокОрганизаций);
