...
Процедура ПриОткрытии(Отказ)
	...
	//++ НЕ УТКА
	НачатьОжиданиеОбновленияИнформационнойПанели();
	//-- НЕ УТКА
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	Если ИмяСобытия = "Запись_РесурсныеСпецификации" Тогда
		...
		НачатьОбновлениеИнформационнойПанелиКлиент();
...
Процедура ОперацияНаименованиеПриИзменении(Элемент)
	...
	//++ НЕ УТКА
	ОперацияНаименованиеПриИзмененииНаКлиенте();
	//-- НЕ УТКА
...
&НаКлиенте
Процедура ОперацияНаименованиеАвтоПодбор(Элемент, Текст, ДанныеВыбора, ПараметрыПолученияДанных, Ожидание, СтандартнаяОбработка)
	
	//++ НЕ УТКА
	Если НЕ ОперацияДоступностьЭлементов Тогда
		Возврат;
	КонецЕсли;
	
	РедакторПроизводственногоПроцессаКлиент.ОперацияПоКлассификаторуАвтоПодбор(Текст, ДанныеВыбора, СтандартнаяОбработка, "Наименование");
	//-- НЕ УТКА
...
&НаКлиенте
Процедура ОперацияНаименованиеОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
...
&НаКлиенте
Процедура ОперацияНаименованиеНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	
	//++ НЕ УТКА
	ТекущиеДанные = Элементы.ПроизводственныйПроцесс.ТекущиеДанные;
	Если ТекущиеДанные = Неопределено
		ИЛИ НЕ ОперацияДоступностьЭлементов Тогда
		Возврат;
	КонецЕсли;
	
	РедакторПроизводственногоПроцессаКлиент.ОткрытьФормуВыбораОперацииПоКлассификатору(ЭтотОбъект, ПроизводственныйПроцессОперация);
	//-- НЕ УТКА
...
&НаКлиенте
Процедура ОперацияКТОПриИзменении(Элемент)
	
	//++ НЕ УТКА
	ОбработкаВыбораОперацииПоКлассификатору(ПроизводственныйПроцессОперация.КТО);
	//-- НЕ УТКА
...
#Область ОбработчикиСобытийЭлементовТаблицыФормыЭтапВидыРабочихЦентров
...
#Область ОбработчикиСобытийЭлементовТаблицыФормыЭтапУслугиПереработчика
...
#Область ОбработчикиСобытийЭлементовТаблицыФормыОперацияДопРеквизиты
...
&НаКлиенте
Процедура ПроизводственныйПроцессПодобратьПоКТО(Команда)
...
Процедура УстановитьДоступностьЭлементовПроизводственногоПроцесса()
	...
	МассивЭлементов.Добавить("ПроизводственныйПроцессПодобратьПоКТО");
	...
	ОбщегоНазначенияУТКлиентСервер.УстановитьСвойствоЭлементовФормы(
		Элементы, МассивЭлементов, "Доступность", ПроизводственныйПроцессПрочитан);
...
&НаКлиентеНаСервереБезКонтекста
Функция ПроизводственныйПроцессСледующийЭлемент(Форма, Курсор = Неопределено, ВложенныеУровни = Истина)
	
	СписокЭлементов = Форма.ПроизводственныйПроцесс.ПолучитьЭлементы(); // КоллекцияСтрокДерева
	
	Если Курсор = Неопределено Тогда
		Если СписокЭлементов.Количество() > 0 Тогда
			Курсор = СписокЭлементов[0];
		КонецЕсли;
		Возврат Курсор <> Неопределено; // Булево
	КонецЕсли;
	
	СписокЭлементовВложенный = Курсор.ПолучитьЭлементы();
	Если СписокЭлементовВложенный.Количество() > 0 И ВложенныеУровни Тогда
		Курсор = СписокЭлементовВложенный[0];
	Иначе
		Родитель = Курсор.ПолучитьРодителя();
		Если Родитель <> Неопределено Тогда
			СписокЭлементов = Родитель.ПолучитьЭлементы();
		КонецЕсли;
		ТекущийИндекс = СписокЭлементов.Индекс(Курсор);
		Если ТекущийИндекс < СписокЭлементов.Количество()-1 Тогда
			Курсор = СписокЭлементов[ТекущийИндекс+1];
		ИначеЕсли Родитель <> Неопределено Тогда
			Курсор = Родитель;
			Возврат ПроизводственныйПроцессСледующийЭлемент(Форма, Курсор, Ложь);
		Иначе
			Курсор = Неопределено;
		КонецЕсли;
	КонецЕсли;
...
&НаКлиентеНаСервереБезКонтекста
Функция ПроизводственныйПроцессСтрокаВыделена(Форма, ДанныеСтроки, ВыделенныеСтроки)
	...
	Если ДанныеСтрокиРодителя <> Неопределено Тогда
		Возврат ПроизводственныйПроцессСтрокаВыделена(Форма, ДанныеСтрокиРодителя, ВыделенныеСтроки); // Булево
	Иначе
		Возврат Ложь;
	КонецЕсли;
...
&НаСервере
Процедура ПроизводственныйПроцессДобавитьОперациюНаСервере(ИдентификаторСтроки, ИсточникЗаполнения = Неопределено, РеквизитыИсточника = Неопределено)
	...
	Если ИсточникЗаполнения <> Неопределено Тогда
		ЗаполнитьЗначенияСвойств(ОперацияОбъект, ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ИсточникЗаполнения, РеквизитыИсточника));
	КонецЕсли;
...
Процедура ВыбратьТехнологическийПроцесс(Режим)
	...
	ВидыИзделий.Добавить(ОсновноеИзделиеВидНоменклатуры);
	Если ЗначениеЗаполнено(ОсновноеИзделиеВидНоменклатуры) Тогда
		ВидыИзделий.Добавить(ПредопределенноеЗначение("Справочник.ВидыНоменклатуры.ПустаяСсылка"));
	КонецЕсли;
	
	СтруктураОтбора = Новый Структура;
	СтруктураОтбора.Вставить("Подразделение",  ДанныеСтрокиЭтапа.Подразделение);
	СтруктураОтбора.Вставить("Статус",         ПредопределенноеЗначение("Перечисление.СтатусыТехнологическихПроцессов.Действует"));
	СтруктураОтбора.Вставить("ДляВидаИзделий", ВидыИзделий);
...
&НаСервере
Процедура ВыбратьТехнологическийПроцессЗавершениеНаСервере(ТехнологическийПроцесс, ДополнительныеПараметры)
	
	КэшЭлементов      = ПроизводственныйПроцессПолучитьКэшЭлементов();
	ДанныеСтрокиЭтапа = ПроизводственныйПроцесс.НайтиПоИдентификатору(ДополнительныеПараметры.ИдентификаторСтроки);
	ПоШаблону         = (ДополнительныеПараметры.Режим = "Шаблон");
	
	ПараметрыВыборки = Справочники.ТехнологическиеПроцессы.ПараметрыВыборкиДанных(?(ПоШаблону, "", "Операции"));
	ПараметрыВыборки.ОбновитьКлючиСвязи = Истина;
	ПараметрыВыборки.СмещениеФормул     = Объект.МатериалыИУслуги.Количество();
	ДанныеТП = Справочники.ТехнологическиеПроцессы.ДанныеТехнологическогоПроцесса(ТехнологическийПроцесс, ПараметрыВыборки);
...
&НаКлиентеНаСервереБезКонтекста
Функция ПроизводственныйПроцессПолучитьТекущийЭтап(Форма, ТекущиеДанные = Неопределено)
	...
	Пока ДанныеСтроки <> Неопределено
		И ДанныеСтроки.ВидЭлемента <> "Этап" Цикл
...
&НаКлиентеНаСервереБезКонтекста
Функция ЭтоСтрокаТехнологическийПроцесс(ДанныеСтроки)
	
	Если ДанныеСтроки = Неопределено Тогда
		Возврат Ложь;
	КонецЕсли;
	
	Возврат ДанныеСтроки.ВидЭлемента = "Операция" И ЗначениеЗаполнено(ДанныеСтроки.ТехнологическийПроцесс);
	
КонецФункции

#Область ВыделитьВТехнологическийПроцесс
...
Функция ПроверитьВозможностьВыделитьВТехнологическийПроцесс(ИдентификаторыСтрок, ДополнительныеПараметры)
	...
	Если НЕ ЗначениеЗаполнено(ИдентификаторыСтрок) Тогда
		ТекстПредупреждения = НСтр("ru = 'Необходимо выделить строки.';
									|en = 'Select the lines.'");
	Иначе
		ДополнительныеПараметры.Вставить("ИдентификаторыСтрок", ИдентификаторыСтрок);
		СобратьДанныеДляПереносаВТехпроцесс(ДополнительныеПараметры);
		Если ДополнительныеПараметры.КоличествоОпераций = 0 Тогда
			ТекстПредупреждения = НСтр("ru = 'Выделенные строки не содержат операций.';
										|en = 'The selected lines do not contain operations.'");
		ИначеЕсли ДополнительныеПараметры.Подразделения.Количество() > 1 Тогда
			ТекстПредупреждения = НСтр("ru = 'Нельзя выделять операции, выполняемые в разных подразделениях.';
										|en = 'Cannot select operations performed in different business units.'");
		КонецЕсли;
	КонецЕсли;
...
// Таблица операций для переноса в техпроцесс.
// 
// Возвращаемое значение:
//  ТаблицаЗначений - Таблица операций для переноса в техпроцесс конструктор:
// * ДанныеСтроки - ДанныеФормыЭлементДерева
// * Ссылка - СправочникСсылка.ТехнологическиеОперации
// * Наименование - Строка
// * ТехнологическийПроцесс - СправочникСсылка.ТехнологическиеПроцессы
// * СвязанныеМатериалы - Булево
// * СвязанныеТрудозатраты - Булево
// * ОперацияОбъект - СправочникОбъект.ТехнологическиеОперации
&НаСервере
Функция ТаблицаОперацийДляПереносаВТехпроцессКонструктор()
	
	ТаблицаОпераций = Новый ТаблицаЗначений;
	
	ТаблицаОпераций.Колонки.Добавить("ДанныеСтроки");
	ТаблицаОпераций.Колонки.Добавить("Ссылка",                    Новый ОписаниеТипов("СправочникСсылка.ТехнологическиеОперации"));
	ТаблицаОпераций.Колонки.Добавить("Наименование",              Новый ОписаниеТипов("Строка"));
	
	ТаблицаОпераций.Колонки.Добавить("ТехнологическийПроцесс",    Новый ОписаниеТипов("СправочникСсылка.ТехнологическиеПроцессы"));
	ТаблицаОпераций.Колонки.Добавить("Коэффициент",               Новый ОписаниеТипов("Число"));
	
	ТаблицаОпераций.Колонки.Добавить("ОперацияОбъект",            Новый ОписаниеТипов("СправочникОбъект.ТехнологическиеОперации"));
	ТаблицаОпераций.Колонки.Добавить("СвязанныеРесурсы",          Новый ОписаниеТипов());
...
&НаСервере
Процедура СобратьДанныеДляПереносаВТехпроцесс(ДополнительныеПараметры)
	...
	ТехнологическиеПроцессы    = Новый Массив;
	ТаблицаОпераций            = ТаблицаОперацийДляПереносаВТехпроцессКонструктор();
	НаименованиеЭтапа          = "";
	МожноПодключитьКЭтапу      = Истина;
	ПоследовательностьПрервана = Ложь;
	
	ДанныеСтроки = Неопределено;
	Пока ПроизводственныйПроцессСледующийЭлемент(ЭтотОбъект, ДанныеСтроки) Цикл
		
		Если НЕ ДанныеСтроки.ВидЭлемента = "Операция" ИЛИ ЭтоСтрокаТехнологическийПроцесс(ДанныеСтроки) Тогда
			Продолжить;
		КонецЕсли;
		
		Если ПроизводственныйПроцессСтрокаВыделена(ЭтотОбъект, ДанныеСтроки, ДополнительныеПараметры.ИдентификаторыСтрок) Тогда
		
			ДанныеСтрокиЭтапа    = ПроизводственныйПроцессПолучитьТекущийЭтап(ЭтотОбъект, ДанныеСтроки);
			ДанныеСтрокиРодителя = ДанныеСтроки.ПолучитьРодителя();
			
			НоваяСтрока = ТаблицаОпераций.Добавить();
			ЗаполнитьЗначенияСвойств(НоваяСтрока, ДанныеСтроки);
			НоваяСтрока.ДанныеСтроки = ДанныеСтроки;
			
			Этапы.Добавить(ДанныеСтрокиЭтапа.Ссылка);
			Подразделения.Добавить(ДанныеСтрокиЭтапа.Подразделение);
			НаименованиеЭтапа = ДанныеСтрокиЭтапа.Наименование;
			
			Если ЭтоСтрокаТехнологическийПроцесс(ДанныеСтрокиРодителя) Тогда
				КэшЭлемента = ПроизводственныйПроцессПолучитьКэшЭлемента(КэшЭлементов, ДанныеСтрокиРодителя, Истина);
				НоваяСтрока.ТехнологическийПроцесс = КэшЭлемента.Объект.ТехнологическийПроцесс;
				НоваяСтрока.Коэффициент = КэшЭлемента.Объект.КоэффициентТехнологическогоПроцесса;
				ТехнологическиеПроцессы.Добавить(НоваяСтрока.ТехнологическийПроцесс);
			Иначе
				НоваяСтрока.Коэффициент = 1;
			КонецЕсли;
			
			ПрочитатьСвязанныеРесурсыОперации(НоваяСтрока, ДополнительныеПараметры);
			
			Если ПоследовательностьПрервана Тогда
				МожноПодключитьКЭтапу = Ложь;
			КонецЕсли;
			
		Иначе
			
			Если ТаблицаОпераций.Количество() > 0 Тогда
				ПоследовательностьПрервана = Истина;
			КонецЕсли;
	...
	ДополнительныеПараметры.Вставить("АдресТаблицыОпераций",    ПоместитьВоВременноеХранилище(ТаблицаОпераций, УникальныйИдентификатор));
	ДополнительныеПараметры.Вставить("Этапы",                   ОбщегоНазначенияКлиентСервер.СвернутьМассив(Этапы));
	ДополнительныеПараметры.Вставить("Подразделения",           ОбщегоНазначенияКлиентСервер.СвернутьМассив(Подразделения));
	ДополнительныеПараметры.Вставить("ТехнологическиеПроцессы", ОбщегоНазначенияКлиентСервер.СвернутьМассив(ТехнологическиеПроцессы));
	ДополнительныеПараметры.Вставить("НаименованиеЭтапа",       НаименованиеЭтапа);
	ДополнительныеПараметры.Вставить("ДоступностьРеквизитов",   ДоступностьРеквизитов);
	ДополнительныеПараметры.Вставить("ДанныеТехпроцессов",      Неопределено);
	ДополнительныеПараметры.Вставить("КоличествоОпераций",      ТаблицаОпераций.Количество());
	
	МожноПодключитьКЭтапу = МожноПодключитьКЭтапу
								И ДополнительныеПараметры.Этапы.Количество() = 1
								И ДополнительныеПараметры.ТехнологическиеПроцессы.Количество() = 0;
	ДополнительныеПараметры.Вставить("МожноПодключитьКЭтапу",   МожноПодключитьКЭтапу);
...
&НаКлиентеНаСервереБезКонтекста
Функция СвязанныеРесурсыКонструктор()
	
	Результат = Новый Структура;
	
	Результат.Вставить("ОсновныеТЧ",  "МатериалыИУслуги,Трудозатраты");
	Результат.Вставить("ЗависимыеТЧ", "СоответствиеСвойств,ОтборПоСвойствам");
...
&НаСервере
Процедура ПрочитатьСвязанныеРесурсыОперации(СтрокаТаблицыОпераций, ДополнительныеПараметры)
	
	СвязанныеРесурсы = СвязанныеРесурсыКонструктор();
	ОтборОперация    = Новый Структура("ОперацияРедактирование", СтрокаТаблицыОпераций.Ссылка);
	ОтборКлючСвязи   = Новый Структура("КлючСвязи");
	
	Если ЗначениеЗаполнено(СтрокаТаблицыОпераций.ТехнологическийПроцесс) Тогда
		ДанныеТехпроцессов = Неопределено;
		Если НЕ ДополнительныеПараметры.Свойство("ДанныеТехпроцессов", ДанныеТехпроцессов) Тогда
			ДанныеТехпроцессов = Новый Соответствие;
			ДополнительныеПараметры.Вставить("ДанныеТехпроцессов", ДанныеТехпроцессов);
		КонецЕсли;
		ИсточникДанных = ДанныеТехпроцессов.Получить(СтрокаТаблицыОпераций.ТехнологическийПроцесс);
		Если ИсточникДанных = Неопределено Тогда
			ПараметрыВыборки = Справочники.ТехнологическиеПроцессы.ПараметрыВыборкиДанных();
			ПараметрыВыборки.ОбновитьКлючиСвязи = Истина;
			ИсточникДанных = Справочники.ТехнологическиеПроцессы.ДанныеТехнологическогоПроцесса(
				СтрокаТаблицыОпераций.ТехнологическийПроцесс, ПараметрыВыборки);
			ДанныеТехпроцессов.Вставить(СтрокаТаблицыОпераций.ТехнологическийПроцесс, ИсточникДанных);
		КонецЕсли;
	Иначе
		ИсточникДанных = Объект;
	КонецЕсли;
	
	Для каждого ИмяТЧ Из СтрРазделить(СвязанныеРесурсы.ОсновныеТЧ, ",") Цикл
		НайденныеСтроки = ИсточникДанных[ИмяТЧ].НайтиСтроки(ОтборОперация);
		Если ЗначениеЗаполнено(НайденныеСтроки) Тогда
			СвязанныеРесурсы.Вставить(ИмяТЧ, НайденныеСтроки);
			Для каждого Строка Из НайденныеСтроки Цикл
				ОтборКлючСвязи.КлючСвязи = Строка.КлючСвязи;
				Для каждого ИмяЗависимойТЧ Из СтрРазделить(СвязанныеРесурсы.ЗависимыеТЧ, ",") Цикл
					НайденныеЗависимыеСтроки = ИсточникДанных[ИмяЗависимойТЧ].НайтиСтроки(ОтборКлючСвязи);
					Если ЗначениеЗаполнено(НайденныеЗависимыеСтроки) Тогда
						Если НЕ СвязанныеРесурсы.Свойство(ИмяЗависимойТЧ) Тогда
							СвязанныеРесурсы.Вставить(ИмяЗависимойТЧ, Новый Массив);
						КонецЕсли;
						ОбщегоНазначенияКлиентСервер.ДополнитьМассив(СвязанныеРесурсы[ИмяЗависимойТЧ], НайденныеЗависимыеСтроки);
					КонецЕсли;
				КонецЦикла;
			КонецЦикла;
			ДополнительныеПараметры.Вставить(ИмяТЧ, Истина);
			СтрокаТаблицыОпераций.СвязанныеРесурсы = СвязанныеРесурсы;
		КонецЕсли;
	КонецЦикла;
...
&НаСервере
Процедура ПеренестиСвязанныеРесурсыОперации(СтрокаТаблицыОпераций, ТехПроцессОбъект, РезультатНастройки)
	
	СвязанныеРесурсы = СтрокаТаблицыОпераций.СвязанныеРесурсы;
	Если НЕ ЗначениеЗаполнено(СвязанныеРесурсы) Тогда
		Возврат;
	КонецЕсли;
	
	ОперацияСсылка     = СтрокаТаблицыОпераций.ОперацияОбъект.ПолучитьСсылкуНового();
	СоответствиеКлючей = Новый Соответствие();
	
	Для каждого ИмяТЧ Из СтрРазделить(СвязанныеРесурсы.ОсновныеТЧ, ",") Цикл
		Если РезультатНастройки[ИмяТЧ] И СвязанныеРесурсы.Свойство(ИмяТЧ) Тогда
			Для каждого СтрокаТЧ Из СвязанныеРесурсы[ИмяТЧ] Цикл
				...
				СоответствиеКлючей.Вставить(СтрокаТЧ.КлючСвязи, НоваяСтрока.КлючСвязи);
				ИмяРеквизита = "КоличествоУпаковок";
				Если НЕ ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(НоваяСтрока, ИмяРеквизита) Тогда
					ИмяРеквизита = "Количество";
				КонецЕсли;
				НоваяСтрока[ИмяРеквизита] = НоваяСтрока[ИмяРеквизита] * СтрокаТаблицыОпераций.Коэффициент;
			КонецЦикла;
		КонецЕсли;
	КонецЦикла;
	
	Для каждого ИмяЗависимойТЧ Из СтрРазделить(СвязанныеРесурсы.ЗависимыеТЧ, ",") Цикл
		Если СвязанныеРесурсы.Свойство(ИмяЗависимойТЧ) Тогда
			Для каждого СтрокаТЧ Из СвязанныеРесурсы[ИмяЗависимойТЧ] Цикл
				ТабличнаяЧасть = ТехПроцессОбъект[ИмяЗависимойТЧ]; // ТабличнаяЧасть
				НовыйКлюч = СоответствиеКлючей.Получить(СтрокаТЧ.КлючСвязи);
...
&НаСервере
Процедура ПереназначитьИсключитьСвязанныеРесурсы(СтрокаТаблицыОпераций, ОперацияКонтейнерСсылка, РезультатНастройки)
	
	СвязанныеРесурсы = СтрокаТаблицыОпераций.СвязанныеРесурсы;
	Если НЕ ЗначениеЗаполнено(СвязанныеРесурсы)
			ИЛИ ЗначениеЗаполнено(СтрокаТаблицыОпераций.ТехнологическийПроцесс) Тогда
		Возврат;
	КонецЕсли;
	
	Для каждого ИмяТЧ Из СтрРазделить(СвязанныеРесурсы.ОсновныеТЧ, ",") Цикл
		Если СвязанныеРесурсы.Свойство(ИмяТЧ) Тогда
			Для каждого СтрокаТЧ Из СвязанныеРесурсы[ИмяТЧ] Цикл
				Если РезультатНастройки[ИмяТЧ] Тогда
					Объект[ИмяТЧ].Удалить(СтрокаТЧ);
				Иначе
					СтрокаТЧ.ОперацияРедактирование = ОперацияКонтейнерСсылка;
				КонецЕсли;
			КонецЦикла;
		КонецЕсли;
	КонецЦикла;
...
&НаКлиенте
Процедура ВыделитьВТехнологическийПроцессЗавершение(РезультатНастройки, ДополнительныеПараметры) Экспорт
	
	Если ЗначениеЗаполнено(РезультатНастройки) Тогда
		
		Результат = ВыделитьВТехнологическийПроцесс(РезультатНастройки, ДополнительныеПараметры);
		
		Если ЗначениеЗаполнено(Результат) Тогда
			Если НЕ РезультатНастройки.ПодключитьКЭтапу
...
&НаСервере
Функция ВыделитьВТехнологическийПроцесс(РезультатНастройки, ДополнительныеПараметры)
	...
	ТехПроцессОбъект.Заполнить(Неопределено);
	
	ТехПроцессОбъект.Наименование   = РезультатНастройки.НаименованиеТехпроцесса;
	ТехПроцессОбъект.Подразделение  = ДополнительныеПараметры.Подразделения[0];
	ТехПроцессОбъект.ДляВидаИзделий = ОсновноеИзделиеВидНоменклатуры;
	
	ТаблицаОпераций = ПолучитьИзВременногоХранилища(ДополнительныеПараметры.АдресТаблицыОпераций); // ТаблицаОперацийДляПереносаВТехпроцессКонструктор
	
	#Область ПодготовкаДанных
	
	Для Индекс = 0 По ТаблицаОпераций.Количество() - 1 Цикл
		
		СтрокаТаблицы = ТаблицаОпераций[Индекс];
		
		КэшЭлемента = ПроизводственныйПроцессПолучитьКэшЭлемента(КэшЭлементов, СтрокаТаблицы.ДанныеСтроки, Истина);
		
		ОперацияОбъект = КэшЭлемента.Объект.Скопировать(); // СправочникОбъект.ТехнологическиеОперации
		СтрокаТаблицы.ОперацияОбъект = ОперацияОбъект;
		
		ОперацияОбъект.УстановитьСсылкуНового(Справочники.ТехнологическиеОперации.ПолучитьСсылку());
		ОперацияОбъект.Владелец   = ТехПроцессСсылка;
		ОперацияОбъект.Количество = ОперацияОбъект.Количество * СтрокаТаблицы.Коэффициент;
		
		Если РезультатНастройки.Перенумеровать Тогда
			ОперацияОбъект.НомерОперации          = Индекс + 1;
			ОперацияОбъект.НомерСледующейОперации = 0;
			Если Индекс > 0 Тогда
				ТаблицаОпераций[Индекс-1].ОперацияОбъект.НомерСледующейОперации = Индекс + 1;
			КонецЕсли;
		КонецЕсли;
		
		ПеренестиСвязанныеРесурсыОперации(СтрокаТаблицы, ТехПроцессОбъект, РезультатНастройки);
	...
	Попытка
		...
		Для Индекс = 0 По ТаблицаОпераций.Количество() - 1 Цикл
			...
			ОперацияОбъект = ТаблицаОпераций[Индекс].ОперацияОбъект; // СправочникОбъект
	...
	ПодключитьТехнологическийПроцессКЭтапу(КэшЭлементов, ТехПроцессСсылка, ТаблицаОпераций, РезультатНастройки);
...
&НаСервере
Процедура ПодключитьТехнологическийПроцессКЭтапу(КэшЭлементов, ТехПроцессСсылка, ТаблицаОпераций, РезультатНастройки)
	
	Если НЕ РезультатНастройки.ПодключитьКЭтапу Тогда
		Возврат;
	КонецЕсли;
	
	ДанныеСтрокиЭтапа     = ТаблицаОпераций[0].ДанныеСтроки.ПолучитьРодителя(); // ДанныеФормыЭлементДерева
	КоллекцияЭлементов    = ДанныеСтрокиЭтапа.ПолучитьЭлементы(); // ДанныеФормыКоллекцияЭлементовДерева
	ДанныеСтрокиПервой    = ТаблицаОпераций[0].ДанныеСтроки;
	ДанныеСтрокиПоследней = ТаблицаОпераций[ТаблицаОпераций.Количество()-1].ДанныеСтроки;
	СмещениеНумерации     = ТаблицаОпераций.Количество() - 1;
	
	ПараметрыДобавления = Новый Структура("НомерОперации,НомерСледующейОперации", ДанныеСтрокиПервой.Номер, ДанныеСтрокиПервой.НомерСледующего);
	Если КоллекцияЭлементов.Индекс(ДанныеСтрокиПоследней) = КоллекцияЭлементов.Количество()-1 Тогда
		ПараметрыДобавления.НомерСледующейОперации = 0;
	КонецЕсли;
	
	ДанныеТП = Справочники.ТехнологическиеПроцессы.ДанныеТехнологическогоПроцесса(
		ТехПроцессСсылка, Справочники.ТехнологическиеПроцессы.ПараметрыВыборкиДанных("Операции"));
	ИдентификаторСтроки = ДобавитьТиповойТехнологическийПроцесс(КэшЭлементов, ДанныеСтрокиЭтапа, ДанныеТП, ПараметрыДобавления);
	ДанныеСтрокиТП = ПроизводственныйПроцесс.НайтиПоИдентификатору(ИдентификаторСтроки);
	
	Для Индекс = 0 По ТаблицаОпераций.Количество() - 1 Цикл
		
		СтрокаТаблицыОпераций = ТаблицаОпераций[Индекс];
		
		КэшЭлемента  = ПроизводственныйПроцессПолучитьКэшЭлемента(КэшЭлементов, СтрокаТаблицыОпераций.ДанныеСтроки, Истина);
		КэшЭлемента.Удален = Истина;
		КоллекцияЭлементов.Удалить(СтрокаТаблицыОпераций.ДанныеСтроки);
		
		ПереназначитьИсключитьСвязанныеРесурсы(СтрокаТаблицыОпераций, ДанныеСтрокиТП.Ссылка, РезультатНастройки);
		
	КонецЦикла;
	
	ПроизводственныйПроцессСкорректироватьПозициюСтроки(ЭтотОбъект, ДанныеСтрокиТП);
	
	Если СмещениеНумерации > 0 Тогда
		Для ИндексСтроки = КоллекцияЭлементов.Индекс(ДанныеСтрокиТП) + 1 По КоллекцияЭлементов.Количество() - 1 Цикл
			
			ДанныеСтроки = КоллекцияЭлементов.Получить(ИндексСтроки);
			
			ДанныеСтроки.Номер                        = ДанныеСтроки.Номер - СмещениеНумерации;
			ДанныеСтроки.НомерСледующего              = ДанныеСтроки.НомерСледующего - СмещениеНумерации;
			КэшЭлемента = ПроизводственныйПроцессПолучитьКэшЭлемента(КэшЭлементов, ДанныеСтроки, Истина);
			КэшЭлемента.Объект.НомерОперации          = ДанныеСтроки.Номер;
			КэшЭлемента.Объект.НомерСледующейОперации = ДанныеСтроки.НомерСледующего;
			КэшЭлемента.Записывать = Истина;
			
		КонецЦикла;
	КонецЕсли;
	
	ПроизводственныйПроцессОбновитьПредставления(ДанныеСтрокиТП.ВидЭлемента, ДанныеСтрокиТП.Ссылка, ДанныеСтрокиТП.Наименование, ЭтотОбъект);
	ПроизводственныйПроцессОбновитьИнформационныеПанели();
	ПроизводственныйПроцессСкорректироватьСчетчикЭлементов(ДанныеСтрокиТП.ВидЭлемента, -ТаблицаОпераций.Количество());
...
&НаСервере
Процедура ОперацияКТОПриИзмененииНаСервере()
	
	РедакторПроизводственногоПроцесса.КТОПриИзменении(ПроизводственныйПроцессОперация, ЭтотОбъект, "Операция");
...
&НаКлиенте
Процедура ОперацияНаименованиеПриИзмененииНаКлиенте()
...
Процедура ОбработкаВыбораОперацииПоКлассификатору(ВыбранноеЗначение, ДобавитьНовую = Ложь)
	...
	Если ДобавитьНовую Тогда
		...
		Если ДанныеСтрокиЭтапа <> Неопределено Тогда
			...
			ЗаполняемыеРеквизиты = Новый Структура("КТО,Наименование,ВидОперации", "Ссылка");
			
			ПроизводственныйПроцессДобавитьОперациюНаСервере(ДанныеСтрокиЭтапа.ПолучитьИдентификатор(), ВыбранноеЗначение, ЗаполняемыеРеквизиты);
			
		КонецЕсли;
		
	Иначе
		
		ПроизводственныйПроцессОперация.КТО = ВыбранноеЗначение;
		
		ОперацияКТОПриИзмененииНаСервере();
...
&НаКлиентеНаСервереБезКонтекста
Процедура СброситьИнформационнуюПанель(Форма)
	
	Форма.ИнформационнаяСтрока = Неопределено;
	Форма.Элементы.ИнформационнаяПанельСтраницы.ТекущаяСтраница = Форма.Элементы.ИнформационнаяПанельСтраницаСкрыть;
	
	Если Форма.Объект.Статус <> ПредопределенноеЗначение("Перечисление.СтатусыСпецификаций.Действует") Тогда
		Возврат;
	КонецЕсли;
	
	Если Форма.Объект.ВариантПодбораВДокументы <> ПредопределенноеЗначение("Перечисление.ВариантыПодбораСпецификацииВДокументы.Автоматически") Тогда
		Форма.ИнформационнаяСтрока = Новый ФорматированнаяСтрока(
			НСтр("ru = 'Спецификация исключена из автоматического выбора в документы.';
				|en = 'Bill of materials is excluded from the automatic selection to documents.'"),,
			WebЦвета.Кирпичный);
		Форма.Элементы.ИнформационнаяПанельСтраницы.ТекущаяСтраница = Форма.Элементы.ИнформационнаяПанельСтраницаОтобразить;
	КонецЕсли;
	
КонецПроцедуры

&НаКлиенте
Процедура НачатьОбновлениеИнформационнойПанелиКлиент()
	
	//++ НЕ УТКА
	Если ТребуетсяОбновлениеИнформационнойПанелиВФоне(ЭтотОбъект) Тогда
	...
	СброситьИнформационнуюПанель(ЭтотОбъект);
...
&НаКлиентеНаСервереБезКонтекста
Функция ТребуетсяОбновлениеИнформационнойПанелиВФоне(Форма)
	
	Возврат Форма.Объект.Статус = ПредопределенноеЗначение("Перечисление.СтатусыСпецификаций.Действует")
		И Форма.Объект.ВариантПодбораВДокументы = ПредопределенноеЗначение("Перечисление.ВариантыПодбораСпецификацииВДокументы.Автоматически");
	
КонецФункции

&НаКлиенте
Процедура НачатьОжиданиеОбновленияИнформационнойПанели()
...
Процедура ОбработатьРезультатОбновленияИнформационнойПанели(Форма, Результат)
	...
	Если Результат.Статус = "Выполнено"
		И ОбработатьРезультатОбновленияИнформационнойПанелиНаСервере(Результат) Тогда
			
		ТекстСообщения = НСтр("ru = 'Спецификация может быть вытеснена более приоритетными';
								|en = 'Bill of materials may be pushed out by more prioritized BOMs'");
...
&НаСервереБезКонтекста
Функция ОбработатьРезультатОбновленияИнформационнойПанелиНаСервере(Результат)
	
	Спецификации = ПолучитьИзВременногоХранилища(Результат.АдресРезультата);
	
	Возврат ЗначениеЗаполнено(Спецификации);
