...
Функция ДанныеСпецификацииПоНоменклатуре(ДанныеПоНоменклатуре, ПараметрыВыборки, Отбор = Неопределено) Экспорт
	...
	ПрименитьОтборПоДаннымСпецификации(ДанныеСпецификации, Отбор, ПараметрыВыборки);
...
Функция ДанныеСпецификации(СписокСтруктураДанных, ПараметрыВыборки) Экспорт
	...
	|
	|	ЕСТЬNULL(ВозвратныеОтходы.Номенклатура.ВидНоменклатуры, ЗНАЧЕНИЕ(Справочник.ВидыНоменклатуры.ПустаяСсылка)) КАК ВидНоменклатуры,
...
//++ НЕ УТКА
	...
	ТекстЗапроса = УправлениеДаннымиОбИзделиях.ТекстЗапросаВТОтборПоСвойствам("ВТОтборПоСвойствамНастройки");
...
//-- НЕ УТКА
	...
	|
	|	ЕСТЬNULL(ВтИзделия.Номенклатура.ЕдиницаИзмерения, ЗНАЧЕНИЕ(Справочник.УпаковкиЕдиницыИзмерения.ПустаяСсылка))  КАК ЕдиницаИзмерения,
	|	ЕСТЬNULL(ВтИзделия.Номенклатура.ТипНоменклатуры, ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.ПустаяСсылка)) КАК ТипНоменклатуры,
	|
	|	ВЫБОР
...
//-- НЕ УТКА
	...
	Если ПереченьДанных.Найти("МатериалыИУслуги") <> Неопределено Тогда
		...
		|	ЕСТЬNULL(МатериалыИУслуги.Номенклатура.ТипНоменклатуры, ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.ПустаяСсылка)) КАК ТипНоменклатуры,
		|
		|	ВЫБОР 
		...
		|	ЕСТЬNULL(МатериалыИУслуги.Номенклатура.ЕдиницаИзмерения, ЗНАЧЕНИЕ(Справочник.УпаковкиЕдиницыИзмерения.ПустаяСсылка)) КАК ЕдиницаИзмерения,
		|	МатериалыИУслуги.Упаковка                         КАК Упаковка,
		|	НЕОПРЕДЕЛЕНО                                      КАК ДанныеУпаковки,
		|
		|	ЕСТЬNULL(МатериалыИУслуги.Номенклатура.ЕдиницаИзмерения.ТипИзмеряемойВеличины = ЗНАЧЕНИЕ(Перечисление.ТипыИзмеряемыхВеличин.КоличествоШтук), ЛОЖЬ) КАК ЕдиницаИзмеренияШтучная,
		|	ЕСТЬNULL(&ТекстЗапросаТипИзмеряемойВеличины = ЗНАЧЕНИЕ(Перечисление.ТипыИзмеряемыхВеличин.КоличествоШтук), ЛОЖЬ)                   КАК УпаковкаШтучная,
		|
		|	ОсновноеВыходноеИзделие.Количество                КАК КоличествоИзделие,
		|
		...
		|
		//++ Локализация
		|	ЕСТЬNULL(МатериалыИУслуги.Номенклатура.ПрослеживаемыйТовар, ЛОЖЬ) КАК ПрослеживаемыйТовар,
		//-- Локализация
		|
...
//++ НЕ УТКА
	...
	Если ПереченьДанных.Найти("Операции") <> Неопределено Тогда
		...
		|	Операции.ПередаточнаяПартия     КАК ПередаточнаяПартия,
		|
		...
		|	ОперацииТП.ПередаточнаяПартия     КАК ПередаточнаяПартия,
		|
		...
		|	ПРЕДСТАВЛЕНИЕ(Операции.Операция) КАК ОперацияПредставление,
		...
		|	Операции.ТехнологическийПроцесс              КАК ТехнологическийПроцесс,
		|	Операции.КоэффициентТехнологическогоПроцесса КАК КоэффициентТехнологическогоПроцесса,
		|	Операции.КоэффициентТехнологическогоПроцесса КАК КоэффициентТехнологическогоПроцессаНаЕдиницуПартииВыпуска,
		|	Операции.СодержитТехнологическийПроцесс      КАК СодержитТехнологическийПроцесс,
...
//++ НЕ УТКА
	...
	Если ДанныеСпецификации.Свойство("Операции") Тогда
		...
		РассчитатьКоэффициентТехнологическогоПроцесса(ПартияВыпуска, ДанныеСпецификации.Операции);
	КонецЕсли;
//-- НЕ УТКА
...
Процедура РассчитатьКоэффициентТехнологическогоПроцесса(ПартияВыпуска, Операции)
	
	Для каждого СтрокаОперация Из Операции Цикл
		СтрокаОперация.КоэффициентТехнологическогоПроцесса = СтрокаОперация.КоэффициентТехнологическогоПроцессаНаЕдиницуПартииВыпуска
			* ПартияВыпуска.Числитель / ПартияВыпуска.Знаменатель;
	КонецЦикла;
...
Функция ПолучитьПараметрыАвтовыбора(Таблицы, МенеджерВременныхТаблиц, ПараметрыВыборки)
	...
	ПараметрыАвтовыбора = УправлениеДаннымиОбИзделиях.ПараметрыАвтовыбораКонструктор();
...
// Определяет, принадлежит ли строка заданной операции
// 
// Параметры:
//  ДанныеОперации - СтрокаТаблицыЗначений
//  СтрокаНорматива - СтрокаТаблицыЗначений
//  ЭтоИзделие - Булево - это изделие
// 
// Возвращаемое значение:
//  Булево - Строка данных спецификации принадлежит операции
Функция СтрокаДанныхСпецификацииПринадлежитОперации(ДанныеОперации, СтрокаНорматива, ЭтоИзделие = Ложь) Экспорт
	
	Возврат (ДанныеОперации.Операция = СтрокаНорматива.Операция
				И (ЭтоИзделие
					ИЛИ ДанныеОперации.ОперацияРодитель = СтрокаНорматива.ОперацияРодитель))
			ИЛИ (ЭтоИзделие
				И ДанныеОперации.ИсходныйНомерСледующейОперации = 0
				И ДанныеОперации.ОперацияРодитель = СтрокаНорматива.Операция)
			ИЛИ (НЕ ЭтоИзделие
				И ДанныеОперации.ИсходныйНомерОперации = 1
				И ДанныеОперации.ОперацияРодитель = СтрокаНорматива.Операция);
...
Процедура ПрименитьОтборПоДаннымСпецификации(ДанныеСпецификации, Отбор, ПараметрыВыборки = Неопределено) Экспорт
	...
	Если ПараметрыВыборки = Неопределено Тогда
		ИменаТаблиц = ПереченьДанныхСпецификации();
	Иначе
	...
	Для каждого ИмяТаблицы Из ИменаТаблиц Цикл
		Таблица = Неопределено; // ТаблицаЗначений
		Если ДанныеСпецификации.Свойство(ИмяТаблицы, Таблица) Тогда
			ДанныеСпецификации.Вставить(ИмяТаблицы, Таблица.Скопировать(Отбор));
		КонецЕсли;
	КонецЦикла;
