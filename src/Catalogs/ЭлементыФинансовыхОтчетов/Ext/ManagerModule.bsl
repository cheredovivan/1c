...
Функция ФормаПриСозданииНаСервере(Форма) Экспорт
	...
	Если Не Записывать Тогда
		...
		ТаблицыЭлемента = "ОперандыФормулы, РеквизитыВидаЭлемента, ЭлементыТаблицы, ДополнительныеПоля, ПравилаРасчета,
...
Процедура УстановитьНастройкиОтбора(Реквизиты, КомпоновщикПриемник = Неопределено, ВидЭлемента, ДополнительныйОтбор) Экспорт
	...
	Если ВидЭлемента = Перечисления.ВидыЭлементовФинансовогоОтчета.СтатьяБюджетов
		...
		ИЛИ ВидЭлемента = Перечисления.ВидыЭлементовФинансовогоОтчета.НефинансовыйПоказатель
		ИЛИ ВидЭлемента = Перечисления.ВидыЭлементовФинансовогоОтчета.НастройкаПравилРасчета Тогда
	...
	СоответствиеЭлементовОтбора = Новый Соответствие;
	СоответствиеЭлементовОтбора.Вставить("ИспользоватьФильтрПоОрганизации", "Организация");
	СоответствиеЭлементовОтбора.Вставить("ИспользоватьФильтрПоПодразделению", "Подразделение");
	СоответствиеЭлементовОтбора.Вставить("ИспользоватьФильтрПоСценарию", "Сценарий");
	Если ВидЭлемента <> Перечисления.ВидыЭлементовФинансовогоОтчета.НефинансовыйПоказатель Тогда
		СоответствиеЭлементовОтбора.Вставить("ИспользоватьФильтрПоСтатусу", "Статус");
	КонецЕсли;
	...
	Если ВидЭлемента = Перечисления.ВидыЭлементовФинансовогоОтчета.ВсеПоказателиБюджетов
		...
		Для Каждого КлючИЗначение Из СоответствиеЭлементовОтбора Цикл
			Если Реквизиты[КлючИЗначение.Ключ] Тогда
				ПоляОтбора.Вставить(КлючИЗначение.Значение);
			КонецЕсли;
		КонецЦикла;
	...
	Если ВидЭлемента = Перечисления.ВидыЭлементовФинансовогоОтчета.ВсеПоказателиБюджетов
		...
		Для Каждого КлючИЗначение Из СоответствиеЭлементовОтбора Цикл
			Если НЕ Реквизиты[КлючИЗначение.Ключ] Тогда
				Отбор = КомпоновщикПриемник.Настройки.Отбор;
				Элемент = ФинансоваяОтчетностьСервер.НайтиЭлементОтбора(Отбор, КлючИЗначение.Значение);
...
Процедура ЗаполнитьОбъектПоДаннымВременногоХранилища(Объект, ДанныеОбъекта, Код, Владелец, Родитель)
	...
	ИменаТабличныхЧастей = "РеквизитыВидаЭлемента, ОперандыФормулы, ЭлементыТаблицы, ДополнительныеПоля, ЭлементыОформления, ОформляемыеСтроки, ОформляемыеКолонки, РасшифровкаПолейОтбораЭО, ИсточникиЗначений, ПравилаРасчета";
	
	ЗаполнитьЗначенияСвойств(Объект, ДанныеОбъекта, , "Код, Владелец" + Суффикс + ", " + ИменаТабличныхЧастей);
	
	МассивИменТабличныхЧастей = СтроковыеФункцииКлиентСервер.РазложитьСтрокуВМассивПодстрок(ИменаТабличныхЧастей, ",", , Истина);
	Для Каждого ИмяТабличнойЧасти Из МассивИменТабличныхЧастей Цикл
		Объект[ИмяТабличнойЧасти].Загрузить(ДанныеОбъекта[ИмяТабличнойЧасти]);
	КонецЦикла;
