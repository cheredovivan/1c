...
Процедура УстановитьНастройкиВидаБюджета()
	...
	Если ВидЭлемента = Перечисления.ВидыЭлементовФинансовогоОтчета.ВсеПоказателиБюджетов
		...
		// это операнд формулы.
		// Также фильтры могут быть по статьям и показателям бюджетов с настроенными правилами расчета.
		...
		Если ВидЭлемента = Перечисления.ВидыЭлементовФинансовогоОтчета.СтатьяБюджетов Тогда
			Если НЕ ВидРодителя = Перечисления.ВидыЭлементовФинансовогоОтчета.ПроизводныйПоказатель
				И НЕ ВидРодителя = Перечисления.ВидыЭлементовФинансовогоОтчета.СтатьяБюджетов
				И ПравилаРасчета.Количество() = 0 Тогда
		...
		Если СброситьИспользованиеФильтров И ПравилаРасчета.Количество() = 0 Тогда
