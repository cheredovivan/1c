...
Функция ОпределитьЗаменыТиповДанныхУчетаИсточниковДанных(ПоКорреспондирующимРегистрам, ПоРегистрамЛимитов = Неопределено) Экспорт
	...
	ИсточникиДанных.Добавить(Справочники.НастройкиХозяйственныхОпераций.ВозвратНеперечисленныхДС);
	...
	// Приход - Контрагенты на ПрочиеАктивыПассивы
	ИсточникиДанных = Новый Массив;
	ИсточникиДанных.Добавить(Справочники.НастройкиХозяйственныхОпераций.ВнутренняяПередачаДенежныхСредств);
	Если ПоКорреспондирующимРегистрам Тогда
		ЗаменаТипаДанныхУчета = Новый Структура("Исходный, Новый", Перечисления.ТипыДанныхУчета.Контрагенты,
			Перечисления.ТипыДанныхУчета.ПрочиеАктивыПассивы);
	Иначе
...
Процедура ВыполнитьЗаменуИсточникаДанныхПравила(Правило, ПоКорреспондирующимРегистрам)
	...
	Иначе
		...
		ИначеЕсли ИсточникДанных = Справочники.НастройкиХозяйственныхОпераций.НачисленияПоДепозитам
			...
			Настройки = Правило.КомпоновщикНастроек.Получить();
			Если ТипЗнч(Настройки) = Тип("НастройкиКомпоновкиДанных") Тогда
				КомпоновкаДанныхКлиентСервер.ДобавитьОтбор(Настройки, "КорТипДенежныхСредств", Перечисления.ТипыДенежныхСредств.Депозиты, ВидСравненияКомпоновкиДанных.Равно, Истина);
				Правило.КомпоновщикНастроек = Новый ХранилищеЗначения(Настройки);
				Правило.ПредставлениеОтбора = Строка(Настройки.Отбор);
			КонецЕсли;
		КонецЕсли;
