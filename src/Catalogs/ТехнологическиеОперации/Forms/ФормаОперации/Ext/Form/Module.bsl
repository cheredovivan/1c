...
Процедура ОбработкаПроверкиЗаполненияНаСервере(Отказ, ПроверяемыеРеквизиты)
	...
	Для каждого Строка Из ДопРеквизиты Цикл
		...
		Если Строка.ЗначениеМин > Строка.ЗначениеМакс И Строка.ЗначениеМакс <> 0 Тогда
			...
			ОбщегоНазначения.СообщитьПользователю(ТекстСообщения,, Путь,, Отказ);
...
Процедура КТОПриИзменении(Элемент)
	...
	ОбработкаВыбораОперацииПоКлассификатору(Объект.КТО);
...
Процедура НаименованиеАвтоПодбор(Элемент, Текст, ДанныеВыбора, ПараметрыПолученияДанных, Ожидание, СтандартнаяОбработка)
	...
	Если НЕ ДоступностьЭлементов
		ИЛИ НЕ ХранитьОперацииВРесурсныхСпецификациях Тогда
		Возврат;
	КонецЕсли;
...
Процедура НаименованиеНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	РедакторПроизводственногоПроцессаКлиент.ОткрытьФормуВыбораОперацииПоКлассификатору(ЭтотОбъект, Объект);
...
Процедура ПриЧтенииСозданииНаСервере()
	...
	Элементы.Наименование.КнопкаВыбора = ХранитьОперацииВРесурсныхСпецификациях;
...
&НаСервере
Процедура КТОПриИзмененииНаСервере()
	
	РедакторПроизводственногоПроцесса.КТОПриИзменении(Объект, ЭтотОбъект);
...
&НаКлиенте
Процедура ОбработкаВыбораОперацииПоКлассификатору(ВыбранноеЗначение)
	
	Объект.КТО = ВыбранноеЗначение;
	
	КТОПриИзмененииНаСервере();
