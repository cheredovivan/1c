...
Функция ТекстЗапросаРеквизитыРасчетаВремениВыполненияОперации(ИмяТаблицыОперации) Экспорт
	...
	ТекстЗапроса = 
		...
		|	ТехнологическиеОперации.ВремяПЗЕдИзм КАК ВремяПЗЕдИзм,
		|	ВЫБОР
...
// Текст запроса для получения Вида рабочего центра и Рабочего центра.
// 
// Параметры:
//  ИмяТаблицыОперации - Строка
//  Поля - Строка - Поля
//  ВыводитьПсевдонимы - Булево
// 
// Возвращаемое значение:
//  Строка - Текст запроса РЦВРЦ
Функция ТекстЗапросаРЦВРЦ(ИмяТаблицыОперации, Поля = "", ВыводитьПсевдонимы = Истина) Экспорт
	
	ТекстЗапроса = "";
	
	ИмяПоля = "ВидРабочегоЦентра";
	Если ПустаяСтрока(Поля) ИЛИ СтрНайти(Поля, ИмяПоля) > 0 Тогда
		ТекстЗапроса = ТекстЗапроса + СтрШаблон("ВЫБОР
		|		КОГДА ТехнологическиеОперации.РабочийЦентр ССЫЛКА Справочник.ВидыРабочихЦентров
		|			ТОГДА ТехнологическиеОперации.РабочийЦентр
		|		ИНАЧЕ
		|			ТехнологическиеОперации.РабочийЦентр.ВидРабочегоЦентра
		|	КОНЕЦ %1,
		|", ?(ВыводитьПсевдонимы, "КАК "+ИмяПоля, ""));
	КонецЕсли;
	
	ИмяПоля = "РабочийЦентр";
	Если ПустаяСтрока(Поля) ИЛИ СтрНайти(Поля, ИмяПоля) > 0 Тогда
		ТекстЗапроса = ТекстЗапроса + СтрШаблон("ВЫБОР
		|		КОГДА ТехнологическиеОперации.РабочийЦентр ССЫЛКА Справочник.РабочиеЦентры
		|			ТОГДА ТехнологическиеОперации.РабочийЦентр
	...
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "ТехнологическиеОперации", ИмяТаблицыОперации);
...
Функция ТаблицаОперацииКонструктор() Экспорт
	...
	ТаблицаОперации.Колонки.Добавить("ЕдиницаИзмерения",      Новый ОписаниеТипов("СправочникСсылка.УпаковкиЕдиницыИзмерения"));
	
	ТаблицаОперации.Колонки.Добавить("ИдентификаторОперации", Новый ОписаниеТипов("Число", Новый КвалификаторыЧисла(25,0)));
	ТаблицаОперации.Колонки.Добавить("ПроизводственнаяОперация", Новый ОписаниеТипов("ДокументСсылка.ПроизводственнаяОперация2_2"));
	...
	ТаблицаОперации.Колонки.Добавить("ПередаточнаяПартия", Новый ОписаниеТипов("Число", Новый КвалификаторыЧисла(15,3)));
	...
	//
	ТипыТехпроцесса = Новый Массив;
	ТипыТехпроцесса.Добавить(Тип("СправочникСсылка.МаршрутныеКарты"));
	ТипыТехпроцесса.Добавить(Тип("СправочникСсылка.ТехнологическиеПроцессы"));
	
	ТаблицаОперации.Колонки.Добавить("СодержитТехнологическийПроцесс", Новый ОписаниеТипов("Булево"));
	ТаблицаОперации.Колонки.Добавить("ТехнологическийПроцесс", Новый ОписаниеТипов(ТипыТехпроцесса));
	...
	ТаблицаОперации.Колонки.Добавить("КоэффициентТехнологическогоПроцессаНаЕдиницуПартииВыпуска", Новый ОписаниеТипов("Число", Новый КвалификаторыЧисла(15,3)));
	...
	//++ Локализация
	ТаблицаОперации.Колонки.Добавить("Должность", Новый ОписаниеТипов("СправочникСсылка.Должности"));
	ТаблицаОперации.Колонки.Добавить("Разряд",    Новый ОписаниеТипов("СправочникСсылка.РазрядыКатегорииДолжностей"));
	//-- Локализация
...
Функция РеквизитыМаршрутаОперации(ВладелецОперации) Экспорт
	...
	Если НЕ ЗначениеЗаполнено(ВладелецОперации) Тогда
		Результат.МаршрутПредставление = ИндивидуальныйТехпроцессПредставление();
	ИначеЕсли ТипЗнч(ВладелецОперации) = Тип("СправочникСсылка.ЭтапыПроизводства") Тогда
	...
	Результат.МаршрутДоступен = ЗначениеЗаполнено(Результат.Маршрут)
		И ?(ТипЗнч(Результат.Маршрут) = Тип("СправочникСсылка.МаршрутныеКарты"),
				ПолучитьФункциональнуюОпцию("ИспользоватьМаршрутныеКарты"),
...
Функция ИндивидуальныйТехпроцессПредставление()
	Возврат НСтр("ru = 'Индивидуальный';
				|en = 'Individual'");
КонецФункции

// Доступные единицы измерения времени технологической операции.
// 
// Возвращаемое значение:
//  Массив из ПеречислениеСсылка.ЕдиницыИзмеренияВремени
Функция ДоступныеЕдиницыИзмеренияВремени() Экспорт
	
	Результат = Новый Массив;
	
	Результат.Добавить(Перечисления.ЕдиницыИзмеренияВремени.Секунда);
	Результат.Добавить(Перечисления.ЕдиницыИзмеренияВремени.Минута);
	Результат.Добавить(Перечисления.ЕдиницыИзмеренияВремени.Час);
	Результат.Добавить(Перечисления.ЕдиницыИзмеренияВремени.Сутки);
	
	Возврат Результат;
	
КонецФункции

Функция ПроверитьИсправитьЕдиницыИзмеренияВремени(Объект)
	
	ОбъектИзменен = Ложь;
	
	СтруктураРеквизитов = Новый Структура;
	СтруктураРеквизитов.Вставить("ВремяШтучное", "ВремяШтучноеЕдИзм");
	СтруктураРеквизитов.Вставить("ВремяПЗ",      "ВремяПЗЕдИзм");
	
	Для каждого КлючИЗначение Из СтруктураРеквизитов Цикл
		
		ЕдиницаИзмерения = Объект[КлючИЗначение.Значение];
		
		Если ЕдиницаИзмерения = Перечисления.ЕдиницыИзмеренияВремени.День Тогда
			Объект[КлючИЗначение.Значение] = Перечисления.ЕдиницыИзмеренияВремени.Сутки;
			ОбъектИзменен = Истина;
		ИначеЕсли ЕдиницаИзмерения = Перечисления.ЕдиницыИзмеренияВремени.Месяц Тогда
			Объект[КлючИЗначение.Ключ]     = Объект[КлючИЗначение.Ключ] * 30;
			Объект[КлючИЗначение.Значение] = Перечисления.ЕдиницыИзмеренияВремени.Сутки;
			ОбъектИзменен = Истина;
		ИначеЕсли ЕдиницаИзмерения = Перечисления.ЕдиницыИзмеренияВремени.Год Тогда
			Объект[КлючИЗначение.Ключ]     = Объект[КлючИЗначение.Ключ] * 365;
			Объект[КлючИЗначение.Значение] = Перечисления.ЕдиницыИзмеренияВремени.Сутки;
			ОбъектИзменен = Истина;
		КонецЕсли;
...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик.Версия = "2.5.23.25";
	Обработчик.РежимВыполнения = "Отложенно";
	Обработчик.Идентификатор = Новый УникальныйИдентификатор("83ad3376-d006-69fc-1122-d6c674aa79b3");
	...
	Обработчик.Комментарий = НСтр("ru = 'Исправляет некорректные единицы измерения.';
									|en = 'Corrects incorrect units of measure.'");
	...
	Изменяемые.Добавить(Метаданные.Справочники.ТехнологическиеОперации.ПолноеИмя());
	Обработчик.ИзменяемыеОбъекты = СтрСоединить(Изменяемые, ",");
	
	Блокируемые = Новый Массив;
	Блокируемые.Добавить(Метаданные.Справочники.ТехнологическиеОперации.ПолноеИмя());
...
Процедура ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	|ИЗ
	|	Справочник.ТехнологическиеОперации КАК Таблица
	|ГДЕ
	|	НЕ (Таблица.ВремяШтучноеЕдИзм = ЗНАЧЕНИЕ(Перечисление.ЕдиницыИзмеренияВремени.ПустаяСсылка)
	|			ИЛИ Таблица.ВремяШтучноеЕдИзм В (&ДоступныеЕдиницыИзмеренияВремени))
	|	ИЛИ НЕ (Таблица.ВремяПЗЕдИзм = ЗНАЧЕНИЕ(Перечисление.ЕдиницыИзмеренияВремени.ПустаяСсылка)
	|			ИЛИ Таблица.ВремяПЗЕдИзм В (&ДоступныеЕдиницыИзмеренияВремени))
	|");
	
	Запрос.УстановитьПараметр("ДоступныеЕдиницыИзмеренияВремени", ДоступныеЕдиницыИзмеренияВремени());
...
Процедура ОбработатьДанныеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		Попытка
			...
			Если ПроверитьИсправитьЕдиницыИзмеренияВремени(Объект) Тогда
				ОбъектИзменен = Истина;
			КонецЕсли;
