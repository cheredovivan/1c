...
Процедура ПроверитьЗаполнениеРеквизитов(Отказ, НазваниеЭтапа = "") Экспорт
	...
	ДоступныеЕдиницыИзмеренияВремени = Справочники.ТехнологическиеОперации.ДоступныеЕдиницыИзмеренияВремени();
	ШаблонСообщения = НСтр("ru = 'Для реквизита ""%1"" введено некорректное значение единицы измерения. Доступные варианты: ""секунда"", ""минута"", ""час"", ""сутки"".';
							|en = 'An incorrect unit of measure is specified for the ""%1"" item. Available options: ""second"", ""minute"", ""hour"", and ""day"".'");
	
	Если ЗначениеЗаполнено(ВремяШтучноеЕдИзм)
		И ДоступныеЕдиницыИзмеренияВремени.Найти(ВремяШтучноеЕдИзм) = Неопределено Тогда
		ТекстСообщения = СтрШаблон(ШаблонСообщения, НСтр("ru = 'Время выполнения';
														|en = 'Execution time'"));
		ОбщегоНазначения.СообщитьПользователю(ТекстСообщения, ЭтотОбъект, "ВремяШтучноеЕдИзм", , Отказ);
	КонецЕсли;
	
	Если ЗначениеЗаполнено(ВремяПЗЕдИзм)
		И ДоступныеЕдиницыИзмеренияВремени.Найти(ВремяПЗЕдИзм) = Неопределено Тогда
		ТекстСообщения = СтрШаблон(ШаблонСообщения, НСтр("ru = 'Время ПЗ';
														|en = 'Setup time'"));
		ОбщегоНазначения.СообщитьПользователю(ТекстСообщения, ЭтотОбъект, "ВремяПЗЕдИзм", , Отказ);
	КонецЕсли;
