...
Функция ПроверитьФлагиПартнераКлиентПоставщик(Партнер,
	...
	Если ЗначениеЗаполнено(Партнер) Тогда
		...
		Если Не ФлагиПартнера.Клиент = Истина И Не ФлагиПартнера.Поставщик = Истина Тогда
			...
			Если ЭтоНовыйДоговор Тогда
				ТекстСообщения = НСтр("ru = 'Создание договора возможно для партнеров с признаками ""Клиент/Поставщик""';
										|en = 'Contract may be created for partners with the ""Customer/Vendor"" flags'");
			Иначе
		...
		ИначеЕсли Не ФлагиПартнера.Поставщик = Истина
			...
			//-- Устарело_Переработка24
			Или Не ОперацииПроизводстваУПереработчика2_5.Найти(ХозяйственнаяОперация) = Неопределено
			//-- НЕ УТ
...
Процедура ПриОпределенииДоступныхВидовОповещенийПользователей(ОписаниеВидовОповещений) Экспорт
	...
	НовыйВидОповещений.Наименование = НСтр("ru = 'Приближение даты окончания действия договора';
											|en = 'Approach of contract expiration date'", ОбщегоНазначения.КодОсновногоЯзыка());
	...
	НовыйВидОповещений.ШаблонТекстаОповещения = "ru = 'Заканчивается срок действия договора [ДатаОкончанияДействия].';
												|en = 'The contract is expiring on [ДатаОкончанияДействия].'"; // @NStr-1
	НовыйВидОповещений.ТекстСгруппированногоОповещения = "ru = 'Заканчивается срок действия договора.';
														|en = 'The contract is expiring.'"; // @NStr-1
	
	НовыйВидОповещений = ОповещенияПользователейОСобытиях.ДобавитьОписаниеВидаОповещенийПользователей(ОписаниеВидовОповещений);
	НовыйВидОповещений.ГруппаВидаОповещений = ГруппаВидаОповещенийДоговоры;
	НовыйВидОповещений.Идентификатор = "88ea3524-fcef-48e5-907f-b339b1886347";
	НовыйВидОповещений.Наименование = НСтр("ru = 'Окончание срока действия договора';
											|en = 'Contract expiration'", ОбщегоНазначения.КодОсновногоЯзыка());
	НовыйВидОповещений.ТипОповещения = Перечисления.ТипыОповещенийПользователей.Неоперативное;
	...
	НовыйВидОповещений.ШаблонТекстаОповещения = "ru = 'Закончился срок действия договора [ДатаОкончанияДействия].';
												|en = 'The contract is expired on [ДатаОкончанияДействия].'"; // @NStr-1
	НовыйВидОповещений.ТекстСгруппированногоОповещения = "ru = 'Закончился срок действия договора.';
														|en = 'The contract is expired.'"; // @NStr-1
КонецПроцедуры
