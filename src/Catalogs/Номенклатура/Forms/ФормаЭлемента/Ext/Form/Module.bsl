				...
				Иначе
					ПараметрыКопированияДополнительныхДанных.Добавить("КопироватьПодакцизныеКатегорииБезКоэффициента",
						НСтр("ru = 'Копировать подакцизные категории (без коэффициента)';
							|en = 'Copy excisable categories (without coefficient)'"),
						Истина);
				КонецЕсли;
...
Процедура ТоварУчаствуетВДеятельностиПоСтавкеНДС0ПриИзменении(Элемент)
	...
	//++ НЕ УТ
	Если Не ТоварУчаствуетВДеятельностиПоСтавкеНДС0 Тогда
		Объект.Операции0 = Неопределено;
	КонецЕсли;
	Элементы.Операции0.Доступность = ТоварУчаствуетВДеятельностиПоСтавкеНДС0;
	Элементы.Операции0.АвтоОтметкаНезаполненного = ТоварУчаствуетВДеятельностиПоСтавкеНДС0;
	//-- НЕ УТ
...
Функция КартинкиРеквизитовСоздаваемогоОбъекта(ПараметрыОткрытия)
	...
	ПараметрыОткрытия.Свойство("ДополнительныеДанные", ДополнительныеДанные);
	
	Если ДополнительныеДанные = Неопределено 
		Или Не ДополнительныеДанные.Свойство("РаспознанныйДокумент", РаспознанныйДокумент) Тогда
