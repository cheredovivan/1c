...
Функция ТекстЗапросаНомераСтрокОсновныхИзделий()
	...
	|
	|ГДЕ
	|	НЕ ТаблицаТовары.ГруппировкаЗатрат = ЗНАЧЕНИЕ(Перечисление.ГруппировкиЗатратВЗаказеПереработчику.ПоЭтапамПроизводства)
	|
	|СГРУППИРОВАТЬ ПО
...
Функция ТекстЗапросаКорректныеПоляПартийПроизводства()
	...
	|	ВЫБОР
	|		КОГДА НЕ ТИПЗНАЧЕНИЯ(Т.Серия) = ТИП(Справочник.СерииНоменклатуры)
