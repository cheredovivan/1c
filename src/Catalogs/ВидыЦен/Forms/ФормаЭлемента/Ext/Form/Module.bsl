...
Процедура ПередЗаписью(Отказ, ПараметрыЗаписи)
	...
	Если УтверждениеВидовЦен // Если у пользоваетля нет прав на запись, то нет смысла задавать ему вопрос
		...
		|en = 'The selected pricing method may contain an arbitrary algorithm.
		|Do you confirm its safety and use?'");
...
Процедура УстановитьПодсказкуСтатусаДействия(Элемент, ЗначениеСтатуса, УтверждениеНедоступно)
	...
	Если ЗначениеСтатуса = ПредопределенноеЗначение("Перечисление.СтатусыДействияВидовЦен.НеДействует") Тогда
		ТекстПодсказки = НСтр("ru = 'Действие вида цен прекращено или не начато';
								|en = 'The price type is no longer valid or has not yet come into effect'");
	КонецЕсли;
	...
	Если ЗначениеСтатуса = ПредопределенноеЗначение("Перечисление.СтатусыДействияВидовЦен.Действует") Тогда
		...
		Если УтверждениеНедоступно Тогда
			ТекстПодсказки = НСтр("ru = 'Установка статуса ""Действует"" недоступно для данного способа задания цены.
				...
				|en = 'Setting the ""Valid"" status is unavailable for this pricing method.
				|Contact a user with the ""Price type approval"" role or an administrator.'");
		Иначе
			ТекстПодсказки = НСтр("ru = 'Вид цен может использоваться в документах и автоматических расчетах';
									|en = 'You can use the price type in documents and automatic calculations'");
		КонецЕсли;
...
Процедура НастроитьФорму()
	...
	Если ИспользоватьПроизвольныеСхемыКомпоновкиДанных Тогда
		...
		ИмяМакетаТиповой20 = Метаданные.Справочники.ВидыЦен.Макеты.Типовой.Имя;
		ИмяМакетаТиповой25 = Метаданные.Справочники.ВидыЦен.Макеты.Типовой2_5.Имя;
		
		ДобавитьПроизвольныйЗапрос20 = Ложь;
		ДобавитьПроизвольныйЗапрос25 = Ложь;
		
		Если ЗначениеЗаполнено(Объект.Ссылка) Тогда
			
			ДобавитьПроизвольныйЗапрос20 = Не ИспользуетсяЦенообразование25
				Или Объект.СхемаКомпоновкиДанных = ИмяМакетаТиповой20;
			
			ДобавитьПроизвольныйЗапрос25 = ИспользуетсяЦенообразование25
				Или Объект.СхемаКомпоновкиДанных = ИмяМакетаТиповой25;
			
		Иначе
			
			Если ИспользуетсяЦенообразование25 Тогда
				ДобавитьПроизвольныйЗапрос25 = Истина;
			Иначе
				ДобавитьПроизвольныйЗапрос20 = Истина;
			КонецЕсли;
			
		КонецЕсли;
		
		Если ДобавитьПроизвольныйЗапрос20 Тогда
			
			Элементы.СхемаКомпоновкиДанных.СписокВыбора.Добавить(ИмяМакетаТиповой20,
				НСтр("ru = 'Произвольный запрос (версия 2.0)';
					|en = 'Arbitrary request (version 2.0)'"));
			
		КонецЕсли;
		
		Если ДобавитьПроизвольныйЗапрос25 Тогда
			
			Элементы.СхемаКомпоновкиДанных.СписокВыбора.Добавить(ИмяМакетаТиповой25,
				НСтр("ru = 'Произвольный запрос (версия 2.5)';
					|en = 'Arbitrary request (version 2.5)'"));
...
Процедура НастроитьВидимостьЭлементовФормы(Форма)
	...
	//++ НЕ УТКА
	Элементы.ИспользоватьПриПередачеПродукцииДавальцу.Видимость = Форма.ИспользоватьПроизводство;
	//-- НЕ УТКА
