...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ТипЗнч(КомандаУПУП) = Тип("КомандаФормы") Тогда
		...
		Иначе
			ЗаголовокУПУП = НСтр("ru = 'Уведомление о предоставлении полномочий представителю - МЧД (ПФР; не применяется)';
								|en = 'Уведомление о предоставлении полномочий представителю - МЧД (ПФР; не применяется)'");
		КонецЕсли;
	...
	Если ТипЗнч(КомандаУПРУП) = Тип("КомандаФормы") Тогда
		...
		Иначе
			ЗаголовокУПРУП = НСтр("ru = 'Уведомление о прекращении полномочий представителя - МЧД (ПФР; не применяется)';
									|en = 'Уведомление о прекращении полномочий представителя - МЧД (ПФР; не применяется)'");
		КонецЕсли;
	...
	Если ТекущиеДанные <> Неопределено Тогда
		...
		ИначеЕсли ТекущиеДанные.НеОтправляетсяВКонтролирующийОрган
			...
			ИЛИ ТекущиеДанныеНаименованиеОтчета =
				...
				И НЕ ОнлайнСервисыРегламентированнойОтчетностиВызовСервера.СобытиеНаступило("освобождении от НДС для у")
			ИЛИ ТекущиеДанныеНаименованиеОтчета = "Аудиторское заключение о консолидированной финансовой отчетности"
				И НЕ ОнлайнСервисыРегламентированнойОтчетностиВызовСервера.СобытиеНаступило("Аудиторское заключение о") Тогда
...
Процедура ОбновитьДанныеПродолжить(ВыбранныеОрганизации)
	...
	Если ЕстьПравоНаДО И ПараметрыПриложения["БРО.БольшеНеПоказыватьСообщенияТребующиеРасшифровки"] <> Истина Тогда
		// исключение показа диалога расшифровки при обмене
