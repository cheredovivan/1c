...
Процедура ОтменитьЗаданиеНаРазмещение(Команда)
	...
	ПоказатьВопрос(Оповещение, НСтр("ru = 'Для отмены размещения необходимо пометить на удаление документ размещения. Вернуться к редактированию будет невозможно. Пометить на удаление?';
									|en = 'To cancel put-away, mark the put-away document for deletion. It will be impossible to get back to editing. Mark the document for deletion?'"), СписокЗначений, 0);
...
Функция ОтменитьЗаданиеНаРазмещениеНаСервере()
	...
	Исключение
		ОбщегоНазначения.СообщитьПользователю(НСтр("ru = 'Не удалось отменить задание на размещение.';
													|en = 'Cannot cancel the put-away task.'"));
