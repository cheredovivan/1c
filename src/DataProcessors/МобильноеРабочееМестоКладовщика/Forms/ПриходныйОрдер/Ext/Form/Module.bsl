...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ЗначениеЗаполнено(НомерВходящегоДокумента) И ЗначениеЗаполнено(ДатаВходящегоДокумента) Тогда
		ИнформацияОДокументе = СтрШаблон(НСтр("ru = '№ %1 от %2';
												|en = 'No. %1 from %2'"), НомерВходящегоДокумента, Формат(ДатаВходящегоДокумента, НСтр("ru = 'ДФ=''дд.ММ.гг ЧЧ:мм''';
																																|en = 'DF=''MM/dd/yy hh:mm tt'''")));
	Иначе
		ИнформацияОДокументе = СтрШаблон(НСтр("ru = '№ %1 от %2';
												|en = 'No. %1 from %2'"), ПрефиксацияОбъектовКлиентСервер.НомерНаПечать(Номер), Формат(Дата, НСтр("ru = 'ДФ=''дд.ММ.гг ЧЧ:мм''';
																																			|en = 'DF=''MM/dd/yy hh:mm tt'''")));
	КонецЕсли;
...
Процедура ЗаполнитьФорму()
	...
	Запрос.Текст = 
		...
		|	ПриходныйОрдерНаТоварыШтрихкодыУпаковок.ШтрихкодУпаковки.Номенклатура КАК ШтрихкодУпаковкиНоменклатура,
		|	ПриходныйОрдерНаТоварыШтрихкодыУпаковок.ШтрихкодУпаковки.Характеристика КАК ШтрихкодУпаковкиХарактеристика
		|ИЗ
...
Процедура ОтменитьПриемкуОрдера(Команда)
	...
	ПоказатьВопрос(Оповещение, НСтр("ru = 'Для отмены приемки необходимо пометить на удаление приходный ордер. Вернуться к редактированию будет невозможно. Пометить на удаление?';
									|en = 'To cancel goods receipt, mark the goods receipt document for deletion. It will be impossible to get back to editing. Mark the document for deletion?'"), СписокЗначений, 0);
...
Функция ОтменитьПриемкуОрдераНаСервере()
	...
	Исключение
		ОбщегоНазначения.СообщитьПользователю(НСтр("ru = 'Не удалось отменить приемку.';
													|en = 'Cannot cancel goods receipt.'"));
