...
Процедура НачалоПериодаПриИзменении(Элемент)
	...
	Если ПериодГлавное.ДатаНачала < ОграничениеСнизу Тогда
		...
		СообщениеОбОшибке = СтрШаблон(НСтр("ru = 'Сверка сальдо ЕНС возможна с %1';
											|en = 'Сверка сальдо ЕНС возможна с %1'"), Формат(ОграничениеСнизу, "ДЛФ=DD"));
...
Процедура НачалоПериодаПлатежиПриИзменении(Элемент)
	...
	Если ПериодПлатежи.ДатаНачала < ОграничениеСнизу Тогда
		...
		СообщениеОбОшибке = СтрШаблон(НСтр("ru = 'Сверка платежей возможна с %1';
											|en = 'Сверка платежей возможна с %1'"), Формат(ОграничениеСнизу, "ДЛФ=DD"));
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(СообщениеОбОшибке);
	КонецЕсли;
...
Процедура НачалоПериодаОтчетыПриИзменении(Элемент)
	...
	Если ПериодОтчеты.ДатаНачала < ОграничениеСнизу Тогда
		...
		СообщениеОбОшибке = СтрШаблон(НСтр("ru = 'Сверка деклараций и уведомлений возможна с %1';
											|en = 'Сверка деклараций и уведомлений возможна с %1'"), Формат(ОграничениеСнизу, "ДЛФ=DD"));
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(СообщениеОбОшибке);
	КонецЕсли;
...
Процедура НачалоПериодаНачисленияПриИзменении(Элемент)
	...
	Если ПериодНачисления.ДатаНачала < ОграничениеСнизу Тогда
		...
		СообщениеОбОшибке = СтрШаблон(НСтр("ru = 'Сверка начислений налогов возможна с %1';
											|en = 'Сверка начислений налогов возможна с %1'"), Формат(ОграничениеСнизу, "ДЛФ=DD"));
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(СообщениеОбОшибке);
	КонецЕсли;
...
Процедура НачалоПериодаУплатыПриИзменении(Элемент)
	...
	Если ПериодУплаты.ДатаНачала < ОграничениеСнизу Тогда
		...
		СообщениеОбОшибке = СтрШаблон(НСтр("ru = 'Сверка уплаты налогов возможна с %1';
											|en = 'Сверка уплаты налогов возможна с %1'"), Формат(ОграничениеСнизу, "ДЛФ=DD"));
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(СообщениеОбОшибке);
	КонецЕсли;
...
Процедура НачалоПериодаПениШтрафыПриИзменении(Элемент)
	...
	Если ПериодПениШтрафы.ДатаНачала < ОграничениеСнизу Тогда
		...
		СообщениеОбОшибке = СтрШаблон(НСтр("ru = 'Сверка пеней и штрафов возможна с %1';
											|en = 'Сверка пеней и штрафов возможна с %1'"), Формат(ОграничениеСнизу, "ДЛФ=DD"));
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(СообщениеОбОшибке);
	КонецЕсли;
...
Процедура НачалоПериодаОперацииЕНППриИзменении(Элемент)
	...
	Если ПериодОперацииЕНПСверка.ДатаНачала < ОграничениеСнизу Тогда
		...
		СообщениеОбОшибке = СтрШаблон(НСтр("ru = 'Сверка операций ЕНП возможна с %1';
											|en = 'Сверка операций ЕНП возможна с %1'"), Формат(ОграничениеСнизу, "ДЛФ=DD"));
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(СообщениеОбОшибке);
	КонецЕсли;
...
Процедура ЗаполнитьСверкуРасхождения(РеквизитСверкаРасхождения, КоличествоРасхождений)
	...
	Иначе
		...
		РеквизитСверкаРасхождения = СтрШаблон(НСтр("ru = '%1';
													|en = '%1'"), ТекстСверкиРасхождения);
	КонецЕсли;
КонецПроцедуры
