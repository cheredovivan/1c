...
Функция ВременнаяТаблицаДанныеЗаполнения()
	...
	ТекстЗапроса =
		...
		|				НЕОПРЕДЕЛЕНО
		//++ НЕ УТКА
		|			КОГДА Т.Заказ ССЫЛКА Документ.ЭтапПроизводства2_2 ТОГДА
		|				Т.Заказ
		//++ Устарело_Переработка24
		|			КОГДА ЗаказыДавальца.Ссылка ЕСТЬ НЕ NULL И ЗаказыДавальца.УправлениеПроизводством2_2 = ЛОЖЬ ТОГДА
		...
		//++ Устарело_Переработка24
		|			КОГДА ЗаказыДавальца.Ссылка ЕСТЬ НЕ NULL И ЗаказыДавальца.УправлениеПроизводством2_2
		|				ТОГДА ЗаказыДавальца.Партнер
		//-- Устарело_Переработка24
		...
		|			ТОГДА ВЫРАЗИТЬ(Т.Назначение КАК Справочник.Назначения).Партнер
		//-- НЕ УТКА
		|			ИНАЧЕ
		|				НЕОПРЕДЕЛЕНО
		...
		//-- Устарело_Переработка24
		|				ЗНАЧЕНИЕ(Перечисление.ТипыНазначений.ДавальческоеМатериалыПодЭтап22))
		|			ТОГДА ВЫРАЗИТЬ(Т.Назначение КАК Справочник.Назначения).Договор
		//-- НЕ УТКА
		|			ИНАЧЕ
		|				НЕОПРЕДЕЛЕНО
