...
Процедура УправлениеФормой()
	...
	Если НаправлениеДокумента = Перечисления.НаправленияРаспознанногоДокумента.Входящий Тогда
		...
		ВОтКонтрагента = ?(ЗначениеЗаполнено(ДанныеЗаголовка.Контрагент),
			СтрШаблон(НСтр("ru = 'от %1';
							|en = ', %1'"), ДанныеЗаголовка.Контрагент),
			"");
	Иначе
		СтрокаТипДокумента = НСтр("ru = 'Реализация';
									|en = 'Sales'");
		ВОтКонтрагента = ?(ЗначениеЗаполнено(ДанныеЗаголовка.Контрагент),
			СтрШаблон(НСтр("ru = 'в %1';
							|en = 'in %1'"), ДанныеЗаголовка.Контрагент),
			"");
	КонецЕсли;
	
	ПредставлениеДатыДокумента = ?(ЗначениеЗаполнено(ДанныеЗаголовка.ДатаДокумента),
		СтрШаблон(НСтр("ru = 'от %1';
						|en = ', %1'"), Формат(ДанныеЗаголовка.ДатаДокумента, "ДЛФ=D")), "");
