...
Процедура ЗагрузитьГруппуВРЦПоследовательно(ПараметрыПланирования, Этап, ИсполнителиЭтапа, ПараметрыРазмещения, Отказ)
	...
	СтруктураПоиска = Новый Структура("ВидРабочегоЦентра, ДатаИнтервала, Этап");
	...
	Пока Индекс <= МаксимальныйИндекс Цикл
		...
		Иначе
			...
			Если ИнтервалРазмещения <> Неопределено Тогда
				СтруктураПоиска.ВидРабочегоЦентра = ИсполнительЭтапа.ВидРабочегоЦентра;
				СтруктураПоиска.ДатаИнтервала = ИнтервалРазмещения.ДатаИнтервала;
				СтруктураПоиска.Этап = Этап.ЭтапПроизводства;
				
				Если ПараметрыРазмещения.ЗагрузкаВРЦ.НайтиСтроки(СтруктураПоиска).Количество() <> 0 Тогда
					НепрерывноеРазмещение = Истина;
					ИнтервалРазмещения = ОчереднойИнтервалРазмещенияББВ(
						ПараметрыПланирования, ИсполнительЭтапа, ИнтервалРазмещения, НепрерывноеРазмещение);
