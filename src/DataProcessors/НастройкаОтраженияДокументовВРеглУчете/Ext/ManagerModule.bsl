...
Функция ДоступныеСчетаУчетаНоменклатуры() Экспорт
	...
	|;
	|/////////////////////////////////////////////////////////////////////////////
	|// Счета учета акцизов при продаже
	|ВЫБРАТЬ
	|	ПланСчетов.Ссылка КАК Ссылка,
	|	ПланСчетов.Код КАК Код
	|ИЗ
	|	ПланСчетов.Хозрасчетный КАК ПланСчетов
	|	ЛЕВОЕ СОЕДИНЕНИЕ ПланСчетов.Хозрасчетный.ВидыСубконто КАК ВидыСубконтоСтавкиНДС
	|	ПО ПланСчетов.Ссылка = ВидыСубконтоСтавкиНДС.Ссылка
	|		И ВидыСубконтоСтавкиНДС.ВидСубконто = (ЗНАЧЕНИЕ(ПланВидовХарактеристик.ВидыСубконтоХозрасчетные.СтавкиНДС))
	|ГДЕ
	|	ПланСчетов.ВидыСубконто.ВидСубконто = ЗНАЧЕНИЕ(ПланВидовХарактеристик.ВидыСубконтоХозрасчетные.НоменклатурныеГруппы)
	|	И ВидыСубконтоСтавкиНДС.Ссылка ЕСТЬ NULL
	|	И ПланСчетов.Вид = ЗНАЧЕНИЕ(ВидСчета.Активный)
	|	И НЕ ПланСчетов.Ссылка В ИЕРАРХИИ (ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.Продажи_ЭкспортныеПошлины))
	|	И НЕ ПланСчетов.НалоговыйУчет
	|	И НЕ ПланСчетов.Забалансовый
	|	И НЕ ПланСчетов.ЗапретитьИспользоватьВПроводках
	|
	|ОБЪЕДИНИТЬ ВСЕ
	...
	СтруктураСчетовУчета.Вставить("АкцизыПриПродаже", МассивСчетовИзРезультатаЗапроса(МассивРезультатов[15]));
	СтруктураСчетовУчета.Вставить("ОтклоненияВСтоимости", МассивСчетовИзРезультатаЗапроса(МассивРезультатов[16]));
...
Функция ТекстЗапросаГруппировкиТаблицыИсключений()
	...
	|	И (&ОрганизацияОтбора = НЕОПРЕДЕЛЕНО И ТаблицаНастроек.Организация В (&Организации)
	|		ИЛИ ТаблицаНастроек.Организация = &ОрганизацияОтбора)
	|СГРУППИРОВАТЬ ПО
