...
Процедура ПланВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	ИначеЕсли ИмяПоля = "СвободныйВыпускИзОбособленныхМатериалов" Тогда
		СтандартнаяОбработка = Ложь;
		ПоказатьПредупреждение(, ТекстПредупрежденияСвободныйВыпускИзОбособленныхМатериалов());
	КонецЕсли;
...
&НаКлиенте
Процедура ИзделияКПредпросмотруВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	
	ИмяПоля = СтрЗаменить(Поле.Имя, "ИзделияКПредпросмотру", "");
	
	Если ИмяПоля = "СвободныйВыпускИзОбособленныхМатериалов" Тогда
		СтандартнаяОбработка = Ложь;
		ПоказатьПредупреждение(, ТекстПредупрежденияСвободныйВыпускИзОбособленныхМатериалов());
	КонецЕсли;
...
Процедура ЗаполнитьИзделияКПредпросмотру(Отказ)
	...
	Для Индекс = 0 По Данные.Изделия.Количество() - 1 Цикл
		...
		ЗаполнитьЗначенияСвойств(НоваяСтрока, СтрокаИзделия,
			"Номенклатура, Характеристика, Назначение, Получатель, Количество");
		
		НоваяСтрокаИтог.Количество = НоваяСтрокаИтог.Количество + СтрокаИзделия.Количество;
		
		Если СтрокаИзделия.СвободныйВыпускИзОбособленныхМатериалов
			И НЕ НоваяСтрокаИтог.СвободныйВыпускИзОбособленныхМатериалов Тогда
			НоваяСтрокаИтог.СвободныйВыпускИзОбособленныхМатериалов = Истина;
		КонецЕсли;
...
Функция ДанныеКФормированию()
	...
	Запрос.Текст = 
		...
		|	Изделия.СвободныйВыпускИзОбособленныхМатериалов КАК СвободныйВыпускИзОбособленныхМатериалов,
		|	ВЫБОР
	...
	ТекстЗапроса =
		...
		|	
		|	МАКСИМУМ(Изделия.СвободныйВыпускИзОбособленныхМатериалов) КАК СвободныйВыпускИзОбособленныхМатериалов,
		|	
...
#Область План
	...
	ПолеЭлемента = Элемент.Поля.Элементы.Добавить();
	ПолеЭлемента.Поле = Новый ПолеКомпоновкиДанных(Элементы.ИзделияКПредпросмотруСвободныйВыпускИзОбособленныхМатериалов.Имя);
	
	ОтборЭлемента = Элемент.Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
	ОтборЭлемента.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("ИзделияКПредпросмотру.СвободныйВыпускИзОбособленныхМатериалов");
	ОтборЭлемента.ВидСравнения = ВидСравненияКомпоновкиДанных.Равно;
	ОтборЭлемента.ПравоеЗначение = Ложь;
...
Процедура НастроитьЭлементыФормы()
	...
	Элементы.ИзделияКПредпросмотруГруппаИзделие.Заголовок = 
		?(ПолучитьФункциональнуюОпцию("ИспользоватьХарактеристикиНоменклатуры"),
			НСтр("ru = 'Номенклатура, Характеристика, Спецификация, Заказ на производство, Получатель, Назначение';
				|en = 'Item, Variant, Bill of materials, Production order, Recipient, Assignment'"),
			НСтр("ru = 'Номенклатура, Спецификация, Заказ на производство, Получатель, Назначение';
				|en = 'Item, Bill of materials, Production order, Recipient, Assignment'"));
...
&НаКлиентеНаСервереБезКонтекста
Функция ТекстПредупрежденияСвободныйВыпускИзОбособленныхМатериалов()
	
	Возврат НСтр("ru = 'Необособленный выпуск из обособленных материалов.
		|Изделие не укрупняется со строками других заказов
		|и строками с другим назначением.
		|Рекомендуется обособить полуфабрикат при выпуске.';
