...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Иначе
		...
		Если ДанныеОперации <> Неопределено Тогда
			...
			ЗаполнитьЗначенияСвойств(ЭтотОбъект, РеквизитыМаршрута, "Маршрут,МаршрутПредставление");
			Элементы.МаршрутСтраницы.ТекущаяСтраница = ?(РеквизитыМаршрута.МаршрутДоступен, Элементы.МаршрутДоступен, Элементы.МаршрутНедоступен);
...
Функция ДанныеОперацииПоКлючу(КлючОперации, ИспользоватьФизЛиц)
	...
	Запрос = Новый Запрос(
		...
		|	ЕСТЬNULL(Операции.Операция.Наименование, Очередь.ПроизводственнаяОперация.Наименование)         КАК Наименование,
		|	ЕСТЬNULL(Операции.Операция.Количество, 1)                                                       КАК КоличествоШтучное,
		|	ЕСТЬNULL(Операции.Операция.ЕдиницаИзмерения, Очередь.ПроизводственнаяОперация.ЕдиницаИзмерения) КАК ЕдиницаИзмерения,
