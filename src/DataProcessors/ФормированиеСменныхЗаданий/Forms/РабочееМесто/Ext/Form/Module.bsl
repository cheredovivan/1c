...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СтрокаПустаяСсылка = "NULL";
	//++ Локализация
	СтрокаПустаяСсылка = "ЗНАЧЕНИЕ(Справочник.Должности.ПустаяСсылка)";
	//-- Локализация
	...
	Если ИсточникВыбора.ИмяФормы = "Обработка.ФормированиеСменныхЗаданий.Форма.ПодборИсполнителей" Тогда
		...
		Если ТипЗнч(ВыбранноеЗначение) = Тип("Структура") Тогда
			...
			ИначеЕсли ВыбранноеЗначение.ДополнительныеПараметры.Режим = "НазначениеНовомуИсполнителю" Тогда
				...
				ОповеститьОбИзмененииНазначенияОпераций();
...
Процедура Подключаемый_ВыполнитьКомандуИнтеграции(Команда)
	...
	ИнтеграцияС1СДокументооборотБазоваяФункциональностьКлиент.ВыполнитьПодключаемуюКомандуИнтеграции(Команда, ЭтотОбъект, Элементы.СменныеЗадания);
...
Процедура УстановитьУсловноеОформление()
	...
	// Смены - несоответствие должности/разряда
	Элемент = УсловноеОформление.Элементы.Добавить();
	
	Элемент.Поля.Элементы.Добавить().Поле = Новый ПолеКомпоновкиДанных(Элементы.СменыПредставлениеСтроки.Имя);
	
	ОтборЭлемента = Элемент.Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
	ОтборЭлемента.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("Смены.НеСоответствиеДолжностиРазряда");
	ОтборЭлемента.ВидСравнения = ВидСравненияКомпоновкиДанных.Равно;
...
Процедура ЗаполнитьСмены()
	...
	//++ Локализация
	ИначеЕсли РежимНазначения = РежимНазначенияИсполнителям()
		И ПолучитьФункциональнуюОпцию("НормированиеИРасценкаРаботПоТарифнымСеткам") Тогда 
		
		КадровыеДанные = ОперативныйУчетПроизводстваЛокализация.ДолжностьРазрядСотрудников(
			ОбщегоНазначенияКлиентСервер.СвернутьМассив(ТаблицаСмены.ВыгрузитьКолонку("ОбъектНазначения")),
			ВыборкаИтоги.ДатаМин);
	//-- Локализация
	КонецЕсли;
	...
	СменыССотрудниками = Новый Массив;
			...
			Иначе
				...
				Если РежимНазначения = РежимНазначенияИсполнителям() Тогда
					...
					Если ЗначениеЗаполнено(КадровыеДанные) Тогда
						...
						СтрокаДанных = КадровыеДанные.Найти(СтрокаОбъект.ОбъектНазначения, "Сотрудник");
						Если ЗначениеЗаполнено(СтрокаДанных) Тогда
							
							СменыССотрудниками.Добавить(ВыборкаСЗ.СменноеЗадание);
							
							СтрокаОбъект.Должность = СтрокаДанных.Должность;
							СтрокаОбъект.ПорядокРазряда = СтрокаДанных.Порядок;
	...
	//++ Локализация
	Если РежимНазначения = РежимНазначенияИсполнителям() Тогда
		ПроверитьДолжностиНазначенныхОпераций(Дерево, СменыССотрудниками);
	КонецЕсли;
	//-- Локализация
...
Процедура ОбновитьСменыПослеНазначенияОпераций(МассивСмен)
	...
	Для каждого СменноеЗадание Из МассивСмен Цикл
		...
		Для каждого СтрокаОбъект Из СтрокаСмена.Строки Цикл
			...
			Если НайденныеСтроки.ВГраница() = -1 Тогда
				...
				СтрокаОбъект.НеСоответствиеДолжностиРазряда = Ложь;
	...
	Иначе
		...
		//++ Локализация
		Если РежимНазначения = РежимНазначенияИсполнителям() Тогда
			ПроверитьДолжностиНазначенныхОпераций(Дерево, МассивСмен);
		КонецЕсли;
		//-- Локализация
...
//++ Локализация
&НаСервере
Процедура ПроверитьДолжностиНазначенныхОпераций(Дерево, СменныеЗадания = Неопределено)
	
	Запрос = Новый Запрос("
	|ВЫБРАТЬ
	|	ОперацииСЗ.СменноеЗадание КАК СменноеЗадание,
	|	ОперацииСЗ.Исполнитель КАК Исполнитель,
	|	ОчередьОпераций.Должность КАК Должность,
	|	ОчередьОпераций.Разряд КАК Разряд,
	|	Разряды.РеквизитДопУпорядочивания КАК Порядок,
	|	0 КАК НеСоответствие
	|ИЗ
	|	РегистрСведений.ОчередьПроизводственныхОпераций КАК ОчередьОпераций
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.ОперацииКСозданиюСменныхЗаданий КАК ОперацииСЗ
	|	ПО ОперацииСЗ.Этап = ОчередьОпераций.Этап
	|	И ОперацииСЗ.ИдентификаторОперации = ОчередьОпераций.ИдентификаторОперации
	|	И &ОтборСменныеЗадания
	|	И ОчередьОпераций.ОсновнаяЗапись
	|	И ОчередьОпераций.Должность <> ЗНАЧЕНИЕ(Справочник.Должности.ПустаяСсылка)
	|	И ОчередьОпераций.Разряд <> ЗНАЧЕНИЕ(Справочник.РазрядыКатегорииДолжностей.ПустаяСсылка)
	|	ЛЕВОЕ СОЕДИНЕНИЕ Справочник.РазрядыКатегорииДолжностей КАК Разряды
	|	ПО ОчередьОпераций.Разряд = Разряды.Ссылка");
	
	Если ЗначениеЗаполнено(СменныеЗадания) Тогда
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "&ОтборСменныеЗадания", "ОперацииСЗ.СменноеЗадание В (&СменныеЗадания)");
		Запрос.УстановитьПараметр("СменныеЗадания", СменныеЗадания);
	Иначе
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "&ОтборСменныеЗадания", "ОперацииСЗ.СменноеЗадание <> ЗНАЧЕНИЕ(Документ.СменноеЗадание.ПустаяСсылка)");
	КонецЕсли;
	
	Результат = Запрос.Выполнить().Выгрузить();
	
	Для Каждого Строка Из Результат Цикл
		СтрокаСЗ = Дерево.Строки.Найти(Строка.СменноеЗадание, "СменноеЗадание", Ложь);
		
		Если ЗначениеЗаполнено(СтрокаСЗ) Тогда
			
			СтрокаОбъект = СтрокаСЗ.Строки.Найти(Строка.Исполнитель, "ОбъектНазначения", Ложь);
			
			Если ЗначениеЗаполнено(Строка.Должность) И ЗначениеЗаполнено(СтрокаОбъект.Должность) Тогда
			
				Если Строка.Должность <> СтрокаОбъект.Должность Тогда
					Строка.НеСоответствие = 1;
				КонецЕсли;
				
				Если Строка.НеСоответствие = 0 И ЗначениеЗаполнено(Строка.Порядок) И ЗначениеЗаполнено(СтрокаОбъект.ПорядокРазряда) Тогда
					
					Если Строка.Порядок > СтрокаОбъект.ПорядокРазряда Тогда
						Строка.НеСоответствие = 1;
					КонецЕсли;
	...
	Результат.Свернуть("СменноеЗадание, Исполнитель", "НеСоответствие");
	...
	Для Каждого Строка Из Результат Цикл
		...
		Если ЗначениеЗаполнено(СтрокаСЗ) Тогда
			...
			СтрокаОбъект.НеСоответствиеДолжностиРазряда = Строка.НеСоответствие > 0;
...
Процедура ДобавитьИсполнителей(СменноеЗадание, Ссылки)
	...
	//++ Локализация
	ПроверятьСоответствиеДолжностей = ПолучитьФункциональнуюОпцию("НормированиеИРасценкаРаботПоТарифнымСеткам")
		И ТипЗнч(НовыеИсполнители[0]) = Тип("СправочникСсылка.Сотрудники");
	
	Если ПроверятьСоответствиеДолжностей Тогда
		
		КадровыеДанные = ОперативныйУчетПроизводстваЛокализация.ДолжностьРазрядСотрудников(НовыеИсполнители, СтрокаСмена.Дата);
	...
	Для каждого Ссылка Из НовыеИсполнители Цикл
		...
		//++ Локализация
		Если ПроверятьСоответствиеДолжностей Тогда
			СтрокаКадровыхДанных = КадровыеДанные.Найти(Ссылка, "Сотрудник");
			СтрокаОбъект.Должность = СтрокаКадровыхДанных.Должность;
			СтрокаОбъект.ПорядокРазряда = СтрокаКадровыхДанных.Порядок;
		КонецЕсли;
		//-- Локализация
...
&НаКлиенте
Процедура ОповеститьОбИзмененииНазначенияОпераций()
	...
	Оповестить("Запись_ОперацииКСозданиюСменныхЗаданий", ПараметрОповещения, ИсточникОповещения);
