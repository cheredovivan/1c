...
Процедура ПодборОперацийИсполнительНачалоВыбораЗавершение(Результат, Параметры) Экспорт
	...
	Если ТекущиеДанные <> Неопределено И ЗначениеЗаполнено(Результат) Тогда
		...
		Если ТипЗнч(Результат) = Тип("СправочникСсылка.Сотрудники")
			И ЗначениеЗаполнено(ТекущиеДанные.Должность) И ЗначениеЗаполнено(ТекущиеДанные.Разряд) Тогда
			ТекущиеДанные.НеСоответствиеДолжностиРазряда = НЕ СоответствиеДолжностиРазрядаИсполнителя(ТекущиеДанные.Исполнитель, ТекущиеДанные.Должность, ТекущиеДанные.Разряд, Дата);
		КонецЕсли;
		//-- Локализация
...
Процедура УстановитьУсловноеОформление()
	...
	Элемент = УсловноеОформление.Элементы.Добавить();
	Элемент.Поля.Элементы.Добавить().Поле = Новый ПолеКомпоновкиДанных(Элементы.ПодборОперацийИсполнитель.Имя);
	
	ОтборЭлемента = Элемент.Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
	ОтборЭлемента.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("Объект.ПодборОпераций.НеСоответствиеДолжностиРазряда");
	ОтборЭлемента.ВидСравнения = ВидСравненияКомпоновкиДанных.Равно;
	ОтборЭлемента.ПравоеЗначение = Истина;
...
Процедура ДобавитьСтрокуВПодборОпераций(КлючОперации)
	...
	НайденныеСтроки = Объект.ПодборОпераций.НайтиСтроки(Новый Структура("КлючОперации", КлючОперации));
	Если НайденныеСтроки.ВГраница() <> -1 Тогда
		ПоказатьПредупреждение(,НСтр("ru = 'Выбранная операция уже добавлена.';
									|en = 'The selected operation is already added.'"));
		Возврат;
	КонецЕсли;
...
Функция ПеренестиВДокументВФоне()
	...
	Операции = РеквизитФормыВЗначение("Объект").ПодборОпераций.Выгрузить();
	Операции.Колонки.Удалить("НомерСтроки");
	ПроизводствоСервер.СвернутьТаблицуЗначений(Операции,,"Количество");
	
	ПараметрыПроцедуры = Новый Структура;
	ПараметрыПроцедуры.Вставить("Операции", Операции);
...
//++ Локализация
&НаСервереБезКонтекста
Функция СоответствиеДолжностиРазрядаИсполнителя(Исполнитель, Должность, Разряд, Дата)
	Возврат ОперативныйУчетПроизводстваЛокализация.СоответствиеДолжностиРазрядаИсполнителя(Исполнитель, Должность, Разряд, Дата);
КонецФункции
//-- Локализация
