...
Процедура ОбновитьЭУОтложенноеПодключение(ОшибкаСоздания)
	...
	Если ОшибкаСоздания Тогда
		ТекстЗаголовка = НСтр("ru = 'При подключении возникли ошибки. Подробности в журнале регистрации.';
								|en = 'Errors occurred when connecting. See the event log for details.'");
	Иначе
		...
		|en = 'The connection request is sent.
		|You can close this form. The connection will continue.
		|The connection will take some time (from several minutes to several hours).
		|After connecting, an email will be sent to the responsible person.'");
	КонецЕсли;
...
Процедура ОбновитьОписаниеПубликуемойСтруктуры()
	...
	Иначе
		...
		ТекстЗаголовкаПриПодключении = НСтр("ru = 'Структура Компании в приложении 1С:Персонал будет соответствовать Структуре предприятия.';
											|en = 'Company structure in 1C:Human Resources will correspond to the enterprise structure.'");
	КонецЕсли;
