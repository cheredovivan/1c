	...
	Если Не Результат
		...
		ШаблонСообщения = 
			НСтр("ru = 'Количество товара ""%1"" для заказа у ""%2"" должно быть кратно %3.';
				|en = 'The quantity of the ""%1"" item for the order from ""%2"" must be a multiple of %3.'");
...
Процедура ЗагрузитьРезультатыПоиска(АдресРезультата)
	...
	Для каждого ЭлементСписка Из Контрагенты Цикл
		...
		НовыйЭлемент.Заголовок = НСтр("ru = 'Кратность упаковки';
										|en = 'Packaging multiplicity'", ОбщегоНазначения.КодОсновногоЯзыка());
