...
//   * Предшественник - Неопределено,ДокументСсылка.ЭтапПроизводства2_2
Функция РеквизитыДеленияКонструктор() Экспорт
	
	Результат = РегистрыСведений.ТехнологическиеПроцессыЭтаповПроизводства.ОграниченияТехпроцессаКонструктор();
...
// Изменяет реквизиты деления этапа с учетом особенностей их хранения
// 
// Параметры:
//  Этап - ДокументСсылка.ЭтапПроизводства2_2
//  РеквизитыДеления - см. РеквизитыДеленияКонструктор
//  Режим - Число - режим работы (0 - все, 1 - только регистр реквизитов деления, 2 - только регистр данных техпроцесса)
Процедура ИзменитьРеквизитыДеленияЭтапа(Этап, РеквизитыДеления, Режим = 0) Экспорт
	...
	Если Режим = 0 ИЛИ Режим = 1 Тогда
		...
		ЗаполнитьЗначенияСвойств(Запись, РеквизитыДеления);
		Запись.Этап = Этап;
		Набор.Записать();
	...
	Если Режим = 0 ИЛИ Режим = 2 Тогда
		...
		Набор = РегистрыСведений.ТехнологическиеПроцессыЭтаповПроизводства.СоздатьНаборЗаписей();
		Набор.Отбор.Этап.Установить(Этап);
		Набор.Прочитать();
		...
		Иначе
			Запись = Набор.Добавить();
			ЗаполнитьЗначенияСвойств(Запись, РедакторПроизводственногоПроцессаВызовСервера.ПараметрыТехпроцессаЭтапа(Этап));
		КонецЕсли;
		ЗаполнитьЗначенияСвойств(Запись, РеквизитыДеления);
		Запись.Этап = Этап;
		Набор.Записать();
...
Функция ДеревоПроизводственногоПроцесса(ПартияПроизводства, ПолучитьОперации) Экспорт
	...
	Дерево.Колонки.Добавить("ПроизводственнаяОперация", Новый ОписаниеТипов("ДокументСсылка.ПроизводственнаяОперация2_2"));
	...
	Пока ВыборкаЭтапы.Следующий() Цикл
		...
		Если Операции <> Неопределено Тогда
			...
			Для каждого НайденнаяСтрока Из Операции.НайтиСтроки(СтруктураПоиска) Цикл
				...
				Иначе
					СтрокаОперация.Картинка = БиблиотекаКартинок.РедакторПроизводственногоПроцессаДокумент;
				КонецЕсли;
...
Функция ТекстЗапросаВТОчередьОперацийДляДеления(ТекстОтборЭтапы)
	...
	ПараметрыТекстаЗапроса.Отборы.Добавить("Очередь.Этап В (&ТекстОтборЭтапы)");
	...
	ТекстЗапроса = РегистрыСведений.ОчередьПроизводственныхОпераций.ТекстЗапросаОчередьОпераций(ПараметрыТекстаЗапроса) + "
		...
		|	Очередь.Этап КАК Этап,
		|	Очередь.ИдентификаторОперации КАК ИдентификаторОперации
		|ПОМЕСТИТЬ ВТИндивидуальные
		|ИЗ
		|	ВТОчередь КАК Очередь
		|ГДЕ
		|	Очередь.Этап В (&ТекстОтборЭтапы)
		|	И Очередь.Операция = ЗНАЧЕНИЕ(Справочник.ТехнологическиеОперации.ПустаяСсылка)
		...
		|	Очередь.Запланировано - Очередь.ТребуетПовторения КАК ОперацийВсего,
		|	ВЫБОР
		|		КОГДА Очередь.Запланировано > (Очередь.Создано + ЕСТЬNULL(Смены.Назначено, 0))
		|				И Индивидуальные.ИдентификаторОперации ЕСТЬ NULL
		|			ТОГДА Очередь.Запланировано - (Очередь.Создано + ЕСТЬNULL(Смены.Назначено, 0))
		|		ИНАЧЕ 0
		|	КОНЕЦ                                             КАК ОперацийКНазначению,
		|	Очередь.Создано <> 0
		|		ИЛИ Смены.Назначено ЕСТЬ НЕ NULL              КАК Начата,
		|	Очередь.Брак > 0                                  КАК ЕстьБрак,
		|	Индивидуальные.ИдентификаторОперации ЕСТЬ НЕ NULL КАК ЭтоИндивидуальнаяОперация
		...
		|
		|	ЛЕВОЕ СОЕДИНЕНИЕ ВТИндивидуальные КАК Индивидуальные
		|	ПО Очередь.Этап = Индивидуальные.Этап
		|		И Очередь.ИдентификаторОперации = Индивидуальные.ИдентификаторОперации
		|
		|ГДЕ
		|	Очередь.Этап В (&ТекстОтборЭтапы)
		|
