...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не Объект.Этап.Пустая() Тогда
		...
		Если Выборка.Следующий() Тогда
			...
			ЗаполнитьЗначенияСвойств(Объект, Выборка,
				"Операция, ИдентификаторОперации, ПроизводственнаяОперация, ЭтоПерваяОперация, ЭтоПоследняяОперация");
		КонецЕсли;
	КонецЕсли;
	...
	|ИЗ
	|	Документ.ЭтапПроизводства2_2 КАК ЭтапПроизводства
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.ТехнологическиеПроцессыЭтаповПроизводства КАК Техпроцессы
	|		ПО ЭтапПроизводства.Ссылка = Техпроцессы.Этап
	|		И Техпроцессы.ТипТехнологическогоПроцесса = ЗНАЧЕНИЕ(Перечисление.ТипыТехнологическихПроцессовЭтапа.Индивидуальный)
	|ГДЕ
	|	ЭтапПроизводства.ПартияПроизводства = &ПартияПроизводства
	|	И ЭтапПроизводства.Проведен");
	Запрос.УстановитьПараметр("ПартияПроизводства", Объект.ПартияПроизводства);
	Элементы.ДекорацияЕстьИндивидуальныйТехпроцесс.Видимость = НЕ Запрос.Выполнить().Пустой();
	...
	ДоляСтоимостиМенееПроцентаПредставление = НСтр("ru = '<0,01%';
													|en = '<0,01%'");
...
Процедура ЭтапПредставлениеНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	Если ОперацияВыбрана(Объект) Тогда
...
Процедура ЭтапПредставлениеВыборНаСервере(КэшированныеЗначения)
	...
	Если ОперацияВыбрана(Объект) Тогда
		...
		СтрокаОперация = Дерево.Строки.Найти(Объект.ИдентификаторОперации, "ИдентификаторОперации", Истина);
...
&НаКлиенте
Процедура ДоляСтоимостиМенееПроцентаПредставлениеНажатие(Элемент, СтандартнаяОбработка)
...
Процедура НастроитьЗависимыеЭлементыФормы(Форма, СписокРеквизитов = "")
	...
	Если СтруктураРеквизитов.Свойство("СпособРаспределенияЗатратНаВыходныеИзделия")
		...
		Элементы.ДоляСтоимостиМенееПроцентаПредставление.ПоложениеЗаголовка = ?(РаспределениеПоДолям,
			ПоложениеЗаголовкаЭлементаФормы.Нет,
			ПоложениеЗаголовкаЭлементаФормы.Авто);
		Элементы.ДоляСтоимостиМенееПроцентаПредставление.Гиперссылка = НЕ РаспределениеПоДолям;
	...
	Если Элементы.Партии.ТекущаяСтрока <> Неопределено
		...
		Если СтруктураРеквизитов.Свойство("ТекущаяПартия")
			...
			Элементы.ЕстьОшибкиЗаполненияДолиСтоимости.Видимость = НЕ ЗначениеЗаполнено(ТекущиеДанные.ДоляСтоимостиПроцент)
				И НЕ ЭтоБракВКонцеПартии;
			Элементы.ДоляСтоимостиМенееПроцентаПредставление.Видимость = Не ТекущиеДанные.ЕстьОшибкиЗаполненияДолиСтоимости
				И Не ЗначениеЗаполнено(ТекущиеДанные.ДоляСтоимостиПроцент);
		КонецЕсли;
...
Функция ДанныеЭтапа(Объект, ИзделиеЕдиницаИзмерения)
	...
	Иначе
		...
		СНачалаЭтапа = Не (
			...
			Или (ОперацияВыбрана(Объект) И Не ЭтоПерваяОперация)
		);
		...
		ОперацияПредставление = ОперацияПредставление(Объект);
		
		ЭтапПредставление = ?(ОперацияВыбрана(Объект),
		...
		ЭтапСпецификацииПредставление = ?(ОперацияВыбрана(Объект),
		...
		Запрос.Текст = "
		|ВЫБРАТЬ
		|	СУММА(Очередь.Требуется)       КАК Требуется,
		...
		|	И (Очередь.Брак <> 0
		|		ИЛИ Очередь.Операция = ЗНАЧЕНИЕ(Справочник.ТехнологическиеОперации.ПустаяСсылка))
		|СГРУППИРОВАТЬ ПО
		|	ВЫБОР
		|		КОГДА Очередь.Операция = ЗНАЧЕНИЕ(Справочник.ТехнологическиеОперации.ПустаяСсылка)
		|			ТОГДА 0
		|		ИНАЧЕ Очередь.ИдентификаторОперации
		|	КОНЕЦ,
		|	Очередь.НомерОперации,
...
&НаСервереБезКонтекста
Функция ОперацияПредставление(Объект)
	
	Результат = "";
	
	Если ОперацияВыбрана(Объект) Тогда
		Источник = ?(НЕ Объект.Операция.Пустая(), Объект.Операция, Объект.ПроизводственнаяОперация);
		Результат = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Источник, "Наименование");
	КонецЕсли;
...
&НаКлиентеНаСервереБезКонтекста
Функция ОперацияВыбрана(ДанныеСтроки)
	
	Возврат ДанныеСтроки.ИдентификаторОперации <> 0;
