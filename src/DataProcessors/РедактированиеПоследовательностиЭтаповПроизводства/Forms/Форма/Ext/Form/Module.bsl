...
Процедура ЭтапыМаршрутнаяКартаПриИзменении(Элемент)
	...
	Если ТекущиеДанные.ТехнологическийПроцесс.Пустая() Тогда
		
		ТекущиеДанные.КоэффициентТехнологическогоПроцесса = 0;
		
	ИначеЕсли ТекущиеДанные.КоэффициентТехнологическогоПроцесса = 0 Тогда
		
		ТекущиеДанные.КоэффициентТехнологическогоПроцесса = 1;
...
Процедура ПроверитьПроизводственныйПроцесс(МассивНепроверяемыхРеквизитов, Отказ)
	...
	Для каждого Строка Из Строки Цикл
		...
		Если Строка.ИспользоватьПооперационноеУправление Тогда
			...
			Если Не ЗначениеЗаполнено(Строка.КоэффициентТехнологическогоПроцесса)
				И НЕ Строка.ТехнологическийПроцесс.Пустая() Тогда
				...
				Поле = ПутьКТабличнойЧасти("Этапы", Строка.НомерСтроки, "КоэффициентТехнологическогоПроцесса");
...
Функция ЗначенияЗаполненияПоЭтапуПроизводства(Строка, ДанныеОбработки)
	...
	Если Не Строка.ТолькоПросмотр Тогда
		...
		Реквизиты.Вставить("ТехнологическийПроцесс", Строка.ТехнологическийПроцесс);
		Реквизиты.Вставить("КоэффициентТехнологическогоПроцесса", Строка.КоэффициентТехнологическогоПроцесса);
	КонецЕсли;
	...
	Если Элементы.ЭтапыГруппаМаршрутнаяКарта.Видимость
		И Строка.ТипТехнологическогоПроцесса <> Перечисления.ТипыТехнологическихПроцессовЭтапа.Индивидуальный Тогда
		ПараметрыТехпроцесса = РедакторПроизводственногоПроцессаВызовСервера.СтандартныйТехпроцессЭтапа(Строка.Ссылка);
		Если Строка.ТехнологическийПроцесс <> ПараметрыТехпроцесса.ТехнологическийПроцесс
			ИЛИ Строка.КоэффициентТехнологическогоПроцесса <> ПараметрыТехпроцесса.КоэффициентТехнологическогоПроцесса Тогда
			ПараметрыТехпроцесса.ТипТехнологическогоПроцесса         = Перечисления.ТипыТехнологическихПроцессовЭтапа.Альтернативный;
			ПараметрыТехпроцесса.ТехнологическийПроцесс              = Строка.ТехнологическийПроцесс;
			ПараметрыТехпроцесса.КоэффициентТехнологическогоПроцесса = Строка.КоэффициентТехнологическогоПроцесса;
		КонецЕсли;
...
Процедура УстановитьУсловноеОформлениеЭтапы()
	...
	ОтборЭлемента = ГруппаОтбораИЛИ.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
	ОтборЭлемента.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("Объект.Этапы.ТехнологическийПроцесс");
	...
	ОтборЭлемента = Элемент.Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
	ОтборЭлемента.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("Объект.Этапы.ТипТехнологическогоПроцесса");
	ОтборЭлемента.ВидСравнения = ВидСравненияКомпоновкиДанных.Равно;
	ОтборЭлемента.ПравоеЗначение = Перечисления.ТипыТехнологическихПроцессовЭтапа.Индивидуальный;
...
Процедура НастроитьЗависимыеЭлементыФормы(Форма, СписокРеквизитов = "")
	...
	Если СтруктураРеквизитов.Свойство("ИспользоватьПооперационноеУправление")
		...
		Элементы.ЭтапыГруппаМаршрутнаяКарта.Видимость =
			...
			И Не Объект.ТипПроизводственногоПроцесса = ПредопределенноеЗначение("Перечисление.ТипыПроизводственныхПроцессов.БезСпецификаций");
...
Функция ВременныеТаблицыДанныхОбъекта(СписокЭтапов) 
	...
	|	ПО Таблица.Ссылка = Техпроцесс.Этап
	|
	|Где
	...
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса,
		"&ПараметрыТехпроцесса",
		РедакторПроизводственногоПроцесса.ТекстЗапросаПараметрыТехпроцессаЭтапа("Таблица"));
