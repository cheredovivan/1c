	...
	Если РегламентныеЗаданияСервер.РаботаСВнешнимиРесурсамиЗаблокирована() Тогда
		...
		РазрешенаРаботаССервисамиИнтернетПоддержки = 
			(Не ОбщегоНазначения.РазделениеВключено()
				И ПараметрыБлокировки.СервисыИнтернетПоддержки[НСтр("ru = 'Сервисы аутентификации';
																	|en = 'Authentication services'")].РазрешенаРабота)
			И ПараметрыБлокировки.СервисыИнтернетПоддержки[НСтр("ru = 'Подключение тестовых периодов';
																|en = 'Activating trial periods'")].РазрешенаРабота;
	КонецЕсли;
