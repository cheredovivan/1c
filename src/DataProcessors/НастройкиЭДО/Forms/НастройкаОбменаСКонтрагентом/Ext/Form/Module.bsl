...
Асинх Процедура ПриИзмененииФорматаЭД(ЭтоИзмененияПризнакаАвтоматическогоОбновленияФормата = Ложь)
	...
	СтрокаСКоличествомКонтрагентов = СтроковыеФункцииКлиентСервер.СтрокаСЧисломДляЛюбогоЯзыка(
		НСтр("ru = ';%1 контрагентом;;%1 контрагентами;%1 контрагентами;';
			|en = ';%1 counterparty;;;;%1 counterparties'"),
	...
	ТекстВопроса = СтрШаблон(НСтр("ru = 'Формат ""%1"" указан в настройках ЭДО еще с %2.
		...
		|en = 'The ""%1"" format is specified in the EDI settings since %2.
		|Do you want to set the selected format for these settings?'"),
...
Процедура ПослеИзмененияФорматаДляВсехКонтрагентов(Результат, ДополнительныеПараметры) Экспорт
	...
	ПоказатьПредупреждение(, НСтр("ru = 'Изменение формата в настройках ЭДО с контрагентами выполнено';
									|en = 'The format in the EDI settings with counterparties is changed'"));
