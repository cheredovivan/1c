...
Функция ВыполнитьОбъединениеСправочников(Параметры)
	...
	Результат.Вставить("ОбработаноЭтапов",       0);
	...
	Попытка
		...
		Если НЕ Результат.ЕстьОшибки Тогда
			ЗафиксироватьТехпроцессНачатыхЭтапов(Результат);
...
Процедура ОтключениеИспользованияМаршрутныхКартВФоне(Параметры, АдресХранилища) Экспорт
	
	Результат = ВыполнитьОтключениеИспользованияМаршрутныхКарт(Параметры);
	
	ПоместитьВоВременноеХранилище(Результат, АдресХранилища);
	
КонецПроцедуры

Функция ВыполнитьОтключениеИспользованияМаршрутныхКарт(Параметры)
	...
	Попытка
		...
		Если НЕ Константы.ХранитьОперацииВРесурсныхСпецификациях.Получить() Тогда
			ЗафиксироватьТехпроцессНачатыхЭтапов(Результат);
			Константы.ХранитьОперацииВРесурсныхСпецификациях.Установить(Истина);
		КонецЕсли;
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(
			СобытиеОтключениеИспользованияМаршрутныхКарт(),
			УровеньЖурналаРегистрации.Ошибка,,,
			ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
...
Процедура ЗафиксироватьТехпроцессНачатыхЭтапов(Счетчики)
	...
	Пока Выборка.Следующий() Цикл
		...
		РегистрыСведений.ТехнологическиеПроцессыЭтаповПроизводства.ЗаписатьПараметрыТехпроцесса(Выборка.Этап, ПараметрыТехпроцесса);
		Счетчики.ОбработаноЭтапов = Счетчики.ОбработаноЭтапов + 1;
...
Функция СобытиеСозданиеТиповыхТехнологическихПроцессов()
	...
	Возврат СтрШаблон("%1.%2",
			 НСтр("ru = 'НСИ производства';
					|en = 'Manufacturing master data'", ОбщегоНазначения.КодОсновногоЯзыка()),
			 НСтр("ru = 'Создание типовых технологических процессов.';
					|en = 'Generating standard technological processes.'", ОбщегоНазначения.КодОсновногоЯзыка()));
...
Функция СобытиеПереносОперацийВРесурсныеСпецификации()
	...
	Возврат СтрШаблон("%1.%2",
			 НСтр("ru = 'НСИ производства';
					|en = 'Manufacturing master data'", ОбщегоНазначения.КодОсновногоЯзыка()),
			 НСтр("ru = 'Перенос операций в ресурсные спецификации.';
					|en = 'Transferring operations to bills of materials.'", ОбщегоНазначения.КодОсновногоЯзыка()));
	
КонецФункции

Функция СобытиеОтключениеИспользованияМаршрутныхКарт()
	
	Возврат СтрШаблон("%1.%2",
			 НСтр("ru = 'НСИ производства';
					|en = 'Manufacturing master data'", ОбщегоНазначения.КодОсновногоЯзыка()),
			 НСтр("ru = 'Отключение использования маршрутных карт.';
					|en = 'Disable using route sheets.'", ОбщегоНазначения.КодОсновногоЯзыка()));
