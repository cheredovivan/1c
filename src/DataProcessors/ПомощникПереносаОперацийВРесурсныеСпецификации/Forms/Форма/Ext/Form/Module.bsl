...
Процедура ОтключитьИспользованиеМаршрутныхКарт(Команда)
	...
	НачатьОтключениеИспользованияМаршрутныхКарт();
...
&НаКлиенте
Процедура НачатьОтключениеИспользованияМаршрутныхКарт()
	
	Если ДлительнаяОперацияОбработкаДанных <> Неопределено Тогда
		Возврат;
	КонецЕсли;
	
	Элементы.ГруппаШаг3_Обработка.ТекущаяСтраница = Элементы.ГруппаШаг3_Обработка_Ожидание;
	
	НачатьОтключениеИспользованияМаршрутныхКартНаСервере();
	
	Если ДлительнаяОперацияОбработкаДанных <> Неопределено Тогда
		
		ОписаниеОповещения = Новый ОписаниеОповещения(
			"ОтключениеИспользованияМаршрутныхКартЗавершение",
			ЭтотОбъект);
...
&НаСервере
Процедура НачатьОтключениеИспользованияМаршрутныхКартНаСервере()
	
	ПараметрыВыполнения = ДлительныеОперации.ПараметрыВыполненияВФоне(УникальныйИдентификатор);
	ПараметрыВыполнения.ЗапуститьВФоне              = Истина;
	ПараметрыВыполнения.ОжидатьЗавершение           = 0;
	ПараметрыВыполнения.НаименованиеФоновогоЗадания = НСтр("ru = 'Отключение использования маршрутных карт.';
															|en = 'Disable using route sheets.'");
	
	ПараметрыПроцедуры = Новый Структура;
	
	ДлительнаяОперацияОбработкаДанных = ДлительныеОперации.ВыполнитьВФоне(
		"Обработки.ПомощникПереносаОперацийВРесурсныеСпецификации.ОтключениеИспользованияМаршрутныхКартВФоне",
		ПараметрыПроцедуры,
		ПараметрыВыполнения);
...
&НаКлиенте
Процедура ОтключениеИспользованияМаршрутныхКартЗавершение(Результат, ДополнительныеПараметры) Экспорт
	
	ДлительнаяОперацияОбработкаДанных = Неопределено;
	
	Если Результат <> Неопределено И Результат.Статус = "Выполнено" Тогда
		
		РезультатЗадания = ОбработатьРезультатОтключенияИспользованияМаршрутныхКарт(Результат);
	
		Если РезультатЗадания.ЕстьОшибки Тогда
					
			Сообщение = НСтр("ru = 'Не выполнено.
			|	Подробности см. в журнале регистрации';
			|en = 'Not completed.
		...
		Иначе
			...
			Сообщение = НСтр("ru = 'Выполнено успешно.';
							|en = 'Completed successfully.'");
			Картинка  = БиблиотекаКартинок.Информация32;
		...
		ПоказатьОповещениеПользователя(НСтр("ru = 'Отключение использования маршрутных карт';
											|en = 'Disable using route sheets'"),,
			Сообщение,
			Картинка);
...
&НаСервере
Функция ОбработатьРезультатОтключенияИспользованияМаршрутныхКарт(ДлительнаяОперация)
	...
	Элементы.ГруппаШаг3_Обработка.ТекущаяСтраница = Элементы.ГруппаШаг3_Обработка_Пустая;
...
Процедура НастроитьЗависимыеЭлементыФормы(Форма, СписокРеквизитов = "")
	...
	Если СтруктураРеквизитов.Свойство("ОтключениеИспользованияМаршрутныхКарт")
		...
		Элементы.ОтключитьИспользованиеМаршрутныхКарт.Доступность = ЭлементыДоступны;
