...
Процедура УдалитьОперацииНеактуальныхЭтапов(ПараметрыПланирования)
	...
	|		И ОчередьПроизводственныхОпераций.ОсновнаяЗапись
	|ГДЕ
...
Процедура ЗафиксироватьСозданныеПартииОпераций(ПараметрыПланирования, МенеджерВременныхТаблиц)
	...
	|	ВЫБОР
	|		КОГДА ВТПартииОпераций.Операция = ЗНАЧЕНИЕ(Справочник.ТехнологическиеОперации.ПустаяСсылка)
	|			ТОГДА ВТПартииОпераций.ПроизводственнаяОперация
	...
	|	ЕСТЬNULL(ВТПартииОпераций.Операция.Количество, 1) КАК Коэффициент,
	|	СУММА(ВЫБОР
	...
	|	РабочийЦентрПроизводственнойОперации
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	...
	|	втФиксированныеПартии.ПроизводственнаяОперация КАК ПроизводственнаяОперация,
	...
	|	втФиксированныеПартии.ПроизводственнаяОперация
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|	ЕСТЬNULL(ПартииОпераций.Операция.ВариантНаладки, ПартииОпераций.ПроизводственнаяОперация.ВариантНаладки) КАК ВариантНаладки,
	...
	|	ЕСТЬNULL(ПартииОпераций.Операция.ПередаточнаяПартия, ПартииОпераций.ПроизводственнаяОперация.ПередаточнаяПартия) КАК ПередаточнаяПартия,
	...
	|	ЕСТЬNULL(ПартииОпераций.Операция.Загрузка, ПартииОпераций.ПроизводственнаяОперация.Загрузка) КАК Загрузка,
	...
	|			И втФиксированныеПартии.РабочийЦентрПроизводственнойОперации = Расписание.РабочийЦентр
	|ГДЕ
	...
	ИсточникДанныхОперации = РегистрыСведений.ОчередьПроизводственныхОпераций.ТекстЗапросаИсточникДанныхОперации("ПартииОпераций");
	
	Запрос.Текст = СтрЗаменить(
		Запрос.Текст,
		"&ТекстРеквизитыИсполнителяОперации",
		Справочники.ТехнологическиеОперации.ТекстЗапросаРеквизитыИсполнителяОперации(ИсточникДанныхОперации));
		
	Запрос.Текст = СтрЗаменить(
		Запрос.Текст,
		"&ТекстРеквизитыРасчетаВремениВыполненияОперации",
		Справочники.ТехнологическиеОперации.ТекстЗапросаРеквизитыРасчетаВремениВыполненияОперации(ИсточникДанныхОперации));
...
Функция ВыгрузитьТаблицуОпераций(ПараметрыПланирования, МенеджерВременныхТаблиц)
	...
	ДополнительныеПоля.Добавить(СтрШаблон("%1.%2 КАК %2", РегистрыСведений.ОчередьПроизводственныхОпераций.ТекстЗапросаИсточникДанныхОперации(), "РабочийЦентр"));
	ДополнительныеПоля.Добавить("МАКСИМУМ(Очередь.Порядок) КАК Порядок");
	...
	ПараметрыТекстаЗапроса.Отборы.Добавить(ТекстОтбора);
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ РАЗРЕШЕННЫЕ
	|	Очередь.Наименование КАК Наименование,
	...
	|	Очередь.Загрузка КАК Загрузка,
	|	Очередь.Непрерывная КАК Непрерывная,
	|	Очередь.ВариантНаладки КАК ВариантНаладки,
	|	Очередь.ПередаточнаяПартия КАК ПередаточнаяПартия,
	|	Очередь.Запланировано - Очередь.Выполнено - Очередь.Выполняется - Очередь.Пропущено КАК Количество,
	|	Очередь.Запланировано - Очередь.Выполнено - Очередь.Выполняется - Очередь.Пропущено КАК КоличествоРазместить,
	|	ЕСТЬNULL(Очередь.Операция.Количество, 1) КАК Коэффициент,
	|	ВЫБОР
	|		КОГДА Очередь.РабочийЦентр ССЫЛКА Справочник.ВидыРабочихЦентров
	|			ТОГДА ВЫБОР
	|					КОГДА ЕСТЬNULL(ВЫРАЗИТЬ(Очередь.РабочийЦентр КАК Справочник.ВидыРабочихЦентров).ПараллельнаяЗагрузка, ЛОЖЬ)
	|						ТОГДА Очередь.ВремяШтучное + Очередь.ВремяПЗ
	|					ИНАЧЕ Очередь.ВремяШтучное * (Очередь.Запланировано - Очередь.Выполнено - Очередь.Выполняется - Очередь.Пропущено) + Очередь.ВремяПЗ
	|				КОНЕЦ
	|		ИНАЧЕ ВЫБОР
	|				КОГДА ЕСТЬNULL(ВЫРАЗИТЬ(Очередь.РабочийЦентр КАК Справочник.РабочиеЦентры).ВидРабочегоЦентра.ПараллельнаяЗагрузка, ЛОЖЬ)
	|					ТОГДА Очередь.ВремяШтучное + Очередь.ВремяПЗ
	|				ИНАЧЕ Очередь.ВремяШтучное * (Очередь.Запланировано - Очередь.Выполнено - Очередь.Выполняется - Очередь.Пропущено) + Очередь.ВремяПЗ
	|			КОНЕЦ
	|	КОНЕЦ * ВЫБОР
	...
	ИсточникДанныхОперации = РегистрыСведений.ОчередьПроизводственныхОпераций.ТекстЗапросаИсточникДанныхОперации();
	...
	ТаблицаОпераций = МассивРезультатов[МассивРезультатов.ВГраница()-1].Выгрузить();
...
Функция ТекстЗапросаОперацииДляПланирования()
	...
	|			ИЛИ Очередь.Подразделение = &Подразделение)
	|	И (НЕ &ОтборРаспоряжения
	|			ИЛИ Очередь.Распоряжение В (&Распоряжения))
	|	И (НЕ &ОтборЭтапыПроизводства
	|			ИЛИ Очередь.Этап В (&ЭтапыПроизводства))
	|	И Очередь.Этап.Статус <> ЗНАЧЕНИЕ(Перечисление.СтатусыЭтаповПроизводства2_2.Завершен)
	|	И НЕ Очередь.ВАрхиве";
	...
	|ИЗ
	|	ВТОчередь КАК ОчередьПроизводственныхОпераций
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ГрафикЭтаповПроизводства2_2 КАК График
	...
	|ГДЕ
	|	ОчередьПроизводственныхОпераций.Запланировано - ОчередьПроизводственныхОпераций.Пропущено - ОчередьПроизводственныхОпераций.Выполнено > 0
...
Процедура ЗафиксироватьОшибкуРабочиеЦентрыНеПодошлиПоЗагрузке(ПараметрыПланирования, РабочийЦентр, Операция)
	...
	Если Не ПроблемаЗафиксирована Тогда
		...
		Если ТипЗнч(РабочийЦентр) = Тип("СправочникСсылка.РабочиеЦентры") Тогда
			Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Рабочий центр ""%1"" не подходит по максимальной загрузке для операции ""%2""';
					|en = '""%1"" work center does not fit maximum load for the ""%2"" operation'"),
				РабочийЦентр, Операция.Наименование);
		Иначе
			Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Рабочие центры вида ""%1"" не подходят по максимальной загрузке для операции ""%2""';
					|en = 'Work centers of the ""%1"" type do not fit maximum load for the ""%2"" operation'"),
				РабочийЦентр, Операция.Наименование);
		КонецЕсли;
		...
		Если ЗначениеЗаполнено(Операция.Операция) Тогда
			Текст = Текст + " " + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'маршрута ""%1""';
					|en = 'of the ""%1"" route'"),
				ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Операция.Операция, "Владелец"));
		КонецЕсли;
