...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ИспользоватьУчастки = ПроизводствоСервер.ИспользоватьУчасткиПриПооперационномПланировании();
	...
	ИнтеграцияС1СДокументооборотБазоваяФункциональность.ПриСозданииНаСервере(ЭтотОбъект, Элементы.КомандыОперацииГлобальныеКоманды);
...
Процедура Подключаемый_ВыполнитьКомандуИнтеграции(Команда)
	...
	ИнтеграцияС1СДокументооборотБазоваяФункциональностьКлиент.ВыполнитьПодключаемуюКомандуИнтеграции(Команда, ЭтотОбъект, Элементы.Операции);
...
Процедура УстановитьОтборПоСостоянию()
	...
	Если НаборСостояний.ВГраница() <> -1 Тогда
		...
		Для каждого СостояниеЗначение Из НаборСостояний Цикл
			...
			ИначеЕсли СостояниеЗначение = Перечисления.СостоянияВыполненияОпераций.КВыполнению Тогда
			...
			ИначеЕсли СостояниеЗначение = Перечисления.СостоянияВыполненияОпераций.Выполнена Тогда
...
Процедура УстановитьСвойстваДинамическогоСпискаВыполнениеОпераций()
	...
	ИмяПоля = "ТехнологическийПроцессПредставление";
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&ТехнологическийПроцессПредставление",
		РедакторПроизводственногоПроцесса.ПоляЗапросаПараметрыТехпроцессаЭтапа(, "ТехпроцессПереопределяемый", ИмяПоля)[ИмяПоля]);
