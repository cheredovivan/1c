	...
	Иначе
		Если ЗначениеЗаполнено(ПериодАктуальныхДанных) Тогда
			ТекстНадписи = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Данные по взаиморасчетам актуальны на %1';
						|en = 'AR/AP accounting data is relevant as of %1'", ОбщегоНазначения.КодОсновногоЯзыка()),
				Формат(ПериодАктуальныхДанных, "ДЛФ=DT;"));
		Иначе
			ТекстНадписи = НСтр("ru = 'Данные по взаиморасчетам неактуальны';
								|en = 'AR/AP accounting data is outdated'",
