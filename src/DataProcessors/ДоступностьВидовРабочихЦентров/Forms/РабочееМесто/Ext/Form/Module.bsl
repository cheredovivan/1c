...
Функция ЗаписатьНастройкуНаСервере()
	...
	ТекстЗапроса = 
		...
		|	ТабличнаяЧасть.РабочийЦентр КАК РабочийЦентр
		|ИЗ
		|	Документ.ДоступностьРабочихЦентров.Доступность КАК ТабличнаяЧасть
		|ГДЕ
		|	ТабличнаяЧасть.Ссылка В (
		|		ВЫБРАТЬ
		|			Т.Ссылка
		|		ИЗ
		|			ВТДоступностьРЦ КАК Т)
		|	И ТабличнаяЧасть.РабочийЦентр В (
		|		ВЫБРАТЬ
		|			Т.Ссылка
		|		ИЗ
		|			Справочник.РабочиеЦентры КАК Т
		|		ГДЕ
		|			Т.ВидРабочегоЦентра В (&ВидыРабочихЦентров)
		|			И Т.ПометкаУдаления)";
	ТекстыЗапроса.Добавить(ТекстЗапроса);
	...
	ДоступностьВидовРабочихЦентров = РезультатыЗапроса[КоличествоРезультатов - 4].Выгрузить();
	ДоступностьВидовРабочихЦентров.Индексы.Добавить("ВидРабочегоЦентра, ДатаИнтервала");
	
	ДоступностьРабочихЦентров = РезультатыЗапроса[КоличествоРезультатов - 3].Выгрузить();
	ДоступностьРабочихЦентров.Индексы.Добавить("ВидРабочегоЦентра, ДатаГрафика");
	
	СменыКУдалению = РезультатыЗапроса[КоличествоРезультатов - 2].Выгрузить();
	СменыКУдалению.Индексы.Добавить("Ссылка");
	
	РабочиеЦентрыКУдалению = РезультатыЗапроса[КоличествоРезультатов - 1].Выгрузить();
	РабочиеЦентрыКУдалению.Индексы.Добавить("Ссылка");
	...
	ПоискВидДата = Новый Структура("ВидРабочегоЦентра, ДатаГрафика");
	ПоискРЦСмена = Новый Структура("РабочийЦентр, Смена");
	ПоискРЦ = Новый Структура("РабочийЦентр");
	ПоискСмена = Новый Структура("Смена");
	ПоискСсылка = Новый Структура("Ссылка");
	...
	Для каждого ДатаИВидРЦ Из РасписаниеПоДатам Цикл
		ВидРабочегоЦентра = ДатаИВидРЦ.ВидРабочегоЦентра;
		ДатаГрафика = ДатаИВидРЦ.ДатаГрафика;
		
		ПоискВидДата.ВидРабочегоЦентра = ВидРабочегоЦентра;
		ПоискВидДата.ДатаГрафика = ДатаГрафика;
		
		ДоступностьВидаРЦ = ДоступностьРабочихЦентров.НайтиСтроки(ПоискВидДата);
		РасписаниеВидаРЦ = РасписаниеГрафиковРаботыРЦ.НайтиСтроки(ПоискВидДата);
		
		Попытка
			Если ДоступностьВидаРЦ.Количество() <> 0 Тогда
				СтрокаДокумент = ДоступностьВидаРЦ[0]; // СтрокаТаблицыЗначений
				ДокументОбъект = СтрокаДокумент.Ссылка.ПолучитьОбъект();
				ДокументОбъект.Заблокировать();
				
				// Удаление старых данных
				Для каждого СтрокаРасписание Из РасписаниеВидаРЦ Цикл
					ЗаполнитьЗначенияСвойств(ПоискРЦСмена, СтрокаРасписание);
					
					Для каждого СтрокаИнтервал Из ДокументОбъект.Интервалы.НайтиСтроки(ПоискРЦСмена) Цикл
						ДокументОбъект.Интервалы.Удалить(СтрокаИнтервал);
					КонецЦикла;
					
					Для каждого СтрокаДоступность Из ДокументОбъект.Доступность.НайтиСтроки(ПоискРЦСмена) Цикл
						ДокументОбъект.Доступность.Удалить(СтрокаДоступность);
					КонецЦикла;
				КонецЦикла;
				
				ПоискСсылка.Ссылка = ДокументОбъект.Ссылка;
				Для каждого СтрокаСмена Из СменыКУдалению.НайтиСтроки(ПоискСсылка) Цикл
					ПоискСмена.Смена = СтрокаСмена.Смена;
					
					Для каждого СтрокаИнтервал Из ДокументОбъект.Интервалы.НайтиСтроки(ПоискСмена) Цикл
						ДокументОбъект.Интервалы.Удалить(СтрокаИнтервал);
					КонецЦикла;
					
					Для каждого СтрокаДоступность Из ДокументОбъект.Доступность.НайтиСтроки(ПоискСмена) Цикл
						ДокументОбъект.Доступность.Удалить(СтрокаДоступность);
					КонецЦикла;
				КонецЦикла;
				Для каждого СтрокаРабочийЦентр Из РабочиеЦентрыКУдалению.НайтиСтроки(ПоискСсылка) Цикл
					ПоискРЦ.РабочийЦентр = СтрокаРабочийЦентр.РабочийЦентр;
					
					Для каждого СтрокаИнтервал Из ДокументОбъект.Интервалы.НайтиСтроки(ПоискРЦ) Цикл
						ДокументОбъект.Интервалы.Удалить(СтрокаИнтервал);
					КонецЦикла;
					
					Для каждого СтрокаДоступность Из ДокументОбъект.Доступность.НайтиСтроки(ПоискРЦ) Цикл
						ДокументОбъект.Доступность.Удалить(СтрокаДоступность);
					КонецЦикла;
				КонецЦикла;
			Иначе
			...
			РучныеИзменения.Очистить();
			ТаблицаДоступность.Очистить();
			...
			Для каждого СтрокаДоступность Из ТаблицаДоступность Цикл
				...
				ЗаполнитьЗначенияСвойств(ПоискРЦСмена, СтрокаДоступность);
				Если РучныеИзменения.НайтиСтроки(ПоискРЦСмена).Количество() > 0 Тогда
