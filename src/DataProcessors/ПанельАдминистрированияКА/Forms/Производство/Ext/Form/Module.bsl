...
&НаКлиенте
Процедура ИспользоватьСтатусыАктовВыполненныхВнутреннихРаботПриИзменении(Элемент)
...
//++ НЕ УТКА
	...
	Если РеквизитПутьКДанным = "ИспользоватьДополнительныйРазрезТарифнойСетки" Тогда
		КонстантаИмя = "ДополнительныйРазрезТарифнойСетки"
	КонецЕсли;
	
	Если КонстантаИмя = "НормированиеИРасценкаРаботПоТарифнымСеткам" 
		И НЕ НаборКонстант[КонстантаИмя] Тогда
		ИспользоватьДополнительныйРазрезТарифнойСетки = Ложь;
	КонецЕсли;
...
Процедура НастроитьЗависимыеЭлементыФормы(РеквизитПутьКДанным = "")
	...
	Если РеквизитПутьКДанным = "" 
		...
		Элементы.ИспользоватьСтатусыАктовВыполненныхВнутреннихРабот.Видимость = ИспользуетсяПроизводство22();
...
Процедура ОтображениеПредупрежденияПриРедактировании(РеквизитПутьКДанным)
	...
	СтруктураКонстант = Новый Структура(
		...
		|ИспользоватьСтатусыАктовВыполненныхВнутреннихРабот,
