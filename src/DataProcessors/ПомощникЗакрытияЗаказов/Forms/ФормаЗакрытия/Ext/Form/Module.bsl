...
Процедура ЗаполнитьОтклоненияПоЗаказамПереработчикам()
	...
	|		ОстаткиКОформлению.Склад КАК Склад,
	|		ОстаткиКОформлению.Серия КАК Серия,
	|		СУММА(ОстаткиКОформлению.КОформлениюКоличествоОборот -
	|			ОстаткиКОформлению.ОформленоКоличествоОборот) КАК КОформлениюОстаток,
	|		0 КАК ОстатокПоОрдерам,
	|		СУММА(- ОстаткиКОформлению.КОформлениюКоличествоОборот) КАК СверхЗаказаПоОрдерам,
	|		СУММА(ОстаткиКОформлению.ЗаказаноКоличествоОборот) КАК Заказано,
	|		СУММА(ОстаткиКОформлению.ЗаказаноКоличествоОборот -
	|			ОстаткиКОформлению.КОформлениюКоличествоОборот) КАК КВыполнению
	|	ИЗ
	|		#РаспоряженияНаОтгрузкуОборотыРазвернутая1 КАК ОстаткиКОформлению
	|	СГРУППИРОВАТЬ ПО
	...
	|	ИМЕЮЩИЕ
	|		СУММА(ОстаткиКОформлению.КОформлениюКоличествоОборот -
	|			ОстаткиКОформлению.ОформленоКоличествоОборот) <> 0
	|		ИЛИ СУММА(- ОстаткиКОформлению.КОформлениюКоличествоОборот) <> 0
	|		ИЛИ СУММА(ОстаткиКОформлению.ЗаказаноКоличествоОборот) <> 0
	|		ИЛИ СУММА(ОстаткиКОформлению.ЗаказаноКоличествоОборот -
	|			ОстаткиКОформлению.КОформлениюКоличествоОборот) <> 0
	|	
	|	ОБЪЕДИНИТЬ ВСЕ
	|
	|	ВЫБРАТЬ
	...
	|	ТоварыКОтгрузке.Распоряжение    КАК Заказ,
	|	ТоварыКОтгрузке.Склад           КАК Склад,
	|	СУММА(ТоварыКОтгрузке.КОформлениюКоличествоОборот
	|		* (&ДопустимоеОтклонениеОтгрузкиПриемкиМерныхТоваров / 100)) КАК ДопустимоеОтклонение
	|ИЗ
	|	#РаспоряженияНаОтгрузкуОборотыРазвернутая2 КАК ТоварыКОтгрузке
	|СГРУППИРОВАТЬ ПО
	|	ТоварыКОтгрузке.Номенклатура,
	|	ТоварыКОтгрузке.Характеристика,
	|	ТоварыКОтгрузке.Серия,
...
Процедура ЗаполнитьОтклоненияПоЗаказамДавальца()
	...
	|	ОстаткиКОформлению.Серия                      КАК Серия, 
	|	СУММА(ОстаткиКОформлению.КОформлениюКоличествоОборот) -
	|		СУММА(ОстаткиКОформлению.ОформленоКоличествоОборот) КАК КОформлениюКонечныйОстаток,
	|	СУММА(ОстаткиКОформлению.КОтгрузкеКоличествоОборот) -
	|		СУММА(ОстаткиКОформлению.ОтгруженоКоличествоОборот) КАК КПередачеКонечныйОстаток,
	|	СУММА(ОстаткиКОформлению.КОформлениюКоличествоОборот) КАК КОформлениюПриход,
	|	СУММА(ОстаткиКОформлению.КОтгрузкеКоличествоОборот) КАК КПередачеПриход,
	|	СУММА(ОстаткиКОформлению.ЗаказаноКоличествоОборот) КАК ЗаказаноПриход
	...
	|ИМЕЮЩИЕ
	|	СУММА(ОстаткиКОформлению.КОформлениюКоличествоОборот) -
	|		СУММА(ОстаткиКОформлению.ОформленоКоличествоОборот) <> 0
	|	ИЛИ СУММА(ОстаткиКОформлению.КОтгрузкеКоличествоОборот) -
	|		СУММА(ОстаткиКОформлению.ОтгруженоКоличествоОборот) <> 0
	|	ИЛИ СУММА(ОстаткиКОформлению.КОформлениюКоличествоОборот) <> 0
	|	ИЛИ СУММА(ОстаткиКОформлению.КОтгрузкеКоличествоОборот) <> 0
	|	ИЛИ СУММА(ОстаткиКОформлению.ЗаказаноКоличествоОборот) <> 0
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
