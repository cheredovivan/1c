...
Процедура СформироватьДокументыВводаОстатковЗаказыНаПроизводство()
	...
	Для Каждого СтрокаЗаказ Из ТаблицаЗаказов Цикл
		...
		Для Каждого СтрокаТЧ Из ДокОст.Трудозатраты Цикл
			Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(ДокОст.Трудозатраты,  "Отменено") И СтрокаТЧ.Отменено Тогда
				Продолжить;
			КонецЕсли;
			КодСтрокиНайден = Ложь;
		...
		Для Каждого СтрокаТЧ Из МассивСтрокВОКУдалению Цикл
			Если ДокОст.ВозвратныеОтходыГрафик.Количество()> 0 Тогда
...
Процедура СформироватьДокументыВводаОстатковЗаказыДавальцу()
	...
	|ТабРег.Склад КАК Склад,
	|ТабРег.Серия КАК Серия,
	|(ТабРег.ЗаказаноКоличествоОборот -
	|	ТабРег.ОформленоКоличествоОборот) КАК Количество
	|ПОМЕСТИТЬ Продукция
	|ИЗ #РаспоряженияНаОтгрузкуОборотыРазвернутая КАК ТабРег
	|ЛЕВОЕ СОЕДИНЕНИЕ
	...
	|ГДЕ
	|	(ТабРег.ЗаказаноКоличествоОборот -
	|		ТабРег.ОформленоКоличествоОборот) > 0
	|;
	|////////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|ТабРег.ЗаказПоставщику КАК Заказ,
	...
	ПараметрыФормированияТаблицы.КонецПериода = "&ГраницаОст";
	ПараметрыФормированияТаблицы.ТекстУсловия =
		"ТИПЗНАЧЕНИЯ(Распоряжение) = ТИП(Документ.ЗаказДавальца)
		|		И Распоряжение.Статус <> ЗНАЧЕНИЕ(Перечисление.СтатусыЗаказовДавальцев.Закрыт)";
...
Процедура СформироватьДокументыВводаОстатковЗаказыПереработчику()
	...
	|	(ТабРег.ЗаказаноКоличествоОборот -
	|		ТабРег.ОформленоКоличествоОборот) КАК Количество,
	|	(ТабРег.ЗаказаноСуммаОборот -
	|		ТабРег.ОформленоСуммаОборот) КАК Сумма
	|ПОМЕСТИТЬ Материалы
	|ИЗ #РаспоряженияНаОтгрузкуОборотыРазвернутая КАК ТабРег
	|ГДЕ
	|	(ТабРег.ЗаказаноКоличествоОборот -
	...
	ПараметрыФормированияТаблицы.ТекстУсловия =
		...
		|		И Распоряжение.Статус <> ЗНАЧЕНИЕ(Перечисление.СтатусыЗаказовПереработчикам.Закрыт)";
	ПараметрыФормированияТаблицы.Группировка = "Распоряжение, Номенклатура, Характеристика, КодСтроки, Склад, Серия";
	ПараметрыФормированияТаблицы.Показатели = "Заказано, Оформлено";
	ПараметрыФормированияТаблицы.Ресурсы = "Количество, Сумма";
	...
	Для Каждого СтрокаЗаказ Из ТаблицаЗаказов Цикл
		...
		ОчиститьНаборыЗаписейДвиженийДокумента(МассивРегистровНакопления, Заказ);
		// Удалим движения по регистру Заказы... по этому заказу до даты свертки, чтобы не мешал контроль остатков.
		УдалитьДвиженияПоЗаказу("РаспоряженияНаОтгрузкуИВозврат", "Распоряжение", Заказ);
		УдалитьДвиженияПоЗаказу("ЗаказыПоставщикам", "ЗаказПоставщику", Заказ);
...
Процедура СформироватьДокументыВводаОстатковЗаказыПереработчику2_5()
	...
	ПараметрыФормированияТаблицы.КонецПериода = "&ГраницаОст";
	ПараметрыФормированияТаблицы.ТекстУсловия =
		"ТИПЗНАЧЕНИЯ(Распоряжение) = ТИП(Документ.ЗаказПереработчику2_5)
		|		И Распоряжение.Статус <> ЗНАЧЕНИЕ(Перечисление.СтатусыЗаказовПереработчикам.Закрыт)";
	ПараметрыФормированияТаблицы.Группировка = "Распоряжение, Номенклатура, Характеристика, КодСтроки, Склад, Серия";
	ПараметрыФормированияТаблицы.Показатели = "Заказано, Оформлено";
	ПараметрыФормированияТаблицы.Ресурсы = "Количество, Сумма";
