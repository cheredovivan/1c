...
Процедура ЗаполнитьТабличныйДокументСчетФактура(ТабличныйДокумент, ДанныеДляПечати, ОбъектыПечати, ОпцииПечатиСчетаФактуры) Экспорт
	...
	Пока ДанныеПечати.Следующий() Цикл
		...
		Иначе
			...
			ИначеЕсли ДействующиеПостановления.Постановление2026 Тогда
				ИспользоватьПечатныеФормыСФ2_5 = ПолучитьФункциональнуюОпцию("ИспользоватьПечатныеФормыСФ2_5");
				Если ИспользоватьПечатныеФормыСФ2_5 Тогда
					ОпцииПечатиСчетаФактуры.ИспользоватьПечатныеФормыСФ2_5 = ИспользоватьПечатныеФормыСФ2_5;
			...
			Если ДанныеДляПечати.Свойство("РезультатПоПоставщикам")
				...
				И (ЕстьПостановление981 Или ЕстьПостановление534 Или ЕстьПостановление1096 Или ЕстьПостановление2026) Тогда
			...
			ИначеЕсли ЕстьПостановление534 Или ЕстьПостановление1096 Или ЕстьПостановление2026 Тогда
	...
	//++ НЕ УТ
	Если ЕстьАкцизы И (ЕстьПостановление1096 Или ЕстьПостановление2026) И Не ИспользоватьПечатныеФормыСФ2_5 Тогда
	...
	Для Каждого Элемент Из СтруктураСуммПоСтроке Цикл
			...
			Если ПрослеживаемыеТовары <> Неопределено И ПрослеживаемыеТовары.Количество() > 0 Тогда 
				Для Каждого СтрокаПрослеживаемость Из ПрослеживаемыеТовары Цикл
					СтоимостьПоРНПТСтрокиКомплекта = СтоимостьПоРНПТСтрокиКомплекта(СтрокаТовары.РазницаБезНДСУвеличение, СтрокаПрослеживаемость.ОбщаяСуммаПоКомплекту,
						СтрокаПрослеживаемость.СуммаПоРНПТ, ПараметрыПечати.ЕстьПостановление2026);

					ИтоговыеСуммы["СтоимПрослежУвеличениеВсего"] = ИтоговыеСуммы["СтоимПрослежУвеличениеВсего"] + СтоимостьПоРНПТСтрокиКомплекта;
				КонецЦикла;
			Иначе
				ИтоговыеСуммы["СтоимПрослежУвеличениеВсего"] = ИтоговыеСуммы["СтоимПрослежУвеличениеВсего"] + ?(ПараметрыПечати.ЕстьПостановление2026, Окр(СтрокаТовары.РазницаБезНДСУвеличение, 2), Окр(СтрокаТовары.РазницаБезНДСУвеличение));
			КонецЕсли;
		ИначеЕсли РассчитатьИтогиСтоимостиПрослеж И Элемент.Ключ = "СтоимПрослежУменьшение" Тогда		
			Если ПрослеживаемыеТовары <> Неопределено И ПрослеживаемыеТовары.Количество() > 0 Тогда 
				...
				КонецЦикла;
			Иначе
				ИтоговыеСуммы["СтоимПрослежУменьшениеВсего"] = ИтоговыеСуммы["СтоимПрослежУменьшениеВсего"] + ?(ПараметрыПечати.ЕстьПостановление2026, Окр(СтрокаТовары.РазницаБезНДСУменьшение, 2), Окр(СтрокаТовары.РазницаБезНДСУменьшение));
			КонецЕсли;
		КонецЕсли;
	КонецЦикла;
...
Процедура ЗаполнитьРеквизитыСтрокиТовара2_5(СтрокаТовары, ПрослеживаемыеТовары, ОбластьМакета, НомерСтроки, ПараметрыПечати, ВыводитьКодНоменклатуры = Истина, ВыводитьКодТНВД = Неопределено, ВыводитьСерии = Ложь)
	...
	Если ПараметрыПечати.Корректировочный Тогда
			...
			Для Каждого СтрокаПрослеживаемость Из ПрослеживаемыеТовары Цикл
				...
				Если ПараметрыПечати.ЕстьПостановление1096 Тогда
					ИзменениеСтоимости = СтоимостьПоРНПТСтрокиКомплекта(СтрокаТовары.СуммаБезНДС, СтрокаПрослеживаемость.ОбщаяСуммаПоКомплекту,
						СтрокаПрослеживаемость.СуммаПоРНПТ, ПараметрыПечати.ЕстьПостановление2026) - СтоимостьПоРНПТСтрокиКомплекта(СтрокаТовары.СуммаБезНДСДо, СтрокаПрослеживаемость.ОбщаяСуммаПоКомплекту,
						СтрокаПрослеживаемость.СуммаПоРНПТ, ПараметрыПечати.ЕстьПостановление2026);
		...
		ИначеЕсли ПараметрыПечати.ЕстьПрослеживаемыеТовары Тогда
			...
			Если ЗначениеЗаполнено(СтрокаТовары.КоличествоПоРНПТДо) Тогда
				СтруктураПараметров.Вставить("СтоимПрослежДо",                  ?(ПараметрыПечати.ЕстьПостановление2026, Окр(СтрокаТовары.СуммаБезНДСДо, 2), Окр(СтрокаТовары.СуммаБезНДСДо)));
			Иначе
				СтруктураПараметров.Вставить("СтоимПрослежДо",                  "");
			КонецЕсли;

			Если ЗначениеЗаполнено(СтрокаТовары.КоличествоПоРНПТ) Тогда
				СтруктураПараметров.Вставить("СтоимПрослежПосле",               ?(ПараметрыПечати.ЕстьПостановление2026, Окр(СтрокаТовары.СуммаБезНДС, 2), Окр(СтрокаТовары.СуммаБезНДС)));
			Иначе	
			...
			Если ЗначениеЗаполнено(СтрокаТовары.КоличествоПоРНПТДо) ИЛИ ЗначениеЗаполнено(СтрокаТовары.КоличествоПоРНПТ) Тогда
				СтруктураПараметров.Вставить("СтоимПрослежУвеличение",          ?(ПараметрыПечати.ЕстьПостановление2026, Окр(СтрокаТовары.РазницаБезНДСУвеличение, 2), Окр(СтрокаТовары.РазницаБезНДСУвеличение)));
				СтруктураПараметров.Вставить("СтоимПрослежУменьшение",          ?(ПараметрыПечати.ЕстьПостановление2026, Окр(СтрокаТовары.РазницаБезНДСУменьшение, 2), Окр(СтрокаТовары.РазницаБезНДСУменьшение)));
				Если ПараметрыПечати.ЕстьПостановление1096 Тогда
	...
	Иначе
		...
		Если ПрослеживаемыеТовары <> Неопределено И ПрослеживаемыеТовары.Количество() > 0 Тогда
			...
			Для Каждого СтрокаПрослеживаемость Из ПрослеживаемыеТовары Цикл
				...
				Если ОбщегоНазначенияУТКлиентСервер.ЕстьРеквизитОбъекта(СтрокаПрослеживаемость, "НомерСтрокиСФ") Тогда
					...
					СтоимПрослеж = СтоимПрослеж + ?(ПараметрыПечати.ЕстьПостановление2026, Окр(СтрокаПрослеживаемость.СуммаПоРНПТ, 2), Окр(СтрокаПрослеживаемость.СуммаПоРНПТ)) + Символы.ПС;
		...
		ИначеЕсли СтрокаТовары.КоличествоПоРНПТ <> 0 Тогда
			...
			СтруктураПараметров.Вставить("СтоимПрослеж",                    ?(ПараметрыПечати.ЕстьПостановление2026, Окр(СтрокаТовары.СуммаБезНДС, 2), Окр(СтрокаТовары.СуммаБезНДС)));
		Иначе
	...
	Если Не СтруктураПараметров.ОблагаетсяАкцизом Или Не ПараметрыПечати.ЕстьПостановление1096 Тогда
...
Функция СтоимостьПоРНПТСтрокиКомплекта(Знач СуммаБезНДС, Знач ОбщаяСуммаПоКомплекту, Знач СуммаПоРНПТ, Знач ЕстьПостановление2026 = Ложь)
	...
	Стоимость = ?(ЕстьПостановление2026, Окр(Стоимость, 2), Окр(Стоимость));
