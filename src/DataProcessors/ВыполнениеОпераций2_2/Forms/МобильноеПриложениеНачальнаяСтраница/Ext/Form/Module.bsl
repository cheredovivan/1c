...
Процедура ЗаполнитьДанныеВнутриЗамераВремени()
	...
	Запрос = Новый Запрос(
		...
		|				ТОГДА 1
		|			КОГДА ЗНАЧЕНИЕ(Перечисление.СтатусыПроизводственныхОпераций.Формируется)
		|				ТОГДА 1
		|			ИНАЧЕ 0
		|		КОНЕЦ) КАК Созданные
		|ИЗ
...
Функция ЗапросНаНеназначенныеОперации()
	...
	Если ИспользуютсяСменныеЗадания Тогда
		...
		|	ПО ОперацииСЗ.Этап = Очередь.Этап
		|		И ОперацииСЗ.Операция = Очередь.Операция
		|		И ОперацииСЗ.ИдентификаторОперации = Очередь.ИдентификаторОперации
		...
		Если УчастокВНастройках Тогда
			...
			Запрос.Текст = СтрЗаменить(Запрос.Текст, "И &ТекстОтборПоУчастку",
			"	И (ЕСТЬNULL(ОперацииСЗ.Операция.РабочийЦентр, Очередь.ПроизводственнаяОперация.РабочийЦентр) = НЕОПРЕДЕЛЕНО
			|		ИЛИ ЕСТЬNULL(ОперацииСЗ.Операция.РабочийЦентр, Очередь.ПроизводственнаяОперация.РабочийЦентр) = ЗНАЧЕНИЕ(Справочник.РабочиеЦентры.ПустаяСсылка)
			|		ИЛИ ЕСТЬNULL(ОперацииСЗ.Операция.РабочийЦентр, Очередь.ПроизводственнаяОперация.РабочийЦентр) = ЗНАЧЕНИЕ(Справочник.ВидыРабочихЦентров.ПустаяСсылка)
			|		ИЛИ ЕСТЬNULL(ОперацииСЗ.Операция.РабочийЦентр, Очередь.ПроизводственнаяОперация.РабочийЦентр) ССЫЛКА Справочник.РабочиеЦентры
			|			И ЕСТЬNULL(ОперацииСЗ.Операция.РабочийЦентр.Участок, Очередь.ПроизводственнаяОперация.РабочийЦентр.Участок) = &Участок
			|		ИЛИ ЕСТЬNULL(ОперацииСЗ.Операция.РабочийЦентр, Очередь.ПроизводственнаяОперация.РабочийЦентр) ССЫЛКА Справочник.ВидыРабочихЦентров
			|			И ИСТИНА В
			|				(ВЫБРАТЬ ПЕРВЫЕ 1
			|					ИСТИНА
			|				ИЗ
			...
			|				ГДЕ
			|					Т.ВидРабочегоЦентра = ЕСТЬNULL(ОперацииСЗ.Операция.РабочийЦентр, Очередь.ПроизводственнаяОперация.РабочийЦентр)
			|					И Т.Участок = &Участок))");
		...
		Если РЦВНастройках Тогда
			...
			Запрос.Текст = СтрЗаменить(Запрос.Текст, "И &ТекстОтборПоРабочемуЦентру",
			"	И (ЕСТЬNULL(ОперацииСЗ.Операция.РабочийЦентр, Очередь.ПроизводственнаяОперация.РабочийЦентр) ССЫЛКА Справочник.ВидыРабочихЦентров
			|			И ЕСТЬNULL(ОперацииСЗ.Операция.РабочийЦентр, Очередь.ПроизводственнаяОперация.РабочийЦентр) = &ВидРЦ)
			|		ИЛИ (ЕСТЬNULL(ОперацииСЗ.Операция.РабочийЦентр, Очередь.ПроизводственнаяОперация.РабочийЦентр) ССЫЛКА Справочник.РабочиеЦентры
			|			И ЕСТЬNULL(ОперацииСЗ.Операция.РабочийЦентр, Очередь.ПроизводственнаяОперация.РабочийЦентр) = &РабочийЦентр)");
	...
	Иначе
		...
		|	И Очередь.ПроизводственнаяОперация = ЗНАЧЕНИЕ(Документ.ПроизводственнаяОперация2_2.ПустаяСсылка)
		|	И &ТекстОтборПоУчастку
		|	И &ТекстОтборПоРабочемуЦентру");
		...
		Если УчастокВНастройках Тогда
			...
			Запрос.Текст = СтрЗаменить(Запрос.Текст, "И &ТекстОтборПоУчастку",
			"	И (ЕСТЬNULL(Очередь.Операция.РабочийЦентр, Очередь.ПроизводственнаяОперация.РабочийЦентр) = НЕОПРЕДЕЛЕНО
			|		ИЛИ ЕСТЬNULL(Очередь.Операция.РабочийЦентр, Очередь.ПроизводственнаяОперация.РабочийЦентр) = ЗНАЧЕНИЕ(Справочник.РабочиеЦентры.ПустаяСсылка)
			|		ИЛИ ЕСТЬNULL(Очередь.Операция.РабочийЦентр, Очередь.ПроизводственнаяОперация.РабочийЦентр) = ЗНАЧЕНИЕ(Справочник.ВидыРабочихЦентров.ПустаяСсылка)
			|		ИЛИ ЕСТЬNULL(Очередь.Операция.РабочийЦентр, Очередь.ПроизводственнаяОперация.РабочийЦентр) ССЫЛКА Справочник.РабочиеЦентры
			|			И ЕСТЬNULL(Очередь.Операция.РабочийЦентр.Участок, Очередь.ПроизводственнаяОперация.РабочийЦентр.Участок) = &Участок
			|		ИЛИ ЕСТЬNULL(Очередь.Операция.РабочийЦентр, Очередь.ПроизводственнаяОперация.РабочийЦентр) ССЫЛКА Справочник.ВидыРабочихЦентров
			|			И ИСТИНА В
			|				(ВЫБРАТЬ ПЕРВЫЕ 1
			|					ИСТИНА
			|				ИЗ
			|					Справочник.РабочиеЦентры КАК Т
			|				ГДЕ
			|					Т.ВидРабочегоЦентра = ЕСТЬNULL(Очередь.Операция.РабочийЦентр, Очередь.ПроизводственнаяОперация.РабочийЦентр)
			|					И Т.Участок = &Участок))");
		...
		Если РЦВНастройках Тогда
			...
			Запрос.Текст = СтрЗаменить(Запрос.Текст, "И &ТекстОтборПоРабочемуЦентру",
			"	И (ЕСТЬNULL(Очередь.Операция.РабочийЦентр, Очередь.ПроизводственнаяОперация.РабочийЦентр) ССЫЛКА Справочник.ВидыРабочихЦентров
			|			И ЕСТЬNULL(Очередь.Операция.РабочийЦентр, Очередь.ПроизводственнаяОперация.РабочийЦентр) = &ВидРЦ)
			|		ИЛИ (ЕСТЬNULL(Очередь.Операция.РабочийЦентр, Очередь.ПроизводственнаяОперация.РабочийЦентр) ССЫЛКА Справочник.РабочиеЦентры
			|			И ЕСТЬNULL(Очередь.Операция.РабочийЦентр, Очередь.ПроизводственнаяОперация.РабочийЦентр) = &РабочийЦентр)");
