...
Процедура УстановитьУсловноеОформление()
	...
	Элемент.Поля.Элементы.Добавить().Поле = Новый ПолеКомпоновкиДанных(Элементы.Операция.Имя);
	
	ОтборЭлемента = Элемент.Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
	ОтборЭлемента.ЛевоеЗначение  = Новый ПолеКомпоновкиДанных("НазначениеОпераций.Операция");
	ОтборЭлемента.ВидСравнения   = ВидСравненияКомпоновкиДанных.НеЗаполнено;
	
	Элемент.Оформление.УстановитьЗначениеПараметра("Текст", Новый ПолеКомпоновкиДанных("НазначениеОпераций.Наименование"));
...
Функция ОткрытьФормуНазначенияОперации(КлючиОпераций, РежимРаботы)
	...
	Для каждого ИмяПараметра Из СтрРазделить("РабочийЦентр,Исполнитель,Участок", ",") Цикл
...
Функция ТекстЗапросаСпискаВыполнениеОпераций()
	...
	ТекстЗапроса =
		"ВЫБРАТЬ
		|	ОчередьОпераций.Распоряжение КАК Распоряжение,
		|	ОчередьОпераций.Этап КАК Этап,
		|	&ПредставлениеЭтапа КАК ПредставлениеЭтапа,
		|	ОчередьОпераций.Подразделение КАК Подразделение,
		|	ОчередьОпераций.Операция КАК Операция,
		|	ОчередьОпераций.ИдентификаторОперации КАК ИдентификаторОперации,
		|	ОчередьОпераций.Запланировано КАК ОжиданиеСоздания,
		|	ВЫБОР
		|		КОГДА
		|			ВЫБОР
		|				КОГДА ОчередьОпераций.Операция.РабочийЦентр ССЫЛКА Справочник.ВидыРабочихЦентров
		|					ТОГДА ЕСТЬNULL(ОчередьОпераций.Операция.РабочийЦентр.ПараллельнаяЗагрузка, ЛОЖЬ)
		|						ИНАЧЕ ЕСТЬNULL(ОчередьОпераций.Операция.РабочийЦентр.ВидРабочегоЦентра.ПараллельнаяЗагрузка, ЛОЖЬ)
		|			КОНЕЦ ТОГДА ОчередьОпераций.ВремяШтучное
		|		ИНАЧЕ ОчередьОпераций.ВремяШтучное * ОчередьОпераций.Запланировано / ОчередьОпераций.Операция.Количество
		|	КОНЕЦ + ОчередьОпераций.ВремяПЗ КАК ВремяВыполненияОжиданиеСоздания,
		|	ОчередьОпераций.МожноВыполнять КАК МожноВыполнять,
		|	ОчередьОпераций.Выполняется КАК Выполняется,
		|	ЕСТЬNULL(Операции.МожноПропустить, ОчередьОпераций.ПроизводственнаяОперация.МожноПропустить) КАК МожноПропустить,
		|	ЕСТЬNULL(Операции.ЕдиницаИзмерения, ОчередьОпераций.ПроизводственнаяОперация.ЕдиницаИзмерения) КАК ЕдиницаИзмерения,
		|	ВЫБОР
		|		КОГДА ЕСТЬNULL(Операции.РабочийЦентр, ОчередьОпераций.ПроизводственнаяОперация.РабочийЦентр) ССЫЛКА Справочник.ВидыРабочихЦентров
		|			ТОГДА ЕСТЬNULL(Операции.РабочийЦентр, ОчередьОпераций.ПроизводственнаяОперация.РабочийЦентр)
		|		ИНАЧЕ (ЕСТЬNULL(Операции.РабочийЦентр, ОчередьОпераций.ПроизводственнаяОперация.РабочийЦентр)).ВидРабочегоЦентра
		|	КОНЕЦ КАК ВидРабочегоЦентра,
		|	ВЫБОР
		|		КОГДА ЕСТЬNULL(Операции.РабочийЦентр, ОчередьОпераций.ПроизводственнаяОперация.РабочийЦентр) ССЫЛКА Справочник.РабочиеЦентры
		|			ТОГДА ЕСТЬNULL(Операции.РабочийЦентр, ОчередьОпераций.ПроизводственнаяОперация.РабочийЦентр)
		|		ИНАЧЕ ЗНАЧЕНИЕ(Справочник.РабочиеЦентры.ПустаяСсылка)
		|	КОНЕЦ КАК РабочийЦентр,
		|	ЕСТЬNULL(Операции.ВариантНаладки, ОчередьОпераций.ПроизводственнаяОперация.ВариантНаладки) КАК ВариантНаладки,
		|	ОчередьОпераций.ВремяОбщее КАК ВремяВыполнения,
		|	ОчередьОпераций.ВремяЕдИзм КАК ВремяЕдИзм,
		|	ОчередьОпераций.Порядок КАК Порядок,
		|	ВЫБОР
		|		КОГДА &ИспользуетсяГрафикПроизводства
		|				И НЕ График.Этап ЕСТЬ NULL
		|			ТОГДА График.Начало
		|		ИНАЧЕ ДОБАВИТЬКДАТЕ(ОчередьОпераций.Распоряжение.НачатьНеРанее, СЕКУНДА, ЕСТЬNULL(НормативныйГрафик.ДлительностьДоЗапуска, 0))
		|	КОНЕЦ КАК ДатаНачала,
		|	ОчередьОпераций.НомерОперации КАК НомерОперации,
		|	ОчередьОпераций.НомерСледующейОперации КАК НомерСледующейОперации,
		|	ЕСТЬNULL(Операции.Описание, ОчередьОпераций.ПроизводственнаяОперация.Комментарий) КАК Описание,
		|	ВЫБОР
		|		КОГДА ОчередьОпераций.МожноВыполнять > 0
		|				И ОчередьОпераций.МожноВыполнять > ОчередьОпераций.Создано - (ОчередьОпераций.Выполняется + ОчередьОпераций.Выполнено + ОчередьОпераций.Пропущено)
		|			ТОГДА ИСТИНА
		|		ИНАЧЕ ЛОЖЬ
		...
		|	ЕСТЬNULL(ОчередьОпераций.Операция.Наименование, ОчередьОпераций.ПроизводственнаяОперация.Наименование) КАК Наименование,
		|	ОчередьОпераций.Состояние КАК Состояние,
		|	ОчередьОпераций.ПроизводственнаяОперация КАК ПроизводственнаяОперация,
		|	ОчередьОпераций.ПроизводственнаяОперация.Участок КАК Участок,
		|	ОчередьОпераций.ПроизводственнаяОперация.Исполнитель КАК Исполнитель
		|ИЗ
		...
		|		ПО ОчередьОпераций.Операция = Операции.Ссылка
		|		{ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ГрафикЭтаповПроизводства2_2 КАК График
		|		ПО ОчередьОпераций.Распоряжение = График.ЗаказНаПроизводство
		|			И ОчередьОпераций.Этап = График.Этап
		...
		|		ПО ОчередьОпераций.Этап = НормативныйГрафик.ЭтапПроизводства}
		|ГДЕ
		|	НЕ ОчередьОпераций.ВАрхиве
		|	И (ОчередьОпераций.ПроизводственнаяОперация = ЗНАЧЕНИЕ(Документ.ПроизводственнаяОперация2_2.ПустаяСсылка)
		|		ИЛИ ЕСТЬNULL(ОчередьОпераций.ПроизводственнаяОперация.Статус,
		|				ЗНАЧЕНИЕ(Перечисление.СтатусыПроизводственныхОпераций.ПустаяСсылка)) = ЗНАЧЕНИЕ(Перечисление.СтатусыПроизводственныхОпераций.Формируется))
		|	И ОчередьОпераций.Запланировано > 0
		...
		|	График.НачалоСледующегоЭтапа КАК НачалоСледующегоЭтапа,
		|	НормативныйГрафик.ДлительностьДоВыпуска КАК ДлительностьДоВыпуска,
		|	ЕСТЬNULL(Очередь.Операция.Наименование, ОчередьОпераций.ПроизводственнаяОперация.Наименование) КАК Наименование,
		|	ОчередьОпераций.Состояние КАК Состояние,
		|	ОчередьОпераций.ПроизводственнаяОперация КАК ПроизводственнаяОперация,
		|	ОчередьОпераций.ПроизводственнаяОперация.Участок КАК Участок,
		|	ОчередьОпераций.ПроизводственнаяОперация.Исполнитель КАК Исполнитель
		...
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ОчередьПроизводственныхОпераций КАК ОчередьОпераций
		|		ПО Очередь.Этап = ОчередьОпераций.Этап
		|		И Очередь.Операция = ОчередьОпераций.Операция
		|		И Очередь.ИдентификаторОперации = ОчередьОпераций.ИдентификаторОперации
		|		И ОчередьОпераций.ОсновнаяЗапись
		|		{ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ГрафикЭтаповПроизводства2_2 КАК График
	...
	Если ИспользуютсяСменныеЗадания Тогда
		...
		ТекстЗапроса = СтрЗаменить(
			...
			РегистрыСведений.ОперацииКСозданиюСменныхЗаданий.ТекстЗапросаВремяОперации("ОчередьОпераций"));
	...
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "И &ТекстОтборПоУчастку",
		?(НЕ ЗначениеЗаполнено(Участок), "",
			"	И (ЕСТЬNULL(Операции.РабочийЦентр, ОчередьОпераций.ПроизводственнаяОперация.РабочийЦентр) = НЕОПРЕДЕЛЕНО
			|		ИЛИ ЕСТЬNULL(Операции.РабочийЦентр, ОчередьОпераций.ПроизводственнаяОперация.РабочийЦентр) = ЗНАЧЕНИЕ(Справочник.РабочиеЦентры.ПустаяСсылка)
			|		ИЛИ ЕСТЬNULL(Операции.РабочийЦентр, ОчередьОпераций.ПроизводственнаяОперация.РабочийЦентр) = ЗНАЧЕНИЕ(Справочник.ВидыРабочихЦентров.ПустаяСсылка)
			|		ИЛИ ЕСТЬNULL(Операции.РабочийЦентр, ОчередьОпераций.ПроизводственнаяОперация.РабочийЦентр) ССЫЛКА Справочник.РабочиеЦентры
			|			И (ЕСТЬNULL(Операции.РабочийЦентр, ОчередьОпераций.ПроизводственнаяОперация.РабочийЦентр)).Участок = &Участок
			|		ИЛИ ЕСТЬNULL(Операции.РабочийЦентр, ОчередьОпераций.ПроизводственнаяОперация.РабочийЦентр) ССЫЛКА Справочник.ВидыРабочихЦентров
			|			И ИСТИНА В
			|				(ВЫБРАТЬ ПЕРВЫЕ 1
			|					ИСТИНА
			|				ИЗ
			...
			|				ГДЕ
			|					Т.ВидРабочегоЦентра = ЕСТЬNULL(Операции.РабочийЦентр, ОчередьОпераций.ПроизводственнаяОперация.РабочийЦентр)
			|					И Т.Участок = &Участок)
			|		ИЛИ ЕСТЬNULL(ОчередьОпераций.ПроизводственнаяОперация.Участок, НЕОПРЕДЕЛЕНО) = &Участок
			|)"));
	
	Документы.ЭтапПроизводства2_2.ВыполнитьПодстановкуПоляПредставлениеЭтапа(
			ТекстЗапроса,
			"&ПредставлениеЭтапа",
			"ОчередьОпераций.Этап");
...
Функция ПроверитьПолучитьВыделенныеВСпискеОперации(ВыделенныеСтроки)
	...
	Иначе
		...
		Для каждого Строка Из ВыделенныеСтроки Цикл
			...
			Иначе
				...
				Ключ = УправлениеПроизводствомКлиентСервер.КлючПроизводственнойОперации(Истина);
				ЗаполнитьЗначенияСвойств(Ключ, Элементы.ВыполнениеОпераций.ДанныеСтроки(Строка));
	...
	Если Результат <> Неопределено Тогда
		Результат = КлючиОпераций(Результат);
	КонецЕсли;
...
&НаСервере
Функция КлючиОпераций(МассивСтруктур)
	
	Результат = Новый Массив;
	
	Для каждого ДанныеКлюча Из МассивСтруктур Цикл
		Результат.Добавить(РегистрыСведений.ОчередьПроизводственныхОпераций.СоздатьКлючЗаписи(ДанныеКлюча));
	КонецЦикла;
