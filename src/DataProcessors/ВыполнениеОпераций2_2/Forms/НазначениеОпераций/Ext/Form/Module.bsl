...
Процедура ОбработкаПроверкиЗаполненияНаСервере(Отказ, ПроверяемыеРеквизиты)
	...
	Если ЗначениеЗаполнено(ПроизводственнаяОперация) Тогда
		МассивНепроверяемыхРеквизитов.Добавить("Операция");
	КонецЕсли;
...
Процедура КоличествоПриИзменении(Элемент)
	...
	НастроитьЗависимыеЭлементыФормы(ЭтотОбъект, "Количество,ВремяВыполнения");
...
Процедура ИсполнительНачалоВыбораЗавершение(Результат, Параметры) Экспорт
	...
	Если ЗначениеЗаполнено(Результат) Тогда
		...
		ИсполнительНачалоВыбораЗавершениеНаСервере();
...
Процедура СменноеЗаданиеНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ОткрытьФорму(
		...
		Новый ОписаниеОповещения("СменноеЗаданиеНачалоВыбораЗавершение",ЭтотОбъект),
...
&НаКлиенте
Процедура ВидыРаботИсполнительНачалоВыбора(Элемент, ДанныеВыбора, ВыборДобавлением, СтандартнаяОбработка)
	
	СтандартнаяОбработка = Ложь;
	
	ОписаниеОповещения = Новый ОписаниеОповещения("ВидыРаботИсполнительНачалоВыбораЗавершение", ЭтотОбъект);
...
&НаКлиенте
Процедура ВидыРаботИсполнительАвтоПодбор(Элемент, Текст, ДанныеВыбора, ПараметрыПолученияДанных, Ожидание, СтандартнаяОбработка)
...
&НаКлиенте
Процедура ВидыРаботИсполнительОкончаниеВводаТекста(Элемент, Текст, ДанныеВыбора, ПараметрыПолученияДанных, СтандартнаяОбработка)
...
Процедура ЗаписатьИЗакрыть(Команда)
	...
	Результат = ОперативныйУчетПроизводстваКлиентСервер.ПараметрыНазначенияПроизводственнойОперацииКонструктор(МножественныйРежим);
	...
	Если Не МножественныйРежим Тогда
		...
		Если ДанныеОперации <> Неопределено Тогда
			ЗаполнитьЗначенияСвойств(
				ЭтотОбъект,
				ДанныеОперации,
				"Организация,Этап,ИдентификаторОперации,Операция,ПроизводственнаяОперация,ВидРабочегоЦентра,ЕдиницаИзмерения,ВремяШтучное,ВремяПЗ,ВремяЕдИзм,КоличествоШтучное,ПараллельнаяЗагрузка,КоэффициентВремениРаботы");
		КонецЕсли;
		...
		Если РедакторПроизводственногоПроцессаКлиентСервер.ЭтоОперацияИндивидуальная(ЭтотОбъект) Тогда
			КоличествоДоступно = КоличествоЗапланировано;
		ИначеЕсли РежимРаботы = РежимРаботыНазначить() Тогда
		...
		//++ Локализация
		Если ПолучитьФункциональнуюОпцию("НормированиеИРасценкаРаботПоТарифнымСеткам")
			И ПараметрыПодразделения.ИспользоватьПерсональныеНаряды 
			И ПроизводствоСервер.ИспользуетсяУчетТрудозатратВРазрезеСотрудников(ТекущаяДатаСеанса()) Тогда
			...
			ДанныеСпецификации = Документы.ЭтапПроизводства2_2.ПолучитьНормативыПоСпецификации(
				...
				Ложь,
				"Трудозатраты",
				Новый Структура("Операция", Операция));
			
			МассивВидыРабот = ДанныеСпецификации.Трудозатраты.ВыгрузитьКолонку("ВидРабот");
			МассивВидыРабот = ОбщегоНазначенияКлиентСервер.СвернутьМассив(МассивВидыРабот);
			
			Для Каждого ВидРабот Из МассивВидыРабот Цикл
				НоваяСтрока = ВидыРабот.Добавить();
				НоваяСтрока.ВидРабот = ВидРабот;
			КонецЦикла;
			
			Если ВидыРабот.Количество() = 1 Тогда
				ДолжностьРазряд = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ВидыРабот[0].ВидРабот, "Должность, КвалификационныйРазряд");
				Должность = ДолжностьРазряд.Должность;
				Разряд    = ДолжностьРазряд.КвалификационныйРазряд;
				ДолжностьРазрядСтрокой = СтрШаблон("%1, %2", Должность, Разряд);
				Если НЕ ЗначениеЗаполнено(Должность) И НЕ ЗначениеЗаполнено(Разряд) Тогда
					ДолжностьРазрядСтрокой = "";
				КонецЕсли;
			КонецЕсли;
...
Процедура НастроитьЭлементыФормы() 
	...
	Элементы.ГруппаВидыРабот.Видимость       = ВидыРабот.Количество() > 1;
	Элементы.ГруппаДолжностьРазряд.Видимость = ВидыРабот.Количество() = 1;
	Элементы.Исполнитель.Видимость           = НЕ ВидыРабот.Количество() > 1;
	...
	Если ОбщегоНазначения.ЭтоМобильныйКлиент() Тогда
		...
		Элементы.Переместить(Элементы.ГруппаЛево, ЭтотОбъект, Элементы.ГруппаДобавитьОперацию);
	...
	Если РедакторПроизводственногоПроцессаКлиентСервер.ЭтоОперацияИндивидуальная(ЭтотОбъект) Тогда
		...
		Элементы.ВремяВыполнения.ТолькоПросмотр     = Истина;
		Элементы.ГруппаДоступно.Видимость           = Ложь;
...
Процедура УстановитьЗаголовкиЭлементовФормы()
	...
	Иначе
		...
		Если НЕ ОбщегоНазначения.ЭтоМобильныйКлиент() Тогда
			...
			Иначе
				ТекстЗаголовок = ТекстЗаголовок + " (" + ПредставлениеЭтапа + ")";
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
...
&НаСервере
Процедура ИсполнительНачалоВыбораЗавершениеНаСервере()
	РасчитатьЗагрузку();
	//++ Локализация
	Элементы.СоответствиеКвалификации.Видимость = НЕ ОперативныйУчетПроизводстваЛокализация.СоответствиеДолжностиРазрядаИсполнителя(Исполнитель, Должность, Разряд, ТекущаяДатаСеанса());
	//-- Локализация
КонецПроцедуры

&НаКлиенте
Процедура ВидыРаботИсполнительНачалоВыбораЗавершение(Результат, Параметры) Экспорт
	
	Если ЗначениеЗаполнено(Результат) Тогда
		
		Элементы.ВидыРабот.ТекущиеДанные.Исполнитель = Результат;
