		...
		Если Модифицированность Тогда
			ОбщегоНазначенияКлиент.СообщитьПользователю(
				НСтр("ru = 'Перед редактированием идентификатора публикации требуется сохранить изменение данных.';
					|en = 'Before editing the publication ID, save the data change.'"));
			Возврат;
		КонецЕсли;
	...
	ПоказатьВопрос(
		...
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Введенное значение должно быть уникально в товарном каталоге ""%1"", иначе может привести к ошибкам публикации товара.
						...
						|en = 'The entered value must be unique in the ""%1"" goods directory. Otherwise, it may lead to item publication errors.
						|
						|Go to editing the item publication ID?'"),
			Запись.УчетнаяЗаписьМаркетплейса),
		РежимДиалогаВопрос.ДаНет,
		,
		КодВозвратаДиалога.Да,
		НСтр("ru = 'Редактирование идентификатора публикации товара';
			|en = 'Edit item publication ID'"));
...
Процедура ОбновитьОписаниеТовараИзСервиса(Команда)
	...
	ПоказатьВопрос(
		...
		НСтр("ru = 'Описание товара будет заменено данными из Ozon.
					|Продолжить?';
					|en = 'The item details will be replaced with data from Ozon.
					|Continue?'"),
		РежимДиалогаВопрос.ДаНет,
		,
		КодВозвратаДиалога.Да,
		НСтр("ru = 'Обновление описания товара из Ozon';
			|en = 'Update item details from Ozon'"));
	...
	ИначеЕсли Не ТекущиеДанные.ВедетсяУчетСертификатовНоменклатуры Тогда
		ПодсказкаПоСертификатам = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Сертификат не используется, т.к. снят признак ""Учет сертификатов номенклатуры"" у номенклатуры <%1>.';
					|en = 'The certificate is not used as the ""Records of quality certificates"" checkbox is cleared for the <%1> item.'"),
				Запись.Номенклатура);
	ИначеЕсли ТекущиеДанные.Статус = ПредопределенноеЗначение("Перечисление.СтатусыОбъектовМаркетплейсаOzon.НеИспользуется")
		...
		ОбщегоНазначенияКлиент.СообщитьПользователю(
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Не удалось изменить идентификатор публикации товара по причине: ""%1"".';
					|en = 'Cannot change the item publication ID. Reason: %1.'"),
