...
// Функция-конструктор таблицы расчета ставок по вариантам н/о. Передается для записи в регистр
// 
...
Функция НовыйРасчетСтавокПоВариантамНалогообложения() Экспорт
	...
	ТаблицаРасчета.Колонки.Добавить("СтавкиНалога",                  Новый ОписаниеТипов("Соответствие"));
	ТаблицаРасчета.Колонки.Добавить("ПротоколРасчетаСтавокНалога",   Новый ОписаниеТипов("Соответствие"));
	
	Возврат ТаблицаРасчета;
	
КонецФункции

// Записывает комплект примененных ставок в разрезе вариантов налогообложения прибыли.
//
// Параметры:
//  Движения					 - КоллекцияДвижений - коллекция, в которую будут помещены записи регистра.
//  РасчетСтавокПоВариантамНалогообложения - см. НовыйРасчетСтавокПоВариантамНалогообложения
...
//  НомерРасчета                 - Число - номер расчета.
//
Процедура ЗаписатьРасчетСтавокНалога(Движения, РасчетСтавокПоВариантамНалогообложения, ПериодРасчета, Организация, ОтчетнаяДата = '0001-01-01', НомерРасчета = 0) Экспорт
	...
	Для Каждого ДанныеРасчета Из РасчетСтавокПоВариантамНалогообложения Цикл
	
		СтавкиНалога                  = ДанныеРасчета.СтавкиНалога;
		ПротоколРасчетаСтавокНалога   = ДанныеРасчета.ПротоколРасчетаСтавокНалога;
		ВариантНалогообложенияПрибыли = ДанныеРасчета.ВариантНалогообложенияПрибыли;
		...
		Для Каждого МетаданныеВидСтавки Из Метаданные.Перечисления.ВидыСтавокОтложенногоНалога.ЗначенияПеречисления Цикл
			...
			Запись.ВариантНалогообложенияПрибыли = ВариантНалогообложенияПрибыли;
...
//  ВариантНалогообложенияПрибыли - СправочникСсылка.ВариантыНалогообложенияПрибыли -
//                                  вариант н/о, по которому применилась ставка.
...
Функция ПрочитатьСтавки(
	...
	Запрос.УстановитьПараметр("ПоВсемВариантамНО", ?(ВариантНалогообложенияПрибыли = Неопределено, Истина, Ложь));
	Запрос.Текст =
	"ВЫБРАТЬ
	...
	|	И ВЫБОР
	|		КОГДА &ПоВсемВариантамНО
	|			ТОГДА ИСТИНА
	|		ИНАЧЕ ПримененныеСтавкиОтложенногоНалога.ВариантНалогообложенияПрибыли = &ВариантНалогообложенияПрибыли
	|	КОНЕЦ
