...
Функция ПервыеУзлы(ДанныеИсточника, СтруктураЗаказаКеш)
	...
	Если ДанныеИсточника.ЭтоСборка Тогда
		...
		ТекстыЗапроса.Добавить(
			...
			|			ИСТИНА
			|		ИЗ
			|			ВТКешСтруктураЗаказа КАК СтруктураЗаказаВыпуск
			|		ГДЕ
			|			СтруктураЗаказаВыпуск.КлючПартия = СтруктураЗаказа.КлючПартия
			|			И СтруктураЗаказаВыпуск.ЭтоВыпуск)
			|
			|ИНДЕКСИРОВАТЬ ПО
			|	КлючПартия
			|;
...
Процедура ЗакешироватьСтруктуруЗаказа(СтруктураЗаказаКеш, ДанныеИсточника, ПолныйРасчет)
	...
	ТекстЗапроса = 
		...
		|			ВТКешКоличествоПартий КАК Т
		|		ГДЕ
		|			Т.КлючПартия = Предшестенники.КлючПартия)
		|
		|ИНДЕКСИРОВАТЬ ПО
		|	КлючПартия
		|;
