...
Процедура УдалитьЗаданияОбесцененияВБудущихПериодах()
	...
	Запрос.УстановитьПараметр("Операция", Справочники.ОперацииЗакрытияМесяца.ОбесценениеВНА);
...
Процедура ПодготовитьДанныеДляФормированияЗаданийПриЗаписи()
	...
	|	ИЗ
	|		РегистрСведений.СоставЕГДС КАК ТаблицаПриЗаписи
	|			ВНУТРЕННЕЕ СОЕДИНЕНИЕ СоставЕГДСПередЗаписью КАК ТаблицаПередЗаписью
	|			ПО ТаблицаПриЗаписи.ВнеоборотныйАктив = ТаблицаПередЗаписью.ВнеоборотныйАктив
	|	ГДЕ
	|			ТаблицаПриЗаписи.Организация <> ТаблицаПередЗаписью.Организация
	|			И ТаблицаПриЗаписи.Регистратор = &Регистратор
	|	) КАК Таблица
	|;
	|";
