	...
	Попытка
		...
		Для каждого Этап Из МассивЭтапов Цикл
			...
			Для Индекс = 0 По МаксимальныйИндекс Цикл
				...
				ЗаполнитьЗначенияСвойств(Набор.Добавить(), Операция);
				
				Если Индекс = МаксимальныйИндекс
					ИЛИ Операция.СменноеЗадание <> Операции[Индекс + 1].СменноеЗадание Тогда
					Набор.Отбор.СменноеЗадание.Установить(Операция.СменноеЗадание);
					...
					Если НЕ ЗначениеЗаполнено(Операция.СменноеЗадание) Тогда
						ЕстьОперацииСПустымЗаданием = Истина;
					КонецЕсли;
				КонецЕсли;
			КонецЦикла;
			
			Если НЕ ЕстьОперацииСПустымЗаданием
				И ЭтапыСПустымЗаданием.Найти(Этап) <> Неопределено Тогда
				Набор = РегистрыСведений.ОперацииКСозданиюСменныхЗаданий.СоздатьНаборЗаписей();
...
Процедура НазначитьОперации(КлючиОпераций, Параметры) Экспорт
	...
	Попытка
		...
		ПараметрыРасчетаОчереди.РассчитыватьСменныеЗадания = Ложь;
		Для каждого Ключ Из КлючиОпераций Цикл
			РегистрыСведений.ОчередьПроизводственныхОпераций.ПересчитатьОчередь(Ключ, ПараметрыРасчетаОчереди);
		КонецЦикла;
...
Процедура РегистрВДокументы(СменноеЗадание, СтатусДокументов) Экспорт
	...
	Попытка
		...
		Для каждого ДанныеЗаполнения Из ДанныеЗаполненияМассив Цикл
			...
			СоздатьПроизводственнуюОперацию(ДанныеЗаполнения);
		...
		БлокировкаДанных.Заблокировать();
		ДанныеЭтапов = Документы.ЭтапПроизводства2_2.ДанныеДляРасчетаОчередиОпераций(Этапы, Истина);
...
Процедура ДокументыВРегистр(СменноеЗадание) Экспорт
	...
	Попытка
		...
		Запрос = Новый Запрос(
			...
			|		ИЛИ Операции.ВключенаВТехпроцесс)
			|
			|СГРУППИРОВАТЬ ПО
		...
		Пока Выборка.Следующий() Цикл
			...
			КонецЕсли;
			Если Выборка.НаОснованииНСИ Тогда
				ОперацияОбъект.Удалить();
			Иначе
				ЗаполнитьЗначенияСвойств(ОперацияОбъект, ОперативныйУчетПроизводстваКлиентСервер.СтруктураИтоговКонструктор());
				ОперацияОбъект.Протокол.Очистить();
				ОперацияОбъект.Количество = Выборка.Количество;
				ОперацияОбъект.Статус = ОперативныйУчетПроизводстваКлиентСервер.НачальныйСтатусПроизводственнойОперации(ОперацияОбъект);
				ОперацияОбъект.СменноеЗадание = Документы.СменноеЗадание.ПустаяСсылка();
		...
		Для каждого Этап Из Этапы Цикл
			РегистрыСведений.ОчередьПроизводственныхОпераций.РассчитатьОчередьПоДаннымЭтапа(
				Этап, ДанныеЭтапов.Получить(Этап), ПараметрыРасчетаОчереди);
		КонецЦикла;
	...
	Попытка
		...
		Если РевизитыЗадания.Статус = Перечисления.СтатусыСменныхЗаданий.Формируется Тогда
			...
			Для каждого Операция Из Операции Цикл
				...
				Если Этапы.Найти(Операция.Этап) = Неопределено Тогда
				
					Этапы.Добавить(Операция.Этап);
			...
			Запрос = Новый Запрос(
				...
				|				И ОперацииНазначение.ИдентификаторОперации = Очередь.ИдентификаторОперации
				|				И Очередь.ОсновнаяЗапись) КАК ВложенныйЗапрос
				|
				|СГРУППИРОВАТЬ ПО
...
Функция ТекстЗапросаВремяОперации(ИмяТаблицыРегистр) Экспорт
	...
	Возврат	СтрЗаменить(
		...
		|		ИНАЧЕ Регистр.ВремяШтучное * ЕСТЬNULL(Регистр.РабочийЦентр.КоэффициентВремениРаботы, 1) * Регистр.Количество / ЕСТЬNULL(Регистр.Операция.Количество, 1)
...
Функция РассчитатьДанныеРегистра(Этап)
	...
	Для каждого Операция Из ОперацииОбработать Цикл
		...
		Для Индекс = 0 По НазначениеПоДатам.ВГраница() Цикл
			...
			ИначеЕсли НЕ РедакторПроизводственногоПроцессаКлиентСервер.ЭтоОперацияИндивидуальная(Операция) Тогда
...
Функция ТекстЗапросаОперацииДляРасчета()
	...
	ПараметрыТекстаЗапроса.РеквизитыВремениВыполненияОперации = Истина;
	...
	ТекстЗапроса = РегистрыСведений.ОчередьПроизводственныхОпераций.ТекстЗапросаОчередьОпераций(ПараметрыТекстаЗапроса) + "
		...
		|	МАКСИМУМ(Операции.Запланировано
		|		- Операции.ТребуетПовторения)    КАК КоличествоВсего,
		|	МАКСИМУМ(ОперацииПредш.Запланировано
		|		- ОперацииПредш.ТребуетПовторения)
		|		- МАКСИМУМ(ОперацииПредш.Брак)                               КАК КоличествоВсегоПредш
		|ПОМЕСТИТЬ ВТСвязиОпераций
		|ИЗ
		|	ВТОчередь КАК Операции
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТОчередь КАК ОперацииПредш
		...
		|		Очередь.Запланировано - Очередь.Создано                             КАК Количество,
		|		Очередь.ВремяШтучное                                                КАК ВремяШтучное,
		|		Очередь.ВремяПЗ                                                     КАК ВремяПЗ,
		|		Очередь.ВремяЕдИзм                                                  КАК ВремяЕдИзм,
		...
		|		Очередь.НомерОперации                                               КАК НомерОперации,
		|		Очередь.НомерСледующейОперации                                      КАК НомерСледующейОперации,
		|		Очередь.Запланировано - Очередь.ТребуетПовторения                   КАК КоличествоВсего,
		|		0                                                                   КАК Порядок
		|	ИЗ
		...
		|			ПО ОперацииКСозданиюСменныхЗаданий.ИдентификаторОперации = Очередь.ИдентификаторОперации
		|	ГДЕ
		...
		|	РегистрСведений.ОперацииКСозданиюСменныхЗаданий КАК Операции
		|		ЛЕВОЕ СОЕДИНЕНИЕ ВТОчередь КАК Очередь
		|		ПО Операции.ИдентификаторОперации = Очередь.ИдентификаторОперации
		|ГДЕ
		|	Операции.Этап = &Этап
		|	И Операции.СменноеЗадание <> ЗНАЧЕНИЕ(Документ.СменноеЗадание.ПустаяСсылка)
		|
		...
		|	0,
		|	ИСТИНА,
		|	СУММА(ПроизвОперация.Количество - ПроизвОперация.КоличествоОтменено)
		|ИЗ
		...
		|		ИЛИ ПроизвОперация.ВключенаВТехпроцесс)
		|	И НЕ ПроизвОперация.ТребуетПовторения
		|	И НЕ ПроизвОперация.Статус В (
		...
		|		КОНЕЦ,
		|		ПроизвОперация.Количество - ПроизвОперация.КоличествоОтменено
		|	ИЗ
		|		ВТСвязиОпераций КАК Связи
	...
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса,
		"&ПоляРабочийЦентр1,",
		Справочники.ТехнологическиеОперации.ТекстЗапросаРЦВРЦ("(ВЫБОР
		|	КОГДА Очередь.Операция <> ЗНАЧЕНИЕ(Справочник.ТехнологическиеОперации.ПустаяСсылка)
		|		ТОГДА Очередь.Операция
		|	ИНАЧЕ Очередь.ПроизводственнаяОперация
		|КОНЕЦ)"));
		
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса,
		"&ПоляРабочийЦентр2,",
		Справочники.ТехнологическиеОперации.ТекстЗапросаРЦВРЦ("(ВЫБОР
		|	КОГДА ОперацииКСозданиюСменныхЗаданий.Операция <> ЗНАЧЕНИЕ(Справочник.ТехнологическиеОперации.ПустаяСсылка)
		|		ТОГДА ОперацииКСозданиюСменныхЗаданий.Операция
		|	ИНАЧЕ Очередь.ПроизводственнаяОперация
		|КОНЕЦ)"));
...
Функция ТекстЗапросаКорректировкаОпераций()
	...
	ПараметрыТекстаЗапроса.ПоказателиВыполнения = Истина;
	ПараметрыТекстаЗапроса.ИмяВыходнойТаблицы = "ВТОчередь";
	ПараметрыТекстаЗапроса.Отборы.Добавить("(Очередь.Этап, Очередь.Операция) В (&ТекстОтбораОперацияЭтап)");
	...
	ТекстЗапроса = РегистрыСведений.ОчередьПроизводственныхОпераций.ТекстЗапросаОчередьОпераций(ПараметрыТекстаЗапроса) + "
		...
		|	Операции.ИдентификаторОперации                                  КАК ИдентификаторОперации,
		|	ОперацииПредш.ИдентификаторОперации                             КАК ИдентификаторОперацииПредш,
		|	ОперацииПредш.Запланировано - ОперацииПредш.ТребуетПовторения   КАК КоличествоВсего
		|ПОМЕСТИТЬ ВТСвязиОпераций
		|ИЗ
		|	РегистрСведений.ОперацииКСозданиюСменныхЗаданий КАК НазначенныеОперации
		|		ЛЕВОЕ СОЕДИНЕНИЕ ВТОчередь КАК Операции
		...
		|		ЛЕВОЕ СОЕДИНЕНИЕ ВТОчередь КАК ОперацииПредш
		...
		|	Операции.Запланировано - Операции.ТребуетПовторения   КАК КоличествоВсего
		|ИЗ
		|	РегистрСведений.ОперацииКСозданиюСменныхЗаданий КАК НазначенныеОперации
		|		ЛЕВОЕ СОЕДИНЕНИЕ ВТОчередь КАК Операции
		|		ПО НазначенныеОперации.Подразделение = Операции.Подразделение
		...
		|		ЕСТЬNULL(Операции.Количество - Операции.КоличествоОтменено, 0)
		|	ИЗ
		|		ВТСвязиОпераций КАК Связи
	...
	ТекстОтбораОперацияЭтап = "
		|	ВЫБРАТЬ
		|		НазначенныеОперации.Этап,
		|		НазначенныеОперации.Операция
		|	 ИЗ
		|		РегистрСведений.ОперацииКСозданиюСменныхЗаданий КАК НазначенныеОперации
		|	 ГДЕ
		|		НазначенныеОперации.СменноеЗадание = &СменноеЗадание";
...
Функция ДанныеЗаполненияКонструктор()
	...
	ДанныеЗаполнения.Вставить("ПроизводственнаяОперация");
...
Функция ДанныеЗаполненияОпераций(СменноеЗадание, ТаблицаОпераций = Неопределено)
	...
	Если ТаблицаОпераций = Неопределено Тогда
		...
		|	Операции.Этап.ПартияПроизводства     КАК ПартияПроизводства,
		|	ЕСТЬNULL(Операции.СменноеЗадание.Участок, ЗНАЧЕНИЕ(Справочник.ПроизводственныеУчастки.ПустаяСсылка))   КАК Участок,
		|	ЕСТЬNULL(Операции.Операция.ВариантНаладки, Очередь.ПроизводственнаяОперация.ВариантНаладки)            КАК ВариантНаладки
		|ИЗ
		|	РегистрСведений.ОперацииКСозданиюСменныхЗаданий КАК Операции
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ОчередьПроизводственныхОпераций КАК Очередь
	...
	Для каждого СтрокаТаблицы Из ТаблицаОпераций Цикл
		...
		ДанныеЗаполнения.РабочийЦентр = ?(ЗначениеЗаполнено(СтрокаТаблицы.РабочийЦентр), СтрокаТаблицы.РабочийЦентр, СтрокаТаблицы.ВидРабочегоЦентра);
		
		ЗаполнитьЗначенияСвойств(ДанныеЗаполнения, СтрокаТаблицы,
			"Распоряжение,Подразделение,Исполнитель,ВариантНаладки,Участок,ПроизводственнаяОперация,Количество,ЭтапСпецификации,ПартияПроизводства");
			
		ЗаполнитьЗначенияСвойств(ДанныеЗаполнения.КлючОперации, СтрокаТаблицы,
	...
	Если ПолучитьФункциональнуюОпцию("ХранитьОперацииВРесурсныхСпецификациях") Тогда
		...
		Для каждого ДанныеЗаполнения Из Результат Цикл
			...
			Если Строка <> Неопределено Тогда
				ДанныеСпецификации = ДанныеСпецификаций[СписокНоменклатуры.Индекс(Строка)];
				Справочники.РесурсныеСпецификации.ПрименитьОтборПоДаннымСпецификации(
								ДанныеСпецификации, Новый Структура("Этап", ДанныеЗаполнения.ЭтапСпецификации));
...
Функция СоздатьПроизводственнуюОперацию(ДанныеЗаполнения)
	
	ШаблонСообщенияСоздание  = НСтр("ru = 'При создании производственной операции ""%1"" возникли ошибки';
									|en = 'Errors occurred when creating routing operation ""%1""'");
	ШаблонИсключенияСоздание = НСтр("ru = 'Ошибка создания производственной операции';
									|en = 'An error occurred when creating routing operation'");
	ШаблонСообщенияДобавление = НСтр("ru = 'Превышено доступное количество операции ""%1""';
									|en = 'Available number of the ""%1"" operation is exceeded'");
	ШаблонИсключенияДобавление = НСтр("ru = 'Ошибка добавления производственной операции в сменное задание';
										|en = 'An error occurred when adding a routing operation to the shift task'");
	
	ОперацииЗаписать = Новый Массив;
	
	Если НЕ ЗначениеЗаполнено(ДанныеЗаполнения.КлючОперации.Операция)
		И ЗначениеЗаполнено(ДанныеЗаполнения.ПроизводственнаяОперация) Тогда
		
		ОперацияОбъектИндивидуальная = ДанныеЗаполнения.ПроизводственнаяОперация.ПолучитьОбъект(); // ДокументОбъект.ПроизводственнаяОперация2_2
		ОперативныйУчетПроизводства.ЗаблокироватьПроизводственнуюОперацию(ОперацияОбъектИндивидуальная);
		
		Если ДанныеЗаполнения.Количество = ОперацияОбъектИндивидуальная.Количество - ОперацияОбъектИндивидуальная.КоличествоОтменено Тогда
			ОперацияОбъект = ОперацияОбъектИндивидуальная;
		ИначеЕсли ДанныеЗаполнения.Количество > ОперацияОбъектИндивидуальная.Количество - ОперацияОбъектИндивидуальная.КоличествоОтменено Тогда
			ОбщегоНазначения.СообщитьПользователю(СтрШаблон(ШаблонСообщенияДобавление, ОперацияОбъектИндивидуальная.Наименование));
			ВызватьИсключение ШаблонИсключенияДобавление;
		Иначе
			Параметры = Документы.ПроизводственнаяОперация2_2.ПараметрыРазбиенияОперацииКонструктор();
			Параметры.Количество = ДанныеЗаполнения.Количество;
			Параметры.ВремяВыполнения = ОперацияОбъектИндивидуальная.ВремяВыполнения * (ДанныеЗаполнения.Количество/ОперацияОбъектИндивидуальная.Количество);
			ОперацияОбъектИндивидуальная.УстановитьРежимРазбиения();
			ОперацияОбъект = Документы.ПроизводственнаяОперация2_2.ВыделитьОперацию(ОперацияОбъектИндивидуальная, Параметры);
			ОперацииЗаписать.Добавить(ОперацияОбъектИндивидуальная);
		КонецЕсли;
		
		ЗаполнитьЗначенияСвойств(ОперацияОбъект, ДанныеЗаполнения, "СменноеЗадание,Статус,Исполнитель,Участок,РабочийЦентр");
		ОперацииЗаписать.Добавить(ОперацияОбъект);
	...
	Для каждого ОперацияОбъект Из ОперацииЗаписать Цикл
		
		Если ДанныеЗаполнения.Статус = Перечисления.СтатусыПроизводственныхОпераций.Выполнена
				И НЕ ОперацияОбъект.ПроверитьЗаполнение() Тогда
				
				ОбщегоНазначения.СообщитьПользователю(СтрШаблон(ШаблонСообщенияСоздание, ОперацияОбъект.Наименование));
				ВызватьИсключение ШаблонИсключенияСоздание;
