...
Функция ДанныеКФормированиюЗаданийНеоперативногоДопроведенияДокументов(НаборЗаписей, ВидыЗаданий,
	...
	// не требуется возвращать документ к полному переотражению в регламентированном учете при изменении сумм в валютах учета.
	// По ним в момент перехода права собственности устанавливается статус "Частично отражен в учете".
	| 
	|	ВЫБОР
	|		КОГДА ТИПЗНАЧЕНИЯ(ДанныеКОбработке.Регистратор) = ТИП(Документ.РеализацияТоваровУслуг)
	|		И ВЫРАЗИТЬ(ДанныеКОбработке.Регистратор КАК
	|			Документ.РеализацияТоваровУслуг).Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыРеализацийТоваровУслуг.Отгружено)
	|		И ВЫРАЗИТЬ(ДанныеКОбработке.Регистратор КАК
	|			Документ.РеализацияТоваровУслуг).ДатаПереходаПраваСобственности <> ДАТАВРЕМЯ(1, 1, 1)
	|			ТОГДА ЛОЖЬ
	|		КОГДА ТИПЗНАЧЕНИЯ(ДанныеКОбработке.Регистратор) = ТИП(Документ.РеализацияУслугПрочихАктивов)
	|		И ВЫРАЗИТЬ(ДанныеКОбработке.Регистратор КАК
	|			Документ.РеализацияУслугПрочихАктивов).Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыРеализацийТоваровУслуг.Отгружено)
	|		И ВЫРАЗИТЬ(ДанныеКОбработке.Регистратор КАК
	|			Документ.РеализацияУслугПрочихАктивов).ДатаПереходаПраваСобственности <> ДАТАВРЕМЯ(1, 1, 1)
	|			ТОГДА ЛОЖЬ
	|		ИНАЧЕ ИСТИНА
	|	КОНЕЦ КАК РегистрироватьКОтражениюВРегламентированномУчете, 
	//-- Локализация
	...
	Для Каждого ТекущийВидЗадания Из ВидыЗаданий Цикл
		...
		Если ТекущийВидЗадания = Справочники.ВидыНеоперативныхЗаданий.ОтражениеВРегламентированномУчете Тогда
			...
			Для Каждого ТекущиеДанные Из ДанныеКФормированиюЗаданий Цикл
				Если ТекущиеДанные.РегистрироватьКОтражениюВРегламентированномУчете 
					И ТипыРегистраторовОтражениеВРегламентированномУчете.СодержитТип(ТипЗнч(ТекущиеДанные.Документ)) Тогда
