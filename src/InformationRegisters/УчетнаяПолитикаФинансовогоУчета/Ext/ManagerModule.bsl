...
Функция ЗначенияПоУмолчанию() Экспорт
	...
	СтруктураЗначений.Вставить("ПроизводитПодакцизныйТовар", Ложь);
	//-- НЕ УТ
...
Функция ОписаниеДействующихПараметров(Организация, ДатаДействия = Неопределено, ДействующиеНастройки = Неопределено, Все = Ложь) Экспорт
	...
	Если ДействующиеНастройки.УчетГотовойПродукцииПоПлановойСтоимости Или Все Тогда
		Если ДействующиеНастройки.СписыватьОтклоненияВСтоимостиПродукцииНаСебестоимостьПродаж Или Все Тогда
			СтрокаОписанияНастроек = СтрокаОписанияНастроек
				+ СтрШаблон(СтрокаШаблонБулево,
	...
	Если ПолучитьФункциональнуюОпцию("ФормироватьРезервыПодОбесценениеЗапасов") Или Все Тогда
		Если ДействующиеНастройки.ВосстанавливатьРезервПриСписанииЗапасовНаРасходы Или Все Тогда
			СтрокаОписанияНастроек = СтрокаОписанияНастроек
				+ СтрШаблон(СтрокаШаблонБулево,
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьВнеоборотныеАктивы2_4") Тогда
		...
		Если РегламентированныйУчетВНА Или Все Тогда
		...
		Если НЕ РегламентированныйУчетВНА Или Все Тогда
			
			Если ДействующиеНастройки.ПорядокУчетаВНАРегл = Перечисления.ПорядокУчетаВНАРегл.Независимый Или Все Тогда
				...
				Если ДействующиеНастройки.ПорядокНачисленияАмортизацииОСРегл = Перечисления.ПорядокНачисленияАмортизации.СоСледующегоМесяца
					...
					И ДействующиеНастройки.ПорядокНачисленияАмортизацииАрендаРегл = Перечисления.ПорядокНачисленияАмортизации.СоСледующегоМесяца Или Все Тогда
		...
		Если ПолучитьФункциональнуюОпцию("ИспользоватьОбесценениеВНА") Тогда
			Если ДействующиеНастройки.ВариантПроводокПоОбесценениюВНА
				= Перечисления.ВариантыПроводокПоОбесценениюВНА.ДоначислятьКорректировкуОбесценения Или Все Тогда
		...
		Если ДействующиеНастройки.ПризнаватьРасходыПоИсследованиям Или Все Тогда
	...
	Если ДействующиеНастройки.ПроизводитПодакцизныйТовар Или Все Тогда
		
		СтрокаОписанияНастроек = СтрокаОписанияНастроек
			+ СтрШаблон(СтрокаШаблонБулево,
				НСтр("ru = 'Производит подакцизный товар (начисление и отражение акциза при продаже)';
					|en = 'Produces excisable goods (excise accrual and recording upon sale)'"));
	КонецЕсли;
		...
		СтрокаОписанияНастроек = СтрокаОписанияНастроек
			+ СтрШаблон(СтрокаШаблонБулево,
				НСтр("ru = 'Импортирует подакцизный товар (начисление и отражение акциза при импорте)';
					|en = 'Imports excisable goods (excise accrual and recording upon import)'"));
			КонецЕсли;
...
Процедура ОписаниеДействующихПараметровЛокализация(СтрокаОписанияНастроек, Организация, ДатаДействия, ДействующиеНастройки, Все = Ложь)
	...
	Если ДействующиеНастройки.УчетАрендыПоФСБУ25_2018 Или Все Тогда
