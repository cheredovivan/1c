...
Процедура ЗаписатьДанныеФискализацииОнлайнЗаказа(
	...
	Попытка
		...
		Если Набор.Количество() > 0
			И Не ТребуетсяГенерацияНовогоИдентификатора(Набор[0])
			И Набор[0].ИдентификаторОперации <> ДанныеОперации.ИдентификаторОперации Тогда
			
			ИнформацияОбОшибке = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Ошибка записи информации фискализации онлайн-заказа.
					|По документу %1 уже начат процесс фискализации.';
					|en = 'An error occurred when writing online order fiscalization information.
					|The fiscalization process is already started for the %1 document.'",
					ОбщегоНазначения.КодОсновногоЯзыка()),
				ДокументОперации);
			
			ОблачныеКассыСлужебный.ЗаписатьИнформациюВЖурналРегистрации(ИнформацияОбОшибке);
		...
		Запись.ИдентификаторОперации   = ДанныеОперации.ИдентификаторОперации;
