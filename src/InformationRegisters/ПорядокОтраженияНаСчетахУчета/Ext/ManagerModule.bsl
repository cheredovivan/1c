...
Функция СписокСчетовУчетаПоРазделуУчета(РазделУчета) Экспорт
	...
	Если РазделУчета = "НоменклатураСобственная" Или РазделУчета = "Номенклатура" Тогда
		...
		СчетаУчетаРегл.Добавить("АкцизыПриПродаже", НСтр("ru = 'При продаже';
														|en = 'On sale'"), ПолучитьФункциональнуюОпцию("ИспользоватьПодакцизныеТовары"));
	...
	Если РазделУчета = "НоменклатураПринятая" Или РазделУчета = "Номенклатура" Тогда
		...
		СчетаУчетаРегл.Добавить("ТоварыПереданныеНаКомиссиюВПути", НСтр("ru = 'В пути';
																		|en = 'In transit'"), ПолучитьФункциональнуюОпцию("ИспользоватьКомиссиюПриЗакупках") И ПолучитьФункциональнуюОпцию("ИспользоватьОтгрузкуБезПереходаПраваСобственности"));
	КонецЕсли;
...
Функция ЭлементыНастройкиСчетовУчетаПоРазделуУчета(РазделУчета) Экспорт
	...
	Если РазделУчета = "НоменклатураСобственная" Или РазделУчета = "Номенклатура" Тогда
		...
		СчетаУчетаРегл.Добавить("Субконто_АкцизыПриПродаже", НСтр("ru = 'Статья учета';
																	|en = 'Accounting item'"), ПолучитьФункциональнуюОпцию("ИспользоватьПодакцизныеТовары"));
...
Процедура ДополнитьТекстЗапросаОперацийДаннымиНоменклатуры(СписокРазделов, МассивТекстовЗапроса)
	...
	|		И НЕ ДвиженияРегистраСебестоимости.ТипМестаХранения В (ЗНАЧЕНИЕ(Перечисление.ТипыМестХранения.Партнер), ЗНАЧЕНИЕ(Перечисление.ТипыМестХранения.Организация), ЗНАЧЕНИЕ(Перечисление.ТипыМестХранения.ДоговорКонтрагента)) КАК ЕстьТоварыНаСкладе,
	|	ДвиженияРегистраСебестоимости.РазделУчета = ЗНАЧЕНИЕ(Перечисление.РазделыУчетаСебестоимостиТоваров.ТоварыПринятыеНаКомиссию)
	|		И ДвиженияРегистраСебестоимости.ТипМестаХранения В (ЗНАЧЕНИЕ(Перечисление.ТипыМестХранения.Партнер), ЗНАЧЕНИЕ(Перечисление.ТипыМестХранения.Организация), ЗНАЧЕНИЕ(Перечисление.ТипыМестХранения.ДоговорКонтрагента)) КАК ЕстьТоварыПереданныеНаКомиссию,
	|	ДвиженияРегистраСебестоимости.РазделУчета = ЗНАЧЕНИЕ(Перечисление.РазделыУчетаСебестоимостиТоваров.ТоварыПринятыеНаКомиссиюВПути) КАК ЕстьТоварыПереданныеНаКомиссиюВПути
	|ПОМЕСТИТЬ ТаблицаДвиженийБезГруппировки
	|ИЗ ДвиженияРегистраСебестоимости КАК ДвиженияРегистраСебестоимости
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ Константы КАК Константы
	|		ПО ИСТИНА
	...
	|	МАКСИМУМ(ВложенныйЗапрос.ЕстьТоварыПереданныеНаКомиссиюВПути) КАК ЕстьТоварыПереданныеНаКомиссиюВПути
	...
	|	ТаблицаДвижений.АналитикаУчета,
	|	ТаблицаДвижений.МестоУчета,
	|	ЗНАЧЕНИЕ(Перечисление.ВидыСчетовРеглУчета.ТоварыПереданныеНаКомиссиюВПути) КАК ВидСчета,
	|	ЛОЖЬ КАК Долгосрочный,
	|	ТаблицаДвижений.ЕстьТоварыПереданныеНаКомиссиюВПути КАК Требуется
	|ИЗ ТаблицаДвиженийНоменклатураПринятая КАК ТаблицаДвижений
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
