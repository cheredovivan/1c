...
Процедура УдалитьЗадания(Распоряжение, Этапы, НомерЗадания = Неопределено) Экспорт
	...
	Попытка
		...
		Запрос = Новый Запрос(
			...
			|					ПО (ИСТИНА)
			|			ГДЕ
			|				ГрафикЭтапа.ЗаказНаПроизводство  = Задания.Распоряжение
			|				И ГрафикЭтапа.Этап               = Задания.ЭтапПроизводства
			|				И ГрафикИнициатора.Этап          = Задания.ЭтапПроизводстваИнициатор
			|				И ГрафикЭтапа.СтатусГрафика      = &РабочийГрафик
			|				И ГрафикИнициатора.СтатусГрафика = &РабочийГрафик
			|				И ГрафикИнициатора.Окончание     <= ГрафикЭтапа.Начало
			|		)
			|
			|ОБЪЕДИНИТЬ
			|
