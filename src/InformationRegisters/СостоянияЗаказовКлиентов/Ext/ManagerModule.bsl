...
Процедура ПриОпределенииДоступныхВидовОповещенийПользователей(ОписаниеВидовОповещений) Экспорт
	...
	НовыйВидОповещений.Наименование = НСтр("ru = 'Ожидающие согласования заказы (Продажи)';
											|en = 'Orders pending approval (Sales)'", ОбщегоНазначения.КодОсновногоЯзыка());
	...
	НовыйВидОповещений.Наименование = НСтр("ru = 'Готовые к закрытию заказы (Продажи)';
											|en = 'Fulfilled orders (Sales)'", ОбщегоНазначения.КодОсновногоЯзыка());
	НовыйВидОповещений.ТипОповещения = Перечисления.ТипыОповещенийПользователей.Неоперативное;
...
Функция ТекстЗапросаДопустимыхОтклонения(СоотвествиеТипов)
	...
	Если СоотвествиеТипов["Документ.ЗаказКлиента"] <> Неопределено
		...
		//++ Устарело_Переработка24
		Или СоотвествиеТипов["Документ.ЗаказДавальца"] <> Неопределено
		//-- Устарело_Переработка24
		Или СоотвествиеТипов["Документ.ЗаказДавальца2_5"] <> Неопределено
		//-- НЕ УТКА
		Тогда
