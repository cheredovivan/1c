...
//
Процедура ОбновитьСостояние(Знач РеализацииЭкспорт, УдалениеПроведения = Ложь) Экспорт
	...
	ТипРегистратора = ?(РеализацииЭкспорт.Количество() > 0, ТипЗнч(РеализацииЭкспорт[0]), Тип("ДокументСсылка.ТаможеннаяДекларацияЭкспорт"));
	
	Если ТипРегистратора = Тип("ДокументСсылка.ТаможеннаяДекларацияЭкспорт") Тогда
		СнятьНеобходимостьНепосредственно = Ложь;
	Иначе
		ТипИзмерения = Метаданные.РегистрыСведений.ТаможенныеДекларацииЭкспортКРегистрации.Измерения.РеализацияЭкспорт.Тип;
	...
	Если СнятьНеобходимостьНепосредственно Тогда
		...
		Для Каждого СтрокаСсылка Из РеализацииЭкспорт Цикл
			Набор = РегистрыСведений.ТаможенныеДекларацииЭкспортКРегистрации.СоздатьНаборЗаписей();
			Набор.Отбор.РеализацияЭкспорт.Установить(СтрокаСсылка);
	...
	Если ТипРегистратора <> Тип("ДокументСсылка.ТаможеннаяДекларацияЭкспорт") Тогда
		Для Каждого СтрокаСсылка Из РеализацииЭкспорт Цикл
			Если ИсправлениеДокументов.ЭтоИсправительныйДокумент(СтрокаСсылка) Тогда 
				Набор = РегистрыСведений.ТаможенныеДекларацииЭкспортКРегистрации.СоздатьНаборЗаписей();
				Набор.Отбор.РеализацияЭкспорт.Установить(ОбщегоНазначения.ЗначениеРеквизитаОбъекта(СтрокаСсылка, "СторнируемыйДокумент"));
				Набор.Записать(Истина);
			КонецЕсли;
		КонецЦикла;
	КонецЕсли;
