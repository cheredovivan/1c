...
Процедура ПриЗаписи(Отказ, Замещение)
	...
	|ИЗ
	|	ТаблицаИзмененийРасчетыПоФинансовымИнструментам КАК Изменения
	|ГДЕ
	|	ТИПЗНАЧЕНИЯ(&Регистратор) В (ТИП(Документ.ПоступлениеУслугПоАренде), ТИП(Документ.ИзменениеУсловийДоговораАренды),
	|			ТИП(Документ.ПоступлениеБезналичныхДенежныхСредств))";
	
	ИзмененияНДСУП = Запрос.Выполнить().Выгрузить();
	УчетНДСУП.ОтразитьВУчетеНДСИзменениеРасчетовПоФинансовымИнструментам(ИзмененияНДСУП);
