...
Функция ТаблицаМатериалыЗаказаНаПроизводствоКОбеспечению(Заказ) Экспорт
	...
	|	(ЗаказыМатериалов.ЗаказаноКоличествоОборот -
	|		ЗаказыМатериалов.КОформлениюКоличествоОборот) КАК Количество
	|ПОМЕСТИТЬ ВтЗаказыКлиентов
	|ИЗ
	|	#РаспоряженияНаОтгрузкуОборотыРазвернутая КАК ЗаказыМатериалов
	|ГДЕ
	|	(ЗаказыМатериалов.ЗаказаноКоличествоОборот -
	|		ЗаказыМатериалов.КОформлениюКоличествоОборот) > 0
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	...
	|ИЗ
	|	РегистрНакопления.ЗаказыМатериаловСУчетомКорректировок.Обороты(,,, Распоряжение = &Заказ) КАК ЗаказыМатериалов
	|ГДЕ
	...
	|		ВЫБОР
	|			КОГДА МатериалыИУслуги.Номенклатура.ТипНоменклатуры = ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.Работа)
	|				ТОГДА МатериалыИУслуги.Этап.Подразделение
	|			ИНАЧЕ ДанныеУчета.Склад
	|		КОНЕЦ КАК Склад,
	|		ВЫБОР
	|			КОГДА ДанныеУчета.ДатаПотребности <> ДАТАВРЕМЯ(1, 1, 1)
	|				ТОГДА ДанныеУчета.ДатаПотребности
	|			ИНАЧЕ Продукция.НачатьНеРанее
	...
	|		Материалы.Склад КАК Склад,
	|		ВЫБОР
	|			КОГДА Материалы.Ссылка.НеОтгружатьЧастями
	|				ТОГДА Материалы.Ссылка.ДатаОтгрузки
	...
	ПараметрыФормированияТаблицы.ТекстУсловия =
		...
		|		ИЗ
		|			ВтЗаказыПереработчику КАК Заказы)";
	ПараметрыФормированияТаблицы.Группировка = "Распоряжение, КодСтроки, Номенклатура, Характеристика, Склад";
