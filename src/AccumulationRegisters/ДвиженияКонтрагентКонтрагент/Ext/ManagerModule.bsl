	...
	Для Каждого ТекущийВидЗадания Из ВидыЗаданий Цикл
		...
		Если ТекущийВидЗадания = Справочники.ВидыНеоперативныхЗаданий.ОтражениеВРегламентированномУчете Тогда
			...
			Для Каждого ТекущиеДанные Из ДанныеКРегистрацииЗаданий Цикл
				Если ТипыРегистраторовОтражениеВРегламентированномУчете.СодержитТип(ТипЗнч(ТекущиеДанные.Регистратор)) Тогда
					...
					НоваяСтрока.ДатаОтражения = НачалоДня(ТекущиеДанные.Период);
				КонецЕсли;
			КонецЦикла;
		...
		Если ТекущийВидЗадания = Справочники.ВидыНеоперативныхЗаданий.ОтражениеВМеждународномУчете Тогда
			...
			Для Каждого ТекущиеДанные Из ДанныеКРегистрацииЗаданий Цикл
				Если ТипыРегистраторовОтражениеДокументовВМеждународномУчете.СодержитТип(ТипЗнч(ТекущиеДанные.Регистратор)) Тогда
					НоваяСтрока = ТаблицаДокументовКОтражению.Добавить();
					НоваяСтрока.Организация = ТекущиеДанные.Организация;
					НоваяСтрока.Документ = ТекущиеДанные.Регистратор;
					НоваяСтрока.ДатаОтражения = НачалоДня(ТекущиеДанные.Период);
					НоваяСтрока.Период = НачалоДня(ТекущиеДанные.Период);
				КонецЕсли;
			КонецЦикла;
