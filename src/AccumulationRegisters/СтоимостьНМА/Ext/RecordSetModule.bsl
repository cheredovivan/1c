...
Процедура ПодготовитьДанныеДляРегистрацииЗаданийНеоперативнойОчереди()
	...
	Если ПолучитьФункциональнуюОпцию("НоваяАрхитектураВзаиморасчетов") 
		...
		|	НачальныеЗаписи.РезервПереоценкиСтоимости КАК РезервПереоценкиСтоимости
		|ПОМЕСТИТЬ ВТ_ОчередьЗаданийНачальныеЗаписи
		|ИЗ
		|	РегистрНакопления.СтоимостьНМА КАК НачальныеЗаписи
		|ГДЕ
...
Процедура СоздатьЗаданияНеоперативнойОчередиПриЗаписи()
	...
	Если ПолучитьФункциональнуюОпцию("НоваяАрхитектураВзаиморасчетов") 
		...
		|		-Записи.РезервПереоценкиСтоимости КАК РезервПереоценкиСтоимости
		|	ИЗ
		|		РегистрНакопления.СтоимостьНМА КАК Записи
		|	ГДЕ
		...
		|ИМЕЮЩИЕ
		|	СУММА(Таблица.Стоимость) <> 0 ИЛИ СУММА(Таблица.РезервПереоценкиСтоимости) <> 0
		|;
