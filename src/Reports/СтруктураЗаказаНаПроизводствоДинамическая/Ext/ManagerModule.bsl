...
Функция ТекстЗапросаЗапущеноВЭтапахВТ()
	...
	ТекстЗапросаЗапущеноВЭтапахВТ = 
		...
		|	МИНИМУМ(ВложенныйЗапрос.ЖелаемаяДата) КАК ЖелаемаяДата
		|ПОМЕСТИТЬ ВТДатыПотребностиПартийЗапуска
		|ИЗ
		|	(
		|		ВЫБРАТЬ
		|			Выпуск.Этап            КАК Этап,
		|			МИНИМУМ(График.Начало) КАК ЖелаемаяДата
		|		ИЗ
		...
		|			ПО Потребление.ЗаказНаПроизводство = График.ЗаказНаПроизводство
		|				И График.СтатусГрафика = &СтатусРабочийГрафик
		|				И Потребление.КлючПартия = График.КлючПартия
		|				И Потребление.Этап = График.Этап
		|		ГДЕ
		|			Выпуск.Номенклатура = &ЭтапНоменклатура
		|			И Выпуск.Характеристика = &ЭтапХарактеристика
		|			И Выпуск.Склад = &ЭтапСклад
		|			И Выпуск.Назначение = &ЭтапНазначение
		|			И Выпуск.ЗаказНаПроизводство = &Заказ
		|			И Выпуск.Этап В (&Этапы)
		|
		|		СГРУППИРОВАТЬ ПО
		|			Выпуск.Этап
		|		
		|		ИМЕЮЩИЕ
		|			СУММА(Выпуск.РаспределеноИзПартий + Выпуск.РаспределеноИзПартийОбособленно) > 0
		|		
		|		ОБЪЕДИНИТЬ ВСЕ
		|		
		|		ВЫБРАТЬ
		|			СтруктураЗаказа.ЗаказНаПоступление КАК Этап,
		|			МИНИМУМ(График.Начало)             КАК ЖелаемаяДата
		|		ИЗ
		|			РегистрСведений.СтруктураЗаказа КАК СтруктураЗаказа
		|			
		...
		|			И СтруктураЗаказа.ЗаказНаПоступление В (&Этапы)
		|		
		|		СГРУППИРОВАТЬ ПО
		|			СтруктураЗаказа.ЗаказНаПоступление
		|		
		|		ИМЕЮЩИЕ
		|			СУММА(СтруктураЗаказа.РаспределеноИзЗапасов + СтруктураЗаказа.РаспределеноИзЗапасовОбособленно) > 0
		|	) КАК ВложенныйЗапрос
		|
		|СГРУППИРОВАТЬ ПО
		...
		|	СтруктураЗаказа.КлючПартия            КАК КлючПартия,
		|	СтруктураЗаказа.КлючНоменклатура      КАК КлючНоменклатура,
		|	ВЫРАЗИТЬ(СтруктураЗаказа.Этап КАК Документ.ЭтапПроизводства2_2).Спецификация       КАК Спецификация,
		|	ВЫРАЗИТЬ(СтруктураЗаказа.Этап КАК Документ.ЭтапПроизводства2_2).ПартияПроизводства КАК ПартияПроизводства,
		|	СтруктураЗаказа.ЗаказНаПроизводство   КАК ЗаказНаПроизводство,
		|	ЕСТЬNULL(ДатыПотребности.ЖелаемаяДата, ДАТАВРЕМЯ(1,1,1))    КАК ЖелаемаяДата,
		|	ЗНАЧЕНИЕ(Перечисление.ТипыПроизводственныхПроцессов.Сборка) КАК ТипПроцесса,
		|	0                                     КАК НомерСтроки,
		|	СУММА(СтруктураЗаказа.Запущено)       КАК Требуется,
		|	0                                     КАК Запланировано,
		...
		|	ПО СтруктураЗаказа.Этап = ДатыПотребности.Этап
		|ГДЕ
		|	СтруктураЗаказа.Номенклатура = &ЭтапНоменклатура
		|	И СтруктураЗаказа.Характеристика = &ЭтапХарактеристика
		|	И СтруктураЗаказа.Склад = &ЭтапСклад
		|	И СтруктураЗаказа.Назначение = &ЭтапНазначение
		|	И СтруктураЗаказа.ЗаказНаПроизводство = &Заказ
		|	И СтруктураЗаказа.Этап В (&Этапы)
		|
		|СГРУППИРОВАТЬ ПО
		|	СтруктураЗаказа.Номенклатура,
		|	СтруктураЗаказа.Характеристика,
		...
		|	СтруктураЗаказа.КлючПартия,
		|	СтруктураЗаказа.КлючНоменклатура,
		|	ВЫРАЗИТЬ(СтруктураЗаказа.Этап КАК Документ.ЭтапПроизводства2_2).Спецификация,
		|	ВЫРАЗИТЬ(СтруктураЗаказа.Этап КАК Документ.ЭтапПроизводства2_2).ПартияПроизводства,
		|	СтруктураЗаказа.ЗаказНаПроизводство,
		|	ЕСТЬNULL(ДатыПотребности.ЖелаемаяДата, ДАТАВРЕМЯ(1,1,1))
		|
		|ИМЕЮЩИЕ
		|	СУММА(СтруктураЗаказа.Запущено) <> 0
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		...
		|	СУММА(Обеспечение.Запущено)               КАК Запланировано
		|ПОМЕСТИТЬ ВТРазмерПартийЗаказ
		|ИЗ
		|	ВТЗаказано КАК Обеспечение
		|ГДЕ
...
Функция ТекстЗапросаЭтоПолуфабрикат(ИмяВнешнейТаблицы = "СтруктураЗаказа") Экспорт
