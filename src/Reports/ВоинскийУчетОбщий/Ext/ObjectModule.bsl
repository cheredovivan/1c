...
Процедура ВывестиМакетРаспискаПриПриемеДокументов(ДокументРезультат, ДанныеОтчета, ДатаОтчета)
	...
	ДокументыВоинскогоУчета = ВоинскийУчет.ВидыДокументовВоинскогоУчета();
	...
	Для Каждого ТекСтрока Из ДанныеОтчета Цикл
		...
		Для Каждого ДанныеДокумента Из ДанныеДокументовРаботника Цикл 
			Если ДанныеДокумента.ВидДокумента = Справочники.ВидыДокументовФизическихЛиц.ВоенныйБилет
				...
				Расписка.Параметры.ВидДокумента = ?(ДанныеДокумента.ВидДокумента = Справочники.ВидыДокументовФизическихЛиц.ВоенныйБилет,
...
Процедура ВывестиМакетКарточкаГражданинаПодлежащегоВоинскомуУчету(ДокументРезультат, РезультатКомпоновки, ДатаОтчета, НастройкиОтчета)
	...
	СписокСотрудников = ОбщегоНазначения.ВыгрузитьКолонку(ДанныеОтчета.Строки, "Сотрудник");
	ДанныеРабочихМест = РабочиеМестаСотрудников(СписокСотрудников, ДатаОтчета, НастройкиПечатныхФорм);
...
Функция РабочиеМестаСотрудников(СписокСотрудников, ДатаОтчета, НастройкиПечатныхФорм)
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыРасширеннаяПодсистемы.РасчетЗарплатыРасширенная.РазрядыКатегорииДолжностей") Тогда 
		ОписательВременныхТаблиц = КадровыйУчет.ОписательВременныхТаблицДляСоздатьВТКадровыеДанныеСотрудников(Запрос.МенеджерВременныхТаблиц, "ВТРабочиеМестаСотрудников");
		КадровыйУчет.СоздатьВТКадровыеДанныеСотрудников(ОписательВременныхТаблиц, Истина, "РазрядКатегория");
		Запрос.Текст =
			"ВЫБРАТЬ РАЗРЕШЕННЫЕ
			...
			|	КадровыеДанные.РазрядКатегория КАК Разряд,
			|	ВЫБОР
			|		КОГДА КадровыеДанные.РазрядКатегория ЕСТЬ NULL
			|			ТОГДА """"
			|		ИНАЧЕ КадровыеДанные.РазрядКатегория.НаименованиеПолное
			|	КОНЕЦ КАК РазрядНаименованиеПолное
			|ПОМЕСТИТЬ ВТКадровыеСобытия
			|ИЗ
			|	ВТРабочиеМестаСотрудников КАК РабочиеМестаСотрудников
			|		ЛЕВОЕ СОЕДИНЕНИЕ ВТКадровыеДанныеСотрудников КАК КадровыеДанные
			|		ПО РабочиеМестаСотрудников.Сотрудник = КадровыеДанные.Сотрудник
			|			И РабочиеМестаСотрудников.Период = КадровыеДанные.Период";
		Запрос.Выполнить();
	Иначе 
		Запрос.Текст =
			"ВЫБРАТЬ РАЗРЕШЕННЫЕ
	...
	Запрос.Текст =
		...
		|	КадровыеСобытия.ДокументОснование КАК Регистратор,
		|	КадровыеСобытия.ДокументОснование.Номер КАК РегистраторНомер,
		|	КадровыеСобытия.ДокументОснование.НомерПриказа КАК РегистраторНомерПриказа,
		|	КадровыеСобытия.ДокументОснование.Дата КАК РегистраторДата
		|ИЗ
	...
	Пока Выборка.СледующийПоЗначениюПоля("Сотрудник") Цикл
		...
		Пока Выборка.Следующий() Цикл
			...
			Если НастройкиПечатныхФорм.ВыводитьПолнуюИерархиюПодразделений И ЗначениеЗаполнено(Выборка.Подразделение) Тогда
				СтруктураСтроки.Вставить("ПодразделениеПеревода", Выборка.Подразделение.ПолноеНаименование());
			Иначе
			...
			СтруктураСтроки.Вставить("ОснованиеПеревода", НСтр("ru = 'Пр';
					...
					+ НомерПриказа + " " + НСтр("ru = 'от';
												|en = 'dated'") + " " + Формат(ДатаПриказа, "ДЛФ=D"));
