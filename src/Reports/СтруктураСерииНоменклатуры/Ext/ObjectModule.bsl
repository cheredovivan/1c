...
Функция ИспользованныеСерии(СписокНоменклатуры, ОтборПоХарактеристике)
	...
	|	ИЗ
	|		Документ.РаспределениеПроизводственныхЗатрат.ВыпускиБезРаспоряжения КАК ТаблицаТовары
	|	ГДЕ
	...
	|		ТаблицаТовары.Характеристика    КАК Характеристика,
	|		ТаблицаТовары.Серия             КАК Серия,
	|		ТаблицаТовары.Количество        КАК Израсходовано
	|	ИЗ
	...
	|		И НЕ СпрПартииПроизводства.ПометкаУдаления
	|		
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ПоступлениеСерийПриПроизводствеБезЗаказа КАК ПоступлениеСерий
	|		ПО ПоступлениеСерий.ПартияПроизводства = СпрПартииПроизводства.Ссылка
	|		
	|	ГДЕ
	|		ТИПЗНАЧЕНИЯ(ТаблицаТовары.Склад) = ТИП(Справочник.Склады)
	...
	|		ПоступлениеСерий.Характеристика           КАК ХарактеристикаСписка,
	|		ПоступлениеСерий.Серия                    КАК СерияСписка,
	|		ИспользованныеСерии.Ссылка.Номенклатура   КАК Номенклатура,
	|		ИспользованныеСерии.Ссылка.Характеристика КАК Характеристика,
	|		ИспользованныеСерии.Ссылка.Серия          КАК Серия,
	|		ИспользованныеСерии.Количество            КАК Израсходовано
	|	ИЗ
	|		Документ.РаспределениеПроизводственныхЗатрат.ПартииПроизводства КАК ИспользованныеСерии
	|		
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ПоступлениеСерийПриПроизводствеБезЗаказа КАК ПоступлениеСерий
	|		ПО ПоступлениеСерий.ПартияПроизводства = ИспользованныеСерии.ПартияПроизводства
	|		
	|	ГДЕ
	|		НЕ ИспользованныеСерии.Ссылка.Серия = ЗНАЧЕНИЕ(Справочник.СерииНоменклатуры.ПустаяСсылка)
	|	
	|	ОБЪЕДИНИТЬ ВСЕ
	|	
	...
	|		ИспользованныеСерии.Количество     КАК Израсходовано
	|	ИЗ
	|		Документ.РаспределениеПроизводственныхЗатрат.Серии КАК ИспользованныеСерии
	|		
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.РаспределениеПроизводственныхЗатрат.ПартииПроизводства КАК ПартииРаспределения
	|		ПО ПартииРаспределения.Ссылка = ИспользованныеСерии.Ссылка
	|		
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ПоступлениеСерийПриПроизводствеБезЗаказа КАК ПоступлениеСерий
	|		ПО ПоступлениеСерий.ПартияПроизводства = ПартииРаспределения.ПартияПроизводства
	|		
	|	ГДЕ
	|		НЕ ИспользованныеСерии.Ссылка.Серия = ЗНАЧЕНИЕ(Справочник.СерииНоменклатуры.ПустаяСсылка)
	|	
	...
	|		И НЕ СпрПартииПроизводства.ПометкаУдаления
	|		
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ПоступлениеСерийПриПроизводствеНаСтороне КАК ПоступлениеСерий
	|		ПО ПоступлениеСерий.ПартияПроизводства = СпрПартииПроизводства.Ссылка
	|
	|	ГДЕ
	...
	|	ОБЪЕДИНИТЬ ВСЕ
	|	
	// Производство на стороне по этапам
	|	ВЫБРАТЬ
	|		ПоступлениеСерий.Номенклатура   КАК НоменклатураСписка,
	|		ПоступлениеСерий.Характеристика КАК ХарактеристикаСписка,
	|		ПоступлениеСерий.Серия          КАК СерияСписка,
	...
	|		
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ПоступлениеСерийПриПроизводствеНаСтороне КАК ПоступлениеСерий
	|		ПО ПоступлениеСерий.ПартияПроизводства = ДанныеЭтапа.ПартияПроизводства
	|
	|	ГДЕ
	|		ТаблицаТовары.Серия <> ЗНАЧЕНИЕ(Справочник.СерииНоменклатуры.ПустаяСсылка)
	//-- НЕ УТКА
