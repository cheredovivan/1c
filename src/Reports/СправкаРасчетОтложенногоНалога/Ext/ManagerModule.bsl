...
Функция НастроитьКолонкиОтчета(КомпоновщикНастроек, СтавкиНалога, НачалоПериода, КонецПериода, Организация, ВыводитьСтавкиНалога)
...
Процедура ВывестиПорядокРасчета(ТабличныйДокумент, ВыводитьРасчетВременныхРазниц, СтавкиНалога, ВыводитьСтавкиНалога, КонецПериода, Организация)
	...
	Если ИмяШаблонаПорядокРасчета = "РазныеСтавкиНалога" Тогда
		...
		Если ВыводитьСтавкиНалога Тогда
		...
		Иначе
			ПараметрыТекста.Вставить("СтавкаНалогаБудущийНалог", НСтр("ru = 'Ставка налога';
																		|en = 'Tax rate'") + " ");
			ПараметрыТекста.Вставить("СтавкаНалогаТекущийНалог", НСтр("ru = 'Ставка налога';
																		|en = 'Tax rate'") + " ");
		КонецЕсли;
		
	ИначеЕсли ТипЗнч(СтавкиНалога) = Тип("Число") И ВыводитьСтавкиНалога Тогда
