...
Процедура ПолучитьНаборДанныхНачислениеРеализацияСложныйУчетНДС(Запрос, СтруктураВнешнихНаборов) 
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ОтгрузкаТовары.Ссылка КАК СчетФактура,
	|	ВЫБОР
	|		КОГДА ТоварыЗаказаКлиента.СтавкаНДС ЕСТЬ NULL
	|			ТОГДА &СтавкаНДСОтгрузкаТоваров
	|		ИНАЧЕ
	|			ТоварыЗаказаКлиента.СтавкаНДС
	|	КОНЕЦ КАК СтавкаНДС,
	|	СУММА(ВЫБОР
	|		КОГДА ЕСТЬNULL(ТоварыЗаказаКлиента.Количество, 0) = 0
	|			ТОГДА ЕСТЬNULL(СуммыОтгрузки.СуммаБезНДСРегл, 0)
	|		ИНАЧЕ
	|			ЕСТЬNULL(СуммыЗаказа.СуммаБезНДСРегл, 0) * ОтгрузкаТовары.Количество / ТоварыЗаказаКлиента.Количество
	|	КОНЕЦ) КАК Сумма,
	|	СУММА(ВЫБОР
	|		КОГДА ЕСТЬNULL(ТоварыЗаказаКлиента.Количество, 0) = 0
	|			ТОГДА 0
	|		ИНАЧЕ
	|			ЕСТЬNULL(СуммыЗаказа.СуммаНДСРегл, 0) * ОтгрузкаТовары.Количество / ТоварыЗаказаКлиента.Количество
	|	КОНЕЦ) КАК СуммаНДС
	|ПОМЕСТИТЬ ВТОтгрузкиБезСчетаФактуры
	|ИЗ
	|	Документ.ОтгрузкаТоваровКлиенту.ВидыЗапасов КАК ОтгрузкаТовары
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.ЗаказКлиента.Товары КАК ТоварыЗаказаКлиента
	|			ПО ОтгрузкаТовары.ЗаказКлиента = ТоварыЗаказаКлиента.Ссылка
	|				И ОтгрузкаТовары.КодСтроки = ТоварыЗаказаКлиента.КодСтроки
	|				И НЕ ОтгрузкаТовары.ЗаказКлиента.ЭтоЗаказКакСчет
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.ТребуетсяОформлениеСчетаФактуры КАК ТребуетсяСчетФактура
	|			ПО (ОтгрузкаТовары.Ссылка = ТребуетсяСчетФактура.Основание)
	...
	|			ПО СтавкиНДСНоменклатуры.Номенклатура = ОтгрузкаТовары.АналитикаУчетаНоменклатуры.Номенклатура
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ОсновныеСтавкиНДС.СрезПоследних(
	|												&НачалоПериода,
	|												Страна = &СтранаРегистрации
	|											) КАК ОсновныеСтавкиНДС
	|			ПО ИСТИНА
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.СуммыДокументовВВалютахУчета КАК СуммыОтгрузки
	|			ПО ОтгрузкаТовары.Ссылка = СуммыОтгрузки.Регистратор
	|				И ОтгрузкаТовары.ИдентификаторСтроки = СуммыОтгрузки.ИдентификаторСтроки
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.СуммыДокументовВВалютахУчета КАК СуммыЗаказа
	|			ПО ТоварыЗаказаКлиента.Ссылка = СуммыЗаказа.Регистратор
	|				И ТоварыЗаказаКлиента.ИдентификаторСтроки = СуммыЗаказа.ИдентификаторСтроки
	|ГДЕ
	|	ОтгрузкаТовары.Ссылка.Дата МЕЖДУ &НачалоПериода И &КонецПериода
	|	И ОтгрузкаТовары.Ссылка.Организация В (&Организация)
	|	И НЕ ОтгрузкаТовары.Ссылка.Исправление
	...
	|
	|ВЫБРАТЬ
	|	ОтгрузкиБезСчетаФактуры.СчетФактура КАК ДокументРегистратор,
	|	ОтгрузкиБезСчетаФактуры.СтавкаНДС,
	|	ЗНАЧЕНИЕ(Перечисление.ВидыЦенностей.Товары),
	|	2 КАК РангОшибки
	|ИЗ
	|	ВТОтгрузкиБезСчетаФактуры КАК ОтгрузкиБезСчетаФактуры
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|
	|ВЫБРАТЬ
	|	ВТОтгрузкиБезСчетаФактуры.СчетФактура КАК СчетФактура,
	|	ВТОтгрузкиБезСчетаФактуры.СтавкаНДС,
	|	ВТОперацииСОшибками.ДокументРегистратор,
	|	СУММА(ВТОтгрузкиБезСчетаФактуры.Сумма) КАК Сумма,
	|	СУММА(ВТОтгрузкиБезСчетаФактуры.СуммаНДС) КАК СуммаНДС,
	|	МИНИМУМ(ВЫБОР
	|			КОГДА ВТОперацииСОшибками.РангОшибки <> 2
	|				ТОГДА ВТОперацииСОшибками.РангОшибки
	|			КОГДА &ПрименяетсяОсвобождениеОтУплатыНДС
	...
	|	ЗНАЧЕНИЕ(Перечисление.ВидыЦенностей.Товары),
	|	ВТОтгрузкиБезСчетаФактуры.СчетФактура,
	|	ЛОЖЬ,
	|	НЕОПРЕДЕЛЕНО
	|ИЗ
	|	ВТОтгрузкиБезСчетаФактуры КАК ВТОтгрузкиБезСчетаФактуры
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВТОперацииСОшибками КАК ВТОперацииСОшибками
	|		ПО ВТОтгрузкиБезСчетаФактуры.СчетФактура = ВТОперацииСОшибками.ДокументРегистратор
	|			И ВТОтгрузкиБезСчетаФактуры.СтавкаНДС = ВТОперацииСОшибками.СтавкаНДС
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВТ_ОперацииПоСтавкеБезНДС КАК ВТ_ОперацииПоСтавкеБезНДС
	|		ПО ВТОтгрузкиБезСчетаФактуры.СчетФактура = ВТ_ОперацииПоСтавкеБезНДС.ДокументРегистратор
	|	
	|СГРУППИРОВАТЬ ПО
	|	ВТОтгрузкиБезСчетаФактуры.СчетФактура,
	|	ВТОтгрузкиБезСчетаФактуры.СтавкаНДС,
	...
	Запрос.Текст = СтрЗаменить(Запрос.Текст,
		"&СтавкаНДСОтгрузкаТоваров",
		УчетНДСУП.ТекстЗапросаСтавкаНДС(
			"ОтгрузкаТовары.Ссылка.НалогообложениеНДС",
			"ОтгрузкаТовары.АналитикаУчетаНоменклатуры.Номенклатура"));
		
	УчетНДСУП.УстановитьПараметрыЗапросаСтавкиНДС(Запрос.Параметры, Запрос.Параметры.Организация[0].Значение, Запрос.Параметры.НачалоПериода);
	Запрос.УстановитьПараметр("СтранаРегистрации",
		ЗначениеНастроекКлиентСерверПовтИсп.СтранаРегистрацииОрганизации(Запрос.Параметры.Организация[0].Значение));
...
Процедура ПолучитьНаборДанныхНачислениеВосстановлениеСАвансовВыданныхУпрощенныйУчетНДС(Запрос, СтруктураВнешнихНаборов) 
	...
	|	СУММА(ВложенныйЗапрос.СуммаБезНДСОборот) КАК СуммаБезНДСОборот,
	|	СУММА(ВложенныйЗапрос.НДСОборот) КАК НДСОборот
	...
	|					И ВидЦенности = ЗНАЧЕНИЕ(Перечисление.ВидыЦенностей.АвансыВыданные)
	|					И (ЗаписьДополнительногоЛиста
	|						И КорректируемыйПериод <= &КонецПериода)) КАК НДСЗаписиКнигиПродажОбороты) КАК ВложенныйЗапрос
	|СГРУППИРОВАТЬ ПО
...
Процедура ПолучитьНаборДанныхНачислениеРеализация0(Запрос, СтруктураВнешнихНаборов) 
	...
	|	ВТОтгрузкаПоСтавке0.КодОперации0 КАК КодОперации0
	...
	|	НалоговаяБазаПоОтгрузкам.КодОперации0 КАК КодВидаОперации
	|ИЗ
	|	НалоговаяБазаПоОтгрузкам КАК НалоговаяБазаПоОтгрузкам
	|ГДЕ
	|	НалоговаяБазаПоОтгрузкам.СуммаНДС <> 0
	|	ИЛИ НалоговаяБазаПоОтгрузкам.Сумма <> 0	
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
...
Процедура ПолучитьНаборДанныхВычетСАвансовПолученныхУпрощенныйУчетНДС(Запрос, СтруктураВнешнихНаборов) 
	...
	|					И ВидЦенности В (&ВидыЦенностейАвансыПолученные)
	|					И (ЗаписьДополнительногоЛиста
	|						И КорректируемыйПериод <= &КонецПериода)) КАК НДСЗаписиКнигиПокупокОбороты) КАК ВложенныйЗапрос
	|
	|СГРУППИРОВАТЬ ПО
	|	ВложенныйЗапрос.СчетФактура,
	|	ВложенныйЗапрос.ДоговорКонтрагента,
...
Процедура ПолучитьНаборДанныхВычетПоПриобретеннымЦенностям0(Запрос, СтруктураВнешнихНаборов) 
	...
	|	СоставКнигиПокупок.Период КАК Период,
	...
	|	ДокументыОтгрузки.СчетФактура КАК СчетФактураНаНеподтвержденнуюРеализацию0
	|ПОМЕСТИТЬ ВТСоставКнигиПокупокПоПриобретеннымЦенностям0
	|ИЗ
	|	РегистрНакопления.НДСЗаписиКнигиПокупок КАК СоставКнигиПокупок
	|		ЛЕВОЕ СОЕДИНЕНИЕ ДокументыОтгрузки КАК ДокументыОтгрузки
	|		ПО ДокументыОтгрузки.ДокументОтгрузки = СоставКнигиПокупок.ДокументОтгрузки
	|ГДЕ
	...
	|									ЗНАЧЕНИЕ(Перечисление.СобытияПоНДСПокупки.ПредъявленНДСКВычетуВПоследующихПериодахПодтвержденаСтавка0),
	|									ЗНАЧЕНИЕ(Перечисление.СобытияПоНДСПокупки.ПредъявленНДСКВычетуВПоследующихПериодахНеПодтвержденаСтавка0),
	|									ЗНАЧЕНИЕ(Перечисление.СобытияПоНДСПокупки.ПодтвержденаСтавка0))
	...
	|	СоставКнигиПокупок.НомерСтроки,
	|	СоставКнигиПокупок.ВидДеятельностиНДС,
	|	ПодтверждениеНулевойСтавкиНДССостав.СчетФактураНаНеподтвержденнуюРеализацию0
	|ИЗ
	|	РегистрНакопления.НДСЗаписиКнигиПокупок КАК СоставКнигиПокупок
	...
	|	СоставКнигиПокупок.НомерСтроки,
	|	СоставКнигиПокупок.ВидДеятельностиНДС,
	|	СоставКнигиПокупок.СчетФактураНаНеподтвержденнуюРеализацию0
	...
	|	ВЫБОР КОГДА ДляОпределенияСобытияПодтвержденияСтавки0.Событие = ЗНАЧЕНИЕ(Перечисление.СобытияПоНДСПокупки.ПредъявленНДСКВычетуВПоследующихПериодахНеПодтвержденаСтавка0)
	|		ТОГДА ВЫБОР
	...
	|			И (НДСЗаписиКнигиПродаж.СчетФактура = ДляОпределенияСобытияПодтвержденияСтавки0.СчетФактураНаНеподтвержденнуюРеализацию0)
	|			И ВЫБОР КОГДА ДляОпределенияСобытияПодтвержденияСтавки0.Событие = ЗНАЧЕНИЕ(Перечисление.СобытияПоНДСПокупки.ПредъявленНДСКВычетуВПоследующихПериодахПодтвержденаСтавка0)
	|				ТОГДА ИСТИНА
	|				ИНАЧЕ НАЧАЛОПЕРИОДА(ДляОпределенияСобытияПодтвержденияСтавки0.Период, КВАРТАЛ) = НАЧАЛОПЕРИОДА(НДСЗаписиКнигиПродаж.Период, КВАРТАЛ)
	|			КОНЕЦ
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ДанныеПервичныхДокументов КАК ДанныеПервичныхДокументов
	...
	|	НДСЗаписиКнигиПродаж.Событие,
	|	ВЫБОР КОГДА ДляОпределенияСобытияПодтвержденияСтавки0.Событие = ЗНАЧЕНИЕ(Перечисление.СобытияПоНДСПокупки.ПредъявленНДСКВычетуВПоследующихПериодахПодтвержденаСтавка0)
	|		ТОГДА НАЧАЛОПЕРИОДА(НДСЗаписиКнигиПродаж.ДатаСобытия, КВАРТАЛ)
	|		ИНАЧЕ НЕОПРЕДЕЛЕНО
	|	КОНЕЦ
	|ИЗ
	...
	|	И НДСЗаписиКнигиПродаж.Активность
	|		ИЛИ ДляОпределенияСобытияПодтвержденияСтавки0.ВидДеятельностиНДС = ЗНАЧЕНИЕ(Перечисление.ТипыНалогообложенияНДС.Космос)
	|		И ДляОпределенияСобытияПодтвержденияСтавки0.Событие = ЗНАЧЕНИЕ(Перечисление.СобытияПоНДСПокупки.ПредъявленНДСКВычету0))
	|	И ДляОпределенияСобытияПодтвержденияСтавки0.СчетФактура.Проведен
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
