...
#Область ТекстЗапроса
	Запрос.Текст =
		...
		|	КоличествоВыпуска.Серия КАК Серия,
		|	ПоследниеКалькуляции.НазначениеПродукции = Множители.Назначение КАК НазначениеСходится
		|
		|ПОМЕСТИТЬ КалькуляцииПродукцииПредварительная
		|ИЗ
		...
		|			И (ПоследниеКалькуляции.НазначениеПродукции = Множители.Назначение
		|				ИЛИ Множители.Назначение = ЗНАЧЕНИЕ(Справочник.Назначения.ПустаяСсылка))
		...
		|	ВЗ.КалькуляцияЗаказа КАК КалькуляцияЗаказа,
		|	ВЗ.КоличествоПродукцииФакт КАК КоличествоПродукцииФакт,
		|	ВЗ.Организация КАК Организация,
		|	ВЗ.Серия КАК Серия,
		|	МАКСИМУМ(ВЗ.НазначениеСходится) КАК НазначениеСходится
		|ПОМЕСТИТЬ ИзмеренияСПодходящимиНазначениями
		|ИЗ
		|	КалькуляцииПродукцииПредварительная КАК ВЗ
		|
		|СГРУППИРОВАТЬ ПО
		|	ВЗ.Продукция,
		...
		|	ВЗ.Партия,
		|	ВЗ.КалькуляцияЗаказа,
		|	ВЗ.КоличествоПродукцииФакт,
		|	ВЗ.Организация,
		|	ВЗ.Серия
		|
		|ИНДЕКСИРОВАТЬ ПО
		...
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	МАКСИМУМ(ДД.Калькуляция) КАК Калькуляция,
		|	МАКСИМУМ(ДД.СпецификацияПлановойКалькуляции) КАК СпецификацияПлановойКалькуляции,
		...
		|	ДД.КалькуляцияЗаказа КАК КалькуляцияЗаказа,
		|	ДД.КоличествоПродукцииФакт КАК КоличествоПродукцииФакт,
		|	ДД.Организация КАК Организация,
		|	ДД.Серия КАК Серия,
		|	ДД.НазначениеСходится КАК ОтборПоНазначению
		|ПОМЕСТИТЬ КалькуляцииПродукции
		|ИЗ
		|	КалькуляцииПродукцииПредварительная КАК ДД
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ИзмеренияСПодходящимиНазначениями КАК ИСПН
		|		ПО ДД.Продукция = ИСПН.Продукция
		|			И ДД.ХарактеристикаПродукции = ИСПН.ХарактеристикаПродукции
		|			И ДД.НазначениеПродукции = ИСПН.НазначениеПродукции
		|			И ДД.Партия = ИСПН.Партия
		|			И ДД.КалькуляцияЗаказа = ИСПН.КалькуляцияЗаказа
		|			И ДД.КоличествоПродукцииФакт = ИСПН.КоличествоПродукцииФакт
		|			И ДД.Организация = ИСПН.Организация
		|			И ДД.Серия = ИСПН.Серия
		|			И ДД.НазначениеСходится = ИСПН.НазначениеСходится
		|
		|СГРУППИРОВАТЬ ПО
		|	ДД.Продукция,
		...
		|	ДД.Партия,
		|	ДД.КалькуляцияЗаказа,
		|	ДД.КоличествоПродукцииФакт,
		|	ДД.Организация,
		|	ДД.Серия,
		|	ДД.НазначениеСходится
		|
		|ИНДЕКСИРОВАТЬ ПО
		|	Продукция,
		|	ХарактеристикаПродукции,
		...
		|			И (КалькуляцииПродукции.НазначениеПродукции = МножителиПартийИПолуфабрикатов.Назначение
		|				ИЛИ НЕ КалькуляцииПродукции.ОтборПоНазначению)
		|			И (ВЫБОР
		...
		|	МножителиПолуфабрикатов.КлючПартия КАК КлючПартия
		|ПОМЕСТИТЬ МножителиПродукции
		|ИЗ
		|	МножителиПолуфабрикатов КАК МножителиПолуфабрикатов
		|ГДЕ
		|	МножителиПолуфабрикатов.ЭтоПродукция
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		...
		|			И МножителиПродукции.КлючПартия = МатериальныеЗатраты.КлючПартия
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|ВЫБРАТЬ
		...
		|	МножителиПродукции.КлючПартия КАК КлючПартия
		|ИЗ
		|	МножителиПродукции КАК МножителиПродукции
		...
		|			И МножителиПродукции.КлючПартия = ПлановыеТрудозатраты.КлючПартия
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|ВЫБРАТЬ
		...
		|			И МножителиПродукции.КлючПартия = ПлановыеПрочиеЗатраты.КлючПартия
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		...
		|			И МножителиПолуфабрикатов.КлючПартия = ПлановыеМатериальныеЗатраты.КлючПартия
		|ГДЕ
		|	НЕ МножителиПолуфабрикатов.ЭтоПродукция
		|
		|СГРУППИРОВАТЬ ПО
		...
		|	МножителиПолуфабрикатов.КлючПартия
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|ВЫБРАТЬ
		...
		|			И МножителиПолуфабрикатов.КлючПартия = ПлановыеТрудозатраты.КлючПартия
		|ГДЕ
		|	НЕ МножителиПолуфабрикатов.ЭтоПродукция
		|
		|СГРУППИРОВАТЬ ПО
		...
		|			И МножителиПолуфабрикатов.КлючПартия = ПлановыеПрочиеЗатраты.КлючПартия
		|ГДЕ
		|	НЕ МножителиПолуфабрикатов.ЭтоПродукция
		|
		|СГРУППИРОВАТЬ ПО
		...
		|	ПлановыеЗатратыПродукции.КлючПартия КАК КлючПартия
		|
		...
		|	ПлановыеЗатратыПолуфабрикатов.КлючПартия КАК КлючПартия
		|ИЗ
		...
		|		ПлановыеДанные.Назначение,
		|		ПлановыеДанные.ПартияСпецификацияПродукции,
		|		ПлановыеДанные.КлючПартия КАК КлючПартия,
		|		ПлановыеДанные.Полуфабрикат,
		|		ПлановыеДанные.ХарактеристикаПолуфабриката,
		|		ПлановыеДанные.Затрата,
		|		ПлановыеДанные.ХарактеристикаЗатраты,
