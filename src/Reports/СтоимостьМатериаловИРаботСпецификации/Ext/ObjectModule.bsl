...
Перем Основание, ВидЦены, Валюта, Период, КэшированныеЗначения, Организация;
...
Процедура ПередЗагрузкойНастроекВКомпоновщик(Контекст, КлючСхемы, КлючВарианта, НовыеНастройкиКД, НовыеПользовательскиеНастройкиКД) Экспорт
	...
	Если ТипЗнч(Контекст) = Тип("ФормаКлиентскогоПриложения") Тогда
		...
		ОтображатьОрганизацию = Ложь;
		
		//++ Локализация
		ДополнительныйРазрезТарифнойСетки = Константы.ДополнительныйРазрезТарифнойСетки.Получить();
		ОтображатьОрганизацию = ДополнительныйРазрезТарифнойСетки = Перечисления.ДополнительныеРазрезыТарифнойСетки.Организация;
		//-- Локализация
		
		МенятьПользовательскиеНастройки = Ложь;
		МенятьНастройкиКД = Ложь;
		
		Если Не ОтображатьОрганизацию Тогда
			ПараметрОрганизация = КомпоновкаДанныхКлиентСервер.ПолучитьПараметр(
				КомпоновщикНастроекФормы.ПользовательскиеНастройки, "Организация");
			ПараметрОрганизация.РежимОтображения = РежимОтображенияЭлементаНастройкиКомпоновкиДанных.Недоступный;
			
			МассивПолейКД = Новый Массив(); 
			ПолеКомпоновки = Новый ПолеКомпоновкиДанных("Организация");
			РасчетСебестоимостиПрикладныеАлгоритмы.ДобавитьВыбранныеПоляКомпоновкиДанныхВМассив(
				КомпоновщикНастроекФормы.Настройки.Структура, МассивПолейКД, Ложь, ПолеКомпоновки);
			Для Каждого ПолеКД Из МассивПолейКД Цикл
				ПолеКД.Использование = Ложь;	
			КонецЦикла;

			МенятьПользовательскиеНастройки = Истина;
			МенятьНастройкиКД = Истина;
		КонецЕсли;
...
Процедура УстановитьОбязательныеНастройки(ПользовательскиеНастройкиМодифицированы)
	...
	ПараметрОснование = КомпоновкаДанныхКлиентСервер.ПолучитьПараметр(КомпоновщикНастроек.Настройки, "Основание");
...
Функция СтоимостьМатериаловИРабот()
	...
	МассивТекстовЗапроса.Добавить(
		РегистрыНакопления.ПлановыеТрудозатраты.ТекстЗапросаВТРасценкиТрудозатратСрезПоследних());
		
	Если ЦенообразованиеВызовСервера.ИспользуетсяЦенообразование25() Тогда
		МассивТекстовЗапроса.Добавить( ТекстЗапросаСтоимостьМатериаловИРабот2_5());
	Иначе
		МассивТекстовЗапроса.Добавить(ТекстЗапросаСтоимостьМатериаловИРабот());
	КонецЕсли;
...
Функция ПолучитьУслугиПереработчиков(Этапы)
	Запрос = Новый Запрос();
	...
	Для Каждого Этап Из Этапы Цикл
		Если Этап.ПроизводствоНаСтороне Тогда
			Для Каждого УслугаПереработчика Из Этап.УслугиПереработчика Цикл
				НоваяСтрока = Результат.Добавить();
				ЗаполнитьЗначенияСвойств(НоваяСтрока, УслугаПереработчика);
				НоваяСтрока.ПодразделениеЭтапа = Этап.Подразделение;
			КонецЦикла;
		КонецЕсли;
	КонецЦикла;
...
Процедура ДанныеСпецификацииВМенеджерВременныхТаблиц(МенеджерВременныхТаблиц)
	...
	Запрос.УстановитьПараметр("УслугиПереработчиков", ПолучитьУслугиПереработчиков(ТабличныеЧасти.Этапы));
	...
	|	ВыходныеИзделия.ПодразделениеЭтапа	КАК ПодразделениеЭтапа
	...
	|	МатериалыИУслуги.ПодразделениеЭтапа	КАК ПодразделениеЭтапа
	|ПОМЕСТИТЬ МатериалыИУслугиПредварительная
	|ИЗ
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	УслугиПереработчиков.Номенклатура КАК Номенклатура,
	|	УслугиПереработчиков.Характеристика КАК Характеристика,
	|	УслугиПереработчиков.СтатьяКалькуляции КАК СтатьяКалькуляции,
	|	УслугиПереработчиков.Количество КАК Количество,
	|	УслугиПереработчиков.ПодразделениеЭтапа КАК ПодразделениеЭтапа
	|ПОМЕСТИТЬ УслугиПереработчиковПредварительная
	|ИЗ
	|	&УслугиПереработчиков КАК УслугиПереработчиков
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|ИЗ
	|	МатериалыИУслугиПредварительная КАК МатериалыИУслуги
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	|	УслугиПереработчиков.Номенклатура КАК Номенклатура,
	|	УслугиПереработчиков.Характеристика КАК Характеристика,
	|	УслугиПереработчиков.Номенклатура.ЕдиницаИзмерения КАК ЕдиницаИзмерения,
	|	УслугиПереработчиков.СтатьяКалькуляции КАК СтатьяКалькуляции,
	|	ВЫРАЗИТЬ("""" КАК СТРОКА(100)) КАК Описание,
	|	УслугиПереработчиков.Количество КАК Количество,
	|	ЛОЖЬ ИспользуетсяАвтовыбор,
	|	&Спецификация КАК Спецификация,
	|	ЗНАЧЕНИЕ(Документ.ЗаказНаПроизводство.ПустаяСсылка) КАК ЗаказНаПроизводство,
	|	0 КАК КодСтрокиЗаказаНаПроизводство,
	|	&Период КАК НачатьНеРанее,
	|	УслугиПереработчиков.ПодразделениеЭтапа КАК ПодразделениеЭтапа
	|ИЗ
	|	УслугиПереработчиковПредварительная КАК УслугиПереработчиков
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ ПЕРВЫЕ 1
	...
	|	0 КАК КодСтрокиЗаказаНаПроизводство,
	|	&Период КАК НачатьНеРанее,
	|	ВозвратныеОтходы.ПодразделениеЭтапа	КАК ПодразделениеЭтапа
	...
	|	0 КАК КодСтрокиЗаказаНаПроизводство,
	|	&Период КАК НачатьНеРанее,
	|	Трудозатраты.ПодразделениеЭтапа КАК ПодразделениеЭтапа,
...
Функция ТекстЗапросаСтоимостьМатериаловИРабот()
	...
	|	ВТСпецификацияМатериалы.ПодразделениеЭтапа КАК ПодразделениеЭтапа
	...
	|	ВТСпецификацияОтходы.ПодразделениеЭтапа КАК ПодразделениеЭтапа
	|ИЗ
	...
	|	ВТМатериальные.ПодразделениеЭтапа КАК ПодразделениеЭтапа,
	...
	|	ВТСпецификацияТрудозатраты.ПодразделениеЭтапа КАК ПодразделениеЭтапа,
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВТРасценкиТрудозатратСрезПоследних КАК Расценки
	|		ПО ВТСпецификацияТрудозатраты.ВидРабот = Расценки.ВидРабот
	|			И ВТСпецификацияТрудозатраты.ПодразделениеЭтапа = Расценки.Подразделение
	|			И ВТСпецификацияТрудозатраты.Этап = Расценки.Этап
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	СуммыЗатрат.Организация КАК Организация,
	|	СуммыЗатрат.ПодразделениеЭтапа КАК ПодразделениеЭтапа,
	...
	|	&Основание,
	|	ВЫБОР КОГДА ВЫРАЗИТЬ(СуммыЗатрат.СтатьяКалькуляции КАК Справочник.СтатьиКалькуляции).ТипЗатрат = Значение(Перечисление.ТипыЗатрат.ВозвратныеОтходы)
...
Функция ТекстЗапросаСтоимостьМатериаловИРабот2_5()
	...
	|	&Основание,
	|	ВЫБОР
	|		КОГДА ВЫРАЗИТЬ(СуммыЗатрат.СтатьяКалькуляции КАК Справочник.СтатьиКалькуляции).ТипЗатрат = Значение(Перечисление.ТипыЗатрат.ВозвратныеОтходы)
	|			ТОГДА -1 * СуммыЗатрат.СуммаСпецификация
	|		ИНАЧЕ СуммыЗатрат.СуммаСпецификация
	|	КОНЕЦ КАК Сумма
	|ИЗ
