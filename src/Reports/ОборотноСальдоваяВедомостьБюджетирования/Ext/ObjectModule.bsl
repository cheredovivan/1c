...
Функция ОборотыПлан(НастройкиОтчета)
	...
	СКДПравилаРасчета = КомпоновкаДанныхСервер.ПустаяСхема();
	НДПравилаРасчета = КомпоновкаДанныхСервер.ДобавитьПустойНаборДанных(СКДПравилаРасчета,
		Тип("НаборДанныхЗапросСхемыКомпоновкиДанных"),
		"ПравилаРасчета");
	
	НДПравилаРасчета.Запрос = 
	"ВЫБРАТЬ
	...
	|	(ВЫРАЗИТЬ(ПравилаРасчета.Приемник КАК Справочник.ПоказателиБюджетов)).* КАК ПоказательБюджетов,
	|	(ВЫРАЗИТЬ(ПравилаРасчета.Источник КАК Справочник.СтатьиБюджетов)).* КАК СтатьяБюджетов}";
	НДПравилаРасчета.АвтоЗаполнениеДоступныхПолей = Истина;
	...
	КомпоновкаДанныхКлиентСервер.СкопироватьОтборКомпоновкиДанных(СКДПравилаРасчета, Настройки, НастройкиОтчета);
	
	ПравилаРасчетаПоказателей = ФинансоваяОтчетностьСервер.ВыгрузитьРезультатСКД(СКДПравилаРасчета, Настройки);
	
	ЗапросПоПравиламРасчета = БюджетированиеСервер.СформироватьЗапросКИсточникуДанныхПоПравиламРасчета(
		ПравилаРасчетаПоказателей.ВыгрузитьКолонку("ПравилоРасчета"), "План");
	ТекстЗапроса = ЗапросПоПравиламРасчета.ТекстЗапроса;
	Если ПустаяСтрока(ТекстЗапроса) Тогда
		ТекстЗапроса = БюджетированиеСервер.ПолучитьТекстЗапросаКИсточникуДанныхПоказателейБезПравилРасчета();
		ТекстЗапроса = СхемыЗапросов.УстановитьПомещениеВоВременнуюТаблицу(ТекстЗапроса, "Данные");
	...
	Иначе
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "ПериодПланирования МЕЖДУ &ВыражениеНачалоПериода И &ВыражениеКонецПериода",
			"ПериодПланирования МЕЖДУ &ВыражениеНачалоПериода И &ВыражениеКонецПериода И Статус В (&ОборотыВСтатусах)");
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&УсловияПоТаблицеИсточникаИПоляКомпановки",
			"ИСТИНА {(Сценарий).* КАК Сценарий, (Организация).* КАК Организация, (Подразделение).* КАК Подразделение, (Валюта).* КАК Валюта}");
		ТекстЗапроса = СхемыЗапросов.ДобавитьПолеВыборкиВЗапрос(ТекстЗапроса, "ПсевдонимТаблицыИсточника.Регистратор", "Регистратор");
	...
	Набор.Запрос = ТекстЗапроса + ОбщегоНазначения.РазделительПакетаЗапросов() + Набор.Запрос;
	Для Каждого ПараметрЗапроса Из ЗапросПоПравиламРасчета.ПараметрыЗапроса Цикл
		КомпоновкаДанныхСервер.ДобавитьПараметр(СхемаКомпоновкиПлана, ПараметрЗапроса.Ключ, , ПараметрЗапроса.Значение,
			Истина, Истина);
	КонецЦикла;
