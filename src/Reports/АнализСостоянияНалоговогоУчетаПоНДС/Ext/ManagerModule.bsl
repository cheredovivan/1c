...
Функция ПолучитьНаборДанныхНачислениеРеализацияСложныйУчетНДС(Запрос)
	...
	Запрос.УстановитьПараметр("НалогообложенияБезНДС", УчетНДСУПКлиентСервер.ТипыНалогообложенияСкрывающиеРеквизитыНДС());
	...
	НачисленияПоРеализацииВключенные = МассивРезультатов[МассивРезультатов.ВГраница()].Выбрать();
	
	ОтгрузкиТоваров = МассивРезультатов[МассивРезультатов.ВГраница()-1].Выгрузить();
	Для Каждого СтрокаОтгрузки Из ОтгрузкиТоваров Цикл
		Если Не ЗначениеЗаполнено(СтрокаОтгрузки.СуммаНДС) Тогда 
			СтрокаОтгрузки.СуммаНДС = УчетНДСУПКлиентСервер.РассчитатьСуммуНДС(СтрокаОтгрузки.Сумма, СтрокаОтгрузки.СтавкаНДС);
		КонецЕсли;
	КонецЦикла;
	ОтгрузкиВключенныеВБазу = ОтгрузкиТоваров.Итог("СуммаНДС");
	
	ВключенныеВБазу   = 0; 
	Если НачисленияПоРеализацииВключенные.Следующий() Тогда
		ВключенныеВБазу   = НачисленияПоРеализацииВключенные.СуммаНДС;
	КонецЕсли;
	ВключенныеВБазу = ВключенныеВБазу + ОтгрузкиВключенныеВБазу;
