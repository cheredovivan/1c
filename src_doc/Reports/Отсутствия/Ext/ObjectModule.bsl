...
Процедура ПриКомпоновкеРезультата(ДокументРезультат, ДанныеРасшифровки, СтандартнаяОбработка)
	...
	ТаблицаДниОтсутствия.Колонки.Добавить("Отсутствовал");
	Если ПараметрСчитатьДниОтсутствия.Значение Тогда
		
		СотрудникиОтбора = ?(ПараметрПодразделениеДней.Использование,
			Сотрудники.СотрудникиПодразделения(ПараметрПодразделениеДней.Значение, Истина, Истина),
			Сотрудники.ВсеСотрудники());
		...
		Запрос = Новый Запрос(
			...
			|ГДЕ
			|	Сотрудники.Ссылка В (&СотрудникиОтбора)");
		
		Запрос.УстановитьПараметр("СотрудникиОтбора", СотрудникиОтбора);
		...
		Пока Выборка.Следующий() Цикл
			...
			СтрокаДниОтсутствия.СотрудникДней = Выборка.Сотрудник;
			СтрокаДниОтсутствия.ПодразделениеДней = Выборка.Подразделение;
			
			ГрафикРаботыСотрудника = ГрафикиРаботы.ГрафикРаботыСотрудника(Выборка.Сотрудник);
			...
			СтрокаДниОтсутствия.Отсутствовал = 0;
			...
			Пока ПроверяемаяДата < ПараметрПериод.Значение.ДатаОкончания Цикл
				...
				Если ЗначениеЗаполнено(ГрафикРаботыСотрудника) Тогда
					...
					Если РабочиеДниГрафика[ПроверяемаяДата] = Неопределено Тогда
						РабочиеДниГрафика[ПроверяемаяДата] = 
							ГрафикиРаботы.ЭтоРабочийДень(ПроверяемаяДата, ГрафикРаботыСотрудника);
					КонецЕсли;
				...
				Если ЕстьОтсутствие Тогда
					СтрокаДниОтсутствия.Отсутствовал = СтрокаДниОтсутствия.Отсутствовал + 1;
