...
&НаКлиенте
Процедура РасположениеЗаголовокНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	
	ОткрытьФорму("ПланВидовХарактеристик.ДополнительныеРеквизитыИСведения.Форма.ВыборРасположения",
		Новый Структура("Набор, ТекущееЗначение", Объект.НаборСвойств, Объект.РасположениеИмя),
		Элемент);

КонецПроцедуры

&НаКлиенте
Процедура РасположениеЗаголовокОчистка(Элемент, СтандартнаяОбработка)
	
	Объект.РасположениеИмя = "";

КонецПроцедуры

&НаКлиенте
Процедура РасположениеЗаголовокОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	
	СтандартнаяОбработка = Ложь;
	Объект.РасположениеИмя = ВыбранноеЗначение.Имя;
	Объект.РасположениеЗаголовок = ВыбранноеЗначение.Заголовок;
	Модифицированность = Истина;
