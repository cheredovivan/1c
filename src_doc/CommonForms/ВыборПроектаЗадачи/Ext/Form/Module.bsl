...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СотрудникиПользователя = Новый ФиксированныйМассив(Сотрудники.СотрудникиПользователя());
	
	Проекты.Параметры.УстановитьЗначениеПараметра("СотрудникиПользователя", СотрудникиПользователя);
	
	Если РольДоступна("КонтрольПроектов") Или РаботаСПроектами.НетСвоихПроектов(СотрудникиПользователя) Тогда 
...
Процедура ДеревоПроектовПередНачаломИзменения(Элемент, Отказ)
	...
	Если ЗначениеЗаполнено(ТекущиеДанные.Ссылка) Тогда
		...
		Если ТекущиеДанные.ЭтоЗадача Тогда
			КлючеваяОперация = РаботаСПроектамиКлиентСервер.КлючеваяОперацияОткрытиеПроектнойЗадачи();
		...
		Иначе
			КлючеваяОперация = РаботаСПроектамиКлиентСервер.КлючеваяОперацияОткрытиеПроекта();
			ОценкаПроизводительностиКлиент.ЗамерВремени(КлючеваяОперация);
...
Процедура НайтиПроектыИПроектныеЗадачи()
	...
	Запрос.Текст = 
		...
		|			И ПроектыПроектнаяКоманда.Исполнитель В (&СотрудникиПользователя)
		|
		|		ОБЪЕДИНИТЬ ВСЕ
		|
		|		ВЫБРАТЬ
		...
		|	ИЛИ Проекты.Руководитель В (&СотрудникиПользователя))
		|
		|ОБЪЕДИНИТЬ
		|
		|ВЫБРАТЬ
...
Процедура МК_НастроитьЭлементыФормы()
	...
	Элементы.ФормаВыбрать.Отображение = ОтображениеКнопки.Картинка;
	Элементы.ФормаВыбрать.Картинка = БиблиотекаКартинок.ЗаписатьИЗакрыть;
	
	Элементы.СтрокаПоиска.ОтображениеПодсказки = ОтображениеПодсказки.Нет;
	Элементы.СтрокаПоиска.ПоложениеЗаголовка = ПоложениеЗаголовкаЭлементаФормы.Лево;
	Элементы.СтрокаПоиска.РастягиватьПоГоризонтали = Истина;
