...
Процедура ПодпискиУведомленийВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	ИначеЕсли Поле = Элементы.ПодпискиУведомленийЧат И ДанныеСтроки.ЗапретИзмененияЧат Тогда
		ТекстПредупреждения = НСтр("ru = 'Настройка установлена Администратором системы и не может быть изменена.'");
		ПоказатьПредупреждение(, ТекстПредупреждения);
	ИначеЕсли Поле = Элементы.ПодпискиУведомленийTelegram И ДанныеСтроки.ЗапретИзмененияTelegram Тогда
		ТекстПредупреждения = НСтр("ru = 'Настройка установлена Администратором системы и не может быть изменена.'");
		ПоказатьПредупреждение(, ТекстПредупреждения);
	ИначеЕсли Поле = Элементы.ПодпискиУведомленийВКонтакте И ДанныеСтроки.ЗапретИзмененияВКонтакте Тогда
		ТекстПредупреждения = НСтр("ru = 'Настройка установлена Администратором системы и не может быть изменена.'");
		ПоказатьПредупреждение(, ТекстПредупреждения);
	КонецЕсли;
...
Процедура ПодпискиПоОбъектамВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	ИначеЕсли Поле = Элементы.ПодпискиПоОбъектамTelegramСтрокой Тогда
	...
	ИначеЕсли Поле = Элементы.ПодпискиПоОбъектамВКонтактеСтрокой Тогда
...
Процедура СпособыУведомленияПриНачалеРедактирования(Элемент, НоваяСтрока, Копирование)
	...
	Если Копирование Тогда
		...
		ТекущиеДанные.ВидКонтактнойИнформации = Неопределено;
		ТекущиеДанные.Представление = ТекущиеДанные.ДанныеСпособа;
	КонецЕсли;
...
&НаКлиенте
Процедура ОтключитьУведомленияЧерезTelegram(Команда)
	
	ОтключитьУведомленияЧерезTelegramНаСервере();
	ПоказатьПредупреждение(, НСтр("ru = 'Уведомления через Telegram отключены'"));
	
КонецПроцедуры

&НаКлиенте
Процедура ОтключитьУведомленияЧерезВКонтакте(Команда)
	
	ОтключитьУведомленияЧерезВКонтактеНаСервере();
	ПоказатьПредупреждение(, НСтр("ru = 'Уведомления через ВКонтакте отключены'"));
...
Процедура ПолучитьПодпискиПоОбъектам()
	...
	Для Каждого ДанныеПодписокПоОбъекту Из ДанныеПодписокПоОбъектам.Строки Цикл
		...
		Для Каждого ДанныеПодпискиПоВидуСобытия Из ДанныеПодписокПоОбъекту.Строки Цикл
			...
			Для Каждого ДанныеПодписки Из ДанныеПодпискиПоВидуСобытия.Строки Цикл
				Если ДанныеПодписки.СпособУведомления = Перечисления.СпособыУведомления.Telegram Тогда
					СтрокаПодписки.Telegram = ДанныеПодписки.Значение;
				КонецЕсли;
			КонецЦикла;
			...
			Для Каждого ДанныеПодписки Из ДанныеПодпискиПоВидуСобытия.Строки Цикл
				Если ДанныеПодписки.СпособУведомления = Перечисления.СпособыУведомления.ВКонтакте Тогда
					СтрокаПодписки.ВКонтакте = ДанныеПодписки.Значение;
				КонецЕсли;
			КонецЦикла;
...
Процедура ПолучитьСпособыУведомления()
	...
	ДанныеСпособовУведомления = 
		РегистрыСведений.СпособыУведомленияПользователей.ПолучитьСпособыУведомленияПользователя(Пользователь);
	...
	Для Каждого Элемент Из СпособыУведомления Цикл
		...
		Для Каждого Элемент2 Из ИсходныеСпособыУведомления Цикл
			Если Элемент.СпособУведомления = Элемент2.СпособУведомления
				...
				И Элемент.Сотрудник = Элемент2.Сотрудник
		...
		Если Не Найден Тогда
			СпособУведомления = 
				Новый Структура("СпособУведомления, ВидКонтактнойИнформации, ДанныеСпособа");
...
Процедура ПолучитьНастройки()
	...
	Для Каждого Подписка Из ДоступныеПодписки Цикл
		...
		НоваяСтрока.Telegram = РегистрыСведений.НастройкиУведомлений.ПолучитьПодписку(
			Пользователь,
			Подписка.ВидСобытия,
			Перечисления.СпособыУведомления.Telegram);
		НоваяСтрока.ЗапретИзмененияTelegram = Не РазрешеноИзменятьНастройки
			И РегистрыСведений.НастройкиУведомлений.ПолучитьПодпискуПоУмолчанию(
				Подписка.ВидСобытия,
				Перечисления.СпособыУведомления.Telegram);
				
		НоваяСтрока.ВКонтакте = РегистрыСведений.НастройкиУведомлений.ПолучитьПодписку(
			Пользователь,
			Подписка.ВидСобытия,
			Перечисления.СпособыУведомления.ВКонтакте);
		НоваяСтрока.ЗапретИзмененияВКонтакте = Не РазрешеноИзменятьНастройки
			И РегистрыСведений.НастройкиУведомлений.ПолучитьПодпискуПоУмолчанию(
				Подписка.ВидСобытия,
				Перечисления.СпособыУведомления.ВКонтакте);
...
Процедура УстановитьВидимостьЭлементов()
	...
	Элементы.ПодпискиУведомленийПоPush.Видимость = ПолучитьФункциональнуюОпцию("ИспользоватьPushУведомления");
...
Процедура ОбновитьНастройкиTelegram()
	...
	УведомленияЧерезTelegramПодключены =
		РаботаСУведомлениями.УведомленияЧерезTelegramПодключены(Пользователь);
	
	ОбновитьИнструкциюTelegram();
	
	Элементы.СтраницаОтправкаTelegram.Видимость = ИспользоватьУведомленияЧерезTelegram;
	
	Элементы.ПодпискиУведомленийTelegram.Видимость = УведомленияЧерезTelegramПодключены;
	Элементы.ПодпискиПоОбъектамTelegramСтрокой.Видимость = УведомленияЧерезTelegramПодключены;
	Элементы.ОтключитьУведомленияЧерезTelegram.Видимость = УведомленияЧерезTelegramПодключены;
...
Процедура ОбновитьИнструкциюTelegram()
	...
	ИначеЕсли ИспользоватьУведомленияЧерезTelegram И Не УведомленияЧерезTelegramПодключены Тогда
		
		// Рекомендуем подключиться пользователю к боту в Telegram.
		ИмяБотаTelegram = Константы.ИмяБотаTelegram.Получить();
		
		ШаблонИнструкции =
			СтрШаблон(
				НСтр("ru = 'Для получения уведомлений через Telegram необходимо подключиться к боту, который будет рассылать уведомления. Для этого необходимо:
					 |1. Открыть Telegram в телефоне или на компьютере, найти с помощью поиска бота <a href =""https://telegram.me/%1"">%1</a> или воспользуйтесь QR-кодом ниже.
					 |2. Написать ему в сообщении логин для входа в Документооборот <b>%2</b>.
					 |3. После этого в Документообороте бот запросит вас через Обсуждения подтвердить, что это именно вы.'"),
				ИмяБотаTelegram,
				ИмяПользователяИБ());
		...
		ДанныеQRКода = УправлениеПечатью.ДанныеQRКода(СсылкаTelegram, 0, 250);
		УстановитьПривилегированныйРежим(Ложь);
		QRКодТелеграм = ПоместитьВоВременноеХранилище(ДанныеQRКода, УникальныйИдентификатор);
		
	ИначеЕсли ИспользоватьУведомленияЧерезTelegram И УведомленияЧерезTelegramПодключены Тогда
		
		// Рекомендуем настроить какие уведомления нужно получать через Telegram.
		ИмяБотаTelegram = Константы.ИмяБотаTelegram.Получить();
		
		ШаблонИнструкции =
			СтрШаблон(
				НСтр("ru = 'Уведомления через Telegram подключены, они будут поступать от имени бота <a href =""https://telegram.me/%1"">%1</a>.
					|
					|На закладке ""Подписки"" можно указать, какие уведомления будут поступать через Telegram.'"),
				ИмяБотаTelegram);
	...
	Элементы.QRКодТелеграм.Видимость = ЗначениеЗаполнено(QRКодТелеграм);
...
Процедура ОбновитьНастройкиВКонтакте()
	...
	УведомленияЧерезВКонтактеПодключены =
		РаботаСУведомлениями.УведомленияЧерезВКонтактеПодключены(Пользователь);
	
	ОбновитьИнструкциюВКонтакте();
	
	Элементы.СтраницаОтправкаВКонтакте.Видимость = ИспользоватьУведомленияЧерезВКонтакте;
	
	Элементы.ПодпискиУведомленийВКонтакте.Видимость = УведомленияЧерезВКонтактеПодключены;
	Элементы.ПодпискиПоОбъектамВКонтактеСтрокой.Видимость = УведомленияЧерезВКонтактеПодключены;
	Элементы.ОтключитьУведомленияЧерезВКонтакте.Видимость = УведомленияЧерезВКонтактеПодключены;
...
Процедура ОбновитьИнструкциюВКонтакте()
	...
	ИначеЕсли ИспользоватьУведомленияЧерезВКонтакте И Не УведомленияЧерезВКонтактеПодключены Тогда
		...
		ШаблонИнструкции =
			СтрШаблон(
				НСтр("ru = 'Для получения уведомлений через ВКонтакте необходимо написать сообществу, которое будет рассылать уведомления. Для этого необходимо:
					|1. Открыть ВКонтакте в телефоне или на компьютере, найти с помощью поиска сообщество <a href =""%1"">%2</a> или воспользоваться QR-кодом ниже.
					|2. Написать сообществу в сообщении логин для входа в Документооборот <b>%3</b>.
					|3. После этого в Документообороте бот запросит вас через Обсуждения подтвердить, что это именно вы.'"),
		...
		УстановитьПривилегированныйРежим(Истина);
		ДанныеQRКода = УправлениеПечатью.ДанныеQRКода(СсылкаГруппыВКонтакте, 0, 250);
		УстановитьПривилегированныйРежим(Ложь);
		QRКодВКонтакте = ПоместитьВоВременноеХранилище(ДанныеQRКода, УникальныйИдентификатор);
		
	ИначеЕсли ИспользоватьУведомленияЧерезВКонтакте И УведомленияЧерезВКонтактеПодключены Тогда
		
		// Рекомендуем настроить какие уведомления нужно получать через ВКонтакте.
		ИмяГруппыВКонтакте = Константы.ИмяГруппыВКонтакте.Получить();
		СсылкаГруппыВКонтакте = Константы.СсылкаГруппыВКонтакте.Получить();
		
		ШаблонИнструкции =
			СтрШаблон(
				НСтр("ru = 'Уведомления через ВКонтакте подключены, они будут поступать от имени сообщества <a href =""%1"">%2</a>.
					|
					|На закладке ""Подписки"" можно указать, какие уведомления будут поступать через ВКонтакте.'"),
				СсылкаГруппыВКонтакте,
				ИмяГруппыВКонтакте);
	...
	Элементы.QRКодВКонтакте.Видимость = ЗначениеЗаполнено(QRКодВКонтакте);
	
КонецПроцедуры

&НаСервере
Функция ИмяПользователяИБ()
...
&НаСервере
Процедура ОтключитьУведомленияЧерезTelegramНаСервере()
	
	РегистрыСведений.СпособыУведомленияПользователей.ОчиститьНастройкиОтправкиПользователя(
		Пользователь,
		Перечисления.СпособыУведомления.Telegram);
	ОбновитьНастройкиTelegram();
...
&НаСервере
Процедура ОтключитьУведомленияЧерезВКонтактеНаСервере()
