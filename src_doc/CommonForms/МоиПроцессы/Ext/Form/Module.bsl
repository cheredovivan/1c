...
Процедура СписокПроцессовПередНачаломДобавления(Элемент, Отказ, Копирование, Родитель, Группа, Параметр)
	...
	Если Копирование И ТекущиеДанные <> Неопределено Тогда
		...
		ИначеЕсли ТипКопируемогоПроцесса = Тип("БизнесПроцессСсылка.Подписание") Тогда
			ИмяТипа = "Подписание";
...
Процедура ПриИзмененииПараметровЭкрана()
	...
	Если МассивИнформаций[0].Ширина > МассивИнформаций[0].Высота Тогда
		ЭтаФорма.Группировка = ГруппировкаПодчиненныхЭлементовФормы.ГоризонтальнаяЕслиВозможно;
	Иначе
...
Процедура ФайлыОткрытьДляПросмотра(Команда)
	...
	РаботаСБизнесПроцессамиКлиент.ОткрытьТекущийФайлДляПросмотра(ЭтотОбъект, Элементы.ДеревоПриложений);
...
Процедура ФайлыРедактировать(Команда)
	...
	РаботаСБизнесПроцессамиКлиент.РедактироватьТекущийФайл(
		ЭтотОбъект, Элементы.ДеревоПриложений, ДеревоПриложений.ПолучитьЭлементы().Количество());
...
Процедура ФайлыСохранитьКак(Команда)
	...
	РаботаСБизнесПроцессамиКлиент.СохранитьТекущийФайл(ЭтотОбъект, Элементы.ДеревоПриложений);
...
Процедура ОткрытьФайл(Команда)
	...
	ДанныеФайла = РаботаСФайламиВызовСервера.ДанныеФайлаДляОткрытия(ТекущиеДанные.Ссылка, Неопределено,
...
Функция ЭтоПоддерживаемыйБизнесПроцесс(Ссылка)
	...
	ТипыПоддерживаемыхПроцессов.Добавить(Тип("БизнесПроцессСсылка.Подписание"));
...
Процедура ОткрытьТекущуюСтроку()
	...
	Если Элементы.СписокПроцессов.ТекущийЭлемент = Элементы.СписокПроцессовОсновнойПредмет Тогда
		Если ТипЗнч(ТекущиеДанные.ОсновнойПредмет) = Тип("Строка")
			...
			РаботаСКомплекснымиБизнесПроцессамиКлиент.ВыполнитьЗамерОткрытияФормыКП(ТекущиеДанные.Процесс);
		...
		Если ТипЗнч(Строка.Ссылка) = Тип("СправочникСсылка.Файлы") Тогда
			Если ЗначениеЗаполнено(Строка.Ссылка) Тогда
				...
				Если ЗначениеЗаполнено(Строка.Редактирует) Тогда
					Если СотрудникиВызовСервера.ПользовательИЕгоСотрудники(Пользователи.ТекущийПользователь()).Найти(Строка.Редактирует) <> Неопределено Тогда
...
Процедура НастроитьЭлементыФормыДляМобильногоУстройства()
	...
	Элементы.СписокПроцессовСостояниеКонтроля.КартинкаЗначений = БиблиотекаКартинок.МК_КоллекцияСтрокКонтроль;
	Элементы.СписокПроцессовСостояниеКартинка.КартинкаЗначений = БиблиотекаКартинок.МК_КоллекцияСостоянияПроцессовИЗадач;
