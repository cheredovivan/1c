...
Функция ПредставленияКолонок(ИмяТаблицы) 
	...
	Если ИмяТаблицы = "ПапкиДокументов" Тогда
	...
	ИначеЕсли ИмяТаблицы = "ПапкиПроектов" Тогда
		...
		Стр = Представления.Добавить();
		Стр.ИмяЭлемента = "ТаблицаПравПравоЧтенияСтрокой";
		Стр.Подсказка = НСтр("ru = 'Чтение папок'");
		
		Стр = Представления.Добавить();
		Стр.ИмяЭлемента = "ТаблицаПравПравоИзмененияПапокСтрокой";
		Стр.Заголовок = НСтр("ru = 'Изменение'");
		Стр.Подсказка = НСтр("ru = 'Изменение папок'");
		
		// Права содержимого не настраиваются.
		Ширина = 0;
		Элементы.ПраваСодержимого.Видимость = Ложь;
		АктуальныеКолонкиПрав = Новый Массив;
		АктуальныеКолонкиПрав.Добавить(Элементы.ТаблицаПравПравоЧтенияСтрокой);
		АктуальныеКолонкиПрав.Добавить(Элементы.ТаблицаПравПравоИзмененияПапокСтрокой);
		АктуальныеКолонкиПрав.Добавить(Элементы.ТаблицаПравПравоУправленияПравамиСтрокой);
		АктуальныеКолонкиПрав.Добавить(Элементы.ТаблицаПравПередаватьДочернимСтрокой);
		Для Каждого Колонка Из АктуальныеКолонкиПрав Цикл
			Колонка.РастягиватьПоГоризонтали = Ложь;
			Колонка.АвтоМаксимальнаяШирина = Ложь;
			Колонка.МаксимальнаяШирина = 3;
		КонецЦикла;
...
Процедура ЗаполнитьИменаНастроек()
	...
	Если ИмяТаблицы = "ПапкиДокументов" Тогда
		ИменаНастроек.ПравоЧтения = "ЧтениеПапокИДокументовПредприятия";
		ИменаНастроек.ПравоИзмененияПапок = "ИзменениеПапокДокументовПредприятия";
		ИменаНастроек.ПравоДобавленияДанных = "ДобавлениеПапокИДокументовПредприятия";
		ИменаНастроек.ПравоИзмененияДанных = "ИзменениеПапокИДокументовПредприятия";
		ИменаНастроек.ПравоУдаленияДанных = "ПометкаУдаленияПапокИДокументовПредприятия";
	ИначеЕсли ИмяТаблицы = "ПапкиФайлов" Тогда
...
Функция ПроверитьРазрешениеНаУправлениеПравами(ПропуститьВызовИсключения = Ложь, ОписаниеПредупреждения = "")
	...
	РазрешеноУправлениеПравами = ДокументооборотПраваДоступа.ПраваПользователяПоОбъекту(СсылкаНаОбъект).УправлениеПравами;
...
Функция ЗаписатьСервер(ПроверятьПраваРодителей)
	...
	Попытка
		...
		ПротоколированиеРаботыСотрудников.ЗаписатьИзменениеПрав(СсылкаНаОбъект);
...
Функция ЕстьОшибкиВПравахРодителей()
	...
	СвернутаяТаблицаПравПапки.Колонки.Добавить("Пользователь", Метаданные.ОпределяемыеТипы.КонтейнерыСотрудников.Тип);
...
Функция ПользователиСПравомЧтенияПапки(СвернутаяТаблицаПравПапки)
	...
	Запрос = Новый Запрос(
		...
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ НастройкиПрав КАК НастройкиПрав
		|		ПО СотрудникиВКонтейнерах.Контейнер = НастройкиПрав.Пользователь
		...
		|	И СотрудникиПользователей.Пользователь <> СотрудникиВКонтейнерах.Контейнер
		|
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
...
&НаКлиенте
Процедура ТаблицаПравПользовательОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
...
Процедура ПодборПользователей(Команда)
	...
	ПараметрыФормы.Вставить("ЗаголовокФормы", НСтр("ru = 'Подбор сотрудников, подразделений, групп'"));
	ПараметрыФормы.Вставить("ЗаголовокСпискаАдреснойКниги", НСтр("ru = 'Все участники'"));
