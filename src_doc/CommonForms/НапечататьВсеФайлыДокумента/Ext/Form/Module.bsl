...
Процедура Напечатать(Команда)
	...
	Если ДанныеФайлов.Количество() < 10 Тогда	
	...
	Иначе
		...
		Состояние(НСтр("ru = 'Печать файлов...'"), 0);
		
		НомерФайла = 0; 
		ПодключитьОбработчикОжидания("ПошаговаяПечать", 0.2, Истина);
...
&НаКлиенте
Процедура ПошаговаяПечать()
	
	Если НомерФайла >= ДанныеФайлов.Количество() Тогда
		Закрыть();
		Возврат;
	КонецЕсли;	
	
	Прогресс = Цел(НомерФайла * 100 / ДанныеФайлов.Количество());
	Состояние(НСтр("ru = 'Печать файлов...'"), Прогресс);
	
	ТекДанные = ДанныеФайлов[НомерФайла];    
	
	ДанныеФайловЛокальные = Новый Массив;           
	ДанныеФайловЛокальные.Добавить(ТекДанные);
	КомандыРаботыСФайламиКлиент.НапечататьФайлы(ДанныеФайловЛокальные);
