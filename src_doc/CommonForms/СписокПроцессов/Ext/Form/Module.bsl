...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Элементы.ПоТипуПроцесса.СписокВыбора.Добавить("Подписание", НСтр("ru = 'Подписание'"));	
	...
	Иначе
		...
		Если НЕ ЗначениеЗаполнено(ПоИсполнителю) Тогда
			ПоИсполнителю = ПредопределенноеЗначение("Справочник.Сотрудники.ПустаяСсылка");
		КонецЕсли;
...
Процедура СписокВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	Если Поле.Имя = "СостояниеКонтроля" Тогда
		КонтрольКлиент.ОбработкаКомандыКонтроль(ТекущиеДанные.Ссылка, ЭтаФорма);
	Иначе
		РаботаСКомплекснымиБизнесПроцессамиКлиент.ВыполнитьЗамерОткрытияФормыКП(ТекущиеДанные.Ссылка);
...
Процедура СписокПередНачаломДобавления(Элемент, Отказ, Копирование, Родитель, 
	...
	Если Копирование Тогда
		...
		ИначеЕсли ТипЗнч(ЗначениеКопирования) = Тип("БизнесПроцессСсылка.Подписание") Тогда
			ИмяТипа = "Подписание";
		...
		Если ИмяТипа = "КомплексныйПроцесс" Тогда
		...
		ОткрытьФорму("БизнесПроцесс." + ИмяТипа + ".ФормаОбъекта", ПараметрыФормы, ЭтотОбъект, 
	...
	Иначе // создание
		...
		ПараметрыФормы = Новый Структура;
		Если ПоТипуПроцесса <> "ВсеТипы" Тогда
...
//        - ПоИсполнителю - СправочникСсылка.Сотрудники или СправочникСсылка.ПолныеРоли
//
...
Процедура НастроитьЭлементыФормыДляМобильногоУстройства()
	...
	// Настроим группу быстрого отбора.
	...
	// Выровняем элементы группы быстрого отбора.
	ВыравниваниеЭлементовФормыМК(Элементы.ПоТипуПроцесса);
	ВыравниваниеЭлементовФормыМК(Элементы.ПоСостоянию);
	ВыравниваниеЭлементовФормыМК(Элементы.ПоИсполнителю);
	ВыравниваниеЭлементовФормыМК(Элементы.ПоПроекту);
	
	// Настроим кнопку очистки быстрого отбора.
	Элементы.ОчиститьБыстрыеОтборы.Отображение = ОтображениеКнопки.Текст;
	Элементы.ОчиститьБыстрыеОтборы.Заголовок = НСтр("ru = 'Очистить отбор'");
	Элементы.ОчиститьБыстрыеОтборы.Ширина = 0;
	
	// Настроим строку поиска для списка.
	...
	Элементы.Автор.Шрифт = Новый Шрифт(ШрифтыСтиля.МелкийШрифтТекста);
	...
	// Настроим формат для даты и сроков.
	ИспользоватьДатуИВремяВСрокахЗадач = ПолучитьФункциональнуюОпцию("ИспользоватьДатуИВремяВСрокахЗадач");
	// Формат срока исполнения.
	...
	Элементы.СостояниеКонтроля.КартинкаЗначений = БиблиотекаКартинок.МК_КоллекцияСтрокКонтроль;
	Элементы.ВажностьКартинка.КартинкаЗначений = БиблиотекаКартинок.МК_КоллекцияСтрокВариантыВажности;
	Элементы.СостояниеКартинка.КартинкаЗначений = БиблиотекаКартинок.МК_КоллекцияСостоянияПроцессовИЗадач
	
КонецПроцедуры

&НаСервере
Процедура ВыравниваниеЭлементовФормыМК(Элемент)
