...
Процедура ВыбратьСрокИсполнения()
	...
	Если ЭтаФорма.ОписаниеОповещенияОЗакрытии = Неопределено Тогда
		ОповеститьОВыборе(СрокИсполнения);
	Иначе
...
// Выделяет вариант времени в таблице ВариантыВремени соответствующий
...
//   * РабочиеДаты - СписокЗначений - см. функцию РабочиеДаты.
//
&НаСервереБезКонтекста
...
Процедура НастроитьФормуДляМобильногоПриНеобходимости()
	...
	Элементы.ГруппаБыстрыеСроки.Видимость = Ложь;
	Элементы.МК_БыстрыеСрокиКомПанель.Видимость = Истина;
	
	ПоложениеКоманднойПанели = ПоложениеКоманднойПанелиФормы.Авто;
	Элементы.Переместить(Элементы.Готово, КоманднаяПанель);
	Элементы.Переместить(Элементы.Отмена, КоманднаяПанель);
		
	Элементы.ВариантыВремени.ВариантУправленияВысотой = ВариантУправленияВысотойТаблицы.ВСтрокахТаблицы;
	Элементы.ВариантыВремени.ВысотаВСтрокахТаблицы = 4;
	Элементы.ГруппаВремя.РастягиватьПоВертикали = Ложь;	
