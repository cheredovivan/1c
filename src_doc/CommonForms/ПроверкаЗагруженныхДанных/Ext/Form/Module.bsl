...
Процедура ОтменитьПроверку(Команда)
	...
	Если Элементы.Страницы.ТекущаяСтраница = Элементы.СтраницаДокументыПредприятия Тогда
		ВыбранныйЭлемент = Элементы.ДокументыПредприятия;
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не ПравоДоступа("Чтение", Метаданные.Справочники.ДокументыПредприятия) Тогда 
		Элементы.СтраницаДокументыПредприятия.Видимость = Ложь;
	КонецЕсли;
	...
	Контрагенты.Параметры.УстановитьЗначениеПараметра("ДатаОкончания", ПериодЗагрузки.ДатаОкончания);
...
Процедура УстановитьОтборыВсехСписков()
	...
	УстановитьОтборПроверен(ДокументыПредприятия);
	УстановитьОтборПроверен(Контрагенты);
	
	УстановитьОтборПоПериодуЗагрузки(ДокументыПредприятия);
	УстановитьОтборПоПериодуЗагрузки(Контрагенты);
	
	УстановитьОтборПоПериодуПроверки(ДокументыПредприятия);
	УстановитьОтборПоПериодуПроверки(Контрагенты);
	
	УстановитьОтборПоПроверил(ДокументыПредприятия);
	УстановитьОтборПоПроверил(Контрагенты);	
	
	УстановитьОтборПодписан(ДокументыПредприятия);
	УстановитьОтборПодписан(Контрагенты);
	
	УстановитьОтборПоОрганизации(ДокументыПредприятия);
	УстановитьОтборПоСтатусуПроверкиЭП(ДокументыПредприятия);
...
&НаКлиенте
Процедура ДокументыПредприятияВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	СтандартнаяОбработка = Ложь;
	ПараметрыОткрытия = Новый Структура;
	ПараметрыОткрытия.Вставить("ПроверкаЗагруженныхДанных", Истина);
	ДелопроизводствоКлиент.ОткрытьДокументПредприятия(Элемент.ТекущиеДанные.Ссылка, ПараметрыОткрытия);
КонецПроцедуры
...
Процедура ОбработкаОповещенияСервер()
	ПолучитьКоличествоВсехОбъектовСервер();
	Элементы.ДокументыПредприятия.Обновить();
	Элементы.Контрагенты.Обновить();
КонецПроцедуры

&НаСервере
Процедура ПолучитьКоличествоВсехОбъектовСервер()
	КоличествоДокументовПредприятия = ПолучитьКоличествоОбъектовСервер("ДокументыПредприятия");
...
Процедура СтраницыПриСменеСтраницы(Элемент, ТекущаяСтраница)
	...
	Если ТекущаяСтраница = Элементы.СтраницаДокументыПредприятия Тогда
		ПриАктивизацииСтроки(Элементы.ДокументыПредприятия);
...
Процедура УстановитьРеквизиты(Команда)
	...
	Если Элементы.Страницы.ТекущаяСтраница = Элементы.СтраницаДокументыПредприятия Тогда	
		Для каждого ВыделеннаяСтрока Из Элементы.ДокументыПредприятия.ВыделенныеСтроки Цикл
...
Процедура ЗаполнитьСписокЭП(Документ)
	...
	РаботаСЭП.ЗаполнитьСписокПодписей(Документ, ТаблицаПодписей, 
		УникальныйИдентификатор, Неопределено, ВсегоПодписей);
