...
Процедура УстановитьДоступностьИТипШаблонаОснования()
	...
	Если ТипЗнч(Предмет) = Тип("СправочникСсылка.ДокументыПредприятия") Тогда
		Если ТипЗнч(ШаблонОснование) <> Тип("СправочникСсылка.ШаблоныДокументов") Тогда
			ШаблонОснование = ПредопределенноеЗначение("Справочник.ШаблоныДокументов.ПустаяСсылка");
		КонецЕсли;
