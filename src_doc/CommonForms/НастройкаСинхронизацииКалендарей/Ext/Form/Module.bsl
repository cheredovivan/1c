...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Для Каждого ВидЗагрузки Из ВидыЗагрузкиКалендарей() Цикл
		Элементы.КалендариПодключениеВидЗагрузки.СписокВыбора.Добавить(ВидЗагрузки.Ключ, ВидЗагрузки.Значение);
		Элементы.КалендариНастройкаВидЗагрузки.СписокВыбора.Добавить(ВидЗагрузки.Ключ, ВидЗагрузки.Значение);
	КонецЦикла;
...
Процедура ЗаполнитьНаСервере()
	...
	УчетнаяЗапись = ДанныеУчетнойЗаписи(ПользователиДокументооборот.ФизЛицоПользователя());
...
&НаСервереБезКонтекста
Функция ДанныеУчетнойЗаписи(ФизЛицо)
...
Процедура ЗавершитьПодключениеВФоне(ДлительнаяОперация)
	...
	Если ДлительнаяОперация.Статус = "Выполнено" Тогда
		...
		КонецЕсли;
		Если ЗначениеЗаполнено(Узел) Тогда
			ЗаполнитьКалендариФизЛица(Узел);
		КонецЕсли;
		Если СтатусСинхронизации = СтатусыСинхронизации()["НеНастроенаПодключение"] Тогда
...
&НаСервере
Процедура ЗаполнитьКалендариФизЛица(Узел)
	...
	Если Не РезультатЗапроса.Пустой() Тогда
		...
		Пока Выборка.Следующий() Цикл
			...
			НоваяСтрока.ВидЗагрузки = ?(Выборка.БезОписания, ВидыЗагрузкиКалендарей()["БезОписания"], 
				ВидыЗагрузкиКалендарей()["Полностью"]);
...
Процедура ЗаписатьДанныеУчетнойЗаписи()
	...
	Если Не ЗначениеЗаполнено(Узел) Тогда
		Узел = ПланыОбмена.СинхронизацияКалендарей.СсылкаНовогоУзла(
			ПользователиДокументооборот.ФизЛицоПользователя(), ТипСинхронизации(ТипУчетнойЗаписи));
...
Процедура ЗаписатьНастройкиiCloud(Узел)
	...
	ПарольiCloud = ?(ЗначениеЗаполнено(ПарольiCloud), УникальныйИдентификатор, "");
...
Функция ДлительнаяОперацияСинхронизации()
	...
	КонецЕсли;
	ПараметрыВыполненияВФоне.Вставить("НаименованиеФоновогоЗадания",
		СтрШаблон(НСтр("ru = 'Синхронизация календаря пользователя %1'"), ПользователиДокументооборот.ФизЛицоПользователя()));
	ПараметрыВыполненияВФоне.ОжидатьЗавершение = 0;
	ДлительнаяОперация = ДлительныеОперации.ВыполнитьВФоне(
