...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	Если ИмяСобытия = "ЗакрытьФормуПомощникаОтправки" И Источник = ЭтаФорма Тогда
		...
		ПараметрыОповещения = Новый Структура("МожноЗакрытьФормуПомощникаОтправки", Истина);	
		Оповестить("ПомощникОтправить_ПроверкаВозможностиЗакрытияФормы", ПараметрыОповещения);
		
		Если ПараметрыОповещения.МожноЗакрытьФормуПомощникаОтправки Тогда
			Закрыть(ЗначениеЗакрытия);
		КонецЕсли;

	ИначеЕсли ИмяСобытия = "ЗакрытьФормуПомощникаОтправкиБезусловно" Тогда
...
Процедура ЗаполнитьФормуПоДаннымПомощника(ПомощникНаСервере) Экспорт
	...
	СоответствиеСтрокВариантов = Новый Соответствие;
	...
	Пока ЗначениеЗаполнено(СтрокиКОбработке) Цикл
		...
		ЭлементРодитель = СоответствиеСтрокВариантов[СтрокаВДереве.Родитель];
		...
		СоответствиеСтрокВариантов[СтрокаВДереве] = СтрокаВариантаОтправки;
	...
	Если ПомощникОтправить.ИспользуетсяРежимОтправкиЗадачи(ПомощникНаСервере) Тогда
		Элементы.ВариантыОтправки.Отображение = ОтображениеТаблицы.Список;
	КонецЕсли;
...
Процедура ОбновитьОписаниеВарианта()
	...
	Если Не ЗначениеЗаполнено(ОписаниеВарианта) Тогда
		...
		Если ТипЗнч(ТекущиеДанные.Вариант) = Тип("СправочникСсылка.ШаблоныКомплексныхБизнесПроцессов") Тогда
...
Процедура ОбработатьВыборВарианта()
	...
	ПараметрыДляФормы = ПараметрыВарианта(ТекущийВариант, Помощник);
	
	ПомощникОтправитьКлиент.ОбработатьВыборВарианта(
		ТекущийВариант, ПараметрыДляФормы, ЭтаФорма);
