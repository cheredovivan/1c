...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	КонецЕсли;	
	Элементы.КачествоПреобразованияPDFВКартинки.Видимость = ИспользоватьImageMagickДляПреобразованияВPDF;
...
Процедура ВнешнееСобытие(Источник, Событие, Данные)
	...
	Если Источник = "TWAIN" И Событие = "ImageAcquired" Тогда
		...
		ДатаImageAcquired = ОбщегоНазначенияКлиент.ДатаСеанса();
		...
		ПодключитьОбработчикОжидания("ПроверитьПриходEndBatch", 60, Истина);
...
Процедура ПриОткрытии(Отказ)
	...
	Если ТехнологияРаспознаванияШтрихкодов = ПредопределенноеЗначение("Перечисление.ТехнологииРаспознаванияШтрихкодов.Документооборот") Тогда
		Если Не ПроинициализироватьКомпонентуРаспознаванияШтрихкодов() Тогда
			...
			Элементы.ВерсияКомпонентыРаспознаванияШтрихкодов.Видимость = Ложь;
		Иначе                         
			
			ВерсияКомпонентыРаспознаванияШтрихкодов = КомпонентаРаспознаванияШтрихкодов.Версия();
			Элементы.ВерсияКомпонентыРаспознаванияШтрихкодов.Видимость = Истина;
...
Процедура ЗагрузитьИзКаталога(Команда)
	...
	СпособПреобразованияВPDF = 1; // image magick, а не как в форме настроек сканирования
...
Процедура ОткрытьСписокНераспознанныхИзображений(Команда)
	...
	"МассивПутейНераспознанныхФайлов, ТекстНастроек, ТипЗагрузки, ПутьКПрограммеКонвертации, ФорматХраненияОдностраничный, ФорматХраненияМногостраничный, УдалятьФайлыПослеЗагрузки, ФайлыДляУдаления, СпособПреобразованияВPDF", 
		...
		ФайлыДляУдаления, СпособПреобразованияВPDF);
...
Процедура ОбработкаЗакрытия(ОписаниеОповещения)
	...
	ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		...
		КоличествоНеразмещенных, СтрокаПроУдаление + ?(ПустаяСтрока(СтрокаПроУдаление), "", " "));
...
Процедура ПрочитатьНастройкиСканирования()
	...
	Если ФорматСканированногоИзображения.Пустая() Тогда
		...
		ОбщегоНазначения.ХранилищеОбщихНастроекСохранить("НастройкиСканирования/ФорматСканированногоИзображения", ИдентификаторКлиента, ФорматСканированногоИзображения);
	КонецЕсли;
	...
	Если ФорматХраненияОдностраничный.Пустая() Тогда
		ФорматХраненияОдностраничный = Перечисления.ФорматыХраненияОдностраничныхФайлов.PNG;
		ОбщегоНазначения.ХранилищеОбщихНастроекСохранить("НастройкиСканирования/ФорматХраненияОдностраничный", ИдентификаторКлиента, ФорматХраненияОдностраничный);
	КонецЕсли;
	...
	Если ФорматХраненияМногостраничный.Пустая() Тогда
		...
		ОбщегоНазначения.ХранилищеОбщихНастроекСохранить("НастройкиСканирования/ФорматХраненияМногостраничный", ИдентификаторКлиента, ФорматХраненияМногостраничный);
	КонецЕсли;
	...
	АвтоматическаяПодачаДокументов = ОбщегоНазначения.ХранилищеОбщихНастроекЗагрузить("НастройкиСканирования/АвтоматическаяПодачаДокументов", ИдентификаторКлиента);
	...
	Если ПоказыватьДиалогСканераЗагрузка = Неопределено Тогда
		...
		ОбщегоНазначения.ХранилищеОбщихНастроекСохранить("НастройкиСканирования/ПоказыватьДиалогСканера", ИдентификаторКлиента, ПоказыватьДиалогСканераЗагрузка);
	КонецЕсли;
	...
	Если ИмяУстройства = Неопределено Тогда
		ИмяУстройства = "";
		ОбщегоНазначения.ХранилищеОбщихНастроекСохранить("НастройкиСканирования/ИмяУстройства", ИдентификаторКлиента, ИмяУстройства);
	КонецЕсли;
	...
	КачествоПреобразованияPDFВКартинкиЗагрузка = 
		ХранилищеОбщихНастроек.Загрузить("НастройкиСканирования/КачествоПреобразованияPDFВКартинки", ИдентификаторКлиента);
	Если КачествоПреобразованияPDFВКартинкиЗагрузка = Неопределено Тогда
		КачествоПреобразованияPDFВКартинкиЗагрузка = 150;
		ОбщегоНазначения.ХранилищеОбщихНастроекСохранить("НастройкиСканирования/КачествоПреобразованияPDFВКартинки", ИдентификаторКлиента, КачествоПреобразованияPDFВКартинкиЗагрузка);
	КонецЕсли;
	КачествоПреобразованияPDFВКартинки = КачествоПреобразованияPDFВКартинкиЗагрузка;
...
Процедура ЗаполнитьДеревоИстории()
	...
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "1", Строка(ЧислоСеансовСканированияДляОтображения));
...
Процедура ЗагрузитьСоСканераПродолжение(ВыбранноеУстройство, Параметры) Экспорт
	...
	АвтоматическаяПодачаДокументовЧисло = РаботаСоСканеромКлиент.ПолучитьНастройку(ВыбранноеУстройство, "FEEDER");
	АвтоматическаяПодачаДокументов = ?((АвтоматическаяПодачаДокументовЧисло = 1), Истина, Ложь);
...
Процедура СохранитьНастройки()
	...
	ОбщегоНазначения.ХранилищеОбщихНастроекСохранить("ПотоковоеСканирование", "КаталогиЗагрузки", МассивКаталоги);	
	ОбщегоНазначения.ХранилищеОбщихНастроекСохранить("ПотоковоеСканирование", "ЗадаватьВопросОПродолженииСканирования", ЗадаватьВопросОПродолженииСканирования);
	ОбщегоНазначения.ХранилищеОбщихНастроекСохранить("ПотоковоеСканирование", "ПорядокПодачиБумаги", ПорядокПодачиБумаги);
	ОбщегоНазначения.ХранилищеОбщихНастроекСохранить("ПотоковоеСканирование", "Рекурсивно", Рекурсивно);
	ОбщегоНазначения.ХранилищеОбщихНастроекСохранить("ПотоковоеСканирование", "УдалятьФайлыПослеЗагрузки", УдалятьФайлыПослеЗагрузки);
	ОбщегоНазначения.ХранилищеОбщихНастроекСохранить("ПотоковоеСканирование", "РежимРазделенияПоДокументамПотоковогоСканирования", РежимРазделенияПоДокументамПотоковогоСканирования);
	ОбщегоНазначения.ХранилищеОбщихНастроекСохранить("ПотоковоеСканирование", "РаспределениеИзображенийПоФайлам", РаспределениеИзображенийПоФайлам);
	ОбщегоНазначения.ХранилищеОбщихНастроекСохранить("ПотоковоеСканирование", "ПоложенияШтрихКода", ПоложенияШтрихКода);
	ОбщегоНазначения.ХранилищеОбщихНастроекСохранить("ПотоковоеСканирование", "ИмяКомпьютераABBYY", ИмяКомпьютераABBYY);
	ОбщегоНазначения.ХранилищеОбщихНастроекСохранить("ПотоковоеСканирование", "ИмяСценарияABBYYШтрихкоды", ИмяСценарияABBYYШтрихкоды);
	ОбщегоНазначения.ХранилищеОбщихНастроекСохранить("ПотоковоеСканирование", "ИмяСценарияABBYYТекст", ИмяСценарияABBYYТекст);
	ОбщегоНазначения.ХранилищеОбщихНастроекСохранить("ПотоковоеСканирование", "ОчищатьИсториюСканирования", ОчищатьИсториюСканирования);
	ОбщегоНазначения.ХранилищеОбщихНастроекСохранить("ПотоковоеСканирование", "ЧислоСеансовХраненияИсторииСканирования", ЧислоСеансовХраненияИсторииСканирования);
	ОбщегоНазначения.ХранилищеОбщихНастроекСохранить("ПотоковоеСканирование", "ЧислоСеансовСканированияДляОтображения", ЧислоСеансовСканированияДляОтображения);
	ОбщегоНазначения.ХранилищеОбщихНастроекСохранить("ПотоковоеСканирование", "ВысотаЗоныШтрихКода", ВысотаЗоныШтрихКода);
	ОбщегоНазначения.ХранилищеОбщихНастроекСохранить("ПотоковоеСканирование", "СмещениеX", СмещениеX);
	ОбщегоНазначения.ХранилищеОбщихНастроекСохранить("ПотоковоеСканирование", "СмещениеY", СмещениеY);
	ОбщегоНазначения.ХранилищеОбщихНастроекСохранить("ПотоковоеСканирование", "ШиринаЗоныШтрихКода", ШиринаЗоныШтрихКода);
	ОбщегоНазначения.ХранилищеОбщихНастроекСохранить("ПотоковоеСканирование", "ФорматХраненияМногостраничныйДляЗагрузкиКаталога", ФорматХраненияМногостраничныйДляЗагрузкиКаталога);
	ОбщегоНазначения.ХранилищеОбщихНастроекСохранить("ПотоковоеСканирование", "ТехнологияРаспознаванияШтрихкодов", ТехнологияРаспознаванияШтрихкодов);
	ОбщегоНазначения.ХранилищеОбщихНастроекСохранить("ПотоковоеСканирование", "ШагСканирования", ШагСканирования);
	...
	ОбщегоНазначения.ХранилищеОбщихНастроекСохранить(
		"НастройкиСканирования/КачествоПреобразованияPDFВКартинки",
		ИдентификаторКлиента,
		КачествоПреобразованияPDFВКартинки);
...
Процедура ПрочитатьНастройки()
	...
	СпособПреобразованияВPDF = ОбщегоНазначения.ХранилищеОбщихНастроекЗагрузить("НастройкиСканирования/СпособПреобразованияВPDF", ИдентификаторКлиента);
...
&НаКлиенте
Процедура ПроверитьПриходEndBatch()
	
	// Спустя 60 сек после ImageAcquired - до сих пор нет EndBatch
	// Тогда эмулируем EndBatch.
	Если ЗначениеЗаполнено(ДатаImageAcquired)
		И (Не ЗначениеЗаполнено(ДатаEndBatch)
		Или ДатаImageAcquired > ДатаEndBatch) Тогда
...
&НаКлиенте
Процедура ОбработатьEndBatch()

	ПродолжитьСканирование = Ложь;
	
	ДатаEndBatch = ОбщегоНазначенияКлиент.ДатаСеанса();
...
Процедура ВыполнитьЗагрузкуИзКаталога()
	...
	МассивФайлов = НайтиФайлы(КаталогЗагрузки, ПолучитьМаскуВсеФайлы(), Рекурсивно);
	...
	Для Каждого Файл Из МассивФайлов Цикл
		...
		Если Файл.ЭтоФайл() Тогда
			...
			// Локализация Конец
			Если РаботаСФайламиКлиентСервер.ЭтотФайлМожноРаспознать(РасширениеБезТочки, ПараметрыПроверки) Тогда
...
Процедура ВыполнитьЗагрузкуСоСканераПродолжение(ВыбранноеУстройство, Параметры) Экспорт
	...
	РаботаСФайламиСлужебныйКлиент.ЗаписатьЖурналСканирования("НачатьСканирование.Старт", ТекстСобытия);
	...
	Попытка
		ПараметрыПриложения["СтандартныеПодсистемы.КомпонентаTwain"].НачатьСканирование(ПоказыватьДиалогСканера, ВыбранноеУстройство, ФорматКартинки, 
			...
			АвтоматическаяПодачаДокументов);	
	Исключение
		ПредставлениеОшибки = ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		РаботаСФайламиСлужебныйКлиент.ЗаписатьЖурналСканирования("НачатьСканирование", ПредставлениеОшибки, Истина);
		ВызватьИсключение;
	КонецПопытки;
...
Функция ПолучитьПервыйДокументСДаннымШтрихКодом(ШтрихКод)
	...
	Для Каждого ШтрихКодТекущий Из Штрихкоды Цикл  
		...
		МассивШК = ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(ШтрихКодТекущий);
		
		МассивНайденных = ШтрихкодированиеСервер.НайтиОбъектыПоШтрихкоду(МассивШК);
		
		Для Каждого НайденныйОбъект Из МассивНайденных Цикл
			
			СсылкаВладельца = НайденныйОбъект.Ключ;
			
			Если ТипЗнч(СсылкаВладельца) = Тип("СправочникСсылка.ДокументыПредприятия") Тогда
	...
	Иначе	 // АББИ
		...
		Если Не Результат.IsFailed Тогда
			...
			Если Результат.Statistics.TotalCharacters = 0 Тогда
				...
				МассивФайлов = НайтиФайлы(КаталогРезультатов, ПолучитьМаскуВсеФайлы());
				Для Каждого Файл Из МассивФайлов Цикл
					Если РаботаСФайламиКлиентСервер.ЭтоРасширениеКартинки(НРег(Файл.Расширение)) Тогда
						ЭтоПустойЛист = Ложь;
						Прервать;
					КонецЕсли;	
				КонецЦикла;	
			...
			УдалитьФайлы(КаталогРезультатов, ПолучитьМаскуВсеФайлы());
...
&НаКлиенте
Функция РаспознатьШтрихКодОдногоФайла(Знач ИмяФайлаКартинки, ПрерватьПроцессСканирования)
	...
	Если НРег(Файл.Расширение) = НРег(".pdf") Тогда
		...
		// UNC путь
		Если СтрДлина(ИмяФайлаКартинки) > 2 И Лев(ИмяФайлаКартинки, 2) = "\\" Тогда
			
#Если Не ВебКлиент Тогда
			ИмяВременное = ПолучитьИмяВременногоФайла("pdf");
			КопироватьФайл(ИмяФайлаКартинки, ИмяВременное);
			ИмяФайлаКартинки = ИмяВременное;
#КонецЕсли
			
		КонецЕсли;	
		
		ФайлКартинкиДляПути = Новый Файл(ИмяФайлаКартинки);
		ВременнаяПапкаДляРазархивирования = ФайлКартинкиДляПути.Путь;
			
		МассивПутейPng = РаботаСКартинкамиКлиент.РазделитьМногоСтраничныйФайл(
			ИмяФайлаКартинки, ВременнаяПапкаДляРазархивирования, ПутьКПрограммеКонвертации, КачествоПреобразованияPDFВКартинки);	

		ЧислоПутей = МассивПутейPng.Количество(); 
		
		Если ЧислоПутей = 0 Тогда
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		...
		// отсортируем по дате
		Для Каждого ПутьФайла Из МассивПутейPng Цикл
...
&НаСервереБезКонтекста
Процедура ВыделитьФрагментКартинки(ДвДанныеПолнойКартинки, ДвДанныеФрагмента,
	ПоложенияШтрихКода, СмещениеX, СмещениеY, ШиринаЗоныШтрихКода, ВысотаЗоныШтрихКода, Расширение)   
	
	ФайлИзображение = ПолучитьИмяВременногоФайла(Расширение); 
	ДвДанныеПолнойКартинки.Записать(ФайлИзображение);
	
	ФайлФрагмент = ПолучитьИмяВременногоФайла(Расширение);
	
	ПараметрыФрагмента = РаботаСКартинками.ПараметрыПолученияФрагментаСоШтрихкодом();
	ПараметрыФрагмента.ФайлИзображение = ФайлИзображение;
	ПараметрыФрагмента.ФайлФрагмент = ФайлФрагмент;
	ПараметрыФрагмента.ПоложенияШтрихКода = ПоложенияШтрихКода;
	ПараметрыФрагмента.ШиринаЗоныШтрихКода = ШиринаЗоныШтрихКода;
	ПараметрыФрагмента.ВысотаЗоныШтрихКода = ВысотаЗоныШтрихКода;
	ПараметрыФрагмента.ОтступСлева = СмещениеX;
	ПараметрыФрагмента.ОтступСверху = СмещениеY;
	
	РаботаСКартинками.ПолучитьФрагментИзображенияСоШтрихкодом(ПараметрыФрагмента); 
	
	ДвДанныеФрагмента = Новый ДвоичныеДанные(ФайлФрагмент);
...
#Если Не ВебКлиент И Не МобильныйКлиент Тогда
	...
	Если ПоложенияШтрихКода <> ПредопределенноеЗначение("Перечисление.ПоложенияШтрихКода.НеЗадано") Тогда
		...
		ДвДанныеПолнойКартинки = Новый ДвоичныеДанные(ПутьФайлаКартинки);  
		ДвДанныеФрагмента = Неопределено;
		ВыделитьФрагментКартинки(ДвДанныеПолнойКартинки, ДвДанныеФрагмента,
			ПоложенияШтрихКода, СмещениеX, СмещениеY, ШиринаЗоныШтрихКода, ВысотаЗоныШтрихКода,
			Файл.Расширение);
		
		ДвДанныеФрагмента.Записать(ПутьВременногоФайла); 
...
Процедура РазмещениеВБазе(ВладельцыСНомерами)
	...
	Для Каждого ПараКлючЗначение Из ВсеВладельцы Цикл
		...
		Для Каждого ОписаниеФайла Из ДанныеВладельца.МассивОписанийФайлов Цикл
			...
			// Если МассивПутейКартинок содержит несколько и настройка Все в один файл - вызываем Объединить В Многостраничный Файл
			...
			Если НомерФайла <> Неопределено Тогда
				...
				Если ТипЗагрузки = ПредопределенноеЗначение("Перечисление.ТипЗагрузкиПотоковогоСканирования.ИзКаталога") Тогда
					...
					ИмяБезРасширения = ФайлДляИмени.ИмяБезРасширения;
				КонецЕсли;	
				
				ИмяФайла = РаботаСФайламиВызовСервера.ИмяСканированногоФайла(
					НомерФайла, ПрефиксИнформационнойБазы, ТекущийВладелец, ИмяБезРасширения, ИмяОригинальногоФайла);
	...
	Элементы.ДекорацияТекстУспешно.Доступность = (КоличествоУспешноРазмещенных <> 0);
	...
	Если ПродолжитьСканирование Тогда
		...
		Попытка
			ПараметрыПриложения["СтандартныеПодсистемы.КомпонентаTwain"].НачатьСканирование(
				ПоказыватьДиалогСканера, ВыбранноеУстройство, ФорматКартинки, 
				Разрешение, Цветность, Поворот, РазмерБумаги, 
				ПараметрСжатие,
				ДвустороннееСканированиеПотоковое,
				АвтоматическаяПодачаДокументов);
...
&НаКлиенте
Процедура КачествоПреобразованияPDFВКартинкиОчистка(Элемент, СтандартнаяОбработка)
	СтандартнаяОбработка = Ложь;
КонецПроцедуры
