...
Процедура СоздатьПредмет(Команда)
	...
	ИначеЕсли ТипПредмета = "Справочник.ДокументыПредприятия" Тогда
		ИмяФормыОбъекта = "Справочник.ДокументыПредприятия.ФормаОбъекта";
		ВидСправочникаШаблонов = "ШаблоныДокументов";
	...
	Если Не ПустаяСтрока(ВидСправочникаШаблонов) Тогда 
		Если НЕ ЗначениеЗаполнено(ШаблонДокумента) Тогда 
			Если КоличествоДоступныхШаблоновДокументов.ДокументыПредприятия > 0 Тогда
		...
		КонецЕсли
	Иначе
		ВызватьИсключение ДелопроизводствоКлиент.ТекстОшибки_ЗапретСозданияДокумента();
	КонецЕсли;
...
Процедура СоздатьПредметПродолжение(Результат, Параметры) Экспорт
	...
	Если ЗначениеЗаполнено(ПредметОснование) Тогда
		...
		Если ТипПредмета = "Справочник.ДокументыПредприятия"
			И ТипШаблонДокумента = Тип("СправочникСсылка.ШаблоныДокументов")
			И ТипПредметОснование = Тип("СправочникСсылка.ДокументыПредприятия") Тогда
	...
	Если ТипПредмета = "Справочник.Контрагенты" Тогда
	...
	Если ЗначениеЗаполнено(Предмет) Тогда
		...
		Если РаботаСРабочимиГруппами.ПоОбъектуВедетсяАвтоматическоеЗаполнениеРабочейГруппы(ОбъектДляРасширенияРГ)
			...
			ТекстВопроса = НСтр("ru = 'Внимание!
				|Предмет %1""%2"" станет доступен всем участникам этого процесса.
				|Добавить%3?'");
...
Процедура УстановитьКоличествоДоступныхШаблоновДокументов()
	...
	Если ПравоДоступа("Чтение", Метаданные.Справочники.ШаблоныДокументов) Тогда
		ЗапросКолВнутр = Новый Запрос("ВЫБРАТЬ РАЗРЕШЕННЫЕ Шаблоны.Код ИЗ Справочник.ШаблоныДокументов КАК Шаблоны");
		КоличествоДоступныхШаблоновДокументов.Вставить("ДокументыПредприятия", ЗапросКолВнутр.Выполнить().Выбрать().Количество());
	КонецЕсли;
