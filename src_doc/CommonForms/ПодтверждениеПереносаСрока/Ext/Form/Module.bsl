...
Процедура ЗаполнитьДеревоПроцессов(Процесс, НовыйСрокИсполнения)
	...
	ИначеЕсли ВедущийПроцессЯвляетсяКомплексным Тогда
		...
		Пока ЗначениеЗаполнено(НовыйВедущийПроцесс) Цикл
			...
			ОбработкаПроцесса = РегистрыСведений.ПроцессыОбработокОбъектов.ОбработкаПоПроцессу(НовыйВедущийПроцесс);
			ЭтоПроцессОбработки = ЗначениеЗаполнено(ОбработкаПроцесса);
			
			Если ЗначениеЗаполнено(РеквизитыВедущегоПроцесса.Схема) Тогда
			
				ПараметрыСхемы = ?(ЭтоПроцессОбработки, Справочники.ПараметрыСхемДляОбработокОбъектов.ПараметрыПоСхеме(
					РеквизитыВедущегоПроцесса.Схема), Справочники.ПараметрыСхемДляКомплексныхПроцессов.ПараметрыПоСхеме(
				...
				ДанныеПараметров = ?(ЭтоПроцессОбработки,
