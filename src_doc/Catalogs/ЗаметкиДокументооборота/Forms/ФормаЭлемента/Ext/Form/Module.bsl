...
Процедура МК_НастроитьЭлементыФормы()
	...
	Элементы.ДекорацияПустота.Видимость = Ложь;
	
	Элементы.Изменена.ПоложениеЗаголовка = ПоложениеЗаголовкаЭлементаФормы.Лево;
	Элементы.Изменена.Заголовок = НСтр("ru = 'Изменена'");
	
	Элементы.МК_НижнийБлок.Видимость = Истина;
	МК.ПреобразоватьКнопкуВАкцентную(ЭтотОбъект, МК_ЭлементыСтиля, Элементы.МК_ЗаписатьИЗакрыть, Элементы.МК_НижнийБлок);
	
	Элементы.ГруппаПодШапка.Группировка = ГруппировкаПодчиненныхЭлементовФормы.Вертикальная;
	
	МК.ОформитьКнопкуФильтра(Элементы.СоздатьНовуюТему);
	Элементы.Изменена.ГоризонтальноеПоложение = ГоризонтальноеПоложениеЭлемента.Лево;
	МК.УстановитьБазовуюНастройкуДополнительнойТаблицы(Элементы.ДеревоТем);
	МК.УстановитьНовуюВысотуДополнительнойТаблицы(Элементы.ДеревоТем, 3);
