...
Процедура ЗаполнитьДеревоПереписки()
	...
	|		ПО ВходящееПисьмо.Ссылка = СвязиОбъектов.Объект
	|			И (СвязиОбъектов.ТипСвязи = ЗНАЧЕНИЕ(Справочник.ТипыСвязей.ПолученоВОтветНаПисьмо))
	|ГДЕ
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.СвязиОбъектов КАК СвязиОбъектов
	|		ПО ИсходящееПисьмо.Ссылка = СвязиОбъектов.Объект
	|			И (СвязиОбъектов.ТипСвязи = ЗНАЧЕНИЕ(Справочник.ТипыСвязей.ПисьмоОтправленоВОтветНа)
	|				ИЛИ СвязиОбъектов.ТипСвязи = ЗНАЧЕНИЕ(Справочник.ТипыСвязей.ПересылкаПисьма))
	|ГДЕ
