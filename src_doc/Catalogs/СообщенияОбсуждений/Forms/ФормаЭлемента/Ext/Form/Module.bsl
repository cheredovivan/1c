...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ВозможноВедениеРабочейГруппы() Тогда
		РаботаСРабочимиГруппами.ДокументУстановитьУсловноеОформлениеСпискаРабочейГруппы(ЭтотОбъект);
	КонецЕсли;
...
Процедура ПриОткрытии(Отказ)
	...
	Если ЭтоМобильныйКлиент Тогда
		МК_ПоказатьСкрытьФайлыПродолжение();
	КонецЕсли;
...
Процедура ПослеЗаписи(ПараметрыЗаписи)
	...
	КлючеваяОперация = РаботаСОбсуждениямиКлиентСервер.КлючеваяОперацияЗаписьСообщения();
...
Процедура ГолосованиеПриИзменении(Элемент)
	...
	Если ЭтоМобильныйКлиент Тогда
		Элементы.МК_Проголосовать.Доступность = Элементы.Проголосовать.Доступность;
	КонецЕсли;
...
Процедура ФайлыПередНачаломДобавления(Элемент, Отказ, Копирование, Родитель, Группа)
	...
	Если ЭтоМобильныйКлиент Тогда
		МК_ОбновитьЗаголовокФайлов(ЭтотОбъект);
	КонецЕсли;
...
Процедура ФайлыПеретаскивание(Элемент, ПараметрыПеретаскивания, СтандартнаяОбработка, Строка, Поле)
	...
	РаботаСФайламиКлиент.ОбработкаПеретаскиванияВЛинейныйСписок(
		ПараметрыПеретаскивания, ВладелецФайлаСписка, ЭтотОбъект, НеОткрыватьКарточкуПослеСозданияИзФайла);
	Элементы.Файлы.Обновить();
...
Процедура СохранитьКак(Команда)
	...
	Иначе
		ДанныеФайла = РаботаСФайламиВызовСервера.ДанныеФайлаДляСохранения(
			Элементы.Файлы.ТекущаяСтрока, Неопределено, УникальныйИдентификатор);
...
Процедура ЗакончитьРедактирование(Команда)
	...
	ПараметрыОбновленияФайла = РаботаСФайламиКлиент.ПараметрыОбновленияФайла(Неопределено, 
		Элементы.Файлы.ТекущаяСтрока, УникальныйИдентификатор);
...
Процедура ЗакончитьРедактированиеПервогоФайла(Команда)
	...
	ПараметрыОбновленияФайла = РаботаСФайламиКлиент.ПараметрыОбновленияФайла(Неопределено, 
		ФайлПрикрепленный, УникальныйИдентификатор);
...
Процедура ДобавитьГолосование(Команда)
	...
	Если ЭтоМобильныйКлиент Тогда
		МК_ПереключитьНаСтраницу(ЭтотОбъект, Элементы.СтраницаГолосование, Элементы.МК_КнопкаСтраницаГолосование);
	КонецЕсли;
...
Процедура Прокомментировать(Команда)
	...
	ПараметрыФормы.Вставить("ТекстНовогоСообщения", ГолосПользователя);
	
	РаботаСОбсуждениямиКлиент.СоздатьСообщение(ПараметрыФормы);
...
&НаКлиенте
Процедура МК_СтраницаСообщение(Команда)
	
	МК_ПереключитьНаСтраницу(ЭтотОбъект, Элементы.СтраницаСообщение,
		Элементы.МК_КнопкаСтраницаСообщение);
		
КонецПроцедуры

&НаКлиенте
Процедура МК_СтраницаГолосование(Команда)
	
	МК_ПереключитьНаСтраницу(ЭтотОбъект, Элементы.СтраницаГолосование,
		Элементы.МК_КнопкаСтраницаГолосование);
...
Процедура МК_СтраницаРабочаяГруппа(Команда)
	...
	МК_ПереключитьНаСтраницу(ЭтотОбъект, Элементы.СтраницаРабочаяГруппа,
		Элементы.МК_КнопкаСтраницаРабочаяГруппа);
		
КонецПроцедуры

&НаКлиенте
Процедура МК_ПоказатьНаЧтоОтвечаем(Команда)
	
	МК_ПоказатьНаЧтоОтвечаемПродолжение();
...
Процедура ОбновитьФайлСообщения()
	...
	Если МассивФайлов.Количество() > 0 Тогда
		...
		Если Сотрудники.ЭтоСотрудникТекущегоПользователя(ФайлРедактирует) Тогда
...
Процедура УстановитьДоступностьФайловыхКоманд()
	...
	Если ФайлПрикрепленный <> Неопределено Тогда
		...
		Элементы.РедактироватьПервый.Доступность = 
			(Не ЗначениеЗаполнено(ФайлРедактирует)
...
Процедура УстановитьДоступностьКоманд(ТекущиеДанные)
	...
	Иначе	
		...
		УстановитьДоступностьКоманды(Элементы.Редактировать, Не ЗначениеЗаполнено(Редактирует) И Не РежимЧтенияСообщения);
		УстановитьДоступностьКоманды(Элементы.КонтекстноеМенюФайлыРедактировать, 
			Не ЗначениеЗаполнено(Редактирует) И Не РежимЧтенияСообщения);
		...
		УстановитьДоступностьКоманды(Элементы.Освободить,
			ЗначениеЗаполнено(Редактирует) И Не РежимЧтенияСообщения);
...
Процедура УстановитьВидимостьЭлементов()
	...
	ПользовательИЕгоСотрудники 
		= СотрудникиВызовСервера.ПользовательИЕгоСотрудники(ПользователиКлиентСервер.ТекущийПользователь());
	Элементы.НадписьАвторДата.Видимость = 
		Не (ПользовательИЕгоСотрудники.Найти(Объект.Автор) <> Неопределено );
	...
	Если ЭтоМобильныйКлиент Тогда
		...
		// Форма
		Элементы.МК_ЗаписатьИЗакрыть.Видимость = Не РежимЧтенияСообщения;
		Элементы.ФормаЗаписатьИЗакрыть.Видимость = Ложь;
		
		Элементы.МК_Закрыть.Видимость = РежимЧтенияСообщения;
		Элементы.ФормаЗакрыть.Видимость = Ложь;
		
		Элементы.МК_Ответить.Видимость = Не СозданиеСообщения;
		
		// Сообщение
		Элементы.МК_ПоказатьНаЧтоОтвечаем.Видимость = УказанРодитель;
		Элементы.ГруппаНаЧтоОтвечаем.Видимость = Ложь;
		МК_КлиентСервер.ОформитьКнопкуСворачиваемойГруппы(
			МК_ЭлементыСтиля, Элементы.МК_ПоказатьНаЧтоОтвечаем, Элементы.ГруппаНаЧтоОтвечаем.Видимость, Ложь);
			
		// Файлы
		Элементы.МК_ФайлыДобавитьФайл.Доступность = Не РежимЧтенияСообщения;
		Элементы.Файлы.ПоложениеКоманднойПанели = ПоложениеКоманднойПанелиЭлементаФормы.Нет;
		
		// Голосование
		Элементы.МК_ГолосованиеРедактировать.Видимость = Истина;
		Элементы.МК_ГолосованиеДействия.Видимость = Истина;
		Элементы.КоманднаяПанельГолосование.Видимость = Ложь;
		
		Элементы.МК_Удалить.Видимость = ГолосованиеВключено И Не РежимЧтенияСообщения;
		Элементы.МК_Проголосовать.Видимость = ГолосованиеВПроцессе;
		Элементы.МК_Прокомментировать.Видимость = ГолосованиеВПроцессе;
		Элементы.МК_ОтменитьГолос.Видимость = ГолосованиеВПроцессе;
		Элементы.МК_Изменить.Видимость = ГолосованиеВПроцессе И Не РежимЧтенияСообщения;
		Элементы.МК_ДобавитьВариант.Видимость = ГолосованиеРедактируется;

		Элементы.МК_Проголосовать.Доступность = ГолосованиеДоступноДляУчастия И ВыбранВариантГолосования;
		Элементы.МК_Прокомментировать.Доступность = ГолосованиеДоступноДляУчастия И ПользовательПроголосовал;
		Элементы.МК_ОтменитьГолос.Доступность = ГолосованиеДоступноДляУчастия И ПользовательПроголосовал;
		
		Элементы.ГруппаГолосование.Видимость = Истина;
		Элементы.Голосование.Видимость = ГолосованиеВПроцессе;
		Элементы.ВсегоГолосов.Видимость = ГолосованиеВПроцессе;
		
		// Рабочая группа
		Элементы.МК_РабочаяГруппаКоманднаяПанель.Видимость = Истина;
		Элементы.РабочаяГруппаТаблица.ПоложениеКоманднойПанели = ПоложениеКоманднойПанелиЭлементаФормы.Нет;
		
		// Команды
		Элементы.КоманднаяПанельДобавление.Видимость = Ложь;
		Элементы.МК_КоманднаяПанельДобавление.Видимость = Не РежимЧтенияСообщения;
		
		Элементы.ГруппаФайл.Видимость = Ложь;
		Элементы.СтраницаФайлы.Видимость = (КоличествоФайлов >= 1);
		
		ОтображатьЗакладки = Элементы.СтраницаГолосование.Видимость Или Элементы.СтраницаРабочаяГруппа.Видимость;
		
		Элементы.МК_ДобавитьФайл.Видимость = (КоличествоФайлов = 0) И Не РежимЧтенияСообщения;
		Элементы.МК_ДобавитьГолосование.Видимость = Не ГолосованиеВключено;
		Элементы.МК_Доступ.Видимость = ВозможноВедениеРабочейГруппы() И Не РежимЧтенияСообщения И Не ТемаРабочаяГруппа;
		
		// Закладки
		Элементы.Страницы.ОтображениеСтраниц = ОтображениеСтраницФормы.Нет;
		Элементы.МК_ГруппаПереключателиСтраниц.Видимость = ОтображатьЗакладки;
		
		Элементы.МК_КнопкаСтраницаСообщение.Видимость = Элементы.СтраницаСообщение.Видимость;
		Элементы.МК_ГруппаФайлы.Видимость = Элементы.СтраницаФайлы.Видимость;
		Элементы.МК_КнопкаСтраницаГолосование.Видимость = Элементы.СтраницаГолосование.Видимость;
		Элементы.МК_КнопкаСтраницаРабочаяГруппа.Видимость = Элементы.СтраницаРабочаяГруппа.Видимость;
...
Процедура УстановитьРежимЧтения()
	...
	ИначеЕсли ЗначениеЗаполнено(Объект.Ссылка) Тогда
		...
		Если ДокументооборотПраваДоступаПовтИсп.ВключеноИспользованиеПравДоступа() Тогда
			...
			ПраваПоОбъекту = ДокументооборотПраваДоступа.ПраваПользователяПоОбъекту(
				Объект.Ссылка, Пользователи.ТекущийПользователь());
		...
		Иначе
			...
			Если Не (Объект.Автор = Пользователи.ТекущийПользователь()
	...
	Если УстановитьРежимЧтения Тогда
		...
		Если ЭтоМобильныйКлиент Тогда
			МК_КлиентСервер.АдаптироватьHtmlПодЭкранМобильногоПриНеобходимости(ТекстСообщенияHTML);
		КонецЕсли;
...
Процедура ВыбратьРодителя()
	...
	ОткрытьФорму("Справочник.СообщенияОбсуждений.ФормаВыбора", ПараметрыФормы, ЭтотОбъект);
...
Процедура МК_НастроитьЭлементыФормы()
	...
	Элементы.МК_ОбщаяГруппа.Видимость = Истина;
	Элементы.Переместить(Элементы.ГруппаТема, Элементы.МК_ОбщаяГруппа);
	Элементы.Переместить(Элементы.МК_ГруппаПереключателиСтраниц, Элементы.МК_ОбщаяГруппа);
	Элементы.Переместить(Элементы.Страницы, Элементы.МК_ОбщаяГруппа);
	Элементы.Переместить(Элементы.МК_НижнийБлок, Элементы.МК_ОбщаяГруппа);
	
	Если Элементы.ФормаЗаписать.Видимость Тогда
		// Если не режим чтения в ком. панели отображаем кнопку записать.
	...
	Иначе
		// Отображаем кнопку ответить
		Элементы.ФормаОтветитьНаСообщение.Отображение = ОтображениеКнопки.Картинка;
		Элементы.ФормаОтветитьНаСообщение.Картинка = БиблиотекаКартинок.МК_КомандаОтветить;
		Элементы.ФормаЗаписать.ПоложениеВКоманднойПанели = ПоложениеКнопкиВКоманднойПанели.ВКоманднойПанели;
	КонецЕсли;
	
	МК_ПоложениеВКоманднойПанели = ПоложениеКнопкиВКоманднойПанели.ВДополнительномПодменю;

	Элементы.ФормаУстановитьПометкуУдаления.ПоложениеВКоманднойПанели = МК_ПоложениеВКоманднойПанели;
	Элементы.ОтправитьОбъекты.ПоложениеВКоманднойПанели = МК_ПоложениеВКоманднойПанели;
	
	Элементы.МК_КомандыТемы.Видимость = Истина;
	Элементы.Переместить(Элементы.ЗакрытьТему, Элементы.МК_КомандыТемы);
	Элементы.Переместить(Элементы.ОткрытьТему, Элементы.МК_КомандыТемы);
	
	Элементы.МК_НижнийБлок.Видимость = Истина;
	
	Элементы.ГиперссылкаНеУказаноНаЧтоОтвечаем.Шрифт = ШрифтыСтиля.ОбычныйШрифтТекста;
	
	Если Элементы.ТекстСообщенияHTML.Видимость Тогда
		МК_КлиентСервер.АдаптироватьHtmlПодЭкранМобильногоПриНеобходимости(ТекстСообщенияHTML);
	КонецЕсли;
	
	// Сообщение
	МК_КлиентСервер.ОформитьКнопкуФильтра(МК_ЭлементыСтиля, Элементы.ЦитироватьИсходноеСообщение, Ложь, Ложь);
	
	Элементы.ГруппаНаЧтоОтвечаем.Поведение = ПоведениеОбычнойГруппы.Обычное;
	Элементы.ГруппаНаЧтоОтвечаем.ОтображатьЗаголовок = Ложь;
	Элементы.ГруппаНаЧтоОтвечаем.Отображение = ОтображениеОбычнойГруппы.Нет;
	...
	// Голосование
	Элементы.ВариантыГолосования.Шапка = Ложь;
	Элементы.ВариантыГолосованияНомерСтроки.Ширина = 1;
	...
	Элементы.РабочаяГруппаТаблицаУчастник.Ширина = 20;
	
	УстановитьВидимостьЭлементов();
	
	МК_ПереключитьНаСтраницу(ЭтотОбъект, Элементы.СтраницаСообщение, Элементы.МК_КнопкаСтраницаСообщение);

КонецПроцедуры

&НаКлиенте
Процедура МК_ПоказатьСкрытьФайлыПродолжение()
	...
	МК_КлиентСервер.ОформитьПроизвольныйЭлементПоСостоянию(
		МК_ЭлементыСтиля, Элементы.МК_ПоказатьСкрытьФайлы, Элементы.Файлы.Видимость);
...
Процедура МК_ОбновитьЗаголовокФайлов(Форма)
	
	Форма.Элементы.МК_ФайлыЗаголовок.Заголовок = 
		СтрШаблон(
			НСтр("ru = 'Файлы (%1)'"),
			Формат(Форма.КоличествоФайлов, "ЧН=0; ЧГ="));
...
&НаКлиенте
Процедура МК_ПоказатьНаЧтоОтвечаемПродолжение()
	
	Элементы.ГруппаНаЧтоОтвечаем.Видимость = Не Элементы.ГруппаНаЧтоОтвечаем.Видимость;
	МК_КлиентСервер.ОформитьКнопкуСворачиваемойГруппы(
			МК_ЭлементыСтиля, Элементы.МК_ПоказатьНаЧтоОтвечаем, Элементы.ГруппаНаЧтоОтвечаем.Видимость, Ложь);
...
Процедура МК_СброситьОформлениеПереключателейСтраниц(Форма)
	...
	МК_КлиентСервер.ОформитьКнопкуФильтра(
		Форма.МК_ЭлементыСтиля, Форма.Элементы.МК_КнопкаСтраницаСообщение, Истина, Ложь);
		
	МК_КлиентСервер.ОформитьКнопкуФильтра(
		Форма.МК_ЭлементыСтиля, Форма.Элементы.МК_КнопкаСтраницаГолосование, Истина, Ложь);
