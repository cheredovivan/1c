...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если НЕ Параметры.Свойство("СписокКатегорий") Тогда
		Если ТипЗнч(Параметры.ОбъектыДляКатегорий) = Тип("Массив")
			И Параметры.ОбъектыДляКатегорий.Количество() = 1 Тогда
			МассивКатегорий = РаботаСКатегориямиДанных.СписокКатегорийОбъекта(Параметры.ОбъектыДляКатегорий[0]);
...
#Область ОбработчикиСобытийЭлементовТаблицыФормыВыбранныеКатегорий
...
Процедура ЗаписатьКатегорииУОбъектов(Объекты, СписокКатегорий, Замена)
	...
	Попытка
		Для Каждого Объект Из Объекты Цикл
			...
			ПраваСтруктура = ДокументооборотПраваДоступа.ПраваПользователяПоОбъекту(Объект.Значение);
			
			ЕстьПравоИзмененияКатегорий = Ложь;
			
			Если ПраваСтруктура.Изменение Тогда      
				
				ЕстьПравоИзмененияКатегорий = Истина;
				
				Если ДелопроизводствоКлиентСервер.ЭтоДокументПредприятия(Объект.Значение) 
					И Не Делопроизводство.ПолеДокументаДоступноПоСостоянию("Категории", Объект.Значение) Тогда
					ЕстьПравоИзмененияКатегорий = Ложь;
				КонецЕсли;
					
			КонецЕсли;		
					
			Если ЕстьПравоИзмененияКатегорий Тогда
...
Процедура НастроитьЭлементыФормыДляМобильногоУстройства()
	...
	Элементы.ДобавитьКСпискуВыбранных.Заголовок = "Добавить к списку выбранных";
	Элементы.Переместить(Элементы.ДобавитьКСпискуВыбранных, Элементы.ДеревоКатегорий.КонтекстноеМеню, Элементы.ДеревоКатегорий.КонтекстноеМеню);
