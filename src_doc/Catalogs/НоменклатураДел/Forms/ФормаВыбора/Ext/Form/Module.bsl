...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.Свойство("ФормаДокумента") И ЗначениеЗаполнено(Параметры.ФормаДокумента) Тогда 
		...
		ФормыДокументов.Добавить(Параметры.ФормаДокумента);
		ФормыДокументов.Добавить(Перечисления.ВариантыФормДокументов.БумажнаяИлиЭлектронная);
		Список.Параметры.УстановитьЗначениеПараметра("ФормыДокументов", ФормыДокументов);
	КонецЕсли;
...
Процедура УстановитьОтборСписка()
	...
	Если ЗначениеЗаполнено(Подразделение) Тогда 
		...
		// Если для текущего подразделения нет раздела, то даем списывать ближайшее в родительское
		ПустоеИПодразделения = Делопроизводство.ПодразделенияРазделовВключаяРодительскиеИПустое(
			Подразделение, ПоказыватьДелаЗа);

		ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбора(Группа,
			"Раздел.Подразделение",
			ПустоеИПодразделения,
			ВидСравненияКомпоновкиДанных.ВСписке);
