...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не ВстроеннаяПочтаСервер.ВстроеннаяПочтаИспользуется() Тогда
		...
		Отказ = Истина;
		ТекстСообщения = НСтр("ru = 'Для создания правила обработки писем требуется включить персональную настройку встроенной почты.'");
...
Процедура ПередЗаписьюНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	Для Каждого Строка Из ТаблицаУсловия Цикл
		...
		Если Строка.Используется Тогда
			...
			Если ЗначениеЗаполнено(ТекущийОбъект.Описание) Тогда
				ТекущийОбъект.Описание = ТекущийОбъект.Описание + "; ";
			КонецЕсли;			
			ТекущийОбъект.Описание = ТекущийОбъект.Описание 
			+ Строка(Строка.ВидУсловия) + " " + Строка.ПредставлениеЗначенияУсловия;	
...
Процедура ЗначенияУсловияНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ИначеЕсли ТекущийВидУсловия = ПредопределенноеЗначение("Перечисление.ВидыУсловийОтбораИсходящихПисем.ТолькоДляПользователей") Тогда
		...
		ПараметрыФормы = Новый Структура;
		ПараметрыФормы.Вставить("ЗаголовокФормы", НСтр("ru = 'Выбор сотрудников'"));
