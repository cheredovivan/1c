...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Элементы.ПоказыватьРолиПомеченныеНаУдаление1.Пометка = ПоказыватьРолиПомеченныеНаУдаление;
	Элементы.ПоказыватьНедействительныхИсполнителей.Пометка = ПоказыватьНедействительныхИсполнителей;
	Элементы.ПоказыватьНедействительныхИсполнителей1.Пометка = ПоказыватьНедействительныхИсполнителей;
	
	ПоказатьПомеченныеРоли();
	ПоказатьНедействительныхПользователей();    
	
	Если РольДоступна("ДобавлениеИзменениеИсполнителейРолейПоОбъектамАдресации")
		Или РольДоступна("КонтрольЗадачИБизнесПроцессов")
		Или РольДоступна("УправлениеБизнесПроцессами")
		Или РольДоступна("ПолныеПрава") Тогда   
		
		Элементы.КомПанельОбщаяВерх.Видимость = Ложь; 
		ЭтаФорма.ПоложениеКоманднойПанели = ПоложениеКоманднойПанелиФормы.Нет;
		
	Иначе	
		
		Элементы.ГруппаКомПанелей.Видимость = Ложь;
	...
	МультиязычностьСервер.ПриСозданииНаСервере(ЭтотОбъект, , "Роли");
...
Процедура ИсполнителиПередНачаломДобавления(Элемент, Отказ, Копирование, Родитель, Группа, Параметр)
	...
	Если Элементы.Роли.ТекущиеДанные.ИспользуетсяСОбъектамиАдресации Или Копирование Тогда
...
Процедура ЗавершитьВыборИсполнителейСервер(ВыбранныеИсполнители, ДопПараметры) Экспорт
	...
	Для Каждого Эл Из ВыбранныеИсполнители Цикл
		...
		Запись = РегистрыСведений.ИсполнителиРолей.СоздатьМенеджерЗаписи();
