...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Параметры.Свойство("ОбъектКТ", ОбъектКТ);
	Если Не ЗначениеЗаполнено(ОбъектКТ) Тогда
		Возврат;
	КонецЕсли;
...
Процедура ПриОткрытии(Отказ)
	...
	Если Не ЗначениеЗаполнено(ОбъектКТ) Тогда
		ПоказатьПредупреждение( , НСтр("ru = 'Справочник открывается только в интерфейсе программы (Виды проектов)!'"));
		Отказ = Истина;
	КонецЕсли;
