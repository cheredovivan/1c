...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не ЕстьПравоНаИзменение(Объект.ОбъектКТ) Тогда
		...
		Если Сотрудники.ЭтоСотрудникПользователя(Объект.Ответственный) Тогда
			Элементы.Исполнено.ТолькоПросмотр = Ложь;
			Элементы.ФактическийСрок.ТолькоПросмотр = Ложь;
		КонецЕсли;
		
		Если Сотрудники.ЭтоСотрудникПользователя(Объект.Проверяющий) Тогда
...
Процедура ПослеЗаписи(ПараметрыЗаписи)
	...
	КлючеваяОперация = РаботаСПроектамиКлиентСервер.КлючеваяОперацияЗаписьКТ();
...
&НаКлиенте
Процедура ПроверяющийОбработкаВыбора(Элемент, ВыбранноеЗначение, ДополнительныеДанные, СтандартнаяОбработка)
...
Процедура УстановитьДоступность()
	...
	Если Объект.Проверено И Сотрудники.ЭтоСотрудникПользователя(Объект.Ответственный) Тогда
