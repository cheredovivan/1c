...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СпискиВыборка.Добавить("ОбъектКТНаИсполнение");
	...
	Если Не ЗначениеЗаполнено(ОтветственныйНаКонтроль) Тогда
		ОтветственныйНаКонтроль = Справочники.Сотрудники.ПустаяСсылка();
	КонецЕсли;
	Если Не ЗначениеЗаполнено(ОтветственныйНаПроверку) Тогда
		ОтветственныйНаПроверку = Справочники.Сотрудники.ПустаяСсылка();
	КонецЕсли;
...
Процедура ПриЗагрузкеДанныхИзНастроекНаСервере(Настройки)
	...
	ПараметрыОтбора.Вставить("ОбъектКТ", Настройки.Получить("ОбъектКТНаИсполнение"));
	...
	ОбщегоНазначенияДокументооборотКлиентСервер.ПоказатьСкрытьКнопкуОчисткиОтбора(Элементы.ОбъектКТНаИсполнение, ОбъектКТНаИсполнение);	
...
Процедура ОтветственныйНаПроверкуПриИзменении(Элемент)
	...
	Если Не ЗначениеЗаполнено(ОтветственныйНаПроверку) Тогда
		ОтветственныйНаПроверку = ПредопределенноеЗначение("Справочник.Сотрудники.ПустаяСсылка");
	КонецЕсли;
...
&НаКлиенте
Процедура ОтветственныйНаПроверкуНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	
	СтандартнаяОбработка = Ложь;
	СотрудникиКлиент.ВыбратьСотрудникаИзАдреснойКниги(Элемент, ОтветственныйНаПроверку);
...
Процедура ОтветственныйНаКонтрольПриИзменении(Элемент)
	...
	Если Не ЗначениеЗаполнено(ОтветственныйНаКонтроль) Тогда
		ОтветственныйНаКонтроль = ПредопределенноеЗначение("Справочник.Сотрудники.ПустаяСсылка");
	КонецЕсли;
...
&НаКлиенте
Процедура ОтветственныйНаКонтрольНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	
	СтандартнаяОбработка = Ложь;
	СотрудникиКлиент.ВыбратьСотрудникаИзАдреснойКниги(Элемент, ОтветственныйНаКонтроль);
...
Процедура ОбъектКТНаИспонениеПриИзменении(Элемент)
	...
	ПараметрыОтбора = Новый Соответствие;
	Параметрыотбора.Вставить("ОбъектКТ", ?(ОбъектКТНаИсполнение = Неопределено,
										ПредопределенноеЗначение("Справочник.Проекты.ПустаяСсылка"),
										ОбъектКТНаИсполнение));
	
	УстановитьОтборСписка(НаИсполнение, ПараметрыОтбора);		
	
	ОбщегоНазначенияДокументооборотКлиентСервер.ПоказатьСкрытьКнопкуОчисткиОтбора(Элемент, ОбъектКТНаИсполнение);
...
&НаКлиенте
Процедура ОбновитьСписокНаИсполнение(Команда)
...
Процедура СформироватьСпискиВыбораОбъектовКТ(СпискиВыбора)
	...
	Для Каждого Список Из СпискиВыбора Цикл
		...
		Если Список.Значение = "ОбъектКТНаИсполнение" Тогда
	...
	Запрос.УстановитьПараметр("Ответственный", Сотрудники.ОсновнойСотрудник());
