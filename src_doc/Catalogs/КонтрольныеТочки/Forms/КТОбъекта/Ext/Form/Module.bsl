...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ЭтоОтветственныйСотрудник = Сотрудники.ЭтоСотрудникТекущегоПользователя(
		КонтрольныеТочки.ОтветственныйОбъектаКТ(ОбъектКТ));
	Если Не ЕстьПравоНаИзменениеПроекта
		Или Не ИспользоватьВидыПроектов
		Или Не ЭтоОтветственныйСотрудник Тогда
		Элементы.ФормаЗаполнитьПоШаблону.Видимость = Ложь;
	КонецЕсли;
		
	Если ЕстьПравоНаИзменениеПроекта
		И ИспользоватьВидыПроектов
		И ЭтоОтветственныйСотрудник
...
Процедура ОтборОтветственныйПриИзменении(Элемент)
	...
	Если Не ЗначениеЗаполнено(Ответственный) Тогда
		Ответственный = ПредопределенноеЗначение("Справочник.Сотрудники.ПустаяСсылка");
	КонецЕсли;
...
Процедура ОтборОтветственныйНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	СтандартнаяОбработка = Ложь;
	СотрудникиКлиент.ВыбратьСотрудникаИзАдреснойКниги(Элемент, Ответственный);

КонецПроцедуры

&НаКлиенте
Процедура ОтборОтветственныйОбработкаВыбора(Элемент, ВыбранноеЗначение, ДополнительныеДанные, СтандартнаяОбработка)
...
Процедура СписокВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	КлючеваяОперация = РаботаСПроектамиКлиентСервер.КлючеваяОперацияОткрытиеКТ();
	ОценкаПроизводительностиКлиент.ЗамерВремени(КлючеваяОперация);
	
	ПараметрыФормы = Новый Структура;
	ПараметрыФормы.Вставить("Ключ", ДанныеСтроки.Ссылка);
	ОткрытьФорму("Справочник.КонтрольныеТочки.Форма.ФормаЭлемента", ПараметрыФормы);
...
Процедура СоздатьКТ(Копирование)
	...
	ОткрытьФорму("Справочник.КонтрольныеТочки.Форма.ФормаЭлемента", ПараметрыФормы, ЭтотОбъект);
...
Процедура ЗаполнитьПоШаблонуНаСервере()
	...
	Пока Выборка.Следующий() Цикл
		...
		Если Не ЗначениеЗаполнено(КТ.Проверяющий) Тогда
			КТ.Проверяющий = Сотрудники.ОсновнойСотрудникПользователя(
...
Процедура ЗагрузитьКТИзXMLНаСервере(Знач АдресВременногоХранилища, Знач ОбъектКТ)
	...
	Пока ЧтениеXML.Прочитать() Цикл
		...
		Если ЧтениеXML.ТипУзла = ТипУзлаXML.НачалоЭлемента И ЧтениеXML.ЛокальноеИмя = "ГруппаКТ" Тогда
			...
			СсылкаСотрудника = Справочники.Сотрудники.ПолучитьСсылку(ИД);
			СсылкаПользователя = Справочники.Пользователи.ПолучитьСсылку(ИД);
			
			Если ОбщегоНазначения.СсылкаСуществует(СсылкаСотрудника) Тогда
				НоваяГруппа.Ответственный = СсылкаСотрудника;
			Иначе
				НоваяГруппа.Ответственный = 
					Сотрудники.ОсновнойСотрудникПользователя(СсылкаПользователя);
			КонецЕсли;
		...
		ИначеЕсли ЧтениеXML.ТипУзла = ТипУзлаXML.НачалоЭлемента И ЧтениеXML.ЛокальноеИмя = "КТ" Тогда
			...
			Если ОбщегоНазначения.СсылкаСуществует(СсылкаСотрудника) Тогда
				НоваяКТ.Ответственный = СсылкаСотрудника;
			Иначе
				НоваяКТ.Ответственный = 
					Сотрудники.ОсновнойСотрудникПользователя(СсылкаПользователя);
			КонецЕсли;
			...
			Если ОбщегоНазначения.СсылкаСуществует(СсылкаСотрудника) Тогда
				НоваяКТ.Проверяющий = СсылкаСотрудника;
			Иначе
				НоваяКТ.Проверяющий = 
					Сотрудники.ОсновнойСотрудникПользователя(СсылкаПользователя);
			КонецЕсли;
...
Функция ИзменитьГруппуКТ(Знач МассивКТ, Знач НоваяГруппаКТ)
	...
	Для Каждого КонтрольнаяТочка Из МассивКТ Цикл
		Если Не ДокументооборотПраваДоступа.ПраваПользователяПоОбъекту(КонтрольнаяТочка).Изменение Тогда 
