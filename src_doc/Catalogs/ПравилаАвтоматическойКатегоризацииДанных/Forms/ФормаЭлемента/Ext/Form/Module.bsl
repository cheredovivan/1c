...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Объект.Ссылка.Пустая() Тогда
		...
		Для Каждого Тип Из Перечисления.ТипыОбъектов Цикл    
			...
			Если Тип = Перечисления.ТипыОбъектов.Контрагенты
				Или Тип = Перечисления.ТипыОбъектов.ДействиеЗадачи 
				Или Тип = Перечисления.ТипыОбъектов.Задача 
				Или Тип = Перечисления.ТипыОбъектов.ЗадачиПроцессов 
				Или Тип = Перечисления.ТипыОбъектов.Пользователи
				Или Тип = Перечисления.ТипыОбъектов.НаименованияОписанияЗадач Тогда
				Продолжить;
			КонецЕсли;
...
Процедура ПередЗаписьюНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	Если НЕ ЗначениеЗаполнено(СписокДопустимыхТипов) Тогда
		...
		ОбщегоНазначения.СообщитьПользователю(Текст,, "ТипыДанныхДляПравила",, Отказ);
	КонецЕсли;
	...
	Если Объект.Условия.Количество() = 0 Тогда
		...
		ОбщегоНазначения.СообщитьПользователю(Текст,, "Объект.Условия",, Отказ);	
	КонецЕсли;
...
#Область ОбработчикиСобытийЭлементовТаблицыФормыТипыДанныхДляПравила
...
Процедура НастроитьФормуДляМобильногоПриНеобходимости()
	...
	Элементы.Переместить(Элементы.Используется, Элементы.ГруппаАвторИДата);
	
	Элементы.ГруппаКатегория.ОтображатьЗаголовок = Ложь;
	
	Элементы.ПоКакимУсловиямМК.Видимость = Истина;
	Элементы.Переместить(Элементы.Условия, Элементы.ПоКакимУсловиямМК);
	Элементы.Условия.Шапка = Ложь;
	Элементы.ТекстУсловия.Видимость = Ложь;
	
	Элементы.ДляКакихДанныхМК.Видимость = Истина;
	Элементы.Переместить(Элементы.ТипыДанныхДляПравила, Элементы.ДляКакихДанныхМК);
	
	Элементы.Категория.ПоложениеЗаголовка = ПоложениеЗаголовкаЭлементаФормы.Лево;
	Элементы.Используется.ОтображениеПодсказки = ОтображениеПодсказки.Нет;
	
	Элементы.ТипыДанныхДляПравила.ИспользованиеТекущейСтроки = ИспользованиеТекущейСтрокиТаблицы.Выбор;
