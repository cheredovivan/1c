...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ЗначениеЗаполнено(Объект.ВладелецФайла) Тогда
		Если ТипЗнч(Объект.ВладелецФайла) = Тип("СправочникСсылка.ВидыДокументов") Тогда
			ИмяТипаВладельцаФайла = НСтр("ru = 'Документ'");
		КонецЕсли;
	КонецЕсли;
	...
	ИмяТипаДокументыПредприятия = Метаданные.Справочники.ВидыДокументов.ПолноеИмя();
...
Процедура ПередЗаписьюНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	ДвоичныеДанныеФайла = АвтозаполнениеШаблоновФайловВызовСервера.ПолучитьДвоичныеДанныеФайла(ТекущийОбъект.ШаблонФайла);
		...
		Если Стр <> Неопределено Тогда 
			Для Каждого НастройкаЗамены из ТекущийОбъект.ДанныеДляАвтозаполнения Цикл
				...
				ИначеЕсли СтрЧислоВхождений(Стр,НастройкаЗамены.ЗаменяемаяСтрока) = 0 Тогда
					ОписаниеОшибкиСтроки = ОписаниеОшибкиСтроки + Символы.ВК + НастройкаЗамены.ЗаменяемаяСтрока + Символы.ВК;
...
Процедура ВладелецФайлаНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	СтандартнаяОбработка = Ложь;
	ВыбранныйТип = ИмяТипаДокументыПредприятия;
...
Процедура ДанныеДляАвтозаполненияВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	ПараметрыОткрытияФормы.Вставить("ТипДокумента", НСтр("ru = 'Документ'"));
...
Функция ПолучитьДвоичныеДанные(Файл)
	...
	Возврат АвтозаполнениеШаблоновФайловВызовСервера.ПолучитьДвоичныеДанныеФайла(Файл);
...
Процедура ПроверитьСкрипт(Команда)
	...
	ОчиститьСообщения();
	ТекущиеДанные = Элементы.ДанныеДляАвтозаполнения.ТекущиеДанные;
	Если ТекущиеДанные = Неопределено Тогда
		Возврат;
	КонецЕсли;
	...
	Если Не ЗначениеЗаполнено(ТекущиеДанные.ВыражениеОбработкиРезультатаЗамены) Тогда 
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ru = 'Выберите скрипт для проверки!'"),,
			"ДокументДляПроверки");
		Возврат;
	КонецЕсли;
	
	ПолеФайла = ТекущиеДанные.ТермДляЗамены;
	Если Не ЗначениеЗаполнено(ПолеФайла) Тогда
		ПолеФайла = ТекущиеДанные.ЗаменяемаяСтрока;
	КонецЕсли;	
	
	РезультатОбработки = ПроверитьСкриптНаСервере(ДокументДляПроверки, 
		ТекущиеДанные.ВыражениеОбработкиРезультатаЗамены,
		ПолеФайла, ТекущиеДанные.ПолучитьИдентификатор());
...
Функция ПроверитьСкриптНаСервере(ДокументДляПроверки, Скрипт, ПолеФайла, Идентификатор)
	...
	Исключение
		...
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			СтрокаОшибки,,
			"ДанныеДляАвтозаполнения[" + Формат(Идентификатор, "ЧГ=0") + "].ВыражениеОбработкиРезультатаЗамены");
