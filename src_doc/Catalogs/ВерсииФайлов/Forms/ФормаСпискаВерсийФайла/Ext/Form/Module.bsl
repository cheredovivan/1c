...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ВидимостьКомандыСравнить = 
		Не ОбщегоНазначенияКлиентСервер.ЭтоВебКлиент();
...
Функция СменитьАктивнуюВерсиюФайла(Версия, СтрокаОшибки = "")
	...
	ДанныеФайла = РаботаСФайламиВызовСервера.ДанныеФайла(, Версия);
	...
	Попытка
		...
		ТекстХранилище = РегистрыСведений.ТекстыВерсийФайлов.ПолучитьТекстХранилище(Версия);
		РегистрыСведений.ТекстыФайлов.ДобавитьЗапись(Версия.Владелец, ТекстХранилище);
		...
		Описание = СтрШаблон(НСтр("ru = 'Смена активной версии на версию %1'"), Версия);
		ПротоколированиеРаботыСотрудников.ЗаписатьИзменение(ФайлОбъект.Ссылка, Описание);
...
Процедура СписокВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	РаботаСФайламиКлиент.Открыть(
		РаботаСФайламиВызовСервера.ДанныеФайлаДляОткрытия(Неопределено, ТекущиеДанные.Ссылка, УникальныйИдентификатор, , , НеПолучатьВизуализациюЭП),
