...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ЗначениеЗаполнено(Параметры.ЗначениеКопирования) Тогда 
		ЗаполнитьПоЗначениюКопирования();
	...
	Если Не ЗначениеЗаполнено(Объект.Адресат) Тогда
		Объект.Адресат = ПредопределенноеЗначение("Справочник.Сотрудники.ПустаяСсылка");
	КонецЕсли;
	...
	УстановитьЗаголовокУчастниковРабочейГруппы();
	...
	// Работа со сторонами
	РаботаСПодписямиДокументов.ЗаполнитьТаблицуСторон(ЭтаФорма, Истина);
	Если ВидДокументаКэш.ВестиУчетСторон И Не ЗначениеЗаполнено(Объект.Ссылка) Тогда
		ОбновитьРеквизитыСторон();
	КонецЕсли;
	
	РаботаСШаблонамиДокументовСервер.УстановитьТипыМестаИОтветственногоЗаХранение(ЭтаФорма);
	ОбновитьПредставлениеВажности(Объект.Важность, ЭтотОбъект);
...
Процедура ПриОткрытии(Отказ)
	...
	ШаблоныДокументовКлиентЛокализация.ПриОткрытииШаблонДокумента(ЭтотОбъект, Отказ);
...
Процедура ПриЧтенииНаСервере(ТекущийОбъект)
	...
	ШаблоныДокументовЛокализация.ПриЧтенииНаСервере_ФормаЭлемента(ТекущийОбъект, ЭтотОбъект);
...
Процедура ПередЗаписьюНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	ТекущийОбъект.ДополнительныеСвойства.Вставить("ВестиУчетСторон", ВидДокументаКэш.ВестиУчетСторон);
		...
		Пока Индекс >= 0 Цикл
			...
			Попытка
				...
				РаботаСФайламиВызовСервера.ОбновитьВерсиюВФайле(
					Приемник, Версия, ТекстХранилище); 
					
				Если ЗначениеЗаполнено(ФайлТаблицы.РольФайлаСсылка) Тогда
					РегистрыСведений.РолиФайловДокументов.ЗаписатьРольФайла(Приемник, ФайлТаблицы.РольФайлаСсылка); 	
				КонецЕсли;	
...
Процедура ПередЗаписью(Отказ, ПараметрыЗаписи)
	...
	ОбщегоНазначенияДокументооборотКлиент.УдалитьДублиСтрокТаблицы(Объект.РабочаяГруппаДокумента, "Участник");	
...
Процедура ПослеЗаписи(ПараметрыЗаписи)
	...
	Если ПараметрыЗаписи.ЭтоНовыйОбъект
			Или ПараметрыЗаписи.Свойство("ИзменилосьНаименование") И ПараметрыЗаписи.ИзменилосьНаименование Тогда
		Оповестить("ИзмененШаблонДокумента", СтруктураОповещения);
	КонецЕсли;
...
Процедура АдресатНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	СтандартнаяОбработка = Ложь;
	СотрудникиКлиент.ВыбратьСотрудникаИзАдреснойКниги(Элемент, Объект.Адресат);
...
&НаКлиенте
Процедура ТематикаНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	
	СтандартнаяОбработка = Ложь;
	ПараметрыОткрытия = Новый Структура;
	ПараметрыОткрытия.Вставить("ВидДокумента", Объект.ВидДокумента);
	ПараметрыОткрытия.Вставить("ТекущаяСтрока", Объект.Тематика);
	
	ОткрытьФорму("Справочник.ТематикиДокументов.ФормаВыбора", ПараметрыОткрытия, Элемент);
...
Процедура ФормаДокументаОбработкаВыбора(Элемент, ВыбранноеЗначение, ДополнительныеДанные, СтандартнаяОбработка)
	...
	ШаблоныДокументовКлиентЛокализация.ФормаДокументаОбработкаВыбора_ФормаЭлемента(
		Элемент, ВыбранноеЗначение, ДополнительныеДанные, СтандартнаяОбработка, ЭтотОбъект);
...
Процедура ОтветственныйЗаХранениеОчистка(Элемент, СтандартнаяОбработка)
	...
	СтандартнаяОбработка = Ложь;
	Объект.ОтветственныйЗаХранение = ПредопределенноеЗначение("Справочник.Сотрудники.ПустаяСсылка");
...
Процедура АдресатВходящейКорреспонденцииОбработкаВыбора(Элемент, ВыбранноеЗначение, ДополнительныеДанные, СтандартнаяОбработка)
	...
	СотрудникиКлиент.СотрудникОбработкаВыбора(
		Объект, "АдресатВходящейКорреспонденции", ВыбранноеЗначение, СтандартнаяОбработка);
	
КонецПроцедуры

&НаКлиенте
Процедура АдресатОбработкаВыбора(Элемент, ВыбранноеЗначение, ДополнительныеДанные, СтандартнаяОбработка)
	
	СотрудникиКлиент.СотрудникОбработкаВыбора(
		Объект, "Адресат", ВыбранноеЗначение, СтандартнаяОбработка);
...
Процедура ОтветственныйЗаХранениеОбработкаВыбора(Элемент, ВыбранноеЗначение, ДополнительныеДанные, СтандартнаяОбработка)
	...
	СотрудникиКлиент.СотрудникОбработкаВыбора(
		Объект, "ОтветственныйЗаХранение", ВыбранноеЗначение, СтандартнаяОбработка);
...
Процедура ОтветственныйОбработкаВыбора(Элемент, ВыбранноеЗначение, ДополнительныеДанные, СтандартнаяОбработка)
	...
	СотрудникиКлиент.СотрудникОбработкаВыбора(
		Объект, "Ответственный", ВыбранноеЗначение, СтандартнаяОбработка);
...
Процедура ФайлыВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	Если Поле = Элементы.Условие Тогда
		Возврат;
	КонецЕсли;
	...
	ИначеЕсли Поле = Элементы.ФайлыРольФайла Тогда 
		...
		ПараметрыОткрытия = Новый Структура("ВидДокумента", Объект.ВидДокумента);
...
Процедура ПродолжитьАвтоЗаполнение(Строка)
	...
	ПараметрыФормы.Вставить("ВестиУчетТоваровИУслуг", ВидДокументаКэш.ВестиУчетТоваровИУслуг);
...
Процедура ВложенияСортироватьПоИмени(Команда)
	...
	РаботаСФайламиВызовСервера.СортироватьФайлыПоИмениДляШаблонаВидаДокумента(Объект.Ссылка);
...
Процедура ОдинКонтрагентПродолжение(Результат, Параметры)Экспорт 
	...
	Если Объект.Контрагенты.Количество() > 0 Тогда
		...
		Если ВидДокументаКэш.ЯвляетсяИсходящейКорреспонденцией Тогда
			Объект.Способ = Строка.Способ;
		КонецЕсли;
	Иначе	
		Объект.Контрагент = Неопределено;
		Объект.КонтактноеЛицо = Неопределено;
		Если ВидДокументаКэш.ЯвляетсяИсходящейКорреспонденцией Тогда
			Объект.Способ = Неопределено;
		КонецЕсли;
	КонецЕсли;
...
Процедура ЗаполнитьПравило(Команда)
	...
	#Если ВебКлиент Тогда
		Сообщение = НСтр("ru = 'В web-клиенте нельзя осуществлять заполнение правил.'");
		ПоказатьПредупреждение(, Сообщение);
		Возврат;
	#КонецЕсли
...
Процедура ОбновитьВидимостьРеквизитов()
	...
	Элементы.ГруппаЕдиницыИзмеренияСрока.Видимость = ПараметрыФункциональныхОпций.ВидДокумента.ИспользоватьСрокИсполнения;
	Если ПолучитьФункциональнуюОпцию("ИспользоватьГрафикиРаботы") Тогда
		
		Элементы.ЕдиницыИзмерения.Видимость = Ложь;
		
		ВариантыСлова = КалендарныеГрафикиДокументооборот.СклонениеДней(Объект.ДлительностьИсполнения);
		Элементы.СрокИсполненияУказанВРабочихДнях.СписокВыбора[0].Представление = ВариантыСлова.Календарные;
		Элементы.СрокИсполненияУказанВРабочихДнях.СписокВыбора[1].Представление = ВариантыСлова.Рабочие;
		
	Иначе
		Элементы.СрокИсполненияУказанВРабочихДнях.Видимость = Ложь;
	КонецЕсли;
	
	// Доп. реквизиты
	СписокДопРеквизитов = РаботаСШаблонамиДокументовСервер.ПолучитьНаборДопРеквизитовДокумента(
		"ДокументыПредприятия", Объект.ВидДокумента, Объект);
	...
	Элементы.ДопРеквизиты.Видимость = КоличествоСвойств > 0;
	
	Если ВидДокументаКэш.ВестиУчетПоОрганизациям И Не ВидДокументаКэш.ВестиУчетСторон Тогда
	...
	Элементы.НоменклатураДел.Видимость = ВидДокументаКэш.ВестиУчетПоНоменклатуреДел;
	Элементы.СтраницаТовары.Видимость = ВидДокументаКэш.ВестиУчетТоваровИУслуг;
	Элементы.Адресат.Видимость = ВидДокументаКэш.ВестиУчетПоАдресатам;
	...
	Если Объект.ФормаДокумента = ПредопределенноеЗначение("Перечисление.ВариантыФормДокументов.Электронная") Тогда 
	...
	Если ВидДокументаКэш.УчитыватьМестоХранения <> ВариантНеУчитывать Тогда
		Элементы.МестоХранения.Видимость = ПоказыватьПоляХранения;
	Иначе
		Элементы.МестоХранения.Видимость = Ложь;
	КонецЕсли;
	
	Если ВидДокументаКэш.УчитыватьОтветственногоЗаХранение <> ВариантНеУчитывать Тогда
		Элементы.ОтветственныйЗаХранение.Видимость = ПоказыватьПоляХранения;
	Иначе
		Элементы.ОтветственныйЗаХранение.Видимость = Ложь;
	КонецЕсли;
	
	Элементы.Тематика.Видимость = ВидДокументаКэш.ВестиУчетПоТематикам;
	
	// Локализация
	Элементы.ВидОбращения.Видимость = ВидДокументаКэш.ЯвляетсяОбращениемОтГраждан;
	// Локализация Конец
	Элементы.АдресатВходящейКорреспонденции.Видимость = ВидДокументаКэш.ЯвляетсяВходящейКорреспонденцией;
	Элементы.СпособПолучения.Видимость = ВидДокументаКэш.ЯвляетсяВходящейКорреспонденцией;
	Элементы.СпособОтправки.Видимость = ВидДокументаКэш.ЯвляетсяИсходящейКорреспонденцией;
	Элементы.МногоКонтрагентов.Видимость = Не ВидДокументаКэш.ЯвляетсяВходящейКорреспонденцией;
	Элементы.Важность.Видимость = ВидДокументаКэш.ИспользоватьВажность;
...
Процедура УстановитьВидимостьКонтрагентов()
	...
	Элементы.ГруппаКонтрагент.Видимость = 
		ВидДокументаКэш.ВестиУчетПоКонтрагентам
...
Процедура ВыполнитьЗаполнениеПравилаПоОснованию(Строка)
	...
	Если Строка.Расширение = "doc" Или Строка.Расширение = "docx" Тогда
		...
		АвтозаполнениеШаблоновФайловКлиент.СоздатьЗаполнитьПравилоПоОснованию(Объект.ВидДокумента, ЭтотОбъект);
...
&НаСервере
Процедура УстановитьЗаголовокУчастниковРабочейГруппы()
...
&НаСервере
Процедура ЗаполнитьПоЗначениюКопирования()
	...
	Если ПодчиненныеФайлы.Количество() > 0 Тогда
		...
		Для Каждого ПодчиненныйФайл Из ПодчиненныеФайлы Цикл
			...
			МассивСтрок = ТаблицаПравилаАвтозаполнения.НайтиСтроки(Новый Структура("ШаблонФайла",
				ПодчиненныйФайл.Ссылка));
			...
			Если МассивРоли.Количество() <> 0 Тогда
				НоваяСтрока.РольФайлаСсылка = МассивРоли[0];
			КонецЕсли;	
...
Процедура НастроитьФормуДляМобильногоПриНеобходимости()
	...
	ВыровнятьЭлементДляМобильного(Элементы.Автор);
	ВыровнятьЭлементДляМобильного(Элементы.Содержание);
	ВыровнятьЭлементыГруппыДляМобильного(Элементы.ГруппаЗапрет);
	...
	ВыровнятьЭлементДляМобильного(Элементы.ВидДокумента);
	...
	Элементы.СтраницаКонтрагенты.ЦветФона = ЦветФонаГрупп;
	...
	Элементы.СтраницаТовары.ЦветФона = ЦветФонаГрупп;
	
	// Страница "Категории".
	Элементы.КатегорииПодобратьКатегории.Отображение = ОтображениеКнопки.Текст;
	Элементы.СтраницаКатегории.ЦветФона = ЦветФонаГрупп;
	
	// Страница "Доступ".
	ВыровнятьЭлементДляМобильного(Элементы.ЗапретитьАвтоматическоеРасширениеДоступа);
	Элементы.ЗапретитьАвтоматическоеРасширениеДоступа.ОтображениеПодсказки = ОтображениеПодсказки.Нет;
	Элементы.РабочаяГруппаДокумента.Шапка = Ложь;
	Элементы.РабочаяГруппаИконка.Видимость = Ложь;
	Элементы.РабочаяГруппаДокументаИзменение.Видимость = Ложь;
	Элементы.РабочаяГруппаДокументаПустаяКолонка.Видимость = Ложь;
	Элементы.СтраницаДоступКДокументу.ЦветФона = ЦветФонаГрупп;
