...
//  ДополнительныеСведения - Структура, будут заполнены в элементе справочника.
//  МЧД	- СправочникСсылка.МЧД003
// 
// Возвращаемое значение:
...
Функция ЗагрузитьЭлементИзФайлаОбмена(ДанныеДляЗагрузки, ТребуетсяПроверкаМЧДНаКлиенте, ЭтоРучноеСоздание, ОбновлятьСуществующий = Ложь,
	ДополнительныеСведения, МЧД = Неопределено, РезультатПроверкиНаСервере = Неопределено)
...
Функция ДанныеДоверенностиДляКонтейнера(Доверенность) Экспорт
	...
	Реквизиты = "ФайлМЧД, ДатаВыдачи, СрокДействия, НомерДоверенности, Доверители, Представители,  СтатусВРеестреФНС";
