...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Элементы.СертификатыФизическоеЛицо.Видимость И ДляПоискаФизическихЛиц.Количество() > 0 Тогда
		...
		ФизическиеЛицаСертификата = ЭлектроннаяПодписьСлужебный.ПолучитьФизическиеЛицаПоПолюСертификатаКомуВыдан(ДляПоискаФизическихЛиц);
		Для Каждого ТекущаяСтрока Из Сертификаты Цикл
			...
			МассивЗначений = ФизическиеЛицаСертификата.Получить(ТекущаяСтрока.КомуВыдан);
...
Процедура НачалоВыбораФизическогоЛица(СтандартнаяОбработка)
	...
	ФизическиеЛицаСертификата = Результат.ФизическиеЛица.Получить(ТекущиеДанные.КомуВыдан);
	
	СтандартнаяОбработка = Ложь;
	
	Если ФизическиеЛицаСертификата = Неопределено Тогда
	...
	Если ФизическиеЛицаСертификата.Количество() = 1 Тогда
		Если ТекущиеДанные.ФизическоеЛицо <> ФизическиеЛицаСертификата[0] Тогда
			ТекущиеДанные.ФизическоеЛицо = ФизическиеЛицаСертификата[0];
		Иначе
			ОбработкаВыбора = Новый ОписаниеОповещения("ПриЗакрытииФормыВыбораФизическогоЛица", ЭтотОбъект);
			ПараметрыФормы = Новый Структура;
			ПараметрыФормы.Вставить("РежимВыбора", Истина);
	...
	Отбор.ПравоеЗначение = ФизическиеЛицаСертификата;
