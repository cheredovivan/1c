...
Процедура ЗаполнитьПоставляемыйПрофиль(Знач Профиль, Знач ОбновитьГруппыДоступа) Экспорт
	...
	ДанныеПрофиля = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(
		Профиль, "ИдентификаторПоставляемыхДанных, ИмяПредопределенныхДанных");
	
	ПоставляемыеПрофили = УправлениеДоступомСлужебныйПовтИсп.ОписаниеПоставляемыхПрофилей();
	СвойстваПрофиля = ПоставляемыеПрофили.ОписанияПрофилей.Получить("" + ДанныеПрофиля.ИдентификаторПоставляемыхДанных);
	
	// Для Администратора исключение - у него ПоставляемыеПрофили.ОписанияПрофилей лежит имя, а не идентификатор:
	Если СвойстваПрофиля = Неопределено И ДанныеПрофиля.ИмяПредопределенныхДанных = "Администратор" Тогда
		СвойстваПрофиля = ПоставляемыеПрофили.ОписанияПрофилей.Получить(ДанныеПрофиля.ИмяПредопределенныхДанных);
	КонецЕсли;
