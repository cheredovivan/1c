...
Процедура ПриОткрытии(Отказ)
	...
	Если ПроцессЗадачаЗавершен Тогда 
		...
		ИначеЕсли ДействияКлиентСервер.ЭтоДействие(Объект.Предмет) Тогда 
			ТекстСообщения = НСтр("ru = 'Нельзя поставить на контроль завершенное или помещенное в историю действие.'");
		Иначе
...
Процедура ПередЗаписью(Отказ, ПараметрыЗаписи)
	...
	КлючеваяОперация = КонтрольКлиентСервер.КлючеваяОперацияПостановкаНаКонтроль();
	...
	Если Элементы.Добавить.Видимость = Истина Тогда
		ОбщегоНазначенияДокументооборотКлиент.УдалитьДублиСтрокТаблицы(Объект.Исполнители, "Исполнитель");
	КонецЕсли;
...
Процедура ПередЗаписьюНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	ТекущийОбъект.ДатаПостановкиНаКонтроль = ТекущаяДатаСеанса();
...
Процедура ВывестиПодписьЧислаДней()
	...
	Иначе	
		ЧислоДней = РазностьДат(ОбщегоНазначенияКлиент.ДатаСеанса(), Объект.СрокИсполнения);
...
Процедура МК_НастроитьЭлементыФормы()
	...
	Элементы.ИсполнителиИсполнитель.ПоложениеЗаголовка = ПоложениеЗаголовкаЭлементаФормы.Нет;
