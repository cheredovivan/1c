...
Процедура ЗаполнитьСписокКонтроля()	
	...
	ТекстЗапросаПоДействиям = ДействияСервер.ТекстЗапросаДействияПредмета(Предмет, Ложь);
	Если ЗначениеЗаполнено(ТекстЗапросаПоДействиям) Тогда 
		Запрос = Новый Запрос(ТекстЗапросаПоДействиям);
		Запрос.УстановитьПараметр("Предметы", Предметы);
	...
	ОбщегоНазначенияКлиентСервер.ДополнитьМассив(МассивПредметов, Предметы);
...
Процедура ТаблицаКонтроляПередУдалениемПродолжение(Результат, Параметры) Экспорт 
	...
	ПараметрыОповещения = КонтрольКлиентСервер.ПараметрыОповещения(ТекущиеДанные);
