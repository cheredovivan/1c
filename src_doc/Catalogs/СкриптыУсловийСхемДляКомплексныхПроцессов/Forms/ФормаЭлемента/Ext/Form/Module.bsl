...
Процедура ЗаполнитьКонтекстВыполнения()
	...
	СтрокаКонтекст.Представление = НСтр("ru = 'Результат'");
	СтрокаКонтекст.Тип = ПредставлениеТиповКонтекста(
		ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(Тип("Булево")));
	СтрокаКонтекст.ПолныйПуть = "Параметры.Результат";
...
Процедура ПроверитьВыражениеСервер()
	...
	Попытка
		РезультатВыполнения = 
			Справочники.СкриптыУсловийСхемДляКомплексныхПроцессов.ВычислитьВыражениеСкриптаУсловия(
			Объект.Выражение, Процесс.ПолучитьОбъект());
		ИтогПроверки = ?(РезультатВыполнения, НСтр("ru = 'Истина'"), НСтр("ru = 'Ложь'"));
	Исключение
		...
		ИтогПроверки = НСтр("ru = 'Ошибка.'") + Символы.ПС + Описание;
	КонецПопытки;
