...
Функция УдалитьФайл(ПутьКФайлу)
	Возврат ФайловыеФункции.УдалитьФайл(ПутьКФайлу);
КонецФункции
...
Процедура ПослеОбработкиФайлов(Результат, ДополнительныеПараметры) Экспорт
	...
	Если ЧислоУдаленныхФайлов > 0 И Не СкопироватьФайлыПередУдалением Тогда
		ЧислоУдаленныхФайлов = ЧислоУдаленныхФайлов - 1; // -1 т.к. там счетчик на 1 выше
	КонецЕсли;	
...
Процедура ЗаполнитьТаблицуЛишнихФайлов()
	...
	ТаблицаФайловНаДиске = РаботаСФайламиВызовСервера.ЛишниеФайлыНаДиске();
	ПутьКТому = СокрЛП(ФайловыеФункцииСлужебный.ПолныйПутьТома(ТомХраненияФайлов));
	...
	РаботаСФайламиВызовСервера.ЗаполнитьЛишниеФайлы(ТаблицаФайловНаДиске, ТомХраненияФайлов);
