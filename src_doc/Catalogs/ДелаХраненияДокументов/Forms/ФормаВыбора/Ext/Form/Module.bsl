...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	КонецЕсли;
	Если Параметры.Свойство("ГрифДоступа") И ЗначениеЗаполнено(Параметры.ГрифДоступа) Тогда 
		ПараметрыФормы.Вставить("ГрифДоступа", Параметры.ГрифДоступа);
	КонецЕсли;     
	
	Если ТипЗнч(Параметры.РежимВыбора) = Тип("Строка") Тогда
		РежимВыбораПриОткрытии = Параметры.РежимВыбора;
	КонецЕсли;	
	...
	Если Параметры.РежимВыбора = "ИзКарточкиДокумента" Тогда 
		...
		Иначе
			ПоказыватьТолькоПодходящиеДела = ХранилищеОбщихНастроек.Загрузить(
				"ФормаВыбораДелаХраненияДокументов", "ПоказыватьТолькоПодходящиеДела");
		КонецЕсли;
	...
	ИначеЕсли Параметры.РежимВыбора = "ИзПередачиДелВАрхив" 
		...
		КонецЕсли;
		Если ЗначениеЗаполнено(Параметры.Подразделение) Тогда 
			ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбора(Список.Отбор, "Подразделение", Параметры.Подразделение);
		КонецЕсли;
		...
		Если Параметры.Свойство("ФормаДокумента") И ЗначениеЗаполнено(Параметры.ФормаДокумента) Тогда 
			Список.Параметры.УстановитьЗначениеПараметра("ФормаДокументов", Параметры.ФормаДокумента);
		КонецЕсли; 
		
		Если Параметры.Свойство("ГрифДоступа") И ЗначениеЗаполнено(Параметры.ГрифДоступа) Тогда 
			Список.Параметры.УстановитьЗначениеПараметра("ГрифДоступа", Параметры.ГрифДоступа);
		КонецЕсли; 
...
Процедура ПоказыватьТолькоПодходящиеДелаПриИзменении(Элемент)
	...
	Иначе                   
		Если РежимВыбораПриОткрытии <> "Да" И РежимВыбораПриОткрытии <> "Yes" Тогда
			УстановитьНаСписокСтандартныйОтбор(ПараметрыФормы);
		Иначе	
			УстановитьНаСписокПустойОтбор(ПараметрыФормы);
		КонецЕсли;
	КонецЕсли;
...
Процедура УстановитьНаСписокСтандартныйОтбор(Параметры)
	...
	Если Параметры.Свойство("ФормаДокумента") И ЗначениеЗаполнено(Параметры.ФормаДокумента) Тогда 
		Список.ТекстЗапроса = Список.ТекстЗапроса + "
		|И СправочникДела.ФормаДокументов = &ФормаДокумента";
		Список.Параметры.УстановитьЗначениеПараметра("ФормаДокумента", Параметры.ФормаДокумента);
	КонецЕсли;
	
	Если Параметры.Свойство("ГрифДоступа") И ЗначениеЗаполнено(Параметры.ГрифДоступа) Тогда
		ГрифыДоступа = Новый СписокЗначений;
		ГрифыДоступа.Добавить(Параметры.ГрифДоступа);
		Если Параметры.РежимВыбора = "ИзКарточкиДокумента" Тогда
			ГрифыДоступа.Добавить(Справочники.ГрифыДоступа.ПустаяСсылка());
		КонецЕсли;
		Список.ТекстЗапроса = Список.ТекстЗапроса + "
		|И СправочникДела.НоменклатураДел.ГрифДоступа В (&ГрифыДоступа)";
		Список.Параметры.УстановитьЗначениеПараметра("ГрифыДоступа", ГрифыДоступа);
	КонецЕсли;
	...
	Если Параметры.Свойство("Подразделение") И ЗначениеЗаполнено(Параметры.Подразделение) Тогда
		...
		Если Параметры.РежимВыбора = "ИзКарточкиДокумента" Тогда
			...
			Пока ЗначениеЗаполнено(ТекущееПодразделение) Цикл
				...
				Запрос.УстановитьПараметр("ДатаРегистрации", ?(ЗначениеЗаполнено(Параметры.ДатаРегистрации),
					Параметры.ДатаРегистрации, ТекущаяДата()));	
...
&НаСервере
Процедура УстановитьНаСписокПустойОтбор(Параметры)
...
Процедура СохранитьНастройку()
	...
	ОбщегоНазначения.ХранилищеОбщихНастроекСохранить("ФормаВыбораДелаХраненияДокументов", "ПервыйЗапуск", Ложь);
	ОбщегоНазначения.ХранилищеОбщихНастроекСохранить("ФормаВыбораДелаХраненияДокументов", "ПоказыватьТолькоПодходящиеДела", ПоказыватьТолькоПодходящиеДела);
