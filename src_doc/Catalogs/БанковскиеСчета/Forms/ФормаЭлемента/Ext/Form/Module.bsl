...
Процедура ЗаполнитьТекстКорреспондента()
	...
	СтрКорреспондента = "";
	Если ТипЗнч(Владелец) = Тип("СправочникСсылка.Организации") Или ТипЗнч(Владелец) = Тип("СправочникСсылка.Контрагенты") Тогда
		СтрКорреспондента = Юрлица.ПредставлениеЮрлицаНаДату(Владелец, Объект.ДатаОткрытия);
	КонецЕсли;
