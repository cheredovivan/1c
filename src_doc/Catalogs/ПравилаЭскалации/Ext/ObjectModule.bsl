...
Процедура ОбработкаЗаполнения(ДанныеЗаполнения, ТекстЗаполнения, СтандартнаяОбработка)
		...
		Пока ОбрабатываемыеШаблоны.Количество() > 0 Цикл
			...
			Если ЗначениеЗаполнено(СхемаШаблона) Тогда
				...
				Для Каждого ИмяЭлементаИНастройка Из ДанныеПараметров.НастройкиЭлементов Цикл
					...
					ТипНастройки = ТипЗнч(НастройкаЭлемента);
					Если Не ОписаниеТиповНастроек.СодержитТип(ТипЗнч(НастройкаЭлемента))
						Или Не ЗначениеЗаполнено(НастройкаЭлемента) Тогда
						
						Продолжить;					
					КонецЕсли;
					Если ТипНастройки = Тип("СправочникСсылка.ШаблоныКомплексныхБизнесПроцессов") Тогда
						ОбрабатываемыеШаблоны.Добавить(НастройкаЭлемента);
					Иначе
						НайденныеШаблоны.Добавить(НастройкаЭлемента);
					КонецЕсли;
				КонецЦикла;				
	...
	Если НаправлениеЭскалации = Неопределено Тогда
		НаправлениеЭскалации = Справочники.Сотрудники.ПустаяСсылка();
	КонецЕсли;
...
Процедура ПередЗаписью(Отказ)
	...
	Если Не ЗначениеЗаполнено(ТипПредметаЭскалации) Тогда
		ТипПредметаЭскалации = Перечисления.ТипыПредметовЭскалации.Процессы;
	КонецЕсли;
	
	Если Обработки.Количество() <> 0 Тогда
		Если ЗначениеЗаполнено(Обработки[0].НастройкаОбработки) Тогда
			НастройкаОбработки = Обработки[0].НастройкаОбработки;
		КонецЕсли;	
	КонецЕсли;		
...
Процедура ОпределитьПриоритет()
	...
	Если Не ЗначениеЗаполнено(Приоритет) Тогда
		...
		Приоритет = Справочники.ПравилаЭскалации.МаксимальныйПриоритет(НастройкаОбработки) + 100;
		
	КонецЕсли;
	
	Если ДополнительныеСвойства.Свойство("ОтключитьПроверкуУникальностиПриоритета")
		Или ПараметрыСеанса.ЗагрузкаОбработанныхДанныхИзДругойСистемы Тогда
		Возврат;
	КонецЕсли;
	
	ПравилоСПриоритетом = Справочники.ПравилаЭскалации.ПравилоСПриоритетом(Ссылка, Приоритет);
