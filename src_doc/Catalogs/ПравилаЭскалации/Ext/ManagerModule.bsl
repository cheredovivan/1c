...
Процедура ЗадатьПриоритет(Знач ПравилоЭскалации, Знач Приоритет) Экспорт
	...
	Попытка
		...
		Блокировка.Добавить("Справочник.ПравилаЭскалации");
		Блокировка.Заблокировать();
		
		НастройкаОбработки = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ПравилоЭскалации, "НастройкаОбработки");
		
		Если ЗначениеЗаполнено(НастройкаОбработки) Тогда
			
			Если Приоритет < ГраницаДляДетальныхПравил() Тогда
				Приоритет = ГраницаДляДетальныхПравил() + 1000;
			КонецЕсли;
...
Функция ГраницаДляДетальныхПравил()
	Возврат 100000; // 100.тыс
КонецФункции	
...
//  Число - Приоритет.
//
Функция МаксимальныйПриоритет(НастройкаОбработки) Экспорт
	...
	Запрос = Новый Запрос(
		...
		|ГДЕ
		|	ПравилаЭскалацииЗадач.НастройкаОбработки = &НастройкаОбработки
		|	И ПравилаЭскалацииЗадач.ПометкаУдаления = ЛОЖЬ");

	Запрос.УстановитьПараметр("НастройкаОбработки", Справочники.НастройкиОбработкиВидовОбъектов.ПустаяСсылка());
	...
	ГраницаДляДетальныхПравил = ГраницаДляДетальныхПравил();
	
	Если Не ЗначениеЗаполнено(НастройкаОбработки) Тогда
		
		Если МаксимальныйПриоритет >= ГраницаДляДетальныхПравил - 200 Тогда
			МаксимальныйПриоритет = ГраницаДляДетальныхПравил - 200; // без указания настройки - не более 9900
		КонецЕсли;
			
	Иначе	
		
		Если МаксимальныйПриоритет = 0 Тогда
			МаксимальныйПриоритет = ГраницаДляДетальныхПравил; // с указанием настройки - от 10тыс и выше
		КонецЕсли;	
...
Процедура НормализоватьПриоритеты() Экспорт
	...
	Попытка
		...
		Запрос = Новый Запрос(
			...
			|	И ПравилаЭскалацииЗадач.НастройкаОбработки = ЗНАЧЕНИЕ(Справочник.НастройкиОбработкиВидовОбъектов.ПустаяСсылка)
			|
			|УПОРЯДОЧИТЬ ПО
		...
		ТекущийПриоритет = ГраницаДляДетальныхПравил() - 100;
		Выборка = Запрос.Выполнить().Выбрать();
		Пока Выборка.Следующий() Цикл
			ТекущийПриоритет = ТекущийПриоритет + 100;
			ПравилоЭскалацииЗадачОбъект = Выборка.Ссылка.ПолучитьОбъект();
...
Функция НовыйПриоритетПриПовышении(СледующееПравило, СледующееЗаСледующим)
	...
	Иначе
		...
		НастройкаОбработки = СледующееПравило.НастройкаОбработки;
		Если Не ЗначениеЗаполнено(НастройкаОбработки) Тогда
			Если НовыйПриоритет >= ГраницаДляДетальныхПравил() Тогда
				НовыйПриоритет = ГраницаДляДетальныхПравил() - 1000;
			КонецЕсли;	
		КонецЕсли;
...
Функция НовыйПриоритетПриПонижении(ПредыдущееПравило, ПредыдущееЗаПредыдущим)
	...
	Иначе
		...
		НастройкаОбработки = ПредыдущееПравило.НастройкаОбработки;
		Если ЗначениеЗаполнено(НастройкаОбработки) Тогда
			Если НовыйПриоритет < ГраницаДляДетальныхПравил() Тогда
				НовыйПриоритет = ГраницаДляДетальныхПравил();
			КонецЕсли;	
		КонецЕсли;
	КонецЕсли;
...
Функция СледующееПоПриоритету(ПравилоЭскалации, Шаблон = Неопределено)
	...
	Если ЗначениеЗаполнено(НастройкаОбработки) Тогда
		Запрос.Текст = СтрЗаменить(Запрос.Текст,
		"И ПравилаЭскалацииЗадач.ПометкаУдаления = ЛОЖЬ",  
		"И ПравилаЭскалацииЗадач.ПометкаУдаления = ЛОЖЬ И ПравилаЭскалацииЗадач.НастройкаОбработки <> ЗНАЧЕНИЕ(Справочник.НастройкиОбработкиВидовОбъектов.ПустаяСсылка)");
	Иначе
		Запрос.Текст = СтрЗаменить(Запрос.Текст,
		"И ПравилаЭскалацииЗадач.ПометкаУдаления = ЛОЖЬ",  
		"И ПравилаЭскалацииЗадач.ПометкаУдаления = ЛОЖЬ И ПравилаЭскалацииЗадач.НастройкаОбработки = ЗНАЧЕНИЕ(Справочник.НастройкиОбработкиВидовОбъектов.ПустаяСсылка)");
