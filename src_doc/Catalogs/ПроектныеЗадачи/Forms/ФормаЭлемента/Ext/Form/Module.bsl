...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не ДокументооборотПраваДоступа.ПраваПользователяПоОбъекту(Объект.Владелец).Изменение Тогда 
	...
	Если ДелопроизводствоКлиентСервер.ЭтоДокумент(Объект.Предмет) Тогда
		ПраваНаПредмет = ДокументооборотПраваДоступа.ПраваПользователяПоОбъекту(Объект.Предмет);
		ЕстьПравоИзмененияПредмета = ПраваНаПредмет.Изменение;
	КонецЕсли;
...
Процедура ПриЧтенииНаСервере(ТекущийОбъект)
	...
	РаботаСБизнесПроцессамиВызовСервера.ФормаБизнесПроцессаУстановитьТипПредмета(ЭтотОбъект);
	
	Если МиграцияДанныхИзВнешнихСистемСервер.ЭтоОбъектИзДругойСистемы(ТекущийОбъект.ИсточникДанных) Тогда
		НеРедактируетсяОписание = НСтр("ru = 'Проектная задача загружена из сторонней системы, редактирование запрещено.'");
...
Процедура ПослеЗаписи(ПараметрыЗаписи)
	...
	КлючеваяОперация = РаботаСПроектамиКлиентСервер.КлючеваяОперацияЗаписьПроектнойЗадачи();
...
Процедура ПредметПриИзменении(Элемент)
	...
	Иначе
		ЕстьПравоИзмененияПредмета = Ложь;
	КонецЕсли;
...
Процедура ИсполнителиПриНачалеРедактирования(Элемент, НоваяСтрока, Копирование)
	...
	Если НоваяСтрока Тогда 
		Элементы.Исполнители.ТекущиеДанные.Исполнитель = СотрудникиПустаяСсылка;
	КонецЕсли;	
...
Процедура ИсполнителиИсполнительНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	РаботаСАдреснойКнигойКлиент.ОткрытьАдреснуюКнигу(ПараметрыФормы, Элементы.ИсполнителиИсполнитель);		
...
&НаКлиенте
Процедура ПередУдалениемПродолжение(Результат, Параметры) Экспорт 
	...
	УдалениеСервер(ПометкаУдаления); 
	
	ОповеститьОбИзменении(Объект.Ссылка);

	МассивИзмененныхЗадач = Новый Массив;
	МассивИзмененныхЗадач.Добавить(Объект.Ссылка);
	
	ПараметрОповещения = Новый Структура;
	ПараметрОповещения.Вставить("Проект", Объект.Владелец);
	ПараметрОповещения.Вставить("Родитель", Объект.Родитель);
	ПараметрОповещения.Вставить("ПроектнаяЗадача", Объект.Ссылка);
	ПараметрОповещения.Вставить("МассивИзмененныхЗадач", МассивИзмененныхЗадач);
...
&НаСервере
Процедура УдалениеСервер(ПометкаУдаления)
	
	Объект.ПометкаУдаления = ПометкаУдаления;
	Записать();
	Прочитать();
	
	ОбщегоНазначенияДокументооборот.ПоказатьНадписьПометкиУдаления(ЭтотОбъект, ПометкаУдаления);
...
Процедура УстановитьДоступность()
	...
	Элементы.ИсполнителямРазрешеноИзменятьПредмет.Доступность = 
		ДелопроизводствоКлиентСервер.ЭтоДокумент(Объект.Предмет)
		И Не Объект.Веха И ЕстьПравоИзмененияПредмета;
...
Процедура МК_НастроитьЭлементыФормы()
	...
	// Реквизиты.
	МК.ВыровнятьЭлементФормы(Элементы.Наименование);
	МК.ВыровнятьЭлементФормы(Элементы.КодСДР);
	МК.ВыровнятьЭлементФормы(Элементы.Владелец);
	МК.ВыровнятьЭлементФормы(Элементы.Родитель);
	МК.ВыровнятьЭлементФормы(Элементы.ТипПредмета);
	МК.ВыровнятьЭлементФормы(Элементы.Предмет);
	
	Элементы.ИсполнителямРазрешеноИзменятьПредмет.ПоложениеЗаголовка = ПоложениеЗаголовкаЭлементаФормы.Лево;
	Элементы.ИсполнителямРазрешеноИзменятьПредмет.ГоризонтальноеПоложениеВГруппе = ГоризонтальноеПоложениеЭлемента.Лево;
	
	МК.ВыровнятьЭлементФормы(Элементы.ТекущийПланЕдиницаДлительности);
	МК.ВыровнятьЭлементФормы(Элементы.ЕдиницаДлительностиФакт);
	
	Элементы.ГруппаТекущийПлан.ЦветФона = ЦветаСтиля.МК_ЦветФонаГруппы;
	Элементы.ГруппаФакт.ЦветФона = ЦветаСтиля.МК_ЦветФонаГруппы;
	
	Элементы.Веха.ПоложениеЗаголовка = ПоложениеЗаголовкаЭлементаФормы.Лево;
	
	// Исполнители.
	Элементы.Исполнители.Шапка = Ложь;
	Элементы.МК_ИсполнителиГруппаКолонок.Видимость = Истина;
	
	Элементы.Переместить(Элементы.ИсполнителиНомерСтроки, Элементы.МК_ИсполнителиГруппаКолонок, Элементы.МК_ИсполнителиПерваяКолонка);
	Элементы.Переместить(Элементы.ИсполнителиГруппаИсполнитель, Элементы.МК_ИсполнителиПерваяКолонка);
	Элементы.Переместить(Элементы.ИсполнителиТекущийПланТрудозатраты, Элементы.МК_ИсполнителиВтораяКолонка);
	Элементы.Переместить(Элементы.ИсполнителиТрудозатратыФакт, Элементы.МК_ИсполнителиВтораяКолонка);
	
	МК.ВыровнятьЭлементФормы(Элементы.ТекущийПланТрудозатраты);
	МК.ВыровнятьЭлементФормы(Элементы.ТекущийПланЕдиницаТрудозатрат);
	
	МК.ВыровнятьЭлементФормы(Элементы.ТрудозатратыФакт);
	МК.ВыровнятьЭлементФормы(Элементы.ЕдиницаТрудозатратФакт);
	
	Элементы.ОткрытьФактическиеТрудозатраты.Заголовок = НСтр("ru = 'Открыть фактические трудозатраты'");
	
	// Предшественники.
	Элементы.Предшественники.Шапка = Ложь;
	
	// Настройки.
	МК.ВыровнятьЭлементФормы(Элементы.Важность);
	МК.ВыровнятьЭлементФормы(Элементы.ГрафикРаботы);
	МК.ВыровнятьЭлементФормы(Элементы.ТипОграничения);
	МК.ВыровнятьЭлементФормы(Элементы.ДатаОграничения);
