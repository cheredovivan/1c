...
//   СправочникСсылка.ПапкиПисем
//
Функция КорневаяПапка(Папка) Экспорт
	
	Родитель = Папка;       
	КорневаяПапкаВозврат = Папка;       
	
	Пока ЗначениеЗаполнено(Родитель) Цикл
		
		КорневаяПапкаВозврат = Родитель;
		
		Родитель = ОбщегоНазначенияДокументооборот.ЗначениеРеквизитаОбъектаВПривилегированномРежиме(
			Родитель, "Родитель");
		
	КонецЦикла;
	
	Возврат КорневаяПапкаВозврат;
