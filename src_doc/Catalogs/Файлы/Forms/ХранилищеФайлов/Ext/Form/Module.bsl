...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Список.Параметры.УстановитьЗначениеПараметра("ПользовательИЕгоСотрудники", СотрудникиВызовСервера.ПользовательИЕгоСотрудники(Пользователи.ТекущийПользователь()));
...
Процедура ОбработкаОжидания()
	...
	Если Элементы.Папки.ТекущаяСтрока <> Список.Параметры.Элементы.Найти("Владелец").Значение Тогда
		...
		Если ЭтоМобильныйКлиент Тогда
			МК_КлиентСервер.ОформитьКнопкуФильтра(МК_ЭлементыСтиля, Элементы.МК_ИзменитьРазрез, Истина, Истина);
			
			Если Элементы.Папки.ТекущиеДанные = Неопределено Тогда
				Элементы.МК_ИзменитьРазрез.Заголовок = НСтр("ru = 'Выбрать папку'");
			Иначе
				Элементы.МК_ИзменитьРазрез.Заголовок = Элементы.Папки.ТекущиеДанные.Наименование;
			КонецЕсли;

			Элементы.МК_Страницы.ТекущаяСтраница = Элементы.МК_СтраницаСписок;
		КонецЕсли;
...
Процедура СоздатьФайлВыполнить()
	...
	Если ВидПросмотра = ПредопределенноеЗначение("Перечисление.ВидыПросмотраСпискаОбъектов.ПоПапкам") Тогда
		...
		Попытка
			РаботаСФайламиКлиент.ДобавитьФайл(Неопределено, Элементы.Папки.ТекущаяСтрока, ЭтотОбъект, , Истина);
		Исключение
			ПоказатьПредупреждение(, СтрШаблон(
			                 НСтр("ru = 'Ошибка создания нового файла: %1'"),
			                 КраткоеПредставлениеОшибки(ИнформацияОбОшибке()) ));
...
Процедура ПродолжитьПеретаскивание(ВладелецФайла, ПараметрыВыполнения) Экспорт
	...
	Если ТипЗнч(ПараметрыПеретаскивания.Значение) = Тип("Файл") И ПараметрыПеретаскивания.Значение.ЭтоФайл() = Истина Тогда
		Попытка
			РаботаСФайламиКлиент.СоздатьДокументНаОсновеФайла(ПараметрыПеретаскивания.Значение.ПолноеИмя,ВладелецФайла, 
				ЭтаФорма, Истина, , , СписокКатегорий);
		Исключение
			ПоказатьПредупреждение(,СтрШаблон(
			                 НСтр("ru = 'Ошибка создания нового файла: %1'"),
			                 КраткоеПредставлениеОшибки(ИнформацияОбОшибке()) ));
...
Процедура ПапкиПеретаскивание(Элемент, ПараметрыПеретаскивания, СтандартнаяОбработка, Строка, Поле)
	...
	Если ТипЗнч(ПараметрыПеретаскивания.Значение) = Тип("Файл") И ПараметрыПеретаскивания.Значение.ЭтоФайл() = Истина Тогда
		...
		Попытка
			РаботаСФайламиКлиент.СоздатьДокументНаОсновеФайла(ПараметрыПеретаскивания.Значение.ПолноеИмя, Строка, 
				ЭтаФорма, Истина);
		Исключение
			ПоказатьПредупреждение(,СтрШаблон(
			                 НСтр("ru = 'Ошибка создания нового файла: %1'"),
...
Процедура УстановитьДоступностьКоманд(РедактируетТекущийПользователь, Редактирует, ПодписанЭП, Зашифрован)
	...
	Элементы.Освободить.Доступность = ЗначениеЗаполнено(Редактирует);
	Элементы.КонтекстноеМенюСписокОсвободить.Доступность = ЗначениеЗаполнено(Редактирует);
	
	Элементы.Занять.Доступность = Не ЗначениеЗаполнено(Редактирует) И НЕ ПодписанЭП;
	Элементы.КонтекстноеМенюСписокЗанять.Доступность = Не ЗначениеЗаполнено(Редактирует) И НЕ ПодписанЭП;
	...
	Элементы.ФормаПодписать.Доступность = Не ЗначениеЗаполнено(Редактирует) И НЕ Зашифрован;
	Элементы.ФормаПодписатьКонтекст.Доступность = Не ЗначениеЗаполнено(Редактирует) И НЕ Зашифрован;
	
	Элементы.ФормаДобавитьЭПИзФайла.Доступность = Не ЗначениеЗаполнено(Редактирует) И НЕ Зашифрован;
	Элементы.ФормаДобавитьЭПИзФайлаКонтекст.Доступность = Не ЗначениеЗаполнено(Редактирует) И НЕ Зашифрован;
	...
	Элементы.ФормаЗашифроватьКонтекст.Доступность = Не ЗначениеЗаполнено(Редактирует) И НЕ Зашифрован;
	...
	ТаймлистСтатус = ПредопределенноеЗначение("Перечисление.ТаймлистСтатусы.ПустаяСсылка");
	ТаймлистЗадание = ПредопределенноеЗначение("Перечисление.ТаймлистТипыЗаданий.ПустаяСсылка");
	СтатусРаспознавания = ПредопределенноеЗначение("Перечисление.СтатусыРаспознаванияТекста.ПустаяСсылка");
	
	Если ЭтоРасширениеТаймлист Тогда
		ДанныеФайлаТаймлист = ТаймлистВызовСервера.ДанныеФайлаДляОткрытия(ТекущиеДанные.Ссылка);
		ТаймлистСтатус = ДанныеФайлаТаймлист.Статус;
		ТаймлистЗадание = ДанныеФайлаТаймлист.Задание;
	ИначеЕсли РасширениеПоддерживается Тогда
		СтатусРаспознавания = РаспознаваниеВызовСервера.СтатусРаспознаванияТекста(ТекущиеДанные.ТекущаяВерсия);
	КонецЕсли;
	...
	Элементы.ФормаРаспознать.Доступность = 
		...
		И (Не ЭтоРасширениеТаймлист
				...
				И СтатусРаспознавания <> ПредопределенноеЗначение("Перечисление.СтатусыРаспознаванияТекста.Распознано")
				И СтатусРаспознавания <> ПредопределенноеЗначение("Перечисление.СтатусыРаспознаванияТекста.Распознается")
			Или ЭтоРасширениеТаймлист
				И ДоступноРаспознаваниеТаймлист
				И Не ЗначениеЗаполнено(ТаймлистСтатус)
				И Не ЗначениеЗаполнено(ТаймлистЗадание))
		И Не ЗначениеЗаполнено(Редактирует);
	
	Элементы.СписокКонтекстноеМенюРаспознать.Доступность = Элементы.ФормаРаспознать.Доступность;
	// Локализация Конец
...
Процедура ПереключитьВидПросмотра()
	...
	Если ВидПросмотра = Перечисления.ВидыПросмотраСпискаОбъектов.Списком Тогда
		...
		Если ЭтоМобильныйКлиент Тогда
			Элементы.МК_ВерхнийБлок.Видимость = Ложь;
			Элементы.ЕстьКатегории.Видимость = Ложь;
		КонецЕсли;
	...
	ИначеЕсли ВидПросмотра = Перечисления.ВидыПросмотраСпискаОбъектов.ПоПапкам Тогда
		...
		Если ЭтоМобильныйКлиент Тогда
			...
			Элементы.МК_ИзменитьРазрез.Заголовок = Элементы.Папки.ТекущаяСтрока.Наименование;
			Элементы.ЕстьКатегории.Видимость = Ложь;
			Элементы.Создать.Видимость = Ложь;
		КонецЕсли;
...
Процедура ПолучитьКоличествоКатегорийДляПоиска()
	...
	Если ЭтоМобильныйКлиент Тогда
		...
		Если КоличествоВыбранныхКатегорий = 0 Тогда
			Элементы.МК_ИзменитьРазрез.Заголовок = НСтр("ru = 'Без категории'");
			МК_КлиентСервер.ОформитьКнопкуФильтра(МК_ЭлементыСтиля, Элементы.МК_ИзменитьРазрез, Истина, Ложь);
		Иначе
			Элементы.МК_ИзменитьРазрез.Заголовок = СтрШаблон(НСтр("ru = 'Категории (%1)'"), КоличествоВыбранныхКатегорий);
			МК_КлиентСервер.ОформитьКнопкуФильтра(МК_ЭлементыСтиля, Элементы.МК_ИзменитьРазрез, Истина, Истина);
		КонецЕсли;
...
Процедура Распознать(Команда)
	...
	ВыделенныеФайлыТаймлист = ВыделенныеФайлыТаймлист(Элементы.Список.ВыделенныеСтроки);
	...
	Если ВыделенныеФайлыТаймлист.Количество() > 0 Тогда
		...
		ДополнительныеПараметрыОповещения = Новый Структура("ВыделенныеФайлы, Обработчик",
			Элементы.Список.ВыделенныеСтроки, Обработчик);
		
		Оповещение = Новый ОписаниеОповещения("ПослеУказанияКоличестваСпикеров", ЭтотОбъект,
			ДополнительныеПараметрыОповещения);
		
		ТаймлистКлиент.НачатьРасшифровку(ЭтотОбъект, ВыделенныеФайлыТаймлист, Оповещение);
...
// Локализация
&НаКлиенте
Функция ВыделенныеФайлыТаймлист(ВыделенныеСтроки)
	...
	Для Каждого Строка Из ВыделенныеСтроки Цикл
		...
		Если ТаймлистКлиентСервер.ЭтоРасширениеФайлаТаймлист(ДанныеСтроки.Расширение) Тогда
			
			ФайлыТаймлист.Добавить(ДанныеСтроки.Ссылка);
...
&НаКлиенте
Процедура МК_ИзменитьРазрез(Команда)
	
	Элементы.МК_Страницы.ТекущаяСтраница = Элементы.МК_СтраницаДеревья;
...
Процедура МК_НастроитьЭлементыФормы()
	...
	// Командная панель.
	Элементы.ГруппаКомандФайлов.Видимость = Ложь;
	Элементы.ФормаЭПИШифрование.Видимость = Ложь;
	Элементы.ГруппаКомандыФормыЧастые.Видимость = Ложь;
	Элементы.ДополнениеСтрокаПоиска.Видимость = Ложь;
	Элементы.ФормаГруппаПоиск.Видимость = Ложь;
	Элементы.Справка.Видимость = Ложь;
	
	// Контекстное меню.
	Элементы.ГруппаШтрихкодыКонтекст.Видимость = Ложь;
	Элементы.ГруппаШтрихкоды.Видимость = Ложь;
	Элементы.ФормаГруппаСоздатьНаОсновании.Видимость = Ложь;
	Элементы.ВыбратьКатегорииКонекст.Видимость = Ложь;
	...
	// Категории.
	Элементы.ДеревоКатегорий.Шапка = Ложь;
	...
	Элементы.Переместить(Элементы.КартинкаИНаименование, Элементы.МК_ГруппаКолонок);
	Элементы.Переместить(Элементы.СписокВажныеРеквизиты, Элементы.МК_ГруппаКолонок);

	Элементы.ВладелецФайла.Видимость = Ложь;
	Элементы.Редактирует.Видимость = Ложь;
	...
	Элементы.ТекущаяВерсияДатаМодификацииФайла.Видимость = Ложь;
	Элементы.ТекущаяВерсияРазмер.Видимость = Ложь;
	...
	Элементы.Переместить(Элементы.СтраницыСписокФайлов, Элементы.МК_СтраницаСписок);
	Элементы.Переместить(Элементы.Страницы, Элементы.МК_СтраницаДеревья);
	Элементы.МК_Страницы.ТекущаяСтраница = Элементы.МК_СтраницаСписок;
	
	// Разрезы.
	
	Если ВидПросмотра = Перечисления.ВидыПросмотраСпискаОбъектов.ПоПапкам Тогда
		Элементы.МК_ИзменитьРазрез.Заголовок = ПапкаПриОткрытии.Наименование;
	ИначеЕсли ВидПросмотра = Перечисления.ВидыПросмотраСпискаОбъектов.ПоКатегориям Тогда
		Элементы.МК_ИзменитьРазрез.Заголовок = ВыбранныеКатегорииПриОткрытии.Количество();
	КонецЕсли;
	
	
	МК_КлиентСервер.ОформитьКнопкуФильтра(МК_ЭлементыСтиля, Элементы.МК_ИзменитьРазрез, Истина, Истина);
	МК_КлиентСервер.ОформитьАкцентнуюКнопку(Элементы.МК_СкрытьРазрезы, МК_ЭлементыСтиля);
	Элементы.Переместить(Элементы.МК_СкрытьРазрезы, Элементы.МК_СтраницаДеревья);
	
	// Нижний блок.
	Элементы.ГруппаПоиск.Видимость = Ложь;
	МК.ПреобразоватьКнопкуВАкцентную(ЭтотОбъект, МК_ЭлементыСтиля, Элементы.СоздатьФайл, Элементы.МК_СтраницаСписок);
