...
Функция ПолучитьТекстВПоле()
	...
	Если HTMLДокумент <> Неопределено Тогда
		...
		ЕстьПолеbody = ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(HTMLДокумент, "body");
		
		Если ЕстьПолеbody Тогда
			
			ЕстьПолеinnerHTML = ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(HTMLДокумент.body, "innerHTML");
			
			Если ЕстьПолеinnerHTML Тогда
				
				ТекстinnerHTML = HTMLДокумент.body.innerHTML;
				
			КонецЕсли;
			
		КонецЕсли;
		
		ТекстВПоле = "<html><body>" + ТекстinnerHTML + "</body></html>";
		
		// head
		ТекстHead = "";
		
		ЕстьПолеhead = ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(HTMLДокумент, "head");
		
		Если ЕстьПолеhead Тогда
