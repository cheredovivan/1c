...
Процедура СписокВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	Если Элементы.Список.ТекущиеДанные <> Неопределено Тогда
		ПроверитьПредельныйРазмерФайла(МаксРазмерФайла,
			...
			Элементы.Список.ТекущиеДанные.ТекущаяВерсияРасширение,
			ВыбраннаяСтрока);
	КонецЕсли;
...
Процедура СоздатьФайл(Команда)
	...
	Попытка
		...
		Если ТекущиеДанные.ЭтоШаблонДокумента Тогда
			ПоказатьПредупреждение(, НСтр("ru = 'Добавление файлов нужно делать в карточке вида или шаблона документа.'"));
			Возврат;
		КонецЕсли;	
...
&НаКлиенте
Процедура ПроверитьПредельныйРазмерФайла(МаксРазмерФайла, РазмерФайла, РасширениеФайла, ФайлСсылка)
	
	РазмерВМб = РазмерФайла / (1024 * 1024);
	РазмерВМбМакс = МаксРазмерФайла / (1024 * 1024);
	РазмерФайлаПревышаетМаксимальноДопустимый = РазмерФайла > МаксРазмерФайла;
	
	// Локализация
	Если РазмерФайлаПревышаетМаксимальноДопустимый
		И ТаймлистКлиентСервер.ЭтоРасширениеФайлаТаймлист(РасширениеФайла, Истина)
		И ТаймлистВызовСервера.РазрешеноДобавлятьБольшиеФайлы(Строка(ФайлСсылка), РазмерВМб) Тогда
...
Процедура ЗаполнитьДерево(ШаблонДокумента, ВладелецФайла)
	...
	Если ЗначениеЗаполнено(ШаблонДокумента) Тогда 
		...
		НоваяСтрока.ЭтоШаблонДокумента = Истина;
	...
	ИначеЕсли ЗначениеЗаполнено(ВладелецФайла)
		...
		Запрос = Новый Запрос(
			...
			|	Справочник.Файлы КАК Файлы
			|ГДЕ
			|	(Файлы.ВладелецФайла ССЫЛКА Справочник.ШаблоныДокументов)
			|;
			|
			|////////////////////////////////////////////////////////////////////////////////
			|ВЫБРАТЬ
		...
		Запрос.УстановитьПараметр("ВидДокумента", ОбщегоНазначения.ЗначениеРеквизитаОбъекта(
...
Процедура МК_НастроитьЭлементыФормы()
	...
	Элементы.СоздатьФайл.Отображение = ОтображениеКнопки.Картинка;
	...
	Элементы.ДеревоВладельцев.ИспользованиеТекущейСтроки = ИспользованиеТекущейСтрокиТаблицы.ОтображениеВыделенияИВыбор;
	
	Элементы.МК_ГруппаПапки.Видимость = Истина;
	Элементы.Переместить(Элементы.ДеревоВладельцев, Элементы.МК_ГруппаПапки);
	
	Элементы.МК_ГруппаПапки.Поведение = ПоведениеОбычнойГруппы.Всплывающая;
	Элементы.МК_ГруппаПапки.ОтображатьЗаголовок = Истина;
	Элементы.МК_ГруппаПапки.Заголовок = НСтр("ru = 'Папка'");
	Элементы.МК_ГруппаПапки.Скрыть();
	
	Элементы.ДатаСоздания.Видимость = Ложь;
	Элементы.Ссылка.Видимость = Ложь;
	Элементы.Расширение.Видимость = Ложь;
	Элементы.ПодписанЭП.Видимость = Ложь;
