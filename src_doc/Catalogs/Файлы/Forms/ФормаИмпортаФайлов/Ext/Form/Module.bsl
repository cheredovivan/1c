...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если НЕ Параметры.Свойство("ПапкаДляДобавления")
		И Не Параметры.Свойство("СозданиеФормыДляРасположенияДопреквизитов") Тогда 	
	...
	Если Параметры.Свойство("ДоступнаОтметкаОригинала") Тогда
		Элементы.Оригинал.ТолькоПросмотр = Не Параметры.ДоступнаОтметкаОригинала;
	КонецЕсли;      
	
	ЕстьТекстовыеФайлы = Ложь;
	...
	Если Параметры.МассивИменФайлов <> Неопределено Тогда
		Для Каждого ПутьФайла Из Параметры.МассивИменФайлов Цикл
			...
			Если ФайловыеФункцииКлиентСервер.РасширениеФайлаВСписке(СписокРасширенийТекстовыхФайлов, ФайлПеренесенный.Расширение) Тогда
				ЕстьТекстовыеФайлы = Истина;
			КонецЕсли;	
	...
	Если Не ЕстьТекстовыеФайлы Тогда
...
#Если МобильныйКлиент Тогда
	Для Каждого СтрокаВыбранногоФайла Из ВыбранныеФайлы Цикл
		ВыбранныйФайл = Новый Файл(СтрокаВыбранногоФайла.Путь);
		СведенияОЗагружаемомКонтенте = МК_Клиент.СведенияОЗагружаемомКонтенте(ВыбранныйФайл);
		СтрокаВыбранногоФайла.Наименование = СведенияОЗагружаемомКонтенте.Имя;
		СтрокаВыбранногоФайла.ИндексКартинки =
			ФайловыеФункцииКлиентСервер.ПолучитьИндексПиктограммыФайла(
				СведенияОЗагружаемомКонтенте.Расширение);
	КонецЦикла;
#КонецЕсли
...
Процедура ДобавитьВыполнить()
	...
	Для Каждого СтрокаСписка Из ВыбранныеФайлы Цикл
		ВыбранныеФайлыСписокЗначений.Добавить(СтрокаСписка.Путь, СтрокаСписка.Наименование);
	КонецЦикла;
	ВыбранныеФайлыСписокЗначений.СортироватьПоПредставлению();
