...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Права = ДокументооборотПраваДоступа.ПраваПользователяПоОбъекту(Файл);
...
Процедура СохранитьКак(Команда)
	...
	Если ВыборФайла.Выбрать() Тогда
		...
		КодировкаТекстаДляЗаписи = ?(КодировкаТекстаДляЗаписи = "utf-8_WithoutBOM", "utf-8", КодировкаТекстаДляЗаписи);
...
Процедура ЗаписатьТекст()
	...
	Если Модифицированность Тогда
		...
		КодировкаТекстаДляЗаписи = ?(КодировкаТекстаФайла = "utf-8_WithoutBOM", "utf-8", КодировкаТекстаФайла);
...
Процедура ОбработкаВыбора(ВыбранноеЗначение, ИсточникВыбора)
	...
	ИначеЕсли ВРег(ИсточникВыбора.ИмяФормы) = ВРег("Справочник.Файлы.Форма.ВыборКодировки") Тогда
		...
		ТекстПоля = Текст.ПолучитьТекст();
		ТекстПоля = ОбработкаСтрокиXML.УдалитьНедопустимыеСимволыXML(ТекстПоля);
	...
	Оповестить(
		"Запись_Файл", 
		Новый Структура("Событие, Файл, Владелец", 
			"ДанныеФайлаИзменены", 
			Файл, ВладелецФайла),
			Файл);
...
Процедура ПрочитатьТекст()
	...
	КодировкаТекстаДляЧтения = ?(КодировкаТекстаФайла = "utf-8_WithoutBOM", "utf-8", КодировкаТекстаФайла);
	...
	Если ДанныеФайла.Размер < ПредельныйРазмерФайла Тогда
		...
		ТекстПоля = Текст.ПолучитьТекст();
		ТекстПоля = ОбработкаСтрокиXML.УдалитьНедопустимыеСимволыXML(ТекстПоля);		
		ТекстHtml = ПолучитьHtml(ТекстПоля);
...
Процедура МК_НастроитьЭлементыФормы()
	...
	Элементы.ГруппаКомандаДекорация.Видимость = Ложь;
	
	Для каждого Элемент из Элементы.ГруппаПодменю.ПодчиненныеЭлементы Цикл
		Если ТипЗнч(Элемент) = Тип("КнопкаФормы") Тогда
			Элементы.Переместить(Элемент, КоманднаяПанель);
