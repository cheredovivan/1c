...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ПоказыватьДиалогСканера = Неопределено Тогда
		...
		ОбщегоНазначения.ХранилищеОбщихНастроекСохранить("НастройкиСканирования/ПоказыватьДиалогСканера", 
	...
	Если ФорматСканированногоИзображения.Пустая() Тогда
		...
		ОбщегоНазначения.ХранилищеОбщихНастроекСохранить("НастройкиСканирования/ФорматСканированногоИзображения", 
			ИдентификаторКлиента, ФорматСканированногоИзображения);
	КонецЕсли;

	Если ФорматСканированногоИзображения = Перечисления.ФорматыХраненияОдностраничныхФайлов.BMP Тогда
		ФорматСканированногоИзображения = Перечисления.ФорматыСканированногоИзображения.PNG;
		ОбщегоНазначения.ХранилищеОбщихНастроекСохранить("НастройкиСканирования/ФорматСканированногоИзображения", 
			ИдентификаторКлиента, ФорматСканированногоИзображения);
	КонецЕсли;
	...
	Если ФорматХраненияОдностраничный.Пустая() Тогда
		...
		ОбщегоНазначения.ХранилищеОбщихНастроекСохранить("НастройкиСканирования/ФорматХраненияОдностраничный", 
			ИдентификаторКлиента, ФорматХраненияОдностраничный);
	КонецЕсли;

	Если ФорматХраненияОдностраничный = Перечисления.ФорматыХраненияОдностраничныхФайлов.BMP Тогда
		ФорматХраненияОдностраничный = Перечисления.ФорматыХраненияОдностраничныхФайлов.PNG;
		ОбщегоНазначения.ХранилищеОбщихНастроекСохранить("НастройкиСканирования/ФорматХраненияОдностраничный", 
			ИдентификаторКлиента, ФорматХраненияОдностраничный);
	КонецЕсли;
	...
	Если ФорматХраненияМногостраничный.Пустая() Тогда
		...
		ОбщегоНазначения.ХранилищеОбщихНастроекСохранить("НастройкиСканирования/ФорматХраненияМногостраничный", 
	...
	КаталогЖурналаСканирования = ОбщегоНазначения.ХранилищеОбщихНастроекЗагрузить(
		...
		ИдентификаторКлиента);
	СпособПреобразованияВPDF = ОбщегоНазначения.ХранилищеОбщихНастроекЗагрузить(
		"НастройкиСканирования/СпособПреобразованияВPDF", 
		ИдентификаторКлиента);
...
Процедура ОК(Команда)
	...
	МассивСтруктур.Добавить (СформироватьНастройку("АвтоматическаяПодачаДокументов", 
		АвтоматическаяПодачаДокументов, ИдентификаторКлиента));
	...
	МассивСтруктур.Добавить (СформироватьНастройку("ИспользоватьКаталогЖурналаСканирования", ИспользоватьКаталогЖурналаСканирования, ИдентификаторКлиента));
	МассивСтруктур.Добавить (СформироватьНастройку("КаталогЖурналаСканирования", КаталогЖурналаСканирования, ИдентификаторКлиента));
	МассивСтруктур.Добавить (СформироватьНастройку("СпособПреобразованияВPDF", СпособПреобразованияВPDF, ИдентификаторКлиента));
	...
	Если ИспользоватьКаталогЖурналаСканирования И ЗначениеЗаполнено(КаталогЖурналаСканирования) Тогда
		РаботаСоСканеромКлиент.УстановитьИмяЛогФайла(КаталогЖурналаСканирования);
	КонецЕсли;
...
Процедура ОбновитьСостояние()
	...
	Если РаботаСоСканеромКлиент.ПроинициализироватьКомпоненту() Тогда
		...
		Элементы.ВерсияКомпонентыСканирования.Ширина = 7; 
		...
		Если РаботаСоСканеромКлиентПовтИсп.ДоступнаКомандаСканировать() Тогда
			...
			Если Не ПустаяСтрока(ИмяУстройства) Тогда
				...
				АвтоматическаяПодачаДокументовЧисло = РаботаСоСканеромКлиент.ПолучитьНастройку(ИмяУстройства, "FEEDER");
...
Процедура УстановитьСтандартныеНастройки(Команда)
	...
	Если Не ЗначениеЗаполнено(Цветность) Тогда
		Цветность = ПредопределенноеЗначение("Перечисление.ЦветностиИзображения.Цветное");
	КонецЕсли;	    
	
	ПоказатьОповещениеПользователя(НСтр("ru = 'Установлены стандартные настройки'"));
...
Процедура ОтработатьИзмененияИспользоватьImageMagick()
	...
	Элементы.СпособПреобразованияВPDF.Доступность = ИспользоватьImageMagickДляПреобразованияВPDF;
	Если Не ИспользоватьImageMagickДляПреобразованияВPDF Тогда
		СпособПреобразованияВPDF = 0; // встроенный
	КонецЕсли;	
...
Процедура ИнформацияДляТехническойПоддержкиНажатие(Элемент)
	...
	РаботаСФайламиСлужебныйКлиент.ПолучитьТехническуюИнформацию(НСтр("ru='Отправка технической информации из формы настроек.'"), Неопределено);
...
&НаКлиенте
Процедура ФорматХраненияМногостраничныйОчистка(Элемент, СтандартнаяОбработка)
	СтандартнаяОбработка = Ложь;
КонецПроцедуры

&НаКлиенте
Процедура ЦветностьОчистка(Элемент, СтандартнаяОбработка)
	СтандартнаяОбработка = Ложь;
КонецПроцедуры
