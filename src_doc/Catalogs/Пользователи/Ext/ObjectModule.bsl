...
Процедура ОбработатьИзменениеВлияющихДанных() Экспорт
	...
	ВлияющийОбъектМетаданных = "Справочник.Пользователи";
...
Процедура ПередЗаписью(Отказ)
	...
	ДополнительныеСвойства.Вставить(
		...
		Справочники.Пользователи.ЗначенияПараметровОбновленияДанныхКэширующихОбъектов(ЭтотОбъект));
	
	ДополнительныеСвойства.Вставить(
		"ПараметрыОбновленияСоставовГруппПользователей",
		Справочники.Пользователи.ЗначенияПараметровСоставовГруппПользователей(ЭтотОбъект));
...
Процедура ПриЗаписи(Отказ)
	...
	Справочники.Пользователи.ОбновитьДанныеКэширующихОбъектов(
	...
	ПредФизЛицо = ПредыдущиеЗначенияРеквизитов.ФизЛицо;
	Если ФизЛицо <> ПредФизЛицо Тогда
		
		Сотрудник = Сотрудники.ОсновнойСотрудникФизЛица(ПредФизЛицо);
		Если ЗначениеЗаполнено(Сотрудник) Тогда
			РегистрыСведений.ОсновныеСотрудники.УстановитьОсновногоСотрудника(ФизЛицо, Сотрудник);
		КонецЕсли;
...
// Если пользователь помечается на удаление, то и его уведомления тоже надо пометить.
Процедура ПометитьНаУдалениеУведомления()
	...
	Запрос = Новый Запрос(
		"ВЫБРАТЬ
		|	Т.Ссылка КАК Уведомление
		|ИЗ
		|	Справочник.УведомленияПрограммы КАК Т
		|ГДЕ
	...
	Пока Выборка.Следующий() Цикл
		УведомлениеОбъект = Выборка.Уведомление.ПолучитьОбъект();
