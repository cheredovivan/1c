...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ЕстьПравоРедактирования = ЭтоПолноправныйПользователь
	...
	Если Объект.Ссылка = ЧатБот И ЗначениеЗаполнено(ПользовательИБИмя) Тогда
		Элементы.ПользовательИБИмя1.ОтображениеПредупрежденияПриРедактировании =
			ОтображениеПредупрежденияПриРедактировании.Отображать;
		Элементы.ПользовательИБИмя1.ПредупреждениеПриРедактировании =
			РаботаСЧатБотом.ТекстПредупрежденияИзменятьЧатБотаНеРекомендуется();
		Элементы.ПользовательИБИмя2.ОтображениеПредупрежденияПриРедактировании =
			ОтображениеПредупрежденияПриРедактировании.Отображать;
		Элементы.ПользовательИБИмя2.ПредупреждениеПриРедактировании =
			РаботаСЧатБотом.ТекстПредупрежденияИзменятьЧатБотаНеРекомендуется();
	КонецЕсли;
	
	Элементы.ФормаОчиститьИсториюВыбора.Видимость = ЭтоПолноправныйПользователь;
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	Если ИмяСобытия = ФизическиеЛицаКлиент.ИмяОповещенияОбИзмененииФИО()
		И Параметр = Объект.ФизЛицо Тогда
...
Процедура ПослеЗаписи(ПараметрыЗаписи)
	...
	Оповестить("Запись_Пользователь", Объект.Ссылка, ЭтотОбъект);
...
Процедура Склонения(Команда)
	...
	МодульСклонениеПредставленийОбъектовКлиент.ПоказатьСклонение(ЭтотОбъект, Объект.Наименование, ПараметрыСклонения);
...
Асинх Процедура ОчиститьИсториюВыбораПользователя(Команда)
	...
	Если ОчиститьИсторию = КодВозвратаДиалога.Да Тогда
		...
		ИсторияВыбораВызовСервера.ОчиститьИсториюВыбораПользователя(Объект.Ссылка); 
...
Процедура НастроитьФорму(ТекущийОбъект, ПриСозданииНаСервере = Ложь, ПараметрыЗаписи = Неопределено)
	...
	Элементы.СвойстваПользователяИБ.Видимость = ДействияВФорме.СвойстваПользователяИБ = "Редактирование";
...
Процедура НастроитьОтображениеГруппыПометкаУдаления()
	...
	Элементы.ГруппаПометкаУдаления.Видимость = 
		ЗначениеЗаполнено(Объект.Ссылка) И (Объект.ПометкаУдаления Или Объект.Недействителен); 
	
	ТекстСообщения = ""; 
	Если Объект.ПометкаУдаления И Объект.Недействителен Тогда
		ТекстСообщения = НСтр("ru = 'Пользователь недействителен, помечен на удаление.'");
	ИначеЕсли Объект.ПометкаУдаления Тогда
		ТекстСообщения = НСтр("ru = 'Пользователь помечен на удаление'");
	ИначеЕсли Объект.Недействителен Тогда
		ТекстСообщения = НСтр("ru = 'Пользователь недействителен'");
	КонецЕсли;
	Элементы.ПометкаУдаленияНадпись.Заголовок = ТекстСообщения;
