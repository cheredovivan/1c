...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Элементы.ОчиститьИсториюВыбораВсехПользователей.Видимость = Пользователи.ЭтоПолноправныйПользователь();
...
Процедура ПользователиСписокПриПолученииДанныхНаСервере(ИмяЭлемента, Настройки, Строки)
	...
	ОтображаемыеПользователи = Новый Массив;
	
	Для Каждого ДанныеСтроки Из Строки Цикл
		ОтображаемыеПользователи.Добавить(ДанныеСтроки.Ключ);
	КонецЦикла;
	
	СотрудникиПоПользователям = Сотрудники.СотрудникиПоПользователям(ОтображаемыеПользователи, Ложь);
	
	Для Каждого ДанныеСтроки Из Строки Цикл
		СотрудникиПользователя = СотрудникиПоПользователям[ДанныеСтроки.Ключ];
		ДанныеСтроки.Значение.Данные.СотрудникиПользователя = 
			СтрСоединить(СотрудникиПользователя, Символы.ПС);
	КонецЦикла;
...
Процедура ГруппыИПолномочия(Команда)
	...
	ПараметрыФормы.Вставить("Субъект", Элементы.ПользователиСписок.ТекущаяСтрока);
