...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.Свойство("ВариантИспользования") Тогда
		ВариантИспользования = Параметры.ВариантИспользования;
	КонецЕсли;
	...
	Если Параметры.Свойство("Ключ") Тогда
		...
		Если Выборка.Следующий() Тогда
			...
			ПеренастройкаСуществующей = Истина;
		КонецЕсли;
	Иначе
...
Процедура Назад(Команда)
	...
	ИначеЕсли ТекущаяСтраница = Элементы.НастройкаOAuth Тогда
		ПредыдущаяСтраница = Элементы.ДополнительныеНастройки;
...
Процедура ПоказатьВопросПередЗакрытиемФормы()
	...
	ПоказатьВопрос(ОписаниеОповещения, НСтр("ru = 'Введенные данные не записаны. Закрыть форму?'"), Кнопки,,
		КодВозвратаДиалога.Отмена, НСтр("ru = 'Настройка почты'"));
КонецПроцедуры
...
Процедура ПерейтиНаСледующуюСтраницу()
	...
	Если ТекущаяСтраница = Элементы.НастройкаУчетнойЗаписи И ИспользоватьOAuth Тогда
	...
	ИначеЕсли ТекущаяСтраница = Элементы.ДополнительныеНастройки Тогда
		...
		Если Не ИспользоватьOAuth Тогда
			СледующаяСтраница = Элементы.ПроверкаНастроекУчетнойЗаписи;
		Иначе	
			СледующаяСтраница = Элементы.НастройкаOAuth;
			ВыполнитьЗапросOAuth();
		КонецЕсли;	
...
Процедура ПроверитьЗаполнениеНаСтраницеНастройкаУчетнойЗаписи(Отказ)
	...
	Если Не ИспользоватьOAuth И ПустаяСтрока(ПарольДляПолученияПисем) Тогда
...
Функция НастройкиПоУмолчанию(АдресЭлектроннойПочты, Пароль)
	...
	Настройки.Вставить("ПериодХраненияСообщенийНаСервере", 0);
...
Процедура СоздатьУчетнуюЗапись()
	...
	Попытка
		...
		Если Не ПеренастройкаСуществующей Тогда
			НовСотрудник = 
				Сотрудники.ОсновнойСотрудникПользователя(
				ПользователиКлиентСервер.ТекущийПользователь());
			Если УчетнаяЗапись.ОтветственныеЗаОбработкуПисем.Найти(НовСотрудник, "Сотрудник") = Неопределено Тогда
				НовСтрока = УчетнаяЗапись.ОтветственныеЗаОбработкуПисем.Добавить();
				НовСтрока.Сотрудник = НовСотрудник;
			КонецЕсли;	
		КонецЕсли;	
			
		УчетнаяЗапись.ИспользоватьOAuth = ИспользоватьOAuth;	
		...
		Если ИспользоватьOAuth Тогда
			ЗаписатьПараметрыТокена(УчетнаяЗаписьСсылка);
		Иначе	
...
Процедура ИспользоватьOAuthПриИзменении(Элемент)
	...
	Элементы.Пароль.Видимость = Не ИспользоватьOAuth;
	Элементы.ПарольДляОтправкиПисем.Видимость = Не ИспользоватьOAuth;
	Элементы.ПарольДляПолученияПисем.Видимость = Не ИспользоватьOAuth;
	
	Если ИспользоватьOAuth Тогда
		СпособНастройки = "Вручную";
		Элементы.СпособНастройки.Доступность = Ложь;
	КонецЕсли;	
...
&НаКлиенте
Процедура ВыполнитьЗапросOAuth()
...
Процедура ВыполнитьHTTPСоединение(СтрокаCode)
	...
	РаботаСПочтовымиСообщениями.ВыполнитьHTTPСоединение(СтрокаCode, ЭтаФорма,
		СерверВходящейПочты, СерверИсходящейПочты);
	
КонецПроцедуры

&НаСервере
Процедура ЗаписатьПараметрыТокена(УчетнаяЗаписьСсылка)
	
	УстановитьПривилегированныйРежим(Истина);
	ОбщегоНазначения.ЗаписатьДанныеВБезопасноеХранилище(УчетнаяЗаписьСсылка, 
