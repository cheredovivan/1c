...
Функция ПраваПравообладателейПоОбъектам(
	...
	Запрос = Новый Запрос(
		...
		|		И ДескрипторыДляОбъектов.ТипДескриптора = &ТипПоРабочейГруппе
		|ГДЕ
	...
	Пока Выборка.Следующий() Цикл
		...
		Для Каждого СтрокаПравПоДескрипторам Из СтрокиПравПоДескрипторам Цикл
			...
			// Чтение - все, кто есть в РС прав по дескрипторам:
...
Функция ПечатьКарточки(МассивОбъектов, ОбъектыПечати, ПараметрыПечати)
	...
	Для Каждого ОбъектПечати Из МассивОбъектов Цикл
		...
		Фото = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ОбъектПечати.Автор, "Владелец.ФайлФотографии");
		Если ЗначениеЗаполнено(Фото) Тогда
			ДвоичныеДанныеФото = Фото.Получить();
		...
		Если СведенияОТеме <> Неопределено Тогда
			...
			АвторПоследнегоСообщения = СотрудникиВызовСервера.ЛюбойПользовательСотрудника(
...
Функция СоздатьНовуюТему(РеквизитыТемы, Автор = Неопределено, РабочаяГруппаДобавить = Неопределено, РабочаяГруппаУдалить = Неопределено) Экспорт
	...
	Если Не ЗначениеЗаполнено(Автор) Тогда
		Автор = СотрудникиВызовСервера.ОсновнойСотрудникПользователя(Пользователи.ТекущийПользователь());
	КонецЕсли;
