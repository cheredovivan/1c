...
Процедура СписокВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	ИначеЕсли Поле.Имя = "КартинкаФайлы" Или Поле.Имя = "ГиперссылкаФайлы" Тогда
		...
		ДействиеПриОткрытии = "ОткрытьФайлы";
		РаботаСОбсуждениямиКлиент.ОткрытьСообщение(ВыбраннаяСтрока, ДействиеПриОткрытии);
		
	ИначеЕсли Поле.Имя = "КартинкаГолосование" Или Поле.Имя = "ГиперссылкаГолосование" Тогда
		
		СтандартнаяОбработка = Ложь;
		
		ДействиеПриОткрытии = "ОткрытьГолосование";
		РаботаСОбсуждениямиКлиент.ОткрытьСообщение(ВыбраннаяСтрока, ДействиеПриОткрытии);
		
	ИначеЕсли ТипЗнч(ВыбраннаяСтрока) = Тип("СправочникСсылка.СообщенияОбсуждений") Тогда
		
		СтандартнаяОбработка = Ложь;
		
		РаботаСОбсуждениямиКлиент.ОткрытьСообщение(ВыбраннаяСтрока);
...
Процедура ДеревоСообщенийВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	ИначеЕсли Поле.Имя = "ДеревоКартинкаФайлы" Или Поле.Имя = "ДеревоГиперссылкаФайлы" Тогда
		...
		КонецЕсли;
		ДействиеПриОткрытии = "ОткрытьФайлы";
		РаботаСОбсуждениямиКлиент.ОткрытьСообщение(Сообщение, ДействиеПриОткрытии);
	...
	Иначе
		...
		Если Сообщение = Неопределено Тогда
			Возврат;
		КонецЕсли;
		РаботаСОбсуждениямиКлиент.ОткрытьСообщение(Сообщение);
...
Процедура ДеревоСообщенийПередНачаломИзменения(Элемент, Отказ)
	...
	Если Элемент.ТекущаяСтрока <> Неопределено Тогда
		РаботаСОбсуждениямиКлиент.ОткрытьСообщение(Элемент.ТекущиеДанные.Ссылка);
	КонецЕсли;
...
&НаКлиенте
Процедура ДеревоСообщенийОбработкаЗапросаОбновления()
	
	Элементы.ДеревоСообщений.Обновить();
...
Процедура Прокомментировать(Команда)
	...
	ПараметрыФормы.Вставить("Сообщение", ТекущееСообщение);
...
Процедура ПраваДоступа(Команда)
	...
	ОткрытьФорму("ОбщаяФорма.ДокументооборотПраваДоступаПоОбъекту", ПараметрыФормы, ЭтотОбъект, Тема);
...
Процедура ОбработкаОжиданияПрочтения()
	...
	Если ТекущиеДанныеСообщения <> Неопределено Тогда
		...
		Если ТекущиеДанныеСообщения.Свойство("ТекстСообщения")
			...
			КлючеваяОперация = РаботаСОбсуждениямиКлиентСервер.КлючеваяОперацияПросмотрСообщения();
...
Процедура ОбновитьЗаголовок()
	...
	Заголовок = ПредставлениеСообщенийТемы + " " + ПредставлениеЗаголовка;
...
Процедура ПереключитьВидПросмотра(ОбновитьДанныеОтображения)
	...
	Если ВидПросмотра =
		...
		Если ЭтоМобильныйКлиент Тогда
			Элементы.Переместить(Элементы.СправочникСообщенияОбсужденийПечатьКарточки, Элементы.Список.КонтекстноеМеню);
		КонецЕсли;
	...
	ИначеЕсли ВидПросмотра =
		...
		Если ЭтоМобильныйКлиент Тогда
			Элементы.Переместить(Элементы.СправочникСообщенияОбсужденийПечатьКарточки, Элементы.ДеревоСообщений.КонтекстноеМеню);
		КонецЕсли;
...
Процедура ОбновитьЗадачиПоПриложению()
	...
	ЗаголовокКомандыЗадачиПоПриложению = РаботаСЗадачами.ЗаголовокКомандыЗадачиПоПриложению(ТекущееСообщение);
...
Процедура МК_НастроитьЭлементыФормы()
	...
	Элементы.ОбластьЧтения.Видимость = Ложь;
	
	// Список
	Элементы.СписокГруппы.Видимость = Ложь;
	...
	Элементы.ПометитьКакПрочтенныеСписок.Видимость = Ложь;
	Элементы.СписокУстановитьПометкуУдаления.Видимость = Ложь;
	...
	Элементы.СписокКонтекстноеМенюГруппа.Видимость = Ложь;
	
	Элементы.Переместить(Элементы.ОтправитьОбъекты, Элементы.Список.КонтекстноеМеню);
	Элементы.Переместить(Элементы.СписокПоказатьОтветыНаСообщение, Элементы.Список.КонтекстноеМеню);
	
	Элементы.Переместить(Элементы.СписокНовоеСообщение, Элементы.СтраницаСписок);
	МК_КлиентСервер.ОформитьАкцентнуюКнопку(Элементы.СписокНовоеСообщение, МК_ЭлементыСтиля);
	
	Элементы.ТолькоНепрочтенные.ПоложениеВКоманднойПанели = ПоложениеКнопкиВКоманднойПанели.ВКоманднойПанелиИВДополнительномПодменю;
	Элементы.ТолькоНепрочтенные.Отображение = ОтображениеКнопки.Картинка;
	Элементы.ТолькоНепрочтенные.Картинка = БиблиотекаКартинок.МК_ФильтрНеПрочтенные;
	
	// Дерево
	Элементы.ДеревоСообщенийОбновить.Видимость = Ложь;
	
	Элементы.ДеревоГруппы.Видимость = Ложь;
	Элементы.МК_ДеревоГруппаКолонок.Видимость = Истина;
	
	Элементы.Переместить(Элементы.ДеревоСообщенийАвтор, Элементы.МК_ДеревоПерваяСтрока);
	Элементы.Переместить(Элементы.ДеревоСообщенийДатаСообщения, Элементы.МК_ДеревоПерваяСтрока);
	Элементы.Переместить(Элементы.ДеревоСообщенийТекстСообщенияСписок, Элементы.МК_ДеревоПерваяКолонка);
	
	Элементы.ДеревоСообщенийАвтор.Шрифт = ШрифтыСтиля.МелкийШрифтТекста;
	Элементы.ДеревоСообщенийАвтор.Ширина = 23;
	
	Элементы.ДеревоСообщенийДатаСообщения.Шрифт = ШрифтыСтиля.МелкийШрифтТекста;
	Элементы.ДеревоСообщенийДатаСообщения.Формат = НСтр("ru='ДФ=''dd.MM.yyyy HH:mm'''");
	Элементы.ДеревоСообщенийТекстСообщенияСписок.Высота = 3;
	Элементы.ДеревоСообщенийДатаСообщения.Ширина = 10;
	
	Элементы.ДеревоСообщенийКонтекстноеМенюГруппа.Видимость = Ложь;
	
	Элементы.Переместить(Элементы.ПометитьКакПрочтенныеДерево, Элементы.ДеревоСообщений.КонтекстноеМеню);
	Элементы.Переместить(Элементы.ДеревоСообщенийСоздать, Элементы.ДеревоСообщений.КонтекстноеМеню);
	Элементы.Переместить(Элементы.ОтправитьОбъектыДерево, Элементы.ДеревоСообщений.КонтекстноеМеню);
	Элементы.Переместить(Элементы.ДеревоСообщенийУдалить, Элементы.ДеревоСообщений.КонтекстноеМеню);
	
	Элементы.Переместить(Элементы.ДеревоСообщенийНовоеСообщение, Элементы.СтраницаДерево);
	МК_КлиентСервер.ОформитьАкцентнуюКнопку(Элементы.ДеревоСообщенийНовоеСообщение, МК_ЭлементыСтиля);
	
	Элементы.РазвернутьНовыеСообщения.ПоложениеВКоманднойПанели = ПоложениеКнопкиВКоманднойПанели.ВКоманднойПанели;
	Элементы.РазвернутьНовыеСообщения.Отображение = ОтображениеКнопки.Картинка;
	Элементы.РазвернутьНовыеСообщения.Картинка = БиблиотекаКартинок.МК_ФильтрНеПрочтенные;
