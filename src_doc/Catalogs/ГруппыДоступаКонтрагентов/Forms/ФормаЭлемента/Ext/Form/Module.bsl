...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ТолькоПросмотр Или Не ДокументооборотПраваДоступа.ПраваПользователяПоОбъекту(Объект.Ссылка).Изменение Тогда
		...
		Элементы.ПредупреждениеОЗапрещенных.Подсказка = НСтр(
			"ru = 'Надпись кликабельна для того, у кого есть доступ редактировать группу доступа'");
	КонецЕсли;
...
Функция ОбновитьИнформациюОЗапрещенныхУчастниках_НачатьНаСервере(Знач УникальныйИдентификаторФормы, Знач ГруппаДоступа)
	...
	Возврат ДлительныеОперации.ВыполнитьФункцию(
		ПараметрыВыполнения,
		"Справочники.ГруппыДоступаКонтрагентов.ОбъектыСЗапрещеннымиУчастникамиДоступа", 
		Истина, ГруппаДоступа);

КонецФункции

&НаСервереБезКонтекста
Функция КолвоОбъектовСЗапрещеннымиУчастникамиДоступа_НеВФоне(Знач ГруппаДоступа)
	
	Возврат Справочники.ГруппыДоступаКонтрагентов.ОбъектыСЗапрещеннымиУчастникамиДоступа(Истина, ГруппаДоступа);
