...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не ПользователиСерверПовтИсп.ЭтоПолноправныйПользовательИБ()
		И ПользовательИЕгоСотрудники.Найти(Объект.Автор) = Неопределено Тогда 
...
#КонецЕсли
	...
	ДобавитьКартинкуИзБуфераКакФайлЗавершение(Истина, ПутьКФайлу);
	
КонецПроцедуры

&НаКлиенте
Процедура ДобавитьКартинкуИзБуфераКакФайлЗавершение(Результат, ПутьКФайлу) Экспорт
...
Процедура ВыбратьПользователей(МножественныйВыбор)
	...
	Если МножественныйВыбор Тогда
		...
		РежимРаботыФормы = 2;
		ЗаголовокФормы = НСтр("ru = 'Подбор сотрудников группы доступа общего шаблона письма'");
