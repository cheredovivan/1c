...
// 
// Параметры:
//  ДескрипторСсылка - СправочникСсылка.ДескрипторыДоступаРегистров -
//  ПротоколРасчетаПрав - Массив Из Структура, Неопределено -
//  Немедленно - Неопределено, Булево -
...
Функция УдалитьПорциюУстаревшихДанных() Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		ФрагментТекстаЗапроса = ФрагментТекстаЗапроса + "ВЫБРАТЬ ПЕРВЫЕ 100
			...
			|ИЗ
			|	Справочник.ДескрипторыДоступаРегистров КАК ДескрипторыДоступаРегистров
			|		ЛЕВОЕ СОЕДИНЕНИЕ &ИмяРегистраБезКавычек КАК РегистрДанных
			|		ПО ДескрипторыДоступаРегистров.ОбъектДоступа1 = &РегистрДанныхОбъектДоступа1";
			
		Если ЗначениеЗаполнено(СведенияОПолях.ОбъектДоступа2) Тогда
			ФрагментТекстаЗапроса = ФрагментТекстаЗапроса + "
				|			И ДескрипторыДоступаРегистров.ОбъектДоступа2 = &РегистрДанныхОбъектДоступа2";
		КонецЕсли;
		
		Если ЗначениеЗаполнено(СведенияОПолях.ОбъектДоступа3) Тогда
			ФрагментТекстаЗапроса = ФрагментТекстаЗапроса + "
				|			И ДескрипторыДоступаРегистров.ОбъектДоступа3 = &РегистрДанныхОбъектДоступа3";
		КонецЕсли;
		
		ТекстЗапросаПроверки = ТекстЗапросаПроверки + ФрагментТекстаЗапроса + "
			|ГДЕ
			|	ДескрипторыДоступаРегистров.Ссылка = &Дескриптор
			|	И НЕ &РегистрДанныхОбъектДоступа1 ЕСТЬ NULL";
		
		ТекстЗапроса = ТекстЗапроса + ФрагментТекстаЗапроса + "
			|ГДЕ
			|	ДескрипторыДоступаРегистров.Регистр = &ИмяРегистраВКавычках
			|	И &РегистрДанныхОбъектДоступа1 ЕСТЬ NULL";
		
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&ИмяРегистраБезКавычек", Выборка.Регистр);
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&ИмяРегистраВКавычках", """" + Выборка.Регистр + """");
		
		ПсевдонимСТочкой = "РегистрДанных.";
		ТекстЗапроса = СтрЗаменить(
			ТекстЗапроса, "&РегистрДанныхОбъектДоступа1", ПсевдонимСТочкой + СведенияОПолях.ОбъектДоступа1);
		ТекстЗапроса = СтрЗаменить(
			ТекстЗапроса, "&РегистрДанныхОбъектДоступа2", ПсевдонимСТочкой + СведенияОПолях.ОбъектДоступа2);
		ТекстЗапроса = СтрЗаменить(
			ТекстЗапроса, "&РегистрДанныхОбъектДоступа3", ПсевдонимСТочкой + СведенияОПолях.ОбъектДоступа3);
		
		ТекстЗапросаПроверки = СтрЗаменить(ТекстЗапросаПроверки, "&ИмяРегистраБезКавычек", Выборка.Регистр);
		ТекстЗапросаПроверки = СтрЗаменить(ТекстЗапросаПроверки, "&ИмяРегистраВКавычках", """" + Выборка.Регистр + """");
		ТекстЗапросаПроверки = СтрЗаменить(
			ТекстЗапросаПроверки, "&РегистрДанныхОбъектДоступа1", ПсевдонимСТочкой + СведенияОПолях.ОбъектДоступа1);
		ТекстЗапросаПроверки = СтрЗаменить(
			ТекстЗапросаПроверки, "&РегистрДанныхОбъектДоступа2", ПсевдонимСТочкой + СведенияОПолях.ОбъектДоступа2);
		ТекстЗапросаПроверки = СтрЗаменить(
			ТекстЗапросаПроверки, "&РегистрДанныхОбъектДоступа3", ПсевдонимСТочкой + СведенияОПолях.ОбъектДоступа3);
	...
	Пока Выборка.Следующий() Цикл
		...
		РезультатЗапросаПроверки = ЗапросДляПроверкиСсылок.Выполнить(); //@skip-check query-in-loop - обработка порциями
	...
	ЗаписьЖурналаРегистрации(
		...
		СтрШаблон(НСтр("ru = 'Процедура завершена успешно, обработано %1 записей'"), Выборка.Количество()));
...
// 
// Возвращаемое значение:
//  СправочникОбъект.ДескрипторыДоступаРегистров - Создать новый дескриптор
Функция СоздатьНовыйДескриптор(ИдентификаторОбъектаМетаданных, Запись) Экспорт
	...
	Дескриптор.Регистр = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ИдентификаторОбъектаМетаданных, "ПолноеИмя");
...
// 
// Параметры:
//  Дескриптор - СправочникОбъект.ДескрипторыДоступаРегистров
// 
// Возвращаемое значение:
//  Неопределено
//  СправочникСсылка.ДескрипторыДоступаРегистров
...
//  ИдРегистра - СправочникСсылка.ИдентификаторыОбъектовМетаданных - Назначенный переданным дескрипторам.
...
Процедура РассчитатьПраваЗапросом(Дескрипторы, ИдРегистра, ПротоколРасчетаПрав = Неопределено) Экспорт
	...
	Если Не ЗначениеЗаполнено(ИдРегистра) Тогда
		// Ненормальная ситуация, мог придти еще не записанный дескриптор (ссылка назначена, но еще не записана).
		Фраза = СтрШаблон(
			НСтр("ru = '%1. Источник %2'"),
			ДокументооборотПраВаДоступа.Текст_ПустойИОМ(), "ДескрипторыДоступаРегистров.РассчитатьПраваЗапросом");
	...
	Пока ВыборкаПоДескрипторам.Следующий() Цикл
		...
		// Запись новых прав.
		Если ДокументооборотПраваДоступа.ЗаписатьРассчитанныеПраваПоштучно(
			АктуальныхЗаписейВсего, ИзмененныхЗаписейВсего) Тогда
			...
			Пока Выборка.Следующий() Цикл
				...
				Запись = РегистрыСведений.ПраваПоДескрипторамДоступаРегистров.СоздатьМенеджерЗаписи();
