...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ЭтоМобильныйКлиент Тогда
		ЭтоМобильныйКлиентСШирокимЭкраном = МК_КлиентСервер.ЭтоМобильныйСБольшимЭкраном();
	...
	ОтображатьПолеСотрудник = РаботаСРабочимКалендаремСервер.ПолучитьПерсональнуюНастройку(
	...
	ДоступноИзменениеКалендаря = РаботаСРабочимКалендаремСервер.ДоступноИзменениеКалендаря(
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	Если ИмяСобытия = "Запись_ЗанятостьФизЛица"
		Или ИмяСобытия = "Запись_Мероприятие"
		Или ИмяСобытия = "Запись_ДоступноеВремяФизЛица"
...
Процедура СотрудникПриИзменении(Элемент)
	...
	ДоступноИзменениеКалендаря = РаботаСРабочимКалендаремСервер.ДоступноИзменениеКалендаря(ТекущийПользователь, Объект.Сотрудник);
...
Процедура ПланировщикПередНачаломРедактирования(Элемент, НовыйЭлемент, СтандартнаяОбработка)
	...
	РаботаСРабочимКалендаремКлиент.ОбработкаПередНачаломРедактированиемЭлемента(Элемент,
		НовыйЭлемент, СтандартнаяОбработка, СотрудникиКлиентПовтИсп.ФизЛицоСотрудника(Объект.Сотрудник));
...
Процедура ПланировщикПередСозданием(Элемент, Начало, Конец, Значения, Текст, СтандартнаяОбработка)
	...
	РаботаСРабочимКалендаремКлиент.ОбработкаПередСозданием(
		...
		СтандартнаяОбработка,
		Ложь,
		СотрудникиКлиентПовтИсп.ФизЛицоСотрудника(Объект.Сотрудник),
...
Процедура ПланировщикПриОкончанииРедактирования(Элемент, НовыйЭлемент, ОтменаРедактирования)
	...
	РаботаСРабочимКалендаремКлиент.ОбработкаОкончанияРедактированияЭлемента(
		...
		Планировщик, НастройкиОтображения, УникальныйИдентификатор,
		СотрудникиКлиентПовтИсп.ФизЛицоСотрудника(Объект.Сотрудник));
...
&НаКлиенте
Процедура МК_ТекущаяСтраница(Команда)

	МК_НастроитьКнопкиПереключенияСтраниц(ЭтотОбъект, Истина);
...
Процедура ОбновитьПредставлениеМестногоЧасовогоПояса()
	...
	Если НастройкаРежимОтображенияМестногоВремени <> Перечисления.РежимыОтображенияМестногоВремени.ВремяСеанса Тогда
		...
		ЧасовойПоясПользователя = РаботаСЧасовымиПоясамиПовтИсп.ЧасовойПоясПользователя(
			Сотрудники.ПользовательСотрудника(Объект.Сотрудник));
		...
		Если ЧасовойПоясПользователя <> ЧасовойПоясПоУмолчанию Тогда
			ПредставлениеМестногоЧасовогоПояса =
				РаботаСЧасовымиПоясами.ПредставлениеЧасовогоПоясаДО(ЧасовойПоясПользователя);
		Иначе
			ПредставлениеМестногоЧасовогоПояса = "";
		КонецЕсли;
...
Процедура УстановитьВидимостьЭлементовФормы()
	...
	ФизЛицоСотрудника = Сотрудники.ФизЛицоСотрудника(Объект.Сотрудник);
	...
	Элементы.Автор.Видимость = Не Сотрудники.ЭтоСотрудникиОдногоФизЛица(
		Объект.Сотрудник, Объект.Автор);
...
Процедура ИнициализироватьКалендарь()
	...
	ЧасовойПоясТекущегоПользователя =
		РаботаСЧасовымиПоясамиПовтИсп.ЧасовойПоясПользователя(Пользователи.ТекущийПользователь());
...
Процедура ОбновитьНастройкиОтображения()
	...
	НастройкиОтображения.ФизЛица.Добавить(Сотрудники.ФизЛицоСотрудника(Объект.Сотрудник));
...
Процедура ОбновитьНастройкиОтображенияКлиент()
	...
	НастройкиОтображения.ФизЛица.Очистить();
	НастройкиОтображения.ФизЛица.Добавить(
		СотрудникиКлиентПовтИсп.ФизЛицоСотрудника(Объект.Сотрудник));
...
Процедура МК_НастроитьЭлементыФормы()
	...
	Элементы.ГруппаЗаписьКалендаря.РастягиватьПоГоризонтали = Истина;
	
	Элементы.Описание.Высота = 1;
	
	Элементы.ГруппаДатаВремя.ГоризонтальныйИнтервал = ИнтервалМеждуЭлементамиФормы.Половинный;
	
	Элементы.ГруппаДатаВремя.Группировка =
		ГруппировкаПодчиненныхЭлементовФормы.ГоризонтальнаяЕслиВозможно;
	
	Элементы.ГруппаНастройкиВремени.Группировка =
		ГруппировкаПодчиненныхЭлементовФормы.ГоризонтальнаяЕслиВозможно;
	
	Элементы.ГруппаЦвет.Группировка =
		ГруппировкаПодчиненныхЭлементовФормы.ГоризонтальнаяЕслиВозможно;
	
	Элементы.ГруппаЦвет.ГоризонтальныйИнтервал = ИнтервалМеждуЭлементамиФормы.Нет;

	Элементы.ГруппаДанныеЗаписиКалендаря.ОтображатьЗаголовок = Истина;
	
	Элементы.ГруппаДанныеЗаписиКалендаря.Поведение = ПоведениеОбычнойГруппы.Свертываемая;
	
	ЗаголовокГруппыСДаннымиЗаписями =НСтр("ru ='Прочее (цвет, напомнить, ...)'");

	Элементы.ГруппаДанныеЗаписиКалендаря.Заголовок = ЗаголовокГруппыСДаннымиЗаписями;

	Элементы.ГруппаДанныеЗаписиКалендаря.ЗаголовокСвернутогоОтображения =
		ЗаголовокГруппыСДаннымиЗаписями;
	
	Если Не ЭтоМобильныйКлиентСШирокимЭкраном Тогда
		МК_НастроитьКнопкиПереключенияСтраниц(ЭтотОбъект);
	Иначе
		Если ОбщегоНазначенияКлиентСервер.ТипУстройства() <> "Планшет" Тогда
			Элементы.ГруппаДанныеЗаписиКалендаря.Скрыть();
		КонецЕсли;
	КонецЕсли;
	...
	Элементы.Переместить(Элементы.Описание,
		Элементы.ГруппаЗаписьКалендаря,
		Элементы.ГруппаДатаВремя);
	
	Элементы.Описание.Ширина = 0;
	Элементы.Описание.РастягиватьПоВертикали = Истина;
	
	Элементы.Переместить(Элементы.ДатаОкончания,
		Элементы.ГруппаДатаОкончания,
		Элементы.ДатаОкончанияВремя);
	
	Элементы.ДекорацияРазделительВремени.Видимость = Ложь;
	
	Элементы.МК_НадписьЗаголовокС.Видимость = Истина;
	Элементы.МК_НадписьЗаголовокПо.Видимость = Истина;
	
	Элементы.ГруппаДатаНачала.Ширина = 15;
	Элементы.ГруппаДатаОкончания.Ширина = 15;
	
	Элементы.Переместить(Элементы.ПредставлениеМестногоЧасовогоПояса,
		Элементы.ГруппаЗаписьКалендаря,
		Элементы.ГруппаНастройкиВремени);
	
	Элементы.Переместить(Элементы.ДлительностьСтр,
		Элементы.ГруппаЗаписьКалендаря,
		Элементы.ГруппаНастройкиВремени);
	...
	Элементы.Переместить(Элементы.ИнтервалВремениСтрокой,
		Элементы.ГруппаНапоминаниеОтносительноДатыНачала,
	...
	Элементы.Переместить(Элементы.Предмет,
		Элементы.ГруппаЗаписьКалендаря,
		Элементы.ГруппаДанныеЗаписиКалендаря);
	
	Элементы.Переместить(Элементы.Состояние,
		Элементы.ГруппаЗаписьКалендаря,
		Элементы.ГруппаДанныеЗаписиКалендаря);
	...
	Элементы.КоманднаяПанельЦвета.Видимость = Ложь;
	...
	МК.ПреобразоватьКнопкуВАкцентную(ЭтотОбъект,
		...
		Элементы.МК_Группа_Низ);

КонецПроцедуры

&НаКлиентеНаСервереБезКонтекста
Процедура МК_НастроитьКнопкиПереключенияСтраниц(Форма, Сменить = Ложь)
	...
	Если Сменить Тогда
		...
		Если Форма.МК_ИндексТекущейСтраницы = 0 Тогда
			Форма.МК_ИндексТекущейСтраницы = 1;
		Иначе
			Форма.МК_ИндексТекущейСтраницы = 0;
		КонецЕсли;
		
		Элементы.МК_Страницы.ТекущаяСтраница =
			Элементы.МК_Страницы.ПодчиненныеЭлементы[Форма.МК_ИндексТекущейСтраницы];
		
		МК_КлиентСервер.ОформитьДополнительнуюКнопкуПоСостоянию(ЭлементыСтиля,
			Элементы.МК_Основное,
			Форма.МК_ИндексТекущейСтраницы = 0);
		
		МК_КлиентСервер.ОформитьДополнительнуюКнопкуПоСостоянию(ЭлементыСтиля,
			Элементы.МК_Календарь,
			Форма.МК_ИндексТекущейСтраницы = 1);
	Иначе
		Форма.МК_ИндексТекущейСтраницы = 0;
...
&НаСервере
Процедура МК_ИнициализироватьКалендарь()
	
	ЭтоМобильныйСБольшимЭкраном = МК_КлиентСервер.ЭтоМобильныйСБольшимЭкраном();
		
	Элементы.МК_Страницы.Видимость = Не ЭтоМобильныйСБольшимЭкраном;
	Элементы.МК_ГруппаЗаголовокСтраниц.Видимость = Не ЭтоМобильныйСБольшимЭкраном;
	
	Элементы.ГруппаКалендарь.Видимость = ПоказыватьКалендарь;
	
	Если Не ЭтоМобильныйСБольшимЭкраном Тогда
		
		Элементы.Переместить(Элементы.ГруппаЗаписьКалендаря,
			Элементы.МК_СтраницаЗаписьКалендаря);
		
		Элементы.Переместить(Элементы.ГруппаКалендарь,
			Элементы.МК_СтраницаКалендарь);

			Элементы.ГруппаКалендарь.РастягиватьПоГоризонтали = Истина;
			Элементы.МК_ГруппаЗаголовокСтраниц.Видимость = ПоказыватьКалендарь;
			Элементы.МК_СтраницаКалендарь.Видимость = ПоказыватьКалендарь;
	Иначе
		Элементы.Переместить(Элементы.ГруппаЗаписьКалендаря,
			Элементы.МК_Содержимое);
		Элементы.ГруппаКалендарь.Ширина = 20;
		Элементы.ГруппаКалендарь.РастягиватьПоГоризонтали = Ложь;
		Элементы.Переместить(Элементы.ГруппаКалендарь,
			Элементы.МК_Содержимое);
	КонецЕсли;
