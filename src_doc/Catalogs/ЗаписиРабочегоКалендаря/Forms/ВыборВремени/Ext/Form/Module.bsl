...
Процедура ОтобразитьКалендарьЗанятости()
	...
	ФизЛицаПоСотрудникам = Сотрудники.ФизЛицаПоСотрудникам(ПолучитьМассивУчастников(Ложь));
	
	Для Каждого СотрудникФизЛицо Из ФизЛицаПоСотрудникам Цикл
		
		Если НастройкиОтображения.ПредставленияИзмерений[СотрудникФизЛицо.Значение] <> Неопределено Тогда
			Продолжить;
		КонецЕсли;
		
		НастройкиОтображения.ФизЛица.Добавить(СотрудникФизЛицо.Значение);
		
		НастройкиОтображения.ПредставленияИзмерений[СотрудникФизЛицо.Значение] =
			РаботаСЗадачамиПовтИсп.ПредставлениеУчастника(СотрудникФизЛицо.Ключ);
...
Процедура ОбновитьРекомендации()
	...
	ФизЛицаУчастников = Сотрудники.ФизЛицаСотрудников(МассивУчастников);
	
	МассивОбязательныхУчастников = ПолучитьМассивУчастников(Истина);
	ФизЛицаОбязательныхУчастников = Сотрудники.ФизЛицаСотрудников(МассивОбязательныхУчастников);
	
	ТаблицаРекомендаций = РаботаСРабочимКалендаремСервер.ПолучитьТаблицуРекомендаций(ФизЛицаУчастников,
		ОтображаемаяДата, ОтображаемаяДатаКонецПериода, Длительность, Мероприятие);
	ТаблицаРекомендаций.Колонки.Добавить("ДоступныВсе");
	ТаблицаРекомендаций.ЗаполнитьЗначения(Истина, "ДоступныВсе");
	
	Если ТаблицаРекомендаций.Количество() = 0 Тогда
		
		ТаблицаРекомендацийОбязательныхУчастников =
			РаботаСРабочимКалендаремСервер.ПолучитьТаблицуРекомендаций(ФизЛицаОбязательныхУчастников,
				ОтображаемаяДата, ОтображаемаяДатаКонецПериода, Длительность, Мероприятие);
	...
	ТаблицаРекомендаций.Сортировать("ДоступныВсе Убыв, ДатаНачала");
...
&НаСервере
Функция ПолучитьРекомендациюСтрокой(РекомендацияДатаНачала, РекомендацияДатаОкончания, ДоступныВсе)
	...
	Если ДоступныВсе Тогда
		ДоступныеСотрудники = НСтр("ru = 'Свободны все участники'");
	Иначе
		ДоступныеСотрудники = НСтр("ru = 'Свободны только обязательные участники'");
	КонецЕсли;
	ДобавитьЗначениеКСтрокеЧерезРазделитель(РекомендацияСтрокой, "   ", ДоступныеСотрудники);
	...
	Для Каждого СтрокаРекомендации Из ТаблицаРекомендаций Цикл
		...
		Если ТребуетсяКорректировкаПоВремениОтображения() 
			...
			ДобавитьРекомендацию(СтрокаРекомендации.ДатаНачала, НачалоТекущегоДня + ОкончаниеОтображения,
				СтрокаРекомендации.ДоступныВсе);
...
&НаСервере
Процедура ДобавитьРекомендацию(РекомендацияДатаНачала, РекомендацияДатаОкончания, ДоступныВсе)
	...
	НоваяРекомендация.РекомендацияСтрокой = ПолучитьРекомендациюСтрокой(РекомендацияДатаНачала,
		РекомендацияДатаОкончания, ДоступныВсе);
