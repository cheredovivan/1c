...
Процедура ОбработкаЗаполнения(ДанныеЗаполнения, СтандартнаяОбработка)
	...
	Если ТипЗнч(ДанныеЗаполнения) = Тип("Структура") Тогда
		...
		Если ДанныеЗаполнения.Свойство("Предмет") И ЗначениеЗаполнено(ДанныеЗаполнения.Предмет) Тогда
			...
			ИначеЕсли ТипЗнч(ДанныеЗаполнения.Предмет) = Тип("ДокументСсылка.ВходящееПисьмо")
				...
				Событие = РаботаСАнализомТекстаВызовСервера.НайтиСобытие(ТекстПисьма, ДанныеЗаполненияОбъект.Дата);
			...
			ИначеЕсли ТипЗнч(ДанныеЗаполнения.Предмет) = Тип("СправочникСсылка.Контроль") Тогда 
				...
				Если ЗначениеЗаполнено(ДанныеЗаполнения.Предмет.СрокИсполнения)
					...
					Иначе
						ДатаОкончания = КонецДня(ДанныеЗаполнения.Предмет.СрокИсполнения)
					КонецЕсли;
			...
			ИначеЕсли ТипЗнч(ДанныеЗаполнения.Предмет) = Тип("ДокументСсылка.Задача") Тогда
				
				Задача = ДанныеЗаполнения.Предмет;
				ЗадачаОбъект = Задача.ПолучитьОбъект();
				
				Описание = ЗадачаОбъект.ОписаниеПростойТекст();
				
			ИначеЕсли ТипЗнч(ДанныеЗаполнения.Предмет) = Тип("ДокументСсылка.ДействиеЗадачи") Тогда
				
				ДействиеЗадачи = ДанныеЗаполнения.Предмет;
				ДействиеЗадачиОбъект = ДействиеЗадачи.ПолучитьОбъект();
				
				Описание = ДействиеЗадачиОбъект.ОписаниеПростойТекст();
		...
		Если ДанныеЗаполнения.Свойство("ПовторяющеесяСобытие") И ЗначениеЗаполнено(ДанныеЗаполнения.ПовторяющеесяСобытие)
			...
			ЗаполнитьЗначенияСвойств(ЭтотОбъект, ДанныеЗаполнения.ПовторяющеесяСобытие,
				"ВесьДень, ДатаНачала, ДатаОкончания, Описание, Сотрудник, Предмет, Состояние");
	...
	Если ЭтоНовый() Тогда
		...
		Если Не ЗначениеЗаполнено(Состояние)
			И ТипЗнч(Предмет) <> Тип("ДокументСсылка.Отсутствие")
			И Не Сотрудники.ЭтоСотрудникиОдногоФизЛица(Сотрудник, Автор) Тогда
...
Процедура ПередЗаписью(Отказ)
	...
	Если Связанная
		И Не ДополнительныеСвойства.Свойство("ИзменениеСвязаннойЗаписи")
		И Не ПараметрыСеанса.ЗагрузкаОбработанныхДанныхИзДругойСистемы Тогда
		
		ФизЛицоСотрудника = Сотрудники.ФизЛицоСотрудника(Сотрудник);
		...
		Если ЗначениеЗаполнено(Предмет) Тогда
			ТекстРекомендованногоДействия = СтрШаблон(
				...
				Предмет,
				ТипЗнч(Предмет))
		ИначеЕсли ЗначениеЗаполнено(СинхронизацияКалендарей.УзлыДляВыгрузки(ФизЛицоСотрудника)) Тогда
	...
	Если ДатаНачала >= ДатаОкончания И Не ПараметрыСеанса.ЗагрузкаОбработанныхДанныхИзДругойСистемы Тогда
		Отказ = Истина;
...
Процедура ПриЗаписи(Отказ)
	...
	Если Не ПараметрыСеанса.ЗагрузкаОбработанныхДанныхИзДругойСистемы Тогда
		...
		Если ПометкаУдаления И ПолучитьФункциональнуюОпцию("ИспользоватьPushУведомления") Тогда
		...
		Если НЕ ПометкаУдаления И ПолучитьФункциональнуюОпцию("ИспользоватьPushУведомления") Тогда
			...
			// Для новых записей календаря созданными для других сотрудников создаем уведомления всегда.
			...
			Если ДополнительныеСвойства.Свойство("ЭтоНовый", ЭтоНовый)
				...
				МП_ОтправкаУведомлений.ЗаписатьСообщениеВРегистрPushУведомления(
					ПользовательСотрудника, Ссылка, ТекущаяДата() + 10, Сообщение);
			...
			Если УстановитьНапоминание Тогда
				...
				Если Не ЭтоНовый Тогда
					Набор = РегистрыСведений.СообщенияPushУведомлений.СоздатьНаборЗаписей();
					Набор.Отбор.Пользователь.Установить(ПользовательСотрудника);
					Набор.Отбор.ОбъектУведомления.Установить(Ссылка);
					Набор.Записать(Истина);
				КонецЕсли;
				...
				МП_ОтправкаУведомлений.ЗаписатьСообщениеВРегистрPushУведомления(
					ПользовательСотрудника, Ссылка,
					ДатаНачала - ИнтервалВремениНапоминания, Сообщение);
...
Функция ПолучитьДатуНачалаИДатуОкончания(ДатаЗаписи)
	...
	Если ЗначениеЗаполнено(ДатаОкончанияПовторения) И ДатаЗаписи > КонецДня(ДатаОкончанияПовторения) Тогда
		Возврат Неопределено;
	КонецЕсли;
