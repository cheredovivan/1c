...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ИмяОбработчика1 = "Документы.ВходящееПисьмо.ОбработатьДанныеДляПереходаНаНовуюВерсию_3_0_17_17";
	ИмяОбработчика2 = "Справочники.АдресатыПочтовыхСообщений.ОбработатьДанныеДляПереходаНаНовуюВерсию_3_0_17_17";
	ПервоначальноеЗаполнениеЗавершено 
		= ОбщегоНазначенияДокументооборот.ОтложенноеОбновлениеЗавершено(ИмяОбработчика1)
		И ОбщегоНазначенияДокументооборот.ОтложенноеОбновлениеЗавершено(ИмяОбработчика2);
		
	Если Не ПервоначальноеЗаполнениеЗавершено Тогда
		Список.ТекстЗапроса = ВстроеннаяПочтаСервер.ТекстЗапросаДо3_0_17(); 
	КонецЕсли;	
	...
	Список.Параметры.УстановитьЗначениеПараметра("ФизическоеЛицо", ПараметрыСеанса.ФизическоеЛицоПользователя);
...
#КонецЕсли	
	...
	ПодключитьОбработчикОжидания("АвтозаписьНастроек", 1 * 60); // 5 * 60
...
Процедура АвтозаписьНастроек()
	...
	Если СохранятьНастройки И НастройкиИзменены Тогда
...
Процедура ПометитьКакПрочтенныеПисьма(Письма)
	...
	КлючеваяОперация = "ПочтаПометитьПисьмоКакПрочитанное";
...
Процедура УстановитьОтборПоПапкеКлиент(Папка, ТекущаяСтрока = Неопределено)
	...
	Если ТипЗнч(Папка) = Тип("СправочникСсылка.ПапкиПоиска") Тогда
		...
		КлючеваяОперация = "ПочтаВыполнениеКомандыОтборПоПапкеПоиска";
		
	ИначеЕсли ТипЗнч(Папка) = Тип("СправочникСсылка.ПапкиПисем") Тогда
		
		КлючеваяОперация = "ПочтаВыполнениеКомандыОтборПоПапке";
...
Процедура ОбработчикСписокПриАктивизацииСтроки()
	...
	Если ОбластьЧтенияВключена Тогда
		...
		Если ЗначениеЗаполнено(ТекущееПисьмо) Тогда
			...
			КлючеваяОперация = "ПочтаОтображениеПисьмаВОбластиЧтения";
			ОценкаПроизводительностиКлиент.ЗамерВремени(КлючеваяОперация);
...
Функция ОбработатьАктивизациюСтроки(
	...
	ПротоколированиеРаботыСотрудников.ЗаписатьОткрытие(Письмо);
...
Процедура СписокВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	ИначеЕсли Элемент.ТекущийЭлемент = Элементы.ЕстьДокументы Тогда
		
		СтандартнаяОбработка = Ложь;
		
		ЕстьДокументы = ВстроеннаяПочтаСервер.ДокументыНаОсновании(ВыбранноеПисьмо);
		
		Если ТипЗнч(ЕстьДокументы) <> Тип("Строка") Тогда // ссылка на документ
			ПоказатьЗначение(, ЕстьДокументы);
		ИначеЕсли ЗначениеЗаполнено(ЕстьДокументы) Тогда	
			...
			ОткрытьФорму("ОбщаяФорма.СвязиОбъекта",
				Новый Структура("ОсновнойОбъект, ТолькоВажные, ТолькоСвязиВОтветНа, ТолькоПисьмоОснование", 
					ВыбранноеПисьмо, Ложь, Ложь, Истина),,
				Новый УникальныйИдентификатор);
	...
	ИначеЕсли ВстроеннаяПочтаКлиентСервер.ЭтоВходящееПисьмо(ВыбранноеПисьмо) Тогда
		...
		Если ДействиеПриВыбореВходящегоПисьма =
			...
			СтандартнаяОбработка = Ложь;
			ВстроеннаяПочтаКлиент.ОткрытьПисьмо(ВыбранноеПисьмо);
	...
	ИначеЕсли ВстроеннаяПочтаКлиентСервер.ЭтоИсходящееПисьмо(ВыбранноеПисьмо) Тогда	
...
Процедура ПредпросмотрHTMLПриНажатии(Элемент, ДанныеСобытия, СтандартнаяОбработка)
	...
	СтандартнаяОбработка = Ложь;
	ВстроеннаяПочтаКлиент.ОткрытьСсылку(ДанныеСобытия.Href, ДанныеСобытия.Element, ТекущееПисьмо, Элемент.Документ, ЭтаФорма);
...
Процедура ПереместитьВПапкуКлиент(Письма, Папка)
	...
	КлючеваяОперация = "ПочтаПереносПисьмаИзПапкиВПапку";
	ОценкаПроизводительностиКлиент.ЗамерВремени(КлючеваяОперация);
...
Процедура УстановитьОтборСпискаКлиент()
	...
	УстановитьОтборСпискаИПроверитьТипОтбора(БыстрыйПоискВключен);
	
КонецПроцедуры

&НаСервере
Функция ЕстьОтборПоТекстуПисем()
	
	Если Не БыстрыйПоискВключен Тогда
		Возврат Ложь;
	КонецЕсли;
	
	Если Не ИспользоватьППДДляПоискаПоТекстуПисем Тогда
		Возврат Ложь;
	КонецЕсли;
...
&НаСервере
Функция ЕстьОтборПоПериоду()
	
	Для Каждого Строка Из ОтборВПапке Цикл
		
		Если (Строка.Параметр = "Дата_С" Или Строка.Параметр = "Дата_По")
			И ЗначениеЗаполнено(Строка.Значение) Тогда
...
&НаСервере
Функция ПапкаПоискаДляПоискаПоТексту(Папка = Неопределено)
	
	ИмяПапки = НСтр("ru = 'Для поиска по тексту писем'");
	
	Запрос = Новый Запрос;
	Запрос.Текст = 
		"ВЫБРАТЬ ПЕРВЫЕ 1
		|	ПапкиПоиска.Ссылка КАК Ссылка
		|ИЗ
		|	Справочник.ПапкиПоиска КАК ПапкиПоиска
		|ГДЕ
		|	ПапкиПоиска.Наименование = &Наименование
		|	И ПапкиПоиска.ПометкаУдаления = ИСТИНА
		|	И ПапкиПоиска.Ответственный = &Ответственный";
		
	Запрос.УстановитьПараметр("Ответственный", Пользователи.ТекущийПользователь());
	Запрос.УстановитьПараметр("Наименование", ИмяПапки);
	
	ПапкаСсылка = Неопределено;
	Результат = Запрос.Выполнить();
	Если Не Результат.Пустой() Тогда
		ТаблицаПапок = Результат.Выгрузить();
		ПапкаСсылка = ТаблицаПапок[0].Ссылка;
	Иначе	
		ПапкаОбъект = Справочники.ПапкиПоиска.СоздатьЭлемент();
		ПапкаОбъект.Наименование = ИмяПапки;
		ПапкаОбъект.ПометкаУдаления = Истина;
		ПапкаОбъект.Ответственный = Пользователи.ТекущийПользователь();
		ПапкаОбъект.Записать();
		ПапкаСсылка = ПапкаОбъект.Ссылка;
	КонецЕсли;
	
	ПапкаПоискаОбъект = ПапкаСсылка.ПолучитьОбъект();
	
	ЗаполнитьТаблицуУсловий();
	
	СоответствиеИмен = Новый Соответствие;
	
	СоответствиеИмен.Вставить("Адресаты", Перечисления.ВидыУсловийПоискаПисем.ПолученоОт);
	СоответствиеИмен.Вставить("Тема", Перечисления.ВидыУсловийПоискаПисем.ТемаСодержитУказанныеСлова);
	СоответствиеИмен.Вставить("Текст", Перечисления.ВидыУсловийПоискаПисем.ТекстСодержитУказанныеСлова);
	СоответствиеИмен.Вставить("Дата_С", Перечисления.ВидыУсловийПоискаПисем.ЗаПериод);
	СоответствиеИмен.Вставить("Дата_По", Перечисления.ВидыУсловийПоискаПисем.ЗаПериод);
	СоответствиеИмен.Вставить("Флаг", Перечисления.ВидыУсловийПоискаПисем.ОтмеченоФлагом);
	СоответствиеИмен.Вставить("Комментарий", Перечисления.ВидыУсловийПоискаПисем.КомментарийСодержитСлова);
	
	ЕстьОтборПериода = ЕстьОтборПоПериоду();
	
	// тут заполняем УсловияПоискаПисем из ОтборВПапке
	Для Каждого строка Из ОтборВПапке Цикл
		
		ИмяПараметра = Строка.Параметр;
		ВидВПапкеПоиска = СоответствиеИмен[ИмяПараметра];
		
		Если ЗначениеЗаполнено(Строка.Значение)
			И Строка.Значение <> Неопределено Тогда
			ВстроеннаяПочтаСервер.УстановитьДоступностьКомандыСбросаОтбора(Истина, ЭтаФорма);
		КонецЕсли;	
		
		Если ВидВПапкеПоиска <> Неопределено Тогда
			
			Для Каждого СтрокаУсловия Из УсловияПоискаПисем Цикл
				
				Если СтрокаУсловия.ВидУсловия = ВидВПапкеПоиска Тогда
					
					Если СтрокаУсловия.ВидУсловия <> Перечисления.ВидыУсловийПоискаПисем.ЗаПериод Тогда
						
						Если ЗначениеЗаполнено(строка.Значение) Тогда
							
							Если СтрокаУсловия.ВидУсловия = Перечисления.ВидыУсловийПоискаПисем.ПолученоОт Тогда
								СтрокаУсловия.ВидУсловия = Перечисления.ВидыУсловийПоискаПисем.ПерепискаС;
							КонецЕсли;	
							
							СтрокаУсловия.ЗначенияУсловия.Добавить(строка.Значение);
							СтрокаУсловия.ПредставлениеЗначенияУсловия = строка.ПредставлениеПараметра;
						КонецЕсли;
					ИначеЕсли ЕстьОтборПериода Тогда // за период
						...
						СтрокаУсловия.ЗначенияУсловия.Добавить(Дата);
						СтрокаУсловия.ПредставлениеЗначенияУсловия = СтрокаУсловия.ПредставлениеЗначенияУсловия + Строка(Дата) + " ";
	...
	ПапкаДляПоиска = Папка;
	Если ПапкаДляПоиска = Неопределено Тогда
		ПапкаДляПоиска = ТекущаяПапка;
	КонецЕсли;	
	
	Для Каждого СтрокаУсловия Из УсловияПоискаПисем Цикл
		
		Если СтрокаУсловия.ВидУсловия = Перечисления.ВидыУсловийПоискаПисем.Удалено Тогда
			
			Если ОтображатьУдаленныеПисьмаИПапки = Ложь Тогда
				СтрокаУсловия.ЗначенияУсловия.Очистить();
				СтрокаУсловия.ЗначенияУсловия.Добавить(Ложь);
				СтрокаУсловия.ПредставлениеЗначенияУсловия = Строка(Ложь);
			Иначе
				СтрокаУсловия.ЗначенияУсловия.Очистить();
			КонецЕсли;	
			
		ИначеЕсли СтрокаУсловия.ВидУсловия = Перечисления.ВидыУсловийПоискаПисем.ВыбратьПервые Тогда
			
			СтрокаУсловия.ЗначенияУсловия.Очистить();
			СтрокаУсловия.ЗначенияУсловия.Добавить(1000);
			СтрокаУсловия.ПредставлениеЗначенияУсловия = Строка(1000);
			
		ИначеЕсли СтрокаУсловия.ВидУсловия = Перечисления.ВидыУсловийПоискаПисем.ХранитсяВПапке Тогда
			
			СтрокаУсловия.ЗначенияУсловия.Очистить();
			СтрокаУсловия.ЗначенияУсловия.Добавить(ПапкаДляПоиска);
			СтрокаУсловия.ПредставлениеЗначенияУсловия = Строка(ПапкаДляПоиска);
			
		ИначеЕсли СтрокаУсловия.ВидУсловия = Перечисления.ВидыУсловийПоискаПисем.ОтноситсяКУчетнойЗаписи Тогда	
			
			УчетнаяЗапись = ВстроеннаяПочтаСерверПовтИсп.УчетнаяЗаписьДляПапки(ПапкаДляПоиска);
			Если УчетнаяЗапись <> Неопределено Тогда
				
				СтрокаУсловия.ЗначенияУсловия.Очистить();
				СтрокаУсловия.ЗначенияУсловия.Добавить(УчетнаяЗапись);
				СтрокаУсловия.ПредставлениеЗначенияУсловия = Строка(УчетнаяЗапись);
			...
			Для Каждого СтрокаТекущейПапки Из НастройкиПапки Цикл
				Если Строка.ВидУсловия = СтрокаТекущейПапки.ВидУсловия Тогда
					...
					Строка.ПредставлениеЗначенияУсловия = 
						ПредставлениеСпискаЗначенийДляУсловия(
							Строка.ВидУсловия, 
...
Функция ПредставлениеСпискаЗначенийДляУсловия(ВидУсловия, СписокЗначений)
	...
	Если ВидУсловия = ПредопределенноеЗначение("Перечисление.ВидыУсловийПоискаПисем.ТемаСодержитУказанныеСлова")
		...
		ИЛИ ВидУсловия = ПредопределенноеЗначение("Перечисление.ВидыУсловийПоискаПисем.КомментарийСодержитСлова")
		 Тогда
...
&НаСервере
Процедура ВыполнитьОтборПоТекстуПисем(Папка = Неопределено)
	...
	ИспользованОтборПоСкрытойПапкеПоиска = Истина;
	
	ПапкаПоиска = ПапкаПоискаДляПоискаПоТексту(Папка);      
	...
	УстановитьОтборПоПапкеСервер(ПапкаПоиска, , Ложь);

КонецПроцедуры                   


&НаСервере
// Проверяет, не включен ли поиск по тексту писем - тогда неявно меняет на папку поиска
Процедура УстановитьОтборСпискаИПроверитьТипОтбора(ПоискВключен)
	
	ЕстьОтборПоТексту = ЕстьОтборПоТекстуПисем();
	
	Если ЕстьОтборПоТексту И ТипЗнч(ТекущаяПапка) = Тип("СправочникСсылка.ПапкиПисем") Тогда
		
		ВыполнитьОтборПоТекстуПисем();
		
	Иначе	
		
		Если ИспользованОтборПоСкрытойПапкеПоиска Тогда
			
			ИспользованОтборПоСкрытойПапкеПоиска = Ложь;
			
			ПапкаОтбора = ТекущаяПапка;
			УстановитьОтборПоПапкеСервер(ПапкаОтбора);
				
		КонецЕсли;	
		
		ВстроеннаяПочтаСервер.УстановитьОтборСписка(ПоискВключен, ЭтаФорма);
	КонецЕсли;	
...
Процедура ВложенияВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	Если Элемент.ТекущиеДанные <> Неопределено Тогда
		...
		ИначеЕсли ВстроеннаяПочтаКлиентСервер.ЭтоФайлПриглашение(Элемент.ТекущиеДанные.Представление, РасширенияICS) Тогда	
				
			ВстроеннаяПочтаКлиент.ОткрытьПриглашение(ТекущееПисьмо, Элемент.ТекущиеДанные.Ссылка, УникальныйИдентификатор);
...
&НаСервере
Процедура УстановитьОтборПоПапкеСервер(Папка, ТекущаяСтрока = Неопределено, МенятьТекущуюПапку = Истина)
	
	Если МенятьТекущуюПапку Тогда
		ТекущаяПапка = Папка;
	КонецЕсли;	
	...
	Если ТипЗнч(Папка) = Тип("СправочникСсылка.ПапкиПоиска") Тогда
		...
		Если ИспользованОтборПоСкрытойПапкеПоиска Тогда
			ВстроеннаяПочтаСервер.УстановитьОтборСписка(БыстрыйПоискВключен, ЭтаФорма); // наложим отбор, т.к. раньше его не было
		КонецЕсли;
