...
Процедура ПоказатьФотоПользователя(Контакт, УникальныйИдентификатор, Фотография)
	...
	ТекущийПолучательПредставление = Строка(Контакт);
	...
	ИначеЕсли ТипКонтакта = Тип("СправочникСсылка.Сотрудники") Тогда
		ДополнительноеОписаниеКонтакта = ОписаниеСотрудникаДляКарточки(Контакт, ТекущийПолучательПредставление);
...
&НаСервереБезКонтекста
Функция ОписаниеСотрудникаДляКарточки(Контакт, ТекущийПолучательПредставление)
	...
	ТекущийПолучательПредставление = Выборка.Наименование;
...
Функция ОписаниеПользователяДляКарточки(Контакт)
	...
	ОписаниеПользователяПодразделение = Строка(
		Сотрудники.ПодразделениеСотрудника(Сотрудники.ОсновнойСотрудникПользователя(Контакт)));
