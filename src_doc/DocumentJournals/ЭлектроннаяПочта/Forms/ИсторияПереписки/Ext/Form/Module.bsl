...
&НаКлиенте
Процедура МК_ПоказатьСкрытьВложения(Команда)
	
	Элементы.Вложения.Видимость = Не Элементы.Вложения.Видимость;
	МК_ПоказатьСкрытьВложенияНаСервере();
...
Процедура УстановитьФлаги(Флаг)
	...
	ТекущиеДанные.НомерФлага = РаботаСФлагамиОбъектовСервер.ПолучитьНомерФлага(Флаг);	
	
	МассивДляОбработки = Новый Массив;
	МассивДляОбработки.Добавить(ТекущиеДанные.Ссылка);
	РаботаСФлагамиОбъектовСервер.УстановитьФлагОбъектам(МассивДляОбработки, Флаг);
	Оповестить("ИзменилсяФлаг", МассивДляОбработки);
	
КонецПроцедуры

&НаСервереБезКонтекста
Функция ПереключитьФлагСервер(Письмо)
	
	ОсновнойФлаг = ВстроеннаяПочтаСервер.ПолучитьПерсональнуюНастройку("ОсновнойФлаг");
	
	РаботаСФлагамиОбъектовСервер.ПереключитьФлагПисьма(Письмо);
		
	НовыйФлаг = РегистрыСведений.ФлагиОбъектов.ПолучитьФлаг(Письмо, ПараметрыСеанса.ФизическоеЛицоПользователя);
	Возврат НовыйФлаг;	
...
Процедура ДеревоПисемВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	Если Элемент.ТекущийЭлемент = Элементы.ДеревоПисемНомерФлага Тогда
		
		ТекущиеДанные = ПолучитьТекущиеДанные();
		
		Если ВстроеннаяПочтаКлиентСервер.ЭтоПисьмо(ТекущиеДанные.Ссылка) Тогда
			
			ОсновнойФлаг = ПереключитьФлагСервер(ТекущиеДанные.Ссылка);
			ТекущиеДанные.НомерФлага = РаботаСФлагамиОбъектовСервер.ПолучитьНомерФлага(ОсновнойФлаг);	
...
&НаКлиенте
Процедура ДеревоПисемОбработкаЗапросаОбновления(Элемент)
...
Процедура ОбработатьАктивизациюСтрокиСписка()
	...
	Если ПоложениеОбластиЧтения <> "Отключена" Тогда
		...
		Если ВстроеннаяПочтаКлиентСервер.ЭтоПисьмо(Элементы.ДеревоПисем.ТекущиеДанные.Ссылка) Тогда
			...
			Если ЭтоМобильныйКлиент Тогда
				МК_КлиентСервер.АдаптироватьHtmlПодЭкранМобильногоПриНеобходимости(Предпросмотр);
			КонецЕсли;
...
Процедура ЗаполнитьУзелДереваПереписки(ЭлементыДерева, ПисьмоРодитель, Знач КорневойУровень)
	...
	Если ВстроеннаяПочтаКлиентСервер.ЭтоВходящееПисьмо(ПисьмоРодитель) Тогда
		...
		Флаг = РегистрыСведений.ФлагиОбъектов.ПолучитьФлаг(СтрокаКорня.Ссылка, 
			ПараметрыСеанса.ФизическоеЛицоПользователя);
		СтрокаКорня.НомерФлага = РаботаСФлагамиОбъектовСервер.ПолучитьНомерФлага(Флаг);	
	...
	ИначеЕсли ДелопроизводствоКлиентСервер.ЭтоВходящийДокумент(ПисьмоРодитель) Тогда
		...
		РеквизитыПисьма = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(
			ПисьмоРодитель,
			"ДатаРегистрации, ПодписалОтКонтрагента, Контрагент, Заголовок, Адресат, ПометкаУдаления");
			
		СтрокаКорня.Ссылка = ПисьмоРодитель;
		СтрокаКорня.Дата = РеквизитыПисьма.ДатаРегистрации;
		СтрокаКорня.ПометкаУдаления = РеквизитыПисьма.ПометкаУдаления;
		
		Если ЗначениеЗаполнено(РеквизитыПисьма.ПодписалОтКонтрагента) Тогда
			СтрокаКорня.ОтКого = РеквизитыПисьма.ПодписалОтКонтрагента;
		Иначе
			СтрокаКорня.ОтКого = РеквизитыПисьма.Контрагент;
		КонецЕсли;
	...
	ИначеЕсли ДелопроизводствоКлиентСервер.ЭтоИсходящийДокумент(ПисьмоРодитель) Тогда
		...
		КорреспонденцияДокумента = РаботаСКорреспонденцией.КорреспонденцияДокумента(ПисьмоРодитель);
		Если ЗначениеЗаполнено(КорреспонденцияДокумента) Тогда
			
			Корреспонденты = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(
				КорреспонденцияДокумента, "Корреспонденты");
			Корреспонденты = Корреспонденты.Выгрузить();	
			
			Если Корреспонденты.Количество() > 1 Тогда
				СтрокаКорня.Кому = Строка(Корреспонденты.Количество()) + ": ";
			КонецЕсли;
			
			ПервыйПолучатель = Истина;
			Для Каждого Получатель Из Корреспонденты Цикл
...
#Область СлужебныеПроцедурыИФункции_ОбластьЧтения
...
Процедура ОбновитьПоложениеОбластиЧтенияСервер()
	...
	Если ПоложениеОбластиЧтения = "Снизу" Тогда
		...
		Элементы.СтраницаПредпросмотрСВложениями.Группировка = ГруппировкаПодчиненныхЭлементовФормы.ГоризонтальнаяВсегда;
...
Процедура МК_НастроитьЭлементыФормы()
	...
	СоставКоманднойПанелиНаМобильномУстройстве.Добавить(Элементы.ГруппаКоманднаяПанель);
	
	МК_КлиентСервер.АдаптироватьHtmlПодЭкранМобильногоПриНеобходимости(Предпросмотр);
	
	Элементы.Ответить.Видимость = Ложь;
	Элементы.ОтветитьВсем.Видимость = Ложь;
	Элементы.Переслать.Видимость = Ложь;
	Элементы.Обновить.Видимость = Ложь;
	Элементы.ОбластьЧтенияСнизу.Видимость = Ложь;
	
	Элементы.ОбластьЧтенияСправа.ПоложениеВКоманднойПанели = ПоложениеКнопкиВКоманднойПанели.ВДополнительномПодменю;
	Элементы.ОбластьЧтенияОтключена.ПоложениеВКоманднойПанели = ПоложениеКнопкиВКоманднойПанели.ВДополнительномПодменю;
	Элементы.ИзменитьШрифтОтображения.ПоложениеВКоманднойПанели = ПоложениеКнопкиВКоманднойПанели.ВДополнительномПодменю;
	
	Элементы.ОбластьЧтения.Видимость = ЭтоМобильноеУстройствоСБольшимЭкраном;
	Элементы.СтраницыПредпросмотр.Видимость = ЭтоМобильноеУстройствоСБольшимЭкраном;
	
	Если ЭтоМобильноеУстройствоСБольшимЭкраном Тогда
		Элементы.ДеревоПисем.ИспользованиеТекущейСтроки = ИспользованиеТекущейСтрокиТаблицы.ОтображениеВыделения;
	Иначе 
		Элементы.ДеревоПисем.ИспользованиеТекущейСтроки = ИспользованиеТекущейСтрокиТаблицы.Выбор;
	КонецЕсли;
	
	Элементы.ДеревоПисем.Шапка = Ложь;
	Элементы.МК_ГруппаКолонок.Видимость = Истина;
	
	Элементы.ДеревоПисемКому.Видимость = Ложь;
	Элементы.ДеревоПисемВид.Видимость = Ложь;

	Элементы.Переместить(Элементы.ДеревоПисемВажностьНомерКартинки, Элементы.МК_ПерваяКолонка);
	Элементы.Переместить(Элементы.ДеревоПисемЕстьВложения, Элементы.МК_ПерваяКолонка);
	
	Элементы.Переместить(Элементы.ДеревоПисемГруппаАдресаты, Элементы.МК_ВтораяКолонка);
	Элементы.Переместить(Элементы.ДеревоПисемТема, Элементы.МК_ВтораяКолонка);
	
	Элементы.Переместить(Элементы.ДеревоПисемПолучено, Элементы.МК_ТретьяКолонка);
	
	Элементы.ДеревоПисемПолучено.Ширина = 5;
	Элементы.ДеревоПисемПолучено.Шрифт = ШрифтыСтиля.МелкийШрифтТекста;
	Элементы.ДеревоПисемПолучено.Формат = НСтр("ru='ДФ=''dd.MM.yy'''");
	
	Элементы.ДеревоПисемВажностьНомерКартинки.Ширина = 1;
	Элементы.ДеревоПисемВажностьНомерКартинки.Высота = 1;
	Элементы.ДеревоПисемВажностьНомерКартинки.РастягиватьПоВертикали = Ложь;
	Элементы.ДеревоПисемВажностьНомерКартинки.РастягиватьПоГоризонтали = Ложь;
	
	Элементы.ДеревоПисемЕстьВложения.Ширина = 1;
	Элементы.ДеревоПисемЕстьВложения.Высота = 1;
	Элементы.ДеревоПисемЕстьВложения.РастягиватьПоВертикали = Ложь;
	Элементы.ДеревоПисемЕстьВложения.РастягиватьПоГоризонтали = Ложь;

	Элементы.МК_ГруппаВложения.Видимость = Истина;
	Элементы.Переместить(Элементы.Вложения, Элементы.МК_ГруппаВложения);
...
&НаСервере
Процедура МК_ПоказатьСкрытьВложенияНаСервере()
	...
	Элементы.МК_ВложенияНадпись.Заголовок = 
		СтрШаблон(
			НСтр("ru = 'Вложения (%1)'"),
			Формат(КоличествоВложений, "ЧН=0; ЧГ="));

	МК.ОформитьКнопкуСворачиваемойГруппы(Элементы.МК_ПоказатьСкрытьВложения, Элементы.Вложения.Видимость);
