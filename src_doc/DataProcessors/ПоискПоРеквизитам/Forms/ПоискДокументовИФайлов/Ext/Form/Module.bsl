...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Иначе
		...
		ВидПоиска = Перечисления.ВидыПоиска.ПоискДокументыПредприятия;
	...
	Делопроизводство.СписокДокументовУсловноеОформлениеПомеченныхНаУдаление(РезультатДокументыПредприятия);
	...
	РезультатДокументыПредприятия.Параметры.УстановитьЗначениеПараметра("ТекущаяДата", НачалоДня(ТекущаяДатаСеанса()));
	...
	Если НастройкиФормы = Неопределено Или НастройкиФормы.Получить("ПоказыватьУдаленные") = Неопределено Тогда
		Элементы.ДокументыПредприятияСписокПоказыватьУдаленные.Пометка = ПоказыватьУдаленные;
	КонецЕсли;
	
	Элементы.ДокументыПредприятияСписокЕстьКатегорииДанных.Видимость = ПолучитьФункциональнуюОпцию("ИспользоватьКатегорииДанных");
...
Процедура ПриЗагрузкеДанныхИзНастроекНаСервере(Настройки)
	...
	Элементы.ДокументыПредприятияСписок.Видимость = Ложь;	
...
&НаКлиенте
Процедура ДокументыПредприятияСписокВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	
	Если ТипЗнч(ВыбраннаяСтрока) = Тип("СтрокаГруппировкиДинамическогоСписка") Тогда
		Возврат;
	КонецЕсли;	
	
	ТекущиеДанные = Элементы.ДокументыПредприятияСписок.ТекущиеДанные;
	...
	Если Поле = Элементы.ДокументыПредприятияСписокФайлы Тогда
		ПараметрыОткрытия = Новый Структура("ОткрытьЗакладкуФайлы", Истина);
		ДелопроизводствоКлиент.ОткрытьДокументПредприятия(ТекущиеДанные.Ссылка__, ПараметрыОткрытия);
		Возврат;
	КонецЕсли;
	
	Если Поле = Элементы.ДокументыПредприятияСписокПодписанЭП Тогда
		ПараметрыОткрытия = Новый Структура("ОткрытьЗакладкуЭП", Истина);
		ДелопроизводствоКлиент.ОткрытьДокументПредприятия(ТекущиеДанные.Ссылка__, ПараметрыОткрытия);
		Возврат;
	КонецЕсли;
	
	Если Поле = Элементы.ДокументыПредприятияСписокЗадачи Тогда
	...
	Если Поле = Элементы.ДокументыПредприятияСписокЕстьКатегорииДанных Тогда
		ПараметрыОткрытия = Новый Структура("ОткрытьЗакладкуКатегории", Истина);
		ДелопроизводствоКлиент.ОткрытьДокументПредприятия(ТекущиеДанные.Ссылка__, ПараметрыОткрытия);
		Возврат;
	КонецЕсли;
	
	Если Поле = Элементы.ДокументыПредприятияСписокСостояниеКонтроля Тогда
		КонтрольКлиент.ОбработкаКомандыКонтроль(ТекущиеДанные.Ссылка__, ЭтаФорма);
		Возврат;
	КонецЕсли;
	
	ДелопроизводствоКлиент.ОткрытьДокументПредприятия(ТекущиеДанные.Ссылка__);
	
КонецПроцедуры

&НаКлиенте
Процедура ДокументыПредприятияСписокПередНачаломИзменения(Элемент, Отказ)
	...
	ОткрытьФорму("Справочник.ДокументыПредприятия.ФормаОбъекта", ПараметрыОткрытия, Элементы.ДокументыПредприятияСписок);
...
Процедура ОбщийПоискРеквизитыВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	Если Поле = Элементы.ОбщийПоискРеквизитыПредставлениеРеквизита
		...
		Или Поле = Элементы.ПоискДокументыПредприятияРеквизитыПорядковыйНомер
		Или Поле = Элементы.ПоискДокументыПредприятияРеквизитыПредставлениеРеквизита
...
&НаКлиенте
Процедура ИмпортФайловВыполнитьДокументыПредприятия(Команда)
	...
	#Если Не ВебКлиент Тогда
		...
		Если ДиалогОткрытияФайла.Выбрать() Тогда
			...
			ОткрытьФорму("Справочник.ДокументыПредприятия.Форма.ФормаИмпортаФайлов", ПараметрыИмпорта, ЭтаФорма);
		КонецЕсли;
	#Иначе	
...
Процедура ВыбратьКатегории(Команда)
	...
	РаботаСКатегориямиДанныхКлиент.ОткрытьФормуПодбораКатегорийДляСпискаОбъектов(
		ТекущийЭлемент, Истина, ОписаниеОповещения, ЭтотОбъект);
...
Процедура ДобавитьВКомплект(Команда)
	...
	Если Элементы.РезультатыПоиска.ТекущаяСтраница = Элементы.РезультатДокументыПредприятия Тогда
		ТекущийСписок = Элементы.ДокументыПредприятияСписок;
...
&НаСервере
Процедура СброситьРезультатыПоиска(ПравоНаВнутренние = Ложь, ПравоНаФайлы = Ложь)
	
	ВестиУчетСканКопийОригиналовДокументов = 
		ПолучитьФункциональнуюОпцию("ВестиУчетСканКопийОригиналовДокументов");
		
	ПравоНаВнутренние = ПравоДоступа("Чтение", Метаданные.Справочники.ДокументыПредприятия);
	ПравоНаФайлы = ПравоДоступа("Чтение", Метаданные.Справочники.Файлы);
	
	Элементы.РезультатДокументыПредприятия.Видимость = ПравоНаВнутренние;	
	Элементы.РезультатФайлы.Видимость = ПравоНаФайлы;
	
	Если ПравоНаВнутренние Тогда
		РезультатДокументыПредприятия.Параметры.УстановитьЗначениеПараметра(
			"ВестиУчетСканКопийОригиналовДокументов", 
			ВестиУчетСканКопийОригиналовДокументов);
		РезультатДокументыПредприятия.Отбор.Элементы.Очистить();
		РезультатДокументыПредприятия.Параметры.УстановитьЗначениеПараметра("Параметр", 2);
	КонецЕсли;
	...
	Если ПравоНаФайлы Тогда
		...
		РезультатФайлы.Параметры.УстановитьЗначениеПараметра("ПользовательИЕгоСотрудники", 
			...
			Для Каждого ДопРеквизит Из ЭтаФорма[ЭлементФормы.Имя] Цикл
				...
				ИначеЕсли ЗначениеЗаполнено(ДопРеквизит.ЗначениеРеквизита) Тогда
					...
					ИначеЕсли ДопРеквизит.ТипЗначенияРеквизита = "Булево" Тогда
						...
						НовыйЭлементОтбора.ПравоеЗначение = ДопРеквизит.ЗначениеРеквизита;
					Иначе
						Если Строка(ПолеОтбора) = "ВидИТематика" Тогда
						
							ДобавитьВидИТематикуВОтбор(ЭлементыОтбора, ДопРеквизит.ЗначениеРеквизита);
...
&НаСервере
Процедура ДобавитьВидИТематикуВОтбор(ЭлементыОтбора, ВидыИТематики)
	
	ВидыДокументов = Новый СписокЗначений();
	ТематикиДокументов = Новый СписокЗначений();
	
	Для Каждого Элемент Из ВидыИТематики Цикл
		
		Значение = Элемент.Значение;
		Если ТипЗнч(Значение) = Тип("СправочникСсылка.ВидыДокументов") Тогда
		
			ВидыДокументов.Добавить(Значение);
			
		Иначе 
			
			ТематикиДокументов.Добавить(Значение);
	...
	Если ЗначениеЗаполнено(ВидыДокументов) Тогда
		...
		НовыйЭлементОтбора = ЭлементыОтбора.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
		НовыйЭлементОтбора.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("ВидДокумента");
		НовыйЭлементОтбора.Использование = Истина;
		НовыйЭлементОтбора.ВидСравнения = ВидСравненияКомпоновкиДанных.ВСписке;
		НовыйЭлементОтбора.ПравоеЗначение = ВидыДокументов;
		
	КонецЕсли;
	
	Если ЗначениеЗаполнено(ТематикиДокументов) Тогда
	
		НовыйЭлементОтбора = ЭлементыОтбора.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
		НовыйЭлементОтбора.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("Тематика");
		НовыйЭлементОтбора.Использование = Истина;
		НовыйЭлементОтбора.ВидСравнения = ВидСравненияКомпоновкиДанных.ВСписке;
		НовыйЭлементОтбора.ПравоеЗначение = ТематикиДокументов;	
...
Процедура ВыполнитьПоискСервер()
	...
	ПротоколированиеРаботыСотрудников.ЗаписатьПоискПоРеквизитам(Длительность, ОписаниеСобытия, ДополнительныеСведения);
...
Функция СформироватьТекстОписанияПоисковогоЗапроса(ЭлементыОтбораОдногоУровня, ЛогическаяСвязкаУсловий = "и", ПараметрыОтбора = Неопределено)
	...
	Для Каждого УсловиеОтбора Из ЭлементыОтбораОдногоУровня Цикл
		Если ТипЗнч(УсловиеОтбора) = Тип("ЭлементОтбораКомпоновкиДанных")
			...
			ЛевоеЗначение = СтрЗаменить(ЛевоеЗначение, "_0", ")");
			ЛевоеЗначение = СтрЗаменить(ЛевоеЗначение, "_", " ");    
			ТипПериодаДиапазонаТекущий = ЛевоеЗначение;
			
			Если Не РаботаСПериодомИлиДиапазоном Или
				(РаботаСПериодомИлиДиапазоном = Истина И (ТипПериодаДиапазонаТекущий <> ТипПериодаДиапазона)) Тогда
			...
			ТипПериодаДиапазона = ТипПериодаДиапазонаТекущий;
...
Функция ВыполнитьЗапрос(КомпоновщикНастроек, ТипПоиска)
	...
	ПравоНаФайлы = Ложь;
	СброситьРезультатыПоиска(ПравоНаВнутренние, ПравоНаФайлы);
	
	Если ТипПоиска = "ОбщийПоиск" Тогда
		Элементы.ДокументыПредприятияСписок.Видимость = ПравоНаВнутренние;
		Если ПравоНаВнутренние Тогда
			СкопироватьОтбор(КомпоновщикНастроек.Настройки.Отбор.Элементы, РезультатДокументыПредприятия.Отбор.Элементы);
			СкопироватьПараметры(КомпоновщикНастроек.Настройки.ПараметрыДанных.Элементы, РезультатДокументыПредприятия.Параметры.Элементы);
			РезультатДокументыПредприятия.Параметры.УстановитьЗначениеПараметра("Параметр", 1);
		КонецЕсли;
		
	ИначеЕсли ТипПоиска = "ПоискДокументыПредприятия" Тогда
		СкопироватьОтбор(КомпоновщикНастроек.Настройки.Отбор.Элементы, РезультатДокументыПредприятия.Отбор.Элементы);
		СкопироватьПараметры(КомпоновщикНастроек.Настройки.ПараметрыДанных.Элементы, РезультатДокументыПредприятия.Параметры.Элементы);
		РезультатДокументыПредприятия.Параметры.УстановитьЗначениеПараметра("Параметр", 1);
		Элементы.ДокументыПредприятияСписок.Видимость = ПравоНаВнутренние;
	...
	ИначеЕсли ТипПоиска = "РасширенныйПоиск" Тогда
		Если ТипОбъекта = Перечисления.ТипыОбъектов.ДокументыПредприятия Тогда
			СкопироватьОтбор(КомпоновщикНастроекРасширенногоПоиска.Настройки.Отбор.Элементы, РезультатДокументыПредприятия.Отбор.Элементы);			
			СкопироватьПараметры(КомпоновщикНастроекРасширенногоПоиска.Настройки.ПараметрыДанных.Элементы, РезультатДокументыПредприятия.Параметры.Элементы);
			РезультатДокументыПредприятия.Параметры.УстановитьЗначениеПараметра("Параметр", 1);
			Элементы.ДокументыПредприятияСписок.Видимость = ПравоНаВнутренние;
	...
	Если ТипПоиска = "ОбщийПоиск"
		...
		Если ТаблицаРезультата.Колонки.Найти("КолвоВнутренние__") <> Неопределено Тогда
			КолвоДокументыПредприятия = ТаблицаРезультата.ВыгрузитьКолонку("КолвоВнутренние__")[0];
		КонецЕсли;
	...
	Если НЕ ЗначениеЗаполнено(КолвоДокументыПредприятия) Тогда
		КолвоДокументыПредприятия = 0;
	КонецЕсли;
	...
	СуммаВсего = КолвоДокументыПредприятия + КолвоФайлы;
	
	Если СуммаВсего = 0 Тогда
		ВызватьИсключение ("НичегоНеНайдено");
	КонецЕсли;
	
	Элементы.РезультатДокументыПредприятия.Заголовок = НСтр("ru = 'Документы'");
	Элементы.РезультатФайлы.Заголовок = НСтр("ru = 'Файлы'");
	
	Если КолвоДокументыПредприятия > 0 Тогда
		Элементы.РезультатДокументыПредприятия.Заголовок = 
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Документы (%1)'"),
				Строка(КолвоДокументыПредприятия));
	КонецЕсли;
	...
	Элементы.РезультатДокументыПредприятия.Видимость = Ложь;
	Элементы.РезультатФайлы.Видимость = Ложь;
	
	Если ВидПоиска = Перечисления.ВидыПоиска.ПоискДокументыПредприятия
		ИЛИ ВидПоиска = Перечисления.ВидыПоиска.РасширенныйПоиск
		И ТипОбъекта = Перечисления.ТипыОбъектов.ДокументыПредприятия Тогда
		Элементы.РезультатДокументыПредприятия.Видимость = ПравоНаВнутренние;
...
Процедура ОбновитьСоставРеквизитовДляПоиска()
	...
	Для Каждого ДоступноеПолеОтбора Из КомпоновщикНастроекПоиска.Настройки.ПараметрыДанных.ДоступныеПараметры.Элементы Цикл
		...
		Параметр = КомпоновщикНастроекПоиска.Настройки.ПараметрыДанных.Элементы.Найти(Строка(ДоступноеПолеОтбора.Параметр));
		
		СтруктураДанных = Новый Структура;
		СтруктураДанных.Вставить("Наименование", "Параметр" + ДоступноеПолеОтбора.Параметр);
	
		Представление = ДоступноеПолеОтбора.Заголовок;
		СтруктураДанных.Вставить("Представление", Представление);
			
		МассивТипов = Новый Массив;
		МассивТипов.Добавить(ТипЗнч(Параметр.Значение));
	...
	ИспользоватьШифрование = ПолучитьФункциональнуюОпцию("ИспользоватьШифрование");
	...
	Для Каждого ДоступноеПолеОтбора Из КомпоновщикНастроекПоиска.Настройки.ДоступныеПоляОтбора.Элементы Цикл
		...
		Если Не ИспользоватьШифрование И Найти(Строка(ДоступноеПолеОтбора.Поле), "Зашифрован") > 0 Тогда
			Продолжить;
		КонецЕсли;
	...
	Если Найти(ИмяЗначенияПеречисления, "Документ") > 0 ИЛИ Найти(ИмяЗначенияПеречисления, "Файл") > 0 Тогда
		...
		Если ИмяЗначенияПеречисления = "ПоискДокументыПредприятия" Тогда
			ПолучитьДополнительныеСвойстваВидаДокумента(Объект, "ВидыДокументов", ОбъединенныйНаборСвойств);
		КонецЕсли;
	КонецЕсли;
	...
	Для Каждого ДополнительныйРеквизит Из ОбъединенныйНаборСвойств Цикл
		...
		Если ДополнительныйРеквизит.Значение.ТипЗначения.СодержитТип(
				Тип("СправочникСсылка.ВидыДокументов"))
			Или ДополнительныйРеквизит.Значение.ТипЗначения.СодержитТип(
				Тип("СправочникСсылка.ТематикиДокументов")) Тогда
			
			Продолжить;
			
		КонецЕсли;
			
		Если ДополнительныйРеквизит.Значение.ТипЗначения.СодержитТип(Тип("Строка"))
			И ДополнительныйРеквизит.Значение.Представление = "Вид документа и тематика" 
			И НЕ РаботаСТематикамиДокументов.ТематикиИспользуются() Тогда
		
			ДополнительныйРеквизит.Значение.Представление = "Вид документа";
		...
		Если НоваяСтрока.ИмяРеквизита = "ВидИТематика"Тогда
...
Процедура ПоказатьФормуВводаЗначенияРеквизита(Элемент, СтандартнаяОбработка)
	...
	Если Элемент.Родитель.ТекущиеДанные.ТипЗначенияРеквизита = "Массив" Тогда
		...
		ПараметрыФормы = Новый Структура();
		ФормаВводаЗначенияРеквизита = "Обработка.ПоискПоРеквизитам.Форма.ВводЗначенияРеквизита";
		
		Если Элемент.Родитель.ТекущиеДанные.ИмяРеквизита = "ВидИТематика" Тогда
			
			ФормаВводаЗначенияРеквизита = "Обработка.ПоискПоРеквизитам.Форма.ВводВидаИТематики";
...
Функция ПолучитьИмяВременногоСохраненногоПоиска(Настройки, ВидПоиска)
	...
	Если Настройки.Получить("ВидПоиска") = Перечисления.ВидыПоиска.ПоискДокументыПредприятия Тогда
...
Процедура УстановитьДоступностьКоманд(РедактируетТекущийПользователь, Редактирует, ПодписанЭП, Зашифрован)
	...
	Элементы.ФайлыСписокОсвободить.Доступность = ЗначениеЗаполнено(Редактирует);
	Элементы.ФайлыСписокКонтекстноеМеню.ПодчиненныеЭлементы.ФайлыСписокКонтекстноеМенюОсвободить.Доступность = ЗначениеЗаполнено(Редактирует);
	
	Элементы.ФайлыСписокЗанять.Доступность = Не ЗначениеЗаполнено(Редактирует) И НЕ ПодписанЭП;
	Элементы.ФайлыСписокКонтекстноеМеню.ПодчиненныеЭлементы.ФайлыСписокКонтекстноеМенюЗанять.Доступность = Не ЗначениеЗаполнено(Редактирует) И НЕ ПодписанЭП;
	...
	Элементы.ФайлыСписокПодписать.Доступность = Не ЗначениеЗаполнено(Редактирует);
	Элементы.ФайлыСписокКонтекстноеМенюПодписать.Доступность = Не ЗначениеЗаполнено(Редактирует);
	
	Элементы.ФайлыСписокДобавитьЭПИзФайла.Доступность = Не ЗначениеЗаполнено(Редактирует);
	Элементы.ФайлыСписокКонтекстноеМенюДобавитьЭПИзФайла.Доступность = Не ЗначениеЗаполнено(Редактирует);
	...
	Элементы.ФайлыСписокЗашифровать.Доступность = Не ЗначениеЗаполнено(Редактирует) И НЕ Зашифрован;
	Элементы.ФайлыСписокКонтекстноеМенюЗашифровать.Доступность = Не ЗначениеЗаполнено(Редактирует) И НЕ Зашифрован;
	...
	Элементы.ФайлыСписокОбновитьИзФайлаНаДиске.Доступность = Не ЗначениеЗаполнено(Редактирует) И НЕ Зашифрован И НЕ ПодписанЭП;
	Элементы.ФайлыСписокКонтекстноеМенюОбновитьИзФайлаНаДиске.Доступность = Не ЗначениеЗаполнено(Редактирует) И НЕ Зашифрован И НЕ ПодписанЭП;
	...
	Элементы.ФайлыСписокРаспознать.Доступность = 
		ДоступноРаспознаваниеПоЗапросу И РасширениеПоддерживается И Не ЗначениеЗаполнено(Редактирует);
