...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.ТипЗначения.Типы().Количество() > 1 Тогда
		...
		Для Каждого Тип Из Параметры.ТипЗначения.Типы() Цикл
			...
			Если Метаданные.Справочники.Найти(ИмяТипа) <> Неопределено Тогда	
				...
				Если ОбъектТипа.Метаданные().ДлинаНаименования <> 0 Тогда
					Запрос.Текст = Запрос.Текст + 
						" УПОРЯДОЧИТЬ ПО СправочникВыбора.Наименование";
				ИначеЕсли ИмяТипа = "Сотрудники" Тогда
					Запрос.Текст = Запрос.Текст + 
						" УПОРЯДОЧИТЬ ПО СправочникВыбора.Владелец.Наименование";
				КонецЕсли;	
	...
	ИначеЕсли Параметры.ТипЗначения.Типы().Количество() = 1 Тогда 
		...
		Если Метаданные.Справочники.Найти(ИмяТипа) <> Неопределено Тогда
			...
			ИначеЕсли ИмяТипа = "ТематикиДокументов" Тогда
				...
				ДинамическийСписокЭлементовДляВыбора.ТекстЗапроса = ТекстЗапроса;
				Элементы.ДинамическийСписокЭлементовДляВыбора.Отображение = ОтображениеТаблицы.ИерархическийСписок;
				ДинамическийСписокЭлементовДляВыбора.Группировка.Элементы.Очистить();
				ГруппировкаСписка = ДинамическийСписокЭлементовДляВыбора.Группировка.Элементы;
				ЭлементГруппировки = ГруппировкаСписка.Добавить(Тип("ПолеГруппировкиКомпоновкиДанных"));
				ЭлементГруппировки.Поле = Новый ПолеКомпоновкиДанных("ВидДокумента");
				
				ЭлементыУсловногоОформления = ДинамическийСписокЭлементовДляВыбора.УсловноеОформление.Элементы;
...
Процедура СписокЭлементовДляВыбораВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	Для Каждого ВыделеннаяСтрока Из Элемент.ВыделенныеСтроки Цикл
		Если ТипЗнч(ВыделеннаяСтрока) = Тип("СтрокаГруппировкиДинамическогоСписка") Тогда
			...
			Если ТипЗнч(ВыделеннаяСтрока.Ключ) = Тип("СправочникСсылка.ВидыДокументов") Тогда
			
				СтруктураВыбраннойСсылки = Новый Структура;
				СтруктураВыбраннойСсылки.Вставить("Ссылка", ВыделеннаяСтрока.Ключ);
				СтруктураВыбраннойСсылки.Вставить("ПредставлениеЭлемента", Строка(ВыделеннаяСтрока.Ключ));
