...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	НастройкаВыходаИзПрограммы = ХранилищеОбщихНастроек.Загрузить("ОбщиеНастройкиПользователя",
	...
	Элементы.ВопросСоздаватьПисьмо.Видимость =
		ВстроеннаяПочтаСервер.ВстроеннаяПочтаИспользуется()
			Или ВстроеннаяПочтаСервер.ЛегкаяПочтаИспользуется();
...
&НаКлиенте
Асинх Процедура ОчиститьИсториюВыбора(Команда)
	
	ТекстВопроса = НСтр("ru = 'Для очистки истории выбора программа будет перезагружена. Продолжить?'");
	...
	Если ОчиститьИсторию = КодВозвратаДиалога.Да Тогда
		...
		ЗавершитьРаботуСистемы(Ложь, Истина,"/C ClearChoiceHistory");
