...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Элементы.НастройкаРаспознавания.Видимость = 
		РаботаСФайламиВызовСервера.ПолучитьИспользоватьРаспознаваниеТекста()
	...
	ИспользоватьПредпросмотрФайловЧтение =
		ХранилищеОбщихНастроек.Загрузить(
			"НастройкиРаботыСДокументами",
			"ИспользоватьПредпросмотрФайлов");
	Если ИспользоватьПредпросмотрФайловЧтение = Неопределено Тогда
		ИспользоватьПредпросмотрФайловЧтение = Истина;
	КонецЕсли;			
	ИспользоватьПредпросмотрФайлов = ИспользоватьПредпросмотрФайловЧтение;
	...
	Если Не ЗначениеЗаполнено(ПредпросмотрОфисныхФайлов) Тогда
		...
		Если ЗначениеЗаполнено(СпособСогласованияВРежимеЗамечаний) Тогда
			
			Если СпособСогласованияВРежимеЗамечаний = Перечисления.СпособыСогласованияВРежимеЗамечаний.MicrosoftOfficeWord Тогда
				ПредпросмотрОфисныхФайлов = Перечисления.ПредпросмотрОфисныхФайлов.MSOffice;
			ИначеЕсли СпособСогласованияВРежимеЗамечаний = Перечисления.СпособыСогласованияВРежимеЗамечаний.OpenOfficeOrgWriter
				Или СпособСогласованияВРежимеЗамечаний = Перечисления.СпособыСогласованияВРежимеЗамечаний.LibreOffice Тогда
				ПредпросмотрОфисныхФайлов = Перечисления.ПредпросмотрОфисныхФайлов.OpenOffice;
			ИначеЕсли СпособСогласованияВРежимеЗамечаний = Перечисления.СпособыСогласованияВРежимеЗамечаний.Р7Builder Тогда
				ПредпросмотрОфисныхФайлов = Перечисления.ПредпросмотрОфисныхФайлов.Р7Builder;
			КонецЕсли;	
	...
	Если ПредпросмотрОфисныхФайлов = Перечисления.ПредпросмотрОфисныхФайлов.OpenOfficeMSOffice Тогда
		
		Элементы.ПредпросмотрОфисныхФайлов.СписокВыбора.Добавить(Перечисления.ПредпросмотрОфисныхФайлов.OpenOfficeMSOffice);
		
	КонецЕсли;	        
	
	Если ОбщегоНазначенияКлиентСервер.ЭтоLinuxКлиент() Тогда
		
		Элементы.СпособСравненияВерсийФайлов.СписокВыбора.Добавить(Перечисления.СпособыСравненияВерсийФайлов.Р7Builder);
	...
	КонецЕсли;			
	МасштабироватьPDFИКартинки = МасштабироватьPDFИКартинкиЧтение;
	
	Элементы.ПредпросмотрОфисныхФайлов.Видимость = ИспользоватьПредпросмотрФайлов;
	Элементы.МасштабироватьPDFИКартинки.Видимость = ИспользоватьПредпросмотрФайлов;
	
	Элементы.ГруппаПутьКР7Builder.Видимость 
		= (СпособСравненияВерсийФайлов = Перечисления.СпособыСравненияВерсийФайлов.Р7Builder);	
	...
	Иначе
		...
		Если Не ЗначениеЗаполнено(ПутьКР7Builder) Тогда
			...
			Если ОбщегоНазначенияКлиент.ЭтоLinuxКлиент() Тогда
				ПутьКР7Builder = "/opt/r7-office/documentconstructor/docconstructor";
			КонецЕсли;
...
Процедура СпособСравненияВерсийФайловПриИзменении(Элемент)
	...
	Элементы.ГруппаПутьКР7Builder.Видимость 
		= (СпособСравненияВерсийФайлов = ПредопределенноеЗначение("Перечисление.СпособыСравненияВерсийФайлов.Р7Builder"));	
...
Процедура СохранитьНастройки()
	...
	ДобавитьСтруктуруНастройки(МассивСтруктур, "НастройкиРаботыСДокументами/СпособПроверкиОрфографии", 
		ИдентификаторКлиента, СпособПроверкиОрфографии);
	...
	ДобавитьСтруктуруНастройки(МассивСтруктур, "НастройкиСравненияФайлов", "ПутьКР7Builder", ПутьКР7Builder);
	...
	ДобавитьСтруктуруНастройки(МассивСтруктур, "НастройкиРаботыСДокументами", 
		"ИспользоватьПредпросмотрФайлов", ИспользоватьПредпросмотрФайлов);
	ДобавитьСтруктуруНастройки(МассивСтруктур, 
		"НастройкиРаботыСДокументами/ПредпросмотрОфисныхФайлов", 
		ИдентификаторКлиента, ПредпросмотрОфисныхФайлов);
	ДобавитьСтруктуруНастройки(МассивСтруктур, "НастройкиРаботыСДокументами", 
		"МасштабироватьPDFИКартинки", МасштабироватьPDFИКартинки);
...
&НаКлиенте
Процедура ПредпросмотрОфисныхФайловОчистка(Элемент, СтандартнаяОбработка)
	СтандартнаяОбработка = Ложь;
КонецПроцедуры
