...
Процедура ВыполнитьОчистку(Команда)
	...
	Если Не ЗначениеЗаполнено(ПутьСохраненияУдаляемыхВерсий) Тогда
		ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru = 'Не указан путь сохранения очищаемых версий'"), , "ПутьСохраненияУдаляемыхВерсий");
		Возврат;
	КонецЕсли;	
	
	Если Не ЗначениеЗаполнено(СрокХраненияНеактивныхВерсий) Тогда
		ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru = 'Не указан срок хранения неактивных версий'"), , "СрокХраненияНеактивныхВерсий");
		Возврат;
	КонецЕсли;	
