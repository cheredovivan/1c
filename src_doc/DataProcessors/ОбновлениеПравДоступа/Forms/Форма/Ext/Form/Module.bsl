...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СтрОтбора.Представление = РедакцииКонфигурацииКлиентСервер.Организации();
...
Функция КоличествоДескрипторовКОбновлению()
	...
	Запрос = Новый Запрос(
		...
		|		СОЕДИНЕНИЕ &ТаблицаДескрипторов КАК Таблица ПО &Условие // Заменяемая строка.
		|	ГДЕ
...
Процедура ДополнитьЗапросВСоответствииСОтборами(Запрос)
	...
	Для Каждого СтрокаОтбора Из ВыбранныеСтрокиОтбора Цикл
		...
		ИначеЕсли СтрокаОтбора.Имя = "Папки" Тогда
			...
			ТекстЗапросаДляОтбораПоПапкам = 
				...
				|ПОМЕСТИТЬ ВТДескрипторыОтобранныеПоПапкам
				|ИЗ
			...
			ТекстСоединений = ТекстСоединений +
				"	ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТДескрипторыОтобранныеПоПапкам КАК ДескрипторыОтобранныеПоПапкам
				|	ПО ДескрипторыОтобранныеПоПапкам.Дескриптор = ДескрипторыДоступаОбъектов.Ссылка
				|";
	...
	ЗаменяемоеСоединение = "СОЕДИНЕНИЕ &ТаблицаДескрипторов КАК Таблица ПО &Условие"; // так в исходном запросе.
	Запрос.Текст = СтрЗаменить(Запрос.Текст, ЗаменяемоеСоединение, ТекстСоединений);
