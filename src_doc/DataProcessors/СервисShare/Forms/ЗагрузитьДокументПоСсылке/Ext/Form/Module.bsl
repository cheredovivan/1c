...
Процедура ПредставлениеДокументаВБазеНажатие(Элемент)
	...
	Если ОбщегоНазначенияСлужебныйКлиент.ЭтоНавигационнаяСсылка(СсылкаНаОбъектУчета) Тогда
		ПерейтиПоНавигационнойСсылке(СсылкаНаОбъектУчета);
	Иначе	
...
Процедура ПоискДокументаПоПубличнойСсылке()
	...
	Отказ       = Ложь;
	ТекстОшибки = "";
	РезультаПоиска = ИнтеграцияShare.РезультатПоискаДокументаПоПубличнойСсылке(СсылкаДляСкачиванияДокумента, Отказ, ТекстОшибки);
	...
	Если Отказ Тогда
		...
		Возврат;
	ИначеЕсли РезультаПоиска = Неопределено Тогда
...
Процедура ОткрытьНавигационнуюСсылку(НавигационнаяСсылка)
	...
	СсылкаДляБраузера = ИнтеграцияShareДокументооборотКлиентСервер.СсылкаДляБраузера(НавигационнаяСсылка);
	ФайловаяСистемаКлиент.ОткрытьНавигационнуюСсылку(СсылкаДляБраузера);
