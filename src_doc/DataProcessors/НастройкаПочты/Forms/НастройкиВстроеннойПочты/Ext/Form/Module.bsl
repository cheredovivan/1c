...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.Свойство("ПоказатьУправлениеФО") Тогда
		ИспользоватьВстроеннуюПочту = РегистрыСведений.ИспользованиеПочты.ВстроеннаяПочтаИспользуется();
	...
	УчетныеЗаписи.Параметры.УстановитьЗначениеПараметра("СотрудникиПользователя", СотрудникиПользователя);
...
Процедура ЗагрузитьНастройки()
	...
	ВидСпискаПриОткрытии = ВидСписка;
...
Процедура СохранитьНастройки()
	...
	ВстроеннаяПочтаСервер.УстановитьПерсональнуюНастройку(
		"ВидСписка",
		ВидСписка);                  
	Если ВидСпискаПриОткрытии <> ВидСписка И НРег(ВидСписка) = НРег("Компактный") Тогда
		
		ТаблПапки = РегистрыСведений.ПапкиПисемЧастоИспользуемые.ПолучитьПапкиДляТекущегоПользователя();
		Если ТаблПапки.Количество() = 0 Тогда
			ЗаполнитьПапкиПисемЧастоИспользуемые();
		КонецЕсли;
	...
	ВстроеннаяПочтаСервер.УстановитьПерсональнуюНастройку("ОтправлятьPushУведомленияОНовыхПисьмах",
		ОтправлятьPush);
		
	Для Каждого СтрокаНастройкиОповещений Из НастройкаОповещений Цикл
			
		ПараметрыОповещения = Новый Структура;
		ПараметрыОповещения.Вставить("УчетнаяЗапись", СтрокаНастройкиОповещений.УчетнаяЗапись);
		...
		Иначе 	
			ПараметрыОповещения.Вставить("ПоказыватьВсплывающиеОповещения", 
				СтрокаНастройкиОповещений.ПоказыватьВсплывающиеОповещения);
			ПараметрыОповещения.Вставить("ПоказыватьСписокОповещений", 
				СтрокаНастройкиОповещений.ПоказыватьСписокОповещений);
		КонецЕсли;
			
		ДопОписаниеНастройки = Строка(СтрокаНастройкиОповещений.УчетнаяЗапись);
		
		ВстроеннаяПочтаСервер.УстановитьПерсональнуюНастройку("ОтправлятьPushУведомленияОНовыхПисьмах",
			СтрокаНастройкиОповещений.ОтправлятьPush, ДопОписаниеНастройки);
...
Процедура ОбновитьИнтерфейсКлиент()
	...
	ИмяПараметраИспользованиеВстроеннойПочты = "Почта.ВстроеннаяПочтаИспользуется";
	ПараметрыПриложения[ИмяПараметраИспользованиеВстроеннойПочты] = Неопределено;

	ИмяПараметраИспользованиеВстроеннойИлиЛегкойПочты = "Почта.ИспользованиеВстроеннойИЛегкойПочтыНастройка";
	ПараметрыПриложения[ИмяПараметраИспользованиеВстроеннойИлиЛегкойПочты] = Неопределено;

	ИмяПараметраИспользованиеЛегкойПочты = "Почта.ИспользованиеЛегкойПочты";
	ПараметрыПриложения[ИмяПараметраИспользованиеЛегкойПочты] = Неопределено;
...
Процедура УстановитьФункциональныеОпцииИспользованияПочты()
	...
	ИспользоватьЛегкуюПочту = РегистрыСведений.ИспользованиеПочты.ЛегкаяПочтаИспользуется();
...
&НаКлиенте
Процедура ДобавитьВСписокАвтоподбораСотрудника(Команда)
	
	АдресатыАвтоподбора = АдресатыАвтоподбора();
	
	ПараметрыФормы = Новый Структура;
	ПараметрыФормы.Вставить(
		"ЗаголовокФормы", НСтр("ru = 'Подбор сотрудников в личный список автоподбора'"));
...
Функция АдресатыАвтоподбора()
	...
	Запрос.Текст = 
		...
		|	И ОбъектыДляАвтоподбораАдресатов.ОбъектДанных ССЫЛКА Справочник.Сотрудники";
...
&НаКлиенте
Процедура НастройкаОповещенийОтправлятьPushПриИзменении(Элемент)
	...
	Если ТекущиеДанные.ОтправлятьPush И Не ОтправлятьPush Тогда
		ОтправлятьPush = Истина;
	КонецЕсли;
	
	Если Не ТекущиеДанные.ОтправлятьPush Тогда
		СтруктураОтбора = Новый Структура("ОтправлятьPush", Истина);
		СтрокиСОтправлятьPush = НастройкаОповещений.НайтиСтроки(СтруктураОтбора);
		Если СтрокиСОтправлятьPush.Количество() = 0 Тогда
			ОтправлятьPush = Ложь;
		КонецЕсли;
	КонецЕсли;
...
&НаКлиенте
Процедура ПоказыватьУведомленияНаМобильныхУстройствахПриИзменении(Элемент)
	
	Если ОтправлятьPush И НастройкаОповещений.Количество() = 1 Тогда
		НастройкаОповещений[0].ОтправлятьPush = Истина;
	КонецЕсли; 

	Если Не ОтправлятьPush Тогда
		Для Каждого СтрокаНастройкиОповещения Из НастройкаОповещений Цикл
			СтрокаНастройкиОповещения.ОтправлятьPush = Ложь;
		КонецЦикла;
	КонецЕсли;
...
Процедура ЗаполнитьНастройкиОповещений()
	...
	Запрос.Текст = 
		...
		|ПОМЕСТИТЬ УчетныеЗаписиИОтветственные
		|ИЗ
		...
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ РАЗЛИЧНЫЕ
		|	УчетныеЗаписиИОтветственные.УчетнаяЗапись КАК УчетнаяЗапись,
		|	НастройкиОповещенийОПисьмах.ПоказыватьВсплывающиеОповещения КАК ПоказыватьВсплывающиеОповещения,
		|	НастройкиОповещенийОПисьмах.ПоказыватьСписокОповещений КАК ПоказыватьСписокОповещений,
		|	Истина как ОтправлятьPush
		|ИЗ
		|	УчетныеЗаписиИОтветственные КАК УчетныеЗаписиИОтветственные
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.НастройкиОповещенийОПисьмах КАК НастройкиОповещенийОПисьмах
		|		ПО УчетныеЗаписиИОтветственные.УчетнаяЗапись = НастройкиОповещенийОПисьмах.УчетнаяЗапись
		|			И УчетныеЗаписиИОтветственные.Ответственный = НастройкиОповещенийОПисьмах.Пользователь";
	...
	ИспользуютсяPushУведомления = ПолучитьФункциональнуюОпцию("ИспользоватьPushУведомления");
	
	ОтправлятьPushПерсональнаяНастройка =
		ВстроеннаяПочтаСервер.ПолучитьПерсональнуюНастройку(
			"ОтправлятьPushУведомленияОНовыхПисьмах");
	
	ОтправлятьPush = ?(ТипЗнч(ОтправлятьPushПерсональнаяНастройка) <> Тип("Булево"),
	...
	Для Каждого Строка Из ТаблицаУчетныеЗаписи Цикл
		...
		Если ИспользуютсяPushУведомления Тогда
			
			УчетнаяЗаписьСтрокой = Строка(НоваяСтрока.УчетнаяЗапись);
			
			ОтправлятьPushПерсональнаяНастройкаПоУчетнойЗаписи =
				ВстроеннаяПочтаСервер.ПолучитьПерсональнуюНастройку(
					"ОтправлятьPushУведомленияОНовыхПисьмах",,
					УчетнаяЗаписьСтрокой); 
			
			НоваяСтрока.ОтправлятьPush =
				?(ТипЗнч(ОтправлятьPushПерсональнаяНастройкаПоУчетнойЗаписи) <> Тип("Булево"),
					Истина,
					ОтправлятьPushПерсональнаяНастройкаПоУчетнойЗаписи);
		КонецЕсли;
	...
	Если НастройкаОповещений.Количество() = 0 Тогда 
		...
		Элементы.ОтправлятьУведомленияНаМобильныеУстройства.Видимость = Ложь;
	...
	ИначеЕсли НастройкаОповещений.Количество() = 1 Тогда 
		...
		Элементы.ОтправлятьУведомленияНаМобильныеУстройства.Видимость =Истина;
		Элементы.НастройкаОповещений.Видимость = Ложь;
...
&НаСервере
Процедура ЗаполнитьПапкиПисемЧастоИспользуемые()
