...
&НаСервере
Процедура МК_НастроитьЭлементы()
	...
	Если ПараметрыСеанса.ЭтоМобильныйКлиент Тогда
		...
		МК_КлиентСервер.АдаптироватьHtmlПодЭкранМобильногоПриНеобходимости(ТекстПредупреждения);
		Элементы.ПодтверждениеБулево.ПоложениеЗаголовка = ПоложениеЗаголовкаЭлементаФормы.Лево;
		Элементы.РазрешитьОтправкуСтатистики.ПоложениеЗаголовка = ПоложениеЗаголовкаЭлементаФормы.Лево;
	КонецЕсли;
