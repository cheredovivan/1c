...
Процедура ЗагрузитьПроектСерверПоДвоичнымДаннымФайла(ДвоичныеДанныеПроекта)
	...
	Попытка
		...
		Проект.Руководитель = Сотрудники.ОсновнойСотрудник();
		...
		Пока УзелЗадача <> Неопределено Цикл
			Если НЕ ЗначениеРеквизита(УзелЗадача, "IsNull", Тип("Булево")) Тогда
				...
				Иначе
					...
					ЗадачаОбъект.Важность = Перечисления.ВариантыВажностиОбъектов.Обычная;
							...
							Если УзелРесурс <> Неопределено Тогда
								...
								Если ТипРесурса = "1" Тогда // Сотрудник
									...
									Если ЗначениеЗаполнено(Пользователь) Тогда
										НоваяСтрока.Исполнитель = Сотрудники.ОсновнойСотрудникПользователя(Пользователь); 
									...
									Для Каждого Участник Из Проект.ПроектнаяКоманда Цикл
										Если Участник.Исполнитель = Сотрудники.ОсновнойСотрудникПользователя(Пользователь) 
...
Процедура Загрузить(Команда)
	...
	ФайловыеФункцииСлужебныйКлиент.Расширение1СПредприятияПодключено();
