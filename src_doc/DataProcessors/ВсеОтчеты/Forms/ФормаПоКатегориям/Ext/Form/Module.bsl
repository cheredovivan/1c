...
&НаКлиенте
Процедура ДеревоОтчетовИВариантовОбработкаЗапросаОбновления()
	Элементы.ДеревоОтчетовИВариантов.Обновить();
КонецПроцедуры
...
Процедура ОбработкаОжиданияДеревоОтчетовИВариантовПриАктивизацииСтроки()
	...
	Если НеОтображатьИерархию Тогда
		...
		Элементы.ДеревоОтчетовИВариантовКонтекстноеМенюОткрытьОтчет.Доступность = Истина;
	...
	ИначеЕсли ТекущиеДанные = Неопределено Или ТекущиеДанные.ПолучитьРодителя() = Неопределено Тогда
		...
		Элементы.ДеревоОтчетовИВариантовКонтекстноеМенюОткрытьОтчет.Доступность = Ложь;
...
Процедура ПоказатьСнимокОтчета(ИмяОтчета, КлючВарианта, ОтображатьСнимок)
	...
	Если (Не ОтображатьСнимок) Или (ВариантОтчета = Неопределено)  Тогда
		...
		Возврат;
	КонецЕсли;
	СуффиксЯзыкаТекПользователя = МультиязычностьСервер.СуффиксТекущегоЯзыка();
	ИмяРеквизитаСКартинкой = "ХранилищеСнимокОтчета" + СуффиксЯзыкаТекПользователя;
	ХранилищеСнимок = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ВариантОтчета, ИмяРеквизитаСКартинкой);
	...
	Если Не ЕстьКартинка Тогда
		...
		ДвоичныеДанные = ХранилищеСнимок.Получить();
		ИмяРеквизитаСКартинкой = "ХранилищеСнимокОтчета";
		ЕстьКартинка = ЗначениеЗаполнено(ДвоичныеДанные);
	...
	Если НЕ ВариантОтчета.Пустая() И ЕстьКартинка Тогда
		...
		КартинкаОтчета = ПолучитьНавигационнуюСсылку(ВариантОтчета.Ссылка, ИмяРеквизитаСКартинкой);
...
Процедура ОткрытьОтчетВыполнить()
	...
	ТекстВариантОтчетаНеВыбран = НСтр("ru = 'Выберите какой-либо вариант отчета!'");
	
	Если ТекущиеДанные = Неопределено Тогда
		ПоказатьПредупреждение(, ТекстВариантОтчетаНеВыбран); 
		Возврат;
	КонецЕсли;
...
Процедура НастроитьФормуДляМобильногоПриНеобходимости()
	...
	Элементы.ФормаДополнительныеОтчеты.ПоложениеВКоманднойПанели = ПоложениеКнопкиВКоманднойПанели.ВДополнительномПодменю;
	Элементы.ДеревоОтчетовИВариантовОткрытьОтчет.Видимость = Ложь;
	Элементы.ДеревоОтчетовИВариантовКонтекстноеМенюОткрытьОтчет.Видимость = Ложь;
	Элементы.ДеревоОтчетовИВариантовОбновить.Видимость = Ложь;
	Элементы.ДеревоОтчетовИВариантовКонтекстноеМенюОбновить.Видимость = Ложь;
