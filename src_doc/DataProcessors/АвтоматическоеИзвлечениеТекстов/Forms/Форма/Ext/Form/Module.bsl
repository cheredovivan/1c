...
Процедура ИзвлечениеТекстовКлиент(РазмерПорции = Неопределено)
	...
	Попытка
		...
		Для Индекс = 0 По МассивФайлов.Количество() - 1 Цикл
			...
			ФайлУдален = МассивФайлов[Индекс].ФайлУдален;
			...
			Попытка 
				...
				Если ФайлУдален Тогда
					...
					ЗаписьОшибкиИзвлечения(ФайлИлиВерсияФайла, РезультатИзвлечения, "", Расширение);
			...
			Исключение
				...
				РезультатИзвлечения = "ИзвлечьНеУдалось";
				ЗаписьОшибкиИзвлечения(ФайлИлиВерсияФайла, РезультатИзвлечения, ТекстСообщения, Расширение);
...
Процедура ЗаписьОшибкиИзвлечения(ФайлИлиВерсияФайла, РезультатИзвлечения, ТекстСообщения, Расширение)
	...
	ФайловыеФункции.ЗаписатьРезультатИзвлеченияТекста(ФайлИлиВерсияФайла, РезультатИзвлечения, "", Расширение);
...
Функция ПолучитьФайлыДляИзвлеченияТекста(КоличествоФайловВПорции)
	...
	МассивВерсии = РезультатЗапроса.Выгрузить().ВыгрузитьКолонку("Версия");
	
	ТаблицаВыгрузки = ОбщегоНазначения.ЗначенияРеквизитовОбъектов(МассивВерсии, 
		"Ссылка, Наименование, Расширение, ФайлУдален");
	...
	Для Каждого СтрокаТаблица Из ТаблицаВыгрузки Цикл  
        ...
        Результат.Добавить(Новый Структура("Ссылка, Расширение, Наименование, Кодировка, ФайлУдален", 
            Строка.Ссылка, Строка.Расширение, Строка.Наименование, Кодировка, Строка.ФайлУдален));
