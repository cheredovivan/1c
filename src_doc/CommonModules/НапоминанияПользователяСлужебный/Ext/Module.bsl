...
Процедура ПроверитьИзмененияДатВПредметеПриЗаписи(Источник, Отказ) Экспорт
	...
	// Документобоорот
	Если Источник.ДополнительныеСвойства.Свойство("ПропуститьПроверкуИзмененияДат")
		И Источник.ДополнительныеСвойства.ПропуститьПроверкуИзмененияДат Тогда
		Возврат;
	КонецЕсли;
	// Документобоорот
...
Функция СоздатьНапоминание(ПараметрыНапоминания, ВремяСобытия = Неопределено) Экспорт
	...
	Если Напоминание.СпособУстановкиВремениНапоминания = Перечисления.СпособыУстановкиВремениНапоминания.ОтносительноВремениПредмета Тогда
		...
		// Документооборот
		ИначеЕсли ПараметрыНапоминания.Свойство("ДатаНачалаЗаписиКалендаря") Тогда
			Напоминание.ВремяСобытия = ПараметрыНапоминания.ДатаНачалаЗаписиКалендаря;
...
Функция СписокТекущихНапоминанийПользователя() Экспорт
	...
	Запрос.УстановитьПараметр("ГраницаСрокаНапоминания",
		ТекущаяДатаМестная + ЗапасВремениНапоминанийДляКэша());
	// Документобоорот
	...
	КоличествоЭлементов = Напоминания.Количество();
	Для Индекс = 1 По КоличествоЭлементов Цикл
		Строка = Напоминания[КоличествоЭлементов - Индекс];
		Строка.НавигационнаяСсылка = НавигационнаяСсылкаНапоминания(Строка);
		...
		Если ПраваПоОбъекту.Чтение Тогда
			Продолжить;
		КонецЕсли;
		ОтключитьНапоминание(Строка);
		Напоминания.Удалить(КоличествоЭлементов - Индекс);
	КонецЦикла;
...
Функция ПолучитьИнтервалПроверкиНапоминаний(Пользователь = Неопределено) Экспорт
	...
	Интервал = ОбщегоНазначения.ХранилищеОбщихНастроекЗагрузить(
		"НастройкиНапоминаний", "ИнтервалПроверкиНапоминаний", 1,, ПолучитьИмяПользователяИБ(Пользователь));
