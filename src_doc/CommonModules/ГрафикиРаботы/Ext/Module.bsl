...
// Возвращает график работы, указанный в настройках программы.
//
// Параметры:
//  Нет.
// 
// Возвращаемое значение:
//  СправочникСсылка.ГрафикиРаботы - График работы.
//
...
// Возвращает график работы сотрудника.
// Если у сотрудника не задан персональный график, то возвращаем график подразделения.
// Если у подразделения не задан график, то возвращаем основной график работы.
// 
// Параметры:
//  Сотрудник - СправочникСсылка.Сотрудники, СправочникСсылка.Пользователи - Сотрудник
// 
// Возвращаемое значение:
//  ГрафикРаботы - СправочникСсылка.ГрафикиРаботы - График работы сотрудника
//
Функция ГрафикРаботыСотрудника(Сотрудник) Экспорт
	...
	Запрос = Новый Запрос(
		...
		|ГДЕ
		|	Сотрудники.Ссылка = &Сотрудник
		|	И НЕ ЕСТЬNULL(ПодчиненностьПодразделений.Вышестоящее.ГрафикРаботы,
		|		ЗНАЧЕНИЕ(Справочник.ГрафикиРаботы.ПустаяСсылка)) = ЗНАЧЕНИЕ(Справочник.ГрафикиРаботы.ПустаяСсылка)
		|
		|УПОРЯДОЧИТЬ ПО
		|	Приоритет");
	Запрос.УстановитьПараметр("Сотрудник", Сотрудники.ОсновнойСотрудникПользователя(Сотрудник));

	Выборка = Запрос.Выполнить().Выбрать();
	Если Выборка.Следующий() Тогда
		ГрафикРаботы = Выборка.ГрафикРаботы;
	Иначе
		ГрафикРаботы = ПолучитьОсновнойГрафикРаботы();
	КонецЕсли;
...
Функция ЭтоРабочийДень(ПроверяемаяДата, ГрафикРаботы) Экспорт 
	...
	РабочиеДни = ГрафикиРаботыПовтИсп.ПрочитатьДанныеГрафикаИзРегистра(ГрафикРаботы.Календарь, Год(ПроверяемаяДата));
...
Функция СформироватьТаблицуРабочегоВремени(ГрафикРаботы, ДатаНачала, ДатаОкончания) Экспорт 
	...
	РабочиеДниСоответствие = Новый Соответствие;
	
	РабочиеДни = ГрафикиРаботыПовтИсп.ПрочитатьДанныеГрафикаИзРегистра(ГрафикРаботы.Календарь, Год(ДатаНачала));
	...
	Для Каждого Элемент Из РабочиеДни Цикл
		РабочиеДниСоответствие[Элемент] = Истина;
	КонецЦикла;
	
	Если Год(ДатаОкончания) > Год(ДатаНачала) Тогда
		ВрРабочиеДни = ГрафикиРаботыПовтИсп.ПрочитатьДанныеГрафикаИзРегистра(ГрафикРаботы.Календарь, Год(ДатаОкончания));
	...
	Пока ТекущаяДата < ДатаОкончания Цикл
		...
		ЭтоРабочийДень = РабочиеДниСоответствие[НачалоДня] = Истина;
		...
		Если ОсобоеРабочееВремя.Найти(НачалоДня, "РабочийДень") <> Неопределено Тогда // особый рабочий день
			ВрРабочееВремя = ОсобоеРабочееВремя.Скопировать();
			ВрРабочееВремя.Очистить();
			
			Для Каждого Строка Из ОсобоеРабочееВремя Цикл
				Если Строка.РабочийДень = НачалоДня Тогда 
...
Функция ЭтоРабочаяДатаВремя(ГрафикРаботы, ПроверяемаяДата) Экспорт
	...
	ТаблицаРабочегоВремени = 
		СформироватьТаблицуРабочегоВремени(ГрафикРаботы, НачалоДня(ПроверяемаяДата), КонецДня(ПроверяемаяДата)); 
