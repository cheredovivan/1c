...
// Определяет замещающих и помощников, у которых есть доступ к проектам.
// 
// Параметры:
...
// 
Функция ЗамещающиеИПомощникиПоПроектам(МассивСотрудников) Экспорт
	
	ОбластиЗамещения = ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(
		Справочники.ОбластиЗамещения.Проекты);
	
	ЗамещающиеИПомощникиПоПроектам =
		Справочники.ЗамещающиеИПомощники.АктуальныеЗамещенияСотрудников(
			МассивСотрудников,
			ОбластиЗамещения);
	
	Возврат ЗамещающиеИПомощникиПоПроектам;
	
КонецФункции

// Определяет активные проекты руководителей.
//
// Параметры:
//  Руководители - Массив из СправочникСсылка.Пользователи, СправочникСсылка.Сотрудники.
// 
// Возвращаемое значение:
//  Массив из СправочникСсылка.Проекты - Активные проекты руководителей.
//
Функция АктивныеПроектыРуководителей(Руководители) Экспорт
	
	АктуальныеЗамещения =
		Справочники.ЗамещающиеИПомощники.АктуальныеЗамещения(
			Руководители,
			Справочники.ОбластиЗамещения.Проекты);
	РуководителиСПомощинкамиИЗаместителями =
		АктуальныеЗамещения.ВыгрузитьКолонку("Сотрудник");
	РуководителиСПомощинкамиИЗаместителями =
	...
	Запрос = Новый Запрос(
		...
		|ГДЕ
		|	Проекты.Руководитель В(&Руководители)
	...
	Запрос.УстановитьПараметр("Руководители", РуководителиСПомощинкамиИЗаместителями);
	
	АктивныеПроектыРуководителя = Запрос.Выполнить().Выгрузить().ВыгрузитьКолонку("Проект");
	
	Возврат АктивныеПроектыРуководителя;
	
КонецФункции

// Заполняет коды СДР всех задач проекта 
Процедура ЗаполнитьКодыСДРПроектныхЗадач(Проект, ИдентификаторФормы = Неопределено) Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		Если (Выборка.НомерЗадачиВУровне <> НомерЗадачиВУровне) Или (Выборка.КодСДР <> КодСДР) Тогда 
			ЗаблокироватьДанныеДляРедактирования(Выборка.Ссылка,,ИдентификаторФормы);
			...
			РазблокироватьДанныеДляРедактирования(Выборка.Ссылка, ИдентификаторФормы);
...
//  ИзЕдиницы - ПеречислениеСсылка.ЕдиницыТрудозатрат - Из единицы
//  ВЕдиницу - ПеречислениеСсылка.ЕдиницыТрудозатрат - В единицу
//  ГрафикРаботы - СправочникСсылка.ГрафикиРаботы - График работы
// 
// Возвращаемое значение:
//  Число - Пересчитать трудозатраты
//
Функция ПересчитатьТрудозатраты(Знач Трудозатраты, ИзЕдиницы, ВЕдиницу, ГрафикРаботы) Экспорт
		
	Если ПолучитьФункциональнуюОпцию("ИспользоватьГрафикиРаботы") И ЗначениеЗаполнено(ГрафикРаботы) Тогда
		ИменаРеквизитов = "КоличествоРабочихЧасовВДне, КоличествоРабочихЧасовВНеделе, КоличествоРабочихДнейВМесяце";
		РеквизитыГрафика = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ГрафикРаботы, ИменаРеквизитов);
		КоличествоРабочихЧасовВДне = РеквизитыГрафика.КоличествоРабочихЧасовВДне;
		КоличествоРабочихЧасовВНеделе = РеквизитыГрафика.КоличествоРабочихЧасовВНеделе;
		КоличествоРабочихДнейВМесяце = РеквизитыГрафика.КоличествоРабочихДнейВМесяце;
	Иначе
...
Функция РассчитатьОкончаниеПериода(Знач ПроектнаяЗадача, ДатаНачала, Длительность, ЕдиницаДлительности) Экспорт 
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьГрафикиРаботы") Тогда 
		ДатаОкончания = ГрафикиРаботыПовтИсп.ПолучитьДатуОкончанияСек(ГрафикРаботы, ДатаНачала, ДлительностьЧас * 3600); 
	Иначе
...
Функция РассчитатьДлительностьПериода(Знач ПроектнаяЗадача, ДатаНачала, ДатаОкончания, ЕдиницаДлительности) Экспорт 
	...
	ГрафикРаботы = ПолучитьГрафикРаботыПроектнойЗадачи(ПроектнаяЗадача);
	Если ПолучитьФункциональнуюОпцию("ИспользоватьГрафикиРаботы") Тогда 
		ДлительностьСек = ГрафикиРаботыПовтИсп.ПолучитьДлительностьПериодаСек(ГрафикРаботы, ДатаНачала, ДатаОкончания); 
	Иначе
...
Функция ПолучитьФактическиеТрудозатратыИсполнителейПроектнойЗадачи(
	...
	Пока ВыборкаЗаписей.Следующий() Цикл
		...
		Для Каждого Участник Из ТаблицаИсполнителей Цикл
			Если ТипЗнч(Участник.Исполнитель) = Тип("СправочникСсылка.Сотрудники")
				И Участник.Исполнитель = ВыборкаЗаписей.Сотрудник Тогда 
		...
		Для Каждого Участник Из ТаблицаИсполнителей Цикл
			...
			Если ТипЗнч(ВыборкаЗаписей.Источник) = Тип("ЗадачаСсылка.ЗадачаИсполнителя") Тогда
				...
				Иначе
					ИсполнителиРоли = РегистрыСведений.ИсполнителиРолей.ИсполнителиРоли(Участник.Исполнитель);
					
					СотрудникИсполнителя = 
						Сотрудники.ОсновнойСотрудникПользователя(
						РеквизитыЗадачи.Исполнитель);
					
					ДобавитьТрудозатраты = ИсполнителиРоли.Найти(СотрудникИсполнителя) <> Неопределено;
				КонецЕсли;
			...
			Иначе
				ИсполнителиРоли = РегистрыСведений.ИсполнителиРолей.ИсполнителиРоли(Участник.Исполнитель);
				Если ИсполнителиРоли.Найти(ВыборкаЗаписей.Сотрудник) = Неопределено Тогда
					ИсполнителиРоли = 
...
Функция НайтиИсполнителяРольВКомандеПроекта(
	...
	ПроектнаяКоманда = РеквизитыПроекта.ПроектнаяКоманда.Выгрузить().ВыгрузитьКолонку("Исполнитель");
	Если ПроектнаяКоманда.Количество() = 0 Тогда 
		Возврат Истина;
	КонецЕсли;
	
	ПроектнаяКоманда.Добавить(РеквизитыПроекта.Руководитель);
	
	ОбщегоНазначенияКлиентСервер.ДополнитьМассив(
		ПроектнаяКоманда,
		Сотрудники.ЛюбыеПользователиСотрудников(ПроектнаяКоманда),
		Истина);
	
	ОбщегоНазначенияКлиентСервер.ДополнитьМассив(
		ПроектнаяКоманда,
		Сотрудники.ЛюбыеСотрудникиПользователей(ПроектнаяКоманда),
		Истина);
	...
	СотрудникИсполнителя = 
		Сотрудники.ОсновнойСотрудникПользователя(
		Исполнитель);
	Для Каждого РольИсполнителя Из РегистрыСведений.ИсполнителиРолей.РолиИсполнителя(СотрудникИсполнителя) Цикл
		ПроверяемыеИсполнители[РольИсполнителя] = Истина;
	КонецЦикла;
	
	Для Каждого УчастникКоманды Из ПроектнаяКоманда Цикл
		Если ПроверяемыеИсполнители[УчастникКоманды] = Истина Тогда
			Возврат Истина;
		КонецЕсли;
	КонецЦикла;
...
Процедура ПроверитьВозможностьСтартаБизнесПроцесса(Знач Объект, ВозвращаемыйМассивОшибок, ЕстьПравоЗапуска) Экспорт
	...
	Если ЗначениеЗаполнено(Объект.ПроектнаяЗадача) Тогда
		...
		Если Не ЗначениеЗаполнено(Объект.ГлавнаяЗадача)
			И Не ДокументооборотПраваДоступа.ПраваПользователяПоОбъекту(Объект.Проект).Изменение Тогда
...
Функция ИзменитьПапкуПроектов(МассивПроектов, НоваяПапка) Экспорт
	...
	Для Каждого ПроектСсылка Из МассивПроектов Цикл
		Если Не ДокументооборотПраваДоступа.ПраваПользователяПоОбъекту(ПроектСсылка).Изменение Тогда 
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
...
Функция ПолучитьПроектныеЗадачиПользователя(ДатаНачала, ДатаОкончания, Пользователь) Экспорт
	...
	|		ПО (ПроектныеЗадачиИсполнители.Исполнитель = СотрудникиПользователей.Сотрудник)
	|			И (СотрудникиПользователей.Пользователь В (&Исполнители))
	|ГДЕ
	|	СрокиПроектныхЗадач.ТекущийПланНачало < &ДатаОкончания
	|	И СрокиПроектныхЗадач.ТекущийПланОкончание > &ДатаНачала";
	
	Запрос.УстановитьПараметр("ДатаНачала", ДатаНачала);
	Запрос.УстановитьПараметр("ДатаОкончания", ДатаОкончания);
	
	Исполнители = РегистрыСведений.ИсполнителиРолей.РолиИсполнителей(
...
//
// Параметры:
//  СотрудникиПользователя - Массив Из СправочникСсылка.Сотрудники - см. Сотрудники.СотрудникиПользователя
//
// Возвращаемое значение:
//  Булево - Истина - если у пользователя есть свои проекты.
//
Функция НетСвоихПроектов(СотрудникиПользователя = Неопределено) Экспорт
	...
	|ГДЕ
	|	ПроектыПроектнаяКоманда.Исполнитель В (&СотрудникиПользователя)
	...
	|		И СотрудникиВКонтейнерах.Контейнер ССЫЛКА Справочник.ПолныеРоли
	|		И ПроектыПроектнаяКоманда.Исполнитель ССЫЛКА Справочник.ПолныеРоли
	|ГДЕ
	|	НЕ ПроектыПроектнаяКоманда.Ссылка.ПометкаУдаления
	...
	|ГДЕ
	|	СправочникПроекты.Руководитель В (&СотрудникиПользователя)
	|	И НЕ СправочникПроекты.ПометкаУдаления";
	
	Запрос.Параметры.Вставить("СотрудникиПользователя", СотрудникиПользователя);
...
//
// Параметры:
//   ДокументОбъект - СправочникОбъект.ДокументыПредприятия - проверяемый документ.
//
// Возвращаемое значение:
//   Булево
//
Функция ДокументУникален(ДокументОбъект) Экспорт
	...
	Запрос.Текст =
		...
		|	И ДокументыПредприятия.Проект = &Проект
...
// Заполняет данные ключевых операций по проектам.
// 
// Параметры:
...
Процедура ЗаполнитьДанныеКлючевыхОпераций(ДанныеКлючевыхОпераций) Экспорт
	...
	ДанныеКлючевойОперации.Имя = РаботаСПроектамиКлиентСервер.КлючеваяОперацияОткрытиеСпискаПроектов();
	ДанныеКлючевойОперации.Наименование = НСтр("ru = 'Проекты: Открытие списка проектов'", КодОсновногоЯзыка);
	ДанныеКлючевойОперации.ЦелевоеВремя = 2;
	
	ДанныеКлючевойОперации = ДанныеКлючевыхОпераций.Добавить();
	ДанныеКлючевойОперации.Имя = РаботаСПроектамиКлиентСервер.КлючеваяОперацияОткрытиеПроекта();
	ДанныеКлючевойОперации.Наименование = НСтр("ru = 'Проекты: Открытие проекта'", КодОсновногоЯзыка);
	ДанныеКлючевойОперации.ЦелевоеВремя = 1;
	
	ДанныеКлючевойОперации = ДанныеКлючевыхОпераций.Добавить();
	ДанныеКлючевойОперации.Имя = РаботаСПроектамиКлиентСервер.КлючеваяОперацияЗаписьПроекта();
	ДанныеКлючевойОперации.Наименование = НСтр("ru = 'Проекты: Запись проекта'", КодОсновногоЯзыка);
	ДанныеКлючевойОперации.ЦелевоеВремя = 1;
	
	ДанныеКлючевойОперации = ДанныеКлючевыхОпераций.Добавить();
	ДанныеКлючевойОперации.Имя = РаботаСПроектамиКлиентСервер.КлючеваяОперацияОткрытиеПланаПроекта();
	ДанныеКлючевойОперации.Наименование = НСтр("ru = 'Проекты: Открытие плана проекта'", КодОсновногоЯзыка);
	ДанныеКлючевойОперации.ЦелевоеВремя = 3;
	
	ДанныеКлючевойОперации = ДанныеКлючевыхОпераций.Добавить();
	ДанныеКлючевойОперации.Имя = РаботаСПроектамиКлиентСервер.КлючеваяОперацияОткрытиеПроектнойЗадачи();
	ДанныеКлючевойОперации.Наименование = НСтр("ru = 'Проекты: Открытие проектной задачи'", КодОсновногоЯзыка);
	ДанныеКлючевойОперации.ЦелевоеВремя = 1;
	
	ДанныеКлючевойОперации = ДанныеКлючевыхОпераций.Добавить();
	ДанныеКлючевойОперации.Имя = РаботаСПроектамиКлиентСервер.КлючеваяОперацияЗаписьПроектнойЗадачи();
	ДанныеКлючевойОперации.Наименование = НСтр("ru = 'Проекты: Запись проектной задачи'", КодОсновногоЯзыка);
	ДанныеКлючевойОперации.ЦелевоеВремя = 1;
	
	ДанныеКлючевойОперации = ДанныеКлючевыхОпераций.Добавить();
	ДанныеКлючевойОперации.Имя = РаботаСПроектамиКлиентСервер.КлючеваяОперацияОткрытиеСпискаКТ();
	ДанныеКлючевойОперации.Наименование = НСтр("ru = 'Проекты: Открытие списка контрольных точек'", КодОсновногоЯзыка);
	ДанныеКлючевойОперации.ЦелевоеВремя = 1;
	
	ДанныеКлючевойОперации = ДанныеКлючевыхОпераций.Добавить();
	ДанныеКлючевойОперации.Имя = РаботаСПроектамиКлиентСервер.КлючеваяОперацияОткрытиеКТ();
	ДанныеКлючевойОперации.Наименование = НСтр("ru = 'Проекты: Открытие контрольной точки'", КодОсновногоЯзыка);
	ДанныеКлючевойОперации.ЦелевоеВремя = 1;
	
	ДанныеКлючевойОперации = ДанныеКлючевыхОпераций.Добавить();
	ДанныеКлючевойОперации.Имя = РаботаСПроектамиКлиентСервер.КлючеваяОперацияЗаписьКТ();
	ДанныеКлючевойОперации.Наименование = НСтр("ru = 'Проекты: Запись контрольной точки'", КодОсновногоЯзыка);
	ДанныеКлючевойОперации.ЦелевоеВремя = 1;
...
Процедура РассчитатьПланПроектаПоЗадаче(ПроектнаяЗадача, Знач ПересчитатьПланПоЗадаче = Ложь, МассивИзмененныхЗадач = Неопределено, ДанныеПроекта = Неопределено)
	...
	Если ДанныеПроектнойЗадачи = Неопределено Тогда
		Возврат;
	КонецЕсли;	
    ...
    // установка даты начала
  	Если ДанныеПроектнойЗадачи.СуммарнаяЗадача Тогда 
		...
		Если ПересчитатьПланПоЗадаче Тогда 
			Для Каждого ПодчиненнаяЗадача Из ПодчиненныеЗадачи Цикл
				РассчитатьПланПроектаПоЗадаче(ПодчиненнаяЗадача, Истина, МассивИзмененныхЗадач, ДанныеПроекта);
			КонецЦикла;	
		КонецЕсли;	
	...
	Для Каждого Строка Из ВсеПредшественники Цикл
		...
		Если Строка.ТипЗависимости = Перечисления.ТипыЗависимостейПроектныхЗадач.ОкончаниеНачало Тогда 
			...
			Если ПолучитьФункциональнуюОпцию("ИспользоватьГрафикиРаботы") И ДанныеПроектнойЗадачи.ТекущийПланДлительность <> 0 Тогда 
				...
				врНовыйПланНачало = ГрафикиРаботыПовтИсп.ПолучитьДатуОкончанияСек(ГрафикРаботы, НовыйПланНачало, 1); 
...
Функция НайтиИсполнителяВПроектнойЗадачеСУчетомВхожденияВРоль(
	...
	ИсполнителиЗадачи = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ПроектнаяЗадача, "Исполнители");
	ИсполнителиЗадачи = ИсполнителиЗадачи.Выгрузить().ВыгрузитьКолонку("Исполнитель");
	
	Если ИсполнителиЗадачи.Количество() = 0 Тогда
		Возврат Истина;
	КонецЕсли;
	
	ОбщегоНазначенияКлиентСервер.ДополнитьМассив(
		ИсполнителиЗадачи,
		Сотрудники.ЛюбыеПользователиСотрудников(ИсполнителиЗадачи),
		Истина);
	
	ОбщегоНазначенияКлиентСервер.ДополнитьМассив(
		ИсполнителиЗадачи,
		Сотрудники.ЛюбыеСотрудникиПользователей(ИсполнителиЗадачи),
		Истина);
	...
	Если ЗначениеЗаполнено(ЗадачаИсполнителя) И ТипЗнч(ЗадачаИсполнителя) = Тип("ЗадачаСсылка.ЗадачаИсполнителя") Тогда
		...
		ПроверяемыеИсполнители[РеквизитыЗадачи.Исполнитель] = Истина;
		ПроверяемыеИсполнители[РеквизитыЗадачи.РольИсполнителя] = Истина;
		
		СотрудникИсполнителя = 
			Сотрудники.ОсновнойСотрудникПользователя(
			РеквизитыЗадачи.Исполнитель);
		
		ПроверяемыеИсполнители[СотрудникИсполнителя] = Истина;
	...
	Для Каждого ИсполнительЗадачи Из ИсполнителиЗадачи Цикл
		Если ПроверяемыеИсполнители[ИсполнительЗадачи] <> Неопределено Тогда
...
Функция НайтиИсполнителяВКомандеПроектаСУчетомВхожденияВРоль(
	...
	ПроектнаяКоманда = Проект.ПроектнаяКоманда.Выгрузить().ВыгрузитьКолонку("Исполнитель");
	Если ПроектнаяКоманда.Количество() = 0 Тогда 
		Возврат Истина;
	КонецЕсли;	
	
	ПроектнаяКоманда.Добавить(Проект.Руководитель);
	...
	ИсполнительНайден = Ложь;
	Для Каждого УчастникКоманды Из ПроектнаяКоманда Цикл
		Если ПроверяемыеИсполнители.Получить(УчастникКоманды) <> Неопределено Тогда
			ИсполнительНайден = Истина;
			Прервать;
		КонецЕсли;
	КонецЦикла;
	...
	Пока ВыборкаСледующаяЗадача.Следующий() Цикл
		Если ВыборкаСледующаяЗадача.Исполнитель = 0 Тогда
			...
			РегистрыСведений.ОчередьУведомлений.ДобавитьУведомлениеПоСобытию(
				Сотрудники.ЛюбойПользовательСотрудника(ДанныеПроекта.Руководитель),
		...
		Иначе
			...
			Иначе
				РегистрыСведений.НастройкиОтложенногоСтартаПроцессов.Добавить(БПОбъект.Ссылка,
