...
Функция ОбновитьТокенOAuth(refresh_token, ТокенДоступа, ДействуетДо, АдресСервераSMTP)
	...
	ИначеЕсли СтрНайти(НРег(АдресСервераSMTP), "office365.com") <> 0
		Или СтрНайти(НРег(АдресСервераSMTP), "smtp-mail.outlook.com") <> 0 Тогда
...
Функция ОтправитьПисьма(УчетнаяЗаписьИлиСоединение, Письма, ТекстИсключения = Неопределено) Экспорт
	...
	Если УчетнаяЗапись <> Неопределено Тогда
		...
		Исключение
			ИнформацияОбОшибке = ИнформацияОбОшибке();
			Если ИнформацияОбОшибке.ЯвляетсяОшибкойКатегории(КатегорияОшибки.ОшибкаКонфигурации)
