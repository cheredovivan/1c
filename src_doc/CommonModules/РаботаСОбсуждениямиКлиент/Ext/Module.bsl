...
Процедура СкопироватьТему(Тема) Экспорт
	...
	ПараметрыФормы.Вставить("Основание", Тема);
	
	СоздатьСообщение(ПараметрыФормы);
...
//  ДействиеПриОткрытии - Строка
// 
Процедура ОткрытьСообщение(СообщениеФорума, ДействиеПриОткрытии = "") Экспорт
	
	КлючеваяОперация = РаботаСОбсуждениямиКлиентСервер.КлючеваяОперацияОткрытиеСообщения();
	ОценкаПроизводительностиКлиент.ЗамерВремени(КлючеваяОперация);
	
	ПараметрыФормы = Новый Структура;
	ПараметрыФормы.Вставить("Ключ", СообщениеФорума);
	Если ЗначениеЗаполнено(ДействиеПриОткрытии) Тогда
		ПараметрыФормы.Вставить("ДействиеПриОткрытии", "ОткрытьГолосование");
	КонецЕсли;
...
// 
Процедура ОткрытьТему(ТемаФорума) Экспорт
	
	КлючеваяОперация = РаботаСОбсуждениямиКлиентСервер.КлючеваяОперацияОткрытиеТемы();
	ОценкаПроизводительностиКлиент.ЗамерВремени(КлючеваяОперация);
	
	ПараметрыФормы = Новый Структура;
	ПараметрыФормы.Вставить("Ключ", ТемаФорума);
	ОткрытьФорму("Справочник.ТемыОбсуждений.ФормаОбъекта", ПараметрыФормы);
	
КонецПроцедуры

// Создает сообщение с фиксацией ключевой операции.
// Параметры формы необходимо заполнить согласно используемому методу создания сообщения.
// 
// Параметры:
//  ПараметрыФормы - Структура
//
Процедура СоздатьСообщение(ПараметрыФормы) Экспорт
	
	КлючеваяОперация = РаботаСОбсуждениямиКлиентСервер.КлючеваяОперацияСозданиеСообщения();
	ОценкаПроизводительностиКлиент.ЗамерВремени(КлючеваяОперация);
