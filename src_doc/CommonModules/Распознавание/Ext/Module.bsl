...
// Позволяет дополнить запрос для получения данных распознавания.
// 
// Параметры:
//  Запрос - Запрос
// 
Процедура ДополнитьЗапросПолученияДанных(Запрос) Экспорт
	...
	Запрос.Текст = СтрЗаменить(Запрос.Текст, "&ДополнительныеПоляРаспознаваниеИзображений",
	...
	Запрос.Текст = СтрЗаменить(Запрос.Текст, "&ДополнительныеТаблицыРаспознаваниеИзображений",
...
Функция РаспознатьСПомощьюCuneiForm(ТекущаяВерсия, РасширениеФайлаРезультата, ОписаниеОшибки, РаспознанныйТекст, ПутьКФайлу) Экспорт
	...
	СтатусРаспознаванияТекста = РегистрыСведений.ТекстыВерсийФайлов.ПрочитатьСтатусРаспознавания(ТекущаяВерсия);
	...
	Если Не СтатусРаспознаванияКорректенДляНачалаРаспознавания(
		СтатусРаспознаванияТекста) Тогда
	...
	Попытка
		КодВозврата = ПодключитьВнешнююКомпоненту(
			"ОбщийМакет.КомпонентаЗагрузкиCuneiForm", 
			"CuneiFormLoaderAddIn", 
	...
	Если КодВозврата = Истина Тогда
		...
		РаспознанныйТекст = ОбработкаСтрокиXML.УдалитьНедопустимыеСимволыXML(РаспознанныйТекст);		
...
// Возвращает максимальный размер порции файлов для распознавания через CuneiForm.
//
// Возвращаемое значение:
//  Число
//
Функция МаксимальныйРазмерПорцииCuneiForm() Экспорт
	
	Возврат 100;
	
КонецФункции

// Возвращает максимальный размер порции файлов для автоматического распознавания через сервис распознавания.
//
// Возвращаемое значение:
//  Число
//
Функция МаксимальныйРазмерПорцииСервисРаспознавания() Экспорт
...
//  Булево
//
Функция БазаАвторизованаВСервисеРаспознавания() Экспорт
...
Процедура ВыполнитьАвторизациюЧерезИТС() Экспорт
	...
	Если Не РаспознаваниеЛокализация.ВыполненаАвторизацияЧерезИТС() Тогда
		ВызватьИсключение
	...
	Если Не ОбщегоНазначения.РазделениеВключено() Тогда
		ПараметрыАутентификацииИТС = 
			РаспознаваниеЛокализация.ДанныеАутентификацииПользователяИнтернетПоддержки();
...
// Возвращает описание лимита страниц распознавания под текущей учетной записью ИТС.
//
// Возвращаемое значение:
//  Структура:
//   * ЛимитСтраниц - Число
...
Функция ЛимитСтраницРаспознавания() Экспорт
	...
	КомандаСервису.URLЗапроса = БазовыйURL() + "/users/balance/plain_text_pages";
	...
	Иначе
		...
		ИначеЕсли РезультатКоманды.ОтветСервиса.КодСостояния = КодСостояния_200_ОК() Тогда
			Попытка
				Результат.ЛимитСтраниц = РезультатКоманды.ОтветСервиса.ДесериализованноеЗначение["balance"];
				ДатаОкончанияJSON = РезультатКоманды.ОтветСервиса.ДесериализованноеЗначение["expiration_date"];
				Если ДатаОкончанияJSON <> Неопределено Тогда
					Результат.ДатаОкончания = ПрочитатьДатуJSON(ДатаОкончанияJSON, ФорматДатыJSON.ISO);
				КонецЕсли;
			Исключение
...
Функция ПодключениеТестовогоПериодаДоступно() Экспорт
	...
	Возврат РаспознаваниеЛокализация.ПодключениеТестовогоПериодаДоступно();
...
Функция НачатьРаспознаваниеСПомощьюСервиса(ТекущаяВерсия) Экспорт
	...
	Если Не РаботаСФайламиВызовСервера.ПолучитьИспользоватьРаспознаваниеТекста()
...
Функция ВыполнитьРаспознаваниеСПомощьюСервиса(ТекущаяВерсия, ИдентификаторЗадания) Экспорт
	...
	Если СтатусРаспознаванияТекста <> 
...
// Обработчик регламентного задания КонтрольСрокаДействияТарифаСервисаРаспознавания
//
Процедура КонтрольСрокаДействияТарифаСервисаРаспознавания() Экспорт
	
	Отказ = Ложь;
	РегламентноеЗадание = Метаданные.РегламентныеЗадания.КонтрольСрокаДействияТарифаСервисаРаспознавания;
	...
	Если Не ПолучитьФункциональнуюОпцию("ИспользуетсяРаспознаваниеПриПомощиСервиса") Тогда
		Возврат;
	КонецЕсли;
	...
	КоличествоДней = РаботаСФайламиВызовСервера.ПорогСрокаДействияТарифаСервисаРаспознавания();
	ДатаОкончания = Распознавание.ЛимитСтраницРаспознавания().ДатаОкончания;
	
	РазницаВДнях = (НачалоДня(ДатаОкончания) - НачалоДня(ТекущаяДатаСеанса())) / (60 * 60 * 24);
	
	Если РазницаВДнях <> КоличествоДней Тогда
		Возврат;
	КонецЕсли;
	...
	ТекстУведомления = СтрШаблон(НСтр("ru = 'До конца действия тарифа сервиса распознавания документов осталось %1 дней.'"),
		КоличествоДней);
	...
	Исключение
		ЗаписьЖурналаРегистрации(
			НСтр("ru = 'Распознавание изображений'"),
...
Процедура ПриДобавленииОписанийСервисовСопровождения(СервисыСопровождения) Экспорт
	...
	РаспознаваниеЛокализация.ПриДобавленииОписанийСервисовСопровождения(СервисыСопровождения);
	
КонецПроцедуры

// Возвращает наименование тестового тарифа сервиса 1С:РПД
//
// Возвращаемое значение:
//  Строка
//
Функция НаименованиеТестовогоТарифаСервисаРаспознаванияПервичныхДокументов() Экспорт
	
	Возврат "1С:Распознавание первичных документов тестовый";
...
//
// Параметры:
//  РезультатРаспознавания - Соответствие
//
// Возвращаемое значение:
//  Массив
//
Функция ОшибкиРезультатаРаспознавания(РезультатРаспознавания)
	
	Ошибки = Новый Массив;
	Если ТипЗнч(РезультатРаспознавания) = Тип("Соответствие") Тогда
		Для Каждого Результат Из РезультатРаспознавания Цикл
			Если СтрНайти(Результат.Ключ, "error") Тогда
				Ошибки.Добавить(Результат.Значение);
			КонецЕсли;
		КонецЦикла;
	КонецЕсли;	
...
Функция ТекущиеПараметрыАвторизации()
	...
	ДанныеВБезопасномХранилище = ОбщегоНазначения.ПрочитатьДанныеИзБезопасногоХранилища(
		РаспознаваниеКлиентСервер.ИдентификаторСервиса(), "ИдентификаторИБ, ТокенДоступа, ЛогинПользователяИТС");
...
Процедура ЗаписатьПараметрыАвторизации(Параметры)
	...
	ИдентификаторПодсистемы = РаспознаваниеКлиентСервер.ИдентификаторСервиса();
	НачатьТранзакцию();
	Попытка
...
Функция ПроверитьГотовностьЗаданияРаспознавания(ИдентификаторЗадания, ГруппаЗапроса)
	...
	УведомитьОДостиженииПороговогоЗначенияБаланса();
...
Функция РезультатРаспознавания(ИдентификаторЗадания, ИдентификаторРезультата, ГруппаЗапроса)
	...
	КодСостояния = 0;
	ТекстИсключения = "";
	ОтветСервиса = РезультатКоманды.ОтветСервиса;
	...
	Иначе
		...
		Если КодСостояния = КодСостояния_200_ОК() Тогда
			Попытка
				РезультатРаспознавания = ОтветСервиса.ДесериализованноеЗначение["result"];
				ОшибкиРаспознавания = ОшибкиРезультатаРаспознавания(РезультатРаспознавания);
				Если ОшибкиРаспознавания.Количество() > 0 Тогда
					ТекстИсключения = СтрСоединить(ОшибкиРаспознавания, Символы.ПС);
					Результат.СтрокаВозврата = "Ошибка";
				Иначе
					Результат.РаспознанныйТекст = РезультатРаспознавания;
...
// Формирует уведомления при равенстве текущего баланса значению, заданному в настройках.
//
Процедура УведомитьОДостиженииПороговогоЗначенияБаланса()
	
	ПорогБалансаСтраниц = РаботаСФайламиВызовСервера.ПорогБалансаСтраницСервисаРаспознавания();
	ТекущийБаланс = ЛимитСтраницРаспознавания().ЛимитСтраниц;
	
	Если ТекущийБаланс <> ПорогБалансаСтраниц И ТекущийБаланс > 0 Тогда
		Возврат;
	КонецЕсли;
	...
	ТекстУведомления = ?(ТекущийБаланс <= 0,
		НСтр("ru = 'Лимит страниц в сервисе распознавания исчерпан.'"),
		СтрШаблон(НСтр("ru = 'Осталось %1 страниц в сервисе распознавания.'"), ПорогБалансаСтраниц));
