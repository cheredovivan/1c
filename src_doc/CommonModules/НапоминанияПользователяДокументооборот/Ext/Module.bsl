...
Функция ПодключитьНапоминаниеДоВремениПредмета(Текст, Интервал, Предмет, ИмяРеквизита,
	ВремяСобытия = Неопределено, Пользователь = Неопределено, ДатаНачала = Неопределено) Экспорт
	...
	Если ЗначениеЗаполнено(ДатаНачала) Тогда
		ПараметрыНапоминания.Вставить("ДатаНачалаЗаписиКалендаря", ДатаНачала);
	КонецЕсли;
...
Функция ПолучитьЗначениеДатыНачалаЗаписиКалендаря(ЗаписьРабочегоКалендаря) Экспорт
	...
	РеквизитыЗаписиКалендаря = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(
		ЗаписьРабочегоКалендаря,
		"ДатаНачала, ТипЗаписиКалендаря");
	Если РеквизитыЗаписиКалендаря.ТипЗаписиКалендаря = Перечисления.ТипЗаписиКалендаря.ПовторяющеесяСобытие Тогда
	...
	Если Не ЗначениеЗаполнено(Результат) Тогда
		Результат = РеквизитыЗаписиКалендаря.ДатаНачала;
	КонецЕсли;
...
// Определяет текущие напоминания.
// 
// Возвращаемое значение:
//  Массив из Структура - Текущие напоминания.
//
Функция ТекущиеНапоминания() Экспорт
	
	ТекущиеНапоминания = Новый Массив;
	ТаблицаОповещений = НапоминанияПользователяСлужебный.СписокТекущихНапоминанийПользователя();
	
	Для Каждого Оповещение Из ТаблицаОповещений Цикл
		
		ТекущийСрокНапоминания = РаботаСЧасовымиПоясами.ПривестиКМестномуВремени(ТекущаяДатаСеанса());
		
		// Показываем напоминания, срок которых подойдёт в течении минуты.
		Если (Оповещение.СрокНапоминания - 60) <= ТекущийСрокНапоминания Тогда
			ТекущиеНапоминания.Добавить(Оповещение);
		КонецЕсли;
		
	КонецЦикла;	
	
	Возврат ТекущиеНапоминания;
...
Процедура ВыполнитьДействиеОбщегоОбработчикаОжидания(ПараметрыВыполнения, РезультатыВыполнения) Экспорт
	...
	РезультатыВыполнения.Вставить("ПровереныНапоминания", Ложь);
	РезультатыВыполнения.Вставить("ТекущиеНапоминания", Новый Массив);
	
	Если Не ПараметрыВыполнения.ПроверитьНапоминания Тогда
		Возврат;
	КонецЕсли;
	
	РезультатыВыполнения.ПровереныНапоминания = Истина;
	РезультатыВыполнения.ТекущиеНапоминания = ТекущиеНапоминания();
