...
//Параметры:
// Список - массив ссылок на объекты. Типы объектов могут: Файл, Документ предприятия, Входящий документ, Исходящий документ
...
Процедура ОткрытьФормуПодбораКатегорийДляСпискаОбъектов(Список, ЗаписатьКатегорииПриОК, ОписаниеОповещения,
	ЭтаФорма) Экспорт
	...
	Если Список.ВыделенныеСтроки.Количество() > 0 Тогда
		...
		Для Каждого ВыделеннаяСтрока Из Список.ВыделенныеСтроки Цикл
			...
			Если ТипЗнч(ВыделеннаяСтрока) = Тип("РегистрСведенийКлючЗаписи.ДанныеДокументовПредприятия")
				Или ТипЗнч(ВыделеннаяСтрока) = Тип("РегистрСведенийКлючЗаписи.РеестрДокументовПредприятия") Тогда 
				ДанныеСтроки = Список.ДанныеСтроки(ВыделеннаяСтрока); 
				
				Если ЭтаФорма.ИмяФормы = "Обработка.ПоискПоРеквизитам.Форма.ПоискДокументовИФайлов" Тогда
					МассивОбъектов.Добавить(ДанныеСтроки.Ссылка__);
				Иначе
