...
//
// Параметры:
//   Шаблон - ОпределяемыйТип.ШаблонПроцесса - ссылка на шаблон процесса.
//   ДляСпискаШаблоновПроцессов - Булево - признак получения HTML-обзора для списка шаблонов.
//
// Возвращаемое значение:
//  Строка - HTMLТекст описания шаблона
//
...
Функция ПолучитьОбзорПроцесса(Процесс) Экспорт
	...
	Если ЗначениеЗаполнено(Процесс) Тогда
		...
		ПредметДокумент = ОбзорЗадач.ПолучитьОсновнойПредметДокумент(Предметы);
		Если ЗначениеЗаполнено(ПредметДокумент) Тогда
		
			ОбзорПредмета = ОбзорЗадач.ПолучитьОбзорПредметаЗадачи(ПредметДокумент);
...
Функция ПолучитьОсновнойАдрес(Контакт)
	...
	Если ТипЗнч(Контакт) = Тип("СправочникСсылка.Пользователи")
		Или ТипЗнч(Контакт) = Тип("СправочникСсылка.Сотрудники")
...
Функция ПредставлениеВажности(Важность)
	...
	Если Важность = Перечисления.ВариантыВажностиОбъектов.Высокая Тогда
		Возврат НСтр("ru = 'Высокая'");
	ИначеЕсли Важность = Перечисления.ВариантыВажностиОбъектов.Обычная Тогда
		Возврат НСтр("ru = 'Обычная'");
	ИначеЕсли Важность = Перечисления.ВариантыВажностиОбъектов.Низкая Тогда
