...
Процедура ПриОпределенииПараметровОтправкиЭлектронногоПисьма(ПараметрыОтправки, Знач ПараметрыДокумента) Экспорт
	...
	ПараметрыОтправки.Тема = СтрШаблон("%1 %2", ПараметрыОтправки.Тема,
		ИнтеграцияShareДокументооборотКлиентСервер.ТекстПисьмаИз1СShare());
	
	ТекстПисьма = СтрШаблон(НСТР(
		"ru = 'Здравствуйте, %1.
		|
		|Высылаю Вам электронный документ: %2
		|на сумму %3 %4
		|
		|Данный документ Вы можете просмотреть или загрузить к себе в программу 1С по ссылке:
		|
		|%5
		|
		|'"), ПараметрыДокумента.КонтрагентНаименование, ПараметрыДокумента.ПредставлениеДокумента,
			ПараметрыДокумента.СуммаДокумента, ПараметрыДокумента.ВалютаДокумента,
			ПараметрыДокумента.СсылкаДляСкачиванияДокумента);
...
Процедура ПриОткрытииФормыВыбораФайловДокументаКПубликации(ОповещениеОбработкиРезультатаВыборФайлов, Знач СсылкаНаДокумент,
	...
	ИнтеграцияShareДокументооборотКлиент.ПриОткрытииФормыВыбораФайловДокументаКПубликации(ОповещениеОбработкиРезультатаВыборФайлов,
		СсылкаНаДокумент, РезультатПроверкиПакета, СтандартнаяОбработка);
...
Процедура ПриЗагрузкеДокументаПоСсылке(ДанныеДокумента, СтандартнаяОбработка) Экспорт
	...
	ИнтеграцияShareДокументооборотКлиент.ПриЗагрузкеДокументаПоСсылке(ДанныеДокумента, СтандартнаяОбработка);
