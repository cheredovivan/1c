...
//
// Параметры:
//  Документ - СправочникСсылка.ДокументыПредприятия - Ссылка на входящий документ.
...
Функция Выгрузить(Документ) Экспорт
	...
	ЗаписьXML.ЗаписатьАтрибут("from_system", РаботаСоСтроками.УдалитьНедопустимыеСимволыXML("1С:Документооборот 8"));
		...
		ЗаписьXML.ЗаписатьАтрибут("type", "2"); // Документ предприятия
		...
		ЗаписьXML.ЗаписатьАтрибут("annotation", РаботаСоСтроками.УдалитьНедопустимыеСимволыXML(Строка(Документ.Содержание)));
			...
			ЗаписьXML.ЗаписатьНачалоЭлемента("Author");
				...
				Если ДелопроизводствоКлиентСервер.ЭтоВходящийДокумент(Документ) Тогда
					...
					Если Документ.ВидДокумента.ЯвляетсяОбращениемОтГраждан Тогда
						...
						ЗаписьXML.ЗаписатьНачалоЭлемента("PrivatePersonWithSign");
							...
							ЗаписьXML.ЗаписатьТекст(РаботаСоСтроками.УдалитьНедопустимыеСимволыXML(Строка(Отправитель)));
					...
					Иначе
						...
						РеквизитыОтправителя = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Отправитель, "Наименование, НаименованиеПолное");
						Если ПустаяСтрока(РеквизитыОтправителя.НаименованиеПолное) Тогда
							ЗаписьXML.ЗаписатьАтрибут("organization_string", 
								РаботаСоСтроками.УдалитьНедопустимыеСимволыXML(Строка(РеквизитыОтправителя.Наименование)));
						Иначе
							ЗаписьXML.ЗаписатьАтрибут("organization_string", 
								РаботаСоСтроками.УдалитьНедопустимыеСимволыXML(Строка(РеквизитыОтправителя.НаименованиеПолное)));
						КонецЕсли;
				...
				Иначе
					...
					НаименованиеОрганизации = Юрлица.ПредставлениеЮрлицаНаДату(Документ.Организация, ДатаСведений);
					ЗаписьXML.ЗаписатьАтрибут("organization_string", РаботаСоСтроками.УдалитьНедопустимыеСимволыXML(НаименованиеОрганизации));
...
//
// Параметры:
//  Документ - СправочникОбъект.ДокументыПредприятия - Заполняемый объект, входящий документ.
//  ИмяФайла - Имя файла на сервере, содержащего xml.
				...
				Пока Файл.ПрочитатьАтрибут() Цикл
					...
					ИначеЕсли Файл.Имя = "type" Тогда
						...
						ИначеЕсли Файл.Значение = "2" Тогда
							ДополнительноеСодержание = ДополнительноеСодержание + НСтр("ru = 'Документ'");
						КонецЕсли;	
