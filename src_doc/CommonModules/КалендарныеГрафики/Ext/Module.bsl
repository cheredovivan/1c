...
Функция ПолучитьМассивДатПоКалендарю(Знач Календарь, Знач ДатаОт, Знач МассивДней, Знач РассчитыватьСледующуюДатуОтПредыдущей = Ложь) Экспорт
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	КоличествоДнейВГрафикеПоГодам.Год КАК Год,
	...
	|		ПО КалендарныеГрафики.КоличествоДнейВГрафикеСНачалаГода = КоличествоДнейВГрафикеНаДатуОтсчета.КоличествоДнейВГрафикеСНачалаГода
	|			- КоличествоДнейСУчетомПредыдущихГодов.ДнейВГрафике + ПриращениеДней.КоличествоДней
...
// Процедура создает в справочнике Календари календарь на 2024 год, соответствующий производственному
// календарю Российской Федерации, если в справочнике нет ни одного календаря за этот год.
//
// Параметры:
//	Замещать - булево, если Истина, запись производится вне зависимости от наличия данных за 2024 год.
//
Процедура СоздатьПроизводственныйКалендарьНа2024Год(Замещать = Ложь, ЭтоОбновление = Ложь) Экспорт
	
	Если Не Замещать И ПроизводственныйКалендарьСуществует(2024) Тогда
		Возврат;
	КонецЕсли;

	// Записываем данные за 2024 год
	РабочиеДни = Новый Массив;
	РабочиеДни.Добавить(Дата(2024, 4, 27));
	РабочиеДни.Добавить(Дата(2024, 11, 2));
	РабочиеДни.Добавить(Дата(2024, 12, 28));
	
	ПраздничныеДни = ПраздничныеДни2024();
	
	ЗаполнитьКалендарныйГрафик(2024, РабочиеДни, ПраздничныеДни, ЭтоОбновление);
	
КонецПроцедуры

// Возвращает массив праздничных дней 2024 года.
//
Функция ПраздничныеДни2024()
	
	ПраздничныеДни = Новый Массив;
	
	ПраздничныеДни.Добавить(Дата(2024, 1, 1));
	ПраздничныеДни.Добавить(Дата(2024, 1, 2));
	ПраздничныеДни.Добавить(Дата(2024, 1, 3));
	ПраздничныеДни.Добавить(Дата(2024, 1, 4));
	ПраздничныеДни.Добавить(Дата(2024, 1, 5));
	ПраздничныеДни.Добавить(Дата(2024, 1, 6));
	ПраздничныеДни.Добавить(Дата(2024, 1, 7));
	ПраздничныеДни.Добавить(Дата(2024, 1, 8));
	
	ПраздничныеДни.Добавить(Дата(2024, 2, 23));
	ПраздничныеДни.Добавить(Дата(2024, 3, 8));
	
	ПраздничныеДни.Добавить(Дата(2024, 4, 29));
	ПраздничныеДни.Добавить(Дата(2024, 4, 30));
	ПраздничныеДни.Добавить(Дата(2024, 5, 1));
	
	ПраздничныеДни.Добавить(Дата(2024, 6, 12));
	
	ПраздничныеДни.Добавить(Дата(2024, 5, 9));
	ПраздничныеДни.Добавить(Дата(2024, 5, 10));
	
	ПраздничныеДни.Добавить(Дата(2024, 11, 4));
	
	ПраздничныеДни.Добавить(Дата(2024, 12, 30));
	ПраздничныеДни.Добавить(Дата(2024, 12, 31));
	
	Возврат ПраздничныеДни;
	
КонецФункции

// Процедура создает в справочнике Календари календарь на 2025 год, соответствующий производственному
// календарю Российской Федерации, если в справочнике нет ни одного календаря за этот год.
//
// Параметры:
//	Замещать - булево, если Истина, запись производится вне зависимости от наличия данных за 2025 год.
//
Процедура СоздатьПроизводственныйКалендарьНа2025Год(Замещать = Ложь, ЭтоОбновление = Ложь) Экспорт
	
	Если Не Замещать И ПроизводственныйКалендарьСуществует(2025) Тогда
		Возврат;
	КонецЕсли;

	// Записываем данные за 2025 год
	РабочиеДни = Новый Массив;
	РабочиеДни.Добавить(Дата(2025, 11, 1));
	
	ПраздничныеДни = ПраздничныеДни2025();
	
	ЗаполнитьКалендарныйГрафик(2025, РабочиеДни, ПраздничныеДни, ЭтоОбновление);
	
КонецПроцедуры

// Возвращает массив праздничных дней 2025 года.
//
Функция ПраздничныеДни2025()
	
	ПраздничныеДни = Новый Массив;
	
	ПраздничныеДни.Добавить(Дата(2025, 1, 1));
	ПраздничныеДни.Добавить(Дата(2025, 1, 2));
	ПраздничныеДни.Добавить(Дата(2025, 1, 3));
	ПраздничныеДни.Добавить(Дата(2025, 1, 4));
	ПраздничныеДни.Добавить(Дата(2025, 1, 5));
	ПраздничныеДни.Добавить(Дата(2025, 1, 6));
	ПраздничныеДни.Добавить(Дата(2025, 1, 7));
	ПраздничныеДни.Добавить(Дата(2025, 1, 8));
	
	ПраздничныеДни.Добавить(Дата(2025, 2, 23));
	ПраздничныеДни.Добавить(Дата(2025, 3, 8));
	
	ПраздничныеДни.Добавить(Дата(2025, 5, 1));
	ПраздничныеДни.Добавить(Дата(2025, 5, 2));
	
	ПраздничныеДни.Добавить(Дата(2025, 5, 9));
	ПраздничныеДни.Добавить(Дата(2025, 5, 8));
	
	ПраздничныеДни.Добавить(Дата(2025, 6, 12));
	ПраздничныеДни.Добавить(Дата(2025, 6, 13));
	
	ПраздничныеДни.Добавить(Дата(2025, 11, 3));
	ПраздничныеДни.Добавить(Дата(2025, 11, 4));
	
	ПраздничныеДни.Добавить(Дата(2025, 12, 31));
	
	Возврат ПраздничныеДни;
	
КонецФункции

// Процедура создает в справочнике Календари календарь на 2026 год, соответствующий производственному
// календарю Российской Федерации, если в справочнике нет ни одного календаря за этот год.
//
// Параметры:
//	Замещать - булево, если Истина, запись производится вне зависимости от наличия данных за 2026 год.
//
Процедура СоздатьПроизводственныйКалендарьНа2026Год(Замещать = Ложь, ЭтоОбновление = Ложь) Экспорт
	
	Если Не Замещать И ПроизводственныйКалендарьСуществует(2026) Тогда
		Возврат;
	КонецЕсли;

	// Записываем данные за 2026 год
	РабочиеДни = Новый Массив;
	ПраздничныеДни = ПраздничныеДни2026();
	
	ЗаполнитьКалендарныйГрафик(2026, РабочиеДни, ПраздничныеДни, ЭтоОбновление);
	
КонецПроцедуры

// Возвращает массив праздничных дней 2026 года.
//
Функция ПраздничныеДни2026()
	
	ПраздничныеДни = Новый Массив;
	
	ПраздничныеДни.Добавить(Дата(2026, 1, 1));
	ПраздничныеДни.Добавить(Дата(2026, 1, 2));
	ПраздничныеДни.Добавить(Дата(2026, 1, 3));
	ПраздничныеДни.Добавить(Дата(2026, 1, 4));
	ПраздничныеДни.Добавить(Дата(2026, 1, 5));
	ПраздничныеДни.Добавить(Дата(2026, 1, 6));
	ПраздничныеДни.Добавить(Дата(2026, 1, 7));
	ПраздничныеДни.Добавить(Дата(2026, 1, 8));
	ПраздничныеДни.Добавить(Дата(2026, 1, 9));
	
	ПраздничныеДни.Добавить(Дата(2026, 2, 23));
	ПраздничныеДни.Добавить(Дата(2026, 3, 8));
	ПраздничныеДни.Добавить(Дата(2026, 3, 9));
	
	ПраздничныеДни.Добавить(Дата(2026, 5, 1));
	ПраздничныеДни.Добавить(Дата(2026, 5, 9));
	ПраздничныеДни.Добавить(Дата(2026, 5, 11));
	
	ПраздничныеДни.Добавить(Дата(2026, 6, 12));
	
	ПраздничныеДни.Добавить(Дата(2026, 11, 4));
	
	ПраздничныеДни.Добавить(Дата(2026, 12, 31));
