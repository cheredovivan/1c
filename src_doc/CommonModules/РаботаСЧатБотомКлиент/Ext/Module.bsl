...
Процедура СоздатьЧатБота() Экспорт
	...
	Попытка
		...
		ЭтоПервоеВзаимодействиеСБотом = ЗначениеЗаполнено(ПараметрыРаботыКлиента)
						И ПараметрыРаботыКлиента.Свойство("ЭтоПервоеВзаимодействиеСБотом")
						И ПараметрыРаботыКлиента.ЭтоПервоеВзаимодействиеСБотом;
						
		ОбсуждениеСЧатБотом = РаботаСЧатБотомВызовСервера.ОбсуждениеСЧатБотом(ЭтоПервоеВзаимодействиеСБотом);
...
Процедура ОбработкаОтправкиСообщения(Сообщение, Обсуждение, ДополнительныеПараметры) Экспорт
	...
	Если СледующееСостояние.Предопределенный Тогда
		...
		Если ЗначениеЗаполнено(СледующееСостояние.Действие) Тогда
			...
			ВыполениеДействияДоступно = ВыполнениеДействияДоступно();
			
			Если ВыполениеДействияДоступно.Доступно Тогда
			
				Выполнить(СледующееСостояние.Действие);
				
			Иначе
				
				ЗаписатьОшибкуИСообщитьПользователю(КраткоеПредставлениеОшибки(ИнформацияОбОшибке()),
													ВыполениеДействияДоступно.ТекстОшибки);
			КонецЕсли;
...
Процедура ВыполнитьДействие() Экспорт
	...
	ВыполнениеДействияДоступно = ВыполнениеДействияДоступно();
	
	Если ВыполнениеДействияДоступно.Доступно Тогда
	...
	Иначе
		...
		ЗаписатьОшибкуИСообщитьПользователю(КраткоеПредставлениеОшибки(ИнформацияОбОшибке()),
											ВыполнениеДействияДоступно.ТекстОшибки);
...
Процедура ВыполнитьДействиеБезСостояния(Действие)
	...
	Попытка
		...
		Если ТипЗнч(Действие) = Тип("Строка") Тогда
			Выполнить (Действие);
		Иначе
			ОбработатьДействиеБезСостояния(Действие);
		КонецЕсли;
...
// Обрабатывает действия чат-бота, у которых нет состояния.
// 
// Параметры:
//  Действие - Произвольный.
//
Процедура ОбработатьДействиеБезСостояния(Действие)
	
	Если ТипЗнч(Действие) = Тип("Структура")
		И Действие.ИмяМетода = "ПодключитьУведомленияЧерезВнешнююСистему" Тогда
		
		РаботаСУведомлениямиКлиент.ПодключитьУведомленияЧерезВнешнююСистему(
			Действие.ИдентификаторОбсужденияСВ,
			Действие.ТипВнешнейСистемы);
...
Функция ВыполнениеДействияДоступно()

	ТекстОшибки = "";
	
	#Если ВебКлиент Тогда
	
		ТекстОшибки = НСтр("ru = 'Это действие недоступно в веб-клиенте.
	...
	Если СтрНайти(СистемнаяИнформация.ВерсияОС, "ios") Тогда
	
		ТекстОшибки = НСтр("ru = 'Это действие недоступно на iOS.
							|Работайте в тонком клиенте.'");
	...
	Результат.Вставить("Доступно", Не ЗначениеЗаполнено(ТекстОшибки));
	
	Возврат Результат;

КонецФункции

Процедура ЗаписатьОшибкуИСообщитьПользователю(ПредставлениеОшибки, СообщениеПользователю = "")
