...
Функция ПолучитьТаблицуУчастниковПроцесса(БизнесПроцессОбъект) Экспорт
	...
	ДобавленныеИменаРеквизитов = Новый Соответствие();
	...
	Для Каждого ОписаниеУчастника из ОписанияУчастников Цикл
		...
		Если ОписаниеУчастника.Свойство("ТабличнаяЧасть") 
			...
			ИмяРеквизита = ОписаниеУчастника.ТабличнаяЧасть;
		Иначе
			ИмяРеквизита = ОписаниеУчастника.ИмяУчастника;
		КонецЕсли;
		
		Если ДобавленныеИменаРеквизитов[ИмяРеквизита] <> Неопределено Тогда
			Продолжить;
		КонецЕсли;
		
		ИменаРеквизитов.Добавить(ИмяРеквизита);
		ДобавленныеИменаРеквизитов[ИмяРеквизита] = Истина;
		
	КонецЦикла;
	ИменаРеквизитов = СтрСоединить(ИменаРеквизитов, ", ");
	
	Реквизиты = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(
		БизнесПроцессОбъект.Ссылка, ИменаРеквизитов);
	...
	Для Каждого ОписаниеУчастника из ОписанияУчастников Цикл
		...
		Если ОписаниеУчастника.Свойство("ТабличнаяЧасть") 
			...
			Если ЗначениеЗаполнено(БизнесПроцессОбъект.Ссылка) Тогда
				...
				Если ТипЗнч(Реквизиты[ОписаниеУчастника.ТабличнаяЧасть]) <> Тип("ТаблицаЗначений") Тогда
					Реквизиты[ОписаниеУчастника.ТабличнаяЧасть] = 
						Реквизиты[ОписаниеУчастника.ТабличнаяЧасть].Выгрузить();
				КонецЕсли;
			
				Если ОписаниеУчастника.Свойство("Отбор")
					И ЗначениеЗаполнено(ОписаниеУчастника.Отбор) Тогда
					
					СтараяТабЧасть = Реквизиты[ОписаниеУчастника.ТабличнаяЧасть].НайтиСтроки(
						ОписаниеУчастника.Отбор);
				Иначе
					СтараяТабЧасть = Реквизиты[ОписаниеУчастника.ТабличнаяЧасть];
				КонецЕсли;
			...
			Если ОписаниеУчастника.Свойство("Отбор")
				...
				НоваяТабЧасть = БизнесПроцессОбъект[ОписаниеУчастника.ТабличнаяЧасть].НайтиСтроки(
					ОписаниеУчастника.Отбор);
			Иначе
				НоваяТабЧасть = БизнесПроцессОбъект[ОписаниеУчастника.ТабличнаяЧасть];
			КонецЕсли;
			
			Для Каждого СтрокаНовойТЧ Из НоваяТабЧасть Цикл
				НоваяСтрока = Участники.Добавить();
		...
		Если Источник.Стартован Тогда
			...
			Если ОбновитьРабочиеГруппыПредметов Тогда
				...
				Если ДокументооборотПраваДоступаПовтИсп.ВключеноИспользованиеПравДоступа()
					...
					СотрудникиДляПроверки = Мультипредметность.СотрудникиДляПроверкиПрав(УчастникиДляПроверки);
					...
					Для Каждого Предмет Из ПредметыДляПроверкиПравДоступаУчастников Цикл
						...
						Для Каждого СтрокаСотрудник Из СотрудникиДляПроверки Цикл
