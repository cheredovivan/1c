...
Процедура ВыполнитьИмпортФайловСОткрытиемФормыИмпорта(ВладелецИмпортированныхФайлов, 
	РольФайла = Неопределено, МожноДобавитьТолькоОдинФайл = Ложь) Экспорт
	...
	Если МожноДобавитьТолькоОдинФайл = Истина Тогда
	...
	Если ДиалогОткрытияФайла.Выбрать() Тогда
		...
		ПараметрыИмпорта.Вставить("РольФайла", РольФайла);
...
Процедура ЗапуститьПриложениеПоИмениФайла(ИмяОткрываемогоФайла, ПараметрыВыполнения = Неопределено) Экспорт
...
#Если Не МобильныйКлиент Тогда
	Если ОбщегоНазначенияКлиентСервер.ЭтоWindowsКлиент() Тогда
		...
		Если (СистемнаяИнфо.ТипПлатформы = ТипПлатформы.Windows_x86 
			...
			И ПараметрыВыполнения.Свойство("ОткрыватьPdfВWord")
			И ПараметрыВыполнения.ОткрыватьPdfВWord
			И НРег(Расширение) = НРег("pdf") Тогда
				
				// откроем Word и в нем pdf
				#Если Не ВебКлиент Тогда
					...
					Док = WordApp.Documents.Open(ИмяОткрываемогоФайла);
...
Функция СоздатьДокументНаОсновеФайла(
	...
	РазмерФайлаПревышаетМаксимальноДопустимый = Файл.Размер() > МаксРазмерФайла;
	// Локализация
	Если РазмерФайлаПревышаетМаксимальноДопустимый
		И ТаймлистКлиентСервер.ЭтоРасширениеФайлаТаймлист(РасширениеФайла, Истина)
		И ТаймлистВызовСервера.РазрешеноДобавлятьБольшиеФайлы(Файл.Имя, РазмерВМб) Тогда
	...
	Если ЗначениеЗаполнено(ДополнительныеПараметры) 
		...
		И ДополнительныеПараметры.Свойство("ОригинальноеИмя") Тогда
			...
			Если ДанныеОШтрихкоде <> Неопределено И ДанныеОШтрихкоде.Свойство("ДвоичныеДанныеИзображения") Тогда
				...
				Исключение
					...
					ЖурналРегистрацииКлиент.ДобавитьСообщениеДляЖурналаРегистрации(
						НСтр("ru = 'Файлы.Вставка штрихкода'", ОбщегоНазначенияКлиент.КодОсновногоЯзыка()),
						"Ошибка", ТекстСообщения, , Истина);
				КонецПопытки;
...
Функция ПолучитьЗанятыеWordФайлы(Ссылка, УникальныйИдентификатор, 
...
//  УникальныйИдентификатор - УникальныйИдентификатор - Уникальный идентификатор вызывающей формы.
...
Процедура ОткрытьФайлСОповещением(ОбработчикРезультата, ДанныеФайла, УникальныйИдентификатор = Неопределено) Экспорт
	...
	КлючеваяОперация = РаботаСФайламиКлиентСервер.ИмяКлючевойОперации_ФайлыОткрытиеНаПросмотрДо100Кб();
	Если ДанныеФайла.Размер >= 100 * 1024 И ДанныеФайла.Размер < 1024 * 1024 Тогда
		КлючеваяОперация = РаботаСФайламиКлиентСервер.ИмяКлючевойОперации_ФайлыОткрытиеНаПросмотрДо1Мб();
	ИначеЕсли ДанныеФайла.Размер >= 1024 * 1024 Тогда	
		КлючеваяОперация = РаботаСФайламиКлиентСервер.ИмяКлючевойОперации_ФайлыОткрытиеНаПросмотрБолее1Мб();
	КонецЕсли;	
	...
	Если ДанныеФайла.Свойство("ПолноеИмяФайлаВРабочемКаталогеДляУдаления") 
		И ЗначениеЗаполнено(ДанныеФайла.ПолноеИмяФайлаВРабочемКаталогеДляУдаления) Тогда
		
		УдалитьФайлБезПодтверждения(ДанныеФайла.ПолноеИмяФайлаВРабочемКаталогеДляУдаления);
...
//  ВладелецФайлаСписка - ОпределяемыйТип.ВладелецФайлов - Владелец, к которому будут прикреплены файлы.
//  ЭтаФорма - ФормаКлиентскогоПриложения - Форма, в которую делается перетаскивание.
//  НеОткрыватьКарточкуПослеСозданияИзФайла - Булево - Не открывает карточку файла после создания, если Истина.
//  РольФайла - СправочникСсылка.РолиФайлов - Роль файла.
//  НадоПереименоватьФайлыПоШаблонуПослеДрагдропа - Булево.
//  ЦельЭтоРольФайла - Булево
// 
// Возвращаемое значение:
//  Массив из СправочникСсылка.Файлы - Обработанные файлы.
//
Функция ОбработкаПеретаскиванияВЛинейныйСписок(
	...
	Если ТипЗнч(ПараметрыПеретаскивания.Значение) = Тип("Файл")
		...
		ОбработанныеФайлы = ОбработкаПеретаскиванияФайловСДиска(
	...
	ИначеЕсли ТипЗнч(ПараметрыПеретаскивания.Значение) = Тип("Файл")
		...
		Возврат ОбработанныеФайлы;
		
	ИначеЕсли ТипЗнч(ПараметрыПеретаскивания.Значение) = Тип("СправочникСсылка.Файлы") Тогда
		
		ОбработанныйФайл = ПеренестиФайлВПриложенныеФайлы(
			ПараметрыПеретаскивания.Значение,
			ВладелецФайлаСписка,
			ЭтаФорма);
		Если ЗначениеЗаполнено(ОбработанныйФайл) Тогда
			ОбработанныеФайлы.Добавить(ОбработанныйФайл);
		КонецЕсли;
	...
	ИначеЕсли ТипЗнч(ПараметрыПеретаскивания.Значение) = Тип("Массив") Тогда
		...
		ИначеЕсли ПараметрыПеретаскивания.Значение.Количество() >= 1
			...
			ОбработанныеФайлы = ПеренестиФайлыВПриложенныеФайлы(
				ПараметрыПеретаскивания.Значение,
				ВладелецФайлаСписка,
				ЭтаФорма,
				РольФайла,
...
Процедура ОткрытьФайлДокумента(Файл, Форма, ВизуализацияЭПДобавлена = Ложь) Экспорт 
	...
	Если ДанныеФайла.ТребуетсяСоздатьВизуализациюЭП
		...
		#Если Не ВебКлиент И Не МобильныйКлиент Тогда
			...
			СоздатьВизуализациюЭПВФайлеDocXls(ДанныеФайла, Форма.УникальныйИдентификатор);
		#КонецЕсли	
	...
	Если ДанныеФайла.ВизуализацияЭПДобавлена Тогда
		ВизуализацияЭПДобавлена = Истина;
	КонецЕсли;	
...
Процедура ВыполнитьАвтопомещениеФайлов(ЗанятыеФайлы) Экспорт
	...
	Для Каждого СтрФайла Из ЗанятыеФайлы Цикл
		
		ПараметрыОповещения = Новый Структура("МожноПоместитьФайл, Ссылка", Истина, СтрФайла.Ссылка);	
		Оповестить("ВыполнитьАвтопомещениеФайлов_ПроверкаМожноПоместитьФайл", ПараметрыОповещения);
		
		Если ПараметрыОповещения.МожноПоместитьФайл Тогда
			МассивЗахваченныхФайлов.Добавить(СтрФайла);
		КонецЕсли;
...
Процедура ДобавитьФайл(
	...
	ШаблонаДокумента = Неопределено,
	РольФайла = Неопределено,
	ДоступноТолькоДобавлениеОригинала = Неопределено,
	ДополнительныеПараметры = Неопределено) Экспорт
	...
	ПараметрыВыполнения.Вставить("РольФайла", РольФайла);
	ПараметрыВыполнения.Вставить("ДоступноТолькоДобавлениеОригинала", ДоступноТолькоДобавлениеОригинала);
...
Процедура ОткрытьДиалогСписокЗанятыхФайлов(ПараметрыФормы, ОписаниеОповещения) Экспорт
	...
	ПараметрыФормы.Вставить("Редактирует", ПараметрыФормы.ТекущийПользователь);
	
	ВладелецФайла = ?(ПараметрыФормы.Свойство("ВладелецФайла"), ПараметрыФормы.ВладелецФайла, Неопределено);
	ФайлыДляПроверки = ?(ПараметрыФормы.Свойство("ФайлыДляПроверки"), ПараметрыФормы.ФайлыДляПроверки, Неопределено);
	Редактирует = ?(ПараметрыФормы.Свойство("Редактирует"), ПараметрыФормы.Редактирует, Неопределено);
	СотрудникиТекущегоПользователя = ?(ПараметрыФормы.Свойство("СотрудникиТекущегоПользователя"),
		ПараметрыФормы.СотрудникиТекущегоПользователя.ВыгрузитьЗначения(),
		Неопределено);
	
	КоличествоЗанятыхФайлов = РаботаСФайламиВызовСервера.ПолучитьКоличествоЗанятыхФайлов(
		ВладелецФайла, Редактирует, Истина, СотрудникиТекущегоПользователя, ФайлыДляПроверки);
	...
	Если ТипЗнч(ФайлыДляПроверки) = Тип("Массив") Тогда
		СписокФайловДляПроверки = Новый СписокЗначений;
		СписокФайловДляПроверки.ЗагрузитьЗначения(ФайлыДляПроверки);
		ПараметрыФормы.ФайлыДляПроверки = СписокФайловДляПроверки;
	КонецЕсли;
...
// Очищает рабочий каталог при устаревании.
// 
...
Процедура ВыбратьНастройкиРаспознаванияФайлаПоЗапросу(ОбработчикВыбора, УточнитьСтратегиюПоВозможности = Истина) Экспорт
	...
	ИначеЕсли ПрограммаРаспознавания = ПредопределенноеЗначение("Перечисление.ПрограммыРаспознавания.СервисРаспознавания") Тогда
		...
		ВыполнитьОбработкуОповещения(ОбработчикВыбора, Новый Структура("ЯзыкРаспознавания, СтратегияРаспознавания",
			"7", ПредопределенноеЗначение("Перечисление.СтратегииРаспознаванияТекста.ПоместитьТолькоВТекстовыйОбраз")));
...
#Если Не ВебКлиент И Не МобильныйКлиент Тогда
Функция СоздатьВизуализациюЭПВФайлеDocXls(ДанныеФайла, ИдентификаторФормы) Экспорт
	...
	ИспользоватьВнешнийСервисВставкиЭППрежний = ДанныеФайла.НастройкиШтампаЭП.ИспользоватьВнешнийСервисВставкиЭП;
	...
	ИначеЕсли ДвоичныеДанныеНовые = Неопределено 
			И ИспользоватьВнешнийСервисВставкиЭППрежний И Не ДанныеФайла.НастройкиШтампаЭП.ИспользоватьВнешнийСервисВставкиЭП Тогда
...
// Заканчивает редактирование занятых файлов. - без вопросов.
		...
		Если ЗначениеЗаполнено(ФайлыСтрока.ПолноеИмяФайлаВРабочемКаталоге) Тогда
			...
			Если Файл.Существует() Тогда
				...
				Если УниверсальноеВремяИзмененияНаДиске > ФайлыСтрока.ДатаМодификацииУниверсальнаяВБазе
					...
					ОставитьФайлЗанятым = Ложь;
					
					Если РаботаСФайламиКлиент.ФайлОткрытВнешнейПрограммойДляРедактирования(ПутьКИсходномуФайлу) Тогда
						
						ОставитьФайлЗанятым = Истина;
						
					КонецЕсли;	
					
					Если Не ОставитьФайлЗанятым Тогда
...
Функция СохранитьФайлы(
	...
	Для Каждого СписокСтрока Из ДанныеСохраняемыхФайлов.СписокДанныхФайлов Цикл
		...
		Если ДанныеФайла.ТребуетсяСоздатьВизуализациюЭП
			...
			#Если Не ВебКлиент И Не МобильныйКлиент Тогда
				// для doc файла заполним на клиенте - если не веб клиент
				СоздатьВизуализациюЭПВФайлеDocXls(ДанныеФайла, УникальныйИдентификатор);
			#КонецЕсли	
...
Процедура ОсвободитьФайлБезВопроса(ДанныеФайла, УникальныйИдентификатор = Неопределено) Экспорт
	...
	РасширениеПодключено = РаботаСФайламиСлужебныйКлиент.Расширение1СПредприятияПодключено();
...
Процедура НайтиСлишкомБольшиеФайлы(
	...
	Для Каждого ВыбранныйФайл Из МассивФайлов Цикл
		...
		Если ВыбранныйФайл.Существует() Тогда
			...
			РазмерФайлаПревышаетМаксимальноДопустимый = ВыбранныйФайл.Размер() > МаксРазмерФайла;
			// Локализация
			Если РазмерФайлаПревышаетМаксимальноДопустимый
				И ТаймлистКлиентСервер.ЭтоРасширениеФайлаТаймлист(ВыбранныйФайл.Расширение, Истина)
				И ТаймлистВызовСервера.РазрешеноДобавлятьБольшиеФайлы(ВыбранныйФайл.ПолноеИмя, РазмерВМб) Тогда
				РазмерФайлаПревышаетМаксимальноДопустимый = Ложь;
			КонецЕсли;
			// Локализация Конец
...
Процедура ОткрытьФайлПриложением(ДанныеФайла, ИмяОткрываемогоФайла, 
	УникальныйИдентификатор = Неопределено, ПараметрыВыполнения = Неопределено)
	...
	Если РаботаСФайламиКлиентЛокализация.ОткрытьФайлПриложением(ДанныеФайла, ИмяОткрываемогоФайла,
		УникальныйИдентификатор, ПараметрыВыполнения) Тогда
		Возврат;
	КонецЕсли;
	...
	Если РасширениеПодключено Тогда
		...
		ИсполняемыеТекстовыеФайлы = ФайловыеФункцииКлиентСервер.ИсполняемыеТекстовыеФайлы();
		ЭтоИсполняемыйТекстовыйФайл = ФайловыеФункцииКлиентСервер.РасширениеФайлаВСписке(ИсполняемыеТекстовыеФайлы, ДанныеФайла.Расширение);
		
		ТекстовыеФайлыСпособОткрытия = ПерсональныеНастройкиРаботыСФайлами.ТекстовыеФайлыСпособОткрытия;
		Если (ТекстовыеФайлыСпособОткрытия = ПредопределенноеЗначение("Перечисление.СпособыОткрытияФайлаНаПросмотр.ВоВстроенномРедакторе")
			И Не ОтключитьВстроенныйРедактор)
			Или ЭтоИсполняемыйТекстовыйФайл Тогда
			
			ТекстовыеФайлыРасширение = ПерсональныеНастройкиРаботыСФайлами.ТекстовыеФайлыРасширение;
			Если ФайловыеФункцииКлиентСервер.РасширениеФайлаВСписке(ТекстовыеФайлыРасширение, ДанныеФайла.Расширение) 
				Или ЭтоИсполняемыйТекстовыйФайл Тогда
		...
		Если HtmlФайлыСпособОткрытия = ПредопределенноеЗначение("Перечисление.СпособыОткрытияФайлаНаПросмотр.ВоВстроенномРедакторе")
			И Не ОтключитьВстроенныйРедактор Тогда
		...
		Если ИзображенияСпособОткрытия = ПредопределенноеЗначение(
			...
			И Не ОтключитьВстроенныйРедактор
			И ДанныеФайла.НаЧтение = Истина
			И Не ДанныеФайла.Зашифрован Тогда
...
#КонецЕсли
		...
		Попытка
			...
			ЗапуститьПриложениеПоИмениФайла(ИмяОткрываемогоФайла, ПараметрыВыполнения);	
...
// 
// Параметры:
//  Файлы - Файл, Массив из Файл.
//  ВладелецФайлаСписка - ОпределяемыйТип.ВладелецПрисоединенныхФайлов.
...
//  Массив из СправочникСсылка.Файлы - Обработанные файлы.
// 
Функция ОбработкаПеретаскиванияФайловСДиска(
	Файлы,
	ВладелецФайлаСписка,
	ЭтаФорма,
	НеОткрыватьКарточкуПослеСозданияИзФайла = Ложь,
	РольФайла = Неопределено,
	НадоПереименоватьФайлыПоШаблонуПослеДрагдропа = Ложь,
	ЦельЭтоРольФайла = Ложь)
	...
	Для Каждого Файл из МассивФайлов Цикл
		СписокДляСортировки.Добавить(Файл, Файл.ИмяБезРасширения);
	КонецЦикла;
		
	СписокДляСортировки.СортироватьПоПредставлению();
	
	МассивФайлов = Новый Массив;
	Для Каждого СтрСписка Из СписокДляСортировки Цикл
		МассивФайлов.Добавить(СтрСписка.Значение);
	КонецЦикла;	 
	...
	Для Каждого Файл из МассивФайлов Цикл
		...
		СтруктураФайла = НайденныеФайлы.Получить(Файл.Имя);
		
		Если СтруктураФайла <> Неопределено И СтруктураФайла.Количество > 1 Тогда
			ВызватьИсключение СтрШаблон(НСтр("ru = 'В документе более одного файла с именем ""%1"". Перетаскивание невозможно.'"), Файл.Имя);
		КонецЕсли;	
		
		Если (СтруктураФайла = Неопределено
			Или (СтруктураФайла <> Неопределено И (СтруктураФайла.Количество > 1 Или ЦельЭтоРольФайла))) Тогда
			
			ДополнительныеПараметры = Новый Структура("РольФайла", 
				РольФайла);
			
			ОбработанныйФайл = СоздатьДокументНаОсновеФайла(
				Файл.ПолноеИмя,
				ВладелецФайлаСписка,
				ЭтаФорма,
				НеОткрыватьКарточкуПослеСозданияИзФайла,,,,
				ДополнительныеПараметры,,Проект);
			Если ЗначениеЗаполнено(ОбработанныйФайл) Тогда
				ОбработанныеФайлы.Добавить(ОбработанныйФайл);
			КонецЕсли;
			
		Иначе
			ФайлЭлементСправочника = СтруктураФайла.Файл;
			ДанныеФайла = РаботаСФайламиВызовСервера.ДанныеФайла(ФайлЭлементСправочника);
			
			// Запрещаем перетаскивать сам в себя (из той же карточки докумнта в нее же.)
			Если ДанныеФайла.ПолноеИмяФайлаВРабочемКаталоге <> Файл.ПолноеИмя Тогда
			...
			Иначе
				...
				// тут просто сменим роль
				РаботаСФайламиВызовСервера.ЗаменитьРольФайла(ФайлЭлементСправочника, РольФайла);
				НадоПереименоватьФайлыПоШаблонуПослеДрагдропа = Истина;
				
			КонецЕсли;
			
			ОбработанныеФайлы.Добавить(ФайлЭлементСправочника);
...
// 
// Возвращаемое значение:
//  СправочникСсылка.Файлы, Неопределено - Обработанный файл.
//
Функция ПеренестиФайлВПриложенныеФайлы(ФайлСсылка, ВладелецФайла, ЭтаФорма)
	
	ОбработанныйФайл = Неопределено;
...
//  Массив из СправочникСсылка.Файлы - Обработанные файлы.
//
Функция ПеренестиФайлыВПриложенныеФайлы(МассивФайлов, ВладелецФайла, ЭтаФорма,
	РольФайла = Неопределено, НадоПереименоватьФайлыПоШаблонуПослеДрагдропа = Ложь)
	...
	Иначе
		...
		Результат = РаботаСФайламиВызовСервера.ПолучитьДанныеДляПереносаВПриложенныеФайлы(
			МассивФайлов,
			ВладелецФайла);
		...
		Для Каждого ФайлСсылка Из МассивФайлов Цикл
			...
			ИначеЕсли ЗначениеЗаполнено(РольФайла) Тогда
				МассивРоли.Добавить(ФайлСсылка);
			КонецЕсли;
		КонецЦикла;

		Если МассивРоли.Количество() > 0 Тогда
			РаботаСФайламиВызовСервера.УстановитьРолиФайлов(
				МассивРоли,
				РольФайла);   
			НадоПереименоватьФайлыПоШаблонуПослеДрагдропа = Истина;	
		КонецЕсли;
		...
		Если МассивСкопировать.Количество() > 0 Тогда
			СкопированныеФайлы = РаботаСФайламиВызовСервера.СкопироватьФайлВПриложенных(
				МассивСкопировать,
				ВладелецФайла,
				Ложь,
				Истина);
			ОбщегоНазначенияКлиентСервер.ДополнитьМассив(ОбработанныеФайлы, СкопированныеФайлы);
		КонецЕсли;
		
		Если МассивОбновить.Количество() > 0 Тогда
			
			Приемники = РаботаСФайламиВызовСервера.СписокОбновляемыхФайлов(
				МассивОбновить,
				ВладелецФайла);
			Для Каждого ФайлСсылка Из МассивФайлов Цикл
				Приемник = Приемники.Получить(ФайлСсылка);
			...
			ОбновленныеФайлы = РаботаСФайламиВызовСервера.ОбновитьФайлВПриложенных(
				МассивОбновить,
				Приемники,
				ВладелецФайла,
				Истина);
			ОбщегоНазначенияКлиентСервер.ДополнитьМассив(ОбработанныеФайлы, ОбновленныеФайлы);
...
Процедура СравнитьФайлы(ПутьКФайлу1, ПутьКФайлу2, СпособСравненияВерсийФайлов, 
	ЗаголовокЛевый = "", ЗаголовокПравый = "") Экспорт
	
	Если Не ОбщегоНазначенияКлиентСервер.ЭтоWindowsКлиент()
		И Не СпособСравненияВерсийФайлов = "Р7Builder" Тогда
		Возврат;
	КонецЕсли;	
...
Попытка
		...
		ИначеЕсли СпособСравненияВерсийФайлов = "Р7Builder" Тогда
			СравнитьФайлыР7Builder(ПутьКФайлу1, ПутьКФайлу2);
			
		ИначеЕсли СпособСравненияВерсийФайлов = "СравнениеТабличныхДокументов" Тогда
			
			РаботаСФайламиСлужебныйКлиент.СравнитьТабличныеДокументы(ПутьКФайлу1, ПутьКФайлу2, 
				ЗаголовокЛевый, ЗаголовокПравый);
	...
	Исключение
		...
		Если СпособСравненияВерсийФайлов = "MicrosoftOfficeWord" Тогда
			СообщениеОбОшибке = НСтр("ru = 'Ошибка сравнения файлов средствами MS Word.
			|Проверьте что в настройке Word снята галочка ""Открывать вложения электронной почты и другие нередактируемые файлы в режиме чтения""'")
				 + Символы.ВК + СообщениеОбОшибке;
		КонецЕсли;	
...
Процедура СравнитьФайлыР7Builder(ПутьКФайлу1, ПутьКФайлу2) Экспорт

#Если Не ВебКлиент И НЕ МобильныйКлиент Тогда
	
	ПутьКР7Builder = ФайловыеФункцииСлужебныйКлиентСервер.ПерсональныеНастройкиРаботыСФайлами().ПутьКР7Builder;
	Если Не ЗначениеЗаполнено(ПутьКР7Builder) Тогда
		ВызватьИсключение НСтр("ru = 'Не заполнен ""Путь к Р7 Builder""'");
	КонецЕсли;	
	
	Файл1 = Новый Файл(ПутьКФайлу1);
	Файл2 = Новый Файл(ПутьКФайлу2);
	ПутьФайлаКопия1 = ПолучитьИмяВременногоФайла(Файл1.Расширение);
	ПутьФайлаКопия2 = ПолучитьИмяВременногоФайла(Файл2.Расширение);
	КопироватьФайл(ПутьКФайлу1, ПутьФайлаКопия1);
	КопироватьФайл(ПутьКФайлу2, ПутьФайлаКопия2);
	
	ИмяФайлаСкрипта = ПолучитьИмяВременногоФайла("docbuilder");
	ИмяФайлаРезультата = ПолучитьИмяВременногоФайла("docx");
	
	ЗаписьТекста = Новый ЗаписьТекста(ИмяФайлаСкрипта, "UTF-8");
	
	СтрокаДляФайла = СтрШаблон("builder.OpenFile(""%1"");", ПутьФайлаКопия1);
	СтрокаДляФайла = СтрЗаменить(СтрокаДляФайла, "\", "/");
	ЗаписьТекста.ЗаписатьСтроку(СтрокаДляФайла);
	
	СтрокаДляФайла = СтрШаблон("var file = builderJS.OpenTmpFile(""%1"");", ПутьФайлаКопия2);
	СтрокаДляФайла = СтрЗаменить(СтрокаДляФайла, "\", "/");
	ЗаписьТекста.ЗаписатьСтроку(СтрокаДляФайла);
	
	ЗаписьТекста.ЗаписатьСтроку("AscCommonWord.CompareDocuments(Api, file, null);");	
	ЗаписьТекста.ЗаписатьСтроку("file.Close();");
	
	СтрокаДляФайла = СтрШаблон("builder.SaveFile(""docx"", ""%1"");", ИмяФайлаРезультата);
	СтрокаДляФайла = СтрЗаменить(СтрокаДляФайла, "\", "/");
	ЗаписьТекста.ЗаписатьСтроку(СтрокаДляФайла);
	
	ЗаписьТекста.ЗаписатьСтроку("builder.CloseFile();");
	
	ЗаписьТекста.Закрыть(); 

	ПолнаяКоманднаяСтрока = ПутьКР7Builder + " " + ИмяФайлаСкрипта;
	ЗапуститьПриложение(ПолнаяКоманднаяСтрока, , Истина);
...
Функция ПроверитьЗанятыеФайлыПриЗавершенииРаботы()
	...
	КоличествоЗанятыхФайлов = РаботаСФайламиВызовСервера.ПолучитьКоличествоЗанятыхФайлов(, ТекущийПользователь, Истина, СотрудникиТекущегоПользователя);
	Если КоличествоЗанятыхФайлов = 0 Тогда 
		Возврат Неопределено;
	КонецЕсли;	
...
Процедура СохранитьФайлНаДиск(ДанныеФайла, ВыбранноеПолноеИмяФайла, АдресФайла, Файл, ПутьКФайлуВКеше, СохранятьСРасшифровкой)
	...
	Если ПутьКФайлуВКеше = "" Тогда
		...
		Если ПолучитьФайлы(ПередаваемыеФайлы,, ПутьКФайлу, Ложь) Тогда
			...
			Если ПротоколированиеРаботыСотрудниковКлиентПовтИсп.ПолучитьИспользоватьПротоколированиеРаботыСотрудников() Тогда
				ПротоколированиеРаботыСотрудниковВызовСервера.ЗаписатьСохранитьКак(ДанныеФайла.Ссылка, ВыбранноеПолноеИмяФайла);
			КонецЕсли;
...
Процедура ОткрытьФайлСОповещениемПослеУстановкиРасширения(РасширениеУстановлено, ПараметрыВыполнения) Экспорт
	...
	Подключено = РаботаСФайламиСлужебныйКлиент.Расширение1СПредприятияПодключено();
...
Процедура ОткрытьФайлСОповещениемБезРасширенияПослеНапоминания(Результат, ПараметрыВыполнения) Экспорт
	...
	Если НРег(ПараметрыВыполнения.ДанныеФайла.Расширение) = НРег("mxl")
		И ПараметрыВыполнения.ДанныеФайла.РедактируетТекущийПользователь = Ложь
		И Не ПараметрыВыполнения.ДанныеФайла.Зашифрован Тогда
		
		Документ = РаботаСФайламиВызовСервера.ПолучитьТабличныйДокументПоФайлу(ПараметрыВыполнения.ДанныеФайла);
        Документ.Показать(ПараметрыВыполнения.ДанныеФайла.ПолноеНаименованиеВерсии, ПараметрыВыполнения.ДанныеФайла.ПолноеНаименованиеВерсии); 
	...
	Если ИзображенияСпособОткрытия = ПредопределенноеЗначение(
		...
		ПараметрыФормы = Новый Структура(
			"Файл, ДанныеФайла, ИмяОткрываемогоФайла", ДанныеФайла.Ссылка, ДанныеФайла, ДанныеФайла.ПолноеНаименование);
			
		ОткрытьФорму("Справочник.Файлы.Форма.ПросмотрИзображений", 
			ПараметрыФормы, ПараметрыВыполнения.УникальныйИдентификатор, ДанныеФайла.КодФайла);
...
#Если Не ВебКлиент Тогда
	Если СтрДлина(ПараметрыВыполнения.ПолноеИмяФайла) > ПараметрыВыполнения.МаксимальнаяДлинаПолногоПути Тогда
				...
				МаксимальнаяДлинаСпутем = ПараметрыВыполнения.МаксимальнаяДлинаПолногоПути - 5; // 5 - минимум для "C:\1\"
				
				//ПараметрыВыполнения.ПолноеИмяФайла - урежем, и далее ПолучитьССервераИЗарегистрироватьВЛокальномКэшеФайловПродолжение(ПараметрыВыполнения);
				СколькоНадоОтрезать = СтрДлина(ПараметрыВыполнения.ПолноеИмяФайла) - МаксимальнаяДлинаСпутем + 1;
				
				// урезаем.
				ПараметрыВыполнения.ДанныеФайла.ПолноеНаименованиеВерсии 
					= Лев(ПараметрыВыполнения.ДанныеФайла.ПолноеНаименованиеВерсии, СтрДлина(ПараметрыВыполнения.ДанныеФайла.ПолноеНаименованиеВерсии) - СколькоНадоОтрезать);
				ПараметрыВыполнения.ДанныеФайла.ПолноеНаименованиеВерсии = СокрЛП(ПараметрыВыполнения.ДанныеФайла.ПолноеНаименованиеВерсии);	
...
Процедура ПолучитьССервераИЗарегистрироватьВЛокальномКэшеФайловПродолжение(ПараметрыВыполнения)
	...
	КонецЕсли;
	ФайлНаДискеПоИмени = Неопределено;
...
Процедура ПолучитьССервераИЗарегистрироватьВЛокальномКэшеФайловЗавершение(ПараметрыВыполнения)
	...
	Если ПараметрыВыполнения.ДанныеФайла.Размер <> РазмерФайла Тогда
		...
		Если Не ПараметрыВыполнения.ДанныеФайла.Свойство("ПутьОбновленияИзФайлаНаДиске") Тогда
			...
			Если Не ПараметрыВыполнения.ДанныеФайла.Свойство("ВизуализацияЭПДобавлена")
				Или (ПараметрыВыполнения.ДанныеФайла.Свойство("ВизуализацияЭПДобавлена")
				И Не ПараметрыВыполнения.ДанныеФайла.ВизуализацияЭПДобавлена) Тогда
...
Процедура ОсвободитьМестоВРабочемКаталоге(ОбработчикРезультата, РеквизитыВерсии)
	...
	// кешируем, чтобы не обходить файлы каждый раз.
	ИмяПеременной = "СтандартныеПодсистемы.ДатаПоследнейПроверкиМестоВРабочемКаталоге";
	ДатаПоследнейПроверкиМестоВРабочемКаталоге = ПараметрыПриложения.Получить(ИмяПеременной);
	Если ДатаПоследнейПроверкиМестоВРабочемКаталоге <> Неопределено Тогда
		
		// нашли в параметрах
		
		// прошло менее 4 часов с прошлой проверки - не проверяем
		Если ДатаПоследнейПроверкиМестоВРабочемКаталоге > ТекущаяДата() - 4 * 3600 Тогда
			ВернутьРезультат(ОбработчикРезультата, Неопределено);
			Возврат;
		КонецЕсли;	
	...
	ДатаПоследнейПроверкиМестоВРабочемКаталоге = ТекущаяДата();	
	ПараметрыПриложения[ИмяПеременной] = ДатаПоследнейПроверкиМестоВРабочемКаталоге; // запомним время
...
Процедура УдалитьФайлБезПодтверждения(ПолноеИмяФайла)
	...
	Если Файл.Существует() Тогда
		...
		Если РаботаСФайламиКлиент.ФайлОткрытВнешнейПрограммойДляПросмотра(ПолноеИмяФайла) Тогда
			Возврат;
		КонецЕсли;	
...
Процедура РедактироватьФайлПоСсылкеПослеУстановкиРасширения(РасширениеУстановлено, ПараметрыВыполнения) Экспорт
	...
	ПараметрыВыполнения.Вставить("ДанныеФайла", Неопределено);
	ПараметрыВыполнения.Вставить("РасширениеПодключено", РаботаСФайламиСлужебныйКлиент.Расширение1СПредприятияПодключено());
	...
	ЭтоИсполняемыйФайлБинарный = ФайловыеФункцииКлиентСервер.РасширениеФайлаВСписке(ИсполняемыеБинарныеФайлы, ДанныеФайла.Расширение);
	Если ЭтоИсполняемыйФайлБинарный Тогда
		ПредложитьСохранитьФайл(ДанныеФайла, ПараметрыВыполнения.УникальныйИдентификатор);
		Возврат;
	КонецЕсли;	
...
Процедура ЗакончитьРедактированиеСРасширением(ПараметрыВыполнения)
	...
	Если Не ПараметрыВыполнения.ДанныеФайла.РедактируетТекущийПользователь Тогда
		...
		ТекстОшибки = НСтр("ru = 'Файл не занят вами для редактирования.'");
		ВернутьРезультатПослеПоказаПредупреждения(ПараметрыВыполнения.ОбработчикРезультата, ТекстОшибки, Ложь);
		Возврат;
	КонецЕсли;
...
Процедура ОбновитьИзФайлаНаДискеСОповещениемЗавершение(Результат, ПараметрыВыполнения) Экспорт
	...
	Если Результат = Истина Тогда
		ОповеститьОбИзменении(ПараметрыВыполнения.ДанныеФайла.Ссылка);
		Оповестить("Запись_Файл", 
			Новый Структура("Событие, ИдентификаторРодительскойФормы, Файл", 
				"ДанныеФайлаИзменены", Неопределено, ПараметрыВыполнения.ДанныеФайла.Ссылка), 
			ПараметрыВыполнения.ДанныеФайла.Ссылка);
		Оповестить("Запись_Файл", 
			Новый Структура("Событие, ИдентификаторРодительскойФормы", 
				"ВерсияСохранена", Неопределено), 
			ПараметрыВыполнения.ДанныеФайла.Ссылка);
	КонецЕсли;
...
Процедура СохранитьИзмененияФайлаБезРасширенияПослеНапоминания(Результат, ПараметрыВыполнения) Экспорт
	...
	Если (ТипЗнч(Результат) = Тип("Структура") И Результат.Значение = КодВозвратаДиалога.ОК)
...
Процедура СохранитьИзмененияФайлаБезРасширенияПослеЗагрузкиФайла(Помещен, Адрес, ВыбранноеИмяФайла, ПараметрыВыполнения) Экспорт	
	...
	ВыбранныйПутьКФайлу      = ВыбранноеИмяФайла;
...
Процедура ОбновитьИзФайлаНаДискеПродолжение(ПараметрыВыполнения)
	...
	Файл = Новый Файл(ПараметрыВыполнения.ДиалогПолноеИмяФайла);
	...
	Если ПараметрыВыполнения.ВремяИзмененияВыбранное < ПараметрыВыполнения.ДатаФайлаВБазе Тогда // В хранилище более новый. 
		...
		Если ПараметрыВыполнения.Свойство("ПродолжитьРаботу") Тогда
			ПродолжитьРаботу = ПараметрыВыполнения.ПродолжитьРаботу;
		Иначе	
			ПараметрыВыполнения.Вставить("ПродолжитьРаботу", Ложь);
		КонецЕсли;	
		
		Если Не ПродолжитьРаботу Тогда
			ТекстОшибки = СтрШаблон(
				НСтр("ru = 'Файл ""%1"" в хранилище файлов имеет более позднюю дату изменения (%2), чем выбранный файл (%3).
				           |
				           |Продолжить?'"),
				Строка(ПараметрыВыполнения.ДанныеФайла.Ссылка),
			...
			ПараметрыВыполнения.ОбработчикРезультата = ПодготовитьОбработчикДляДиалога(ПараметрыВыполнения.ОбработчикРезультата);
			Обработчик = Новый ОписаниеОповещения("ОбновитьИзФайлаНаДискеПродолжениеПослеОтветаНаВопрос", ЭтотОбъект, ПараметрыВыполнения);
			
			ПоказатьВопрос(Обработчик, ТекстОшибки, РежимДиалогаВопрос.ДаНет, , КодВозвратаДиалога.Нет);
			Возврат;       
		КонецЕсли;
...
// Продолжение процедуры (см. выше).
Процедура ОбновитьИзФайлаНаДискеПродолжениеПослеОтветаНаВопрос(Ответ, ПараметрыВыполнения) Экспорт
...
Процедура ОбновитьИзФайлаНаДискеПослеУдаленияФайлаИзРабочегоКаталога(Результат, ПараметрыВыполнения) Экспорт
	...
	ПротоколированиеРаботыСотрудниковВызовСервера.ЗаписатьИзменение(
		ПараметрыВыполнения.ДанныеФайла.Ссылка, "Обновить из файла на диске");
...
Процедура ДобавитьНаОсновеШаблонаПослеВыбораШаблона(Результат, ПараметрыВыполнения) Экспорт
	...
	ПараметрыФормы.Вставить("РольФайла", ПараметрыВыполнения.РольФайла);
...
Процедура ДобавитьИзФайловойСистемыСРасширением(ПараметрыВыполнения) Экспорт
	...
	Иначе
		ВернутьРезультат(ПараметрыВыполнения.ОбработчикРезультата, Результат.ФайлСсылка);
	КонецЕсли;
...
#КонецЕсли
	...
	РаботаСФайламиКлиентЛокализация.ДополнитьИмяСозданияФайла(ИмяСоздания, РасширениеФайла,
		ПараметрыВыполнения, АдресВременногоХранилищаФайла, ДополнительныеПараметры);
...
#КонецЕсли
	...
	РаботаСФайламиКлиентЛокализация.ДополнитьПараметрыОповещенияПриСозданииФайла(ПараметрыОповещения,
		АдресВременногоХранилищаФайла, ДополнительныеПараметры);
...
// Продолжение процедуры (см. выше).
Процедура ДобавитьИзФайловойСистемыБезРасширенияПослеЗагрузкиФайла(Помещен, Адрес, 
	ВыбранноеИмяФайла, ПараметрыВыполнения) Экспорт
...
#Иначе
	СведенияОЗагружаемомКонтенте = МК_Клиент.СведенияОЗагружаемомКонтенте(СтруктураПути);
	ТолькоИмя = СведенияОЗагружаемомКонтенте.Имя;
	ТолькоРасширение = СведенияОЗагружаемомКонтенте.РасширениеБезТочки;
#КонецЕсли
	...
	Если ПараметрыВыполнения.НеОткрыватьКарточкуПослеСозданияИзФайла <> Истина Тогда
		...
		ОткрытьФорму("Справочник.Файлы.ФормаОбъекта", ПараметрыФормы, ПараметрыВыполнения.ФормаВладелец, , , , ОбработчикРезультата);
	Иначе
		ВернутьРезультат(ПараметрыВыполнения.ОбработчикРезультата, ФайлСсылка); // В ДО нужно возвращать информацию о созданном файле.
	КонецЕсли;
...
#КонецЕсли
	...
	Если ПротоколированиеРаботыСотрудниковКлиентПовтИсп.ПолучитьИспользоватьПротоколированиеРаботыСотрудников() Тогда
		ПротоколированиеРаботыСотрудниковВызовСервера.ЗаписатьСохранитьКак(ПараметрыВыполнения.ДанныеФайла.Ссылка, ВыборФайла.ПолноеИмяФайла);
	КонецЕсли;
...
#Иначе
			СведенияОЗагружаемомКонтенте = МК_Клиент.СведенияОЗагружаемомКонтенте(ВыбранныйФайл);
			ИмяБезРасширения = СведенияОЗагружаемомКонтенте.Имя;
#КонецЕсли
...
Процедура ИмпортироватьФайл(Знач ВыбранныйФайл, Знач Владелец, ПараметрыВыполнения, 
	...
	Если ПараметрыВыполнения.Свойство("РольФайла") Тогда
		СведенияОФайле.РольФайла = ПараметрыВыполнения.РольФайла;
	КонецЕсли;
	
#Если МобильныйКлиент Тогда
		СведенияОЗагружаемомКонтенте = МК_Клиент.СведенияОЗагружаемомКонтенте(ВыбранныйФайл);
		СведенияОФайле.ИмяБезРасширения = СведенияОЗагружаемомКонтенте.Имя;
		СведенияОФайле.РасширениеБезТочки = СведенияОЗагружаемомКонтенте.РасширениеБезТочки;
#КонецЕсли
...
Функция ФайлОткрытВнешнейПрограммойДляПросмотра(ПолныйПутьКФайлу) Экспорт
	...
	#Иначе
		...
		Если Расширение <> ".doc" И Расширение <> ".docx" И Расширение <> ".pdf"
			И Расширение <> ".ppt" И Расширение <> ".pptx" 
			И Расширение <> ".xls" И Расширение <> ".xlsx" Тогда
			Возврат Ложь;
		КонецЕсли;
...
Процедура ПодписатьФайл(ПрисоединенныйФайл, ИдентификаторФормы, ДополнительныеПараметры = Неопределено,
	...
	Если Не ДополнительныеПараметры.Свойство("ДанныеФайла") Тогда
		ПрисоединенныйФайлДляДанных = ПрисоединенныйФайл;
		Если ТипЗнч(ПрисоединенныйФайл) = Тип("Массив") И ПрисоединенныйФайл.Количество() <> 0 Тогда
			ПрисоединенныйФайлДляДанных = ПрисоединенныйФайл[0];
		КонецЕсли;	
		ДополнительныеПараметры.Вставить("ДанныеФайла", РаботаСФайламиСлужебныйВызовСервера.ДанныеФайла(
			ПрисоединенныйФайлДляДанных));
	КонецЕсли;
	
	ОбработкаРезультата = Неопределено;
	ДополнительныеПараметры.Свойство("ОбработкаРезультата", ОбработкаРезультата);
	
	РаботаСФайламиСлужебныйКлиент.ПодписатьФайл(ПрисоединенныйФайл, ИдентификаторФормы, ОбработкаРезультата);
...
Процедура ПроверитьВыполнениеФоновогоЗадания(РезультатФоновогоЗадания,
	ПараметрыОбработчикаОжидания,
	ЗапущеноФоновоеПереименованиеФайлов, ЗаданиеВыполнено, ЗапуститьОбрабтчикОжидания) Экспорт

	Если ТипЗнч(РезультатФоновогоЗадания) <> Тип("Структура") Тогда
		ПроверкаЗавершиласьСОшибками = Истина;
		ЗапущеноФоновоеПереименованиеФайлов = Ложь;
		Возврат;
	КонецЕсли;	
	
	Если Не РезультатФоновогоЗадания.ЗаданиеВыполнено Тогда
		Попытка
			РезультатФоновогоЗадания.ЗаданиеВыполнено = РаботаСФайламиВызовСервера.ЗаданиеВыполнено(РезультатФоновогоЗадания.ИдентификаторЗадания);
		Исключение
			ПроверкаЗавершиласьСОшибками = Истина;
			ЗапущеноФоновоеПереименованиеФайлов = Ложь;
			Возврат;
	...
	Иначе
		...
		КонецЕсли;
		ЗапуститьОбрабтчикОжидания = Истина;
	КонецЕсли;
	
КонецПроцедуры

// Сравнивает 2 версии файл (txt doc doc odt) с помощью MS Office или OpenOffice
//
// Параметры:
//  ИсходнаяВерсияФайла                - СправочникСсылка.ВерсииФайлов - Первая версия файла для сравнения.
//  ИзмененнаяВерсияФайла                - СправочникСсылка.ВерсииФайлов - Вторая версия файла для сравнения.
...
//
Процедура СравнитьВерсииФайлов(ИсходнаяВерсияФайла, ИзмененнаяВерсияФайла, СпособСравненияВерсийФайлов, ИдентификаторФормы) Экспорт
	
	Если Не ОбщегоНазначенияКлиентСервер.ЭтоWindowsКлиент() Или ОбщегоНазначенияКлиентСервер.ЭтоВебКлиент() Тогда
		ВызватьИсключение НСтр("ru = 'Данная команда поддерживается только в тонком клиенте под управлением ОС Windows.'");
	КонецЕсли;
	
	ВерсииФайлов = Новый Массив;
	ВерсииФайлов.Добавить(ИсходнаяВерсияФайла);
	ВерсииФайлов.Добавить(ИзмененнаяВерсияФайла);
	ДанныеВерсийФайлов = РаботаСФайламиВызовСервера.ДанныеВерсийФайловДляОткрытия(
		ВерсииФайлов, ИдентификаторФормы);
	ДанныеИсходногоФайла = ДанныеВерсийФайлов[ИсходнаяВерсияФайла];
	ДанныеИзмененногоФайла = ДанныеВерсийФайлов[ИзмененнаяВерсияФайла];
	
	ТекстСостояния = НСтр("ru = 'Выполняется сравнение версий файлов...'");
	Состояние(ТекстСостояния);
	
	ПолноеИмяИсходногоФайла = "";
	ПолноеИмяИзмененногоФайла = "";
	
	ПараметрыСравненияФайлов = Новый Структура;
	ПараметрыСравненияФайлов.Вставить("ДанныеИсходногоФайла", ДанныеИсходногоФайла);
	ПараметрыСравненияФайлов.Вставить("ДанныеИзмененногоФайла", ДанныеИзмененногоФайла);
	ПараметрыСравненияФайлов.Вставить("ПолноеИмяИсходногоФайла", ПолноеИмяИсходногоФайла);
	ПараметрыСравненияФайлов.Вставить("ПолноеИмяИзмененногоФайла", ПолноеИмяИзмененногоФайла);
	ПараметрыСравненияФайлов.Вставить("СпособСравненияВерсийФайлов", СпособСравненияВерсийФайлов);
	ПараметрыСравненияФайлов.Вставить("ИдентификаторФормы", ИдентификаторФормы);
	
	Обработчик = Новый ОписаниеОповещения(
		"СравнитьВерсииФайловПослеПолучитьФайлВерсииВРабочийКаталог1",
		ЭтотОбъект,
		ПараметрыСравненияФайлов);
	РаботаСФайламиКлиент.ПолучитьФайлВерсииВРабочийКаталог(
		Обработчик, 
		ПараметрыСравненияФайлов.ДанныеИсходногоФайла,
		ПараметрыСравненияФайлов.ПолноеИмяИсходногоФайла,
		ПараметрыСравненияФайлов.ИдентификаторФормы);
	
КонецПроцедуры

// Обработкчик оповещения процедуры РаботаСФайламиКлиент.СравнитьВерсииФайлов().
Процедура СравнитьВерсииФайловПослеПолучитьФайлВерсииВРабочийКаталог1(Результат, ПараметрыСравненияФайлов) Экспорт
	
	Если Не Результат.ФайлПолучен Тогда
		Возврат;
	КонецЕсли;
	
	ПараметрыСравненияФайлов.ПолноеИмяИсходногоФайла = Результат.ПолноеИмяФайла;
	Обработчик = Новый ОписаниеОповещения(
		"СравнитьВерсииФайловПослеПолучитьФайлВерсииВРабочийКаталог2",
		ЭтотОбъект,
		ПараметрыСравненияФайлов);
	РаботаСФайламиКлиент.ПолучитьФайлВерсииВРабочийКаталог(
		Обработчик, 
		ПараметрыСравненияФайлов.ДанныеИзмененногоФайла,
		ПараметрыСравненияФайлов.ПолноеИмяИзмененногоФайла,
		ПараметрыСравненияФайлов.ИдентификаторФормы);
	
КонецПроцедуры

// Обработкчик оповещения процедуры РаботаСФайламиКлиент.СравнитьВерсииФайловПослеПолучитьФайлВерсииВРабочийКаталог1().
Процедура СравнитьВерсииФайловПослеПолучитьФайлВерсииВРабочийКаталог2(Результат, ПараметрыСравненияФайлов) Экспорт
	
	Если Не Результат.ФайлПолучен Тогда
		Возврат;
	КонецЕсли;
	
	ПараметрыСравненияФайлов.ПолноеИмяИзмененногоФайла = Результат.ПолноеИмяФайла;
	
	РаботаСФайламиКлиент.СравнитьФайлы(
		ПараметрыСравненияФайлов.ПолноеИмяИсходногоФайла,
		ПараметрыСравненияФайлов.ПолноеИмяИзмененногоФайла,
		ПараметрыСравненияФайлов.СпособСравненияВерсийФайлов);
	
	ТекстСостояния = НСтр("ru = 'Сравнение версий файлов выполнено.'");
	Состояние(ТекстСостояния);
	
КонецПроцедуры

// Проверяет переданное расширение файла на поддержку использования сравнения.
// 
// Параметры:
//  Расширение - Строка - Расширение файла.
// 
// Возвращаемое значение:
//  Булево - Истина, если расширение поддерживается для сравнения.
Функция РасширениеПоддерживаетсяДляСравнения(Расширение) Экспорт
	
	РасширениеПоддерживается = (Расширение = "txt" 
		ИЛИ Расширение = "doc" ИЛИ Расширение = "docx" 
		ИЛИ Расширение = "rtf" ИЛИ Расширение = "htm" 
		ИЛИ Расширение = "html" ИЛИ Расширение = "odt"
		ИЛИ Расширение = "mxl");
	...
	Возврат РасширениеПоддерживается;
				
КонецФункции

Процедура ПредложитьСохранитьФайл(ДанныеФайла, УникальныйИдентификатор)
	
	ПараметрыВыполнения = Новый Структура;
	ПараметрыВыполнения.Вставить("ДанныеФайла", ДанныеФайла);
	ПараметрыВыполнения.Вставить("УникальныйИдентификатор", УникальныйИдентификатор);
	
	ОбработчикВопроса = Новый ОписаниеОповещения("ПредложитьСохранитьФайлПослеВопроса", 
		ЭтотОбъект, ПараметрыВыполнения);
	
	ПоказатьВопрос(ОбработчикВопроса, НСтр("ru = 'Это исполняемый файл. Сохранить файл на диск?'"), РежимДиалогаВопрос.ДаНет);
	
КонецПроцедуры	

Процедура ПредложитьСохранитьФайлПослеВопроса(КодВозврата, ПараметрыВыполнения) Экспорт
	...
	ДанныеФайла = РаботаСФайламиВызовСервера.ДанныеФайлаДляСохранения(
		ПараметрыВыполнения.ДанныеФайла.Ссылка, Неопределено, ПараметрыВыполнения.УникальныйИдентификатор);
	
	СохранитьКак(Неопределено, ДанныеФайла, ПараметрыВыполнения.УникальныйИдентификатор);
