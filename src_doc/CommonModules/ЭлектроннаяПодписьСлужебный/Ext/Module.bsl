...
// 
// Параметры:
//  МассивФизическихЛиц - Массив из ОпределяемыйТип.ФизическоеЛицо
// 
// Возвращаемое значение:
...
//
Функция СертификатыФизическихЛиц(МассивФизическихЛиц) Экспорт
...
Процедура СертификатПриИзмененииНаСервере(Форма, ОтпечаткиСертификатовНаКлиенте, Шифрование = Ложь, Расшифровка = Ложь) Экспорт
	...
	Если (ТипЗнч(Форма.ОтборСертификатов) <> Тип("СписокЗначений")
		...
		Если Шифрование Тогда
			ОбщегоНазначения.ХранилищеОбщихНастроекСохранить("Криптография", "СертификатДляШифрования", Форма.Сертификат);
		ИначеЕсли Не Расшифровка Тогда
			ОбщегоНазначения.ХранилищеОбщихНастроекСохранить("Криптография", "СертификатДляПодписания", Форма.Сертификат);
		КонецЕсли;
	...
	ЭлектроннаяПодписьДокументооборотСобытия.ПриИзмененииСертификатаНаСервере(
		Форма, ОтпечаткиСертификатовНаКлиенте, Шифрование, Расшифровка);
...
Функция ПолучитьФизическиеЛицаПоПолюСертификатаКомуВыдан(КомуВыдан) Экспорт
	...
	Пока ВыборкаНаименование.Следующий() Цикл
		...
		ФизическиеЛицаНаименования = Новый Массив;
		
		Пока Выборка.Следующий() Цикл
			ФизическиеЛицаНаименования.Добавить(Выборка.Ссылка);
		КонецЦикла;
		
		ФизическиеЛицаПоФИО.Вставить(ВыборкаНаименование.Наименование, ФизическиеЛицаНаименования); 
