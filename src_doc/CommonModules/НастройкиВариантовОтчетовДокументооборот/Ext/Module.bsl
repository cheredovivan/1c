////////////////////////////////////////////////////////////////////////////////
// Настройки вариантов отчетов Документооборот (сервер)
//
////////////////////////////////////////////////////////////////////////////////
...
Процедура УстановитьНастройкиОтчетаПоУмолчанию(МенеджерОтчета, ПараметрыЗаполненя) Экспорт
	...
	Если ТипЗнч(ПараметрыЗаполненя) = Тип("Структура") И ПараметрыЗаполненя.Свойство("НастройкаВариантаОтчета") Тогда
		...
		Исключение
			...
			ЗаписьЖурналаРегистрации(
				ТекстОшибки, УровеньЖурналаРегистрации.Предупреждение, , НастройкаВариантаОтчета.Ссылка, НСтр(
				"ru = 'Отчет'") + ": """ + ИмяОтчета + """" + Символы.ПС + Символы.ПС + ПодробноеПредставлениеОшибки(
				Инфо));
		...
		КлючВариантаОтчета = НастройкаВариантаОтчета.КлючВарианта;
		ИмяОтчета = НастройкаВариантаОтчета.Отчет.Имя;
		НаименованиеМакетаОтчета = "ОбразецОтчета" + КлючВариантаОтчета;
		...
		// Заполнение образцов отчетов
		СведенияОЯзыках = МультиязычностьСервер.СведенияОЯзыках();
		КодОсновногоЯзыка = СведенияОЯзыках.ОсновнойЯзык;
		
		// Если у отчета есть образцы на разных языках, разместим их в соответствующих реквизитах
		ИмяМакета = НаименованиеМакетаОтчета + "_" + КодОсновногоЯзыка;
		Если Метаданные.Отчеты[ИмяОтчета].Макеты.Найти(ИмяМакета) <> Неопределено Тогда

			ДанныеМакета = Отчеты[ИмяОтчета].ПолучитьМакет(ИмяМакета);
			Если ТипЗнч(ДанныеМакета) = Тип("ДвоичныеДанные") Тогда

				КартинкаОтчет = Новый Картинка(ДанныеМакета);
				Если КартинкаОтчет = Неопределено Тогда
					НастройкаВариантаОтчета.ХранилищеСнимокОтчета =
						Новый ХранилищеЗначения(БиблиотекаКартинок["ОбразецОтчетаПустой" + "_" + КодОсновногоЯзыка]);
				Иначе
			...
			Для Н = 1 По СведенияОЯзыках.КоличествоДополнительныхЯзыков Цикл

				СуффиксЯзыка = МультиязычностьКлиентСервер.СуффиксЯзыка(Н);
				КодДопЯзыка = СведенияОЯзыках[СуффиксЯзыка];
				
				ИмяМакета = НаименованиеМакетаОтчета + "_" + КодДопЯзыка;
				ИмяРеквизита = "ХранилищеСнимокОтчета" + СуффиксЯзыка;
				
				Если Не СведенияОЯзыках.Используется[СуффиксЯзыка] Тогда
					Продолжить;
				КонецЕсли;
				...
				Если Метаданные.Отчеты[ИмяОтчета].Макеты.Найти(ИмяМакета) <> Неопределено Тогда
					...
					Если ТипЗнч(ДанныеМакета) = Тип("ДвоичныеДанные") Тогда
						...
						КартинкаОтчет = Новый Картинка(ДанныеМакета);
						Если КартинкаОтчет = Неопределено Тогда
							НастройкаВариантаОтчета[ИмяРеквизита] =
								Новый ХранилищеЗначения(БиблиотекаКартинок["ОбразецОтчетаПустой" + "_" + КодДопЯзыка]);
						Иначе
							НастройкаВариантаОтчета[ИмяРеквизита] = Новый ХранилищеЗначения(КартинкаОтчет);
						КонецЕсли;
		...
		Иначе
			...
			// Если у отчета нет образцов с указанием языка, используем старую логику для заполнения только на основном языке
			Попытка
				ДанныеМакета = Отчеты[ИмяОтчета].ПолучитьМакет(НаименованиеМакетаОтчета);
				Если ТипЗнч(ДанныеМакета) = Тип("ДвоичныеДанные") Тогда
					КартинкаОтчет = Новый Картинка(ДанныеМакета);
				КонецЕсли;
			Исключение
			...
			Иначе
				...
				НастройкаВариантаОтчета.ХранилищеСнимокОтчета = Новый ХранилищеЗначения(
					БиблиотекаКартинок["ОбразецОтчетаПустой" + "_" + КодОсновногоЯзыка]);
			КонецЕсли;
...
Функция ПолучитьРазделыОтчета(Отчет)
	...
	Раздел = Метаданные.Подсистемы.НСИ.Подсистемы.УправлениеБизнесПроцессами;
	Если ПодсистемаСодержитОтчет(Раздел, Отчет) Тогда
		РазделыОтчета.Добавить(Раздел);
	КонецЕсли;
...
Процедура УдалитьУстаревшиеНастройкиВариантовОтчетов()
	...
	// Удаление отчетов СВД
	
	ИмяОтчета = "ИсходящиеДокументы";
	КлючВарианта = "ОшибкиОтправкиДокументовПоСВД";
	НастройкиВариантовОтчетовДокументооборот.УдалитьНастройкуВариантаОтчета(ИмяОтчета, КлючВарианта);
	
	ИмяОтчета = "? ПолучениеДокументовПоСВД";
	КлючВарианта = "ВсеПоступившиеДокументы";
	НастройкиВариантовОтчетовДокументооборот.УдалитьНастройкуВариантаОтчета(ИмяОтчета, КлючВарианта);
	
	ИмяОтчета = "? ПолучениеДокументовПоСВД";
	КлючВарианта = "ЗависшиеВходящиеСообщения";
	НастройкиВариантовОтчетовДокументооборот.УдалитьНастройкуВариантаОтчета(ИмяОтчета, КлючВарианта);
