...
Процедура ИзменитьПредметКорневогоБизнесПроцесса(
	...
	ВедущаяЗадача = РеквизитыЗадачи.БизнесПроцессВедущаяЗадача; // ЗадачаСсылка.ЗадачаИсполнителя
...
Функция ПолучитьШаблоныПоПредметам(Предметы, Тип, ТолькоНастроенные = Истина) Экспорт
	...
	Запрос = Новый Запрос;
		...
		|	ГДЕ
		|		ДокументыПредприятия.Ссылка В(&ПредметыДокументы)) Как ВсеДанные";
...
Функция ПолучитьСписокТиповПредметовПроцесса(ПроцессСсылка, РольПредмета = Неопределено) Экспорт
	...
	Если РольПредмета = Перечисления.РолиПредметов.Основной Тогда
		СписокТипов = РаботаСФайламиВызовСервера.ПолучитьПредставленияТипов(
			Мультипредметность.ТипыОсновныхПредметовВПользовательскомИнтерфейсе(ПроцессСсылка));
	Иначе
		СписокТипов = РаботаСФайламиВызовСервера.ПолучитьПредставленияТипов(
...
//  СписокЗначений - Получить ограничение типов имени предмета шаблона
...
Процедура ДобавитьПредметВШаблоныПроцесса(ВладелецШаблонов, ШаблоныЭтапов, Знач ПредметыЗадач, СтрокаПредмета) Экспорт
	...
	Для Каждого Шаблон Из ШаблоныЭтапов Цикл
		Если ЗначениеЗаполнено(Шаблон) Тогда
			...
			Если ТипЗнч(Шаблон) = Тип("СправочникСсылка.ИменаПредметов") Тогда
				Продолжить;
			КонецЕсли;	
	...
	Для Каждого Тип Из ТипыШаблонов Цикл
		...
		ТекстЗапроса = ТекстЗапроса +"
			...
			|ИЗ
			|	&Таблица Как Шаблон 
			|	ЛЕВОЕ СОЕДИНЕНИЕ &Предметы КАК ПредметыШаблона 
		...
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса,"&Предметы", СтрШаблон("%1.Предметы", Тип.Значение));
...
Процедура УдалитьПредметИзШаблоновПроцесса(ВладелецШаблонов, ШаблоныЭтапов, ИмяПредмета) Экспорт
	...
	Для Каждого Тип Из ТипыШаблонов Цикл
		...
		ТекстЗапроса = ТекстЗапроса + "
			...
			|		КОНЕЦ) КАК ПредметДоступен
			|ИЗ
			|	&ТаблицаПредметы КАК ПредметыШаблона
			|ГДЕ
		...
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса,"&ТаблицаПредметы", СтрШаблон("%1.Предметы", Тип.Значение));
	...
	Пока Выборка.Следующий() Цикл
		...
		Если МультипредметностьКлиентСервер.ЭтоКомплексныйПроцесс(ШаблонОбъект.Ссылка) Тогда
			...
			Если ЗначениеЗаполнено(ШаблонОбъект.Схема) Тогда
				...
				РаботаСКомплекснымиБизнесПроцессамиСервер.ЗаменитьИмяПредметаВУсловияхСхемы(
					ШаблонОбъект, ИсходноеИмяПредмета, ИмяПредмета);
				...
				ШаблоныДействий = 
					...
					НастройкиДействийИВложенныхПроцессовВДанныхПараметров(
					ДанныеПараметров);
...
Процедура ОбновитьШаблоныПодДаннымПредметовЗадач(ВладелецШаблонов, ШаблоныЭтапов, Знач ПредметыЗадач, Знач СтрокаПредмета, РолиПредметовЭтапов = Неопределено) Экспорт
	...
	Если ЗначениеЗаполнено(ВладелецШаблонов) Тогда
	
		ТаблицаРолейПредметовЭтапов = 
			Мультипредметность.РолиПредметовДействийПоКомплексномуПроцессу(
			ВладелецШаблонов);
...
Функция ПолучитьИменаПредметовШаблона(Шаблон) Экспорт
	...
	МассивТипов.Добавить(Тип("СправочникСсылка.ШаблоныПодписания"));
...
Процедура ПроцессПриЗакрытииНаСервере(ПроцессСсылка) Экспорт
	...
	Запрос.Текст = 
		...
		|		ЛЕВОЕ СОЕДИНЕНИЕ &ТаблицаПроцессаПредметы КАК Предметы
	...
	Запрос.Текст = СтрЗаменить(Запрос.Текст, "&ТаблицаПроцессаПредметы", СтрШаблон("%1.Предметы", ТаблицаПроцесса));
...
Процедура УдалитьПредметИзШаблона(Шаблон, ИмяПредмета, ПроверяемыеРеквизиты, ИмяПредметаСтрокой)
	...
	Если МультипредметностьКлиентСервер.ЭтоКомплексныйПроцесс(ШаблонОбъект.Ссылка) Тогда
		...
		Если ЗначениеЗаполнено(ШаблонОбъект.Схема) Тогда
			...
			РаботаСКомплекснымиБизнесПроцессамиСервер.НормализоватьНастройкиУсловийИВложенныхОбработокСхемы(
				ШаблонОбъект);
...
Процедура ДобавитьПредметВШаблон(Шаблон, ПредметыЗадач, СтрокаПредмета, СоответствиеДействий, СоответствиеРолей, ПустойПредмет)
	...
	Если ДоступныеРоли = Неопределено Тогда
		ПолноеИмя = ПроцессСсылка.Метаданные().ПолноеИмя();
		Менеджер = ОбщегоНазначения.МенеджерОбъектаПоПолномуИмени(ПолноеИмя); // БизнесПроцессМенеджер
...
// Получает строку имени предмета по ссылке на предмет.
//
Функция ИмяПредметаСтрокой(Предмет) Экспорт
