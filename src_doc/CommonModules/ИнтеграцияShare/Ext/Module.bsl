...
//   * ОбъектУчета - Неопределено, ОпределяемыйТип.ОснованияЭлектронныхДокументовЭДО - ссылка на объек учета.
...
//    при отправки с клиента для передачи в переопределяемые методы.Параметры передаются в метод
...
//   * ВидФайла - Строка - см. ИнтеграцияShareКлиентСервер.ВидыХраненияФайловВБазе. По умолчанию вариант "ПрисоединенныеФайлы".
...
//Параметры объета учета к публикации электронного документа.
//Примечение: нельзя использовать в транзакции, т.к. на нижних уровнях используется попытка при поиске файлов через БИД.
//
// Параметры:
...
Функция РезультатГлобальногоПоискаДокументаПоПубличнойССылке(СсылкаДляСкачивания) Экспорт
	...
	Если РезультатПоискаДокумента.НастройкиЗагрузки.ВыводитьРезультатПоискаПредпросмотрВВебе Тогда
		...
		Результат.Значение      = ИнтеграцияShareДокументооборотКлиентСервер.СсылкаДляБраузера(СсылкаДляСкачивания);
...
Функция ВыполнитьЗапрос(Знач ЗапросСервиса, Отказ, ТекстОшибки = Неопределено)
	...
	Если ЗначениеЗаполнено(ЗапросСервиса.Данные) Тогда
		Если ТипЗнч(ЗапросСервиса.Данные) = Тип("ДвоичныеДанные") Тогда
			Если ЗапросСервиса.Данные.Размер() > 0 Тогда
...
// Возвращаемое значение:
//  Соответствие из КлючИЗначение - коды ошибок при выполненни HTTP-запросов:
...
Процедура ПодготовитьТранспортныйКонтейнер(ДеревоТранспортныйКонтейнер, АдресКаталога, Знач ДанныеДокумента, Ошибки)
	...
	ГлавныйФайл.Вставить("ЕстьПрисоединенныеФайлы",
		ЗначениеЗаполнено(ДанныеДокумента.ДвоичныеДанныеДополнительныхДанных)
		И ДанныеДокумента.ДвоичныеДанныеДополнительныхДанных.Размер() > 0);
...
Процедура ПрочитатьДанныеЭлектронногоДокумента(ДанныеДокумента, Знач ОписаниеЭлектронногоДокумента) Экспорт
...
Процедура ДополнитьДанныеУчастниковОбмена(ДанныеДокумента)
	...
	ЗеркальныйПоиск = ЗначениеЗаполнено(Организация);
	Если Не ЗеркальныйПоиск Тогда
		ОбменСКонтрагентамиПереопределяемый.СсылкаНаОбъектПоИННКПП(
			"Организации", ДанныеДокумента.ДанныеКонтрагента.ИНН, ДанныеДокумента.ДанныеКонтрагента.КПП, Организация);
		ДанныеДокумента.Организация = Организация;
		Контрагент = Неопределено;
		ОбменСКонтрагентамиПереопределяемый.СсылкаНаОбъектПоИННКПП(
			"Контрагенты", ДанныеДокумента.ДанныеОрганизации.ИНН, ДанныеДокумента.ДанныеОрганизации.КПП, Контрагент);
	...
	ДанныеКонтрагента = ?(ЗеркальныйПоиск, ДанныеДокумента.ДанныеКонтрагента, ДанныеДокумента.ДанныеОрганизации);
	Если Не ЗначениеЗаполнено(Контрагент) Тогда
		РеквизитыКонтрагента = Новый Структура;
		РеквизитыКонтрагента.Вставить("ИНН", ДанныеКонтрагента.ИНН);
		РеквизитыКонтрагента.Вставить("КПП", ДанныеКонтрагента.КПП);
		РеквизитыКонтрагента.Вставить("Наименование", ДанныеКонтрагента.Наименование);
		КонтрагентыЛокализация.СоздатьКонтрагентаПоРеквизитам(РеквизитыКонтрагента, ДанныеДокумента.Контрагент);
	КонецЕсли;
...
Функция ДанныеУчастникаОбмена(Знач УчастникОбмена)
	...
	Если ЗначениеЗаполнено(СведенияУчастника.ОГРН)
		И Не ЗначениеЗаполнено(СведенияУчастника.ИНН) Тогда
		Результат.ИНН          = СведенияУчастника.ОГРН;
	Иначе
...
Процедура ЗаполнитьСсылкуНаОбъектУчетаЭлектронногоДокумента(Знач ДанныеЗагрузкиДокумента)
	...
	Если Не ОбщегоНазначения.СсылкаСуществует(СсылкаНаОбъектУчета)
		Или ОбщегоНазначенияДокументооборот.ЗначениеРеквизитаОбъектаВПривилегированномРежиме(СсылкаНаОбъектУчета,
			"ПометкаУдаления") = Истина Тогда
		Возврат;
	КонецЕсли;
