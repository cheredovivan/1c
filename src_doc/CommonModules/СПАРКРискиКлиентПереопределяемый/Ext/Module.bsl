		...
		Если ВРег(ИмяСобытия) = ВРег("СПАРКРиски:ИзмененоСостояниеФоновогоЗадания") Тогда
			Если Источник = Форма.УникальныйИдентификатор Тогда
					...
					Если НЕ ПустаяСтрока(Параметр.АдресХранилищаФоновогоЗадания) Тогда
						...
						Если ТипЗнч(Форма.ИндексыСПАРКРиски) = ТипСтруктура Тогда
							Если Не Форма.ИндексыСПАРКРиски.ДоступностьПодключенияТестовогоПериода = "Подключение" Тогда
								...
								КонецЕсли;
								ИмяПараметраПриложения = "ИнтернетПоддержкаПользователей.СПАРКРиски.КэшИндексов";
						...
						Форма.ИндексыСПАРКРиски.Вставить(
						...
						Форма.ПодключитьОбработчикОжидания(
							"Подключаемый_ОбновитьОтображениеИндексыСПАРК", 0.1, Истина); // Перерисовать форму.
					Иначе
					КонецЕсли;
				ИначеЕсли Параметр.Статус = "Отменено" Тогда
					...
					Форма.ИндексыСПАРКРиски.Вставить(
						"ВидОшибки",
						ПредопределенноеЗначение("Перечисление.ВидыОшибокСПАРКРиски.НеизвестнаяОшибка"));
					Форма.ИндексыСПАРКРиски.Вставить(
						"СостояниеВыводаДанных",
						ПредопределенноеЗначение("Перечисление.СостоянияВыводаИндексовСПАРКРиски.ВКэшеНетДанных"));
					Форма.ИндексыСПАРКРиски.Вставить(
						"СостояниеЗагрузкиДанных",
						ПредопределенноеЗначение("Перечисление.СостоянияЗагрузкиИндексовСПАРКРиски.ОтменаФоновогоЗадания"));
					Форма.ПодключитьОбработчикОжидания(
						"Подключаемый_ОбновитьОтображениеИндексыСПАРК", 0.1, Истина); // Перерисовать форму.
				ИначеЕсли Параметр.Статус = "Ошибка" Тогда
					...
					Форма.ИндексыСПАРКРиски.Вставить(
						"ВидОшибки", ПредопределенноеЗначение("Перечисление.ВидыОшибокСПАРКРиски.НеизвестнаяОшибка"));
					Форма.ИндексыСПАРКРиски.Вставить(
						"СостояниеВыводаДанных",
						ПредопределенноеЗначение("Перечисление.СостоянияВыводаИндексовСПАРКРиски.ВКэшеНетДанных"));
					Форма.ИндексыСПАРКРиски.Вставить(
						"СостояниеЗагрузкиДанных",
						ПредопределенноеЗначение("Перечисление.СостоянияЗагрузкиИндексовСПАРКРиски.ОшибкаФоновогоЗадания"));
					Форма.ПодключитьОбработчикОжидания(
						"Подключаемый_ОбновитьОтображениеИндексыСПАРК", 0.1, Истина); // Перерисовать форму.
