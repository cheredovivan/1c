...
//
Функция ТекстHTMLФотографии(Контакт) Экспорт
	...
	Если ЗначениеЗаполнено(ДвоичныеДанныеФотографии) И ДвоичныеДанныеФотографии.Размер() > 0 Тогда
...
Процедура ЗаписатьИзображение(АдресВременногоХранилищаФайла, УникальныйИдентификатор, Ссылка, Наименование, 
	...
	Попытка
		...
		Если ТипЗнч(Ссылка) = Тип("СправочникСсылка.Пользователи")
			Или ТипЗнч(Ссылка) = Тип("СправочникСсылка.КонтактныеЛица")
		...
		ИначеЕсли ТипЗнч(Ссылка) = Тип("СправочникСсылка.Сотрудники") Тогда
			ФизЛицо = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Ссылка, "Владелец");
		Иначе
			ВызватьИсключение СтрШаблон(
				НСтр("ru = 'Неизвестный тип ссылки %1 (%2).'"),
				Ссылка,
				ТипЗнч(Ссылка));
		КонецЕсли;
...
Функция СформироватьПодсказку(Контакт) Экспорт
	...
	ПодразделениеИмя = Строка(Сотрудники.ПодразделениеСотрудника(Контакт));
