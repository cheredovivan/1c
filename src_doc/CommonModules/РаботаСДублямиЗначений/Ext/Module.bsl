...
//   Массив Из Структура - массив структур, содержащих ссылки на объекты, у которых совпадает хотя бы один
...
Функция НайтиДубли(ИскомыйОбъект, СтруктураПоиска) Экспорт
	...
	МетаданныеОбъекта = ИскомыйОбъект.Ссылка.Метаданные(); // ОбъектМетаданныхСправочник
	...
	Если МетаданныеОбъекта.Иерархический Тогда
		Запрос.Текст = Запрос.Текст + " И НЕ Спр.ЭтоГруппа ";
	КонецЕсли;
