...
Процедура ДобавитьПараметрыФормированияИсходящегоДокумента(ПараметрыВыполнения, ВыборкаНастроек, НаборПараметровФормирования, ТаблицаСвязанныхОбъектовУчета, РезультатДействий)
	...
	Если НастройкиОтправкиЭДО.ТребуетсяОбновлениеФорматовИСвязейВидовФорматовДляУПД_5_03(ПоддерживаемыеФорматы,
		ФорматыЭлектронныхДокументов) Тогда
...
Функция ЗагрузитьОбработатьДанныеОбъектовКонтейнеров(ДанныеОбъектов, КонтекстДиагностики, ОтпечаткиСертификатов,
	...
	Если (ЭлектроннаяПодпись.ПроверятьЭлектронныеПодписиНаСервере()
		...
		Или (АвтоматическоеПолучениеЭДОПовтИсп.ИспользоватьПолучениеИзСервисаЭДО()
...
Функция ДоступныеДействияПоЭДО(СостояниеДокумента, СвойстваДокумента) Экспорт
	...
	ОбменСКонтрагентамиДОСобытия.ПриОпределенииДействийДоступныхПоЭДО(СостояниеДокумента, СвойстваДокумента, Действия);
...
Функция ПереченьСостоянийОтправкиОтветнойПодписи() Экспорт
	...
	ПереченьСостоянийОтправкиОтветнойПодписи.Вставить("НеТребуется", "Не требуется");
