...
Процедура ОбработкаПолученияДанныхВыбора(ДанныеВыбора, Знач Параметры, СтандартнаяОбработка, ОбъектМетаданных) Экспорт
	...
	ПоляПоИ = Новый Массив;
	Если Параметры.Свойство("Отбор") Тогда
		
		НомерПараметра = 0;
		Для Каждого КлючИЗначение Из Параметры.Отбор Цикл
			
			ИмяПараметра = "Значение" + Строка(НомерПараметра);
			ПоляПоИ.Добавить("Таблица." + КлючИЗначение.Ключ + " = &" + ИмяПараметра);
	...
	Если ПоляПоИ.Количество() <> 0 Тогда
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "(&УсловияОтбораПоИ)", СтрСоединить(ПоляПоИ, " И "));
	Иначе
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "(&УсловияОтбораПоИ) И", "");
	КонецЕсли;
