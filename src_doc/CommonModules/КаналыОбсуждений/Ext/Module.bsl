	...
	Для Каждого Участник Из Обсуждение.Участники Цикл
		...
		Иначе
			...
			Запрос = Новый Запрос(
				...
				|					И СотрудникиПользователей.Пользователь = &ТекущийПользователь
...
Функция ИдентификаторыСВКаналовПользователя(Знач Пользователь = Неопределено) Экспорт
	...
	Запрос = Новый Запрос(
		...
		|		ПО СотрудникиВКонтейнерах.Контейнер = КаналыОбсужденийУчастники.Участник
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.СотрудникиПользователей КАК СотрудникиПользователей
		|		ПО СотрудникиВКонтейнерах.Сотрудник = СотрудникиПользователей.Сотрудник
	...
	Пока Выборка.Следующий() Цикл
		Попытка
			ИдентификаторСВ = Обсуждения.ПользовательСистемыВзаимодействия(Выборка.Ссылка, Истина);
