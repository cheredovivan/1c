...
Процедура ДобавитьКартинку(HTMLТекст, Картинка, Ссылка = "", Подсказка = "") 
	...
	ФорматКартинкиЗначение = Картинка.Формат();
	ФорматКартинкиСтрокой = Строка(ФорматКартинкиЗначение);
	Если Картинка.Вид = ВидКартинки.ИзБиблиотеки
		И ФорматКартинкиЗначение = ФорматКартинки.SVG Тогда
		ФорматКартинкиСтрокой = "svg+xml";
	КонецЕсли;
	
	HTMLТекст = HTMLТекст
		+ "<img src='data:image/"
		+ ФорматКартинкиСтрокой
		+ ";base64,"
...
Функция ПолучитьОбзорКонтрагента(Контрагент, ПоказыватьУдаленные) Экспорт 
	...
	ДобавитьЗначениеСКартинкой(HTMLТекст, НСтр("ru = 'E-mail'"),
		БиблиотекаКартинок.НепрочтенноеПисьмоPng, Емайл, "message");
...
// Локализация	
	...
	Если СПАРКРиски.ИспользованиеРазрешено() Тогда 
		...
		ПредставлениеИндексов = СПАРКРиски.ИндексыСПАРККонтрагента(
			Контрагент,
			ОбщегоНазначенияДокументооборотКлиентСервер.ВидКонтрагентаСПАРК(Контрагент));
...
// Локализация Конец 
	...
	Если КоличествоКонтактныхЛиц > 0 
		Или КоличествоКонтактныхЛиц = 0 И Не РеквизитыОбъекта.ЮрФизЛицо = Перечисления.ЮрФизЛицо.ФизЛицо Тогда
	...
	ОбзорОбъектов.ВставитьСсылкиНаВнешниеОбъекты(Контрагент, HTMLТекст);
