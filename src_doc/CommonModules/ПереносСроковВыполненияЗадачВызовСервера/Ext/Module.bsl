...
//              СправочникСсылка.ПолныеРоли - сотрудник для которого переносится срок задачи.
//  СтарыйСрок - Дата - старый срок исполнения задачи.
//  НовыйСрок - Дата - новый срок исполнения задачи.
//
Функция ПолучитьПодписьДлительностьПереноса(Сотрудник, СтарыйСрок, НовыйСрок) Экспорт
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьГрафикиРаботы") Тогда
		Если Сотрудник = Неопределено Тогда
			ГрафикСотрудника = ГрафикиРаботы.ПолучитьОсновнойГрафикРаботы();
		Иначе
			ГрафикСотрудника = ГрафикиРаботы.ГрафикРаботыСотрудника(Сотрудник);
		КонецЕсли;
		ГрафикиРаботы.ПолучитьДлительностьПериода(ГрафикСотрудника, СтарыйСрок, НовыйСрок);
		РазностьВРабочихДнях = КалендарныеГрафики.ПолучитьРазностьДатПоКалендарю(
			ГрафикСотрудника.Календарь,
			СтарыйСрок,
			НовыйСрок);
