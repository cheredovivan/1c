...
Процедура ПриОпределенииОбщихПараметровБазовойФункциональности(ОбщиеПараметры) Экспорт
	...
	Иначе
		ОбщиеПараметры.МинимальнаяВерсияПлатформы = "8.3.27.1644";
	КонецЕсли;
...
Процедура ПриДобавленииОбработчиковУстановкиПараметровСеанса(Обработчики) Экспорт
	...
	УдалениеПомеченныхОбъектовСлужебный.ПриДобавленииОбработчиковУстановкиПараметровСеанса(Обработчики);
...
Процедура ПриДобавленииПараметровРаботыКлиентаПриЗапуске(Параметры) Экспорт
	...
	Параметры.Вставить(
		...
		РаботаСЧасовымиПоясами.ЧасовойПоясПользователя(Пользователи.ТекущийПользователь()));
	...
	Параметры.Вставить("ЧатБотИспользуется", ПолучитьФункциональнуюОпцию("ИспользоватьЧатБота"));
	...
	Если СВЗарегистрирована Тогда 
		...
		Попытка
			Параметры.Вставить("ЭтоПервоеВзаимодействиеСБотом", 
				РаботаСЧатБотомПовтИсп.ЕстьАктивныеСостояния() И Не РаботаСЧатБотом.ОбсуждениеСБотомСуществует());
		Исключение
			...
			ЗаписьЖурналаРегистрации(
				...
				УровеньЖурналаРегистрации.Ошибка,,,
				ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
			Параметры.Вставить("ЭтоПервоеВзаимодействиеСБотом", Ложь);
		КонецПопытки;
	...
	Если ЭтоПолноправный И ПолучитьФункциональнуюОпцию("РегламентированныйУчетОбращений") Тогда
		Параметры.Вставить(
			"ОбновитьКлассификаторОбращенийГраждан", Константы.АктуализироватьКлассификаторОбращенийГраждан.Получить());
	Иначе 
		Параметры.Вставить("ОбновитьКлассификаторОбращенийГраждан", Ложь);
	КонецЕсли;
	// Локализация Конец
	
	Если ЭтоПолноправный Тогда
		// Для выдачи потом сообщения на клиенте:
		Параметры.Вставить(
			"ТекстНекорректныеРолиСотрудников", ДокументооборотПраваДоступа.НекорректныеРолиСотрудников());
	КонецЕсли;
	
	УстановитьПривилегированныйРежим(Истина);
	ПараметрыСеанса.ЭтоНагрузочноеТестирование = Ложь;
	УстановитьПривилегированныйРежим(Ложь);
	// (Далее, см. ОбщегоНазначенияКлиентПереопределяемый.ПослеНачалаРаботыСистемы при проверке параметра запуска может
	// быть установлен).
	...
	ОбновитьЗадачи =
		...
		И Не Обработки.ОбновлениеЗадачПриПереходеНаНовуюВерсию.ОтложенноеОбновлениеЗавершено();
	Параметры.Вставить("ОбновитьЗадачи", ОбновитьЗадачи);
	...
	Параметры.Вставить("СотрудникиТекущегоПользователя", СотрудникиТекущегоПользователя);
	Параметры.Вставить("РолиТекущегоПользователя", РолиТекущегоПользователя);
	Параметры.Вставить("ОсновнойСотрудник", ОсновнойСотрудник);
...
Процедура ПриДобавленииПараметровРаботыКлиента(Параметры) Экспорт
	...
	РаботаСЗадачами.ПриДобавленииПараметровРаботыКлиента(Параметры);
	
	Отсутствия.ПриДобавленииПараметровРаботыКлиента(Параметры);
	
	Сотрудники.ПриДобавленииПараметровРаботыКлиента(Параметры);
	
	ОбработкиОбъектов.ПриДобавленииПараметровРаботыКлиента(Параметры);
...
Процедура ПриДобавленииПереименованийОбъектовМетаданных(Переименования) Экспорт
	...
	ОбщегоНазначения.ДобавитьПереименование(Переименования, "2.0.10.4", СтароеИмя, НовоеИмя, "");
	...
	ОбщегоНазначения.ДобавитьПереименование(Переименования, "2.1.0.11", СтароеИмя, НовоеИмя, "");
	...
	ОбщегоНазначения.ДобавитьПереименование(Переименования, "2.1.7.7", СтароеИмя, НовоеИмя, "");
