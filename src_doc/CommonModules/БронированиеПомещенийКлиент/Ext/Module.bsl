...
Процедура ОбработкаОкончанияРедактирования(Планировщик) Экспорт
	...
	РезультатБронирования = БронированиеПомещенийВызовСервера.ИзменитьБрони(ИзмененныеБрони);
	
	КоличествоБроней = РезультатБронирования.РезультатыБроней.Количество();
	Если КоличествоБроней = 1 Тогда
		ТекстОповещения = НСтр("ru = 'Изменена бронь'");
		Бронь = РезультатБронирования.РезультатыБроней[0].Бронь;
...
Процедура ОбработкаВыбораЭлемента(Планировщик, СтандартнаяОбработка) Экспорт
	...
	Если ЗначениеЭлемента.ВидЭлемента =
		...
		ОткрытьБронь(ЗначениеЭлемента.Ссылка);
...
Процедура СоздатьИЗаписатьБронь(Помещение, ДатаНачала, ДатаОкончания, ВесьДень, Комментарий) Экспорт
	...
	РезультатБронирования = БронированиеПомещенийВызовСервера.СоздатьБронь(Бронь);
	
	РезультатБрони = РезультатБронирования.РезультатыБроней[0];
...
Процедура СоздатьБронь(Помещение, ДатаНачала, ДатаОкончания, ВесьДень = Ложь, Комментарий = "", КоличествоЧеловек = 0) Экспорт
	...
	КлючеваяОперация = БронированиеПомещенийКлиентСервер.КлючеваяОперацияСозданиеБрони();
	ОценкаПроизводительностиКлиент.ЗамерВремени(КлючеваяОперация);
...
// 
Процедура ОткрытьБронь(Бронь) Экспорт
	
	КлючеваяОперация = БронированиеПомещенийКлиентСервер.КлючеваяОперацияОткрытиеБрони();
	ОценкаПроизводительностиКлиент.ЗамерВремени(КлючеваяОперация);
	
	ПараметрыОткрытия = Новый Структура;
	ПараметрыОткрытия.Вставить("Ключ", Бронь);
	ОткрытьФорму("Документ.Бронь.ФормаОбъекта", ПараметрыОткрытия);
...
Процедура ОбработкаВыбораПовторяющегосяСобытияЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	ИначеЕсли Результат = "ИзменитьВсе" Тогда 
		ОткрытьБронь(ДополнительныеПараметры.Бронь);
	КонецЕсли;
