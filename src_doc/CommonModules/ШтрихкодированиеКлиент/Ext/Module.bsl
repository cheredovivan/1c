...
//
//Параметры:
// СтрокаШК - Строка - В ней штрихкод
// ОкноВладелец - ФормаКлиентскогоПриложения
Процедура ПоискПоШтрихкоду(Знач СтрокаШК, ОкноВладелец) Экспорт
	
	Если Не ШтрихкодированиеКлиентПовтИсп.ШтрихкодированиеВключено() Или Не ЗначениеЗаполнено(СтрокаШК) Тогда
		Возврат;
	КонецЕсли;                   
	
	СтрокаШКОригинал = СтрокаШК;
	МассивШК = ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(СтрокаШК);
	
	// добавим первый символ. Временный обход  
	Если СтрДлина(СтрокаШК) = 12  И СтроковыеФункцииКлиентСервер.ТолькоЦифрыВСтроке(СтрокаШК) Тогда
		СтрокаШК = "0" + СтрокаШК;         
		МассивШК.Добавить(СтрокаШК);
	КонецЕсли;
	
	МассивНайденных = ШтрихкодированиеСервер.НайтиОбъектыПоШтрихкоду(МассивШК, Истина); // ИскатьЗадачиПоОбъектам = Истина
	...
	Если МассивНайденных.Количество() = 1 Тогда   
		...
		Если ДанныеДляОткрытияФормы <> Неопределено Тогда   
			...
			ПоказатьЗначение(, ДанныеДляОткрытияФормы.Ключ);
	...
	Иначе
		Текст = НСтр("ru = 'Документы и файлы не найдены (возможно у вас недостаточно прав)'");
		ПоказатьПредупреждение(, Текст);	
	КонецЕсли;
