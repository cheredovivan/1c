...
//
Функция СпособСравненияВерсийФайлов() Экспорт
	...
	ИначеЕсли СпособСравнения = Перечисления.СпособыСравненияВерсийФайлов.Р7Builder Тогда
		Возврат "Р7Builder";
...
Процедура ДобавитьНастройкиРаботыСФайлами(ОбщиеНастройки, ПерсональныеНастройки) Экспорт
	...
	ПерсональныеНастройки.Вставить("ПутьКР7Builder",  ПутьКР7Builder());
	
	ПерсональныеНастройки.Вставить("СпособПроверкиОрфографии",  СпособПроверкиОрфографии());
...
// 
// Параметры:
//  ОбъектФайла - СправочникОбъект.ВерсииФайлов
//  ПутьКФайлу - Строка
//
...
Процедура ПриДобавленииПараметровРаботыКлиентаПриЗапуске(Параметры) Экспорт
	...
	Если ТипЗнч(Пользователь) = Тип("СправочникСсылка.Пользователи") Тогда
		СотрудникиПользователя = СотрудникиВызовСервера.СотрудникиПользователя(Пользователи.ТекущийПользователь());
		КоличествоЗанятыхФайлов = РаботаСФайламиСлужебныйВызовСервера.ПолучитьКоличествоЗанятыхФайлов(, Пользователь, СотрудникиПользователя);
	КонецЕсли;
...
Процедура ЗаполнитьДополнительныеДанныеФайла(Результат, ПрисоединенныйФайл, ВерсияФайла = Неопределено) Экспорт
	...
	Если ЗначениеЗаполнено(ТекущаяВерсияФайла) Тогда
		...
		ПолноеИмяФайлаВРабочемКаталоге = РаботаСФайламиСлужебныйВызовСервера.ПолучитьПолноеИмяФайлаИзРегистра(
...
Функция СоздатьВерсию(ФайлСсылка, СведенияОФайле) Экспорт
	...
	Версия.УдалитьСтатусИзвлеченияТекста = Перечисления.СтатусыИзвлеченияТекстаФайлов.НеИзвлечен;
...
Процедура ПроверитьРазмерФайлаДляЗагрузки(Файл) Экспорт
	...
	РазмерФайлаПревышаетМаксимальноДопустимый = Размер > ОбщиеНастройки.МаксимальныйРазмерФайла;
	// Локализация
	Если РазмерФайлаПревышаетМаксимальноДопустимый
		И ТаймлистКлиентСервер.ЭтоРасширениеФайлаТаймлист(Файл.Расширение, Истина)
		И ТаймлистВызовСервера.РазрешеноДобавлятьБольшиеФайлы(Имя, РазмерВМб) Тогда
	...
	Попытка
		...
		Если МетаданныеСправочника.ПолнотекстовыйПоиск = ИспользованиеПолнотекстовогоПоиска Тогда
			...
			Иначе
				...
				ТекстХранилище = РезультатИзвлеченияТекста.ТекстХранилище;  
				РегистрыСведений.ТекстыФайлов.ДобавитьЗапись(ФайлОбъект.Ссылка, ТекстХранилище);
...
Процедура ПеренестиДанныеЭлектроннойПодписиВРегистрСведений(МассивОбъектов, ПолноеИмяОбъектаМетаданных, ОбъектМетаданных)
	...
	Попытка
		...
		Для Каждого Строка Из Выгрузка.Строки Цикл
			...
			НаборЗаписей.Отбор.Объект.Установить(Строка.ПодписанныйОбъект);
...
Функция ИспользоватьПредпросмотрФайлов() Экспорт
	...
	Возврат ИспользоватьОбзорДокументов И ИспользоватьПредпросмотрФайловОбщая
