...
//
Процедура ИзменитьРольПредмета(Форма, Объект, ВыбраннаяСтрока, ЭтоПроцесс) Экспорт
	...
	ПараметрыФормы.Вставить("ТипПредмета", ТипЗнч(ТекущаяСтрока.Предмет));
...
Процедура ИзменитьРольПредметаПродолжение(РезультатВыбора, Параметры) Экспорт
	...
	Если ЭтоПроцесс Тогда
		...
		Если МультипредметностьКлиентСервер.ЭтоКомплексныйПроцесс(Объект.Ссылка) Тогда
			...
			УстановитьПредметыЗадачПоТочкамМаршрута(Форма, Объект, СтруктураПредмета);
		КонецЕсли;
	Иначе
...
// Открывает форму установки алгоритма проверки по виду предмета, устанавливает условие в табличной части
//
Процедура УстановкаАлгоритмаПроверки(ИменаПредметов, Элемент, Поле, СтандартнаяОбработка) Экспорт
	
	Если Не (Поле.Имя = "ИсполнителиОписаниеУсловия"
		Или Поле.Имя = "ОбъектЭтапыОписаниеУсловия"
		Или Поле.Имя = "ОписаниеУсловия") Тогда
	...
	ОписаниеОповещения = Новый ОписаниеОповещения(
		"УстановкаАлгоритмаПроверкиПродолжение",
		ЭтотОбъект,
		ПараметрыОбработчика);
	ОткрытьФорму(
...
Процедура УстановкаАлгоритмаПроверкиПродолжение(Результат, Параметры) Экспорт
...
// Открывает диалог выбора алгоритма проверки для предмета процесса.
// В диалоге можно выбрать/изменить предмет и алгоритм его проверки. 
// 
// Параметры:
...
//  ОбработчикВыбора – ОписаниеОповещения - обработчик по обработке выбора.
//    Обработчику выбора передается структура с полями ИмяПредмета, АлгоритмПроверки
//
Процедура ВыбратьАлгоритмПроверкиДляПредметаПроцесса(
	ИмяПредмета, АлгоритмПроверки, ПредметыПроцесса, ФормаВладелец, ОбработчикВыбора) Экспорт
	
	ПараметрыФормы = Новый Структура;
	ПараметрыФормы.Вставить("ИмяПредмета", ИмяПредмета);
	ПараметрыФормы.Вставить("АлгоритмПроверки", АлгоритмПроверки);
	ПараметрыФормы.Вставить("ПредметыПроцесса", ПредметыПроцесса);
	
	ОткрытьФорму(
		"ОбщаяФорма.ВыборАлгоритмаПроверкиПредметаПроцесса",
		ПараметрыФормы,
		ФормаВладелец,,,,
		ОбработчикВыбора)
