	...
	Если ЗначениеЗаполнено(НайденныйПользователь) Тогда
		...
		Если ПриНачалеРаботыСистемы
		   ...
		   И ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.УправлениеДоступом") Тогда
			...
			МодульУправлениеДоступомСлужебный.ПриСозданииАдминистратора(НайденныйПользователь,
...
//
// Параметры:
//  Группа - СправочникСсылка.РабочиеГруппы
...
Процедура ОбновитьИерархиюГрупп(Группа, ИзмененияСоставов, Проверять = Истина) Экспорт
	...
	|ИЗ
	|	Справочник.РабочиеГруппы КАК ТекущаяТаблица
	|ГДЕ
	...
	Если ТипЗнч(Группа) = Тип("СправочникСсылка.ГруппыВнешнихПользователей") Тогда
		Запрос.Текст = СтрЗаменить(Запрос.Текст,
			"Справочник.РабочиеГруппы", "Справочник.ГруппыВнешнихПользователей");
	КонецЕсли;
	...
	|	РегистрСведений.ИерархияГруппПользователей КАК ИерархияГруппПользователей
	|ГДЕ
	|	ИерархияГруппПользователей.ГруппаПользователей = ЗНАЧЕНИЕ(Справочник.РабочиеГруппы.ПустаяСсылка)
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ ПЕРВЫЕ 1
...
Процедура ОбновитьСоставГруппыВсеПользователи(Пользователь, ИзмененияСоставов) Экспорт
	...
	|	РегистрСведений.СоставыГруппПользователей КАК СоставыГруппПользователей
	|ГДЕ
	|	СоставыГруппПользователей.ГруппаПользователей = ЗНАЧЕНИЕ(Справочник.РабочиеГруппы.ПустаяСсылка)
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
...
//                      - Массив из СправочникСсылка.РабочиеГруппы
...
Процедура ОбновитьСоставыИерархическихГруппПользователей(ГруппаПользователей, ИзмененияСоставов,
	...
	Если УдалениеГруппы Тогда
		...
		|			ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.РабочиеГруппы.Состав КАК ГруппыПользователейСостав
		|				ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.СотрудникиПользователей КАК СотрудникиПользователей
		|				ПО ГруппыПользователейСостав.Участник = СотрудникиПользователей.Сотрудник
		|				И СотрудникиПользователей.Сотрудник.Действует
		|			ПО (ГруппыПользователейСостав.Ссылка = НижестоящиеГруппы.ГруппаПользователей)
		|				И (ГруппыПользователейСостав.Участник <> ЗНАЧЕНИЕ(Справочник.Сотрудники.ПустаяСсылка))
		|				И (НЕ ГруппыПользователейСостав.Ссылка В
		|						(ВЫБРАТЬ
		|							ВЫРАЗИТЬ(НижестоящиеГруппы.ГруппаПользователей КАК Справочник.РабочиеГруппы) КАК ГруппаПользователей
		|						ИЗ
		...
		|			И (СотрудникиПользователей.Пользователь = СоставыГруппПользователей.Пользователь)
		|ГДЕ
		|	ГруппыПользователейСостав.Участник ЕСТЬ NULL
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
	...
	ИначеЕсли ЗначениеЗаполнено(ГруппаПользователей) Тогда
		...
		|		И НЕ СотрудникиПользователей.Пользователь.Недействителен КАК Используется,
		|	СоставыГруппПользователей.Используется КАК ИспользуетсяБыло
		|ИЗ
		...
		|			И (СоставыГруппПользователей.Пользователь = СотрудникиПользователей.Пользователь)
		|ГДЕ
	...
	Иначе
		...
		|	СоставыГруппПользователей.Пользователь КАК Пользователь
		|ИЗ
		|	Справочник.РабочиеГруппы КАК ГруппыДляОбновления1
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.СоставыГруппПользователей КАК СоставыГруппПользователей
		...
		|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.РабочиеГруппы КАК ГруппыДляОбновления2
		|			ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.ИерархияГруппПользователей КАК НижестоящиеГруппы
		...
		|	СоставыГруппПользователей.Используется КАК ИспользуетсяБыло
		|ИЗ
		|	Справочник.РабочиеГруппы КАК ГруппыДляОбновления
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.ИерархияГруппПользователей КАК НижестоящиеГруппы
	...
	Если ДляВнешнихПользователей Тогда
		...
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "Справочник.РабочиеГруппы",
		...
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "Справочник.Сотрудники",
			"Справочник.ВнешниеПользователи");
		...
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "ГруппыПользователейСостав.Участник",
			"ГруппыПользователейСостав.ВнешнийПользователь");
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "СотрудникиПользователей.Пользователь",
			"ГруппыПользователейСостав.ВнешнийПользователь");
...
Процедура ОбновитьИспользуемостьСоставовГруппПользователей(Пользователь, ИзмененияСоставов) Экспорт
	...
	|	И ТИПЗНАЧЕНИЯ(СоставыГруппПользователей.ГруппаПользователей) = ТИП(Справочник.РабочиеГруппы)
...
Процедура ЗарегистрироватьИзмененияСоставовГрупп(ДанныеДляРегистрации) Экспорт
	...
	|ИЗ
	|	ИзмененияСоставов КАК ИзмененияСоставов
	|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.РабочиеГруппы.Состав КАК ГруппыПользователейСостав
...
Процедура ДобавитьПользователяВГруппу(ГруппаВладелец, ПользовательСсылка, ИмяКолонкиСостава, Добавлен) Экспорт
	...
	Если ГруппаВладелец = Справочники.РабочиеГруппы.ВсеПользователи
	...
	Попытка
		...
		ГруппаВладелецОбъект = ГруппаВладелец.ПолучитьОбъект(); // СправочникОбъект.РабочиеГруппы, СправочникОбъект.ГруппыВнешнихПользователей
...
// Параметры:
//  ПользовательИлиГруппа - СправочникСсылка.РабочиеГруппы
//                        - СправочникСсылка.ГруппыВнешнихПользователей
//                        - СправочникСсылка.Пользователи
//                        - СправочникСсылка.ВнешниеПользователи
//
...
Процедура ПриПолученииДанных(ЭлементДанных, ПолучениеЭлемента, ОтправкаНазад, ОтПодчиненного)
	...
	Если ОтПодчиненного И ОбщегоНазначения.РазделениеВключено() Тогда
		Если ТипЭлемента = Тип("КонстантаМенеджерЗначения.ИспользоватьВнешнихПользователей")
		 ...
		 Или ТипЭлемента = Тип("СправочникОбъект.РабочиеГруппы")
	...
	ИначеЕсли ТипЭлемента = Тип("СправочникОбъект.РабочиеГруппы")
	      Или ТипЭлемента = Тип("СправочникОбъект.ГруппыВнешнихПользователей") Тогда
	...
	Если ТипСсылки = Тип("СправочникСсылка.РабочиеГруппы")
...
Процедура ОбновитьВспомогательныеДанныеЭлементовИзмененныхПриПолученииДанных()
	...
	Попытка
		...
		ОбработатьЗарегистрированноеИзменениеИерархии("ГруппыПользователейИерархия",
			Справочники.РабочиеГруппы.ПустаяСсылка(), ИзмененияСоставов, ОчисткаРегистрации);
