...
Процедура ИзменитьДействияВФорме(Знач ПользовательИлиГруппа, Знач ДействияВФорме) Экспорт
	...
	ДействияВФорме.Роли = "";
	Если ДействияВФорме.СвойстваПользователяИБ = "" Тогда
...
Процедура ПриУстановкеНачальныхНастроек(НачальныеНастройки) Экспорт
	...
	НачальныеНастройки.НастройкиКлиента.ВариантИнтерфейсаКлиентскогоПриложения =
...
Процедура ПриПолученииПрочихНастроек(СведенияОПользователе, Настройки) Экспорт
	...
	Запрос = Новый Запрос(
		"ВЫБРАТЬ
		|	НастройкиРабочихСтолов.Настройка,
		|	НастройкиРабочихСтолов.ЗначениеНастройки
		|ИЗ
		|	РегистрСведений.НастройкиРабочихСтолов КАК НастройкиРабочихСтолов
		|ГДЕ
		|	НастройкиРабочихСтолов.Пользователь = &Пользователь");
	
	Запрос.УстановитьПараметр("Пользователь", СведенияОПользователе.ПользовательСсылка);
	
	Выборка = Запрос.Выполнить().Выбрать();
	Пока Выборка.Следующий() Цикл
		
		ИдентификаторНастройки = Перечисления.НастройкиРабочегоСтола.ИдентификаторНастройки(
			Выборка.Настройка);
		
		СтруктураНастройки = Новый Структура("НазваниеНастройки, КартинкаНастройки, СписокНастроек");
		СтруктураНастройки.НазваниеНастройки = Строка(Выборка.Настройка);
		СтруктураНастройки.КартинкаНастройки = "";
		СтруктураНастройки.СписокНастроек = Новый СписокЗначений;
		СтруктураНастройки.СписокНастроек.Добавить(Выборка.ЗначениеНастройки.Получить());
		
		Настройки.Вставить(ИдентификаторНастройки, СтруктураНастройки);
		
	КонецЦикла;
	
	// Настройки блоков навигации.
	Запрос = Новый Запрос(
		"ВЫБРАТЬ
		|	НастройкиБлоковНавигации.БлокНавигации,
		|	НастройкиБлоковНавигации.ЗначениеНастройки
		|ИЗ
	...
	НастройкиБлоковНавигации = Запрос.Выполнить().Выгрузить();
	Если НастройкиБлоковНавигации.Количество() > 0 Тогда
		
		ИдентификаторНастройки = "НастройкиБлоковНавигации";
		
		СтруктураНастройки = Новый Структура("НазваниеНастройки, КартинкаНастройки, СписокНастроек");
		СтруктураНастройки.НазваниеНастройки = НСтр("ru = 'Настройки блоков навигации'");
		СтруктураНастройки.КартинкаНастройки = "";
		СтруктураНастройки.СписокНастроек = Новый СписокЗначений;
		СтруктураНастройки.СписокНастроек.Добавить(
			ОбщегоНазначения.ТаблицаЗначенийВМассив(НастройкиБлоковНавигации));
...
Процедура ПриСохраненииПрочихНастроек(СведенияОПользователе, Настройки) Экспорт
	...
	Если Перечисления.НастройкиРабочегоСтола.ЭтоИдентификаторНастройки(Настройки.ИдентификаторНастройки) Тогда
		
		Настройка = Перечисления.НастройкиРабочегоСтола.НастройкаПоИдентификатору(
			Настройки.ИдентификаторНастройки);
		
		РегистрыСведений.НастройкиРабочихСтолов.УстановитьЗначениеНастройки(
			СведенияОПользователе.ПользовательСсылка,
			Настройка,
			Настройки.ЗначениеНастройки[0].Значение);	
		
	ИначеЕсли Настройки.ИдентификаторНастройки = "НастройкиБлоковНавигации" Тогда
		
		Для Каждого СтрокаНастройки Из Настройки.ЗначениеНастройки[0].Значение Цикл
			РегистрыСведений.НастройкиБлоковНавигации.ЗаписатьЗначениеНастройки(
				СведенияОПользователе.ПользовательСсылка,
				СтрокаНастройки.БлокНавигации,
				СтрокаНастройки.ЗначениеНастройки.Получить());
		КонецЦикла;
...
//                            - Неопределено
//   * ТекущаяСтрока - СправочникСсылка.РабочиеГруппы - строка, на которую будет выполнено
