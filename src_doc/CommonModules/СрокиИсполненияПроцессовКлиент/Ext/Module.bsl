...
Процедура ЗаполнитьСрокиИсполненияВТаблицеИсполнителейПоТекущейСтроке(
	...
	Иначе
		...
		Если СрокиИсполненияПроцессовКлиентСервер.ЭтоСтрокаОтветственного(Исполнители[0]) Тогда
			ИндексПервойСтроки = 1;
		КонецЕсли;
...
// Изменяет срок исполнения в текущей строке дереве участников по представлению.
//
// Параметры:
//  Форма - ФормаКлиентскогоПриложения - форма процесса или шаблона процесса
//  ЭлементУчастники - ТаблицаФормы - элемент формы дерево участников.
//  РеквизитУчастники - ДанныеФормыКоллекцияЭлементовДерева - дерево участников.
//
Процедура ИзменитьСрокПоПредставлениюВДеревеУчастников(Форма, ЭлементУчастники) Экспорт
	...
	СрокиИсполненияПроцессовКлиентПереопределяемый.
		ПриИзмененииСрокаПоПредставлениюВДеревеУчастников(
		Форма, ЭлементУчастники, СтандартнаяОбработка);
	...
	Если РезультатЗаполнения Тогда
		...
		ЗаполнитьСрокиВДеревеУчастниковПоТекущейСтроке(Форма, ТекущиеДанные);
...
Процедура ВыбратьСрокИсполненияДляСтрокиДереваУчастников(
	...
	СрокиИсполненияПроцессовКлиентПереопределяемый.ПриВыбореСрокаИсполненияДляСтрокиДереваУчастников(
		Форма, ЭлементУчастники, УчастникиПроцесса, СтандартнаяОбработка);
	...
	СкорректироватьДатуОтсчетаДляВыбораСрокВДеревеУчастников(
	...
	ОписаниеОповещения = Новый ОписаниеОповещения(
		"ЗавершитьВыборСрокаИсполненияДляСтрокиДереваУчастников", ЭтотОбъект, ДопПараметры);
...
// Корректирует дату отсчета для выбора срока исполнения участника, в древе участников.
// 
// Параметры:
// 	ДатаОтсчета - Дата - корректируемая дата.
// 	СтрокаУчастника - ДанныеФормыЭлементДерева - строка участника, для которого
//                                               корректируется дата отсчета.
// 	УчастникиПроцесса - ДанныеФормыКоллекцияЭлементовДерева - дерево участников в карточке.
//
Процедура СкорректироватьДатуОтсчетаДляВыбораСрокВДеревеУчастников(
	ДатаОтсчета, СтрокаУчастника, УчастникиПроцесса) Экспорт
	
	// Процедура рассчитывает на то, что структур дерева может принимать один из след. вариантов:
	// 
	// Вариант 1: всего 2 участника, выполняют процесс последовательно.
	//	Участник1
	//	Участник2
	//
	// Вариант 2: вначале процесс выполняет группа (последовательно или параллельно), потом еще один
	// учачастник.
	// 	ГруппаУчастников
	//		УчастникГ1
	//		...
	...
	//	Участник1
	//
	// Вариант 3: Вначале процесс выполняют группы участников, потом еще один участник/
	// Группы идут строго по очереди, участники в группах - последовательно или параллельно.
	// 	Корневая группа
	//		ГруппаУчастников1
	//			УчастникГ11
	//			...
	...
	СтрокиДереваКОбработке = Новый Массив();
	Для Каждого КорневаяСтрока Из УчастникиПроцесса.ПолучитьЭлементы() Цикл
		СтрокиДереваКОбработке.Добавить(КорневаяСтрока);
	КонецЦикла;
	
	ПараллельноеВыполнение =
		ПредопределенноеЗначение("Перечисление.ПорядокВыполненияЭтапаПроцесса.Параллельно");
	
	Пока СтрокиДереваКОбработке.Количество() > 0 Цикл
		
		ТекущаяСтрока = СтрокиДереваКОбработке[0];
		СтрокиДереваКОбработке.Удалить(0);
		
		Если ЗначениеЗаполнено(ТекущаяСтрока.ИдентификаторУчастника) Тогда
			Если ТекущаяСтрока = СтрокаУчастника Тогда
				Прервать;
			КонецЕсли;
			ДатаОтсчета = Макс(ДатаОтсчета, ТекущаяСтрока.СрокИсполнения);
			Продолжить;
		КонецЕсли;
		
		ПодчиненныеСтроки = ТекущаяСтрока.ПолучитьЭлементы();
		КоличествоПодчиненных = ПодчиненныеСтроки.Количество(); 
		ЭтоГруппаУчастников = Истина;
		Для Каждого ПодчиненнаяСтрока Из ПодчиненныеСтроки Цикл
			ЭтоГруппаУчастников = ЗначениеЗаполнено(ПодчиненнаяСтрока.ИдентификаторУчастника);
			Прервать;
		КонецЦикла;
		
		Если Не ЭтоГруппаУчастников Тогда
			
			Индекс = (КоличествоПодчиненных - 1);
			Пока Индекс >= 0 Цикл
				СтрокиДереваКОбработке.Вставить(0 , ПодчиненныеСтроки[Индекс]);
				Индекс = Индекс - 1;
			КонецЦикла;
		...
		ПорядокВыполненияГруппы = ПараллельноеВыполнение;
		Если ТекущаяСтрока.Свойство("ПорядокВыполненияУчастниками") Тогда
			ПорядокВыполненияГруппы = ТекущаяСтрока.ПорядокВыполненияУчастниками;
		КонецЕсли;
		
		Если ПорядокВыполненияГруппы = ПараллельноеВыполнение Тогда
			
			ВГруппеЕстьСтокаУчастника = Ложь;
			Для Каждого ПодчиненнаяСтрока Из ПодчиненныеСтроки Цикл
				Если ПодчиненнаяСтрока = УчастникиПроцесса Тогда
					ВГруппеЕстьСтокаУчастника = Истина;
					Прервать;
				КонецЕсли;
			КонецЦикла;
			
			Если ВГруппеЕстьСтокаУчастника Тогда
				Прервать;
			КонецЕсли;
			
			СрокИсполненияГруппы = Дата(1,1,1);
			Если КоличествоПодчиненных > 0 Тогда
				СрокИсполненияГруппы = ПодчиненныеСтроки[0].СрокИсполнения;
			КонецЕсли;
			ДатаОтсчета = Макс(ДатаОтсчета, СрокИсполненияГруппы);
		...
		Иначе
			...
			Для Каждого ПодчиненнаяСтрока Из ПодчиненныеСтроки Цикл
				Если ПодчиненнаяСтрока = УчастникиПроцесса Тогда
					...
					Прервать;
				КонецЕсли;
				ДатаОтсчета = Макс(ДатаОтсчета, ПодчиненнаяСтрока.СрокИсполнения);
			КонецЦикла;
...
// см. ВыбратьСрокИсполненияДляИсполнителей
//
Процедура ЗавершитьВыборСрокаИсполненияДляСтрокиДереваУчастников(
	РезультатВыбора, ДопПараметры) Экспорт
	...
	СтрокаУчастника = ДопПараметры.СтрокаУчастника;
	Форма = ДопПараметры.Форма;
	
	СтрокаУчастника.СрокИсполнения = РезультатВыбора;
	
	СтрокаУчастника.ВариантУстановкиСрокаИсполнения = ВариантыУстановкиСрокаИсполнения.ТочныйСрок;
	
	ЗаполнитьСрокиВДеревеУчастниковПоТекущейСтроке(Форма, СтрокаУчастника);
...
// Изменяет относительный срок (дни, часы, минуты) в текущей строке дерева участников.
//
// Если задана отрицательное значение параметра Длительность, то срок уменьшается.
// 
// Параметры:
...
// 	Длительность Число - значение, на которое необходимо увеличить срок.
//
Процедура ИзменитьСрокВДеревеУчастников(Форма, ЭлементУчастники, Длительность) Экспорт
...
//
Процедура ЗаполнитьСрокиВДеревеУчастниковПоТекущейСтроке(Форма, СтрокаУчастника) Экспорт
	
	// Меняем срок исполнения у всех участников, которые выполняют подписание вместе с текущим.
	
	СтрокаРодителя = СтрокаУчастника.ПолучитьРодителя();
	Если СтрокаРодителя = Неопределено Тогда
		Возврат;
	КонецЕсли;
	
	ПодчиненныеЭлементы = СтрокаРодителя.ПолучитьЭлементы();
	...
	ПорядокВыполнения = ПараллельноеВыполнение;
	Если СтрокаРодителя.Свойство("ПорядокВыполненияУчастниками") Тогда
		ПорядокВыполнения = СтрокаРодителя.ПорядокВыполненияУчастниками;
	КонецЕсли;
	
	Если ПорядокВыполнения <> ПараллельноеВыполнение Тогда
		Возврат;
	КонецЕсли;
	
	РеквизитыСроков = 
		"СрокИсполнения, СрокИсполненияДни, СрокИсполненияЧасы,
		|СрокИсполненияМинуты, ВариантУстановкиСрокаИсполнения";
	
	Для Каждого ПодчиненнаяСтрока Из ПодчиненныеЭлементы Цикл
		Если ПодчиненнаяСтрока = СтрокаУчастника Тогда
			Продолжить;
		КонецЕсли;
		ЗаполнитьЗначенияСвойств(ПодчиненнаяСтрока, СтрокаУчастника, РеквизитыСроков);
	КонецЦикла;
