...
// - Документы (Массив)
//   - Элемент (СправочникСсылка.ДокументыПредприятия)
// - Шифровать (Булево)
...
Функция Создать() Экспорт
	...
	Результат.Вставить("ДействияПриОтправкеПоПочтеСЭП", ОтправлятьПодписиЭППоПочте);
	// Локализация
	...
	Если Не ЗначениеЗаполнено(НастройкиПрофилейДляОтправки.Профиль) Тогда
		ВызватьИсключение НСтр("ru = 'Укажите один из профилей легкой почты для отправки как основной'");
	КонецЕсли;	
...
Функция СформироватьСтруктуруПочтовогоСообщения(ПараметрыОтправки, УникальныйИдентификатор) Экспорт
	...
	Для каждого Вложение Из ПараметрыОтправки.Вложения Цикл
		Если Вложение.Тип = "СсылкаНаФайл" Тогда
			...
			Если Вложение.Свойство("Представление") Тогда
