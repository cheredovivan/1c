...
Функция ПолучитьСвойстваПодписиПослеПодписания(ДвоичныеДанные, Контекст, ПараметрыПодписи = Неопределено)
	...
	Если Контекст.СертификатВерен <> Неопределено Тогда
		...
		СвойстваПодписи.СертификатДействителен = Контекст.СертификатВерен;
	КонецЕсли;
...
Асинх Процедура ПослеСбораТехническойИнформацииОПрограммах(ИнформацияОПрограммах, Контекст) Экспорт
	...
	Если ЗначениеЗаполнено(КаталогЛогаКомпоненты) Тогда
		...
		Если Ждать Файл.СуществуетАсинх() Тогда
			...
			Если ЗначениеЗаполнено(ДвоичныеДанныеФайла) И ДвоичныеДанныеФайла.Размер() > 0 Тогда	
...
Процедура УстановитьФизическоеЛицо(Форма, КомуВыдан, ФизическоеЛицо)
	...
	ФизическиеЛицаСертификата = Результат.ФизическиеЛица.Получить(КомуВыдан);
	...
	Если ФизическиеЛицаСертификата.Количество() = 1 Тогда
		Если ЭтоФормаЭлемента Тогда
			Форма.Объект.ФизическоеЛицо = ФизическиеЛицаСертификата[0];
		Иначе
			Форма.СертификатФизическоеЛицо = ФизическиеЛицаСертификата[0];
		КонецЕсли;
		Возврат;
	КонецЕсли;
