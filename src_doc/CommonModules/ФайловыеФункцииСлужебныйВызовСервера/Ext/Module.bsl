...
Процедура ПроверитьПодписи(ИсходныеДанные, ДанныеСтрок) Экспорт
	...
	Для каждого СтрокаПодписи Из ДанныеСтрок Цикл
		...
		РезультатПроверки = МодульЭлектроннаяПодпись.ПроверитьПодпись(МенеджерКриптографии,
			ИсходныеДанные, СтрокаПодписи.АдресПодписи, ОписаниеОшибки, СтрокаПодписи.ДатаПодписи, РезультатПроверкиБСП);
			
		РезультатПроверки = РаботаСЭПКлиентСервер.РезультатПроверкиДОПоРезультатуБСП(РезультатПроверкиБСП);	
		...
		СрокПроверкиСертификата = РаботаСЭП.СрокПроверкиДействияСертификата(
			СтрокаПодписи.АдресСертификата, СтрокаПодписи.СрокДействияПоследнейМеткиВремени);
