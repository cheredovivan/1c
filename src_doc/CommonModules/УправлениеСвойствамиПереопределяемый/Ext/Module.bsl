...
Процедура ПриПолученииПредопределенныхНаборовСвойств(Наборы) Экспорт
	...
	СтрокаДерева = Наборы.Строки.Добавить();
	СтрокаДерева.Имя = "Справочник_Сотрудники";
	СтрокаДерева.Идентификатор = Новый УникальныйИдентификатор("2e2f071d-94fc-4f7a-9e43-5896b6ac0663");
	...
	СтрокаДерева = Наборы.Строки.Добавить();
	СтрокаДерева.Имя = "Справочник_ФизическиеЛица";
	СтрокаДерева.Идентификатор = Новый УникальныйИдентификатор("7b96d339-f5e9-452c-b992-468917c40080");
	СтрокаДерева.Используется = Истина;
	СтрокаДерева.ЭтоГруппа = Ложь;
...
Процедура ЗаполнитьНаборыСвойствОбъекта(Знач Объект, ТипСсылки, НаборыСвойств, СтандартнаяОбработка, КлючНазначения) Экспорт
	...
	Если ТипСсылки = Тип("СправочникСсылка.ДокументыПредприятия") Тогда
		...
		Если ОбщегоНазначения.ЭтоСсылка(ТипЗнч(Объект)) Тогда
			...
			Если КэшОбластиПросмотра.Заполнен
				...
				И КэшОбластиПросмотра.ДанныеОбзораДокумента.Ссылка = Объект Тогда
				ОбъектВидДокумента = КэшОбластиПросмотра.ДанныеОбзораДокумента.ВидДокумента;
			ИначеЕсли КэшОбзораДокумента.Заполнен И КэшОбзораДокумента.Документ = Объект Тогда
				ОбъектВидДокумента = КэшОбзораДокумента.РеквизитыДокумента.ВидДокумента;
			Иначе
				ОбъектВидДокумента = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Объект, "ВидДокумента");
			КонецЕсли;
		Иначе
			ОбъектВидДокумента = Объект.ВидДокумента;
		КонецЕсли;
		ЗаполнитьНаборыСвойствПоВидуДокумента(ОбъектВидДокумента, НаборыСвойств);
		
	ИначеЕсли ТипСсылки = Тип("СправочникСсылка.Контрагенты") Тогда	
		
		Если ОбщегоНазначения.ЭтоСсылка(ТипЗнч(Объект)) Тогда
			ОбъектЮрФизЛицо = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Объект, "ЮрФизЛицо");
		Иначе
			ОбъектЮрФизЛицо = Объект.ЮрФизЛицо;
		КонецЕсли;
		ЗаполнитьНаборыСвойствПоВидуКонтрагента(ОбъектЮрФизЛицо, НаборыСвойств);
		
	ИначеЕсли ТипСсылки = Тип("СправочникСсылка.БанковскиеСчета") Тогда 
		
		Если ОбщегоНазначения.ЭтоСсылка(ТипЗнч(Объект)) Тогда
			ОбъектВладелец = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Объект, "Владелец");
		Иначе
			ОбъектВладелец = Объект.Владелец;
		КонецЕсли;
		ЗаполнитьНаборыСвойствБанковскогоСчета(ОбъектВладелец, НаборыСвойств);
		
	ИначеЕсли ОбщегоНазначения.ЭтоБизнесПроцесс(Метаданные.НайтиПоТипу(ТипСсылки)) Тогда
		
		ЗаполнитьДоступныеНаборыСвойствБизнесПроцесса(ОбъектСсылка, НаборыСвойств);
		
	ИначеЕсли ТипСсылки = Тип("ЗадачаСсылка.ЗадачаИсполнителя") Тогда
		ЗаполнитьДоступныеНаборыСвойствЗадачи(Объект, НаборыСвойств);
		
	ИначеЕсли ШаблоныБизнесПроцессовКлиентСервер.ЭтоШаблонПроцесса(ОбъектСсылка) Тогда
	...
	ИначеЕсли ТипСсылки = Тип("СправочникСсылка.Мероприятия") Тогда
		...
		Если ОбщегоНазначения.ЭтоСсылка(ТипЗнч(Объект)) Тогда
			ОбъектВидМероприятия = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Объект, "ВидМероприятия");
		Иначе
			ОбъектВидМероприятия = Объект.ВидМероприятия;
		КонецЕсли;
		ЗаполнитьНаборыСвойствПоВидуМероприятия(ОбъектВидМероприятия, НаборыСвойств);
		
	ИначеЕсли ТипСсылки = Тип("СправочникСсылка.Проекты") Тогда 
		
		Если ОбщегоНазначения.ЭтоСсылка(ТипЗнч(Объект)) Тогда
			ОбъектВидПроекта = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Объект, "ВидПроекта");
		Иначе
			ОбъектВидПроекта = Объект.ВидПроекта;
		КонецЕсли;
		ЗаполнитьНаборыСвойствПроекта(ОбъектВидПроекта, НаборыСвойств);
	...
	ИначеЕсли ТипСсылки = Тип("СправочникСсылка.ПроектныеЗадачи") Тогда 
		...
		ЗаполнитьНаборыСвойствПроектнойЗадачи(
			ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ОбъектВладелец, "ВидПроекта"),
			НаборыСвойств);
...
Процедура ЗаполнитьНаборыСвойствПоВидуДокумента(ВидДокумента, НаборыСвойств) Экспорт
	...
	Если ЗначениеЗаполнено(ВидДокумента) Тогда
		...
		Если КэшОбластиПросмотра.Заполнен
			...
			ВидДокументаНаборСвойств = КэшОбластиПросмотра.ДанныеОбзораДокумента.ВидДокументаНаборСвойств;
		ИначеЕсли КэшОбзораДокумента.Заполнен И КэшОбзораДокумента.ВидДокумента = ВидДокумента Тогда
			ВидДокументаНаборСвойств = КэшОбзораДокумента.КэшВидаДокумента.НаборСвойств;
		Иначе
			ВидДокументаНаборСвойств = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ВидДокумента, "НаборСвойств");
		КонецЕсли;
		Если ЗначениеЗаполнено(ВидДокументаНаборСвойств) Тогда
			ДобавитьСтрокуНабора(НаборыСвойств, ВидДокументаНаборСвойств);
		КонецЕсли;
	КонецЕсли;	
...
Процедура ЗаполнитьНаборыСвойствПроекта(ВидПроекта, НаборыСвойств) Экспорт
	...
	Если ЗначениеЗаполнено(ВидПроекта) Тогда
		ВидПроектаНаборСвойствПроектов = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ВидПроекта, "НаборСвойствПроектов");
		Если ЗначениеЗаполнено(ВидПроектаНаборСвойствПроектов) Тогда
			ДобавитьСтрокуНабора(НаборыСвойств, ВидПроектаНаборСвойствПроектов);
		КонецЕсли;
	КонецЕсли;
...
Процедура ЗаполнитьНаборыСвойствПроектнойЗадачи(ВидПроекта, НаборыСвойств) Экспорт
	...
	Если ЗначениеЗаполнено(ВидПроекта) Тогда
		ВидПроектаНаборСвойствПроектныхЗадач =
			ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ВидПроекта, "НаборСвойствПроектныхЗадач");
		Если ЗначениеЗаполнено(ВидПроектаНаборСвойствПроектныхЗадач) Тогда
			ДобавитьСтрокуНабора(НаборыСвойств, ВидПроектаНаборСвойствПроектныхЗадач);
		КонецЕсли;
	КонецЕсли;
...
Процедура ЗаполнитьНаборыСвойствПоВидуМероприятия(ВидМероприятия, НаборыСвойств) Экспорт
	...
	Если ЗначениеЗаполнено(ВидМероприятия) Тогда
		ВидМероприятияНаборСвойств =
			ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ВидМероприятия, "НаборСвойств");
		Если ЗначениеЗаполнено(ВидМероприятияНаборСвойств) Тогда
			ДобавитьСтрокуНабора(НаборыСвойств, ВидМероприятияНаборСвойств);
		КонецЕсли;
	КонецЕсли;
...
Функция ЭтоНаборСвойствЗадачиПроцесса(НаборСвойств)
	...
	Для Каждого МетаданныеПроцесса Из Метаданные.БизнесПроцессы Цикл
		...
		Для Каждого ТочкаМаршрута Из БизнесПроцессы[МетаданныеПроцесса.Имя].ТочкиМаршрута Цикл
			Если Не ТочкаМаршрутаПоддерживаетНаборыДопРеквизитов(ТочкаМаршрута) Тогда
				Продолжить;
			КонецЕсли;
...
Функция ТочкаМаршрутаПоддерживаетНаборыДопРеквизитов(ТочкаМаршрута)
