...
// Определяет количество новых тем.
// 
// Возвращаемое значение:
//  Число - Количество новых тем.
//
Функция ВсегоНовыхТем() Экспорт
	
	Запрос = Новый Запрос(
		"ВЫБРАТЬ РАЗРЕШЕННЫЕ
		|	КОЛИЧЕСТВО(ТемыСообщений.Ссылка) КАК ВсегоНовыхТем
		|ИЗ
	...
	Возврат Выборка.ВсегоНовыхТем;
	
КонецФункции

// Определяет количество новых ответов.
// 
// Возвращаемое значение:
//  Число - Количество новых ответов.
//
Функция ВсегоНовыхОтветов() Экспорт
	...
	Запрос = Новый Запрос(
		...
		|			И НЕ СообщенияОбсуждений.Автор В (&ВсеСотрудникиТекущегоПользователя)
		|			И СообщенияОбсуждений.ПометкаУдаления = ЛОЖЬ
		|			
		|ГДЕ
		|	МоиТемы.Автор В (&ВсеСотрудникиТекущегоПользователя)
		...
		|			И СообщенияОбсуждений.ПометкаУдаления = ЛОЖЬ
		|ГДЕ
		|	МоиСообщения.Автор В (&ВсеСотрудникиТекущегоПользователя)
		|	И ЕСТЬNULL(СведенияОПрочтении.Прочтен, ЛОЖЬ) = ЛОЖЬ");
	Запрос.УстановитьПараметр("ВсеСотрудникиТекущегоПользователя", Сотрудники.ВсеСотрудникиТекущегоПользователя(Ложь));
	Запрос.УстановитьПараметр("ТекущийПользователь", Пользователи.ТекущийПользователь());
	...
	Запрос.Текст = 
		"ВЫБРАТЬ РАЗРЕШЕННЫЕ
		|	КОЛИЧЕСТВО(СообщенияОбсуждений.Ссылка) КАК ВсегоНовыхОтветов
		|ИЗ
	...
	Возврат Выборка.ВсегоНовыхОтветов;
	
КонецФункции

// Определяет последние новые темы.
// 
// Возвращаемое значение:
//  Массив из Структура - Данные последних новых тем. См. РаботаСВиджетами.ДанныеСтрокиСписка().
//
Функция ПоследниеНовыеТемы() Экспорт
	
	Запрос = Новый Запрос(
		"ВЫБРАТЬ РАЗРЕШЕННЫЕ ПЕРВЫЕ 5
		|	ТемыСообщений.Ссылка КАК Ссылка,
		|	ТемыСообщений.Представление КАК Представление,
		|	СведенияОТемахОбсуждений.ДатаПоследнегоСообщения КАК ДатаПоследнегоСообщения
		|ИЗ
		|	Справочник.ТемыОбсуждений КАК ТемыСообщений
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.СведенияОПрочтении КАК СведенияОПрочтении
		|		ПО ТемыСообщений.Ссылка = СведенияОПрочтении.Объект
		|			И (СведенияОПрочтении.Пользователь = &ТекущийПользователь)
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.СведенияОТемахОбсуждений КАК СведенияОТемахОбсуждений
		|		ПО ТемыСообщений.Ссылка = СведенияОТемахОбсуждений.Тема
		|ГДЕ
		|	ЕСТЬNULL(СведенияОПрочтении.Прочтен, ЛОЖЬ) = ЛОЖЬ
		|	И ТемыСообщений.ПометкаУдаления = ЛОЖЬ
		|
	...
	Выборка = Запрос.Выполнить().Выбрать();
	ПоследниеНовыеТемы = Новый Массив;
	Пока Выборка.Следующий() Цикл
		...
		ДанныеСтрокиСписка.Комментарий = РаботаСВиджетами.ПредставлениеДатыСписка(
			Выборка.ДатаПоследнегоСообщения);
		
		ПоследниеНовыеТемы.Добавить(ДанныеСтрокиСписка);
		
	КонецЦикла;
	
	Возврат ПоследниеНовыеТемы;
...
Функция ИзменитьРазделТем(МассивТем, НовыйРаздел) Экспорт
	...
	Для Каждого ТемаСсылка Из МассивТем Цикл
		Если Не ДокументооборотПраваДоступа.ПраваПользователяПоОбъекту(ТемаСсылка).Изменение Тогда 
...
//
Функция ПредставлениеСообщенияHTML(Сообщение, ФормироватьШапку) Экспорт
	...
	Если ФормироватьШапку Тогда
		...
		ТекстHTMLФотографии = РаботаСФотографиями.ТекстHTMLФотографии(РеквизитыСообщения.Автор);
	...
	РаботаС_HTML.ПрименитьНастройкиОтображение(
		ПредставлениеСообщенияHTML,
		ПолучитьПерсональнуюНастройку("ШрифтОбластиЧтения"));
...
Процедура ЗаполнитьДанныеКлючевыхОпераций(ДанныеКлючевыхОпераций) Экспорт
	...
	ДанныеКлючевойОперации.Имя = РаботаСОбсуждениямиКлиентСервер.КлючеваяОперацияОткрытиеФорума();
	ДанныеКлючевойОперации.Наименование = НСтр("ru = 'Форум: Открытие форума'", КодОсновногоЯзыка);
	ДанныеКлючевойОперации.ЦелевоеВремя = 2;
	
	ДанныеКлючевойОперации = ДанныеКлючевыхОпераций.Добавить();
	ДанныеКлючевойОперации.Имя = РаботаСОбсуждениямиКлиентСервер.КлючеваяОперацияОткрытиеТемы();
	ДанныеКлючевойОперации.Наименование = НСтр("ru = 'Форум: Открытие темы'", КодОсновногоЯзыка);
	ДанныеКлючевойОперации.ЦелевоеВремя = 1;
	
	ДанныеКлючевойОперации = ДанныеКлючевыхОпераций.Добавить();
	ДанныеКлючевойОперации.Имя = РаботаСОбсуждениямиКлиентСервер.КлючеваяОперацияОткрытиеНовыхОтветов();
	ДанныеКлючевойОперации.Наименование = НСтр("ru = 'Форум: Открытие новых ответов'", КодОсновногоЯзыка);
	ДанныеКлючевойОперации.ЦелевоеВремя = 1;
	
	ДанныеКлючевойОперации = ДанныеКлючевыхОпераций.Добавить();
	ДанныеКлючевойОперации.Имя = РаботаСОбсуждениямиКлиентСервер.КлючеваяОперацияОткрытиеСообщения();
	ДанныеКлючевойОперации.Наименование = НСтр("ru = 'Форум: Открытие сообщения'", КодОсновногоЯзыка);
	ДанныеКлючевойОперации.ЦелевоеВремя = 1;
	
	ДанныеКлючевойОперации = ДанныеКлючевыхОпераций.Добавить();
	ДанныеКлючевойОперации.Имя = РаботаСОбсуждениямиКлиентСервер.КлючеваяОперацияСозданиеСообщения();
	ДанныеКлючевойОперации.Наименование = НСтр("ru = 'Форум: Создание сообщения'", КодОсновногоЯзыка);
	ДанныеКлючевойОперации.ЦелевоеВремя = 1;
	
	ДанныеКлючевойОперации = ДанныеКлючевыхОпераций.Добавить();
	ДанныеКлючевойОперации.Имя = РаботаСОбсуждениямиКлиентСервер.КлючеваяОперацияЗаписьСообщения();
	ДанныеКлючевойОперации.Наименование = НСтр("ru = 'Форум: Запись сообщения'", КодОсновногоЯзыка);
	ДанныеКлючевойОперации.ЦелевоеВремя = 2;
	
	ДанныеКлючевойОперации = ДанныеКлючевыхОпераций.Добавить();
	ДанныеКлючевойОперации.Имя = РаботаСОбсуждениямиКлиентСервер.КлючеваяОперацияПросмотрСообщения();
	ДанныеКлючевойОперации.Наименование = НСтр("ru = 'Форум: Просмотр сообщения'", КодОсновногоЯзыка);
	ДанныеКлючевойОперации.ЦелевоеВремя = 1;
