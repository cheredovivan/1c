...
Процедура ПриСозданииНаСервере(Форма, Отказ, СтандартнаяОбработка) Экспорт
	...
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(
		...
		"Показатель",
		"ПодсказкаВвода",
		НСтр("ru = 'Введите слово для фильтра'"));
	
	ЭлементФормыПоказатель = Форма.Элементы.Найти("Показатель");
	Если ЭлементФормыПоказатель <> Неопределено Тогда
		
		ЭлементФормыПоказатель.РасширеннаяПодсказка.Заголовок = НСтр(
			"ru = 'Введите слово для фильтра (название товара, покупателя и пр.).
			 |Используйте:
			 |-, <> если надо исключить значение
			 |>, =, <, <= для фильтров по числам и датам'");
