...
#КонецЕсли
	...
	Если ПустаяСтрока(ИзвлеченныйТекст) Тогда
		...
		СписокРасширенийXlsx = "htm html";
		Если РасширениеФайлаВСписке(СписокРасширенийXlsx, РасширениеИмениФайла) Тогда
			Возврат ИзвлечьТекстИзФайлаHtml(ПолноеИмяФайла, Отказ);		
		КонецЕсли;	
...
// Извлечь текст из файла html и возвратить его в виде строки.
//
Функция ИзвлечьТекстИзФайлаHtml(ПутьКФайлу, Отказ)
...
#Если Не ВебКлиент  И Не МобильныйКлиент Тогда
	...
	ТекстHTML.Прочитать(ПутьКФайлу);
	...
	Если СтрНайти(ТекстHtmlНРег, "charset=utf-8") <> 0 
		...
		ТекстHTML = Новый ТекстовыйДокумент();
		ТекстHTML.Прочитать(ПутьКФайлу, "UTF-8");
		ТекстИзфайла = ТекстHTML.ПолучитьТекст();
		
	КонецЕсли;	
	
	ИзвлеченныйТекст = РаботаС_HTML.ПолучитьТекстИзHTML(ТекстИзфайла);
