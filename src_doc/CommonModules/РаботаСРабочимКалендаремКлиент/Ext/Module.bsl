...
// Открывает форму календаря для физического лица.
//
// Параметры:
//  ФизЛицо - СправочникСсылка.ФизическиеЛица.
//
Процедура ПоказатьКалендарьФизЛица(ФизЛицо) Экспорт
	...
	ПараметрыФормы = Новый Структура("ВариантРаботыФормы, ФизЛицо", "КалендарьСотрудника", ФизЛицо);
...
Процедура ОбработкаПередСозданием(Планировщик, Начало, Конец, Значения, Текст, СтандартнаяОбработка,
	...
	СоздатьЗаписьКалендаря(, Начало, Конец, ВесьДень, Текст, СотрудникФизЛица);
...
// Открывает форму занятости физ. лиц сотрудников.
//
// Параметры:
//  ОписаниеОповещения	 - ОписаниеОповещения	 - Описание оповещения, которое будет вып
//  Сотрудники           - Массив				 - Сотрудники, для которых будет открыта форма занятости.
...
//
Процедура ОткрытьФормуЗанятостиФизЛицСотрудников(ОписаниеОповещения,
	Сотрудники, ДатаНачала, ДатаОкончания, ИсключенияЗанятости) Экспорт
	
	НастройкаВыбораЗанятости =
		Новый Структура("Сотрудники, ДатаНачала, ДатаОкончания, ИсключенияЗанятости");
	НастройкаВыбораЗанятости.Сотрудники = Сотрудники;
	...
	ОткрытьФорму("Справочник.ЗаписиРабочегоКалендаря.Форма.ЗанятостьФизЛицСотрудников",
...
// Устанавливает цвет, которым отображается события в календаре физ. лица и посылает уведомление.
//
// Параметры:
...
Процедура УстановитьПометкиУдаления(Записи, ПовторяющиесяСобытия,
	...
	КлючеваяОперация = РаботаСРабочимКалендаремКлиентСервер.КлючеваяОперацияЗаписьЗаписиКалендаря();
	ОценкаПроизводительностиКлиент.ЗамерВремени(КлючеваяОперация);
...
Процедура ОткрытьФормуНастройкиДоступногоВремени() Экспорт
	...
	ОткрытьФорму("РегистрСведений.ДоступноеВремяФизическихЛиц.Форма.НастройкаДоступногоВремени");
	
КонецПроцедуры

// Проверяет, заняты ли сотрудники в указанное время.
//
// Параметры:
//  МассивСотрудников			 - Массив	 - Сотрудники, для которых выполняется проверка.
...
// 
// Возвращаемое значение:
//  Булево - сотрудники доступны.
//
Функция ПроверитьДоступностьФизЛицСотрудников(
	Знач МассивСотрудников,
	ДатаНачала,
	ДатаОкончания,
	...
	Если ТипЗнч(МассивСотрудников) = Тип("СправочникСсылка.Сотрудники") Тогда
		МассивСотрудников = ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(МассивСотрудников);
	ИначеЕсли ТипЗнч(МассивСотрудников) <> Тип("Массив") Тогда
		Возврат Истина;
	КонецЕсли;
	
	Если Не РаботаСРабочимКалендаремКлиентСервер.ЯвляетсяКорректнымПериодом(ДатаНачала, ДатаОкончания)
		Или МассивСотрудников.Количество() = 0 Тогда
		Возврат Истина;
	КонецЕсли;
	
	ДоступностьФизЛиц = РаботаСРабочимКалендаремСервер.ПроверитьДоступностьФизЛицСотрудников(
		МассивСотрудников, ДатаНачала, ДатаОкончания, ИсключенияЗанятости);
	
	Если Не ЗадаватьВопрос Тогда
		Возврат ДоступностьФизЛиц.ОбщаяЗанятость =
			ПредопределенноеЗначение("Перечисление.СостоянияЗанятости.Доступен");
	КонецЕсли;
	
	ПодписьЗанятыеФизЛица = ПолучитьПодписьЗанятостиФизЛиц(
		ДоступностьФизЛиц.ФизЛицаОтсутствуют,
		ДоступностьФизЛиц.ФизЛицаПодВопросом,
		ДоступностьФизЛиц.ФизЛицаЗанятые);
	
	Если ДоступностьФизЛиц.ОбщаяЗанятость =
		ПредопределенноеЗначение("Перечисление.СостоянияЗанятости.Отсутствует") Тогда
		
		ТекстВопроса = НСтр("ru = 'В выбранное время отсутствуют некоторые сотрудники. Продолжить?'")
			+ ПодписьЗанятыеФизЛица;
		ПоказатьВопрос(ОписаниеОповещения,
			ТекстВопроса, РежимДиалогаВопрос.ДаНет, , КодВозвратаДиалога.Нет,
			НСтр("ru = 'Проверка доступности'"));
		Возврат Ложь;
		
	ИначеЕсли ДоступностьФизЛиц.ОбщаяЗанятость =
...
Процедура СоздатьЗаписьКалендаряПланДня(ЭлементПланДня, НастройкиОтображения, ФизЛицо = Неопределено) Экспорт
	...
	СотрудникФизЛица = Неопределено;
	Если ЗначениеЗаполнено(ФизЛицо) Тогда
		СотрудникФизЛица = СотрудникиКлиентПовтИсп.ОсновнойСотрудникФизЛица(ФизЛицо);
	КонецЕсли;
...
Процедура ПоказатьЗаписьКалендаря(ЗаписьКалендаря, СотрудникЗаписиКалендаря = Неопределено) Экспорт
	...
	ИначеЕсли ТипЗнч(ЗаписьКалендаря) = Тип("СправочникСсылка.Мероприятия") Тогда
		УправлениеМероприятиямиКлиент.ОткрытьМероприятие(ЗаписьКалендаря);
	ИначеЕсли ТипЗнч(ЗаписьКалендаря) = Тип("ДокументСсылка.Отсутствие") Тогда
		ОтсутствияКлиент.ОткрытьОтсутствие(ЗаписьКалендаря);
	КонецЕсли;
...
//
Процедура ПринятьПриглашение(ЭлементыЗаписейКалендаря) Экспорт
	
	Если ЭлементыЗаписейКалендаря.Количество() = 0 Тогда
		ПоказатьПредупреждение(, НСтр("ru = 'Не удалось принять приглашение, т.к. не выбраны записи календаря.'"));
		Возврат;
	КонецЕсли;
	
	ИзмененныеЗаписиКалендаря =
		РаботаСРабочимКалендаремВызовСервера.ПринятьПриглашение(ЭлементыЗаписейКалендаря);
	Если ИзмененныеЗаписиКалендаря.Количество() > 0 Тогда
		Оповестить("Запись_ЗаписьКалендаря", ИзмененныеЗаписиКалендаря);
	КонецЕсли;
...
//                                                   См. РаботаСРабочимКалендаремКлиентСервер.НовыйЭлементЗаписиКалендаря().
//
Процедура ОтклонитьПриглашение(ЭлементыЗаписейКалендаря) Экспорт
	
	Если ЭлементыЗаписейКалендаря.Количество() = 0 Тогда
		ПоказатьПредупреждение(, НСтр("ru = 'Не удалось отклонить приглашение, т.к. не выбраны записи календаря.'"));
		Возврат;
	КонецЕсли;
	
	ИзмененныеЗаписиКалендаря =
		РаботаСРабочимКалендаремВызовСервера.ОтклонитьПриглашение(ЭлементыЗаписейКалендаря);
	Если ИзмененныеЗаписиКалендаря.Количество() > 0 Тогда
		Оповестить("Запись_ЗаписьКалендаря", ИзмененныеЗаписиКалендаря);
	КонецЕсли;
...
//  ФизЛицоКалендаря		 - СправочникСсылка.ФизическиеЛица - Физ. лицо, для календаря которого выполняется обработка.
//
Процедура ОбработкаНачалаСозданияЭлемента(Планировщик, ФизЛицоКалендаря)
	...
	СоздатьЗаписьКалендаря(
		...
		СотрудникиКлиентПовтИсп.ОсновнойСотрудникФизЛица(ДляФизЛица));
...
Процедура ОбработкаСозданиеЭлемента(Планировщик, ДанныеФормыПланировщик, НастройкиОтображения, ФизЛицоКалендаря)
	...
	СозданнаяЗаписьКалендаря.Вставить("Сотрудник", СотрудникиКлиентПовтИсп.ОсновнойСотрудникФизЛица(ДляФизЛица));
...
// Формирует подпись физ. лиц под вопросом и занятых физ. лиц.
//
// Параметры:
//  ФизЛицаОтсутствуют	 - Массив	 - Отсутствующие физ. лица.
//  ФизЛицаПодВопросом	 - Массив	 - физ. лица, занятость которых под вопросом.
//  ФизЛицаЗанятые		 - Массив	 - Занятые физ. лица.
// 
// Возвращаемое значение:
//  Строка - Текстовое представление занятых физ. лиц.
//
Функция ПолучитьПодписьЗанятостиФизЛиц(
	ФизЛицаОтсутствуют, ФизЛицаПодВопросом, ФизЛицаЗанятые)
	
	КоличествоФизЛицОтсутствуют = ФизЛицаОтсутствуют.Количество();
	КоличествоФизЛицПодВопросом = ФизЛицаПодВопросом.Количество();
	КоличествоФизЛицЗанятых = ФизЛицаЗанятые.Количество();
	ВсегоФизЛицПодписи =
		КоличествоФизЛицОтсутствуют
		+ КоличествоФизЛицПодВопросом
		+ КоличествоФизЛицЗанятых;
	Если ВсегоФизЛицПодписи = 0 Тогда
		Возврат "";
	КонецЕсли;
	
	ПодписьЗанятостиФизЛиц = Символы.ПС;
	
	Если КоличествоФизЛицПодВопросом <> 0 Тогда
		ПодписьФизЛицПодВопросом = СтрШаблон(НСтр("ru = 'Под вопросом: %1.'"),
			ПолучитьПодписьФизЛиц(ФизЛицаПодВопросом));
		ПодписьЗанятостиФизЛиц =
			ПодписьЗанятостиФизЛиц + Символы.ПС + ПодписьФизЛицПодВопросом;
	КонецЕсли;
	
	Если КоличествоФизЛицЗанятых <> 0 Тогда
		ПодписьФизЛицЗанятых = СтрШаблон(НСтр("ru = 'Заняты: %1.'"),
			ПолучитьПодписьФизЛиц(ФизЛицаЗанятые));
		ПодписьЗанятостиФизЛиц =
			ПодписьЗанятостиФизЛиц + Символы.ПС + ПодписьФизЛицЗанятых;
	КонецЕсли;
	
	Если КоличествоФизЛицОтсутствуют <> 0 Тогда
		ПодписьФизЛицОтсутствуют = СтрШаблон(НСтр("ru = 'Отсутствуют: %1.'"),
			ПолучитьПодписьФизЛиц(ФизЛицаОтсутствуют));
		ПодписьЗанятостиФизЛиц =
			ПодписьЗанятостиФизЛиц + Символы.ПС + ПодписьФизЛицОтсутствуют;
	КонецЕсли;
	
	Возврат ПодписьЗанятостиФизЛиц;
	
КонецФункции

// Формирует подпись физ. лиц, ограниченную по количеству физ. лиц.
//
// Параметры:
//  СотрудникиПодписи - Массив - Физ. лица, из которых формируется подпись.
//
// Возвращаемое значение:
//  Строка - Текстовое представление физ. лиц.
//
Функция ПолучитьПодписьФизЛиц(ФизЛицаПодписи)
	
	КоличествоФизЛиц = ФизЛицаПодписи.Количество();
	Если КоличествоФизЛиц = 0 Тогда
		Возврат "";
	КонецЕсли;
	
	МаксимальноеКоличествоОтображаемыхФизЛиц = 5;
	КоличествоОтображаемыхФизЛицПриПревышении = 3;
	
	Если КоличествоФизЛиц > МаксимальноеКоличествоОтображаемыхФизЛиц Тогда
		КоличествоОтображаемыхФизЛиц = КоличествоОтображаемыхФизЛицПриПревышении;
	Иначе
		КоличествоОтображаемыхФизЛиц = КоличествоФизЛиц;
	КонецЕсли;
	
	ПодписьФизЛиц = "";
	МассивФизЛиц  = Новый Массив;
	Для Индекс = 0 По КоличествоОтображаемыхФизЛиц - 1 Цикл
		ФизЛицо = ФизЛицаПодписи[Индекс];
		МассивФизЛиц.Добавить(ФизЛицо);
	КонецЦикла;
	
	Для Каждого ФизЛицо Из МассивФизЛиц Цикл
		ДобавитьЗначениеКСтрокеЧерезРазделитель(ПодписьФизЛиц, ", ", ФизЛицо);
	КонецЦикла;
	
	Если КоличествоФизЛиц > МаксимальноеКоличествоОтображаемыхФизЛиц Тогда
		
		НадписьФизЛиц = ПодписьКЧислу(
			КоличествоФизЛиц,
			НСтр("ru = 'сотрудники'"),
			НСтр("ru = 'сотрудника'"),
			НСтр("ru = 'сотрудников'"));
		
		ПодписьФизЛиц = ПодписьФизЛиц
			+ " "
			+ СтрШаблон(НСтр("ru = 'и другие (всего %1 %2)'"),
				Формат(КоличествоФизЛиц, "ЧГ=0"),
				НадписьФизЛиц);
		
	КонецЕсли;
	
	Возврат ПодписьФизЛиц;
...
Процедура ОткрытьФормуЗаписиКалендаря(Параметры, Уникальность = Неопределено)
	...
	Если ТипЗнч(Параметры) = Тип("Структура") И Параметры.Свойство("Ключ") И ЗначениеЗаполнено(Параметры.Ключ) Тогда
		КлючеваяОперация = РаботаСРабочимКалендаремКлиентСервер.КлючеваяОперацияОткрытиеЗаписиКалендаря();
	Иначе
		КлючеваяОперация = РаботаСРабочимКалендаремКлиентСервер.КлючеваяОперацияСозданиеЗаписиКалендаря();
	КонецЕсли;
	ОценкаПроизводительностиКлиент.ЗамерВремени(КлючеваяОперация);
