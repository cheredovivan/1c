...
#Если Не ВнешнееСоединение Тогда
	...
	Если ДоступнаРаботаСоСтраницамиPDFДокумента() Тогда
		...
		Попытка
			...
			ДокументPDF.Прочитать(ПотокДанныхФайла);
		Исключение 
			ПотокДанныхФайла.Закрыть();
			ПотокДанныхФайла = Неопределено;
			ИнформацияОбОшибке = ИнформацияОбОшибке();
			...
			ЗаписьЖурналаРегистрации(
				НСтр("ru = 'Расчет коэффициентов размерности штампа ЭП'", ОбщегоНазначения.КодОсновногоЯзыка()),
...
#Если Не ВнешнееСоединение Тогда
	...
	Попытка 
		...
		ОписаниеШтампаЭП = Новый ОписаниеОтображаемогоОбъектаPDF;
		ОписаниеШтампаЭП.Объект = ШтампЭП;
		ОписаниеШтампаЭП.Лево = СмещениеОтКрая;
		
		ДокументPDF.ДобавитьОтображаемыйОбъект(ОписаниеШтампаЭП);
...
Функция ПоследняяСтраницаДокументаPDF(ДокументPDF)
	...
	Если ДоступнаРаботаСоСтраницамиPDFДокумента() Тогда
		СтраницыДокументаPDF =
			ОбщегоНазначения.ВычислитьВБезопасномРежиме("Параметры.ДокументPDF.Страницы", Новый Структура("ДокументPDF", ДокументPDF));
...
Функция ДоступнаРаботаСоСтраницамиPDFДокумента()
	...
	СистемнаяИнформация = Новый СистемнаяИнформация();
	ВерсияПлатформы = СистемнаяИнформация.ВерсияПриложения;
	Возврат ОбщегоНазначенияКлиентСервер.СравнитьВерсии(ВерсияПлатформы, "8.3.26.1318") >= 0;
