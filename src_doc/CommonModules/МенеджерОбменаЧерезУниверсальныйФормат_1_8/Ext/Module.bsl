////////////////////////////////////////////////////////////////////////////////
// Менеджер обмена через универсальный формат (Документооборот, редакция КОРП 3.0, ED 1.8 от 02.10.2025 10:52:31)
//
////////////////////////////////////////////////////////////////////////////////
...
Функция Подключаемый_ИдентификаторМодуля() Экспорт
	Возврат "09e2690c-f7b3-4574-8c48-41e4e0d382fe";
КонецФункции
#Область ПОД
		...
		ИначеЕсли НаборДопРеквизитов = Неопределено Тогда
			...
			Если НаборДопРеквизитов = Неопределено Тогда
				ИмяПредопределенногоЭлемента = СтрЗаменить(
					СтрЗаменить(
						СтрокаДопРеквизиты.ТипОбъекта, ".", "_"),
...
Функция ЯвляетсяДокументомУдостоверяющимЛичностьИзБазы(ФизическоеЛицо, ВидДокумента, Период)
	...
	// Поэтому сохраняем тот вид документа, который указан в базе на период загрузки.
	...
	Если Выборка.Следующий() И ВидДокумента = Выборка.ВидДокумента Тогда
		Возврат Истина;
	// Локализация
