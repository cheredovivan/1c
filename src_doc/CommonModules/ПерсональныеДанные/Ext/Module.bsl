...
Функция ИзменилсяСписокПерсональныхДанных(Объект) Экспорт
	...
	Запрос.Текст = 
		...
		|ИЗ
		|	&Таблица КАК СубъектыПерсональныхДанных
		|ГДЕ
		|	СубъектыПерсональныхДанных.Ссылка = &Ссылка";
	Запрос.Текст = СтрЗаменить(Запрос.Текст, "&Таблица", 
		СтрШаблон("%1.СубъектыПерсональныхДанных", Объект.Метаданные().ПолноеИмя()));
...
Процедура ЗаполнитьСписокСубъектовПерсональныхДанныхФайла(ФайлОбъект) Экспорт
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьУчетДоступаКПерсональнымДанным") Тогда
		...
		Если ТипЗнч(ФайлОбъект.ВладелецФайла) = Тип("СправочникСсылка.ДокументыПредприятия") Тогда
			
			ФайлОбъект.СубъектыПерсональныхДанных.Очистить();
			
			СубъектыПерсональныхДанных = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(
				ФайлОбъект.ВладелецФайла, "СубъектыПерсональныхДанных");
			СубъектыПерсональныхДанных = СубъектыПерсональныхДанных.Выгрузить();	
