...
функция ТекстЗапросаИзмененийНСИПользователиИРоли();
...
Функция ТекстЗапросаИзмененийПисем()
	...
	Возврат
		...
		|	ЭлектроннаяПочта.ПометкаУдаления
		|	ИЛИ ПапкиПисемДляСинхронизации.Папка ЕСТЬ NULL,
		...
		|	ЭлектроннаяПочта.Ссылка ССЫЛКА Документ.ВходящееПисьмо,
		|	ВТ_ЭлектроннаяПочта.ОтметкаВремени,
...
Функция ТекстЗапросаИзмененийЗадач()
	...
	Возврат
		...
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.ОтметкиВремениСсылочныхОбъектов КАК ОтметкиВремениСсылочныхДанных
		...
		|	ЗадачиПоИсполнителю.ПометкаУдаления
		|	ИЛИ ЗадачиПоИсполнителю.Выполнена
		|	ИЛИ ЗадачиПоИсполнителю.ИсключенаИзПроцесса,
		...
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.ОтметкиВремениСсылочныхОбъектов КАК ОтметкиВремениСсылочныхДанных
		|		ПО СведенияОбИсполнителяхЗадач.Задача = ОтметкиВремениСсылочныхДанных.Ключ
		|ГДЕ
		|	ОтметкиВремениСсылочныхДанных.Отметка > &ОтметкаВремениСборкиДанных
		|	И СведенияОбИсполнителяхЗадач.Участник В (&СотрудникиПользователя)";
...
функция ТекстЗапросаИзмененийПроцессовИсполнения()
	...
	Возврат
		...
		|	ОтметкиВремениСсылочныхДанных.Отметка,
		|	Исполнение.Завершен
		|	ИЛИ Исполнение.ПометкаУдаления";
...
Функция ТекстЗапросаИзмененийМоегоСамочувствия()
	...
	Возврат
		...
		|ГДЕ
		|	ОтметкиВремениСсылочныхДанных.Отметка >= &ОтметкаВремениСборкиДанных
		|	И СотрудникиПользователей.Пользователь = &Пользователь
...
функция ТекстЗапросаИзмененийМероприятий()
	...
	Возврат
		...
		|ПОМЕСТИТЬ ПользовательИЕгоСотрудники
		|ИЗ
		|	РегистрСведений.СотрудникиПользователей КАК СотрудникиПользователей
		|ГДЕ
		...
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ПользовательИЕгоСотрудники КАК ПользовательИЕгоСотрудники
		|		ПО (УчастникиМероприятия.Исполнитель = ПользовательИЕгоСотрудники.Сотрудник)
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.КешИнформацииОбОбъектах КАК КешИнформацииОбОбъектах
		...
		|ГДЕ
		|	СправочникМероприятия.ДатаОкончания >= &ДатаНачалаВыгрузки
		|	И ОтметкиВремениСсылочныхДанных.Отметка >= &ОтметкаВремениСборкиДанных
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		...
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.УчастникиМероприятия КАК УчастникиМероприятия
		|		ПО МП_ИзмененныеСостоянияПриглашенийНаМероприятия.Мероприятие = УчастникиМероприятия.Мероприятие
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ПользовательИЕгоСотрудники КАК ПользовательИЕгоСотрудники
		|		ПО (УчастникиМероприятия.Исполнитель = ПользовательИЕгоСотрудники.Сотрудник)
		|ГДЕ
...
Функция ИзмененияТолькоЗаголовки(ПараметрыСинхронизации) Экспорт
	...
	Запрос.Текст = ТекстЗапросаИзмененийНСИПользователиИРоли();
...
Функция ИзмененияВсеОстальное(ПараметрыСинхронизации, СтруктураРезультатовЗапросаЗаголовки) Экспорт
	...
	ОтметкаПериодаХраненияДанных = ОтметкиВремени.ДатаВОтметкуВремени(ГраницаУстаревания);
	...
	Запрос.УстановитьПараметр("СотрудникиПользователя",
		Сотрудники.ПользовательИЕгоСотрудники(ПараметрыСинхронизации.ТекущийПользовател));
	Запрос.УстановитьПараметр("ФизическоеЛицо", ПараметрыСеанса.ФизическоеЛицоПользователя);	
...
Функция ЗаполнитьВариантыВыполненияЗадачиПоТочкеМаршрута(Задача, ТочкаМаршрута)
	...
	ИначеЕсли ТочкаМаршрута = БизнесПроцессы.Исполнение.ТочкиМаршрута.Контролировать Тогда
...
Процедура ИзмененияВсеОстальноеПоТаблицамЗаголовковЗадач(ПараметрыСинхронизации, 
	...
	ТипПроцессИсполнения = МП_СлужебныйПовтИсп.ТипПроцессИсполнения();

	ИнициализироватьТаблицуПоТипу(СтруктураРезультатовЗапросов, ТипПроцессИсполнения);
	
	Для каждого СтрокаИзменений Из ТаблицаКВыгрузке Цикл
		
		СтрокаИзменений.ОписаниеКраткое =
			МП_Почта.ПолучитьКраткийТекст(СтрокаИзменений.Описание, 500);
		
		СтрокаИзменений.ОписаниеПолное = МП_Служебный.ПолучитьHTMLПредставлениеФормыЗадачи(СтрокаИзменений.Ссылка);
		...
		РеквизитыБП = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(СтрокаИзменений.БизнесПроцесс,
			"Ссылка, Наименование, Автор, Важность, ДатаНачала, ДатаЗавершения, Состояние");
		
		ЗаполнитьЗначенияСвойств(СтруктураРезультатовЗапросов[ТипПроцессИсполнения].Добавить(), РеквизитыБП);
...
функция ИзмененияНастроекВложения(СтруктураРезультатовЗапроса, ПараметрыСинхронизации)
	...
	Запрос.Текст = 
		...
		|		Перечисление.МП_ТипыНастроекПользователей.МаксимальныйРазмерВложения)";
	...
	Запрос.Текст = 
		...
		|	И ИзмененныеНастройкиСинхронизацииСМобильнымКлиентом.Настройка =
		|		ЗНАЧЕНИЕ(Перечисление.МП_ТипыНастроекПользователей.ФорматыПередаваемыхФайлов)
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|ВЫБРАТЬ
		...
		|	И ИзмененныеНастройкиСинхронизацииСМобильнымКлиентом.ОтметкаВремени > &Отметка
		|	И ИзмененныеНастройкиСинхронизацииСМобильнымКлиентом.Настройка =
		|	ЗНАЧЕНИЕ(Перечисление.МП_ТипыНастроекПользователей.ОграничениеФорматовПередаваемыхФайлов)";
...
Функция ИзмененияНастроекСинхронизации(СтруктураРезультатовЗапроса, ПараметрыСинхронизации)
	...
	Настройки.Добавить(Перечисления.МП_ТипыНастроекПользователей.СинхронизацияСамочувствий);
	Настройки.Добавить(Перечисления.МП_ТипыНастроекПользователей.СинхронизацияОтсутствий);
	Настройки.Добавить(Перечисления.МП_ТипыНастроекПользователей.СинхронизацияМероприятий);
	Настройки.Добавить(Перечисления.МП_ТипыНастроекПользователей.СинхронизацияУчетаВремени);
	Настройки.Добавить(Перечисления.МП_ТипыНастроекПользователей.ПроектПоУмолчанию);
	Настройки.Добавить(Перечисления.МП_ТипыНастроекПользователей.СпособУказанияВремени);
	Настройки.Добавить(Перечисления.МП_ТипыНастроекПользователей.ВидРаботПоУмолчанию);
	
	Запрос = Новый Запрос;
	Запрос.Текст = 
		"ВЫБРАТЬ
		|	МП_ИзмененныеНастройкиСинхронизации.Настройка КАК Настройка
		|ИЗ
		|	РегистрСведений.МП_ИзмененныеНастройкиСинхронизации КАК МП_ИзмененныеНастройкиСинхронизации
		|ГДЕ
		|	МП_ИзмененныеНастройкиСинхронизации.Пользователь = &Пользователь
		|	И МП_ИзмененныеНастройкиСинхронизации.Настройка В(&Настройки)
		|	И МП_ИзмененныеНастройкиСинхронизации.ОтметкаВремени > &Отметка";
	...
	Для каждого СтрокаНастройки Из НастройкиТаблица Цикл
		...
		Если СтрокаНастройки.Настройка = Перечисления.МП_ТипыНастроекПользователей.ПроектПоУмолчанию Тогда
			МП_Изменения.ИнициализироватьТаблицуПоТипу(
				СтруктураРезультатовЗапроса, ТипНастройкиСинхронизации);
		...
		Если СтрокаНастройки.Настройка = Перечисления.МП_ТипыНастроекПользователей.СпособУказанияВремени Тогда
			МП_Изменения.ИнициализироватьТаблицуПоТипу(
				СтруктураРезультатовЗапроса, ТипНастройкиСинхронизации);
			СтрокаНастроек = СтруктураРезультатовЗапроса[ТипНастройкиСинхронизации].Добавить();
		...
		Если СтрокаНастройки.Настройка = Перечисления.МП_ТипыНастроекПользователей.ВидРаботПоУмолчанию Тогда
			МП_Изменения.ИнициализироватьТаблицуПоТипу(
				СтруктураРезультатовЗапроса, ТипНастройкиСинхронизации);
...
Процедура ДобавитьПредметыКВыгрузке(СтруктураРезультатовЗапросов, ПараметрыСинхронизации, ПредметыТаблица) Экспорт
	...
	Для каждого СтрокаПредмета Из ПредметыТаблица Цикл
		...
		ПредставлениеДокумент = МП_Служебный.ПолучитьСодержаниеПредмета(Предмет);
...
Функция ДанныеФайлаПоОграничению(Файл, ВладелецФайла, ПараметрыСинхронизации, ТекущаяВерсия) Экспорт
	...
	Иначе
		...
		Исключение
			...
			РегистрыСведений.МП_ПротоколРаботыПользователей.ДобавитьПредупреждение(ТекстПредупреждения, 
