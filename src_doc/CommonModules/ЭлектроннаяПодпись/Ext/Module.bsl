...
Функция ШтампВизуализацииЭлектроннойПодписи(Знач Подпись, Знач ДатаПодписи = Неопределено, 
	...
	Результат = ШтампыВизуализацииЭлектронныхПодписей(ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(Подпись),,
	...
	Если ТипЗнч(ОписаниеШтампов) = Тип("Массив") Тогда
		...
		Для Каждого Штамп Из ОписаниеШтампов Цикл
			...
			Если ОбластьНайдена Тогда
				...
				ОбластьШтампа.СоздатьФорматСтрок();
...
Процедура ДобавитьСтрокиПодписи(ОбъектДанных, СвойстваПодписей, СообщениеЖурналаРегистрации)
	...
	Для Каждого СвойстваПодписи Из МассивСвойствПодписи Цикл
		...
		ЭлектроннаяПодписьДокументооборотСобытия.ПриДобавленииСтрокиПодписи(
			СвойстваПодписи, НоваяЗапись.ИдентификаторПодписи);
