...
// Для совместимости с библиотеками
...
Функция ЧтениеРазрешено(ОписаниеДанных, Пользователь = Неопределено) Экспорт
	...
	СтруктураПрав = ДокументооборотПраваДоступа.ПраваПользователяПоОбъекту(ОписаниеДанных, Пользователь);
	
	Возврат СтруктураПрав.Чтение;
...
// Добавлено для корректной работы БЭД
...
Функция НовоеОписаниеПрофиляГруппДоступа() Экспорт
	...
	НовоеОписание.Вставить("Родитель",		  ""); // поставляемых данных к предопределенному элементу.
...
//                        сервиса
//  Немедленно - Булево - Если Истина, то обновление без постановки в очередь.
//
Процедура ОбновитьРолиПользователей(
			Знач Пользователи1 = Неопределено,
	...
	Если Немедленно <> Истина
		...
		Если ТипЗнч(Пользователи1) <> Тип("Массив") Тогда
			Пользователи1 = ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(Пользователи1);
		КонецЕсли;
		Модуль = РегистрыСведений.ОчередьОбновленияПравДоступа;
		Для Каждого Эл Из Пользователи1 Цикл
			Модуль.Добавить(Модуль.ПриоритетОперативная(), Эл, , "ОбновитьРолиПользователей");
		КонецЦикла;
		Возврат;
	КонецЕсли;
