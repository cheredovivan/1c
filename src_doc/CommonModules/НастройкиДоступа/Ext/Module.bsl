...
// 
// Параметры:
//  ВладелецНастройки - ОпределяемыйТип.КонтейнерыСотрудников - элемент, который наследует настройки.
//
...
Функция ВладельцыНаследуемыхРазрешенийДоступа(ВладелецНастройки) Экспорт
	...
	Если ТипЭлемента = Тип("СправочникСсылка.Сотрудники") Тогда
		Результат = РегистрыСведений.СотрудникиВКонтейнерах.КонтейнерыСотрудников(ВладелецНастройки);
	...
	ИначеЕсли ТипЭлемента = Тип("СправочникСсылка.Пользователи") Тогда
		Результат = Сотрудники.СотрудникиПользователя(ВладелецНастройки);
		КонтейнерыСотрудников = РегистрыСведений.СотрудникиВКонтейнерах.КонтейнерыСотрудников(Результат);
		ОбщегоНазначенияКлиентСервер.ДополнитьМассив(Результат, КонтейнерыСотрудников);
