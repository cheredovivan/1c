...
Процедура ПередНачаломРаботыСистемы(Параметры) Экспорт
	...
	// глобальные переменные ставим	
	СотрудникиТекущегоПользователя = ПараметрыРаботыКлиента.СотрудникиТекущегоПользователя;
	РолиТекущегоПользователя = ПараметрыРаботыКлиента.РолиТекущегоПользователя;
	ОсновнойСотрудник = ПараметрыРаботыКлиента.ОсновнойСотрудник;
...
Процедура ПриНачалеРаботыСистемы(Параметры) Экспорт
	...
	ОбщегоНазначенияКлиентПереопределяемыйЛокализация.ПриНачалеРаботыСистемы(Параметры);
	
	ИнтеграцияПодсистемБПОКлиент.ПриНачалеРаботыСистемы();

#Если Не МобильныйКлиент Тогда
	ОбщегоНазначенияДокументооборотВызовСервера.ПриНачалеРаботыСистемы();
#Иначе
	МК_Клиент.ПриНачалеРаботыСистемы();
#КонецЕсли
	...
	Если ПараметрыРаботыКлиентаПриЗапуске.ОбновитьЗадачи Тогда
		ОткрытьФорму("Обработка.ОбновлениеЗадачПриПереходеНаНовуюВерсию.Форма");
	КонецЕсли;
...
Процедура ПриОбработкеПараметровЗапуска(ПараметрыЗапуска, Отказ) Экспорт
	...
	Для Каждого ПараметрЗапускаПрограммы Из ПараметрыЗапуска Цикл
		
		Если СтрНайти(ПараметрЗапускаПрограммы, "ClearChoiceHistory") <> 0 Тогда
			
			ИсторияВыбораВызовСервера.ОчиститьИсториюВыбораПользователя(, Ложь); 
			ЗавершитьРаботуСистемы(Ложь, Истина, "/C ");
...
Процедура ПослеНачалаРаботыСистемы() Экспорт
	...
	ОбщегоНазначенияКлиентПереопределяемыйЛокализация.ПослеНачалаРаботыСистемы();
	
	// Выдать сообщение, если есть некорректные роли:
	ПараметрыРаботыКлиента = СтандартныеПодсистемыКлиентПовтИсп.ПараметрыРаботыКлиентаПриЗапуске();
	Если ПараметрыРаботыКлиента.Свойство("ТекстНекорректныеРолиСотрудников")
		И ЗначениеЗаполнено(ПараметрыРаботыКлиента.ТекстНекорректныеРолиСотрудников) Тогда
		ПоказатьПредупреждение( , ПараметрыРаботыКлиента.ТекстНекорректныеРолиСотрудников);
	КонецЕсли;
