...
//
Процедура СкорректироватьДатуОтсчета(ДатаОтсчета, ДопПараметры) Экспорт
	
	СтандартнаяОбработка = Истина;
	СрокиИсполненияПроцессовПереопределяемый.ПриКорректировкеДатыОтсчета(
		ДатаОтсчета, ДопПараметры, СтандартнаяОбработка);
	Если Не СтандартнаяОбработка Тогда
		Возврат;
	КонецЕсли;
...
Процедура ЗаполнитьСрокИсполненияЗадачи(Задача,
	...
	Если Задача.СрокИсполнения <> СрокЗадачиПроцесса Тогда
...
// Добавляет срок помощника в данные для расчета сроков.
//
// Параметры:
//  Сроки - ТаблицаЗначений - см. функцию ПустаяТаблицаСроков.
//  Предшественники - ТаблицаЗначений - см. функцию ПустаяТаблицаПредшественников.
...
//  ИмяИдентификатораСрока - Строка - описание (роль) срока в процессе.
//
Процедура ДобавитьСрокПомощникаВДанныеДляРасчета(
	Сроки, Предшественники,
	Процесс,
	СрокУчастникаИзменен,
	ПредшественникиСледующегоЭтапа,
	...
	СтруктураПоиска.Вставить("Помощник", Неопределено);
	ЗаполнитьЗначенияСвойств(СтруктураПоиска, Процесс);
	
	ПараметрыСрокаУчастника.Вставить("Участник", СтруктураПоиска.Помощник);
	ПараметрыСрокаУчастника.Вставить("СрокИсполнения", Процесс.СрокОбработкиРезультатов);
	ПараметрыСрокаУчастника.Вставить("СрокИсполненияДни", Процесс.СрокОбработкиРезультатовДни);
	ПараметрыСрокаУчастника.Вставить("СрокИсполненияЧасы", Процесс.СрокОбработкиРезультатовЧасы);
	ПараметрыСрокаУчастника.Вставить("СрокИсполненияМинуты", Процесс.СрокОбработкиРезультатовМинуты);
...
// Добавляет срок обрабатывающего результат в данные для расчета сроков.
//
// Параметры:
//  Сроки - ТаблицаЗначений - см. функцию ПустаяТаблицаСроков.
//  Предшественники - ТаблицаЗначений - см. функцию ПустаяТаблицаПредшественников.
...
//  ИмяИдентификатораСрока - Строка - описание (роль) срока в процессе.
//
Процедура ДобавитьСрокОбрабатывающегоРезультатВДанныеДляРасчета(
	Сроки, Предшественники,
	Процесс,
	СрокУчастникаИзменен,
	ПредшественникиСледующегоЭтапа,
	...
	ПараметрыСрокаУчастника = Новый Структура;
	ПараметрыСрокаУчастника.Вставить("Участник", Процесс.ОбрабатывающийРезультат);
	ПараметрыСрокаУчастника.Вставить("СрокИсполнения", Процесс.СрокОбработкиРезультатов);
	ПараметрыСрокаУчастника.Вставить("СрокИсполненияДни", Процесс.СрокОбработкиРезультатовДни);
	ПараметрыСрокаУчастника.Вставить("СрокИсполненияЧасы", Процесс.СрокОбработкиРезультатовЧасы);
	ПараметрыСрокаУчастника.Вставить("СрокИсполненияМинуты", Процесс.СрокОбработкиРезультатовМинуты);
...
Функция ИзмененныеСрокиПроцесса(Процесс, Параметры) Экспорт
	...
	Иначе
		...
		ИначеЕсли РеквизитТаблицаСИзмененнымСроком = "Автор"
			Или РеквизитТаблицаСИзмененнымСроком = "Проверяющий"
			Или РеквизитТаблицаСИзмененнымСроком = "Помощник"
...
//                           в тбалице Сроки
//   * ИдентификаторыСроков - Соответствие Из КлючИЗначение - идентификаторов сроков и их расположения в процессе, по которому
...
Процедура РассчитатьСрокИсполненияДляНовогоПроцесса(ПроцессОбъект) Экспорт
	...
	ПроцессОбъект.ДополнительныеСвойства.Вставить("СрокНовогоПроцессаРассчитан", Истина);
	...
	ИначеЕсли ТипПроцесса = Тип("БизнесПроцессСсылка.Подписание") Тогда
		...
		РассчитатьСрокиПодписания(ПроцессОбъект, Параметры);
...
Процедура ЗаполнитьСрокиСоставногоПроцесса(
	...
	Если ТипЗаполняемогоОбъекта = Тип("Структура") Тогда
...
Функция СтруктураДляРасчетаСрокаРассмотрения(СсылкаНаПроцесс = Неопределено) Экспорт
	...
	Если ТипЗнч(СсылкаНаПроцесс) = Тип("БизнесПроцессСсылка.Рассмотрение") Тогда
		РеквизитыДляПолученияДанных = РеквизитыДляРасчета;
	Иначе
		РеквизитыДляПолученияДанных = СтрЗаменить(РеквизитыДляРасчета, "ОбрабатывающийРезолюцию,", "");
	КонецЕсли;
	
	Если ЗначениеЗаполнено(СсылкаНаПроцесс) Тогда
		ЗаполнитьЗначенияСвойств(
			СтруктураДляРасчета,
			ОбщегоНазначения.ЗначенияРеквизитовОбъекта(СсылкаНаПроцесс, РеквизитыДляПолученияДанных));
	КонецЕсли;
...
Функция ДанныеДляРасчетаСроковРассмотрения(Процесс, Параметры) Экспорт
	...
	ИзмененныеСрокиПроцесса = ИзмененныеСрокиПроцесса(Процесс, Параметры);
	СрокИсполнителяИзменен = ИзмененныеСрокиПроцесса.СрокИсполнителяИзменен;
	СрокПомощникаИзменен = ИзмененныеСрокиПроцесса.СрокПомощникаИзменен;
	...
	ДобавитьСрокПомощникаВДанныеДляРасчета(
		Сроки, Предшественники,
		Процесс,
		СрокПомощникаИзменен,
		ПредшественникиСледующегоЭтапа,
		ИдентификаторыСроков,
		"СрокПомощника");
...
Процедура ЗаполнитьСрокиРассмотрения(Процесс, Сроки, ИдентификаторыСроков) Экспорт
	...
	Для Каждого СтрокаСрок Из Сроки Цикл
		...
		ИначеЕсли РольСрокаВПроцесса = "СрокИсполнителя" Тогда
			...
			Иначе
				Процесс.СрокИсполнения = СтрокаСрок.СрокИсполнения;
			КонецЕсли;
		ИначеЕсли РольСрокаВПроцесса = "СрокПомощника" Тогда
			Если Процесс.ВариантУстановкиСрокаОбработкиРезультатов
				= Перечисления.ВариантыУстановкиСрокаИсполнения.ТочныйСрок Тогда
	...
	УчитыватьСрокОбработкиРезультата = ?(ТипЗнч(Процесс) = Тип("БизнесПроцессОбъект.Рассмотрение"),
		ЗначениеЗаполнено(Процесс.ОбрабатывающийРезолюцию), Истина);
	Если Не СрокиУчастниковПроцессаКорректны(Процесс, УчитыватьСрокОбработкиРезультата) Тогда
		СрокИсполненияПроцессаВсемиУчастниками = Дата(1,1,1);
	КонецЕсли;
...
Функция ДанныеДляРасчетаСроковРегистрации(Процесс, Параметры) Экспорт
	...
	Пока ТекущаяИтерация <= КоличествоИтераций Цикл
		...
		ДобавитьСрокОбрабатывающегоРезультатВДанныеДляРасчета(
			Сроки, Предшественники,
			Процесс,
...
Функция СтруктураДляРасчетаСрокаСогласования(СсылкаНаПроцесс = Неопределено) Экспорт
	...
	РеквизитыДляРасчета = "Ссылка, КоличествоИтераций, ОбрабатывающийРезультат,
			|ВариантУстановкиСрокаОбработкиРезультатов, СрокОбработкиРезультатов,
			|СрокОбработкиРезультатовДни, СрокОбработкиРезультатовЧасы,
...
// Возвращает структура для расчета срока процесса/шаблона Подписание по дереву участников в карточке.
//
// Параметры:
//  Процесс - ДанныеФормыСтруктура - объект процесса/шаблона процесса в карточке
//  УчастникиПроцесса - ДанныеФормыДерево - дерево участников в карточке.
//
// Возвращаемое значение:
//  Структура - см. СтруктураДляРасчетаСрокаПодписания
//
Функция СтруктураДляРасчетаСрокаПодписанияПоДаннымПроцессаВКарточке(Процесс, УчастникиПроцесса) Экспорт
	
	СтруктураДляРасчета = СрокиИсполненияПроцессов.СтруктураДляРасчетаСрокаПодписания();
	ЗаполнитьЗначенияСвойств(СтруктураДляРасчета, Процесс,, "Этапы, Участники");
	
	СтруктураДляРасчета.Этапы = Процесс.Этапы.Выгрузить().СкопироватьКолонки ();
	СтруктураДляРасчета.Участники = Процесс.Участники.Выгрузить().СкопироватьКолонки();
	
	РаботаСБизнесПроцессамиКлиентСервер.ЗаполнитьПроцессПодписанияПоДеревуУчастников(
		СтруктураДляРасчета, УчастникиПроцесса);
		
	Возврат СтруктураДляРасчета;
	
КонецФункции

// Заполняет сроки подписания в карточке данных процесса.
// 
// Параметры:
//  Процесс - ДанныеФормыСтруктура - объект процесса/шаблона процесса в карточке
//  УчастникиПроцесса - ДанныеФормыДерево - дерево участников в карточке.
// 	СтруктураРасчета - Структура - см. СтруктураДляРасчетаСрокаПодписания
//
Процедура ЗаполнитьСрокиПодписанияВКарточкеПроцессаПоСтруктуреРасчета(
	Процесс, УчастникиПроцесса, СтруктураРасчета) Экспорт
		
	Процесс.СрокИсполненияПроцесса = СтруктураРасчета.СрокИсполненияПроцесса; 
		
	СтрокиУчастников = Новый Соответствие();
	Для Каждого СтрокаУчастника Из СтруктураРасчета.Участники Цикл
		СтрокиУчастников[СтрокаУчастника.Идентификатор] = СтрокаУчастника;
	КонецЦикла;
	
	ВариантТочныйСрок = Перечисления.ВариантыУстановкиСрокаИсполнения.ТочныйСрок;
	...
	Пока СтрокиДляОбработки.Количество() > 0 Цикл
		...
		СтрокаУчастникаСНовымСроком = СтрокиУчастников[СтрокаДляОбработки.ИдентификаторУчастника];
		
		Если СтрокаУчастникаСНовымСроком <> Неопределено Тогда
			
			Если СтрокаДляОбработки.ВариантУстановкиСрокаИсполнения = ВариантТочныйСрок Тогда
				ЗаполнитьЗначенияСвойств(СтрокаДляОбработки, СтрокаУчастникаСНовымСроком,
...
// Возвращает структура для расчета срока процесса/шаблона Подписание.
//
// Параметры:
//  СсылкаНаПроцесс - БизнесПроцессСсылка.Подписание,
//                    СправочникСсылка.ШаблоныПодписания - ссылка на процесс/шаблон. Если ссылка
//                    заполнена, то возвращает структура, заполненная по данным процесса/шаблона.
//
// Возвращаемое значение:
//  Структура - реквизиты и табличные части процесса/шаблона.
...
//   * СрокИсполненияПроцесса
//
Функция СтруктураДляРасчетаСрокаПодписания(СсылкаНаПроцесс = Неопределено) Экспорт
	...
	Если ЗначениеЗаполнено(СсылкаНаПроцесс) Тогда
		СтруктураДляРасчета = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(СсылкаНаПроцесс, РеквизитыДляРасчета);
		СтруктураДляРасчета.Этапы = СтруктураДляРасчета.Этапы.Выгрузить();
		СтруктураДляРасчета.Участники = СтруктураДляРасчета.Участники.Выгрузить();
	Иначе
		СтруктураДляРасчета = Новый Структура(РеквизитыДляРасчета);
	КонецЕсли;
...
//            СправочникОбъект.ШаблоныПодписания - шаблон/процесс.
//            Структура - см. СтруктураДляРасчетаСрокаПодписания
//  Параметры - Структура - см. ПараметрыДляРасчетаСроков
//
// Возвращаемое значение:
//  Структура - см. ДанныеДляРасчетаСроков
//
Функция ДанныеДляРасчетаСроковПодписания(Процесс, Параметры) Экспорт
	...
	// Формируем последовательность этапов.
	ПоследовательностьЭтапов = Новый Массив();
	ЭтапыИПорядокВыполнения = Новый Соответствие();
	Для Каждого СтрокаЭтапа Из Процесс.Этапы Цикл
		ЭтапыИПорядокВыполнения[СтрокаЭтапа.Идентификатор] = СтрокаЭтапа.ПорядокВыполненияУчастниками;
		ПоследовательностьЭтапов.Добавить(СтрокаЭтапа.Идентификатор);
	КонецЦикла;
	// Если этапов нет, то добавляем фиктивный один этап для подписывающих.
	Если Процесс.Этапы.Количество() = 0 Тогда
		ИДФиктивногоЭтапа = УникальныйИдентификаторПустой();
		Если Процесс.СпособПодписания = 
			Перечисления.СпособыПодписанияПредметаПроцесса.НаБумаге Тогда
		
			ЭтапыИПорядокВыполнения[ИДФиктивногоЭтапа] = 
				Перечисления.ПорядокВыполненияЭтапаПроцесса.Последовательно;
		Иначе
			ЭтапыИПорядокВыполнения[ИДФиктивногоЭтапа] = 
				Перечисления.ПорядокВыполненияЭтапаПроцесса.Параллельно;
		КонецЕсли;
		ПоследовательностьЭтапов.Добавить(ИДФиктивногоЭтапа);
	КонецЕсли;
	// Добавляем фиктивный этап для обрабатывающего результат.
	ИдентификаторЭтапаОбработки = Новый УникальныйИдентификатор();
	ЭтапыИПорядокВыполнения[ИдентификаторЭтапаОбработки] = 
		Перечисления.ПорядокВыполненияЭтапаПроцесса.Последовательно;
	ПоследовательностьЭтапов.Добавить(ИдентификаторЭтапаОбработки);
		
	// Распределяем участников по этапам. 
	ЭтапыИУчастники = Новый Соответствие();
	Для Каждого СтрокаУчастника Из Процесс.Участники Цикл
		
		ИдентификаторЭтапа = СтрокаУчастника.Этап;
		Если СтрокаУчастника.ТочкаМаршрута = ТочкиМаршрута.ОбработатьРезультат Тогда
			ИдентификаторЭтапа = ИдентификаторЭтапаОбработки;
		КонецЕсли;
			
		Если ЭтапыИУчастники[ИдентификаторЭтапа] = Неопределено Тогда
			ЭтапыИУчастники[ИдентификаторЭтапа] = Новый Массив();
		КонецЕсли; 
		ЭтапыИУчастники[ИдентификаторЭтапа].Добавить(СтрокаУчастника);
	...
	Пока ТекущаяИтерация <= КоличествоИтераций Цикл
		...
		Для Каждого ИдентификаторЭтапа Из ПоследовательностьЭтапов Цикл
			...
			УчастникиЭтапа = ЭтапыИУчастники[ИдентификаторЭтапа];
			Если УчастникиЭтапа = Неопределено Тогда
				Продолжить;
			КонецЕсли;
			
			ПредшественникиТекущегоЭтапа = ОбщегоНазначения.СкопироватьРекурсивно(
				ПредшественникиСледующегоЭтапа);
			ПредшественникиСледующегоЭтапа.Очистить();
			
			ПорядокВыполнения = ЭтапыИПорядокВыполнения[ИдентификаторЭтапа];
			...
			Для Каждого СтрокаУчастника Из УчастникиЭтапа Цикл
				...
				Если ЭтоПерваяИтерация Тогда
					ИдентификаторыСроков[IDТекущегоСрокаИсполнения] = СтрокаУчастника.Идентификатор;
				КонецЕсли;
				
				СтрокаСрок = Сроки.Добавить();
				СтрокаСрок.IDСрока = IDТекущегоСрокаИсполнения;
				
				СтрокаСрок.ГрафикРаботы = 
					СрокиИсполненияПроцессовКОРП.ГрафикРаботыУчастникаПроцесса(СтрокаУчастника.Участник);
				
				СтрокаСрок.СрокИсполнения = СтрокаУчастника.СрокИсполнения;
				СтрокаСрок.СрокИсполненияДни = СтрокаУчастника.СрокИсполненияДни;
				СтрокаСрок.СрокИсполненияЧасы = СтрокаУчастника.СрокИсполненияЧасы;
				СтрокаСрок.СрокИсполненияМинуты = СтрокаУчастника.СрокИсполненияМинуты;
				...
				Если ПорядокВыполнения = Перечисления.ПорядокВыполненияЭтапаПроцесса.Параллельно Тогда
					ПредшественникиСледующегоЭтапа.Добавить(IDТекущегоСрокаИсполнения);
				Иначе
					ПредшественникиТекущегоЭтапа.Очистить();
					ПредшественникиТекущегоЭтапа.Добавить(IDТекущегоСрокаИсполнения);
					ПредшественникиСледующегоЭтапа.Очистить();
					ПредшественникиСледующегоЭтапа.Добавить(IDТекущегоСрокаИсполнения);
				КонецЕсли;
...
//  Параметры - Структура - см. ПараметрыДляРасчетаСроков
//
Процедура РассчитатьСрокиПодписания(Процесс, Параметры) Экспорт
	...
	// Подготовим данные для расчета сроков участников процесса.
	ДанныеДляРасчета = ДанныеДляРасчетаСроковПодписания(Процесс, Параметры);
...
//                                          предыдущего.
//
Процедура ЗаполнитьСрокиПодписания(ПроцессОбъект, Сроки, ИдентификаторыСроков,
	ЗаполнятьТолькоПриПревышении = Ложь) Экспорт
	...
	Для Каждого СтрокаСрок Из Сроки Цикл
		...
		Если РольСрокаВПроцесса <> Неопределено Тогда
			СтрокаУчастника = СтрокиУчастников[РольСрокаВПроцесса];
			Если СтрокаУчастника.ВариантУстановкиСрокаИсполнения =
				ВариантыУстановкиСрокаИсполнения.ТочныйСрок Тогда
				
				СтрокаУчастника.СрокИсполненияДни = СтрокаСрок.СрокИсполненияДни;
				СтрокаУчастника.СрокИсполненияМинуты = СтрокаСрок.СрокИсполненияЧасы;
				СтрокаУчастника.СрокИсполненияЧасы = СтрокаСрок.СрокИсполненияМинуты;
			Иначе
				СтрокаУчастника.СрокИсполнения = СтрокаСрок.СрокИсполнения;
			КонецЕсли;
		КонецЕсли;
...
Функция ДлительностьИсполненияПроцесса(Процесс, Знач Смещение = Неопределено) Экспорт
	...
	ИначеЕсли ТипПроцесса = Тип("СправочникСсылка.ШаблоныПодписания") Тогда
		
		Длительность = ДлительностьИсполненияПодписания(Процесс);
...
Функция ДлительностьИсполненияРегистрации(Процесс) Экспорт
	...
	Если ОбщегоНазначения.ЭтоСсылка(ТипПроцесса) Тогда
		СтрокаРеквизитовПроцессов = 
			"Исполнитель, СрокИсполненияДни, СрокИсполненияЧасы, СрокИсполненияМинуты,
			|ОбрабатывающийРезультат, СрокОбработкиРезультатовДни, СрокОбработкиРезультатовЧасы, СрокОбработкиРезультатовМинуты,
			|КоличествоИтераций";
		РеквизитыПроцесса = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(
			Процесс, СтрокаРеквизитовПроцессов);
	Иначе
...
// Возвращает структура для вычисления длительности процесса/шаблона Подписания по данным в карточке.
//
// Параметры:
//  Процесс - ДанныеФормыСтруктура - объект процесса/шаблона процесса в карточке
//  УчастникиПроцесса - ДанныеФормыДерево - дерево участников в карточке.
...
// Возвращаемое значение:
//  Структура
//   * Ссылка - БизнесПроцессСсылка.Подписание, СправочникСсылка.ШаблоныПодписания
//   * СпособПодписания - ПеречислениеСсылка.СпособыПодписанияПредметаПроцесса
//   * Этапы - ТаблицаЗначений - таблица с колонками как в таб. части Процесс.Этапы
//   * Участники - ТаблицаЗначений - таблица с колонками как в таб. части Процесс.Участники
//   * КоличествоИтераций - Число  
//
Функция СтруктураДляВычисленияДлительностиПодписанияПоДаннымВКарточке(Процесс, УчастникиПроцесса) Экспорт
	
	СтруктураДляВычисленияДлительности = Новый Структура(
		"Ссылка,
		|СпособПодписания,
		|Этапы,
		|Участники,
	...
	ЗаполнитьЗначенияСвойств(СтруктураДляВычисленияДлительности, Процесс,, "Этапы, Участники");
	
	СтруктураДляВычисленияДлительности.Этапы = Процесс.Этапы.Выгрузить().СкопироватьКолонки ();
	СтруктураДляВычисленияДлительности.Участники = Процесс.Участники.Выгрузить().СкопироватьКолонки();
		
	РаботаСБизнесПроцессамиКлиентСервер.ЗаполнитьПроцессПодписанияПоДеревуУчастников(
		СтруктураДляВычисленияДлительности, УчастникиПроцесса);
		
	Возврат СтруктураДляВычисленияДлительности;
	
КонецФункции

// Возвращает длительность процесса/шаблона подписания в минутах.
// Используется функцией ДлительностьИсполненияПроцесса.
//
// Параметры:
//  Процесс - СправочникСсыка,
...
//  Число - длительность процесса/шаблона в минутах.
//
Функция ДлительностьИсполненияПодписания(Процесс) Экспорт
	...
	// Формируем последовательность этапов.
	ПоследовательностьЭтапов = Новый Массив();
	ЭтапыИПодокВыполнения = Новый Соответствие();
	Для Каждого СтрокаЭтапа Из Процесс.Этапы Цикл
		ЭтапыИПодокВыполнения[СтрокаЭтапа.Идентификатор] = СтрокаЭтапа.ПорядокВыполненияУчастниками;
		ПоследовательностьЭтапов.Добавить(СтрокаЭтапа.Идентификатор);
	КонецЦикла;
	// Если этапов нет, то добавляем фиктивный один этап для подписывающих.
	Если Процесс.Этапы.Количество() = 0 Тогда
		...
		Если Процесс.СпособПодписания = 
			...
			ЭтапыИПодокВыполнения[ИДФиктивногоЭтапа] = 
				Перечисления.ПорядокВыполненияЭтапаПроцесса.Последовательно;
		Иначе
			ЭтапыИПодокВыполнения[ИДФиктивногоЭтапа] = 
				Перечисления.ПорядокВыполненияЭтапаПроцесса.Параллельно;
	...
	// Добавляем фиктивный этап для обрабатывающего результат.
	ИдентификаторЭтапаОбработки = Новый УникальныйИдентификатор();
	ЭтапыИПодокВыполнения[ИдентификаторЭтапаОбработки] = 
		Перечисления.ПорядокВыполненияЭтапаПроцесса.Последовательно;
		ПоследовательностьЭтапов.Добавить(ИдентификаторЭтапаОбработки);
	...
	Для Каждого ИдентификаторЭтапа Из ПоследовательностьЭтапов Цикл
		...
		ПорядокВыполнения = ЭтапыИПодокВыполнения[ИдентификаторЭтапа];
			
		ДлительностьЭтапа = 0; 
				
		Для Каждого СтрокаУчастника Из УчастникиЭтапа Цикл
			
			ДлительностьДляУчастника = ДлительностьИсполненияУчастникаПроцессаВМинутах(
			...
			Если ПорядокВыполнения = Перечисления.ПорядокВыполненияЭтапаПроцесса.Параллельно Тогда
				ДлительностьЭтапа = Макс(ДлительностьЭтапа, ДлительностьДляУчастника);
			Иначе
				ДлительностьЭтапа = ДлительностьЭтапа + ДлительностьДляУчастника;
			КонецЕсли;
			
		КонецЦикла;
		
		ДлительностьПроцесса = ДлительностьПроцесса + ДлительностьЭтапа;
		
	КонецЦикла;
	
	ДлительностьПроцесса = ДлительностьПроцесса * Процесс.КоличествоИтераций;
...
Функция ДлительностьИсполненияПроцессаУчастникамиВМинутах(
	...
	Если СрокиИсполненияПроцессовКлиентСервер.ЭтоСтрокаОтветственного(ТаблицаИсполнители[0]) Тогда
		ТекущийИндекс = 1;
...
// Возвращает соответствие ключей действий и их сроков из таблицы
// действий.
//
// Параметры:
...
Процедура ЗаполнитьДатыЗавершенияДействий(Процесс, ДатыЗавершенияДействий) Экспорт
	...
	ТипыДействийИРеквизитыПроцесса.Вставить(Тип("БизнесПроцессСсылка.Подписание"), "ШаблонПодписания");
...
// Обновляет признак истекших сроков исполнения в дереве участников
//
// Параметры:
//  РеквизитУчастники - ДанныеФормыДерево - дерево участников.
//  ДатаИсполненияПоУмолчанию - Дата - дата исполнения по умолчанию.
//  ДатыИсполненияУчастников - ДанныеФормыКоллекция - таблица дата исполнения процесса участниками.
//
Процедура ОбновитьПризнакИстекшихСроковВДеревеУчастников(
	РеквизитУчастники, ДатаИсполненияПоУмолчанию, ДатыИсполненияУчастников = Неопределено) Экспорт
	
	СтрокиДляОбработки = Новый Массив;
	Для Каждого СтрокаУчастника Из РеквизитУчастники.ПолучитьЭлементы() Цикл
		СтрокиДляОбработки.Добавить(СтрокаУчастника);		
	КонецЦикла;
	Пока СтрокиДляОбработки.Количество() > 0 Цикл
		СтрокаУчастника = СтрокиДляОбработки[0];
		
		Если Не ЗначениеЗаполнено(СтрокаУчастника.СрокИсполнения) Тогда
			СтрокаУчастника.СрокИсполненияИстек = Ложь;
		Иначе
			
			ДатаИсполнения = Дата(1,1,1);
			Если ДатыИсполненияУчастников <> Неопределено Тогда
				ДатаИсполнения = ДатаИсполненияУчастника(
					СтрокаУчастника.Идентификатор, ДатыИсполненияУчастников);
			КонецЕсли;
			Если ЗначениеЗаполнено(ДатаИсполнения) Тогда
				ДатаИсполнения = Мин(ДатаИсполненияПоУмолчанию, ДатаИсполнения);
			Иначе
			...
			Если СтрокаУчастника.СрокИсполнения < ДатаИсполнения Тогда
				СтрокаУчастника.СрокИсполненияИстек = Истина;
			КонецЕсли;
...
Функция ДанныеДляРасчетаСрокаДействияСоставногоПроцесса(Действие)
	...
	ИначеЕсли ТипДействия = Тип("СправочникСсылка.ШаблоныПодписания")
		...
		СтруктураДляРасчета = СтруктураДляРасчетаСрокаПодписания(Действие);
		ДанныеДляРасчета = ДанныеДляРасчетаСроковПодписания(СтруктураДляРасчета, Параметры);
...
// Проверяет, что срок у нового процесса не заполнен и его можно рассчитать.
// 
// Параметры:
//  Источник - БизнесПроцессОбъект - 
// 
// Возвращаемое значение:
// Булево - Истина, если для процесса нужно рассчитать срок
//  
Функция НужноРассчитатьСрокНовогоПроцесса(Источник) Экспорт
	
	Если  ТипЗнч(Источник) = Тип("БизнесПроцессОбъект.РешениеВопросовВыполненияЗадач") Тогда
		Возврат Ложь;
	КонецЕсли;
	
	Если ЗначениеЗаполнено(Источник.СрокИсполненияПроцесса) Тогда
		Возврат Ложь;
	КонецЕсли;
	
	Если Источник.ДополнительныеСвойства.Свойство("СрокНовогоПроцессаРассчитан") Тогда
		Возврат Ложь;
	КонецЕсли;
	
	Возврат ЗначениеЗаполнено(Источник.ВедущаяЗадача)
		Или ОбщегоНазначенияДокументооборот.ЗначениеРеквизитаОбъектаВПривилегированномРежиме(Источник.Шаблон,
			"ШаблонВКомплексномПроцессе") = Истина;
