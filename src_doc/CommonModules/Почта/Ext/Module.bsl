...
Функция ИнтернетПочтаУстановитьСоединение(
	...
	Исключение
		...
		СообщениеОбОшибке = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()) 
			+ " " + Символы.ПС + " " + Строка(УчетнаяЗапись);
...
Функция ПолучитьСтруктуруПочтовогоСообщенияИзПисьма(Письмо) Экспорт
	...
	Для каждого ИнтернетПочтовыйАдрес Из Письмо.АдресаУведомленияОПрочтении Цикл
		Если ИнтернетПочтовыйАдрес.Адрес <> "@" Тогда
		...
		Исключение
			...
			Для Каждого Строка Из УчетнаяЗапись.ОтветственныеЗаОбработкуПисем Цикл
				...
				РаботаСУведомлениями.ОбработатьУведомлениеПрограммы(
					СообщениеОбОшибкеКопии, Пользователь);
...
Процедура ИнтернетПочтовоеСообщениеДобавитьТекст(ИнтернетПочтовоеСообщение, СтруктураТекста) Экспорт
	...
	Если СтруктураТекста.Свойство("ПроизвольныйТипТекста") И ЗначениеЗаполнено(СтруктураТекста.ПроизвольныйТипТекста) Тогда
		ИнтернетТекстПочтовогоСообщения.ПроизвольныйТипТекста = СтруктураТекста.ПроизвольныйТипТекста;
	КонецЕсли;
...
Функция ПолучитьТипТекстаПочтовогоСообщенияДляИнтернетПочтовогоСообщения(ТипТекста) Экспорт
	...
	ИначеЕсли ТипТекста = Перечисления.ТипыТекстовПочтовыхСообщений.ПроизвольныйТекст Тогда
		Возврат ТипТекстаПочтовогоСообщения.ПроизвольныйТекст;
	Иначе
...
Функция ПолучитьСтруктуруТекстаИзСтруктурыПочтовогоСообщения(Сообщение) Экспорт
	...
	Если Результат.ЗаписиКалендаря.Количество() > 0 Тогда
		
		Для каждого ЗаписьКалендаря Из Результат.ЗаписиКалендаря Цикл
			
			ЗаписьКалендаряЕстьВоВложениях = Ложь;
			
			Для каждого Вложение Из Сообщение.Вложения Цикл
				
				Если ТипЗнч(Вложение) = Тип("Структура")
					И Вложение.Свойство("ТипСодержимого") И НРег(Вложение.ТипСодержимого) = "application/ics"
					И ЗаписьКалендаря.Данные = Вложение.Данные Тогда
					
					ЗаписьКалендаряЕстьВоВложениях = Истина;
					Прервать;
			...
			Если Не ЗаписьКалендаряЕстьВоВложениях Тогда
				Сообщение.Вложения.Добавить(ЗаписьКалендаря);
			КонецЕсли;
...
Функция ПолучитьСтруктуруТекстаИзСтруктурыТекстыПочтовогоСообщения(Тексты) Экспорт
	...
	Результат.Вставить("Кодировка", "");
	Результат.Вставить("ЗаписиКалендаря", Новый Массив());
	...
	Для каждого ТекстПочтовогоСообщения Из Тексты Цикл
		...
		ИначеЕсли ТипТекста = Перечисления.ТипыТекстовПочтовыхСообщений.ПростойТекст Тогда
			...
			Если Найти(Текст, "BEGIN:VCALENDAR") = 1 Тогда
				ТекстVCALENDAR = Текст;
				
				Попытка
					ИмяВременногоФайла = ПолучитьИмяВременногоФайла(".ics");
					ЗаписьТекста = Новый ЗаписьТекста(ИмяВременногоФайла, "CESU-8",, Истина, Символы.ПС);
					ЗаписьТекста.Записать(ТекстVCALENDAR + Символы.ВК + Символы.ПС);
					ЗаписьТекста.Закрыть();
					ДвоичныеДанныеЗаписиКалендаря = Новый ДвоичныеДанные(ИмяВременногоФайла);
					УдалитьФайлы(ИмяВременногоФайла);
					
					ЗаписьКалендаря = Новый Структура;
					ЗаписьКалендаря.Вставить("Данные", ДвоичныеДанныеЗаписиКалендаря);
					ЗаписьКалендаря.Вставить("Идентификатор", "");
					ЗаписьКалендаря.Вставить("Имя", "Приглашение.ics");
					ЗаписьКалендаря.Вставить("ИмяФайла", "Приглашение.ics");
					ЗаписьКалендаря.Вставить("Кодировка", "");
					ЗаписьКалендаря.Вставить("СпособКодирования", "MIME");
					ЗаписьКалендаря.Вставить("ТипСодержимого", "application/ics");
					Результат.ЗаписиКалендаря.Добавить(ЗаписьКалендаря);
				Исключение
					// не бросаем исклчюение
				КонецПопытки;
				
				Если ЗначениеЗаполнено(ТекстПростой) Тогда
					Продолжить;
				КонецЕсли;
