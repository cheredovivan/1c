...
// Отмечает обработку уведомления.
// Данные будут переданы на сервер при следующем вызове общего обработчика ожидания.
//
// Параметры:
...
//
Процедура ОтметитьОбработкуУведомления(ВидСобытия, ОбъектУведомления) Экспорт
	
	ОбработанноеУведомление = Новый Структура("ВидСобытия, ОбъектУведомления, ДатаОбработки");
	ОбработанноеУведомление.ВидСобытия = ВидСобытия;
	ОбработанноеУведомление.ОбъектУведомления = ОбъектУведомления;
	ОбработанноеУведомление.ДатаОбработки = ОбщегоНазначенияКлиент.ДатаСеанса();
	
	ПараметрыПриложения["Уведомления.ОбработанныеУведомления"].Добавить(ОбработанноеУведомление);
...
//
Процедура ПодключитьУведомленияЧерезВнешнююСистему(ИдентификаторОбсужденияСВ, ТипВнешнейСистемы) Экспорт
	
	РаботаСУведомлениямиВызовСервера.ПодключитьУведомленияЧерезВнешнююСистему(
