...
Процедура ЗаписатьИОбработатьВходящиеСообщения(МобильноеПриложение, СтруктурыВходящихСообщений,
	...
	Сообщения = МП_Служебный.ВходящиеНеобработанныеСообщения(МобильноеПриложение);
			...
			Для каждого ОбъектКОбработке Из ОбъектыКОбработке Цикл
				...
				Исключение
					...
					Если СтрНайти(НРег(Инфо.Описание), "info:") > 0 Тогда
						...
						РегистрыСведений.МП_ПротоколРаботыПользователей.ДобавитьИнформацию(
...
Процедура ПодключитьНовоеУстройствоВерсия1(ЗначенияСвойств, МобильноеПриложение, 
	...
	РегистрыСведений.МП_НастройкиПользователей.ЗаписатьНастройку(
		ТекущийПользователь,
		Перечисления.МП_ТипыНастроекПользователей.ПериодПервичнойЗагрузки, 
...
Процедура ЗаписатьИнформациюОКлиентеВерсия1(ЗначенияСвойств, МобильноеПриложение);
	...
	РегистрыСведений.МП_СведенияОПользователях.ЗаписатьСведенияОКлиенте(
...
Процедура ЗаписатьУдалениеОбъекта(ЗначенияСвойств, МобильноеПриложение)
	...
	Попытка
		...
		ПротоколированиеРаботыСотрудников.ЗаписатьИзменение(Ссылка);
...
Процедура ЗаписатьЗаписьРабочегоКалендаряВерсия1(ЗначенияСвойств, МобильноеПриложение)
	...
	НовоеСостояние = МП_СлужебныйПовтИсп.ПолучитьСостояниеЗаписиПоПредставлению(ЗначенияСвойств[8]);

	Цвет = МП_СлужебныйПовтИсп.ПолучитьЦветПоПредставлению(ЗначенияСвойств[7]);
	...
	Если ЭтоСвязаннаяЗапись Тогда
		...
		Если ТипЗнч(Объект.Предмет) = Тип("СправочникСсылка.Мероприятия") Тогда
			...
			РегистрыСведений.УчастникиМероприятия.ИзменитьСостояниеПриглашения(
				...
				МобильноеПриложение.Пользователь),
				НовоеСостояние);
	...
	Объект.ТипЗаписиКалендаря = 
		МП_СлужебныйПовтИсп.ПолучитьТипаЗаписиКалендаряПоПредставлению(ТипЗаписи);
	...
	Если СтрСравнить(ТипЗаписи, "recurrence") = 0 Тогда
		...
		Объект.ЧастотаПовторения = МП_СлужебныйПовтИсп.ПолучитьЧастотуПоПредставлению(
			ЗначенияСвойств[20]);

		Объект.ПравилоОкончанияПовторения = 
			МП_СлужебныйПовтИсп.ПолучитьПравилоОкончанияПовторенияПоПредставлению(
				ЗначенияСвойств[21]);
...
Процедура ЗаписатьПроцессИсполнениеВерсия1(ЗначенияСвойств, МобильноеПриложение)
	...
	Если СтрСравнить(Важность, "low") = 0 Тогда
		Процесс.Важность = Перечисления.ВариантыВажностиОбъектов.Низкая;
	ИначеЕсли СтрСравнить(Важность, "high") = 0 Тогда
		Процесс.Важность = Перечисления.ВариантыВажностиОбъектов.Высокая;
	Иначе
		Процесс.Важность = Перечисления.ВариантыВажностиОбъектов.Обычная;
	КонецЕсли;
	...
	Если Не ОбъектНайденНаСервере Тогда
		ПротоколированиеРаботыСотрудников.ЗаписатьСоздание(Процесс.Ссылка, Истина);
	Иначе
		ПротоколированиеРаботыСотрудников.ЗаписатьИзменение(Процесс.Ссылка);
	КонецЕсли;
...
Процедура ЗаписатьФайлВерсия1(ЗначенияСвойств)
	...
	ПротоколированиеРаботыСотрудников.ЗаписатьИзменение(ФайлСсылка);
...
Процедура ЗаписатьИсходящееПисьмоВерсия1(ЗначенияСвойств, МобильноеПриложение, ПисьмаКОтправке, 
	...
	Письмо.Автор = Сотрудники.ОсновнойСотрудникПользователя(
		ТекущийПользователь);
	...
	Если Не ОбъектНайденНаСервере Тогда
		ПротоколированиеРаботыСотрудников.ЗаписатьСоздание(Письмо.Ссылка, Истина);
	Иначе
		ПротоколированиеРаботыСотрудников.ЗаписатьИзменение(Письмо.Ссылка);
	КонецЕсли;
...
Процедура ЗаписатьНастройкиСинхронизацииПапок(ЗначенияСвойств, МобильныйКлиент)
	...
	Если Папки.Количество() = 0 Тогда
		РегистрыСведений.МП_НастройкиПользователей.ЗаписатьНастройку(
			ТекущийПользователь, Перечисления.МП_ТипыНастроекПользователей.СинхронизацияПочты, Ложь);
		Возврат;
	КонецЕсли;
...
Процедура ЗаписатьПеренаправлениеЗадачиВерсия1(ЗначенияСвойств, МобильноеПриложение)
	...
	РезультатЗапроса = МП_Служебный.ПолучитьРеквизитыЗадачиСПроверкойДоступности(ЗадачаСсылка);
	...
	Попытка
		МП_Служебный.ПроверитьВозможностьПереадресацииЗадачи(ЗадачаСсылка, НовыйИсполнитель);
	Исключение
		ВызватьИсключение;
	КонецПопытки;
	...
	Попытка
		...
		ПроверятьПраваВПривилегированномРежиме = Истина;
		БизнесПроцессыИЗадачиСервер.ПеренаправитьЗадачу(
			ЗадачаСсылка,
			СтруктураПеренаправления,
	...
	ПротоколированиеРаботыСотрудников.ЗаписатьПеренаправлениеЗадачи(ЗадачаСсылка);
...
Процедура ЗаписатьСобытиеПротоколаВерсия1(ЗначенияСвойств, МобильноеПриложение)
	...
	РегистрыСведений.МП_ПротоколРаботыПользователей.ДобавитьЗаписьВПротоколСКлиента(
...
Процедура ЗаписатьРезультатВыполненияЗадачиВерсия1(ЗначенияСвойств, МобильноеПриложение)
	...
	Если ЗадачаОбъект = Неопределено Тогда
		...
		РегистрыСведений.МП_ПротоколРаботыПользователей.ДобавитьОшибкуПоОбъекту(
	...
	// Выполняем провеверку, что задача доступна пользователю по ролям и делегированию.
	РезультатЗапроса = МП_Служебный.ПолучитьРеквизитыЗадачиСПроверкойДоступности(Задача);
	...
	ИначеЕсли ЗадачаОбъект.ТочкаМаршрута = БизнесПроцессы.Рассмотрение.ТочкиМаршрута.Рассмотреть Тогда
		...
		Если РезультатВыполнения = 1 Тогда
			...
			ДействиеЗадачи = ДействияВызовСервера.ДействиеИРеквизитыЗадачи(Задача).Действие;
			...
			ДанныеЗаполнения.Вставить("ВариантРассмотрения", ВариантРассмотрения);
			
			ОбрабатывающийРезолюцию = БизнесПроцессы.Рассмотрение.ОбрабатывающийРезолюцию(
				РеквизитыЗадачи.Источник,
				ДействиеЗадачи);
					
			ДанныеЗаполнения.Вставить("ОбрабатывающийРезолюцию", ОбрабатывающийРезолюцию);
...
Процедура ЗаписатьУстановкуСостоянияПрочтенностиВерсия1(ЗначенияСвойств)
	...
	Если ЗначениеЗаполнено(ОбъектСсылка) Тогда
		...
		ПротоколированиеРаботыСотрудников.ЗаписатьИзменение(ОбъектСсылка);
...
Процедура ЗаписатьФлагОбъектаВерсия1(ЗначенияСвойств)
	...
	РегистрыСведений.ФлагиОбъектов.УстановитьФлаг(ОбъектСсылка,
		Сотрудники.ОсновнойСотрудник(), Флаг);
...
Процедура ЗаписатьМоеОтсутствиеВерсия1(ЗначенияСвойств, МобильноеПриложение)
	...
	Для Каждого ЭлементЗаместитель Из Заместители Цикл
		...
		НоваяСтрокаЗаместители.Заместитель = Сотрудники.ОсновнойСотрудникПользователя(
			Справочники.Пользователи.ПолучитьСсылку(Новый УникальныйИдентификатор(ЭлементЗаместитель[0])));
...
Процедура ЗаписатьМоеСамочувствиеВерсия1(ЗначенияСвойств, МобильноеПриложение)
	...
	СамочувствиеОбъект.Создал = Сотрудники.ОсновнойСотрудник();
...
Процедура ЗаписатьТрудозатратуВерсия1(ЗначенияСвойств, МобильноеПриложение)
	...
	ЕжедневныйОтчет = ЕжОтчет("ЕжедневныйОтчет", День, ТекущийПользователь, ЗначенияСвойств[2]);
	
	НаборЗаписей = РегистрыСведений.ФактическиеТрудозатраты.СоздатьНаборЗаписей();
	НаборЗаписей.Отбор.Сотрудник.Установить(ТекущийПользователь);
	...
	Иначе
		...
		ЗаписьТрудозатрата.Сотрудник = ТекущийПользователь;
	...
	Если ДниНеделиМассивом.Количество() > 0 Тогда
		...
		Для Каждого ДеньНеделиЭлемент Из ДниНеделиМассивом Цикл
			...
			Если ЗначениеЗаполнено(IDЕжедневногоОтчета) Тогда
				НоваяСтрока.ЕжедневныйОтчет = ЕжОтчет("ЕжедневныйОтчет", НоваяСтрока.Дата, ТекущийПользователь,
					IDЕжедневногоОтчета);
			КонецЕсли;
...
Процедура ЗафиксироватьПисьмаКОтправке(МобильноеПриложение, МассивПисем) Экспорт
	...
	Если МассивПисем.Количество() > 0 Тогда
		МП_Почта.ОтправитьПисьмаПолученныеСМобильногоКлиента(МобильноеПриложение, МассивПисем);
	КонецЕсли;
	
КонецПроцедуры

Функция ЕжОтчет(ИмяТипа, Период, Пользователь, ID = Неопределено)
	...
	Запрос.Текст = 
		...
		|	И ЕжОтчет.Сотрудник В(&Сотрудники)";
	Запрос.Текст = СтрЗаменить(Запрос.Текст, "&ЕжОтчет", СтрШаблон("Документ.%1", ИмяТипа));
	Запрос.УстановитьПараметр("День", Период);
	Запрос.УстановитьПараметр("Сотрудники", Сотрудники.СотрудникиПользователя(Пользователь));
	РезультатЗапроса = Запрос.Выполнить();
