...
Функция ИнициализироватьПеременныеОбработчика()
	...
	НапоминанияПользователяДокументооборотКлиент.ИнициализироватьПеременныеОбщегоОбработчикаОжидания();
...
Функция ОбработчикиКЗапуску() Экспорт
	...
	НапоминанияПользователяДокументооборотКлиент.ОбщиеОбработчикиОжиданияКЗапуску(ОбработчикиКЗапуску);
...
Процедура ВыполнитьДействияОбработчикаОжидания(ПараметрыВыполнения) Экспорт
	...
	Если ПараметрыВыполнения.РезультатыПроверкиПочты <> Неопределено Тогда
		...
		Если РезультатыПроверкиПочты.НужноОбновитьСписок Тогда
			...
			Если РезультатыПроверкиПочты.ЧислоНовыхПисем <> 0 Тогда
				...
				СправочникСсылка =
					ПредопределенноеЗначение("Справочник.СлужебныйДляОткрытияСпискаПисем.Умолчательный");
	...
	НапоминанияПользователяДокументооборотКлиент.ВыполнитьДействиеОбщегоОбработчикаОжидания(ПараметрыВыполнения);
