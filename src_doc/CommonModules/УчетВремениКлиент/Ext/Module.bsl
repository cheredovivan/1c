...
// Создаёт ежедневный отчёт на указанную дату.
//
// Параметры:
//  ДатаОтчета - Дата.
//
Функция СоздатьЕжедневныйОтчет(ДатаОтчета) Экспорт
	...
	ЗначенияЗаполнения = Новый Структура("Дата", ДатаОтчета);
	ПараметрыФормы = Новый Структура("ЗначенияЗаполнения", ЗначенияЗаполнения);
	
	ОткрытьФорму(
		"Документ.ЕжедневныйОтчет.ФормаОбъекта",
		ПараметрыФормы,,
		ДатаОтчета);
...
Процедура ДобавитьВОтчетКлиент(
	...
	Иначе
		...
		ПараметрыОписания.Вставить("Объект", Объект);
...
Процедура ЗавершениеФормыДобавленияРаботы(ПараметрыОтчета, Параметры) Экспорт
	...
	Если Параметры.ТипДействия = "ДобавитьВОтчетКлиент" Тогда
		Если ПараметрыОтчета <> Неопределено Тогда
			...
			Оповестить("Запись_ФактическиеТрудозатраты",, Параметры.Объект);
		КонецЕсли;
		Если ПараметрыОповещения <> Неопределено Тогда
			ПоказатьОповещение(
				ПараметрыОповещения, 
