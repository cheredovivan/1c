...
//
// Параметры:
//	ЭтоПервоеВзаимодействиеСБотом - Булево - Истина, если пользователь еще не взаимодействовал с чат-ботом
// ВозвращаемоеЗначение:
//	ОбсуждениеСистемыВзаимодействия - идентификатор обсуждения между текущим пользователем и чат-ботом
//	Или Неопределено, если не задан пользователь ЧатБот
//
Функция ОбсуждениеСЧатБотом(ЭтоПервоеВзаимодействиеСБотом) Экспорт
	
	ЧатБот = РаботаСЧатБотомПовтИсп.ЧатБот();
	Если Не ЗначениеЗаполнено(ЧатБот) Или ЧатБот = Пользователи.ТекущийПользователь() Тогда
	
		Возврат Неопределено;
	
	КонецЕсли; 
	
	Если ЭтоПервоеВзаимодействиеСБотом Тогда
...
Функция СледующееСостояние(
	...
	РаботаСЧатБотом.ОтменитьАктивныеФоновыеЗаданияПоиска();
...
//  ЗаписьБеседы - Структура - см. РаботаСЧатБотомКлиентСервер.НоваяЗаписьБеседы.
//  СообщениеПользователю - строка - текст сообщения.
//
