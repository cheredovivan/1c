...
// Формирует представление навигационной ссылки, с помощью метода ПолучитьПредставленияНавигационныхСсылок.
// В случае, если представление получить не удалось, будет возвращена сама навигационная ссылка.
//
// Параметры:
...
Функция ПолучитьТекстИзHTML(Знач ТекстHTML, Знач Кодировка = Неопределено, Знач ВыделятьСсылки = Истина, УпрощенноеУдалениеСтилей = Ложь) Экспорт
	...
	Если УпрощенноеУдалениеСтилей Тогда
		ТекстHTML = УдалитьВсеТегиССодержимым(ТекстHTML, "style");
	КонецЕсли;
	...
	Если Не УпрощенноеУдалениеСтилей Тогда
...
// Преобразовывает HTML текст в текст
Функция ПолучитьТекстИзHTML_IE(Знач ТекстHTML) Экспорт
	...
	#Иначе
		Попытка
			Текст = ПолучитьПростойТекстИзHTML(ТекстHTML);
			Возврат Текст;
		Исключение
			// При возникновении ошибки возвращаем пустую строку
		КонецПопытки;
	#КонецЕсли
...
//  - это все надо вырезать.  <!--   -->
Процедура УдалитьКомментарии(ТекстПисьма)
	...
	Пока Истина Цикл
		...
		ПозицияНачало1 = СтрНайти(ТекстПисьмаДляПравкиНРег, "<!--", , НачальнаяПозиция);
...
Функция ПолучитьПростойТекстИзHTML(Знач ТекстHTML, Знач Кодировка = Неопределено) Экспорт
	...
	#Если ВебКлиент Тогда
		...
		Возврат РаботаС_HTMLВызовСервера.ПолучитьПростойТекстИзHTML(ТекстHTML, Кодировка);
...
Процедура ДобавитьТегиКСсылкам(ТекстHTML, ЗаполнитьСвойствоTitle = Истина) Экспорт
	...
	ДобавитьТегиКСсылкамСоСхемой(ТекстHTML, "e1ccs/", "v8doc:e1ccs/");
	ДобавитьТегиКСсылкамСоСхемой(ТекстHTML, "tasks/", "v8doc:tasks/");
	...
	СкорректироватьСсылки(ТекстHTML, "e1ccs/", "v8doc:e1ccs/");
	
	// Заполнить title у ссылок
	Если ЗаполнитьСвойствоTitle Тогда
...
//  ДляКарточкиВходящегоПисьма - Булево
//  ТипПисьма - Строка - например "Исходящее"
//
Процедура ПрименитьНастройкиОтображениеПисьма(ТекстHTML, ШрифтОтображения = Неопределено,
	...
	ДляКарточкиВходящегоПисьма = Ложь, ТипПисьма = "") Экспорт
	...
	Пока Не НайденНезначащийСимвол Цикл
		...
		Если ПервыйСимвол = "<" Тогда
			...
			Если ЗаполненаСтруктураТега(СтруктураТега) Тогда
				...
				Если СтруктураТега.ПозицияНачалаОткрывающегоТега = 1
					И СтруктураТега.ПозицияНачалаЗакрывающегоТега <> 0 Тогда
...
Процедура ЗаполнитьСвойствоTitleУСсылок(ТекстHTML)
	...
	Пока ЗаполненаСтруктураТега(Тег) <> 0 Цикл
		...
		Если СтрНачинаетсяС(ТекстHref, "v8doc:") Тогда
			Тег = ПолучитьТегВТекстеHTML(ТекстHTML, "a", Тег.ПозицияОкончанияОткрывающегоТега + 1);
			Продолжить;
		КонецЕсли;
...
Функция СформироватьСсылкуСТегом(Ссылка, Схема, НовоеЗначениеСхемы)
	...
	Если Схема = "e1cib/" Тогда
		ПредставлениеСсылки = ПредставлениеСсылки(Ссылка);
	ИначеЕсли Схема = "tasks/" Тогда
		ПредставлениеСсылки = ПредставлениеКоманднойСсылки(Ссылка);
	КонецЕсли;
...
Функция ПредставлениеКоманднойСсылки(НавигационнаяСсылка)
	
	Если СтрНачинаетсяС(НавигационнаяСсылка, "tasks/ShowCancellationReason/") Тогда
		
		СсылкаНаОбъект = СтрЗаменить(НавигационнаяСсылка, "tasks/ShowCancellationReason/", "");
		Возврат СтрШаблон(НСтр("ru = 'Причина отмены: %1'"),
			ПредставлениеСсылки(СсылкаНаОбъект));
		
	ИначеЕсли СтрНачинаетсяС(НавигационнаяСсылка, "tasks/ShowProcessRollbackReason/") Тогда
		
		СсылкаНаОбъект = СтрЗаменить(НавигационнаяСсылка, "tasks/ShowProcessRollbackReason/", "");
		Возврат СтрШаблон(НСтр("ru = 'Причина отмены выполнения: %1'"),
			ПредставлениеСсылки(СсылкаНаОбъект));
		
	ИначеЕсли СтрНачинаетсяС(НавигационнаяСсылка, "tasks/ChangeDueDate/") Тогда
		
		СсылкаНаОбъект = СтрЗаменить(НавигационнаяСсылка, "tasks/ChangeDueDate/", "");
		Возврат СтрШаблон(НСтр("ru = 'Согласовать переноса срока %1'"),
			ПредставлениеСсылки(СсылкаНаОбъект));
		
	ИначеЕсли СтрНачинаетсяС(НавигационнаяСсылка, "tasks/ShowPerformanceHistory/") Тогда
		
		СсылкаНаОбъект = СтрЗаменить(НавигационнаяСсылка, "tasks/ShowPerformanceHistory/", "");
		Возврат СтрШаблон(НСтр("ru = 'История исполнения(%1)'"),
			ПредставлениеСсылки(СсылкаНаОбъект));
		
	ИначеЕсли СтрНачинаетсяС(НавигационнаяСсылка, "tasks/ShowInvitationHistory/") Тогда
		
		СсылкаНаОбъект = СтрЗаменить(НавигационнаяСсылка, "tasks/ShowInvitationHistory/", "");
		Возврат СтрШаблон(НСтр("ru = 'История приглашения(%1)'"),
			ПредставлениеСсылки(СсылкаНаОбъект));
		
	ИначеЕсли СтрНачинаетсяС(НавигационнаяСсылка, "tasks/ShowSigningHistory/") Тогда
		
		СсылкаНаОбъект = СтрЗаменить(НавигационнаяСсылка, "tasks/ShowSigningHistory/", "");
		Возврат СтрШаблон(НСтр("ru = 'История подписания(%1)'"),
			ПредставлениеСсылки(СсылкаНаОбъект));
		
	ИначеЕсли СтрНачинаетсяС(НавигационнаяСсылка, "tasks/ShowRegistrationHistory/") Тогда
		
		СсылкаНаОбъект = СтрЗаменить(НавигационнаяСсылка, "tasks/ShowRegistrationHistory/", "");
		Возврат СтрШаблон(НСтр("ru = 'История регистрации(%1)'"),
			ПредставлениеСсылки(СсылкаНаОбъект));
		
	ИначеЕсли СтрНачинаетсяС(НавигационнаяСсылка, "tasks/ShowReconciliationHistory/") Тогда
		
		СсылкаНаОбъект = СтрЗаменить(НавигационнаяСсылка, "tasks/ShowReconciliationHistory/", "");
		Возврат СтрШаблон(НСтр("ru = 'История согласования(%1)'"),
			ПредставлениеСсылки(СсылкаНаОбъект));
		
	ИначеЕсли СтрНачинаетсяС(НавигационнаяСсылка, "tasks/ShowConfirmationHistory/") Тогда
		
		СсылкаНаОбъект = СтрЗаменить(НавигационнаяСсылка, "tasks/ShowConfirmationHistory/", "");
		Возврат СтрШаблон(НСтр("ru = 'История утверждения(%1)'"),
			ПредставлениеСсылки(СсылкаНаОбъект));
		
	ИначеЕсли СтрНачинаетсяС(НавигационнаяСсылка, "tasks/ShowApprovalSheet/") Тогда
		
		СсылкаНаОбъект = СтрЗаменить(НавигационнаяСсылка, "tasks/ShowApprovalSheet/", "");
		Возврат СтрШаблон(НСтр("ru = 'Лист согласования документа %1'"),
			ПредставлениеСсылки(СсылкаНаОбъект));
		
	ИначеЕсли СтрНачинаетсяС(НавигационнаяСсылка, "tasks/ListOfDisagreements/") Тогда
		
		СсылкаНаОбъект = СтрЗаменить(НавигационнаяСсылка, "tasks/ListOfDisagreements/", "");
		Возврат СтрШаблон(НСтр("ru = 'Протокол разногласий согласования %1'"),
			ПредставлениеСсылки(СсылкаНаОбъект));
		
	ИначеЕсли СтрНачинаетсяС(НавигационнаяСсылка, "tasks/ShowCorrespondence/") Тогда
		
		СсылкаНаОбъект = СтрЗаменить(НавигационнаяСсылка, "tasks/ShowCorrespondence/", "");
		Возврат СтрШаблон(НСтр("ru = 'Переписка по %1'"),
			ПредставлениеСсылки(СсылкаНаОбъект));
		
	ИначеЕсли СтрНачинаетсяС(НавигационнаяСсылка, "tasks/ShowLinks/") Тогда
		
		СсылкаНаОбъект = СтрЗаменить(НавигационнаяСсылка, "tasks/ShowLinks/", "");
		Возврат СтрШаблон(НСтр("ru = 'Связи(%1)'"),
			ПредставлениеСсылки(СсылкаНаОбъект));
		
	ИначеЕсли СтрНачинаетсяС(НавигационнаяСсылка, "tasks/goods/") Тогда
		
		СсылкаНаОбъект = СтрЗаменить(НавигационнаяСсылка, "tasks/goods/", "");
		Возврат СтрШаблон(НСтр("ru = 'Товары(%1)'"),
			ПредставлениеСсылки(СсылкаНаОбъект));
		
	ИначеЕсли СтрНачинаетсяС(НавигационнаяСсылка, "tasks/ShowHistory/") Тогда
		
		СсылкаНаОбъект = СтрЗаменить(НавигационнаяСсылка, "tasks/ShowHistory/", "");
		Возврат СтрШаблон(НСтр("ru = 'История выполнения(%1)'"),
			ПредставлениеСсылки(СсылкаНаОбъект));
		
	ИначеЕсли СтрНачинаетсяС(НавигационнаяСсылка, "tasks/ShowFile/") Тогда
		
		СсылкаНаОбъект = СтрЗаменить(НавигационнаяСсылка, "tasks/ShowFile/", "");
		Возврат ПредставлениеСсылки(СсылкаНаОбъект);
		
	ИначеЕсли СтрНачинаетсяС(НавигационнаяСсылка, "tasks/ShowTaskAction/") Тогда
		
		СсылкаНаОбъект = СтрЗаменить(НавигационнаяСсылка, "tasks/ShowTaskAction/", "");
		Возврат ПредставлениеСсылки(СсылкаНаОбъект);
		
	ИначеЕсли СтрНачинаетсяС(НавигационнаяСсылка, "tasks/ShowInReplyTo/") Тогда
		
		СсылкаНаОбъект = СтрЗаменить(НавигационнаяСсылка, "tasks/ShowInReplyTo/", "");
		Возврат СтрШаблон(НСтр("ru = 'В ответ на %1'"),
			ПредставлениеСсылки(СсылкаНаОбъект));
...
// Удаляет все вхождения тега с содержимым из текста HTML по названию элемента тега.
//
// Параметры:
//  ТекстHTML - Строка
//  НазваниеЭлемента - Строка
// 
// Возвращаемое значение:
//  Строка
//
Функция УдалитьВсеТегиССодержимым(Знач ТекстHTML, НазваниеЭлемента)
	
	СтруктураТега = ПолучитьТегВТекстеHTML(ТекстHTML, НазваниеЭлемента);
	Пока ЗаполненаСтруктураТега(СтруктураТега) <> 0 Цикл
		ТекстHTML = УдалитьТег(ТекстHTML, СтруктураТега);
		СтруктураТега = ПолучитьТегВТекстеHTML(ТекстHTML, НазваниеЭлемента, СтруктураТега.ПозицияНачалаТега);
	КонецЦикла;
...
// Удаляет указанный тег из текста HTML.
// Перед вызовом необходимо убедиться, что структура тега заполнена, вызвав РаботаС_HTML.ЗаполненаСтруктураТега.
//
// Параметры:
//  ТекстHTML - Строка
//  СтруктураТега - см. РаботаС_HTML.СформироватьСтруктуруТега.
// 
// Возвращаемое значение:
//  Строка
//
Функция УдалитьТег(Знач ТекстHTML, СтруктураТега)
	
	ТекстHTML = Лев(ТекстHTML, СтруктураТега.ПозицияНачалаТега - 1) + Сред(ТекстHTML, СтруктураТега.ПозицияОкончанияТега + 1);
