...
//  Структура - Параметры нечеткого поиска:
// * СоответствиеРеквизитов - Структура - 
// * ВесаРеквизитов - Структура - 
// * РеквизитыПолногоСоответствия - Массив - 
// * ВесаРеквизитовПредварительнойОценки - Структура - 
// * КоличествоКандидатовПредварительнойОценки - Число - 
// * ВесКороткойНГраммы - Число - 
// * ВесДлиннойНГраммы - Число - 
// * ВажностьПолногоСоответствия - Число - 
// * Сигма - Число - 
// * ОтброситьДробнуюЧасть - Булево - 
// * ТекстЗапросаКандидаты - Строка - 
// * КоличествоКандидатовБыстройОценки - Число - 
// * МинимальнаяДлинаСтрокиДляПоиска - Число - 
...
// Параметры нечеткого поиска перечислений.
// 
// Возвращаемое значение:
//  Структура - Параметры нечеткого поиска перечислений:
// * СоответствиеРеквизитов - Структура - 
// * ВесаРеквизитов - Структура - 
// * РеквизитыПолногоСоответствия - Массив - 
// * ВесаРеквизитовПредварительнойОценки - Структура - 
...
// Выполняет нечеткий поиск значений в базе данных.
// 
// Параметры:
//  МетаданныеОбъекта - ОбъектМетаданных
//  ЗначенияРаспознанныхРеквизитов - Структура - поля в зависимости от распознаваемого объекта
//  Отбор - Массив из Структура - набор полей для предварительного отбора кандидатов
// 
// Возвращаемое значение:
//  ТаблицаЗначений - Нечеткий поиск:
// * Ссылка - ЛюбаяСсылка
// * ДополнительнаяСсылка - ЛюбаяСсылка
// * Коэффициент - Число
Функция НечеткийПоиск(МетаданныеОбъекта, ЗначенияРаспознанныхРеквизитов, Отбор = Неопределено) Экспорт
	...
	Если Кандидаты.Количество() > 0 Тогда
		...
		// 3 максимальных кандидатов в результат.
		Для Индекс = 0 По Мин(Кандидаты.Количество() - 1, 2) Цикл 
...
//  Строка
//
Функция СтрокаБезМусора(Знач Строка) Экспорт
...
Функция СходствоДжароВинклера(Знач Строка1, Знач Строка2) Экспорт
	...
	Строка1 = СтрокаБезМусора(Строка1);
	Строка2 = СтрокаБезМусора(Строка2); 
	
	Стр1 = Нрег(СокрЛП(Строка1)); 
	Стр2 = Нрег(СокрЛП(Строка2));
	...
	Если СтрДлина(Стр2) > СтрДлина(Стр1) Тогда
		Буфер = Стр1;
		Стр1 = Стр2;
		Стр2 = Буфер;
	КонецЕсли;
	...
	РасстояниеДжаро = 0; //- расстояние Джаро дж
	РезультатПоДжароВинклеру = 0; //- результат Джаро - Винклера дв
	...
	Для Инд1 = 1 По Длина1 Цикл
		Для Инд2 = Макс(1, Инд1 - ДопРасстояние) По Мин(Длина2, Инд1 + ДопРасстояние) Цикл
			Если Сред(Стр1,Инд1,1) = Сред(Стр2,Инд2,1) И СимволыВторойСтроки[Инд2-1] = 0 Тогда
	...
	Если КолПодходСимв > 0 Тогда
		...
		Для Индекс = 0 По СимволыПервойСтроки.ВГраница() Цикл  
			Если СимволыПервойСтроки[Индекс] = 1 Тогда
				...
				Если Сред(Стр1,Индекс+1,1) <> Сред(Стр2,Поз+1,1) Тогда
		...
		РасстояниеДжаро = 
			(КолПодходСимв/Длина1 + КолПодходСимв/Длина2 + (КолПодходСимв-ПоловинаТранспозиций)/КолПодходСимв)/3; 
		
		Если РасстояниеДжаро >= ПорогУсиления Тогда
			Для Индекс = 1 По Мин(Длина1,Длина2) Цикл
				Если Сред(Стр1,Индекс,1) = Сред(Стр2,Индекс,1) Тогда
			...
			РезультатПоДжароВинклеру = РасстояниеДжаро + (ОбщийПрефикс * Масштаб * (1-РасстояниеДжаро)); 
		Иначе
			// дж < ПорогУсиления - меньше "порога усиления" применения префиксного бонуса
			РезультатПоДжароВинклеру = РасстояниеДжаро; 
		КонецЕсли;
	
	КонецЕсли;
	
	// результат
	Возврат РезультатПоДжароВинклеру;
...
Функция ЗапросПредварительнойОценки(МетаданныеОбъекта, РаспознанныеРеквизиты, Отбор, ПараметрыНечеткогоПоиска)
...
// Кандидаты по предварительной оценке.
// 
// Параметры:
//  МетаданныеОбъекта - ОбъектМетаданных - Метаданные объекта
//  РаспознанныеРеквизиты - Структура - Распознанные реквизиты
//  Отбор - Неопределено, Массив из Строка - Отбор
//  ПараметрыНечеткогоПоиска - Структура - Параметры нечеткого поиска:
// 
// Возвращаемое значение:
//  ТаблицаЗначений - Кандидаты по предварительной оценке
