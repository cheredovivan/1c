...
//  Сотрудник			 - СправочникСсылка.Сотрудники				 - Сотрудник, брони которого отбирать.
//  СотрудникИсключение	 - СправочникСсылка.Сотрудники				 - Сотрудник, брони которого не будут отобраны.
// 
...
Функция ПолучитьНастройкуОтбораБроней(ПараметрПомещение, ДатаНачала, ДатаОкончания,
	ПоказыватьУдаленные = Ложь, Сотрудник = Неопределено, СотрудникИсключение = Неопределено) Экспорт
	
	ТипЗнчСотрудник = ТипЗнч(Сотрудник);
	Если ТипЗнчСотрудник = Тип("СправочникСсылка.Пользователи") Тогда
		ПараметрСотрудник = Новый Массив;
		ПараметрСотрудник.Добавить(
			Сотрудники.ОсновнойСотрудникПользователя(Сотрудник));
	ИначеЕсли ТипЗнчСотрудник = Тип("Массив") Тогда
		ПараметрСотрудник = Сотрудник;
	Иначе
		ПараметрСотрудник = Новый Массив;
	КонецЕсли;
	
	ТипЗнчСотрудникИсключение = ТипЗнч(СотрудникИсключение);
	Если ТипЗнчСотрудникИсключение = Тип("СправочникСсылка.Пользователи") Тогда
		ПараметрСотрудникИсключение = Новый Массив;
		ПараметрСотрудникИсключение.Добавить(
			Сотрудники.ОсновнойСотрудникПользователя(СотрудникИсключение));
	ИначеЕсли ТипЗнчСотрудникИсключение = Тип("Массив") Тогда
		ПараметрСотрудникИсключение = СотрудникИсключение;
	Иначе
		ПараметрСотрудникИсключение = Новый Массив;
	КонецЕсли;
	...
	НастройкаОтбораБроней.Вставить("ПоказыватьОтмененные", Ложь);
	НастройкаОтбораБроней.Вставить("Сотрудник", ПараметрСотрудник);
	НастройкаОтбораБроней.Вставить("СотрудникИсключение", ПараметрСотрудникИсключение);
...
Функция ПолучитьОписаниеБрони(Бронь, ПолноеОписание = Истина) Экспорт
	...
	ДанныеБрони = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Бронь,
		"Ссылка, Помещение, ДатаНачала, ДатаОкончания, ВесьДень, Комментарий, Сотрудник");
...
Функция ПолучитьПерсональнуюНастройку(Настройка) Экспорт
	...
	ИначеЕсли Настройка = "ПоказыватьОтмененные" Тогда
		
		Значение = ОбщегоНазначения.ХранилищеОбщихНастроекЗагрузить(
			"БронированиеПомещений",
			"ПоказыватьОтмененные",
			Ложь);
...
Функция ОтобразитьПланировщикБроней(Планировщик, Параметры) Экспорт
	...
	Для Каждого Бронь Из ОтображаемыеБрони Цикл
		...
		ДобавитьЗначениеКСтрокеЧерезРазделитель(ПредставлениеЭлемента, ", ", Строка(Бронь.Сотрудник));
		...
		Элемент.Значение = Новый Структура(
			"Ссылка, Автор, Сотрудник, ДатаНачала, ДатаОкончания, ВесьДень, ПометкаУдаления,
		...
		Элемент.Картинка = Перечисления.СостоянияБроней.Картинка(Бронь.СостояниеБрони);
...
Процедура БронированиеПриЗаписиПредмета(Источник, Отказ) Экспорт
	...
	Если Не Источник.ДополнительныеСвойства.Свойство("РезультатБронирования") Тогда
		Источник.ДополнительныеСвойства.Вставить("РезультатБронирования",
	...
	Если Не Источник.ДополнительныеСвойства.ЭтоНовый Тогда
		...
		ОтмененныеБрони = ОбработатьИзменениеРеквизитовПредмета(
			Источник,
			Источник.ДополнительныеСвойства.ПредыдущиеЗначенияРеквизитов);
		ОбщегоНазначенияКлиентСервер.ДополнитьМассив(
			Источник.ДополнительныеСвойства.РезультатБронирования.ОтмененныеБрони,
			ОтмененныеБрони);
	...
	Если ТипЗнч(Источник) = Тип("РегистрСведенийНаборЗаписей.СостоянияМероприятий") Тогда
		...
		Для Каждого ЗаписьСостояния Из Источник Цикл
			...
			Если ЗаписьСостояния.Состояние = Перечисления.СостоянияМероприятий.МероприятиеОтменено Тогда
				...
				ОтмененныеБрони = УдалитьАвтоматическиеБрониПоПредмету(ЗаписьСостояния.Мероприятие);
				ОбщегоНазначенияКлиентСервер.ДополнитьМассив(
					Источник.ДополнительныеСвойства.РезультатБронирования.ОтмененныеБрони,
					ОтмененныеБрони);
			...
			ИначеЕсли ЗаписьСостояния.Состояние = Перечисления.СостоянияМероприятий.МероприятиеВСтадииПодготовки
				...
				Если Не МероприятиеПомеченоНаУдаление Тогда
					...
					ОбщегоНазначенияКлиентСервер.ДополнитьМассив(
						Источник.ДополнительныеСвойства.РезультатБронирования.РезультатыБроней,
						МероприятиеОбъект.ДополнительныеСвойства.РезультатБронирования.РезультатыБроней);
...
Функция ДоступноБронированиеПомещения(Помещение) Экспорт
	...
	РеквизитыПомещения = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(
		Помещение,
		"БрониВводитОтветственный, Ответственный");
	Если Не РеквизитыПомещения.БрониВводитОтветственный Тогда
		ДоступноБронированиеПомещения = Истина;
		Возврат ДоступноБронированиеПомещения;
	КонецЕсли;
	
	Если ДокументооборотПраваДоступаПовтИсп.ВключеноИспользованиеПравДоступа() Тогда
		
		ПраваПользователяПоОбъекту =
			ДокументооборотПраваДоступа.ПраваПользователяПоОбъекту(Помещение);
		ДоступноБронированиеПомещения = ПраваПользователяПоОбъекту.Изменение;
	...
	ИначеЕсли ТипЗнч(РеквизитыПомещения.Ответственный) = Тип("СправочникСсылка.Сотрудники") Тогда
		
		ДоступноБронированиеПомещения =
			Сотрудники.ЭтоСотрудникТекущегоПользователя(РеквизитыПомещения.Ответственный);
		
		Возврат ДоступноБронированиеПомещения;
		
	ИначеЕсли ТипЗнч(РеквизитыПомещения.Ответственный) = Тип("СправочникСсылка.Пользователи") Тогда
		
		ДоступноБронированиеПомещения =
			РеквизитыПомещения.Ответственный = Пользователи.ТекущийПользователь();
		
		Возврат ДоступноБронированиеПомещения;
		
	Иначе
		
		ДоступноБронированиеПомещения = Ложь;
...
// 
// Параметры:
//  Предмет - СправочникСсылка.Мероприятия - Предмет.
// 
// Возвращаемое значение:
//  ТаблицаЗначений - Получить автоматические брони по предмету:
//   * Ссылка - ДокументСсылка.Бронь.
//   * ДатаНачал - Дата.
//   * ДатаОкончания - Дата.
// 
Функция АвтоматическиеБрониПоПредмету(Предмет) Экспорт
	...
	Запрос.Текст =
		...
		|	И Бронь.СостояниеБрони <> ЗНАЧЕНИЕ(Перечисление.СостоянияБроней.Отменена)
...
Процедура ЗаполнитьДанныеКлючевыхОпераций(ДанныеКлючевыхОпераций) Экспорт
	...
	ДанныеКлючевойОперации.Имя = БронированиеПомещенийКлиентСервер.КлючеваяОперацияОткрытиеБронирования();
	ДанныеКлючевойОперации.Наименование = НСтр("ru = 'Бронирование: Открытие бронирования помещений'", КодОсновногоЯзыка);
	ДанныеКлючевойОперации.ЦелевоеВремя = 1;
	
	ДанныеКлючевойОперации = ДанныеКлючевыхОпераций.Добавить();
	ДанныеКлючевойОперации.Имя = БронированиеПомещенийКлиентСервер.КлючеваяОперацияОбновлениеПланировщика();
	ДанныеКлючевойОперации.Наименование = НСтр("ru = 'Бронирование: Обновление планировщика'", КодОсновногоЯзыка);
	ДанныеКлючевойОперации.ЦелевоеВремя = 1;
	
	ДанныеКлючевойОперации = ДанныеКлючевыхОпераций.Добавить();
	ДанныеКлючевойОперации.Имя = БронированиеПомещенийКлиентСервер.КлючеваяОперацияОткрытиеБрони();
	ДанныеКлючевойОперации.Наименование = НСтр("ru = 'Бронирование: Открытие карточки брони'", КодОсновногоЯзыка);
	ДанныеКлючевойОперации.ЦелевоеВремя = 1;
	
	ДанныеКлючевойОперации = ДанныеКлючевыхОпераций.Добавить();
	ДанныеКлючевойОперации.Имя = БронированиеПомещенийКлиентСервер.КлючеваяОперацияСозданиеБрони();
	ДанныеКлючевойОперации.Наименование = НСтр("ru = 'Бронирование: Создание брони'", КодОсновногоЯзыка);
	ДанныеКлючевойОперации.ЦелевоеВремя = 1;
	
	ДанныеКлючевойОперации = ДанныеКлючевыхОпераций.Добавить();
	ДанныеКлючевойОперации.Имя = БронированиеПомещенийКлиентСервер.КлючеваяОперацияЗаписьБрони();
	ДанныеКлючевойОперации.Наименование = НСтр("ru = 'Бронирование: Запись брони'", КодОсновногоЯзыка);
	ДанныеКлючевойОперации.ЦелевоеВремя = 1;
...
Функция СформироватьТаблицуБроней()
	...
	ТаблицаБроней.Колонки.Добавить(
		"Автор", Новый ОписаниеТипов("СправочникСсылка.Сотрудники"));
	ТаблицаБроней.Колонки.Добавить(
		"Сотрудник", Новый ОписаниеТипов("СправочникСсылка.Сотрудники"));
	ТаблицаБроней.Колонки.Добавить(
	...
	ТаблицаБроней.Колонки.Добавить(
		"СостояниеБрони", Новый ОписаниеТипов("ПеречислениеСсылка.СостоянияБроней"));
...
Процедура ЗаполнитьТаблицуБронями(ТаблицаБроней, НастройкаОтбораБроней)
	...
	Запрос.Текст = 
		...
		|	Бронь.Предмет,
		|	Бронь.СостояниеБрони КАК СостояниеБрони
		|ИЗ
		|	Документ.Бронь КАК Бронь
		|ГДЕ
		...
		|	И Бронь.СостояниеБрони <> ЗНАЧЕНИЕ(Перечисление.СостоянияБроней.Отменена)
		|	И НЕ Бронь.ПометкаУдаления
		|	И Бронь.Сотрудник В (&Сотрудники)
		|	И НЕ (Бронь.Сотрудник В (&СотрудникиИсключение))";
	...
	Если НастройкаОтбораБроней.ПоказыватьУдаленные Тогда
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "И НЕ Бронь.ПометкаУдаления", "");
	КонецЕсли;
	
	Если НастройкаОтбораБроней.ПоказыватьОтмененные Тогда
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "И Бронь.СостояниеБрони <> ЗНАЧЕНИЕ(Перечисление.СостоянияБроней.Отменена)", "");
	КонецЕсли;
	
	Если НастройкаОтбораБроней.Сотрудник.Количество() <> 0 Тогда
		Запрос.УстановитьПараметр("Сотрудники", НастройкаОтбораБроней.Сотрудник);
	Иначе
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "И Бронь.Сотрудник В (&Сотрудники)", "");
	КонецЕсли;
	
	Если НастройкаОтбораБроней.СотрудникИсключение.Количество() <> 0 Тогда
		Запрос.УстановитьПараметр("СотрудникиИсключение", НастройкаОтбораБроней.СотрудникИсключение);
	Иначе
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "И НЕ (Бронь.Сотрудник В (&СотрудникиИсключение))", "");
	КонецЕсли;
...
Функция ПолучитьРезультатЗапросаПовторяющихсяСобытий(НастройкаОтбораБроней)
	...
	Запрос.Текст = 
		...
		|	Бронь.Сотрудник КАК Пользователь,
		|	Бронь.СостояниеБрони КАК СостояниеБрони
		|ИЗ
		|	Документ.Бронь КАК Бронь
		|ГДЕ
...
Функция ПолучитьОтображаемыеБрони(
	...
	НастройкаОтбораБроней.ПоказыватьОтмененные = ПолучитьПерсональнуюНастройку("ПоказыватьОтмененные");
...
Процедура ЗабронироватьПомещениеДляМероприятия(МероприятиеОбъект)
	...
	Сотрудник = Метаданные.Документы.Бронь.Реквизиты.Сотрудник.Тип.ПривестиЗначение(
	...
	РезультатыБроней = ЗабронироватьПомещение(
	...
	Если Не МероприятиеОбъект.ДополнительныеСвойства.Свойство("РезультатБронирования") Тогда
		МероприятиеОбъект.ДополнительныеСвойства.Вставить("РезультатБронирования",
			БронированиеПомещенийКлиентСервер.РезультатБронирования());
	КонецЕсли;
	ОбщегоНазначенияКлиентСервер.ДополнитьМассив(
		МероприятиеОбъект.ДополнительныеСвойства.РезультатБронирования.РезультатыБроней,
...
//  ДатаНачала             - Дата                                    - Дата, с которой отбираются брони.
//  ДатаОкончания          - Дата                                    - Дата, по которую отбираются брони.
//  Пользователь           - СправочникСсылка.Пользователи           - Пользователь, для которого выполняется бронь.
//  Источник               - СправочникОбъект.Мероприятия            - Предмет, на основании которого создается бронь.
//  ДопустимыеПользователи - Массив из СправочникСсылка.Пользователи - Пользователи, брони которых можно использовать.
// 
// Возвращаемое значение:
//  Массив из Структура - Результат брони:
//   * Бронь                - ДокументСсылка.Бронь - Бронь.
//   * ОжидаетПодтверждения - Булево               - Бронь ожидает подтверждения.
//
Функция ЗабронироватьПомещение(Помещение, ДатаНачала, ДатаОкончания, Пользователь, Источник, ДопустимыеПользователи)
	...
	Для Каждого Время Из ВременаБезБрони Цикл
		БроньОбъект = СоздатьАвтоматическуюБронь(
			Помещение,
			Время.ДатаНачала,
			Время.ДатаОкончания,
			Пользователь,
...
//  Сотрудник     - СправочникСсылка.Сотрудники           - Сотрудник, для которого предназначена бронь.
//  Источник      - СправочникОбъект.Мероприятия          - Предмет брони.
// 
// Возвращаемое значение:
//  ДокументОбъект.Бронь - Созданная бронь.
//
Функция СоздатьАвтоматическуюБронь(Помещение, ДатаНачала, ДатаОкончания, Сотрудник, Источник)
	
	НоваяБронь = Документы.Бронь.СоздатьДокумент();
	НоваяБронь.Предмет = Источник.Ссылка;
	НоваяБронь.Помещение = Помещение;
	НоваяБронь.Сотрудник = Сотрудник;
...
//
// Параметры:
//  Предмет                      - СправочникСсылка.Мероприятия - Предмет, по которому необходимо удалить автоматические брони.
//  ПредыдущиеЗначенияРеквизитов - Структура                    - Предыдущее значение реквизитов предмета.
//                                                                См. Справочник.Мероприятия.ОтслеживаемыеРеквизиты().
// 
// Возвращаемое значение:
//  Массив из ДокументСсылка.Бронь - Брони, которые были отменены в ходе корректировки.
//
Функция ОбработатьИзменениеРеквизитовПредмета(Предмет, ПредыдущиеЗначенияРеквизитов)
	...
	Если УстановилиПометкуУдаления Или ОчистилиДату Или ИзменилосьПомещение Тогда
		ОтмененныеБрони = УдалитьАвтоматическиеБрониПоПредмету(Предмет);
	ИначеЕсли ИзменилиДату Тогда
		ОтмененныеБрони = СкорректироватьАвтоматическиеБрониПоПредмету(Предмет);
	КонецЕсли;
	
	Возврат ОтмененныеБрони;
...
//  Массив из ДокументСсылка.Бронь - Брони, которые были отменены в ходе корректировки.
//
Функция УдалитьАвтоматическиеБрониПоПредмету(Предмет)
	
	ОтмененныеБрони = Новый Массив;
	
	Брони = АвтоматическиеБрониПоПредмету(Предмет.Ссылка);
	Для Каждого Бронь Из Брони Цикл
		БроньОтменена = УдалитьАвтоматическуюБронь(Бронь.Ссылка);
		Если БроньОтменена Тогда
			ОтмененныеБрони.Добавить(Бронь.Ссылка);
		КонецЕсли;
	КонецЦикла;
...
//
// Параметры:
//  Предмет- СправочникСсылка.Мероприятия - Предмет, по которому необходимо выполнить корректировку.
// 
// Возвращаемое значение:
//  Массив из ДокументСсылка.Бронь - Брони, которые были отменены в ходе корректировки.
//
Функция СкорректироватьАвтоматическиеБрониПоПредмету(Предмет)
...
// 
// Возвращаемое значение:
//  Булево - Если Истина - бронь помечена на удаление, если Ложь - очищен предмет брони.
//
Функция УдалитьАвтоматическуюБронь(Бронь)
