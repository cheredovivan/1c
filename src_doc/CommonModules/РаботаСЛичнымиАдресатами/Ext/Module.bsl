...
Функция ЕстьТакойЛичныйАдресат(Элемент) 
	...
	Запрос.Текст
	 = "ВЫБРАТЬ
	   ...
	   |ГДЕ
	   |	ЛичныеАдресаты.Сотрудник В (&СотрудникиПользователя)";
...
Функция ЗаполнитьКонтактИЗаменитьСсылки(ЛичныйАдресат, Контакт, УникальныйИдентификатор = Неопределено,
	...
	Если ТипЗнч(Контакт) = Тип("СправочникСсылка.Контрагенты") Тогда
		...
		Если Контакт.Наименование <> ЛичныйАдресат.Наименование
			Или Контакт.Ответственный <> ЛичныйАдресат.Сотрудник Тогда
	...
	ИначеЕсли ТипЗнч(Контакт) = Тип("СправочникСсылка.КонтактныеЛица") Тогда	
		...
		ПоляКЛ = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Контакт, "Наименование, Должность");
		Если ПоляКЛ.Наименование <> ЛичныйАдресат.Наименование
			Или Строка(ПоляКЛ.Должность) <> ЛичныйАдресат.Должность Тогда
