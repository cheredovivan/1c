...
Процедура ВыполнитьHTTPСоединение(СтрокаCode, Форма,
	СерверВходящейПочты, СерверИсходящейПочты) Экспорт
	
	Форма.ДействуетДо = ТекущаяДатаСеанса();
	Форма.refresh_token = "";
	
	Если РаботаСПочтовымиСообщениямиПереопределяемый.ПолучитьТокенAuth(
			Форма.ТокенДоступа, Форма.ДействуетДо, Форма.refresh_token,
			СерверИсходящейПочты,
			СтрокаCode, 
			Форма.OAuthАвторизацияКлиент,
			Форма.OAuthАвторизацияКлиентСекретныйКод,
			Форма.OAuth_redirection_uri) Тогда
	...
	КодированныйАдресПерехода = КодироватьСтроку(Форма.OAuth_redirection_uri, СпособКодированияСтроки.КодировкаURL);
	...
	Если СтрНайти(НРег(СерверВходящейПочты), "office365.com") <> 0 Тогда
	...
	Форма.ТокенДоступа = ЗначенияОтвета["access_token"];
	Форма.ДействуетДо = ДатаНачала + ЗначенияОтвета["expires_in"];
	Форма.refresh_token = ЗначенияОтвета["refresh_token"];
