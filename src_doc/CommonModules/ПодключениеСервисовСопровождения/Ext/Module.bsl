...
Функция ТестовыеПериодыСервисаСопровождения(Идентификатор) Экспорт
	...
	Если ВыборкаИдентификатор.Следующий() Тогда
		...
		Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
			...
			// Сервис распознавания первичных документов пока недоступен в 1С:Документооборот 3.0.
			Если ВыборкаДетальныеЗаписи.Наименование
				= Распознавание.НаименованиеТестовогоТарифаСервисаРаспознаванияПервичныхДокументов() Тогда
				Продолжить;
			КонецЕсли;
...
Функция СервисыСопровожденияДоступныеДляПодключения(Идентификаторы)
	...
	Запрос.Текст = 
		...
		|			// Встраивание в Документооборот:
		|			// Сервис распознавания первичных документов пока недоступен в 1С:Документооборот 3.0.
		|			И (КэшТестовыхПериодов.Наименование <> &СервисРаспознаванияПервичныхДокументов)
		|			// Конец Встраивание в Документооборот.
		|ГДЕ
	...
	// Встраивание в Документооборот:
	// Сервис распознавания первичных документов пока недоступен в 1С:Документооборот 3.0.
	Запрос.УстановитьПараметр("СервисРаспознаванияПервичныхДокументов",
		Распознавание.НаименованиеТестовогоТарифаСервисаРаспознаванияПервичныхДокументов());
	// Конец Встраивание в Документооборот.
