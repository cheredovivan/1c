...
Процедура ЗаполнитьДополнительныеРеквизитыВФорме(Форма, Объект = Неопределено, ПоляНадписей = Ложь, СкрытьУдаленные = Неопределено) Экспорт
	...
	Форма.ОбъектПоддерживаетПроверкуУсловийДляДопСвойств = 
		УправлениеСвойствамиДокументооборот.ОбъектПоддерживаетПроверкуУсловийДопСвойств(ОписаниеОбъекта);
	
	// Создание элементов формы.
	Для Каждого ОписаниеСвойства Из ОписаниеСвойств Цикл
		
		ЭлементРазмещения = Неопределено;
		ЭлементОриентир = Неопределено;
		Если ЗначениеЗаполнено(ОписаниеСвойства.РасположениеИмя) Тогда
			ПутьФормы = СтрРазделить(Форма.ИмяФормы, ".");
			
			Для каждого Расположение Из СтрРазделить(ОписаниеСвойства.РасположениеИмя, ";", Ложь) Цикл
				ПутьГруппы = СтрРазделить(Расположение, ".(,)", Истина);
				Если ПутьГруппы[0] = ПутьФормы[ПутьФормы.Вграница()] Тогда
					
					Если ПутьГруппы.Количество() < 2 Или ПутьГруппы[1] = "" Тогда
						ЭлементРазмещения = Форма;
					Иначе
						ЭлементРазмещения = Форма.Элементы.Найти(ПутьГруппы[1]);
						Если ЭлементРазмещения = Неопределено Тогда
							Если Пользователи.ЭтоПолноправныйПользователь() Тогда
								ТекстСообщения = СтрШаблон(НСтр(
									"ru = 'Не найдена группа элементов формы ""%1"" для расположения реквизита ""%2""'"),
									ПутьГруппы[1],
					...
					Если ЭлементРазмещения <> Неопределено Тогда
						Если ПутьГруппы.Количество() > 2 Тогда
							Если ПутьГруппы[2] <> "" Тогда
								// Вставка рядом с указанным элементом.
								Если ПутьГруппы[3] = "" Тогда
									// Вставка после указанного элемента.
									Для Каждого Элемент Из ЭлементРазмещения.ПодчиненныеЭлементы Цикл
										Если Элемент.Имя = ПутьГруппы[2] Тогда
											ЭлементОриентир = Истина;
										ИначеЕсли ЭлементОриентир = Истина Тогда
											ЭлементОриентир = Элемент;
											Прервать;
										КонецЕсли;
									КонецЦикла;
								Иначе
									// Вставка перед указанным элементом.
									ЭлементОриентир = Форма.Элементы.Найти(ПутьГруппы[2]);
								КонецЕсли;
							ИначеЕсли ПутьГруппы[3] <> "" Тогда
								// Вставка в начало группы.
								ЭлементОриентир = Неопределено;
								Для Каждого Элемент Из ЭлементРазмещения.ПодчиненныеЭлементы Цикл
									ЭлементОриентир = Элемент;
									Прервать;
							...
							КонецЕсли;
						КонецЕсли;
						Если ЭлементОриентир = Неопределено Тогда
							ЭлементОриентир = Истина;
						КонецЕсли;
						Прервать;
					КонецЕсли;
		...
		ИначеЕсли НаборыСвойствОбъекта.Количество() > 1 Тогда
		...
		ИначеЕсли ЭлементОриентир <> Неопределено И ЭлементОриентир <> Истина Тогда
			Элемент = Форма.Элементы.Вставить(ОписаниеСвойства.ИмяРеквизитаЗначение, Тип("ПолеФормы"), Родитель, ЭлементОриентир);
...
Функция РезультатВычисленияУсловияДокументооборот(Форма, ОписаниеОбъекта, Параметры) Экспорт
	
	Возврат РезультатВычисленияУсловия(Форма, ОписаниеОбъекта, Параметры);
