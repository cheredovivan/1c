...
Функция ЗаполнитьРегистрационныйШтамп(Объект, МакетПечати, ТабличныйДокумент, ДанныеОПоложении, ЗаголовокПриложения)
	...
	Если НЕ КартинкаШтрихкода = Неопределено 
		...
		РегистрационныйНомер.Ширина = 40;
		...
		Если СтрДлина(Надпись.Текст) > 20 Тогда
			ЧислоСтрок = Окр(СтрДлина(Надпись.Текст) / 20.0, 0, РежимОкругления.Окр15как20);	
			Надпись.Высота = Надпись.Высота * ЧислоСтрок;
		КонецЕсли;	
		...
		Если ИспользоватьШК Тогда
			...
			ИначеЕсли СмещениеПоВертикали = "MIN" Тогда	
				Надпись.Верх = Макет.ПолеСверху;
				РегистрационныйНомер.Верх = Надпись.Верх + Надпись.Высота;
				Рисунок.Верх = РегистрационныйНомер.Верх + РегистрационныйНомер.Высота; // штрихкод
			КонецЕсли;
...
Функция ПолучитьШтрихкодНаНаклейке(ПараметрКоманды, ЗаголовокПриложения) Экспорт
	...
	Если ВысотаНаклейки = Неопределено
		...
		ВысотаНаклейки = 20;
		ШиринаНаклейки = 38;
		ОбщегоНазначения.ХранилищеОбщихНастроекСохранить("НастройкиШтрихкода", "ШиринаНаклейки", ШиринаНаклейки);
		ОбщегоНазначения.ХранилищеОбщихНастроекСохранить("НастройкиШтрихкода", "ВысотаНаклейки", ВысотаНаклейки);
	КонецЕсли;
...
Функция ЗаполнитьШтрихкодНаСтранице(Объект, МакетПечати, ТабличныйДокумент, ДанныеОПоложении, ЗаголовокПриложения)
	...
	Если НЕ КартинкаШтрихкода = Неопределено Тогда
		...
		Если ДанныеОПоложении.ПоложениеНаСтранице = Перечисления.МестаВставкиКартинки.ПравыйНижний Тогда
			СмещениеПоГоризонтали = "MAX";
			СмещениеПоВертикали = "MAX";
		ИначеЕсли ДанныеОПоложении.ПоложениеНаСтранице = Перечисления.МестаВставкиКартинки.ПравыйВерхний Тогда
			СмещениеПоГоризонтали = "MAX";
			СмещениеПоВертикали = "MIN";
		ИначеЕсли ДанныеОПоложении.ПоложениеНаСтранице = Перечисления.МестаВставкиКартинки.ЛевыйВерхний Тогда
			СмещениеПоГоризонтали = "MIN";
			СмещениеПоВертикали = "MIN";
		ИначеЕсли ДанныеОПоложении.ПоложениеНаСтранице = Перечисления.МестаВставкиКартинки.ЛевыйНижний Тогда
			СмещениеПоГоризонтали = "MIN";
			СмещениеПоВертикали = "MAX";
		КонецЕсли;
