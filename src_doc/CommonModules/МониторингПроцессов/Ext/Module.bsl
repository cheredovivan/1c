...
Процедура ОбновитьПоказателиПоУмолчанию() Экспорт
	...
	ВыполнитьОбновлениеПоказателейПоУмолчанию(
...
//
// Параметры:
//  Пользователь - СправочникСсылка.Сотрудники - Пользователь, для которого обновляются показатели по умолчанию.
//
...
Функция ОснованияБиблиотечныхПоказателейПользователя(Пользователь)
	...
	Запрос.Текст =
		...
		|ИЗ
		|	Справочник.ШаблоныПодписания КАК ШаблоныПодписания
		|ГДЕ
		|	ШаблоныПодписания.Ответственный = &Пользователь
		|	И ШаблоныПодписания.ПометкаУдаления = ЛОЖЬ
		|	И ШаблоныПодписания.ЭтоГруппа = ЛОЖЬ
		|	И ШаблоныПодписания.ШаблонВКомплексномПроцессе = ЛОЖЬ
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|ВЫБРАТЬ
		...
		|ГДЕ
		|	ПодчиненностьПодразделений.РуководительПодчиненного = &Пользователь
...
Функция ПоказателиПроцессовДляРасчетаЗаАктуальныеПериоды(ДатаРасчета)
	...
	Запрос.Текст =
		...
		|	ПоказателиПроцессов.ВерсияДанных КАК ВерсияДанных
		|ИЗ
		|	Справочник.ПоказателиПроцессов КАК ПоказателиПроцессов
		...
		|			И (ПодпискиНаПоказателиПроцессов.Сотрудник В (&ПустыеСсылкиПользователя))
...
Процедура ВыполнитьЗапросСПодчиненными(СхемаЗапросаСПодчиненными, МенеджерВременныхТаблиц,
	...
	Пока ЕстьДочерниеПроцессы Цикл
		...
		Запрос.Текст = 
			...
			|	ДанныеДляРасчета3 КАК Процесс,
			|	ДанныеДляРасчета4 КАК ОсновнойПроцесс
			|ПОМЕСТИТЬ ДанныеДляРасчета2
			|ИЗ
			|	ДанныеДляРасчета1 КАК ДанныеДляРасчета
			|
			|ОБЪЕДИНИТЬ
			|
			|ВЫБРАТЬ
			...
			|	ВЫБОР
			|		КОГДА ДочерниеБизнесПроцессы.СвязующаяЗадача.Исполнитель = ЗНАЧЕНИЕ(Справочник.Пользователи.ПустаяСсылка)
			|			ИЛИ ДочерниеБизнесПроцессы.СвязующаяЗадача.Исполнитель = ЗНАЧЕНИЕ(Справочник.Сотрудники.ПустаяСсылка)
			|			ИЛИ ДочерниеБизнесПроцессы.СвязующаяЗадача.Исполнитель = НЕОПРЕДЕЛЕНО
			|			
			|			ТОГДА ДочерниеБизнесПроцессы.РодительскийПроцесс
			...
			|		ПО ДанныеДляРасчета3 = ДочерниеБизнесПроцессы.РодительскийПроцесс
			|;
			|
			|////////////////////////////////////////////////////////////////////////////////
			|ВЫБРАТЬ
			|	КОЛИЧЕСТВО(РАЗЛИЧНЫЕ ДанныеДляРасчета3) КАК КоличествоСтарыеДанныеДляРасчета
			|ИЗ
			|	ДанныеДляРасчета1 КАК ДанныеДляРасчета
			|;
			|
			...
			|ИЗ
			|	ДанныеДляРасчета2 КАК ДанныеДляРасчета
			|;
			|
			|////////////////////////////////////////////////////////////////////////////////
			|УНИЧТОЖИТЬ ДанныеДляРасчета1";
			
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "ДанныеДляРасчета1" , "ДанныеДляРасчета" + Формат(
			КоличествоЗапросовДочернихПроцессов, "ЧГ=0"));
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "ДанныеДляРасчета2" , "ДанныеДляРасчета" + Формат(
			КоличествоЗапросовДочернихПроцессов + 1, "ЧГ=0"));
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "ДанныеДляРасчета3" , "ДанныеДляРасчета." + ?(
			КоличествоЗапросовДочернихПроцессов = 0, "Ссылка", "Процесс"));
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "ДанныеДляРасчета4" , ?(
...
Функция АгрегатнаяФункция(ВариантРасчета)
	...
	Если СпособРасчета = Перечисления.СпособыРасчетаПоказателейПроцессов.Количество 
	 ИЛИ СпособРасчета = Перечисления.СпособыРасчетаПоказателейПроцессов.Сумма Тогда
		 СпособРасчета = "Сумма";
	ИначеЕсли СпособРасчета = Перечисления.СпособыРасчетаПоказателейПроцессов.Максимум Тогда
		 СпособРасчета = "Максимум";
	ИначеЕсли СпособРасчета = Перечисления.СпособыРасчетаПоказателейПроцессов.Минимум Тогда
		 СпособРасчета = "Минимум";
	ИначеЕсли СпособРасчета = Перечисления.СпособыРасчетаПоказателейПроцессов.Среднее Тогда
		 СпособРасчета = "Среднее";
	КонецЕсли;
...
Процедура ДобавитьОсобуюЛогикуВариантРасчета(ЗапросДанныеДляРасчета, ОператорВыбратьСхемыЗапроса, ВариантРасчета, НастройкиОтбора)
	...
	Если ВариантРасчета.ДанныеДляРасчета =
		...
		УсловиеСоединения = СтрШаблон(
			...
			|		ИЛИ %2.СвязующаяЗадача.Исполнитель = ЗНАЧЕНИЕ(Справочник.Сотрудники.ПустаяСсылка)
			|		ИЛИ %2.СвязующаяЗадача.Исполнитель = НЕОПРЕДЕЛЕНО)",
			ОсновнойИсточник.Источник.Псевдоним,
...
Функция ИмяИсточника(ТипПроцесса)
	...
	ИмяИсточника = СтрШаблон("БизнесПроцесс.%1", ТРегБезПробелов(ИмяМетаданного));
...
Функция ИмяТипаПредмета(ТипПредмета)
	...
	ИначеЕсли ТипПредмета = Перечисления.ТипыПредметовПоказателейПроцессов.ДокументыПредприятия Тогда
		ИмяБазовогоТипа = "Справочник";
		ИмяМетаданного = Метаданные.Справочники.ДокументыПредприятия.Имя;
...
//
// Параметры:
//  ВидПредмета - СправочникСсылка.ВидыДокументов, СправочникСсылка.ВидыМероприятий - Вид предмета.
//
// Возвращаемое значение:
...
Функция РеквизитВидаПредмета(ВидПредмета)
	...
	Если ТипВидаПредмета = Тип("СправочникСсылка.ВидыДокументов") Тогда
