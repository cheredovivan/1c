...
Процедура СтартоватьИЗакрыть(Форма) Экспорт
	...
	Иначе
		...
		Если ОбработкаОчередиЗаданийКлиент.ОбработатьЗапускПроцессаИзКарточки(Форма, ПараметрыЗаписи) Тогда
			Возврат;
		КонецЕсли;	
...
Процедура ПрерватьБизнесПроцессИзФормыОбъектаОкончание(Форма, Результат) Экспорт
	...
	ПротоколированиеРаботыСотрудниковВызовСервера.ЗаписатьПрерываниеБизнесПроцесса(ОбъектСсылка);
...
Процедура ПрерватьБизнесПроцесс_Продолжение(РезультатПрерывания, Процесс) Экспорт
	...
	ПротоколированиеРаботыСотрудниковВызовСервера.ЗаписатьПрерываниеБизнесПроцесса(Процесс);
