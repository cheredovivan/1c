...
// Определяет итоги затрат времен ежедневных отчетов.
// 
// Возвращаемое значение:
//  Структура - Итоги ежедневных отчетов.
//   * Учтено - Массив из Структура - Данные серии "Учтено". См. РаботаСВиджетами.ДанныеТочкиДиаграммы().
//   * Осталось - Массив из Структура - Данные серии "Осталось". См. РаботаСВиджетами.ДанныеТочкиДиаграммы().
//   * НетОтчета - Массив из Структура - Данные серии "НетОтчета". См. РаботаСВиджетами.ДанныеТочкиДиаграммы().
//
Функция ИтогиЕжедневныеОтчеты() Экспорт
	
	ИтогиЕжедневныеОтчеты = Новый Структура;
	ИтогиЕжедневныеОтчеты.Вставить("Учтено", Новый Массив);
	ИтогиЕжедневныеОтчеты.Вставить("Осталось", Новый Массив);
	ИтогиЕжедневныеОтчеты.Вставить("НетОтчета", Новый Массив);
	...
	Запрос = Новый Запрос(
		...
		|	ЕжедневныйОтчет.Ссылка КАК Отчет
		|ИЗ
		...
		|	И ЕжедневныйОтчет.Сотрудник В(&Сотрудники)");
	
	Запрос.УстановитьПараметр("Сотрудники", 
		Сотрудники.СотрудникиПользователя());
	Запрос.УстановитьПараметр("НачалоНедели", НачалоНедели(ТекущаяДатаСеанса()) - 2 * 86400);
	...
	НачалоОтсчета = НачалоНедели(ТекущаяДатаСеанса()) - 2 * 86400;
	
	// Заполнение точек графика
	Для ИндексТочки = 0 По 8 Цикл
		
		Период = НачалоОтсчета + ИндексТочки * 86400;
		
		ДанныеТочки = РаботаСВиджетами.ДанныеТочкиДиаграммы();
		ДанныеТочки.Ключ = Период;
		ДанныеТочки.ЗначениеТочки = Период;
		ДанныеТочки.Представление = НРег(Формат(Период, "ДФ=ddd"));
		ДанныеТочки.Значение = 0;
		ДанныеТочки.Расшифровка = Неопределено;
		ДанныеТочки.Подсказка = "";
		ИтогиЕжедневныеОтчеты.Учтено.Вставить(ИндексТочки, ДанныеТочки);
		...
		ДанныеТочки.Расшифровка = Неопределено;
		ДанныеТочки.Подсказка = "";
		ИтогиЕжедневныеОтчеты.Осталось.Вставить(ИндексТочки, ДанныеТочки);
		
		ДанныеТочки = РаботаСВиджетами.ДанныеТочкиДиаграммы();
		ДанныеТочки.Ключ = Период;
		ДанныеТочки.ЗначениеТочки = Период;
		ДанныеТочки.Представление = НРег(Формат(Период, "ДФ=ddd"));
		ДанныеТочки.Значение = 10;
		ДанныеТочки.Расшифровка = Период;
		ДанныеТочки.Подсказка = НСтр("ru = 'Нет отчета'");
		ИтогиЕжедневныеОтчеты.НетОтчета.Вставить(ИндексТочки, ДанныеТочки);
	...
	Пока Выборка.Следующий() Цикл
		...
		ДанныеТочки = РаботаСВиджетами.ДанныеТочкиДиаграммы();
		ДанныеТочки.Ключ = Выборка.Период;
		ДанныеТочки.ЗначениеТочки = Выборка.Период;
		ДанныеТочки.Представление = НРег(Формат(Выборка.Период, "ДФ=ddd"));
		ДанныеТочки.Значение = Окр(Учтено / 3600, 1);
		ДанныеТочки.Расшифровка = ПолучитьНавигационнуюСсылку(Выборка.Отчет);
		ДанныеТочки.Подсказка = СтрШаблон(
			НСтр("ru = 'Учтено %1'"),
			УчетВремениКлиентСервер.ЧислоВСтроку(Учтено));
		ИтогиЕжедневныеОтчеты.Учтено[ИндексТочки] = ДанныеТочки;
		
		ДанныеТочки = РаботаСВиджетами.ДанныеТочкиДиаграммы();
		ДанныеТочки.Ключ = Выборка.Период;
		ДанныеТочки.ЗначениеТочки = Выборка.Период;
		ДанныеТочки.Представление = НРег(Формат(Выборка.Период, "ДФ=ddd"));
		ДанныеТочки.Значение = Окр(ОсталосьУчесть / 3600, 1);
		ДанныеТочки.Расшифровка = ПолучитьНавигационнуюСсылку(Выборка.Отчет);
		ДанныеТочки.Подсказка = СтрШаблон(
			НСтр("ru = 'Осталось %1'"),
			УчетВремениКлиентСервер.ЧислоВСтроку(ОсталосьУчесть));
		ИтогиЕжедневныеОтчеты.Осталось[ИндексТочки] = ДанныеТочки;
		...
		ДанныеТочки.Представление = НРег(Формат(Выборка.Период, "ДФ=ddd"));
		ДанныеТочки.Значение = 0;
		ДанныеТочки.Расшифровка = Выборка.Период;
		ДанныеТочки.Подсказка = "";
		ИтогиЕжедневныеОтчеты.НетОтчета[ИндексТочки] = ДанныеТочки;
		
	КонецЦикла;
	
	Возврат ИтогиЕжедневныеОтчеты;
...
// Создает запись в регистре сведений ХронометражРабочегоВремениСотрудников
Процедура СоздатьЗаписьХронометража(Объект, ДатаНачала) Экспорт
	
	// Создать набор записей
	НаборЗаписей = РегистрыСведений.ХронометражРабочегоВремениСотрудников.СоздатьНаборЗаписей();
...
Процедура ОчиститьХронометраж(Объект) Экспорт
	...
	НаборЗаписей = РегистрыСведений.ХронометражРабочегоВремениСотрудников.СоздатьНаборЗаписей();
	НаборЗаписей.Отбор.Сотрудник.Установить(Сотрудники.ОсновнойСотрудник());
	НаборЗаписей.Отбор.Объект.Установить(Объект);
	НаборЗаписей.Записать();
...
Процедура ДобавитьВОтчет(ПараметрыОтчета, ПараметрыОповещения) Экспорт
	...
	Если ПараметрыОтчета.Свойство("Сотрудник") Тогда 
		Сотрудник = ПараметрыОтчета.Сотрудник;
	ИначеЕсли ПараметрыОтчета.Свойство("Пользователь") Тогда 
		Сотрудник = ПараметрыОтчета.Пользователь;
	Иначе 
		Сотрудник = Сотрудники.ОсновнойСотрудник();
	КонецЕсли;
	...
	Если ТипЗнч(Сотрудник) = Тип("СправочникСсылка.Пользователи") Тогда
		НаборЗаписей = РегистрыСведений.ХронометражРабочегоВремениСотрудников.СоздатьНаборЗаписей();
		НаборЗаписей.Отбор.Сотрудник.Установить(Сотрудники.ПользовательСотрудника(Сотрудник));
		НаборЗаписей.Отбор.Объект.Установить(ПараметрыОтчета.Объект);
		НаборЗаписей.Записать();
	КонецЕсли;
	...
	МенеджерЗаписи.НомерДобавления = МаксимальныйНомерДобавления(
		МенеджерЗаписи.Сотрудник, МенеджерЗаписи.ДатаДобавления);
	МенеджерЗаписи.Записать();
	
	ЗначенияКлюча = Новый Структура("Сотрудник, Источник, Проект, ПроектнаяЗадача, ВидРабот, 
		|ДатаДобавления, НомерДобавления, Начало, Окончание");
...
// находит первую запись в регистре ХронометражРабочегоВремениСотрудников с пустой датой окончания (00010101)
Процедура НайтиДатыХронометража(Объект, ДатаНачалаХронометража, ДатаКонцаХронометража) Экспорт
	...
	ДоступноЧтениеХронометража = ПравоДоступа("Чтение", Метаданные.РегистрыСведений.ХронометражРабочегоВремениСотрудников);
	
	Если ДоступноЧтениеХронометража = Ложь Тогда
		Возврат;
	КонецЕсли;
	...
	|ИЗ
	|	РегистрСведений.ХронометражРабочегоВремениСотрудников КАК Хронометраж
	|ГДЕ
	|	Хронометраж.Сотрудник = &ОсновнойСотрудник
	|	И Хронометраж.Объект = &Объект";
	
	Запрос.УстановитьПараметр("ОсновнойСотрудник", Сотрудники.ОсновнойСотрудник());
...
Процедура ПолучитьПараметрыХронометража(Объект, ВключенХронометраж, ДатаНачалаХронометража, ДатаКонцаХронометража, 
	...
	ИначеЕсли ТипЗнч(Объект) = Тип("ДокументСсылка.ДействиеЗадачи") Тогда
		...
		Если КэшОбластиПросмотра.Заполнен И КэшОбластиПросмотра.ДействиеЗадачи = Объект Тогда
			ИсточникДействияЗадачи = КэшОбластиПросмотра.РеквизитыДействияЗадачи.Источник;
		Иначе
			ИсточникДействияЗадачи = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Объект, "Источник");
		КонецЕсли;
		Если ТипЗнч(ИсточникДействияЗадачи) = Тип("ЗадачаСсылка.ЗадачаИсполнителя") Тогда
			ЗадачаПроцесса = ИсточникДействияЗадачи;
		КонецЕсли;
	КонецЕсли;
	...
	Если ЗначениеЗаполнено(ЗадачаПроцесса) Тогда
		...
		Если КэшОбластиПросмотра.Заполнен И КэшОбластиПросмотра.ЗадачаПроцесса = ЗадачаПроцесса Тогда
			ТочкаМаршрута = КэшОбластиПросмотра.РеквизитыЗадачиПроцесса.ТочкаМаршрута;
		Иначе
			ТочкаМаршрута = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ЗадачаПроцесса, "ТочкаМаршрута");
		КонецЕсли;
		ВидыРабот = ПолучитьВидыРабот(ТочкаМаршрута);
	КонецЕсли;
...
// Формирует параметры учета времени.
// 
// Параметры:
//  Ссылка - см. РегистрСведений.ХронометражРабочегоВремениСотрудников.Объект
// 
// Возвращаемое значение:
//  См. УчетВремениКлиентСервер.НовыйПараметрыУчетаВремени
// 
Функция ПараметрыУчетаВремени(Ссылка) Экспорт
	...
	ПроинициализироватьПараметрыУчетаВремени(
		...
		ПараметрыУчетаВремени.ОпцияИспользоватьУчетВремени,
		Ссылка,
	...
	ПараметрыУчетаВремени.ЭлементПереключитьХронометражПометка = ЭлементПереключитьХронометраж.Пометка;
	
	Возврат ПараметрыУчетаВремени;
...
Функция ПолучитьПерсональныеНастройкиУчетаВремениСервер() Экспорт
	...
	Если Не ЗначениеЗаполнено(ДобавлятьРаботуВЕжедневныйОтчетПриВыполненииЗадачи) Тогда
		...
		ОбщегоНазначения.ХранилищеОбщихНастроекСохранить("НастройкиУчетаВремени", 
			"ДобавлятьРаботуВЕжедневныйОтчетПриВыполненииЗадачи",
			ДобавлятьРаботуВЕжедневныйОтчетПриВыполненииЗадачи);
	КонецЕсли;
	
	Если Не ПолучитьФункциональнуюОпцию("ВестиУчетФактическихТрудозатрат")
		И Не ПолучитьФункциональнуюОпцию("ИспользоватьЕжедневныеОтчеты") Тогда
		ДобавлятьРаботуВЕжедневныйОтчетПриВыполненииЗадачи = Ложь;
	КонецЕсли;
...
Функция ЕстьЕжедневныеОтчетыНаДату(Сотрудник, НаДату) Экспорт 
...
Функция МаксимальныйНомерДобавления(Сотрудник, ДатаДобавления) Экспорт 
...
Функция ПолучитьДанныеАвтоподбораРабот(Текст, Сотрудник) Экспорт 
	...
	Пока Выборка.Следующий() Цикл
		...
		Параметр.Вставить("Начало", Дата(1, 1, 1));
		Параметр.Вставить("Окончание", Дата(1, 1, 1));
...
// Возвращает таблицу, содержащую фонд рабочего времени указанных сотрудников за период
// 
// Параметры:
//  СотрудникиИлиПользователи - СправочникСсылка.Сотрудники,СправочникСсылка.Пользователи,Массив - Сотрудник/Пользователь 
//  							или массив из них, по которым считаем фонд
//  ДатаНачала - Дата - начальня дата периода
//  ДатаОкончания - Дата - конечная дата периода
// 
// Возвращаемое значение:
//  ТаблицаЗначений - Фонд рабочего времени:
// * Сотрудник - СправочникСсылка.Сотрудники -
// * ДатаНачала - Дата
// * ДатаОкончания - Дата
// * Длительность - Число
Функция ФондРабочегоВремени(СотрудникиИлиПользователи, ДатаНачала, Знач ДатаОкончания) Экспорт
	
	Если ТипЗнч(СотрудникиИлиПользователи) = Тип("Массив") Тогда
		СотрудникиМассивом = СотрудникиИлиПользователи;
	ИначеЕсли ТипЗнч(СотрудникиИлиПользователи) = Тип("СправочникСсылка.Пользователи") Тогда
		СотрудникиМассивом = ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(
			Сотрудники.ОсновнойСотрудникПользователя(СотрудникиИлиПользователи));
	ИначеЕсли ТипЗнч(СотрудникиИлиПользователи) = Тип("СправочникСсылка.Сотрудники") Тогда
		СотрудникиМассивом = ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(СотрудникиИлиПользователи);
	Иначе
		ВызватьИсключение СтрШаблон(НСтр("ru = 'Неверный тип параметра (%1)'"), ТипЗнч(СотрудникиИлиПользователи));
	КонецЕсли;
	
	ДатаОкончания = КонецДня(ДатаОкончания);
	
	Фонд = Новый ТаблицаЗначений;
	Фонд.Колонки.Добавить("Сотрудник", Новый ОписаниеТипов("СправочникСсылка.Сотрудники"));
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьГрафикиРаботы") Тогда
		ЗаполнитьФондРабочимВременемСУчетомГрафиков(Фонд, СотрудникиМассивом, ДатаНачала, ДатаОкончания);
	Иначе
		ЗаполнитьФондРабочимВременемБезУчетаГрафиков(Фонд, СотрудникиМассивом, ДатаНачала, ДатаОкончания);
	КонецЕсли;
	...
	ОтсутствияПользователейЗаПериодНеВФонде = 
		ОтсутствияПользователейЗаПериодНеВФонде(СотрудникиМассивом, ДатаНачала, ДатаОкончания);
...
Процедура ПриПолученииДоступныхУведомлений(ДоступныеУведомления) Экспорт
	...
	СтрокаУведомления.Представление = НСтр("ru = 'Рабочее время: сводка'");
...
Процедура РассылкаСводокОРабочемВремени() Экспорт
	...
	ОбщегоНазначения.ПриНачалеВыполненияРегламентногоЗадания(
		Метаданные.РегламентныеЗадания.РассылкаСводокОРабочемВремени);
...
// Подсчитывает фактические трудозатраты по источнкиам.
//
// Параметры:
//  Источники - Массив из ЛюбаяСсылка - Источники.
// 
// Возвращаемое значение:
//  Число - Фактические трудозатраты по источникам (в секундах).
//
Функция ФактическиеТрудозатратыПоИсточникам(Источники) Экспорт
	
	УстановитьПривилегированныйРежим(Истина);
	
	Запрос = Новый Запрос(
		"ВЫБРАТЬ
		|	ЕСТЬNULL(СУММА(ФактическиеТрудозатраты.Длительность), 0) КАК ФактическиеТрудозатратыПоИсточникам
		|ИЗ
		|	РегистрСведений.ФактическиеТрудозатраты КАК ФактическиеТрудозатраты
		|ГДЕ
		|	ФактическиеТрудозатраты.Источник В(&Источники)");
	
	Запрос.УстановитьПараметр("Источники", Источники);
	
	Выборка = Запрос.Выполнить().Выбрать();
	
	Возврат ?(Выборка.Следующий(), Выборка.ФактическиеТрудозатратыПоИсточникам, 0);
...
//
Функция ОтсутствияПользователяЗаПериод(Отсутствия, Сотрудник, ДатаНачала, ДатаОкончания)
	ОтсутствияПользователяЗаПериод = Новый Массив;
	
	ОтсутствияПользователя = Отсутствия.НайтиСтроки(Новый Структура("Сотрудник", Сотрудник));
...
//
Процедура ЗаполнитьФондРабочимВременемСУчетомГрафиков(Фонд, Сотрудники, ДатаНачала, ДатаОкончания) Экспорт
...
//
Процедура ЗаполнитьФондРабочимВременемБезУчетаГрафиков(Фонд, Сотрудники, ДатаНачала, ДатаОкончания) Экспорт
...
Функция РабочиеЧасыПользователейЗаПериод(Пользователи, ДатаНачала, ДатаОкончания)
	...
	Запрос.УстановитьПараметр("Сотрудники", 
		Сотрудники.СотрудникиПользователей(Пользователи));
...
Функция ПодписчикиНаСводкиОРабочемВремени()
	...
	Запрос = Новый Запрос(
		"ВЫБРАТЬ РАЗЛИЧНЫЕ
		|	ВЫРАЗИТЬ(ПодписчикиИПолучателиУведомлений.Подписчик КАК Справочник.Пользователи) КАК Подписчик
		...
		|		ПО ПодчиненностьПодразделений.РуководительПодчиненного = ОсновныеСотрудники.Сотрудник
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Подписчики КАК Подписчики
		|		ПО (ОсновныеСотрудники.ФизическоеЛицо = Подписчики.Подписчик.ФизЛицо)
		|ГДЕ
		...
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	РуководимыеПодразделения1.Подписчик КАК Подписчик,
		...
		|ИЗ
		|	Справочник.СтруктураПредприятия КАК СтруктураПредприятия
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РуководимыеПодразделения КАК РуководимыеПодразделения1
		|		ПО СтруктураПредприятия.Родитель = РуководимыеПодразделения1.Подразделение
		|		ЛЕВОЕ СОЕДИНЕНИЕ РуководимыеПодразделения КАК РуководимыеПодразделения2
		|		ПО СтруктураПредприятия.Ссылка = РуководимыеПодразделения2.Подразделение
		|		И РуководимыеПодразделения1.Подписчик = РуководимыеПодразделения2.Подписчик
		...
		|ГДЕ
		|	НЕ СтруктураПредприятия.ПометкаУдаления
		|	И РуководимыеПодразделения2.Подразделение ЕСТЬ NULL
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		...
		|ИЗ
		|	РуководимыеПодразделения КАК РуководимыеПодразделения
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.Сотрудники КАК Сотрудники
		|		ПО РуководимыеПодразделения.Подразделение = Сотрудники.Подразделение
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.ОсновныеСотрудники КАК ОсновныеСотрудники
		|		ПО (Сотрудники.Ссылка = ОсновныеСотрудники.Сотрудник)
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.Пользователи КАК Пользователи
		|		ПО (ОсновныеСотрудники.ФизическоеЛицо = Пользователи.ФизЛицо)
		|		И (НЕ Пользователи.Недействителен)
		|		И (НЕ РуководимыеПодразделения.Подписчик = Пользователи.Ссылка)
		|ИТОГИ
		|ПО
		|	Подписчик,
		|	Подразделение
		...
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.Сотрудники КАК Сотрудники
		|		ПО ПодчиненныеПодразделения.ПодчиненноеПоИерархии = Сотрудники.Подразделение
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.ОсновныеСотрудники КАК ОсновныеСотрудники
		|		ПО (Сотрудники.Ссылка = ОсновныеСотрудники.Сотрудник)
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.Пользователи КАК Пользователи
		|		ПО (ОсновныеСотрудники.ФизическоеЛицо = Пользователи.ФизЛицо)
		|		И (НЕ Пользователи.Недействителен)
		|		И (НЕ ПодчиненныеПодразделения.Подписчик = Пользователи.Ссылка)
		|ИТОГИ
		|ПО
		|	Подписчик,
		|	Подразделение");
