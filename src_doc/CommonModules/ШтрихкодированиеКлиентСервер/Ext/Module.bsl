...
#Если НЕ ВебКлиент И Не МобильныйКлиент Тогда
	...
	Попытка
		...
		Если НаКлиенте Тогда
			Расширение = ОбщегоНазначенияДокументооборотВызовСервера.ЗначениеРеквизитаОбъекта(Объект, "ТекущаяВерсияРасширение"); 
		Иначе	
			Расширение = ОбщегоНазначенияДокументооборотВызовСервера.ЗначениеРеквизитаОбъекта(Объект, "ТекущаяВерсия.Расширение");
		КонецЕсли;	
...
// Настройки расположения - из нее наследуются см. НовыеНастройкиШтрихкода и см. НовыеНастройкиРегНомера
// 
// Возвращаемое значение:
//  Структура:
// * ПоложениеНаСтранице - ПеречислениеСсылка.МестаВставкиКартинки -
// 						 - Неопределено - 
// * СмещениеПоГоризонтали - Строка - 
// * СмещениеПоВертикали - Строка - 
Функция НовыйНастройкиРасположения() Экспорт
	
	Настройки = Новый Структура();
	Настройки.Вставить("ПоложениеНаСтранице", Неопределено);
	Настройки.Вставить("СмещениеПоГоризонтали", "");
	Настройки.Вставить("СмещениеПоВертикали", "");
	Возврат Настройки;
	
КонецФункции

// Создает структуру НастройкиШтрихкода
// 
// Возвращаемое значение:
//  Структура -  Новые настройки штрихкода:
// * ПоложениеНаСтранице - ПеречислениеСсылка.МестаВставкиКартинки -
// 						 - Неопределено - 
//  * СмещениеПоГоризонтали - Строка - Либо число строкой, либо вида "MAX", "MIN"
//  * СмещениеПоВертикали - Строка -
//  * ПоказыватьФормуНастройки - Булево -
//  * ВысотаШК - Число -
//  * ПоказыватьЦифры - Булево -
Функция НовыеНастройкиШтрихкода() Экспорт
	
	НастройкиШтрихкода = НовыйНастройкиРасположения();
	НастройкиШтрихкода.Вставить("ПоказыватьФормуНастройки", Ложь);
	НастройкиШтрихкода.Вставить("ВысотаШК", 0);
	НастройкиШтрихкода.Вставить("ПоказыватьЦифры", Ложь);
	Возврат НастройкиШтрихкода;
	
КонецФункции	

// Создает структуру НастройкиРегНомера
// 
// Возвращаемое значение:
//  Структура:
//  * ПоложениеНаСтранице - ПеречислениеСсылка.МестаВставкиКартинки -
...
//  * СмещениеПоГоризонтали - Строка - Либо число строкой, либо вида "MAX", "MIN"
//  * СмещениеПоВертикали - Строка - 
//  * СтраницаВставки - ПеречислениеСсылка.СтраницаВставкиКартинки, Число, Неопределено - 
Функция НовыеНастройкиРегНомера() Экспорт
	
	НастройкиРегНомера = НовыйНастройкиРасположения();
	НастройкиРегНомера.Вставить("СтраницаВставки", Неопределено);
	НастройкиРегНомера.Вставить("ПоказыватьПриВставке", Истина);
	Возврат НастройкиРегНомера;
...
Функция ВСтрокеТолькоЦифры(Строка) Экспорт 
	...
	ПроверяемоеЗначение = ВРег(Строка);
	
	Для Счетчик = 1 По СтрДлина(ПроверяемоеЗначение) Цикл
		Если СтрНайти(ВсеЦифры, Сред(ПроверяемоеЗначение, Счетчик, 1)) = 0 Тогда
			Результат = Ложь;
			Прервать;
		КонецЕсли;	
	КонецЦикла;
