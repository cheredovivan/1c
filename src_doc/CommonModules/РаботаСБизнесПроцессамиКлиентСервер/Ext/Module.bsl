...
Процедура ЗаполнитьОписаниеТрудозатрат(Форма, ПараметрыОписания,
	...
	Если ПараметрыОписания.Свойство("Исполнители") Тогда
		...
		ОбщиеТрудозатратыИсполнителей = 
			Объект[ИмяТаблицыИсполнители].Итог(ИмяРеквизитаТрудозатратыПланИсполнителя);
	...
	Если ПараметрыОписания.Свойство("ОбрабатывающийРезультат") Тогда
		
		СтрокаОбрабатывающийРезультат = НСтр("ru = 'Обрабатывающий результат - не указано'");
		
		Если ЗначениеЗаполнено(Объект.ТрудозатратыПланОбрабатывающегоРезультат) Тогда
			СтрокаОбрабатывающийРезультат = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Обрабатывающий результат - %1 %2'"),
				Формат(Объект.ТрудозатратыПланОбрабатывающегоРезультат, "ЧДЦ=1; ЧРД=; ЧГ="),
				Форма.ЕдиницаТрудозатрат);
		КонецЕсли;
		...
		ОбщееКоличествоТрудозатрат = ОбщееКоличествоТрудозатрат + Объект.ТрудозатратыПланОбрабатывающегоРезультат;
		
	КонецЕсли;
	
	Если ПараметрыОписания.Свойство("ОбрабатывающийРезолюцию") Тогда
		
		СтрокаАвтор = НСтр("ru = 'ОбрабатывающийРезолюцию - не указано'");
		
		Если ЗначениеЗаполнено(Объект.ТрудозатратыПланПомощника) Тогда
			СтрокаАвтор = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'ОбрабатывающийРезолюцию - %1 %2'"),
				Формат(Объект.ТрудозатратыПланПомощника, "ЧДЦ=1; ЧРД=; ЧГ="),
				Форма.ЕдиницаТрудозатрат);
		КонецЕсли;
		...
		ОбщееКоличествоТрудозатрат = ОбщееКоличествоТрудозатрат + Объект.ТрудозатратыПланПомощника;
...
// Возвращает тип строки "ПодписатьСУчастником" в дереве участников подписания. 
// 
// Возвращаемое значение:
// 	Строка
//
Функция ТипСтроки_ПодписатьСУчастником() Экспорт

	Возврат "ПодписатьСУчастником";

КонецФункции

// Возвращает тип строки "Подписать" в дереве участников подписания. 
// 
// Возвращаемое значение:
// 	Строка
//
Функция ТипСтроки_Подписать() Экспорт

	Возврат "Подписать";

КонецФункции

// Возвращает тип строки "ЭтапПодписания" в дереве участников подписания. 
// 
// Возвращаемое значение:
// 	Строка
//
Функция ТипСтроки_ЭтапПодписания() Экспорт

	Возврат "ЭтапПодписания";

КонецФункции

// Возвращает тип строки "УчастникЭтапаПодписания" в дереве участников подписания. 
// 
// Возвращаемое значение:
// 	Строка
//
Функция ТипСтроки_УчастникЭтапаПодписания() Экспорт

	Возврат "УчастникЭтапаПодписания";

КонецФункции

// Возвращает тип строки "ОбработатьРезультатПодписания" в дереве участников подписания. 
// 
// Возвращаемое значение:
// 	Строка
//
Функция ТипСтроки_ОбработатьРезультатПодписания() Экспорт

	Возврат "ОбработатьРезультатПодписания";

КонецФункции

// Заполняет представление строки этапа подписания в дереве участников.
//
// Параметры:
//  СтрокаЭтапа - ДанныеФормыЭлементДерева
//
Процедура ЗаполнитьПредставлениеСтрокиЭтапаПодписания(СтрокаЭтапа) Экспорт

	СтрокаЭтапа.ЭтапУчастник_Представление = СтрШаблон("%1 (%2)",
		СтрокаЭтапа.НаименованиеЭтапа,
		НРег(СтрокаЭтапа.ПорядокВыполненияУчастниками));

КонецПроцедуры

// Заполняет представление строки участника подписания в дереве участников.
//
// Параметры:
//  СтрокаУчастника - ДанныеФормыЭлементДерева
//
Процедура ЗаполнитьПредставлениеУчастникаПодписания(СтрокаУчастника) Экспорт

	ОтступСлева = "";
	РодительСтроки = СтрокаУчастника.ПолучитьРодителя();
	Если РодительСтроки <> Неопределено
		
		И РодительСтроки.ТипСтроки =
	...
	ЕстьУчастник = ЗначениеЗаполнено(СтрокаУчастника.Участник);
	ЕстьПодписывающий = ЗначениеЗаполнено(СтрокаУчастника.Подписывающий);
	
	Если ЕстьУчастник И ЕстьПодписывающий
		И СтрокаУчастника.Участник <> СтрокаУчастника.Подписывающий Тогда
		
		ПредставлениеПодписывающего = СтрШаблон("%1%2",
			ОтступСлева, Строка(СтрокаУчастника.Подписывающий)); 
		СтрокиПредставления.Добавить(ПредставлениеПодписывающего);
		
		ПредставлениеУчастника = СтрШаблон(
			НСтр("ru = '%1Обеспечивает подписание: %2'"),
			ОтступСлева,
			Строка(СтрокаУчастника.Участник)); 
		
		СтрокиПредставления.Добавить(ПредставлениеУчастника);		
		
	Иначе
		ПредставлениеУчастника = СтрШаблон("%1%2", ОтступСлева, Строка(СтрокаУчастника.Участник)); 
		СтрокиПредставления.Добавить(ПредставлениеУчастника); 
	КонецЕсли;
		
	СтрокаУчастника.ЭтапУчастник_Представление = СтрСоединить(СтрокиПредставления, Символы.ПС);
	
КонецПроцедуры

// Заполняет порядок исполнения в строке дерева Подписать.
// 
// Параметры:
// 	СтрокаПодписать - ДанныеФормыЭлементДерева
// 	СпособПодписания - ПеречислениеСсылка.СпособыПодписанияПредметаПроцесса
//
Процедура ЗаполнитьПорядокВыполненияВСтрокеСЗаданиемПодписать(
	СтрокаПодписать, СпособПодписания) Экспорт
	
	Если СпособПодписания = ПредопределенноеЗначение(
		"Перечисление.СпособыПодписанияПредметаПроцесса.НаБумаге") Тогда
		
		СтрокаПодписать.ПорядокВыполненияУчастниками = ПредопределенноеЗначение(
			"Перечисление.ПорядокВыполненияЭтапаПроцесса.Последовательно");
	
	Иначе
		
		СтрокаПодписать.ПорядокВыполненияУчастниками = ПредопределенноеЗначение(
			"Перечисление.ПорядокВыполненияЭтапаПроцесса.Параллельно");
		
	КонецЕсли;
	
КонецПроцедуры

// Заполняет процесс или шаблон подписания, по данным дерева участников. 
// 
// Параметры:
// 	ПроцессОбъект - ДанныеФормыСтруктура,
// 					Структура             - данные процесса или его шаблона
//                                          с таблицами Этапы и Участники.
// 	УчастникиПроцесса - ДанныеФормыКоллекцияЭлементовДерева - дерево участников в карточке.
// 	
Процедура ЗаполнитьПроцессПодписанияПоДеревуУчастников(ПроцессОбъект, УчастникиПроцесса) Экспорт
	
	ПроцессОбъект.Этапы.Очистить();
	ПроцессОбъект.Участники.Очистить();
	
	СтрокиДереваДляОбработки = Новый Массив;
	Для Каждого КорневойЭлемент Из УчастникиПроцесса.ПолучитьЭлементы() Цикл
		СтрокиДереваДляОбработки.Добавить(КорневойЭлемент);
	КонецЦикла;
	...
	Пока СтрокиДереваДляОбработки.Количество() > 0 Цикл
		...
		Если СтрокаДерева.ТипСтроки = РаботаСБизнесПроцессамиКлиентСервер.ТипСтроки_Подписать()
			
			Или СтрокаДерева.ТипСтроки = 
			РаботаСБизнесПроцессамиКлиентСервер.ТипСтроки_ЭтапПодписания() Тогда
			
			ПодчиненныеСтроки = СтрокаДерева.ПолучитьЭлементы();
			Индекс = ПодчиненныеСтроки.Количество() - 1;
			Пока Индекс >= 0 Цикл
				СтрокиДереваДляОбработки.Вставить(0, ПодчиненныеСтроки[Индекс]);
				Индекс = Индекс - 1;
			КонецЦикла;
		...
		Если СтрокаДерева.ТипСтроки = 
			...
			СтрокаЭтапа = ПроцессОбъект.Этапы.Добавить();
			ЗаполнитьЗначенияСвойств(СтрокаЭтапа, СтрокаДерева);
			СтрокаЭтапа.Идентификатор = СтрокаДерева.ИдентификаторЭтапа;
		...
		Если СтрокаДерева.ТипСтроки = 
			...
			Если ЗначениеЗаполнено(СтрокаДерева.Участник) Тогда
				...
				ЗаполнитьЗначенияСвойств(СтрокаУчастника, СтрокаДерева);
				СтрокаУчастника.Идентификатор = СтрокаДерева.ИдентификаторУчастника;
				СтрокаУчастника.Этап = СтрокаДерева.ИдентификаторЭтапа;
			КонецЕсли;
