...
Функция ПоказыватьPdfСредствами1С() Экспорт
	...
	Возврат РаботаСФайламиСлужебныйПовтИсп.ПоказыватьPdfСредствами1С();
...
Функция ДанныеФайлаИДвоичныеДанные(ФайлИлиВерсияСсылка, АдресПодписи = Неопределено, ИдентификаторФормы = Неопределено) Экспорт
	...
	Если ТипЗнч(ФайлИлиВерсияСсылка) = Тип("СправочникСсылка.Файлы") Тогда
		ВерсияСсылка = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ФайлИлиВерсияСсылка, "ТекущаяВерсия");
	...
	РеквВерсии = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ВерсияСсылка, "ТипХраненияФайла, Том, ПутьКФайлу");
	
	Если РеквВерсии.ТипХраненияФайла = Перечисления.ТипыХраненияФайлов.ВТомахНаДиске Тогда
...
Функция СоздатьВерсию(ФайлСсылка, СведенияОФайле) Экспорт
	...
	Если ТипЗнч(СведенияОФайле.АдресВременногоХранилищаТекста) = Тип("ХранилищеЗначения") Тогда
		...
		ПараметрыЗаписиИзвлеченияТекста.ИзвлеченныйТекст = ТекстХранилище;
...
Функция ЗанятьФайл(ДанныеФайла, СтрокаОшибки = "", 
	...
	Попытка
		...
		Если Не РаботаСФайламиПереопределяемый.ЗаписатьРедактирует(ДанныеФайла, ФайлОбъект) Тогда
		...
		Если ТипЗнч(ДополнительныеСвойства) = Тип("Структура") Тогда
			...
			Если ДополнительныеСвойства.Свойство("ДатаЗаема") Тогда
				ДатаЗаема = ДополнительныеСвойства.ДатаЗаема;
			КонецЕсли;	
	...
	Исключение	
		...
		ЗаписьЖурналаРегистрации(НСтр("ru = 'ЗанятьФайл'"),
			УровеньЖурналаРегистрации.Ошибка,
			,
			ДанныеФайла.Ссылка,
	...
	// именно ДО
	ДанныеФайла = РаботаСФайламиВызовСервера.ДанныеФайла(ДанныеФайла.Ссылка);
...
//
Функция ТехническаяИнформация(ИдентификаторКлиента) Экспорт
	...
	Результат.Вставить("КаталогЖурналаСканирования", КаталогЖурналаСканирования);
...
Функция ПолучитьДанныеФайлаДляОткрытияИЗанятьФайл(ФайлСсылка,
	ДанныеФайла, СтрокаОшибки, УникальныйИдентификатор = Неопределено, РабочийКаталогВладельца = Неопределено,
	...
	ЗаемФайла = Истина;                      
	// именно ДО ДанныеФайлаДляОткрытия
	ДанныеФайла = РаботаСФайламиВызовСервера.ДанныеФайлаДляОткрытия(ФайлСсылка, Неопределено, УникальныйИдентификатор, 
	...
	Если РаботаСФайламиПереопределяемый.ЭтоИсполняемыйФайлБинарный(ДанныеФайла.Расширение) Тогда
		Возврат Результат;
	КонецЕсли;	
	...
	Если Не ЗначениеЗаполнено(ДанныеФайла.Редактирует) Тогда
		...
		СтрокаОшибки = "";
		Если Не ЗанятьФайл(ДанныеФайла, СтрокаОшибки, УникальныйИдентификатор,, ДополнительныеСвойства) Тогда
			Возврат Результат;
		КонецЕсли;
	КонецЕсли;
...
Функция ДанныеФайлаДляОткрытия(ФайлИлиВерсияСсылка, ИдентификаторФормы = Неопределено,
	...
	// именно метод ДО
	ДанныеФайла = РаботаСФайламиВызовСервера.ДанныеФайла(ФайлСсылка, ВерсияСсылка);
	...
	Если ДанныеФайла.Версия <> Неопределено
		...
		// именно метод ДО (ПоместитьФайлВТомеВоВременноеХранилище)
		ДанныеФайла.НавигационнаяСсылкаТекущейВерсии 
			= РаботаСФайламиВызовСервера.ПоместитьФайлВТомеВоВременноеХранилище(ФайлСсылка, ВерсияСсылка, ДанныеФайла, ИдентификаторФормы);
...
Процедура ЗаписатьСтруктуруЗаписиВРегистр(Версия,
	...
	Если Не ЗначениеЗаполнено(НоваяЗапись.Версия) Тогда
		Возврат;
	КонецЕсли;	
...
Функция СведенияОФайлахВРабочемКаталоге(Знач ИменаФайлов) Экспорт
	...
	Блокировка = Новый БлокировкаДанных;
	ЭлементБлокировки = Блокировка.Добавить("РегистрСведений.ФайлыВРабочемКаталогеКомпьютера");
	...
	Попытка
		...
		Запрос.Текст =
		"ВЫБРАТЬ
		|	ФайлыВРабочемКаталоге.Версия КАК Файл,
		...
		|ИЗ
		|	РегистрСведений.ФайлыВРабочемКаталогеКомпьютера КАК ФайлыВРабочемКаталоге
		|ГДЕ
		|	ФайлыВРабочемКаталоге.Путь В(&ИменаФайлов)
		|	И ФайлыВРабочемКаталоге.Идентификатор = &Идентификатор";
...
//
Функция ПолучитьКоличествоЗанятыхФайлов(ВладелецФайла = Неопределено, 
	Редактирует = Неопределено, СотрудникиТекущегоПользователя = Неопределено) Экспорт
	...
	ДобавитьУсловиеРедактирует = Ложь;
	МассивРедактирует = Новый Массив;
	Если Редактирует <> Неопределено Тогда
		МассивРедактирует = Сотрудники.СотрудникиПользователя(Редактирует);
		ДобавитьУсловиеРедактирует = Истина;
	КонецЕсли;
	
	Если СотрудникиТекущегоПользователя <> Неопределено Тогда
		ОбщегоНазначенияКлиентСервер.ДополнитьМассив(
			МассивРедактирует, СотрудникиТекущегоПользователя,Истина);
		ДобавитьУсловиеРедактирует = Истина;
	КонецЕсли;
	
	Если ДобавитьУсловиеРедактирует Тогда
		Запрос.Текст = Запрос.Текст + " И Файлы.Редактирует В (&МассивРедактирует) ";
...
Процедура ДобавитьПодписьКФайлу(ФайлСсылка, СвойстваПодписи, ИдентификаторФормы) Экспорт
	...
	Попытка
		...
		Если ФайлСсылкаПодписан = Ложь Тогда
			...
			ФайлОбъект.ПодписанЭП = Истина;
			ФайлОбъект.ДополнительныеСвойства.Вставить(
...
Процедура ЗаписатьКодировкуВерсииФайлаИИзвлеченныйТекст(ВерсияСсылка, Кодировка, ИзвлеченныйТекст, Расширение) Экспорт
	...
	Файл = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ВерсияСсылка, "Владелец");	
...
Процедура ЗаписатьРезультатИзвлеченияТекста(ВерсияСсылка, РезультатИзвлечения, АдресВременногоХранилищаТекста, Расширение)
...
Функция РезультатУдаленияДанных(ФайлИлиВерсия, УникальныйИдентификатор) Экспорт
	...
	Автор = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ФайлИлиВерсия, "Автор");
	ПользовательИЕгоСотрудники = СотрудникиВызовСервера.ПользовательИЕгоСотрудники(Пользователи.ТекущийПользователь());
	АвторЭтоТекущийПользователь = (ПользовательИЕгоСотрудники.Найти(Автор) <> Неопределено);
	...
	Если Не ЭтоВерсияФайла Тогда
		
		ФайлОбъект = ФайлИлиВерсия.ПолучитьОбъект();
		
		СтрокаОшибки = "";
		Если Не РаботаСФайламиПереопределяемый.ВозможноУдалитьФайл(ФайлОбъект, СтрокаОшибки) Тогда
			Результат.ТекстПредупреждения = СтрокаОшибки;
			Возврат Результат;
		КонецЕсли;
...
Процедура УдалитьДанныеФайла(ФайлИлиВерсия, УникальныйИдентификатор, Результат, АвторизованныйПользователь)
	...
	Пока Выборка.Следующий() Цикл
		...
		АвторЭтоТекущийПользователь = (ПользовательИЕгоСотрудники.Найти(Выборка.Автор) <> Неопределено);
...
Процедура УдалитьДанные(ФайлИлиВерсия, УникальныйИдентификатор)
	...
	ПротоколированиеРаботыСотрудников.ЗаписатьУдалениеФайла(ФайлИлиВерсия);
	
	Если ТипЗнч(ФайлИлиВерсия) = Тип("СправочникСсылка.Файлы") Тогда
		РегистрыСведений.ПредпросмотрФайлов.Удалить(ФайлИлиВерсия);
	КонецЕсли;
