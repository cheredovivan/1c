...
Процедура ПереключитьВидПросмотра(Форма) Экспорт 
	...
	Если Элементы.Найти("Папка") <> Неопределено 
		И ИспользоватьПапкиДокументов Тогда
	...
	Если ИспользоватьПапкиДокументов
		И Элементы.Найти("СтраницаПапки") <> Неопределено Тогда
	...
	Если Элементы.Найти("СписокКонтекстноеМенюРежимПросмотраПоКатегориям") <> Неопределено Тогда 
		Элементы.СписокКонтекстноеМенюРежимПросмотраПоКатегориям.Пометка = Ложь;
	КонецЕсли;	
	Если Элементы.Найти("СписокКонтекстноеМенюРежимПросмотраПоПроектам") <> Неопределено Тогда 
		Элементы.СписокКонтекстноеМенюРежимПросмотраПоПроектам.Пометка = Ложь;
	КонецЕсли;	
	...
	ИначеЕсли ВидПросмотра = Перечисления.ВидыПросмотраСпискаОбъектов.ПоКатегориям Тогда
		...
		Если Форма.ИспользоватьКатегорииДанных Тогда	
			...
			Если Форма.ИмяФормы = "Справочник.ДокументыПредприятия.Форма.ФормаСпискаСПапками" Тогда 
				КлючОбъекта = "СписокДокументовПредприятия";
			КонецЕсли;	
...
//
// Параметры:
//   Документ - СправочникСсылка.ДокументыПредприятия - форма, в которой расположен список.
//
...
Процедура УстановитьДокументамПометкуУдаленияСервер(Документы, Пометка) Экспорт 
	...
	Для каждого Документ Из Документы Цикл
		Права = ДокументооборотПраваДоступа.ПраваПользователяПоОбъекту(Документ);
...
//
Функция ДанныеВыбораСтороны(Текст, ДопустимыОрганизации, ДопустимыКонтрагенты, ДопустимыСотрудники) Экспорт
	
	ДанныеВыбора = Новый СписокЗначений;
	
	Если Не ДопустимыОрганизации
		И Не ДопустимыКонтрагенты
		И Не ДопустимыСотрудники Тогда
	...
	Если ДопустимыКонтрагенты Тогда
		Подзапросы.Добавить(СтрЗаменить(
			...
			?(Подзапросы.Количество() = 0, "РАЗРЕШЕННЫЕ", "")));
	КонецЕсли;
	Если ДопустимыСотрудники Тогда
		Подзапросы.Добавить(СтрЗаменить(
			"ВЫБРАТЬ РАЗРЕШЕННЫЕ ПЕРВЫЕ 20
...
// Устанавливает статус обмена с 1С:Архивом документам.
//
// Параметры:
//   Документы - Массив - ДокументСсылка.ПередачаДелВАрхив - документы, для которых нужно установить статус обмена.
//   Пометка - ПеречислениеСсылка.СтатусыОбменаСАрхивом - устанавливаемый статус.
//
Функция УстановитьДокументамСтатусОбменаСАрхивом(Документы, СтатусОбмена) Экспорт 
	...
	Для Каждого Документ Из Документы Цикл
		...
		Если Не Права.Изменение Тогда
			СообщениеОбОшибке = СтрШаблон(
				НСтр("ru = 'У вас нет прав на изменение документа ""%1"".'"),
				Строка(Документ));
			Результат = Результат + Символы.ПС + СообщениеОбОшибке;
			Продолжить;
		КонецЕсли;
		
		СписокСтатусов = АрхивныеОперацииСервер.ПолучитьСписокВыбораСтатусовВыгрузкиВАрхив(Документ);
		Если СписокСтатусов.Найти(СтатусОбмена) = Неопределено Тогда
			ТекущийСтатусДокумента = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Документ, "СтатусВыгрузкиВАрхив");
			СообщениеОбОшибке = СтрШаблон(
				НСтр("ru = 'Нельзя присвоить статус ""%1"" документу %2 в статусе ""%3"".'"),
				Строка(СтатусОбмена),
				Строка(Документ),
		...
		ЗаблокироватьДанныеДляРедактирования(Документ);
		ДокументОбъект = Документ.ПолучитьОбъект();
		ДокументОбъект.СтатусВыгрузкиВАрхив = СтатусОбмена;
		Если СтатусОбмена = Перечисления.СтатусыОбменаСАрхивом.ПринятВАрхив Тогда
...
// Заполняет данные ПриПолученииДанныхНаСервере, для списка документов и формы Категорий
//
// Параметры:
//   ИмяЭлемента - Строка
