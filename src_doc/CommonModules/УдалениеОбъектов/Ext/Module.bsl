...
Процедура УдалитьПомеченныеОбъекты(
	...
	Найденные.Колонки.Добавить("ПометкаУдаленияДанных", Новый ОписаниеТипов("Булево"));
	...
	СсылочныеОбъекты = Новый Массив();
	Для Каждого Найденный Из Найденные Цикл
		Если ОбщегоНазначения.ЗначениеСсылочногоТипа(Найденный.Данные) Тогда
			СсылочныеОбъекты.Добавить(Найденный.Данные);
		КонецЕсли;
	КонецЦикла;
	ПометкиПоСсылкам = ОбщегоНазначения.ЗначениеРеквизитаОбъектов(СсылочныеОбъекты, "ПометкаУдаления"); //@skip-check bsl-legacy-check-string-literal
	Для Каждого Найденный Из Найденные Цикл
		Если ПометкиПоСсылкам[Найденный.Данные] = Истина Тогда
...
Функция УдалитьОбъект(
	...
	Для Каждого СтрокаСсылки Из ТаблицаСсылок Цикл    
		...
		Если ОбщегоНазначения.ЭтоКонстанта(СтрокаСсылки.Метаданные) Тогда
			//Если константы хранит в себе удаляемый объект, то установим для нее пустую ссылку.
			Если ОбщегоНазначения.ЭтоСсылка(ТипЗнч(СтрокаСсылки.Ссылка)) Тогда
				МенеджерЗначения = Константы[СтрокаСсылки.Метаданные.Имя].СоздатьМенеджерЗначения();
				МенеджерЗначения.Значение = ОбщегоНазначения.МенеджерОбъектаПоСсылке(СтрокаСсылки.Ссылка).ПустаяСсылка();
				ОбновлениеИнформационнойБазы.ЗаписатьДанные(МенеджерЗначения);
			КонецЕсли;
		ИначеЕсли Результат И ОбщегоНазначения.ЭтоОбъектСсылочногоТипа(СтрокаСсылки.Метаданные) Тогда
	...
	Для Каждого СтрокаСсылки Из ТаблицаСсылок Цикл
		...
		Если СтрНачинаетсяС(СтрокаСсылки.ИмяРегистраСведений, "ОтметкиВремени")
			Или СтрНачинаетсяС(СтрокаСсылки.ИмяРегистраСведений, "МП_ОтправленныеСсылочныеДанные") Тогда 
			Продолжить;
		КонецЕсли;
...
Процедура УдалитьДанныеСвязанныхРегистров(УдаляемыеОбъекты)
	...
	ИмяРегистра = "УчастникиПроцессов";
