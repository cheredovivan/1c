...
Процедура ОткрытьФормуПравДоступа(ФормаОбъекта, ИмяОсновногоРеквизита = "Объект") Экспорт
	...
	ЗаписатьОбъектЕслиНовыйИОткрытьПодчиненнуюФорму(
...
Процедура ОбновитьВсеПрава(ОтложенноеОбновлениеПрав) Экспорт
	...
	ДокументооборотПраваДоступаВызовСервера.ОбновитьПраваВсехДанных();
...
Функция ЗаголовокИдетПоискЗапрещенныхУчастников() Экспорт
	
	Возврат НСтр("ru = 'Идет поиск объектов с нарушением доступа...'");
	
КонецФункции

Функция ЗаголовокКолвоЗапрещенныхУчастников(КолвоОбъектов) Экспорт
	
	СтрокаОбнаружено = СтрокаСЧислом(
		НСтр("ru = ';Обнаружен %1 объект;;
		|Обнаружено %1 объекта;Обнаружено %1 объектов;
		|Обнаружено %1 объектов'"),
		КолвоОбъектов, ВидЧисловогоЗначения.Количественное);
	Возврат СтрШаблон(НСтр("ru = '%1 с запрещенными участниками доступа'"), СтрокаОбнаружено);
