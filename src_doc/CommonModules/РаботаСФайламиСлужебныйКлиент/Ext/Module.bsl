...
Процедура СохранитьФайлВместеСПодписью(Файл, ИдентификаторФормы) Экспорт
	...
	ДанныеФайла = РаботаСФайламиВызовСервера.ДанныеФайлаДляСохранения(Файл, , ИдентификаторФормы);
	...
	Если ДанныеФайла.ТребуетсяСоздатьВизуализациюЭП
		...
		#Если Не ВебКлиент И Не МобильныйКлиент Тогда
			...
			РаботаСФайламиКлиент.СоздатьВизуализациюЭПВФайлеDocXls(ДанныеФайла, ИдентификаторФормы);
		#КонецЕсли	
...
Функция ДобавитьИзФайловойСистемыСРасширениемСинхронно(ПараметрыВыполнения) Экспорт
	...
	ФильтрДиалога = ?(ПараметрыВыполнения.Свойство("ФильтрДиалогаВыбора"),
		ПараметрыВыполнения.ФильтрДиалогаВыбора, 
	...
	ФайловыеФункцииСлужебныйКлиентСервер.ПроверитьВозможностьЗагрузкиФайла(ДобавляемыйФайл);
	...
	Иначе
		ИмяСоздания = ДобавляемыйФайл.ИмяБезРасширения;
	КонецЕсли;
	
	ИмяФайла  = ИмяСоздания + ДобавляемыйФайл.Расширение;
	РазмерВМб = ДобавляемыйФайл.Размер() / (1024 * 1024);
...
// Вернет структуру ПараметрыОповещения    
// Параметры:
// Событие - Строка
//    
// Возвращаемое значение:
...
// Сделает Оповестить
// Параметры:
// Файлы - Массив из Файл, Файл
// ПараметрыОповещенияЗаписиФайла  -Структура 
//    
...
// Запишет в журнал регистрации - отложенно, на клиенте добавит в массив
//
// Параметры:
// Событие - Строка
// Комментарий - Строка
...
// Запишет на диск zip с файлами для техподдержки
//
// Параметры:
// ПодробноеПредставлениеОшибки - Строка
// ОповещениеПослеПолученияТехническойИнформации - ОписаниеОповещения
//    
Процедура ПолучитьТехническуюИнформацию(ПодробноеПредставлениеОшибки, ОповещениеПриЗавершении) Экспорт
...
// Продолжение  ПолучитьТехническуюИнформацию
//
// Параметры:
// ИмяКаталога - Строка
// Контекст - Структура
...
#Если Не ВебКлиент Тогда	
	...
	ТехническаяИнформация = РаботаСФайламиСлужебныйВызовСервера.ТехническаяИнформация(ИдентификаторКлиента);
	
	ФайлыЛога = НайтиФайлы(ТехническаяИнформация.КаталогЖурналаСканирования, "*");
	Если ФайлыЛога.Количество() <> 0 Тогда
		ТехническаяИнформация.Вставить("ИмяЛогФайла", ФайлыЛога[0].ПолноеИмя);
	Иначе
		ТехническаяИнформация.Вставить("ИмяЛогФайла", Неопределено);
	КонецЕсли;	
	...
	РезультатИнициализации = Новый Структура("Подключено, ПодключаемыйМодуль",
		Истина, ПараметрыПриложения["СтандартныеПодсистемы.КомпонентаTwain"]);
	СводнаяИнформацияПослеПолученияКомпоненты(РезультатИнициализации, Контекст);
	
#КонецЕсли
КонецПроцедуры

// Продолжение после подключения компоненты
// Параметры:
// РезультатИнициализации - Структура 
// Контекст - Структура
//    
...
// Сделает ВыполнитьОбработкуОповещения
// Параметры:   
// Результат - Булево
// Контекст - Структура    
...
// Вернет структуру ПараметрыОповещения    
// Параметры:
// ПодключаемыйМодуль - ВнешняяКомпонента                                   
// ОповещениеЗавершения  - ОписаниеОповещения
// ОчиститьЖурналКомпоненты - Булево
//    
Процедура УдалитьОшибкуСканирования(ПодключаемыйМодуль, ОповещениеЗавершения = Неопределено, ОчиститьЖурналКомпоненты = Истина) Экспорт
	...
	ОбщегоНазначенияКлиент.ХранилищеОбщихНастроекСохранить("КомпонентаСканирования", "ПараметрыЗаданияСканирования", Неопределено);
...
Процедура ЗакончитьРедактированиеБезРасширения(ПараметрыВыполнения)
	// Веб-клиент без расширения для работы с файлами 1С:Предприятием.
...
Процедура ОбновитьИзФайлаНаДискеПослеУдаленияФайлаИзРабочегоКаталога(Результат, ПараметрыВыполнения) Экспорт
	...
	Обработчик = Новый ОписаниеОповещения("ОбновитьИзФайлаНаДискеПослеПолученияФайлаВРабочийКаталог", 
		ЭтотОбъект, ПараметрыВыполнения);
...
Процедура ПолучитьССервераИЗарегистрироватьВЛокальномКэшеФайловНачатьВыборКаталога(Ответ, ПараметрыВыполнения) Экспорт
	...
	ПараметрыВыполнения.ПолноеИмяФайла = ПараметрыВыполнения.ИмяКаталога 
		+ ФайловыеФункцииСлужебныйКлиентСервер.ПолучитьУникальноеИмяСПутем(
		ПараметрыВыполнения.ИмяКаталога,
		ПараметрыВыполнения.ИмяФайла);
...
Процедура СохранитьКакСРасширениемПослеРасшифровки(ОписаниеДанных, ПараметрыВыполнения) Экспорт
	...
	Иначе
		...
		Если ПараметрыВыполнения.ДанныеФайла.Свойство("ВизуализацияЭПДобавлена")
			И ПараметрыВыполнения.ДанныеФайла.ВизуализацияЭПДобавлена
			И ПараметрыВыполнения.ДанныеФайла.Свойство("НавигационнаяСсылкаТекущейВерсииОригинальная") Тогда
			АдресФайла = ПараметрыВыполнения.ДанныеФайла.НавигационнаяСсылкаТекущейВерсииОригинальная;
		КонецЕсли;	
	...
	Если ПараметрыВыполнения.ДанныеФайла.Свойство("РасширениеОригинальное") Тогда
		Расширение = ПараметрыВыполнения.ДанныеФайла.РасширениеОригинальное;
	КонецЕсли;	

	Если ПараметрыВыполнения.ДанныеФайла.Свойство("РасширениеОригинальноеБезВизуализации") Тогда
		Расширение = ПараметрыВыполнения.ДанныеФайла.РасширениеОригинальноеБезВизуализации;
	КонецЕсли;	
	...
	СтруктураВозврата = Новый Структура("ПолноеИмяФайла", ПолноеИмяФайла);
	
	// Тут сохраним и файл визуализации
	Если ПараметрыВыполнения.ДанныеФайла.Свойство("ВизуализацияЭПДобавлена") 
		И ПараметрыВыполнения.ДанныеФайла.ВизуализацияЭПДобавлена Тогда
		
		АдресФайла = ПараметрыВыполнения.ДанныеФайла.НавигационнаяСсылкаТекущейВерсии;
		Расширение = ПараметрыВыполнения.ДанныеФайла.Расширение;
		
		МаксДлинаИмени = 255;
		ПолноеНаименованиеВерсииДляСохранения = ПараметрыВыполнения.ДанныеФайла.ПолноеНаименованиеВерсии;
		ДобавкаШтампа = НСтр("ru = ' (со штампом ЭП)'");
		ВсяДлина = СтрДлина(Файл.Путь + ПолноеНаименованиеВерсииДляСохранения + "." + ПараметрыВыполнения.ДанныеФайла.Расширение + ДобавкаШтампа);
		Если ВсяДлина > МаксДлинаИмени Тогда
			НаСколькоСократить = ВсяДлина - МаксДлинаИмени;
			ДлинаИмени = СтрДлина(ПолноеНаименованиеВерсииДляСохранения);
			НоваяДлинаИмени = Макс(ДлинаИмени - НаСколькоСократить, 8);
			ПолноеНаименованиеВерсииДляСохранения = Лев(ПолноеНаименованиеВерсииДляСохранения, НоваяДлинаИмени);
		КонецЕсли;	
		
		ИмяСРасширением = ОбщегоНазначенияКлиентСервер.ПолучитьИмяСРасширением(
			ПолноеНаименованиеВерсииДляСохранения + ДобавкаШтампа, 
			ПараметрыВыполнения.ДанныеФайла.Расширение);
		
		ПолноеИмяФайлаСоШтампом = Файл.Путь + ИмяСРасширением; 
		
		Файл = Новый Файл(ПолноеИмяФайлаСоШтампом);

		СтруктураВозврата.Вставить("ПолноеИмяФайлаСоШтампом", Файл.Имя);
		
		Если Файл.Существует() Тогда
			Попытка
				Файл.УстановитьТолькоЧтение(Ложь);
				УдалитьФайлы(ВыборФайла.ПолноеИмяФайлаСоШтампом);
			Исключение
				ЖурналРегистрацииКлиент.ДобавитьСообщениеДляЖурналаРегистрации(СобытиеЖурналаРегистрации(),
					"Предупреждение", ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()),, Истина);
			КонецПопытки;	
		...
		ПередаваемыеФайлы = Новый Массив;
		Описание = Новый ОписаниеПередаваемогоФайла(ПолноеИмяФайлаСоШтампом, АдресФайла);
		ПередаваемыеФайлы.Добавить(Описание);
		...
		Если ПолучитьФайлы(ПередаваемыеФайлы,, ПутьКФайлу, Ложь) Тогда
			...
			УстановитьУниверсальноеВремяИзменения(ПолноеИмяФайлаСоШтампом, 
				ПараметрыВыполнения.ДанныеФайла.ДатаМодификацииУниверсальная);
	...
	ВернутьРезультат(ПараметрыВыполнения.ОбработчикРезультата, СтруктураВозврата);
...
Процедура ОткрытьФайлБезРасширения(Оповещение, ДанныеФайла, ИдентификаторФормы,
	...
	Если ДанныеФайла.Свойство("ВизуализацияЭПДобавлена")
		И ДанныеФайла.ВизуализацияЭПДобавлена
		И ДанныеФайла.Свойство("НавигационнаяСсылкаТекущейВерсииОригинальная") Тогда
			
		Контекст.АдресФайла = ДанныеФайла.НавигационнаяСсылкаТекущейВерсииОригинальная;
...
Процедура ОткрытьФайлБезРасширенияПередачаФайла(Результат, Контекст) Экспорт
	...
	Иначе
		...
		Если Контекст.ДанныеФайла.Свойство("РасширениеОригинальное") Тогда
			Расширение = Контекст.ДанныеФайла.РасширениеОригинальное;
		КонецЕсли;	
	...
	// Тут сохраним и файл визуализации
	Если Контекст.ДанныеФайла.Свойство("ВизуализацияЭПДобавлена")
		И Контекст.ДанныеФайла.ВизуализацияЭПДобавлена Тогда
		
		АдресФайла = Контекст.ДанныеФайла.НавигационнаяСсылкаТекущейВерсии;
		Расширение = Контекст.ДанныеФайла.Расширение;
		
		ИмяСРасширением = ОбщегоНазначенияКлиентСервер.ПолучитьИмяСРасширением(
			Контекст.ДанныеФайла.ПолноеНаименованиеВерсии + НСтр("ru = ' (со штампом ЭП)'"), 
			Контекст.ДанныеФайла.Расширение);
...
Процедура СравнениеВерсийАвтомат(Результат, ПараметрыВыполнения) Экспорт
	...
	Если ПараметрыВыполнения.ТекущийШаг = 5 Тогда
		Если ПараметрыВыполнения.Результат1 И ПараметрыВыполнения.Результат2 Тогда
			...
			Если ДанныеПервогоФайла.Расширение = "txt" И ПараметрыВыполнения.СпособСравненияВерсийФайлов = "Р7Builder" Тогда
...
Процедура ВыполнитьСравнениеФайлов(ПутьКФайлу1, ПутьКФайлу2, СпособСравненияВерсийФайлов, ЗаголовокЛевый = "", ЗаголовокПравый = "") Экспорт
	...
	Попытка
		...
		ИначеЕсли СпособСравненияВерсийФайлов = "Р7Builder" Тогда
			РаботаСФайламиКлиент.СравнитьФайлыР7Builder(ПутьКФайлу1, ПутьКФайлу2);
...
Процедура СравнитьТабличныеДокументы(ПутьКФайлу1, ПутьКФайлу2, ЗаголовокЛевый, ЗаголовокПравый) Экспорт
...
Процедура ДобавитьФайлыРасширениеПредложено(Расширение1СПредприятияПодключено, ДополнительныеПараметры) Экспорт
...
Функция ПроверитьВозможностьЗагрузкиФайла(Файл, ВызыватьИсключение = Истина, ФайлыСОшибками = Неопределено)
	...
	РазмерВМбМакс = ОбщиеНастройки.МаксимальныйРазмерФайла / (1024 * 1024);
	РазмерФайлаПревышаетМаксимальноДопустимый = Файл.Размер() > ОбщиеНастройки.МаксимальныйРазмерФайла;
	
	// Локализация
	Если РазмерФайлаПревышаетМаксимальноДопустимый
		И ТаймлистКлиентСервер.ЭтоРасширениеФайлаТаймлист(Файл.Расширение, Истина)
