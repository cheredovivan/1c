...
Процедура УстановитьДоступностьКомандРаботыСЭлементамиКомплекта(Форма)
	...
	УстановитьДоступностьКоманды(ДоступныеПоля, Элементы.СоставКомплектаПодобратьДокументыПредприятия, 
		Не ТолькоПросмотр И Найти(Форма.ПраваДоступа, "ДокументыПредприятия.Чтение") > 0);
...
Процедура СоставКомплектаВыбор(Форма, Элемент, Поле, СтандартнаяОбработка) Экспорт
	...
	Если ЭтоДокумент(Ссылка) Тогда  
		...
		Если Поле = Форма.Элементы.СоставКомплектаОбработка Тогда  
			...
			ДелопроизводствоКлиент.ОткрытьДокументПредприятия(Ссылка, ПараметрыОткрытия, Элемент);
			Возврат;
		КонецЕсли;
...
// Обработчик команд "СоставКомплектаПодобратьДокументыПредприятия", ...
// Параметры:
// - Форма (ФормаКлиентскогоПриложения)
// - Тип (Строка): "ДокументыПредприятия" | "Файлы"
//
Процедура СоставКомплектаПодобрать(Форма, Тип = Неопределено) Экспорт
	
	Если Не ЭтоДокументПредприятия(Форма.Объект.Ссылка) Тогда
		Возврат;
	КонецЕсли;
	...
	Если Тип = Неопределено Тогда
		...
		Если Найти(Форма.ПраваДоступа, "ДокументыПредприятия.Чтение") > 0 Тогда
			Меню.Добавить("ДокументыПредприятия", НСтр("ru = 'Подобрать документы'"));
		КонецЕсли;
...
Процедура СоставКомплектаПодобратьПродолжение(Результат, Параметры) Экспорт
	...
	Если ВРег(Тип) = ВРег("ДокументыПредприятия") Тогда
		...
		ПараметрыОткрытия.Вставить("Заголовок", НСтр("ru = 'Подбор документов в комплект'"));
		ОткрытьФорму("Справочник.ДокументыПредприятия.Форма.ФормаВыбора", ПараметрыОткрытия, Форма.Элементы.СоставКомплекта);
...
Процедура ЭлементКомплектаОткрытьКарточку(Форма) Экспорт
	...
	Если Не ЗначениеЗаполнено(Форма.Элементы.СоставКомплекта.ТекущиеДанные.Ссылка) Тогда	
		Возврат;
	КонецЕсли;
...
Процедура ДобавитьВКомплект(ВыделенныеСтроки) Экспорт
	...
	ОткрытьФорму("Справочник.ДокументыПредприятия.ФормаВыбора", ПараметрыОткрытия,,,,,
		ОписаниеОповещения,
		РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
...
Процедура ДобавитьВКомплектПродолжение(КомплектДокументов, Параметры) Экспорт
	...
	Если КомплектДокументов <> Неопределено Тогда
		...
		ДелопроизводствоКлиент.ОткрытьДокументПредприятия(КомплектДокументов, ПараметрыОткрытия);
	КонецЕсли;
...
Функция ЭтоДокументПредприятия(Значение)
