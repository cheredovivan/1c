...
Процедура ОбновлениеИнформационнойБазыПерейтиНаВерсию_1_2_1_6() Экспорт
	...
	Выборка = Справочники.ШаблоныДокументов.Выбрать();
	Пока Выборка.Следующий() Цикл
		Объект = Выборка.ПолучитьОбъект();
		Если ЗначениеЗаполнено(Объект.Автор) Тогда
			Объект.Ответственный = 
				Сотрудники.ОсновнойСотрудникПользователя(Объект.Автор);
			Объект.Записать();
		КонецЕсли;
	КонецЦикла;
...
Процедура ПроверитьМаксимальноеЧислоЗаписейИУдалить()
	...
	Запрос.Текст = 
		...
		|УПОРЯДОЧИТЬ ПО
		|	ДатаИспользования";
	Запрос.Текст = СтрЗаменить(Запрос.Текст, "1", Формат(СколькоЗаписейНадоУдалить, "ЧГ="));
	Запрос.УстановитьПараметр("Пользователь", Пользователи.ТекущийПользователь());
	Таблица = Запрос.Выполнить().Выгрузить();
...
Процедура УстановитьТипыМестаИОтветственногоЗаХранение(Форма) Экспорт 
	...
	Если Не ЗначениеЗаполнено(Объект.ОтветственныйЗаХранение) Тогда 
		Объект.ОтветственныйЗаХранение = Справочники.Сотрудники.ПустаяСсылка();
	КонецЕсли;
