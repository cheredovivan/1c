...
Функция ЗаголовокПомощника(Помощник) Экспорт
	...
	ИначеЕсли ПомощникОтправить.ИспользуетсяРежимОтправкиЗадачи(Помощник) Тогда
...
Функция ПараметрыВарианта(Вариант, Помощник) Экспорт
	...
	ИначеЕсли ПомощникОтправитьКлиентСервер.ЭтоВариантСозданияПоручение(Вариант) Тогда
		Параметры = ПомощникОтправить.ПараметрыВариантаСозданияПоручения(Вариант, Помощник);
	ИначеЕсли ПомощникОтправитьКлиентСервер.ЭтоВариантСозданияОзнакомления(Вариант) Тогда
		Параметры = ПомощникОтправить.ПараметрыВариантаСозданияОзнакомления(Вариант, Помощник);
	ИначеЕсли ПомощникОтправитьКлиентСервер.ЭтоВариантСозданияПоШаблону(Вариант) Тогда
		Параметры = ПомощникОтправить.ПараметрыВариантаСозданияПоШаблону(Вариант, Помощник);
	КонецЕсли;
	
	ПомощникОтправитьКорпЛокализация.ПриОпределенииПараметровВарианта(Вариант, Помощник, Параметры);
...
Функция ОписаниеВарианта(Вариант) Экспорт
	...
	ИначеЕсли ПомощникОтправитьКлиентСервер.ЭтоВариантСозданияПоручение(Вариант) Тогда
		ОписаниеВарианта = ПомощникОтправить.ОписаниеВариантаСозданияПоручение(Вариант);
	ИначеЕсли ПомощникОтправитьКлиентСервер.ЭтоВариантСозданияОзнакомления(Вариант) Тогда
		ОписаниеВарианта = ПомощникОтправить.ОписаниеВариантаСозданияОзнакомление(Вариант);
	ИначеЕсли ПомощникОтправитьКлиентСервер.ЭтоВариантСозданияПоШаблону(Вариант) Тогда
		ОписаниеВарианта = ПомощникОтправить.ОписаниеВариантаСозданияПоШаблону(Вариант);
	ИначеЕсли ПомощникОтправитьКлиентСервер.ЭтоВариантСозданияОбработки(Вариант) Тогда
		ОписаниеВарианта = ОписаниеВариантаСозданияОбработки();
	КонецЕсли;
	
	ПомощникОтправитьКОРПЛокализация.ПриОпределенииОписанияВарианта(Вариант, ОписаниеВарианта);
...
//  Булево
//
Функция ИспользуетсяРежимРаботыОтправкаПроектовИлиПроектныхЗадач(Помощник)
...
//  Булево
//
Функция ИспользуетсяРежимРаботыВыборВариантаОтправкиДляДействияКомплексногоПроцесса(Помощник)
...
// Вызывается из ПомощникОтправитьПереопределяемый.ЗаполнитьДеревоВариантовОтправки при
...
Процедура ПриЗаполненииДереваВариантовОтправки(Помощник, СтандартнаяОбработка) Экспорт
	...
	ПомощникОтправить.ДобавитьВДеревоВариантыСозданияЗадач(Помощник);
	ПомощникОтправить.ДобавитьВДеревоВариантыСозданияПроцессов(Помощник);
	ПомощникОтправитьКОРПЛокализация.ПриЗаполненииДереваВариантовОтправки(Помощник, СтандартнаяОбработка);
	
	Если Помощник.РежимРаботы 
	= ПомощникОтправитьКлиентСерверКОРП.РежимРаботыВыборВариантаОтправкиДляДействияКомплексногоПроцесса() Тогда
		ДобавитьВДеревоВариантыСозданияОбработки(Помощник);
	КонецЕсли;
	
КонецПроцедуры

#Область ВариантСоздатьОбработку

// Добавляет в дерево вариант создания обработки
//
// Параметры:
//  Помощник - см. Помощник()
//
Процедура ДобавитьВДеревоВариантыСозданияОбработки(Помощник)
	
	ВариантОтправки = ПомощникОтправитьКлиентСервер.ВариантОтправкиСоздатьОбработку();

	Если Не ПомощникОтправить.ВозможноДобавлениеВарианта(Помощник, ВариантОтправки) Тогда
		Возврат;
	КонецЕсли;
	
	Если Не ПомощникОтправить.ЕстьОбъектыУказанныхТипов(
		Помощник.ОбъектыОтправки,
		ДопустимыеТипыОбъектовДляВариантаСоздатьОбработку()) Тогда
		
		Возврат;
	КонецЕсли;
	
	СтрокаВарианта = Помощник.ДеревоВариантов.Строки.Добавить();
	СтрокаВарианта.Вариант = ВариантОтправки;
	СтрокаВарианта.Представление = ПредставлениеВариантаСоздатьОбработку(Помощник);
	
КонецПроцедуры

// Возвращает допустимые типы объектов для варианта СоздатьОбработку.
...
//
Функция ДопустимыеТипыОбъектовДляВариантаСоздатьОбработку()
...
// Возвращает представление варианта создания обработки
//
// Возвращаемое значение:
//  Строка	
//
Функция ПредставлениеВариантаСоздатьОбработку(Помощник)
	
	Представление = НСтр("ru = 'Обработка предмета'");
...
Процедура ДобавитьВДеревоВариантСоздатьПисьмоНаОсновании(Помощник) Экспорт
	...
	Если Не (ИспользованиеВстроеннойИЛегкойПочтыНастройка.ИспользованиеВстроеннойПочты 
		Или ИспользованиеВстроеннойИЛегкойПочтыНастройка.ИспользованиеЛегкойПочты) Тогда
		Возврат;
	КонецЕсли;
...
Функция ДопустимыеТипыОбъектовДляВариантаСоздатьПисьмоНаОсновании() Экспорт
	...
	Если РегистрыСведений.ИспользованиеПочты.ВстроеннаяПочтаИспользуется()
...
Процедура ЗаполнитьДопустимыеТипыПредметовВПроцессах(Помощник) Экспорт
	...
	ВыборВариантаДляДействияКомплексногоПроцесса = (Помощник.РежимРаботы =
		ПомощникОтправитьКлиентСерверКОРП.РежимРаботыВыборВариантаОтправкиДляДействияКомплексногоПроцесса());
	
	ОтправкаПроектовИлиПроектныхЗадач = (Помощник.РежимРаботы =
		ПомощникОтправитьКлиентСерверКОРП.РежимРаботыОтправкаПроектовИлиПроектныхЗадач());
	
	Если ВыборВариантаДляДействияКомплексногоПроцесса Тогда
		...
		Для Каждого МетаданныеПроцесса Из МетаданныеПроцессов Цикл
			...
			Помощник.ДопустимыеТипыПредметовВПроцессах[ТипШаблонаСтрокой] = 
				МетаданныеПроцесса.ТабличныеЧасти.Предметы.Реквизиты.Предмет.Тип.Типы(); 
			
		КонецЦикла;
		
	ИначеЕсли ОтправкаПроектовИлиПроектныхЗадач Тогда
		
		ПомощникОтправить.ЗаполнитьДопустимыеТипыПредметовВПроцессахПоВводуНаОсновании(Помощник);
		
		// Дополняем типами предметов проектных задач.
		
		ТипыПредметаПроектнойЗадачи = 
			Метаданные.Справочники.ПроектныеЗадачи.Реквизиты.Предмет.Тип.Типы();
		...
		Для Каждого МетаданныеПроцесса Из МетаданныеПроцессов Цикл
			...
			ОписаниеДопустимыхТипов = 
				МетаданныеПроцесса.ТабличныеЧасти.Предметы.Реквизиты.Предмет.Тип;
			
			Для Каждого ТипПредмета Из ТипыПредметаПроектнойЗадачи Цикл
				
				Если Помощник.ДопустимыеТипыПредметовВПроцессах[ТипШаблонаСтрокой].Найти(ТипПредмета) <> Неопределено
					Или Не ОписаниеДопустимыхТипов.СодержитТип(ТипПредмета) Тогда
					
					Продолжить;
				КонецЕсли;
				
				Помощник.ДопустимыеТипыПредметовВПроцессах[ТипШаблонаСтрокой].Добавить(ТипПредмета);
...
Функция ПредставлениеВариантаСозданияПроцессаПоУмолчанию(Вариант, Помощник) Экспорт
	...
	Если ТипВарианта = Тип("СправочникСсылка.ШаблоныКомплексныхБизнесПроцессов") Тогда
	...
	ИначеЕсли ТипВарианта = Тип("СправочникСсылка.ШаблоныПодписания") Тогда
		Представление = ПомощникОтправить.ПредставлениеВариантаПодписанияПоУмолчанию();
...
// Возвращает описание варианта создания обработки
//
// Возвращаемое значение:
//  Строка
//
Функция ОписаниеВариантаСозданияОбработки()
	
	Описание = НСтр("ru = 'Создание обработки документа или мероприятия.'");
...
Функция ПараметрыВариантаСозданияПроцесса(Вариант, Помощник) Экспорт
	...
	ИначеЕсли ИспользуетсяРежимРаботыОтправкаПроектовИлиПроектныхЗадач(Помощник) Тогда
		...
		Для Индекс = 0 По МаксимальныйИндекс Цикл
			...
			// то подставляем его в соответствующий реквизит процесса.
			Если Индекс = 0 Тогда
			...
			СтрокаПредмета.РольПредмета = ?(Индекс = 0, Перечисления.РолиПредметов.Основной,
