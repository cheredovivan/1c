...
Функция УзлыДляВыгрузки(Знач ФизЛицо) Экспорт
	...
	|	И (СинхронизацияКалендарей.ФизическоеЛицо В (&ФизЛица)
	|	ИЛИ СинхронизацияКалендарей.Пользователь В (&Пользователи))
	...
	Запрос.УстановитьПараметр("Пользователи", ПользователиДокументооборот.ПользователиФизЛиц(ФизЛицо));
...
Функция СтруктураiCalendar(СтрокаiCalendar) Экспорт
	...
	Иначе
		...
		Для Каждого EXDATE Из ЧастиEXDATE Цикл
			Для Каждого ИсключениеПовторения Из ИсключенияПовторения(EXDATE) Цикл
...
Функция ВыделитьВремена(СтрокаВремен, Параметр)
	...
	Иначе
		...
		Для Каждого СтрокаВремени Из СтрРазделить(СтрокаВремен, ",") Цикл
			УниверсальноеВремя = Дата(СтрокаВремени);
			ВыделенныеВремена.Добавить(МестноеВремя(УниверсальноеВремя));
		КонецЦикла;
...
Функция НовоеОписаниеКалендаря()
	...
	ОписаниеКалендаря.Вставить("Владелец",					Справочники.ФизическиеЛица.ПустаяСсылка());
...
Процедура ЗарегистрироватьИзменения(ПараметрыПроцедуры)
	...
	|		ПО (СинхронизацияКалендарей.ФизическоеЛицо = Сотрудники.Владелец
	|		ИЛИ СинхронизацияКалендарей.Пользователь.ФизЛицо = Сотрудники.Владелец)
	|ГДЕ
	...
	|	И ЗаписиРабочегоКалендаря.Сотрудник В
	|		(ВЫБРАТЬ РАЗЛИЧНЫЕ
...
Функция ДанныеКалендарейДляВыгрузки(Узлы)
	...
	|	ВЫРАЗИТЬ(ЗаписиРабочегоКалендаряИзменения.Узел КАК ПланОбмена.СинхронизацияКалендарей).ФизическоеЛицо КАК ФизЛицо,
	|	ВЫРАЗИТЬ(ЗаписиРабочегоКалендаряИзменения.Узел КАК ПланОбмена.СинхронизацияКалендарей).Пользователь КАК Пользователь,
	|	ВЫРАЗИТЬ(ЗаписиРабочегоКалендаряИзменения.Узел КАК ПланОбмена.СинхронизацияКалендарей).ТипСинхронизации КАК
	|		ТипСинхронизации,
	...
	|	ВЫРАЗИТЬ(ЗаписиРабочегоКалендаряИзменения.Узел КАК ПланОбмена.СинхронизацияКалендарей).ВремяУведомленийИмпорт КАК
	|		ВремяУведомленийИмпорт,
	|	ВЫРАЗИТЬ(ЗаписиРабочегоКалендаряИзменения.Узел КАК ПланОбмена.СинхронизацияКалендарей).ВремяУведомленийЭкспорт КАК
	|		ВремяУведомленийЭкспорт
	|ИЗ
	...
	|		И ((ВЫРАЗИТЬ(ЗаписиРабочегоКалендаряИзменения.Узел КАК
	|			ПланОбмена.СинхронизацияКалендарей)) = ЗаписиСинхронизацииКалендарей.Узел)
	...
	Пока ВыборкаКалендари.Следующий() Цикл
		...
		Пока ВыборкаСсылки.Следующий() Цикл
			...
			Если Не ЗначениеЗаполнено(ОписаниеКалендаря.События) Тогда
				Если ЗначениеЗаполнено(ВыборкаСсылки.ФизЛицо) Тогда
					ОписаниеКалендаря.Владелец = ВыборкаСсылки.ФизЛицо;
				Иначе
					ОписаниеКалендаря.Владелец = ПользователиДокументооборот.ФизЛицоПользователя(
						ВыборкаСсылки.Пользователь);
				КонецЕсли;
...
Функция ДанныеКалендарейДляЗагрузки(Узлы)
	...
	|	ВЫБОР
	|		КОГДА СинхронизацияКалендарейКалендари.Ссылка.ФизическоеЛицо <> ЗНАЧЕНИЕ(Справочник.ФизическиеЛица.ПустаяСсылка)
	|			ТОГДА СинхронизацияКалендарейКалендари.Ссылка.ФизическоеЛицо
	|		ИНАЧЕ СинхронизацияКалендарейКалендари.Ссылка.Пользователь.ФизЛицо
	|	КОНЕЦ КАК Владелец,
...
Процедура ИзменитьЗаписьКалендаря(Выборка, ОписаниеКалендаря, ПравилоПовторения)
	...
	ИначеЕсли Выборка.ПометкаУдаления И ЗначениеЗаполнено(Выборка.ЗаписьСсылка) Тогда
		Если Сотрудники.ФизЛицоСотрудника(Выборка.ЗаписьСсылка.Автор) = ОписаниеКалендаря.Владелец
			Или Сотрудники.ФизЛицоСотрудника(Выборка.ЗаписьСсылка.Сотрудник) = ОписаниеКалендаря.Владелец Тогда
	...
	Иначе
		...
		Иначе
			...
			ЗаписьОбъект.Автор = Сотрудники.ОсновнойСотрудникФизЛица(ОписаниеКалендаря.Владелец);
			ЗаписьОбъект.Сотрудник = Сотрудники.ОсновнойСотрудникФизЛица(ОписаниеКалендаря.Владелец);
		...
		Если Сотрудники.ФизЛицоСотрудника(ЗаписьОбъект.Автор) = ОписаниеКалендаря.Владелец
			Или Сотрудники.ФизЛицоСотрудника(ЗаписьОбъект.Сотрудник) = ОписаниеКалендаря.Владелец Тогда
			Если ЗаписьОбъект.Связанная Тогда
				ЗаписьОбъект.ДополнительныеСвойства.Вставить("ИзменениеСвязаннойЗаписи", Истина);
			КонецЕсли;
			НормализоватьДаты(ЗаписьОбъект);
		...
		Если УстанавливатьНапоминание Тогда
			Если ЗначениеЗаполнено(ОписаниеКалендаря.ВремяУведомленийИмпорт) И ЗаписьОбъект.ДатаНачала >= НачалоДня(ТекущаяДатаСеанса()) Тогда
				УстановитьНапоминание(
					ЗаписьОбъект.Ссылка,
					ПользователиДокументооборот.ПользовательФизЛица(ОписаниеКалендаря.Владелец),
					ОписаниеКалендаря.ВремяУведомленийИмпорт);
		...
		Если ЗаписьОбъект.Связанная
			...
			И ДокументооборотПраваДоступа.ПраваПользователяПоОбъекту(
				...
				ПользователиДокументооборот.ПользовательФизЛица(ОписаниеКалендаря.Владелец)).Изменение Тогда
...
// Нормализует даты в записи календаря. Внешнии даты могут превышать максимальную дату - 3999 год.
// 
// Параметры:
//  ДанныеСобытия - СправочникОбъект.ЗаписиРабочегоКалендаря
//                - Структура
//                - СтрокаТаблицыЗначений
// 
Процедура НормализоватьДаты(ДанныеСобытия)
	
	МаксимальнаяДата = ОбщегоНазначенияДокументооборотКлиентСервер.МаксимальнаяДата();
	МаксимальнаяДатаНачала = МаксимальнаяДата - 2;
	МаксимальнаяДатаОкончания = МаксимальнаяДата - 1;
	Если ДанныеСобытия.ДатаНачала > МаксимальнаяДатаНачала Тогда
		ДанныеСобытия.ДатаНачала = МаксимальнаяДатаНачала;
	КонецЕсли;
	Если ДанныеСобытия.ДатаОкончания > МаксимальнаяДатаОкончания Тогда
		ДанныеСобытия.ДатаОкончания = МаксимальнаяДатаОкончания;
	КонецЕсли;
