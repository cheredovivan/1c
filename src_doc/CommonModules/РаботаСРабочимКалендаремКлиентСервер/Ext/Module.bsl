...
Процедура ПрименитьНастройкиОтображения(Планировщик, НастройкиОтображения) Экспорт
	...
	ЭтоМобильныйКлиент = МК_КлиентСервер.ЭтоМобильныйКлиент();
	...
	Если Не МК_КлиентСервер.ЭтоМобильныйКлиент() Тогда
		Планировщик.ФорматПеренесенныхЗаголовковШкалыВремени = "ДФ='ddd, d MMM yyyy'";
	Иначе
		Планировщик.ФорматПеренесенныхЗаголовковШкалыВремени = "ДФ='d (ddd)'";
	КонецЕсли;
...
#Если Не ВнешнееСоединение Тогда
	// Прокрутка.
	Если НастройкиОтображения.ПоказыватьПрокрутку Тогда
		Планировщик.АвтоМинимальнаяВысотаСтроки = Ложь;
		РазмерЯчейки30Минут = 30;
		РазмерЯчейки60Минут = 60;
		Если НастройкиОтображения.РазмерЯчейкиВремени = РазмерЯчейки60Минут Тогда
			КоличествоСтрокВЧасе = 1;
		ИначеЕсли НастройкиОтображения.РазмерЯчейкиВремени = РазмерЯчейки30Минут Тогда
			КоличествоСтрокВЧасе = 2;
		Иначе
			КоличествоСтрокВЧасе = 4;
		КонецЕсли;
		БезопасныйОтступВСтроках = 1;
		Планировщик.МинимальнаяВысотаСтроки = КоличествоСтрокВЧасе + БезопасныйОтступВСтроках;
	Иначе
		Планировщик.АвтоМинимальнаяВысотаСтроки = Истина;
		Планировщик.МинимальнаяВысотаСтроки = 0;
	КонецЕсли;
#КонецЕсли
...
Функция ПолучитьИндексЦвета(Цвет, Отработана = Ложь) Экспорт
	...
	Если Отработана Тогда
		ИндексЦвета = 6;
	ИначеЕсли Цвет = ПредопределенноеЗначение("Перечисление.ЦветаРабочегоКалендаря.Нет") Тогда
		ИндексЦвета = 0;
...
// Формирует имя ключевой операции "КалендарьОткрытиеКалендаря".
// 
// Возвращаемое значение:
//  Строка
// 
Функция КлючеваяОперацияОткрытиеКалендаря() Экспорт
	
	КлючеваяОперация = "КалендарьОткрытиеКалендаря";
	
	Возврат КлючеваяОперация;
	
КонецФункции

// Формирует имя ключевой операции "КалендарьОбновлениеКалендаря".
// 
// Возвращаемое значение:
//  Строка
// 
Функция КлючеваяОперацияОбновлениеКалендаря() Экспорт
	
	КлючеваяОперация = "КалендарьОбновлениеКалендаря";
	
	Возврат КлючеваяОперация;
	
КонецФункции

// Формирует имя ключевой операции "КалендарьОткрытиеЗаписиКалендаря".
// 
// Возвращаемое значение:
//  Строка
// 
Функция КлючеваяОперацияОткрытиеЗаписиКалендаря() Экспорт
	
	КлючеваяОперация = "КалендарьОткрытиеЗаписиКалендаря";
	
	Возврат КлючеваяОперация;
	
КонецФункции

// Формирует имя ключевой операции "КалендарьСозданиеЗаписиКалендаря".
// 
// Возвращаемое значение:
//  Строка
// 
Функция КлючеваяОперацияСозданиеЗаписиКалендаря() Экспорт
	
	КлючеваяОперация = "КалендарьСозданиеЗаписиКалендаря";
	
	Возврат КлючеваяОперация;
	
КонецФункции

// Формирует имя ключевой операции "КалендарьЗаписьЗаписиКалендаря".
// 
// Возвращаемое значение:
//  Строка
// 
Функция КлючеваяОперацияЗаписьЗаписиКалендаря() Экспорт
	
	КлючеваяОперация = "КалендарьЗаписьЗаписиКалендаря";
