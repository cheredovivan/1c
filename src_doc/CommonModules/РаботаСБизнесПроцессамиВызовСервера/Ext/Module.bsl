...
Функция ДоступностьИнтерактивнойПометкиУдаленияБизнесПроцесса(Процесс) Экспорт
	...
	Если ЗначениеЗаполнено(Процесс) Тогда
		...
		ПраваПоПроцессу = ДокументооборотПраваДоступа.ПраваПользователяПоОбъекту(Процесс);
...
Процедура ПометитьНаУдалениеБизнесПроцесс(СсылкаНаОбъект) Экспорт
	...
	Если СписокТиповБизнесПроцессов.НайтиПоЗначению(ТипЗнч(СсылкаНаОбъект)) <> Неопределено
		...
		ПраваПоПроцессу = ДокументооборотПраваДоступа.ПраваПользователяПоОбъекту(СсылкаНаОбъект);
		...
		Если ЗначениеЗаполнено(ВедущаяЗадачаПроцесса) Тогда
			ВызватьИсключение НСтр("ru = 'Изменение пометки удаления подпроцессов входящих в комплексный процесс запрещено.'");
		КонецЕсли;
		...
		Исключение
			...
			ЗаписьЖурналаРегистрации("УстановкаСнятиеПометкиУдаленияПроцесса",
				УровеньЖурналаРегистрации.Ошибка,,
				СсылкаНаОбъект,
				ПредставлениеОшибки);
			
			ВызватьИсключение
				НСтр("ru = 'При установке/снятии пометки удаления произошла ошибка.
...
Функция ПолучитьСуммарныеПлановыеТрудозатратыПроцесса(ПроцессОбъект) Экспорт
	...
	Если МетаданныеПроцесса.Реквизиты.Найти("ОбрабатывающийРезультат") <> Неопределено
		И МетаданныеПроцесса.Реквизиты.Найти("ТрудозатратыПланОбрабатывающегоРезультат") <> Неопределено
		И ЗначениеЗаполнено(ПроцессОбъект.ОбрабатывающийРезультат) <> Неопределено Тогда
		Сумма = Сумма + ПроцессОбъект.ТрудозатратыПланОбрабатывающегоРезультат;
	КонецЕсли;
	
	Если МетаданныеПроцесса.Реквизиты.Найти("ОбрабатывающийРезолюцию") <> Неопределено
		И МетаданныеПроцесса.Реквизиты.Найти("ТрудозатратыПланПомощника") <> Неопределено
		И ЗначениеЗаполнено(ПроцессОбъект.ОбрабатывающийРезолюцию) <> Неопределено Тогда
		Сумма = Сумма + ПроцессОбъект.ТрудозатратыПланПомощника;
	КонецЕсли;
...
Функция ПроцессыПоПредметуКоторыеМожетПрерватьТекущийПользователь(Предмет) Экспорт
	...
	Для Каждого Процесс Из ГоловныеПроцессы Цикл
		...
		Если РеквизитыПроцесса.Состояние = 
			...
			Продолжить;
		КонецЕсли;
		ПраваПоОбъекту = ДокументооборотПраваДоступа.ПраваПользователяПоОбъекту(Процесс);
	...
	Для Каждого СтрокаПредмета Из Предметы Цикл
		...
		Если ТипЗнч(Строка.Ссылка) = Тип("СправочникСсылка.Файлы") Тогда
			Если ЗначениеЗаполнено(Строка.Ссылка) Тогда
				...
				Если ЗначениеЗаполнено(Строка.Редактирует) Тогда
					Если СотрудникиВызовСервера.СотрудникиПользователя(Пользователи.ТекущийПользователь()).Найти(Строка.Редактирует) <> Неопределено Тогда
...
// заполним те реквизиты, чьи имена отличаются
Процедура ЗаполнитьСтрокуФайла(Строка, СтрФайлы)
	
	Строка.ТипФайла = СтрФайлы.ИндексКартинки;
	
	Если ЗначениеЗаполнено(Строка.Редактирует) Тогда
		Если Не Строка.РедактируетТекущийПользователь Тогда
			Строка.РедактируетДругойПользователь = Истина;
		КонецЕсли;
	КонецЕсли;
...
Процедура ДобавитьФайлыВДеревоПриложенийПоДокументу(СтрокаПредмета, Предмет, ОтображатьУдаленные)
	
	ОтметитьЗахваченныеФайлы = Истина;
	МассивКомбинацийРолей = Новый Массив;
	
	ТаблицаФайлы = Делопроизводство.ПолучитьДанныеФайловДляСписка(Неопределено, Предмет, 
		ОтображатьУдаленные, ОтметитьЗахваченныеФайлы, МассивКомбинацийРолей);
	...
	Если Не ИспользоватьРолиФайлов Или МассивКомбинацийРолей.Количество() = 0 Тогда
		...
		Для Каждого Стр Из ТаблицаФайлы Цикл
			НовСтр = СтрокаПредмета.Строки.Добавить();
			ЗаполнитьЗначенияСвойств(НовСтр, Стр);
			
			ЗаполнитьСтрокуФайла(НовСтр, Стр);
	...
	Иначе
		...
		Для Каждого ОписаниеРоли Из МассивКомбинацийРолей Цикл
			...
			Ветка = СтрокаПредмета.Строки.Добавить();
			...
			Если Ветка.Строки.Количество() <> 0 Тогда
				...
				+ СтрШаблон(" (%1)", Ветка.Строки.Количество());
			КонецЕсли;	
...
Процедура ДобавитьФайлыВДеревоПриложений(СтрокаПредмета, Предмет, ОтображатьУдаленные)
	...
	Если ДелопроизводствоКлиентСервер.ЭтоДокумент(Предмет) Тогда
		ДобавитьФайлыВДеревоПриложенийПоДокументу(СтрокаПредмета, Предмет, ОтображатьУдаленные);
		Возврат;
	КонецЕсли;	
...
Функция ПредставлениеРезультата(БизнесПроцесс) Экспорт 
	...
	ИначеЕсли ТипЗнч(БизнесПроцесс) = Тип("БизнесПроцессСсылка.Подписание") Тогда 
		Возврат ОбщегоНазначения.ЗначениеРеквизитаОбъекта(БизнесПроцесс, "РезультатПодписания");
...
Процедура ПередЗаписьюБизнесПроцессаПередЗаписью(Источник, Отказ) Экспорт
	...
	Если Не ЭтоНовый Тогда
		РеквизитыПроцесса = ОбщегоНазначенияДокументооборот.
			ЗначенияРеквизитовОбъектаВПривилегированномРежиме(
				Источник.Ссылка,
				"ВедущаяЗадача, ГлавнаяЗадача, Стартован, ПометкаУдаления, Состояние, Завершен");
	КонецЕсли;
	
	Если ЭтоНовый И СрокиИсполненияПроцессов.НужноРассчитатьСрокНовогоПроцесса(Источник) Тогда
		СрокиИсполненияПроцессов.РассчитатьСрокИсполненияДляНовогоПроцесса(Источник);
	КонецЕсли;
	...
	// Заполнение реквизита УзелОбработки и проверка
	// соответствия УзелОбработки текущему узлу.
	ЭтотУзел = ОбменДаннымиДокументооборотПовтИсп.ИдентификаторТекущегоУзла();
	
	Если ЭтоНовый Тогда
		СтартПроцесса = Источник.Стартован;
		Источник.УзелОбработки = ЭтотУзел;
	Иначе
	...
	Если СтартПроцесса
		И Источник.УзелОбработки <> ЭтотУзел Тогда
		
		ТекстИсключения = НСтр("ru = 'Процесс можно стартовать только в узле КОД: ""%1""'");
		ТекстИсключения = СтрШаблон(ТекстИсключения, Источник.УзелОбработки);
	...
	Если Не ЭтоНовый Тогда
		...
		Источник.ДополнительныеСвойства.Вставить(
			"ПредыдущееЗавершен", РеквизитыПроцесса.Завершен);	
	КонецЕсли;
...
Процедура БизнесПроцессПриЗаписиПриЗаписи(Источник, Отказ) Экспорт
	...
	Если ОбычнаяЗапись Тогда
		...
		ПроцессЗавершен = Источник.ДополнительныеСвойства.Свойство("ОбработатьЗавершениеБизнесПроцесса")
			Или (Источник.ДополнительныеСвойства.Свойство("ПредыдущееЗавершен")
			И Источник.Завершен И Не Источник.ДополнительныеСвойства.ПредыдущееЗавершен);
		// Запись результата выполнения процесса
		Если ПроцессЗавершен Тогда
			РаботаСБизнесПроцессами.ОбновитьРезультатВыполненияПроцесса(Источник.Ссылка);
		КонецЕсли;
		...
		Если Источник.ДополнительныеСвойства.ПредыдущаяВедущаяЗадача <> Источник.ВедущаяЗадача
			...
			РаботаСБизнесПроцессами.ОбновитьСведенияОДочернемПроцессе(Источник);
		КонецЕсли;
		...
		Если ПроцессПомеченНаУдаление Тогда
			РегистрыСведений.НастройкиОтложенногоСтартаПроцессов.Удалить(Источник.Ссылка);
		КонецЕсли;
	...
	Если ОбычнаяЗапись Или ТолькоОбновитьДопРеквизитыПроцессаИРабочиеГруппыРодительскихПроцессов Тогда
		...
		Если Источник.ДополнительныеСвойства.Свойство("ДополнительныеРеквизиты") Тогда
			...
			ДопРеквизитыИзменились = РаботаСБизнесПроцессами.ЗначенияДополнительныхРеквизитовИзменились(
				Источник.ДополнительныеСвойства.ДополнительныеРеквизиты,
			...
			Попытка
				...
				Для Каждого Предмет Из Предметы Цикл
					РаботаСБизнесПроцессами.ОбновитьДопРеквизитыУЗадачБизнесПроцесса(Источник, Предмет);
				КонецЦикла;
				Если Не ВнешняяТранзакция Тогда
...
Процедура ОбновитьДоступностьЭлементовФормыПоСостояниюПроцесса(
	...
	ПроцессЕстьВОчередиЗаданий = Ложь;
	

	Форма.ТолькоПросмотр = Форма.ТолькоПросмотр Или ПроцессЕстьВОчередиЗаданий;
	...
	ИначеЕсли Не Форма.ЭтоНовый И Форма.Объект.Состояние = Перечисления.СостоянияБизнесПроцессов.Остановлен Тогда
		...
		Если ПроцессЕстьВОчередиЗаданий Тогда
			Форма.Элементы.ДекорацияОписание.Заголовок = 
				СтрШаблон(НСтр("ru='%1. Процесс находится в очереди заданий для изменения.'"),
					Форма.Элементы.ДекорацияОписание.Заголовок);
		КонецЕсли;
	...
	ИначеЕсли Не Форма.ЭтоНовый И Форма.Объект.Стартован Тогда
		...
		Если ПроцессЕстьВОчередиЗаданий Тогда
			Форма.Элементы.ДекорацияОписание.Заголовок = 
				СтрШаблон(НСтр("ru='%1. Процесс недоступен для изменения, т.к. по нему имеются незавершенные задания в очереди заданий.'"),
					Форма.Элементы.ДекорацияОписание.Заголовок);
		КонецЕсли;
	...
	Если НЕ Форма.Объект.Ссылка.Пустая() Тогда
		ПравоНаИзменениеБизнесПроцесса = 
			ДокументооборотПраваДоступа.ПраваПользователяПоОбъекту(Форма.Объект.Ссылка).Изменение;
	КонецЕсли;
...
Функция ЕстьАктивныеПодзадачи(Задача) Экспорт
	...
	Для Каждого ПроцессСсылка Из МассивПроцессовПолный Цикл
		...
		ПраваПоПроцессу = ДокументооборотПраваДоступа.ПраваПользователяПоОбъекту(ПроцессСсылка);
...
Функция ПроцессыПоПредмету(Предмет)
	...
	ИначеЕсли ТипОбъекта = Тип("СправочникСсылка.Контрагенты") Тогда
		...
		Запрос = Новый Запрос(
			...
			|ГДЕ
			|	ДокументыПредприятияКонтрагенты.Контрагент = &Предмет");
		...
		Для Каждого МетаданныеБП Из МетаданныеБизнесПроцессы Цикл
			...
			Если ЕстьПредметы Тогда
				...
				ТекстЗапросаДляТекущегоПроцесса = 
					...
					|ИЗ
					|	&БизнесПроцессПредметы КАК ПредметыПроцесса
					|ГДЕ
					|	ПредметыПроцесса.Предмет В (&Предметы)";
					
				ТекстЗапросаДляТекущегоПроцесса = СтрЗаменить(
					ТекстЗапросаДляТекущегоПроцесса, "&БизнесПроцессПредметы", СтрШаблон("БизнесПроцесс.%1.Предметы", МетаданныеБП.Имя));
			...
			Если ЕстьПроектыИПроектныеЗадачиПроцессов Тогда
				...
				ТекстЗапросаДляТекущегоПроцесса = 
					...
					|	Процессы.Ссылка
					|ИЗ
					|	&БизнесПроцессИмяПроцесса КАК Процессы
					|ГДЕ
					|	(Процессы.Проект В (&Проекты)
					|			ИЛИ Процессы.ПроектнаяЗадача В (&ПроектныеЗадачи))";
					
				ТекстЗапросаДляТекущегоПроцесса = СтрЗаменить(
					ТекстЗапросаДляТекущегоПроцесса, "&БизнесПроцессИмяПроцесса",
						СтрШаблон("БизнесПроцесс.%1", МетаданныеБП.Имя));
...
Функция ГоловныеПроцессы(Процессы)
	...
	Для Каждого Процесс Из Процессы Цикл
		...
		Если ГоловныеПроцессы.Найти(ГоловнойПроцесс) = Неопределено Тогда
			...
			ПраваПоПроцессу = ДокументооборотПраваДоступа.ПраваПользователяПоОбъекту(
...
Процедура ДобавитьПроцессыИПодчиненныеЗадачи(Дерево, Процессы, ВспомогательныеПроцессы,
	...
	Запрос.Текст = 
		...
		|ИЗ
		|	Процессы КАК Процессы
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.НастройкиОтложенногоСтартаПроцессов КАК ПроцессыДляЗапуска
	...
	Для Каждого ЗадачаУровня Из ЗадачиУровня Цикл
		...
		Если ТипЗнч(РеквизитыЗадачи.Источник) = Тип("СправочникСсылка.ДействияОзнакомления") Тогда
			...
			РеквизитыДействия = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(РеквизитыЗадачи.Источник, 
				"Автор, ОжидатьЗавершения, ПомещеноВИсторию, Предмет");
			...
			Если Истина Тогда   //  всегда
				...
				Если ЧислоУчастниковДействия <= МаксимальноеЧислоУчастниковДляПоказа Тогда // менее 50, покажем всех
					...
					Для Каждого Участник Из УчастникиИДатыДействия Цикл       
						...
						СтрокаЗадача.Наименование = Строка(СтрокаЗадача.Ссылка);
						
						Если НаименованиеИОписаниеПоДействию <> Неопределено 
							И ЗначениеЗаполнено(НаименованиеИОписаниеПоДействию.Наименование) Тогда
							СтрокаЗадача.Наименование = НаименованиеИОписаниеПоДействию.Наименование;
							
						ИначеЕсли ТипЗнч(СтрокаЗадача.Ссылка) = Тип("СправочникСсылка.ДействияОзнакомления") Тогда 	
							СтрокаЗадача.Наименование = СтрокаЗадача.Наименование + " """ + Строка(РеквизитыДействия.Предмет) + """";
...
Процедура ДобавитьПодзадачи(СтрокаЗадача)
	
	Если Не ЗначениеЗаполнено(СтрокаЗадача.Ссылка) Тогда
		Возврат;
	КонецЕсли;	  
	...
	Для Каждого ПодзадачаСтрока Из Подзадачи Цикл
		...
		Для Каждого ДействияЗадачСтрока Из ДействияЗадач Цикл
			...
			СтрокаПодзадача = СтрокаЗадача.Строки.Добавить();   
			
			СтрокаПодзадача.Тип = 1;
			
			СтрокаПодзадача.Ссылка = ДействияЗадачСтрока.Действие;
			СтрокаПодзадача.Исполнитель = ДействияЗадачСтрока.Участник;   
			
			СтрокаПодзадача.Наименование = ПодзадачаСтрока.Заголовок;
			
			СтрокаПодзадача.СрокИсполнения = ПодзадачаСтрока.Срок;    
			Если СтрокаПодзадача.СрокИсполнения < ТекущаяДатаСеанса() Тогда
				СтрокаПодзадача.Просрочена = Истина;
			КонецЕсли;	   
			
			СтрокаПодзадача.ДатаВыполнения = ДействияЗадачСтрока.ДатаВыполнения;
			СтрокаПодзадача.РезультатВыполнения = ДействияЗадачСтрока.РезультатДействияТекстом;
			
			СтрокаПодзадача.СостояниеКартинка = -1; 
			СтрокаПодзадача.Важность =
				Справочники.ПриоритетыЗадач.ИндексКартинкиПриоритета(ПодзадачаСтрока.Приоритет);
			
			Если ДействияЗадачСтрока.СостояниеУчастникаЗадачи = Справочники.СостоянияУчастниковЗадач.Выполнена
				Или ДействияЗадачСтрока.СостояниеУчастникаЗадачи = Справочники.СостоянияУчастниковЗадач.ОжидаетПроверки Тогда
				
				СтрокаПодзадача.СостояниеКартинка = 4;   // галочка выполнено  
				СтрокаПодзадача.ПринятаКИсполнению = Истина;   
				СтрокаПодзадача.Выполнена = Истина;

			ИначеЕсли ДействияЗадачСтрока.СостояниеУчастникаЗадачи = Справочники.СостоянияУчастниковЗадач.ВРаботе  Тогда
			...
			Если Не ДокументооборотПраваДоступа.ПраваПользователяПоОбъекту(
				...
				СтрокаПодзадача.Наименование = НСтр("ru = 'Описание скрыто'");
				СтрокаПодзадача.РезультатВыполнения = "";
			КонецЕсли;			
...
Процедура ЗаполнитьСписокАктивныхЗадачПоПредмету(Предмет, Список)
	...
	Запрос.УстановитьПараметр("СотрудникиПользователя", Сотрудники.ТекущийПользовательИСотрудники());
	Запрос.УстановитьПараметр("ФизическоеЛицо", ПараметрыСеанса.ФизическоеЛицоПользователя);
	...
	// Добавим активные действия задач по приложению.
	Если Метаданные.ОпределяемыеТипы.ПриложениеЗадач.Тип.СодержитТип(ТипЗнч(Предмет))
		Или ТипЗнч(Предмет) = Тип("СправочникСсылка.Проекты") Тогда
		
		УстановитьПривилегированныйРежим(Истина);
		
		ЗадачиПоПриложению = РаботаСЗадачами.ЗадачиПоПриложению(Предмет, Истина);
		Если ТипЗнч(Предмет) = Тип("СправочникСсылка.Проекты") Тогда
			ЗадачиПоПроекту = РаботаСЗадачами.ЗадачиПоПроекту(Предмет);
			ОбщегоНазначенияКлиентСервер.ДополнитьМассив(ЗадачиПоПриложению, ЗадачиПоПроекту, Истина);
		КонецЕсли;
		
		АктивныеДействияПоЗадачам =
			Документы.ДействиеЗадачи.АктивныеДействияПоЗадачам(ЗадачиПоПриложению);
		ДанныеДействийЗадач = РаботаСЗадачами.ДанныеДействийЗадач(
			АктивныеДействияПоЗадачам.ВыгрузитьКолонку("ДействиеЗадачи"));
		ДанныеДействийЗадач.Индексы.Добавить("ДействиеЗадачи");
		УстановитьПривилегированныйРежим(Ложь);
		
		ПраваПоЗадачам = ДокументооборотПраваДоступа.ПраваПользователейПоОднотипнымОбъектам(ЗадачиПоПриложению);
		...
		Для Каждого ЗадачаПоПриложению Из ЗадачиПоПриложению Цикл
			...
			Источник = ОбщегоНазначенияДокументооборот.ЗначениеРеквизитаОбъектаВПривилегированномРежиме(
				ЗадачаПоПриложению, "Источник");
			
			Если ТипЗнч(Источник) = Тип("СправочникСсылка.ДействияОзнакомления") Тогда   
					
				РеквизитыДействия = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Источник, "Автор, ОжидатьЗавершения, ПомещеноВИсторию");     
				СостояниеДействия = РегистрыСведений.СостоянияДействий.СостояниеДействия(Источник);
				...
				ТекущаяОбработка = РегистрыСведений.ДействияОбработкиОбъектов.ОбработкаДействия(Источник);
				СостояниеОбработки = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ТекущаяОбработка, "Состояние");
				Если СостояниеОбработки = Перечисления.СостоянияОбработкиОбъектов.Прервана Тогда
					Продолжить;
				КонецЕсли;		
				
				Если Истина Тогда // всегда
					
					ДействиеОзнакомления = Источник;         
					...
					Если ЧислоУчастниковДействия <= МаксимальноеЧислоУчастниковДляПоказа Тогда // менее 50, покажем всех сотрудников
						...
						Для Каждого Участник Из УчастникиИДатыДействия Цикл  
							...
							Если Участник.Состояние = Перечисления.СостоянияОзнакомления.Ознакомился Тогда
								Продолжить;  // выполненные задачи не показываем
							КонецЕсли;	
							
							СтрокаЗадача = Список.Добавить();
							...
							ИначеЕсли ТипЗнч(СтрокаЗадача.Ссылка) = Тип("СправочникСсылка.ДействияОзнакомления") Тогда 	
								СтрокаЗадача.Наименование = СтрокаЗадача.Наименование + " """ + Строка(Предмет) + """";
					...
					Иначе // более 50, покажем группы именно как группы
						...
						УчастникиДействия = Справочники.ДействияОзнакомления.УчастникиДействия(Источник);
						...
						Для Каждого ТекУчастник Из УчастникиДействия Цикл
							...
							Если ТипЗнч(ТекУчастник.Участник) = Тип("СправочникСсылка.Сотрудники") 
								...
								Если СтрокаРегистра <> Неопределено Тогда
									...
									Если СтрокаРегистра.Состояние = Перечисления.СостоянияОзнакомления.Ознакомился Тогда
										Продолжить;  // выполненные задачи не показываем
									КонецЕсли;	
							...
							Иначе // группа
								...
								Если СтруктураОзнакомления <> Неопределено Тогда                                 
									...
									Если СтруктураОзнакомления.СколькоОзнакомлено = СтруктураОзнакомления.СколькоВсегоНадоОзнакомить Тогда
										Продолжить; // вся группа выполнила
									КонецЕсли;	
							...
							Если Список.Количество() >= МаксимальноеЧислоУчастниковДляПоказа Тогда
		...
		Для Каждого АктивноеДействие Из АктивныеДействияПоЗадачам Цикл
			...
			ДанныеДействия = ДанныеДействийЗадач.Найти(АктивноеДействие.ДействиеЗадачи, "ДействиеЗадачи");
			Если ДанныеДействия = Неопределено Или Не ДанныеДействия.ОжидаетВыполнения Тогда
				Продолжить;
			КонецЕсли;
			
			ЕстьПравоЧтение = ПраваЧтенияПоЗадачам[АктивноеДействие.Задача] = Истина;
...
// Формирует контекст задач.
// 
// Параметры:
//  ЗадачиДляКонтекста - Массив из ДокументСсылка.Задача.
// 
// Возвращаемое значение:
//  Структура - Контекст задач.
//  
Функция КонтекстЗадач(ЗадачиДляКонтекста) Экспорт
	
	УстановитьПривилегированныйРежим(Истина);
	КонтекстЗадач = Новый Структура;
	КонтекстЗадач.Вставить(
		"ЗадачиПоПриложению",
		ЗадачиДляКонтекста);
	КонтекстЗадач.Вставить(
		"РеквизитыЗадач",
	...
	КонтекстЗадач.Вставить(
		"ДанныеЗадач",
		РегистрыСведений.РеестрЗадачПоАвторам.ДанныеЗадач(КонтекстЗадач.ЗадачиПоПриложению));
	КонтекстЗадач.Вставить(
		"АктивныеДействияПоЗадачам",
		Документы.ДействиеЗадачи.АктивныеДействияПоЗадачам(КонтекстЗадач.ЗадачиПоПриложению));
	КонтекстЗадач.Вставить(
		"ДанныеДействийЗадач",
		РаботаСЗадачами.ДанныеДействийЗадач(
			КонтекстЗадач.АктивныеДействияПоЗадачам.ВыгрузитьКолонку("ДействиеЗадачи")));
	КонтекстЗадач.АктивныеДействияПоЗадачам.Индексы.Добавить("Задача");
	КонтекстЗадач.АктивныеДействияПоЗадачам.Индексы.Добавить("ДействиеЗадачи");
	КонтекстЗадач.АктивныеДействияПоЗадачам.Сортировать("Дата");
	КонтекстЗадач.ДанныеДействийЗадач.Индексы.Добавить("ДействиеЗадачи");
	
	ПодчиненныеЗадачи = Новый Соответствие;
	Для Каждого Задача Из КонтекстЗадач.ЗадачиПоПриложению Цикл
		
		РеквизитыЗадачи = КонтекстЗадач.РеквизитыЗадач[Задача];
		Если РеквизитыЗадачи = Неопределено
			Или Не ЗначениеЗаполнено(РеквизитыЗадачи.ВышестоящееДействие) Тогда
			Продолжить;
		КонецЕсли;
		
		Если ПодчиненныеЗадачи[РеквизитыЗадачи.ВышестоящееДействие] = Неопределено Тогда
			ПодчиненныеЗадачи[РеквизитыЗадачи.ВышестоящееДействие] = Новый Массив;
		КонецЕсли;
		
		ПодчиненныеЗадачиПоДействию =
			ПодчиненныеЗадачи[РеквизитыЗадачи.ВышестоящееДействие];
		ПодчиненныеЗадачиПоДействию.Добавить(Задача);
		
		АктивноеДействие = КонтекстЗадач.АктивныеДействияПоЗадачам.Найти(
			РеквизитыЗадачи.ВышестоящееДействие,
			"ДействиеЗадачи");
		Если АктивноеДействие = Неопределено
			Или Не ЗначениеЗаполнено(АктивноеДействие.Источник) Тогда
			Продолжить;
		КонецЕсли;
		
		Если ПодчиненныеЗадачи[АктивноеДействие.Источник] = Неопределено Тогда
			ПодчиненныеЗадачи[АктивноеДействие.Источник] = Новый Массив;
		КонецЕсли;
		
		ПодчиненныеЗадачиПоДействию =
			ПодчиненныеЗадачи[АктивноеДействие.Источник];
		ПодчиненныеЗадачиПоДействию.Добавить(Задача);
		
	КонецЦикла;
	КонтекстЗадач.Вставить("ПодчиненныеЗадачи", ПодчиненныеЗадачи);
	УстановитьПривилегированныйРежим(Ложь);
	
	ПраваПоЗадачам = ДокументооборотПраваДоступа.ПраваПользователейПоОднотипнымОбъектам(
		КонтекстЗадач.ЗадачиПоПриложению,
		Истина,
	...
	КонецЦикла;
	КонтекстЗадач.Вставить("ПраваЧтенияПоЗадачам", ПраваЧтенияПоЗадачам);
	
	Возврат КонтекстЗадач;
	
КонецФункции

// Информация прерывания.
// 
// Параметры:
//  ВыборкаПроцессов - Структура:
//   * ДатаПрерывания - Дата
//   * ПричинаПрерывания - Строка
//   * Прервал - СправочникСсылка.Сотрудники
// 
// Возвращаемое значение:
//  Строка
//
Функция ИнформацияПрерывания(ВыборкаПроцессов)
	
	Текст = СтрШаблон(НСтр("ru ='Процесс прерван: %1'"), Формат(ВыборкаПроцессов.ДатаПрерывания,
		"ДФ='dd.MM.yyyy HH:mm'"));
	
	Если ЗначениеЗаполнено(ВыборкаПроцессов.ПричинаПрерывания) Тогда
		Текст = Текст + Символы.ПС + СтрШаблон(НСтр("ru ='Причина: %1'"), ВыборкаПроцессов.ПричинаПрерывания);
	КонецЕсли;
	
	Если ЗначениеЗаполнено(ВыборкаПроцессов.Прервал) Тогда
		Текст = Текст + Символы.ПС + СтрШаблон(НСтр("ru ='Прервал: %1'"), ВыборкаПроцессов.Прервал);
	КонецЕсли;
