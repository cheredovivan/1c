...
//	ЗаполнятьНеЗаданы - признак заполнения строки предметов словом "Не заданы" если предметы не установлены
//
Функция ПредметыОзнакомленияСтрокой(Предметы, Кавычки = Ложь, ЗаполнятьНеЗаданы = Истина, ТолькоОсновные = Ложь) Экспорт
	...
	Для Каждого СтрокаПредмета Из МассивПредметов Цикл
		Если ТипЗнч(СтрокаПредмета) = Тип("Структура") Тогда
			Если ЗначениеЗаполнено(СтрокаПредмета.Предмет) Тогда
				СтрокаПредметов = СтрокаПредметов + ?(ПустаяСтрока(СтрокаПредметов),"",", ") 
					+ ?(Кавычки,"""","") + ОбщегоНазначенияДокументооборотВызовСервера.ПредметОзнакомленияСтрокой(СтрокаПредмета.Предмет, СтрокаПредмета.ИмяПредмета) + ?(Кавычки,"""","");
			КонецЕсли;
		Иначе
			Если ЗначениеЗаполнено(СтрокаПредмета) Тогда
				СтрокаПредметов = СтрокаПредметов + ?(ПустаяСтрока(СтрокаПредметов),"",", ") 
					+ ?(Кавычки,"""","") + ОбщегоНазначенияДокументооборотВызовСервера.ПредметОзнакомленияСтрокой(СтрокаПредмета) + ?(Кавычки,"""","");
			КонецЕсли;
		КонецЕсли;
	КонецЦикла;
...
// Возвращает строку наименования с перечислением предметов через запятую
//
Функция ПолучитьНаименованиеСПредметамиОзнакомления(ШаблонНаименования, Предметы) Экспорт
	
	Наименование = СокрЛП(ШаблонНаименования);
	ПредставлениеПредметов = ПредметыОзнакомленияСтрокой(Предметы, Истина, Ложь);
	Если ЗначениеЗаполнено(ПредставлениеПредметов) Тогда
		Наименование = Наименование + " " + ПредставлениеПредметов;
	КонецЕсли;
...
Функция ПолучитьТипыДокументов() Экспорт
	...
	ТипыДокументов.Добавить(Тип("СправочникСсылка.ДокументыПредприятия"));
...
Процедура УдалитьУсловияПоИмениПредмета(Форма, ИмяПредмета) Экспорт
	...
	Если ТипЗнч(Форма.Объект.Ссылка) = Тип("БизнесПроцессСсылка.КомплексныйПроцесс") 
		...
		Если РаботаСКомплекснымиБизнесПроцессамиКлиентСервер.ЭтоФормаПроцессаСоСхемой(Форма) Тогда
			ИменаПредметовПроцесса = ИменаПредметовПроцесса(Форма.Объект);
			ИндескПредмета = ИменаПредметовПроцесса.Найти(ИмяПредмета);
			Если ИндескПредмета <> Неопределено Тогда
				ИменаПредметовПроцесса.Удалить(ИндескПредмета);
			КонецЕсли;
			
			РаботаСКомплекснымиБизнесПроцессамиКлиентСервер.НормализоватьНастройкиУсловийСхемыВФорме(
				Форма, ИменаПредметовПроцесса);
		Иначе
...
Процедура ЗаменитьУсловияПоИмениПредмета(Форма, СтароеЗначение, НовоеЗначение) Экспорт
	...
	Если ТипЗнч(Форма.Объект.Ссылка) = Тип("БизнесПроцессСсылка.КомплексныйПроцесс") 
		...
		Если РаботаСКомплекснымиБизнесПроцессамиКлиентСервер.ЭтоФормаПроцессаСоСхемой(Форма) Тогда
			РаботаСКомплекснымиБизнесПроцессамиКлиентСервер.ЗаменитьИмяПредметаВУсловияхСхемыВФорме(
				Форма, СтароеЗначение, НовоеЗначение);
		Иначе
			Для Каждого Строка Из Форма.Объект.ПредшественникиЭтапов Цикл
				Если Строка.ИмяПредметаУсловия = СтароеЗначение Тогда
					Строка.ИмяПредметаУсловия = НовоеЗначение;
...
// Возвращает предметы процесса.
// 
// Параметры:
//  Процесс - ДанныеФормыСтруктура,
//            БизнесПроцессОбъект.<Имя процесса с предметами>,
//            СправочникОбъект.<Имя шаблона процесса с предметами>
// 
// Возвращаемое значение:
//  Соответствие - все предметы процесса.
...
//  
Функция ПредметыПроцесса(Процесс) Экспорт
	
	ПредметыПроцесса = Новый Соответствие();
	Для Каждого СтрокаТаблицы Из Процесс.Предметы Цикл
		ПредметыПроцесса[СтрокаТаблицы.ИмяПредмета] = СтрокаТаблицы.Предмет;
	КонецЦикла;
	
	Возврат ПредметыПроцесса;
	
КонецФункции

// Возвращает имена предметов процесса.
// 
// Параметры:
//  Процесс - ДанныеФормыСтруктура,
//            БизнесПроцессОбъект.<Имя процесса с предметами>,
...
// 
// Возвращаемое значение:
//  Массив - Имена предметов процесса
//
Функция ИменаПредметовПроцесса(Процесс) Экспорт
	
	ИменаПредметовПроцесса = Новый Массив;
	Для Каждого КлючЗначение Из ПредметыПроцесса(Процесс) Цикл
		ИменаПредметовПроцесса.Добавить(КлючЗначение.Ключ);
	КонецЦикла;
	
	Возврат ИменаПредметовПроцесса;
