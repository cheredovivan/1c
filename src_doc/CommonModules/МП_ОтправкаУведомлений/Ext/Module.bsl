...
Процедура ОтправкаPushУведомлений() Экспорт
	...
	ОбщегоНазначения.ПриНачалеВыполненияРегламентногоЗадания(
		Метаданные.РегламентныеЗадания.МП_ОтправкаPushУведомлений, Отказ);
	...
	ВыполнитьОтправкуPushУведомлений();
...
Процедура СозданиеPushУведомленийОНовыхПисьмах() Экспорт
	...
	Отказ = Ложь;
	ОбщегоНазначения.ПриНачалеВыполненияРегламентногоЗадания(
		Метаданные.РегламентныеЗадания.МП_СозданиеPushУведомленийОНовыхПисьмах, Отказ);
...
//
Процедура ЗаписатьСообщениеВРегистрPushУведомления(ПолучательУведомления,
	ОбъектУведомления,
	Дата,
	Сообщение) Экспорт
	
	РегистрыСведений.СообщенияPushУведомлений.ДобавитьЗапись(ПолучательУведомления,
		ОбъектУведомления,
		Дата,
		Сообщение);
...
Процедура СоздатьPushУведомленияОНовыхПисьмах()
	...
	Запрос.Текст = 
		...
		|
		|ВЫБРАТЬ
		|	СведенияОМобильныхОнлайнКлиентах.Пользователь
		|ИЗ
		|	Справочник.Пользователи КАК Пользователи
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.МК_СведенияОМобильныхОнлайнКлиентах КАК СведенияОМобильныхОнлайнКлиентах
		|		ПО (СведенияОМобильныхОнлайнКлиентах.Пользователь = Пользователи.Ссылка)
		|ГДЕ
		|	СведенияОМобильныхОнлайнКлиентах.ИдентификаторПодписчикаУстановлен
		|	И НЕ Пользователи.ПометкаУдаления
		|	И НЕ Пользователи.Недействителен
		|
		|ИНДЕКСИРОВАТЬ ПО
		...
		|			ТОГДА УчетныеЗаписиЭлектроннойПочты.Сотрудник
		...
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.УчетныеЗаписиЭлектроннойПочты.ОтветственныеЗаОбработкуПисем КАК
		|			УчетныеЗаписиЭлектроннойПочты
		|		ПО (СотрудникиПользователей.Сотрудник = УчетныеЗаписиЭлектроннойПочты.Сотрудник)
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ СписокПользователей КАК СписокПользователей
		|		ПО (СотрудникиПользователей.Пользователь = СписокПользователей.Пользователь)
		|ГДЕ
		...
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ОбработкаНовыхПисемДляОтправкиPushУведомлений КАК
		|			ОбработкаНовыхПисемДляОтправкиPushУведомлений
		|		ПО УчетныеЗаписи.Пользователь = ОбработкаНовыхПисемДляОтправкиPushУведомлений.Пользователь
		|ГДЕ
		|	УчетныеЗаписи.Пользователь В
		|		(ВЫБРАТЬ
	...
	Для каждого СтрокаТЧ Из ТаблицаДанных Цикл
		...
		ОтправлятьPushПерсональнаяНастройка = ВстроеннаяПочтаСервер.ПолучитьПерсональнуюНастройку(
			"ОтправлятьPushУведомленияОНовыхПисьмах",
			СтрокаТЧ.Пользователь);
		
		ОтправлятьPushОтПользователя = ?(ТипЗнч(ОтправлятьPushПерсональнаяНастройка) <> Тип("Булево"),
			Истина, ОтправлятьPushПерсональнаяНастройка);	
		
		Если Не ОтправлятьPushОтПользователя Тогда
			Продолжить;
		КонецЕсли;
		
		ДопОписаниеНастройки = Строка(СтрокаТЧ.УчетнаяЗапись);
		
		ОтправлятьPushПерсональнаяНастройкаПоУчетнойЗаписи = ВстроеннаяПочтаСервер.ПолучитьПерсональнуюНастройку(
			"ОтправлятьPushУведомленияОНовыхПисьмах", СтрокаТЧ.Пользователь , ДопОписаниеНастройки); 
			
		ОтправлятьPushОтУчетнойЗаписи = ?(ТипЗнч(ОтправлятьPushПерсональнаяНастройкаПоУчетнойЗаписи) <> Тип("Булево"),
			Истина, ОтправлятьPushПерсональнаяНастройкаПоУчетнойЗаписи);	
		
		СтрокаТЧ.Уведомлять = ОтправлятьPushОтУчетнойЗаписи;
	...
	ЗапросПисьма.Текст =
		...
		|	ТаблицаЗаписей.Уведомлять КАК Уведомлять,
		|	ТаблицаЗаписей.ДатаПроверки КАК ДатаПроверки
		...
		|ВЫБРАТЬ
		|	ТаблицаЗаписей.Пользователь КАК Пользователь,
		|	КОЛИЧЕСТВО(ТаблицаЗаписей.УчетнаяЗапись) КАК УчетнаяЗапись
		|ПОМЕСТИТЬ ТаблицаЗаписейКолВоУчЗаписей
		|ИЗ
		|	ТаблицаЗаписей КАК ТаблицаЗаписей
		|СГРУППИРОВАТЬ ПО
		...
		|	ТаблицаЗаписей.Уведомлять КАК Уведомлять,
		|	ТаблицаЗаписей.ДатаПроверки КАК ДатаПроверки,
		|	ТаблицаЗаписейКолВоУчЗаписей.УчетнаяЗапись КАК КолвоУчЗаписей
		|ПОМЕСТИТЬ ТаблицаЗаписейИТаблицаКолВоУчЗаписей
		|ИЗ
		|	ТаблицаЗаписей КАК ТаблицаЗаписей
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ТаблицаЗаписейКолВоУчЗаписей КАК ТаблицаЗаписейКолВоУчЗаписей
		|		ПО ТаблицаЗаписей.Пользователь = ТаблицаЗаписейКолВоУчЗаписей.Пользователь
		|ГДЕ
		...
		|		ИНАЧЕ ЛОЖЬ
		|	КОНЕЦ КАК Важное,
		|	ТаблицаЗаписейИТаблицаКолВоУчЗаписей.Пользователь КАК Пользователь,
		|	ПисьмаВПапках.Папка КАК Папка,
		|	ТаблицаЗаписейИТаблицаКолВоУчЗаписей.КолвоУчЗаписей КАК КолвоУчЗаписей,
		...
		|			ИЗ
		|				ТаблицаЗаписей КАК ТаблицаЗаписей))
		|		ЛЕВОЕ СОЕДИНЕНИЕ ТаблицаЗаписейИТаблицаКолВоУчЗаписей КАК ТаблицаЗаписейИТаблицаКолВоУчЗаписей
		|		ПО ВходящееПисьмо.УчетнаяЗапись = ТаблицаЗаписейИТаблицаКолВоУчЗаписей.УчетнаяЗапись
		...
		|		ПО ВходящееПисьмо.УчетнаяЗапись = УчетныеЗаписиЭлектроннойПочты.Ссылка
		|ГДЕ
	...
	Пока Выборка.Следующий() Цикл
		...
		Сообщение = СтрШаблон("%1%2 от %3, %4",
			?(Выборка.Важное, НСтр("ru = 'Важное письмо'"), Нстр("ru = 'Письмо'")),
			?(Выборка.КолвоУчЗаписей > 1, " на " + Выборка.АдресЭлектроннойПочты, ""),
			Выборка.ОтправительАдресат,
			Выборка.Тема);
...
Процедура ВыполнитьОтправкуPushУведомлений();

	ИспользоватьPushУведомления = Константы.ВариантИспользованияPushУведомлений.Получить();
	ИспользоватьМобильныеПриложения = Константы.ИспользоватьМобильныеПриложения.Получить();
	
	// Массив записей протокола. Объеденится в строку перед записью в РС.
	Протокол = Новый Массив;
	
	Протокол.Добавить(НСтр("ru = 'Использовать push-уведомления:'") + ИспользоватьPushУведомления);
	...
	Протокол.Добавить(НСтр("ru = 'Дата последней отправки:'") + ДатаПоследнейОтправки);
	Протокол.Добавить(НСтр("ru = 'Текущая дата:'") + ТекущаяДата);
...
//
Функция ДанныеАутентификацииДляОтправкиPushУведомленияМП(Использовать1С, ИспользоватьAPNS, ИспользоватьFCM)

	// По умолчанию никаких данных нет
	ДанныеАутентификации = Неопределено;

	Если Использовать1С = Истина Тогда
		КлючОтправки = СокрЛП(Константы.ЛогинСервисаПередачиУведомлений1С.Получить());
		ДанныеАутентификации = ?(ЗначениеЗаполнено(КлючОтправки), КлючОтправки, Неопределено);
	Иначе
...
//
// Возвращаемое значение:
//  ДанныеАутентификации - Строка - код отправителя
...
Функция ДанныеДляУдаленияИОтправки(ДатаПоследнейОтправки, ТекущаяДата)
	...
	Запрос.Текст = 
		...
		|	СообщенияPushУведомлений.ДатаОтправки КАК ДатаОтправки,
		|	СообщенияPushУведомлений.Сообщение КАК Сообщение,
		|	ВЫРАЗИТЬ(МП_СведенияОПользователях.ПользовательМобильногоПриложения КАК
		|		Справочник.ПользователиМобильногоПриложения).ИдентификаторКлиента КАК ИдентификаторКлиента,
		|	МП_СведенияОПользователях.Описание КАК ОписаниеПользователяМП,
		|	НЕОПРЕДЕЛЕНО КАК ИдентификаторКлиентаМК,
		|	&ТипПриложенияМобильноеПриложение КАК ТипПриложения
		|ИЗ
		|	РегистрСведений.СообщенияPushУведомлений КАК СообщенияPushУведомлений
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.МП_СведенияОПользователях КАК МП_СведенияОПользователях
		|		ПО СообщенияPushУведомлений.Пользователь = МП_СведенияОПользователях.ПользовательМобильногоПриложения.Пользователь
		|ГДЕ
		|	СообщенияPushУведомлений.ДатаОтправки > &ДатаПоследнейОтправки
		|	И СообщенияPushУведомлений.ДатаОтправки <= &ТекущаяДата
		|	И МП_СведенияОПользователях.ПользовательМобильногоПриложения.ИдентификаторУстановлен
		|	И НЕ МП_СведенияОПользователях.ПользовательМобильногоПриложения.ПометкаУдаления
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|ВЫБРАТЬ
...
Функция УдалитьНеактуальныеСообщенияPushУведомлений(РезультатЗапроса, Протокол)
	...
	Протокол.Добавить(НСтр("ru = 'Записей к удалению: '") + ВозвращаемоеЗначение.Всего);
	...
	Протокол.Добавить(НСтр("ru = 'Записей удалено: '") + ВозвращаемоеЗначение.Удалено);
...
//  ИспользоватьМобильныеПриложения - Константа - Показывает что включены push уведомления на мобильные приложения
//
Функция ОтправитьСообщенияИспользуяPushУведомления(
	...
	ДанныеАутентификацииМК = Неопределено;
	ДанныеАутентификацииМК = ДанныеАутентификацииДляОтправкиPushУведомленияМК();
				
	Если ДанныеАутентификацииМК = Неопределено Тогда 
		ЗаписьЖурналаРегистрации(ИмяСобытия, УровеньЖурналаРегистрации.Ошибка,
				, , НСтр("ru = 'Отправка push-уведомлений невозможна т.к. не определены данные аутентификации МК'"));		
		Возврат ВозвращаемоеЗначение;
	КонецЕсли;
	...
	Пока ВыборкаОбъектов.Следующий() Цикл
		...
		Пока ВыборкаТипПриложения.Следующий() Цикл
			...
			Пока ВыборкаСообщений.Следующий() Цикл
				...
				ДоставляемоеУведомление = Новый ДоставляемоеУведомление;
				ДоставляемоеУведомление.Заголовок = Заголовок;
				ДоставляемоеУведомление.Текст = ВыборкаСообщений.Сообщение;
				Если ЗначениеЗаполнено(ОбъектУведомления) Тогда
					ДоставляемоеУведомление.Данные = ЗначениеВСтрокуВнутр(ОбъектУведомления);
				КонецЕсли;
				...
				Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
					...
					Если ВыборкаДетальныеЗаписи.ТипПриложения = ТипПриложенияМобильныйКлиент
						...
						РегистрыСведений.МК_СведенияОМобильныхОнлайнКлиентах.УстановитьОшибкуОтправкиУведомлений(
					...
					ДоставляемоеУведомление.Получатели.Добавить(Идентификатор);
				
				КонецЦикла;
				
				// Если нет получателей выходим.
				Если ДоставляемоеУведомление.Получатели.Количество() = 0 Тогда
					Продолжить;	
				КонецЕсли;
				
				// Отправляем полученные сообщения на Мобильное приложение.
				Если ВыборкаТипПриложения.ТипПриложения = ТипПриложенияМобильноеПриложение Тогда
					РезультатОтправки = ВыполнитьОтправкуСообщения(ДоставляемоеУведомление, 
				...
				ВозвращаемоеЗначение.Всего =
					ВозвращаемоеЗначение.Всего + ДоставляемоеУведомление.Получатели.Количество();
