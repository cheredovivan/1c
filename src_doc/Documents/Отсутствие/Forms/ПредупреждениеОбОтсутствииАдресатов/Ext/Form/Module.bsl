...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Для Каждого Отсутствие Из Параметры.Отсутствия Цикл
		...
		Пока ВыборкаЗамещающих.Следующий() Цикл
			...
			ПараметрыОтбора.Вставить("Заместитель", ВыборкаЗамещающих.Замещающий);
			Если Заместители.НайтиСтроки(ПараметрыОтбора).Количество() = 0 Тогда
				...
				НоваяСтрока.Заместитель = ВыборкаЗамещающих.Замещающий;
			КонецЕсли;
			Если ЗаместителиДляПроверки.Найти(ВыборкаЗамещающих.Замещающий) = Неопределено Тогда
				ЗаместителиДляПроверки.Добавить(ВыборкаЗамещающих.Замещающий);
			КонецЕсли;
		КонецЦикла;
...
Функция ВсеАдресатыОтсутсвуют(Адресаты)
	...
	Для Каждого Адресат Из Адресаты Цикл
		...
		Если Адресат.ТипАдреса <> Перечисления.ТипыАдресатов.Кому Тогда
