...
Процедура ЗаполнитьДескрипторыОбъекта(ОбъектДоступа, ТаблицаДескрипторов, ПротоколРасчетаПрав = Неопределено) Экспорт
	...
	ПользовательСотрудника = Сотрудники.ПользовательСотрудника(ОбъектДоступа.Сотрудник);
	...
	Если ПротоколРасчетаПрав <> Неопределено Тогда
		ЗаписьПротокола = Новый Структура("Элемент, Описание",
			ОбъектДоступа.Сотрудник, НСтр("ru = 'Пользователь'"));
...
Функция ПечатьЕжедневногоОтчета(МассивОбъектов, ОбъектыПечати, ПараметрыПечати) Экспорт
	...
	Для Каждого Объект Из МассивОбъектов Цикл
		...
		РеквизитыОбъекта = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Объект,
			"Дата, Сотрудник, СпособУказанияВремени");
		
		ОбластьШапка.Параметры.Сотрудник = РеквизитыОбъекта.Сотрудник;
		ОбластьШапка.Параметры.Дата = Формат(РеквизитыОбъекта.Дата, "ДЛФ=D");
		ТабличныйДокумент.Вывести(ОбластьШапка);
		
		Работы = УчетВремениВызовСервера.ПолучитьРаботыЗаДень(РеквизитыОбъекта.Дата, РеквизитыОбъекта.Сотрудник);
...
Процедура ЗаполнитьРеквизитДлительностьРабочая(Параметры) Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		Попытка
			...
			Работы = УчетВремениВызовСервера.ПолучитьРаботыЗаДень(Документ.Дата, 
				Сотрудники.ЛюбойПользовательСотрудника(Документ.Сотрудник));
			...
			Для Каждого Работа Из Работы Цикл
				...
				КонецЕсли;
			КонецЦикла;
			Документ.ДлительностьРабочая = УчетВремениКлиентСервер.ЧислоВСтроку(ДлительностьРабочегоВремениСек);
			
			ОбновлениеИнформационнойБазы.ЗаписатьДанные(Документ);
			
			Движения = РегистрыНакопления.РабочееВремяСотрудников.СоздатьНаборЗаписей();
