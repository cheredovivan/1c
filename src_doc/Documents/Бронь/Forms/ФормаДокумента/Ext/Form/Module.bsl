...
Процедура ПриОткрытии(Отказ)
	...
	Если ЕстьПересекающиесяБрони И Не ТолькоПросмотр Тогда
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	Если ИмяСобытия = "Запись_Бронь" Тогда
		...
		Если Объект.Ссылка = Параметр Тогда
			Прочитать();
		ИначеЕсли ТипЗнч(Параметр) = Тип("Массив") И Параметр.Найти(Объект.Ссылка) <> Неопределено Тогда
			Прочитать();
		Иначе
...
Процедура ПослеЗаписиНаСервере(ТекущийОбъект, ПараметрыЗаписи)
	...
	ПараметрыЗаписи.РезультатБронирования.РезультатыБроней.Добавить(ТекущийОбъект.РезультатБрони());
...
Процедура ПослеЗаписи(ПараметрыЗаписи)
	...
	КлючеваяОперация = БронированиеПомещенийКлиентСервер.КлючеваяОперацияЗаписьБрони();
...
Процедура ПользовательОбработкаВыбора(Элемент, ВыбранноеЗначение, ДополнительныеДанные, СтандартнаяОбработка)
	...
	СотрудникиКлиент.СотрудникОбработкаВыбора(Объект, "Пользователь", ВыбранноеЗначение, СтандартнаяОбработка);
...
Процедура УстановитьДоступностьИВидимостьКоманд()
	...
	ЭтоАвтоматическаяБронь = Объект.СпособСозданияБрони = Перечисления.СпособыСозданияБрони.Автоматически
		Или Объект.СпособСозданияБрони = Перечисления.СпособыСозданияБрони.АвтоматическиУказанПредмет;
	
	Элементы.ФормаПовторение.Доступность =
		(Объект.ТипЗаписи <> Перечисления.ТипЗаписиКалендаря.ЭлементПовторяющегосяСобытия)
		И Не ЭтоАвтоматическаяБронь;
...
Процедура УстановитьРежимФормы()
	...
	Если ЗначениеЗаполнено(Объект.Ссылка)
		И Объект.СостояниеБрони <> Перечисления.СостоянияБроней.Отменена
		И ДокументооборотПраваДоступа.ПраваПользователяПоОбъекту(Объект.Ссылка).Изменение
		И Не ЭтоОбъектИзДругойСистемы Тогда
		
		// Режим "Изменение".
		// Настройки элементов формы по умолчанию соответсвуют режиму изменения.
...
Процедура УстановитьФокус()
	...
	ИначеЕсли Не ЗначениеЗаполнено(Объект.КоличествоЧеловек) Тогда
...
Процедура ОбновитьСостояние()
	...
	ИндексКартинкиСостояния = Перечисления.СостоянияБроней.ИндексКартинки(Объект.СостояниеБрони);
	Если Объект.СостояниеБрони = Перечисления.СостоянияБроней.ОжидаетПодтверждения Тогда
		ПредставлениеСостояния = Строка(Объект.СостояниеБрони);
	ИначеЕсли Объект.СостояниеБрони = Перечисления.СостоянияБроней.Отменена Тогда
		ПредставлениеСостояния = СтрШаблон(
			"%1 %2. %3.",
			Строка(Объект.СостояниеБрони),
			Формат(Объект.ДатаОтмены, "ДФ='dd.MM.yyyy HH:mm';"),
			Объект.ПричинаОтмены);
	Иначе
		ПредставлениеСостояния = "";
	КонецЕсли;
	
	Элементы.ИндексКартинкиСостояния.Подсказка = ПредставлениеСостояния;
	Элементы.ИндексКартинкиСостояния.Видимость =
		ИндексКартинкиСостояния <> -1 Или ЗначениеЗаполнено(ПредставлениеСостояния);
...
Процедура МК_НастроитьЭлементыФормы()
	...
	Элементы.Помещение.ОтображениеПодсказки = ОтображениеПодсказки.ОтображатьСправа;
