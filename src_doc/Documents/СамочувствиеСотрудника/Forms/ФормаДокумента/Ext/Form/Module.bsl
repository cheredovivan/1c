...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ТипЗнч(Объект.Сотрудник) = Тип("СправочникСсылка.Пользователи") Тогда
		Подразделение = Сотрудники.ПодразделениеСотрудника(
			Сотрудники.ОсновнойСотрудникПользователя(Объект.Сотрудник));
	Иначе
		Подразделение = Сотрудники.ПодразделениеСотрудника(Объект.Сотрудник);
	КонецЕсли;
	
	Если Не РольДоступна("ПолныеПрава")
		И Не РольДоступна("КонтрольСамочувствияСотрудников")
		И Не СтруктураПредприятия.ЭтоРуководитель(Сотрудники.ОсновнойСотрудник()) Тогда
...
Процедура СотрудникПриИзменении(Элемент)
	...
	Подразделение = СотрудникиВызовСервера.ПодразделениеСотрудника(Объект.Сотрудник);
