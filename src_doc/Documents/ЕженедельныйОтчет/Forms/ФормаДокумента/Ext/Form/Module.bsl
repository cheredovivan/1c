...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ТекущийПользовательСотрудника = Сотрудники.ЛюбойПользовательСотрудника(
		Объект.Сотрудник);
	Если Объект.Сотрудник = Сотрудники.ОсновнойСотрудникПользователя() Тогда
...
Процедура ПослеЗаписи()
	...
	Оповестить("ИзмененЕженедельныйОтчет", ПараметрыОповещения, ЭтотОбъект);
...
Процедура ПодобратьРаботы()
	...
	ОткрытьФорму("РегистрСведений.ФактическиеТрудозатраты.Форма.ФормаПодбораРабот", ПараметрыФормы, ЭтотОбъект);
...
Процедура ПроектЗадачаНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	РаботаСПроектамиКлиент.ВыбратьПроектЗадачу(ЭтотОбъект, ТекущиеДанные.Проект, ТекущиеДанные.ПроектнаяЗадача);
...
Процедура ДниНеделиВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	Если Поле.Имя = "ЕжедневныйОтчет" Тогда
		...
		ПараметрыФормы.ЗначенияЗаполнения.Вставить("Сотрудник", Объект.Сотрудник);
	...
	Если Поле.Имя = "Отсутствие" Тогда
		...
		ПараметрыФормы.Вставить("ЗначенияЗаполнения", Новый Структура);
		ПараметрыФормы.ЗначенияЗаполнения.Вставить("ДатаОтсутствия", Элементы.ДниНедели.ТекущиеДанные.Дата);
		ПараметрыФормы.ЗначенияЗаполнения.Вставить("Сотрудник", Объект.Сотрудник);
...
Процедура МК_СтраницаРаботы(Команда)
	...
	МК_ПереключитьНаСтраницу(ЭтотОбъект, Элементы.ГруппаРаботы,
		Элементы.МК_КнопкаСтраницаРаботы);
		
КонецПроцедуры

&НаКлиенте
Процедура МК_СтраницаДниНедели(Команда)
	
	МК_ПереключитьНаСтраницу(ЭтотОбъект, Элементы.ГруппаДниНедели,
		Элементы.МК_КнопкаСтраницаДниНедели);
		
КонецПроцедуры

&НаКлиенте
Процедура МК_СтраницаДостижения(Команда)
	
	МК_ПереключитьНаСтраницу(ЭтотОбъект, Элементы.ГруппаДостижения,
		Элементы.МК_КнопкаСтраницаДостижения);
		
КонецПроцедуры

&НаКлиенте
Процедура МК_СтраницаНеудачи(Команда)
	
	МК_ПереключитьНаСтраницу(ЭтотОбъект, Элементы.ГруппаНеудачи,
		Элементы.МК_КнопкаСтраницаНеудачи);
		
КонецПроцедуры

&НаКлиенте
Процедура МК_СтраницаПланы(Команда)
	
	МК_ПереключитьНаСтраницу(ЭтотОбъект, Элементы.ГруппаПланы,
		Элементы.МК_КнопкаСтраницаПланы);
...
Функция ЕстьДокументыПользователя()
	...
	|ГДЕ
	|	ЕженедельныйОтчет.Сотрудник = &Сотрудник
...
Процедура ЗаполнитьПоФактическимТрудозатратам()
	...
	Запрос = Новый Запрос(
		...
		|ГДЕ
		|	ФактическиеТрудозатраты.Сотрудник = &Сотрудник
...
Процедура ОбновитьФондРабочегоВремени()
	ФондТаблицей = УчетВремени.ФондРабочегоВремени(Объект.Сотрудник, ДатаНачала, КонецДня(Объект.Дата));
...
Процедура ОбновитьОтсутствия()
	...
	ПользовательМассивом = ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(ТекущийПользовательСотрудника);
...
Процедура МК_НастроитьЭлементыФормы()
	...
	Элементы.ДатаНачала.ФорматРедактирования = НСтр("ru='ДФ=''dd.MM.yy'''");
	Элементы.Дата.ФорматРедактирования = НСтр("ru='ДФ=''dd.MM.yy'''");
	Элементы.Дата.Ширина = 7;
	...
	Элементы.ОтступМК.Видимость = Истина;
	...
	Элементы.ГруппаДлительность.Группировка = ГруппировкаКолонок.Вертикальная;
	Элементы.ДниНедели.ВысотаВСтрокахТаблицы = 0;
	Элементы.ГруппаДниНедели.ЦветФона = WebЦвета.Белый;
	...
	Элементы.Переместить(Элементы.ДеньНедели, Элементы.ДеньНеделиДата);
	Элементы.ДеньНедели.ЦветТекста = ЦветаСтиля.МК_ЦветАвтораИсполнителя;
	Элементы.ДеньНедели.Шрифт = ШрифтыСтиля.МК_ШрифтАвтораИсполнителя;
	
	Элементы.МК_ДниНеделиКоманднаяПанель.Видимость = Истина;
	Элементы.ЗаполнитьДниНедели.Видимость = Ложь;
	
	Элементы.ДатаДня.Формат = НСтр("ru='ДФ=''dd MMMM yyyy'''");
	Элементы.ДатаДня.Ширина = 12;
	
	Элементы.ЕжедневныйОтчет.Ширина = 12;
	...
	Элементы.ГруппаРаботы.ЦветФона = ЦветаСтиля.МК_ЦветФонаГруппы;
	...
	Элементы.Переместить(Элементы.Работа, Элементы.МК_ПерваяКолонка);
	Элементы.Переместить(Элементы.ВидРабот, Элементы.МК_ПерваяКолонка);
	Элементы.Переместить(Элементы.Итого, Элементы.МК_ВтораяКолонка);
	...
	Элементы.Сб.Видимость = Ложь;
	Элементы.Вс.Видимость = Ложь;
	Элементы.Пн.Видимость = Ложь;
	Элементы.Вт.Видимость = Ложь;
	Элементы.Ср.Видимость = Ложь;
	Элементы.Чт.Видимость = Ложь;
	Элементы.Пт.Видимость = Ложь;
	Элементы.ПроектЗадача.Видимость = Ложь;
	
	// Страницы.
	Элементы.ДостиженияШапка.ПодсказкаВвода = НСтр("ru = 'Достижения'");
	Элементы.НеудачиШапка.ПодсказкаВвода = НСтр("ru = 'Неудачи'");
	Элементы.Планы.ПодсказкаВвода = НСтр("ru = 'Планы'");
	
	Элементы.ГруппаДостижения.ЦветФона = ЦветаСтиля.МК_ЦветФонаГруппы;
	Элементы.ГруппаНеудачи.ЦветФона = ЦветаСтиля.МК_ЦветФонаГруппы;
	Элементы.ГруппаПланы.ЦветФона = ЦветаСтиля.МК_ЦветФонаГруппы;
	...
	Элементы.МК_КнопкаСтраницаРаботы.Видимость = Элементы.ГруппаРаботы.Видимость;
	
	Если Элементы.ГруппаРаботы.Видимость Тогда
		МК_ПереключитьНаСтраницу(ЭтотОбъект, Элементы.ГруппаРаботы,
			Элементы.МК_КнопкаСтраницаРаботы);
	Иначе
		МК_ПереключитьНаСтраницу(ЭтотОбъект, Элементы.ГруппаДниНедели,
...
Процедура МК_СброситьОформлениеПереключателейСтраницПредпросмотра(Форма)
	...
	МК_КлиентСервер.ОформитьКнопкуФильтра(
		Форма.МК_ЭлементыСтиля, Форма.Элементы.МК_КнопкаСтраницаДниНедели, Истина, Ложь);
		
	МК_КлиентСервер.ОформитьКнопкуФильтра(
		Форма.МК_ЭлементыСтиля, Форма.Элементы.МК_КнопкаСтраницаДостижения, Истина, Ложь);
		
	МК_КлиентСервер.ОформитьКнопкуФильтра(
		Форма.МК_ЭлементыСтиля, Форма.Элементы.МК_КнопкаСтраницаНеудачи, Истина, Ложь);
		
	МК_КлиентСервер.ОформитьКнопкуФильтра(
		Форма.МК_ЭлементыСтиля, Форма.Элементы.МК_КнопкаСтраницаПланы, Истина, Ложь);
