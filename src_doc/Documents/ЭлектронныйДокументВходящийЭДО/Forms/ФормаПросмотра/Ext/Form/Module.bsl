...
Процедура НастроитьОформлениеФормы(ЭлементСхемы)
	...
	ДанныеПредставленияСостояния = Новый Массив;
	ДанныеПредставленияСостояния.Добавить(НСтр("ru = 'Состояние:'"));
	ДанныеПредставленияСостояния.Добавить(" ");
	...
	КонецЕсли;
	ДанныеПредставленияСостояния.Добавить(
		Новый ФорматированнаяСтрока(ПредставлениеСостояния, ШрифтыСтиля.ЖирныйШрифтБЭД));
	Если ЗначениеЗаполнено(СостояниеДокументаПодробное.Комментарий) Тогда
		КомментарийСостояния = СостояниеДокументаПодробное.Комментарий;
		ДанныеПредставленияСостояния.Добавить(" ");
		ДанныеПредставленияСостояния.Добавить(Новый ФорматированнаяСтрока(БиблиотекаКартинок.Найти,,,, "ПоказатьКомментарийСостояния"));
	КонецЕсли;
	Если ЗначениеЗаполнено(ПричинаОтклонения) Тогда
		ДанныеПредставленияСостояния.Добавить(" ");
		ДанныеПредставленияСостояния.Добавить(НСтр("ru = 'Причина:'"));
		ДанныеПредставленияСостояния.Добавить(" ");
		ДанныеПредставленияСостояния.Добавить(ПричинаОтклонения);
	КонецЕсли;
	
	ДополнительноеСостояние = Неопределено;
	
	Если ДополнительноеСостояние <> Неопределено Тогда
		
		ДанныеПредставленияСостояния.Добавить(" ");
		ДанныеПредставленияСостояния.Добавить(ДополнительноеСостояние);
		
	КонецЕсли;
	
	Элементы.НадписьСостояниеПричина.Заголовок = Новый ФорматированнаяСтрока(ДанныеПредставленияСостояния);
	Элементы.ВидДокумента.Видимость = Элементы.ВидДокумента.СписокВыбора.Количество() > 1;
...
Процедура НастроитьЭлементыОтраженияВУчете()
	...
	Если ЕстьПравоОбработкиДокумента Тогда
		Элементы.ГруппаСоздатьДокументУчета.Видимость = Не ЕстьОдинДУ 
			...
			Или Истина // Создание докумнета учета в ДО возможно для любых форматов. О возможности переопределения ожидаем отклик БЭД
	...
	Если ЕстьОдинДУ Тогда
		...
		// Документооборот
		Если СтрНайти(Строка(Основания[0].ОбъектУчета), "<Объект не найден>") <> 0 Тогда
			
			Элементы.КомандаОткрытьДокументУчета1.Заголовок = ОбменСКонтрагентамиДОСервер.ТекстНадписиНетДоступаКОбъектуУчета();
