...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не ВстроеннаяПочтаСервер.ВстроеннаяПочтаИспользуется() Тогда
		...
		Отказ = Истина;
		ТекстСообщения = НСтр("ru = 'Для просмотра писем включите персональную настройку ""Встроенная почта"".'");
...
&НаКлиенте
Процедура МК_ПоказатьСкрытьПрочиеРеквизиты(Команда)
	
	Если Элементы.Страницы.ТекущаяСтраница = Элементы.Страница Тогда
			Элементы.Страницы.ТекущаяСтраница = Элементы.МК_СтраницаРеквизиты;
			Элементы.МК_ФормаПоказатьСкрытьПрочиеРеквизиты.Пометка = Истина;
		Иначе
			Элементы.Страницы.ТекущаяСтраница = Элементы.Страница;
			Элементы.МК_ФормаПоказатьСкрытьПрочиеРеквизиты.Пометка = Ложь;
	КонецЕсли;
...
Процедура СвязиОбъекта(Команда)
	...
	ОткрытьФорму("РегистрСведений.СвязиОбъектов.Форма.ФормаСвязейОбъекта", ПараметрыФормы);
...
Процедура ВложенияВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	ИначеЕсли ВстроеннаяПочтаКлиентСервер.ЭтоФайлПриглашение(Элемент.ТекущиеДанные.Представление, РасширенияICS) Тогда	
		...
		ВстроеннаяПочтаКлиент.ОткрытьПриглашение(Объект.Ссылка, Элемент.ТекущиеДанные.Ссылка, УникальныйИдентификатор);
...
Процедура ОтобразитьСвязиПисьма()
	...
	ВсеСвязиДокумента = СвязиОбъектов.ПолучитьВсеСвязиОбъекта(Объект.Ссылка, ПолучатьПомеченныеНаУдаление);
	Если ВсеСвязиДокумента.Количество() > 0 Тогда
		...
		Если НайденнаяСтрока <> Неопределено Тогда
			ПолученоВОтветНаДокумент = НайденнаяСтрока.СвязанныйОбъект;			
		...
		Иначе
			...
			Если НайденнаяСтрока <> Неопределено Тогда
				ПолученоВОтветНаПисьмо = НайденнаяСтрока.СвязанныйОбъект;
		...
		Для каждого Строка Из НайденныеСтроки Цикл
			ОтправленОтвет.Добавить(Строка.СвязанныйОбъект);
		КонецЦикла;
		...
		НайденныеСтроки = ВсеСвязиДокумента.НайтиСтроки(СтруктураОтбора);
		Для каждого Строка Из НайденныеСтроки Цикл
			ПересланоПисьмом.Добавить(Строка.СвязанныйОбъект);
		КонецЦикла;
...
Процедура МК_НастроитьЭлементыФормы()
	...
	Элементы.Переместить(Элементы.ПоказатьПараметрыПисьма, Элементы.МК_ПодменюСервис);
	Элементы.Переместить(Элементы.ФормаПоказатьИсходныйТекст, Элементы.МК_ПодменюСервис);
	Элементы.Переместить(Элементы.СвязиОбъекта, Элементы.МК_ПодменюСервис);
	Элементы.Переместить(Элементы.ЗадачиПоПредмету, Элементы.МК_ПодменюСервис);
	Элементы.Переместить(Элементы.ПротоколРаботы, Элементы.МК_ПодменюСервис);
	
	Элементы.Переместить(Элементы.ИзменитьКодировку, Элементы.МК_ПодменюНастройки);
	Элементы.Переместить(Элементы.ИзменитьШрифтОтображения, Элементы.МК_ПодменюНастройки);
	
	// Установим нужный вид некоторых команд.
	Элементы.Ответить.Картинка = БиблиотекаКартинок.МК_КомандаОтветить;
	Элементы.ОтветитьВсем.Картинка = БиблиотекаКартинок.МК_КомандаОтветитьВсем;
	Элементы.Переслать.Картинка = БиблиотекаКартинок.МК_КомандаПереслать;
	Элементы.ПереместитьВКорзину.Картинка = БиблиотекаКартинок.МК_КомандаКорзина;
	Элементы.Контроль.Картинка = БиблиотекаКартинок.МК_КомандаКонтроль;
	...
	Элементы.ГруппаПисьмо.Ширина = 0;
	...
	Элементы.Переместить(Элементы.ГруппаВложенияРеквизиты, Элементы.МК_СтраницаРеквизиты, Элементы.МК_ПоказатьСкрытьПрочиеРеквизиты);
	МК.ВыровнятьЭлементФормы(Элементы.ПисемВПереписке);
	МК.ВыровнятьЭлементФормы(Элементы.ДатаОтправки);
	МК.ВыровнятьЭлементФормы(Элементы.Дата);
	МК.ВыровнятьЭлементФормы(Элементы.ДатаПолучения);
	МК.ВыровнятьЭлементФормы(Элементы.ПредметСтрокой);
	МК.ВыровнятьЭлементФормы(Элементы.ПроектСтрокой);
	...
	Элементы.Переместить(Элементы.ВсеРеквизиты, Элементы.ГруппаВложенияРеквизиты, Элементы.ГруппаДекорации);
	МК.ОформитьКнопкуФильтра(Элементы.ВсеРеквизиты, Истина);
	МК.ОформитьКнопкуФильтра(Элементы.ЗадачиПоПриложению, Истина);
	
	// Вложения.
	Если Вложения.Количество() > 0 Тогда
		
		Элементы.МК_Вложения.Видимость = Истина;
		Элементы.Вложения.Видимость = Не Элементы.Вложения.Видимость;
		МК_ПоказатьСкрытьВложенияНаСервере();
		...
		Элементы.Переместить(Элементы.Вложения, Элементы.МК_Вложения);
	...
	ГруппаПереписка.ЦветФона = ЦветаСтиля.МК_ЦветФонаГруппы;
	ГруппаПереписка.ОтображатьЗаголовок = Ложь;
...
Процедура МК_ПоказатьСкрытьВложенияНаСервере()
	...
	Если КоличествоВложений = 0 Тогда
		Элементы.МК_Вложения.Видимость = Ложь;
	КонецЕсли;
