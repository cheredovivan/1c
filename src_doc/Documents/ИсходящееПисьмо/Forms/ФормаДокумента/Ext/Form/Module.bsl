...
&НаКлиенте
Перем ТекущийНомерУзлаНачалаВыделения; // номер текущего узла в HTML письме, где начинается выделение

&НаКлиенте
Перем ТекущееСмещениеНачалаВыделения; // текущее смещение в узле, где начинается выделение

&НаКлиенте
Перем ТекущийНомерУзлаКонцаВыделения; // номер текущего узла в HTML письме, где заканчивается выделение

&НаКлиенте
Перем ТекущееСмещениеКонцаВыделения; // текущее смещение в узле, где заканчивается выделение
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка) 
	...
	Если Не ВстроеннаяПочтаСервер.ВстроеннаяПочтаИспользуется() Тогда
		...
		Отказ = Истина;
		ТекстСообщения = НСтр("ru = 'Для создания писем включите персональную настройку ""Встроенная почта"".'");
	...
	Если Не ПриложениеЯвляетсяВебКлиентом Тогда
		Команды.ВставитьТекстИзБуфера.СочетаниеКлавиш 
			= Новый СочетаниеКлавиш(Клавиша.V, Ложь, Истина, Ложь);
	КонецЕсли;	
	...
	Если ПисьмоОтправлено Или Объект.ТипОтвета = Перечисления.ТипыОтвета.ПеренаправлениеПисьма Тогда
		...
		Элементы.ВставитьСтроки.Доступность = Ложь;
		...
		Элементы.НайтиАдресатовТекст.Видимость = Ложь;
	...
	Если ПисьмоОтправлено Тогда
		...
		Элементы.ВложенияКонтекстноеМенюДобавитьФайлИзБазы.Видимость = Ложь;
	КонецЕсли;
	
	Если ПриложениеЯвляетсяВебКлиентом Тогда
		Элементы.ВставитьКартинкуИзБуфера.Видимость = Ложь;
		Элементы.ТекстПисьмаHTMLКонтекстноеМенюВставитьКартинкуИзБуфера.Видимость = Ложь;
		Элементы.ВставитьКартинку.Видимость = Ложь;
		Элементы.ТекстПисьмаHTMLКонтекстноеМенюВставитьКартинку.Видимость = Ложь;
	КонецЕсли;
	...
	СписокВыбора.Добавить(ПредопределенноеЗначение("Справочник.ДокументыПредприятия.ПустаяСсылка"), НСтр("ru = 'Документ'"));
	...
	Если СтатусПисьма = 9 Тогда // Отозвано
		Элементы.СтатусПисьма.Гиперссылка = Истина;
	КонецЕсли;
...
Процедура ПриЧтенииНаСервере(ТекущийОбъект)
	...
	ЗаполнитьЕстьДокументыСтрока(ТекущийОбъект.Ссылка);
...
Процедура ПриОткрытии(Отказ)
	...
	ЭтоLinuxКлиент = Истина; // на платформе 8.3.27 всегда работаем в этом режиме, в том числе и в Windows.
...
#Если Не МобильныйКлиент Тогда
	// МобильныйСканер
	Если ПоддерживаетсяМобильныйСканер И (Не ПисьмоОтправлено) Тогда
...
Процедура ПередЗаписьюНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	НедействительныеАдреса = Новый Массив;
	ЕстьДействительныеАдресаты = Ложь;
	ПредставленияНедействительныхАдресатов.Очистить();
	НедействительныеАдресаСоответствие = Новый Соответствие;
	...
	Для Счетчик = 0 По Получатели.Количество() - 1 Цикл
		...
		Если ВыполняетсяОтправка И Не ПустаяСтрока(Строка.Адрес) Тогда		
			
			Если ЗначениеЗаполнено(Строка.Адресат) 
				И ВстроеннаяПочтаСервер.ЭтоНедействительныйАдрес(Строка.Адресат.Адрес) Тогда
				
				НедействительныеАдреса.Добавить(Строка.Адресат.Адрес);
				НедействительныеАдресаСоответствие.Вставить(Строка.Адресат.Адрес, 1);
				
			Иначе
				
				ЕстьДействительныеАдресаты = Истина;
				
			КонецЕсли;	
			
			Если УдалятьНедействительныеАдресатыПриОтправке
				И НедействительныеАдресаСоответствие.Получить(Строка.Адрес) <> Неопределено Тогда
				
				// Этот адресат недействителен и стоит флаг УдалятьНедействительныеАдресатыПриОтправке
				//  - не добавляем адресата в список получателей
				Продолжить; 
		...
		Если Строка.ТипАдреса = Перечисления.ТипыАдресатов.Копия Тогда
		...
		Если Строка.ТипАдреса = Перечисления.ТипыАдресатов.ОбратныйАдрес Тогда
	...
	МассивСтрокОшибокПолучателей = Новый Массив;
	
	// Получаем представления для недействительных адресов
	Если ВыполняетсяОтправка И НедействительныеАдреса.Количество() <> 0 
		И Не УдалятьНедействительныеАдресатыПриОтправке Тогда
		
		ПредставленияНедействительныхАдресатов = ВстроеннаяПочтаСервер.ПолучитьПредставленияНедействительныхПользователей(НедействительныеАдреса);
		
		// Создаем сообщения об ошибках только в случае если нет ни одного действительного адресата (иначе показываем диалог)
		Если Не ЕстьДействительныеАдресаты Тогда
			
			Для Каждого СтрокаСписка Из ПредставленияНедействительныхАдресатов Цикл
				
				СтрокаОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Сотрудник ""%1"" недействителен.
						|Выберите другого получателя письма.'"), СтрокаСписка.Значение);
				МассивСтрокОшибокПолучателей.Добавить(СтрокаОшибки);		
	...
	Если ВыполняетсяОтправка Тогда
		...
		Если Не ПроверитьПисьмоПередОтправкой(ТекущийОбъект, МассивСтрокОшибокПолучателей) Тогда
		...
		Если ТипЗнч(Параметр) = Тип("Структура") Тогда 
			...
			Если ТипЗнч(Результат) = Тип("Массив") И Результат.Количество() <> 0 Тогда
				...
				Иначе 
					...
					ИдСтроки = -1; // идентификатор текущей строки
						
					Если Элементы.Получатели.ТекущаяСтрока <> Неопределено
						И Не ЗначениеЗаполнено(Элементы.Получатели.ТекущиеДанные.Представление) Тогда
						ИдСтроки = Элементы.Получатели.ТекущаяСтрока;
					КонецЕсли;	
					
					ЗанестиВПолучателиВыбранныеСтроки(
						Результат, 
						ИдСтроки , // идентификатор текущей строки
						ПараметрыЗаполненияТекущейСтроки,
						ТипАдреса);
...
#Если Не ВебКлиент Тогда
		// Форма восстановления писем не работает в веб-клиенте.
...
Процедура ЗаполнитьНовоеПисьмо(Параметры)
	...
	Если Параметры.Свойство("ЗначенияЗаполнения")
		...
		Если (Объект.ТипТекста = Перечисления.ТипыТекстовПочтовыхСообщений.HTML 
			И Не ПриложениеЯвляетсяВебКлиентом)
			И Не СкопироватьВложения Тогда
	...
	ИначеЕсли Параметры.Свойство("Основание")
		...
		Если ДелопроизводствоКлиентСервер.ЭтоДокумент(Параметры.Основание)
			...
			Если ДелопроизводствоКлиентСервер.ЭтоДокумент(Параметры.Основание) Тогда
				ДокументПодписан = ДействияСервер.ДокументПодписан(Параметры.Основание);
			КонецЕсли;	
			...
			ДобавитьВложениеФайлы(МассивФайлов, Истина, ДокументПодписан, Параметры.Основание); // ИгнорироватьЗанятость=Истина
		...
		ИначеЕсли ДелопроизводствоКлиентСервер.ЭтоВходящийДокумент(Параметры.Основание) Тогда
			...
			ПредметПереписки = СвязиОбъектов.ПолучитьСвязанныйОбъект(Параметры.Основание, Справочники.ТипыСвязей.ПредметПереписки);
		...
		ИначеЕсли ДелопроизводствоКлиентСервер.ЭтоИсходящийДокумент(Параметры.Основание) Тогда
			...
			ПисьмоОтправленоВОтветНа = СвязиОбъектов.ПолучитьСвязанныйОбъект(Параметры.Основание, Справочники.ТипыСвязей.ОтправленВОтветНа);
			ПредметПереписки = СвязиОбъектов.ПолучитьСвязанныйОбъект(Параметры.Основание, Справочники.ТипыСвязей.ПредметПереписки);
			Если ЗначениеЗаполнено(ПредметПереписки) Тогда
				Объект.Предмет = ПредметПереписки;
			КонецЕсли;
		...
		Если ТипЗнч(Параметры.Файлы) = Тип("Массив") И Параметры.Файлы.Количество() > 0 Тогда
			...
			Для каждого Файл Из Параметры.Файлы Цикл
				...
				ДанныеФайла = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Файл,
					"Наименование, ИндексКартинки, ТекущаяВерсияРазмер, ТекущаяВерсияРасширение, ПодписанЭП");
				...
				ПриложитьПодписи = Истина;
				Если ДанныеФайла.ПодписанЭП И ПриложитьПодписи Тогда // добавим подписи
				
					ЭПФайлаИнфо = ПолучитьИнформациюОбЭПФайла(Файл);
...
&НаКлиенте
Процедура ОтправитьНаКлиенте()           
	
	РазмерВложенийКБайт = ПолучитьСуммарныйРазмерВложений();
	
	КлючеваяОперация = "ПочтаВыполнениеКомандыОтправитьВложенияДо100Кб";
	
	БайтВКилобайте = 1024;         
	Размер100Кб = 100;
	
	Если РазмерВложенийКБайт >= Размер100Кб И РазмерВложенийКБайт < БайтВКилобайте Тогда  
		КлючеваяОперация = "ПочтаВыполнениеКомандыОтправитьВложенияДо1Мб";
	ИначеЕсли РазмерВложенийКБайт >= БайтВКилобайте Тогда
		КлючеваяОперация = "ПочтаВыполнениеКомандыОтправитьВложенияБолее1Мб";
	Иначе	
		КлючеваяОперация = "ПочтаВыполнениеКомандыОтправитьВложенияДо100Кб";
	КонецЕсли;	
...
Процедура ПродолжениеОтправки4()
	...
	Если Не ЗаписатьКлиент(ПараметрыЗаписи) Тогда
		...
		Если ЕстьДействительныеАдресаты И ПредставленияНедействительныхАдресатов.Количество() <> 0 Тогда
			
			ПараметрыФормы = Новый Структура;
			ПараметрыФормы.Вставить("ПредставленияНедействительныхАдресатов", ПредставленияНедействительныхАдресатов);
			
			ОписаниеОповещения = Новый ОписаниеОповещения("ЗавершениеОтправкиНедействительные", ЭтотОбъект);
			
			ОткрытьФорму("Документ.ИсходящееПисьмо.Форма.ВопросПриОтправкеНедействительнымПользователям", 
				ПараметрыФормы,
				ЭтаФорма,,,,
				ОписаниеОповещения,
				РежимОткрытияОкнаФормы.БлокироватьВесьИнтерфейс);
...
&НаКлиенте
Процедура ЗавершениеОтправкиНедействительные(Результат, Параметры) Экспорт
	
	Если Результат <> "УдалитьОтправить" Тогда
		Возврат;
	КонецЕсли;
	
	УдалятьНедействительныеАдресатыПриОтправке = Истина;
	ОтправитьКлиент();
...
&НаСервере
Функция ПроверитьПисьмоПередОтправкой(ТекущийОбъект, МассивСтрокОшибокПолучателей)
	...
	// Адресаты в списке есть, но нет ни одного действительного адресата
	Если Не ЕстьДействительныеАдресаты И СписокУникальныхАдресов.Количество() <> 0 Тогда
		
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ru = 'Нельзя отправить письмо. Необходимо указать хотя бы одного действительного получателя.'"),,
				"Получатели");
	...
	// Адресаты в списке есть (т.к. МассивСтрокОшибокПолучателей непуст), но нет ни одного действительного адресата
	Если Не ЕстьДействительныеАдресаты И МассивСтрокОшибокПолучателей.Количество() <> 0 Тогда
		
		Для Каждого СтрокаОшибки Из МассивСтрокОшибокПолучателей Цикл
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				СтрокаОшибки,,
				"Получатели");
		КонецЦикла;	
	...
	// В списке есть как действительные, так и недействительные адресаты
	// - надо прервать отправку и показать диалог со списком недействительных адресатов
	Если Не УдалятьНедействительныеАдресатыПриОтправке И ЕстьДействительныеАдресаты
		И ПредставленияНедействительныхАдресатов.Количество() <> 0 Тогда
...
Процедура СвязиОбъекта(Команда)
	...
	ОткрытьФорму(
		"РегистрСведений.СвязиОбъектов.Форма.ФормаСвязейОбъекта",
		ПараметрыФормы);
...
Процедура ДобавитьАдресатов(Команда)
	...
	Иначе
		...
		Если ПолучателиТекущиеДанные <> Неопределено 
			...
			Если ПолучателиТекущиеДанные.ТипАдреса = ПредопределенноеЗначение("Перечисление.ТипыАдресатов.Кому") Тогда
				НоваяСтрока.ТипАдреса = ПредопределенноеЗначение("Перечисление.ТипыАдресатов.Копия");
			Иначе
...
Процедура ВыбратьИзАдреснойКниги(ПараметрМножественныйВыбор = Неопределено)
	...
	Если ТипАдреса = ПредопределенноеЗначение("Перечисление.ТипыАдресатов.Кому") Тогда
		ГруппаПоУмолчанию = ПредопределенноеЗначение("Перечисление.ТипыАдресатов.Кому");
	ИначеЕсли ТипАдреса = ПредопределенноеЗначение("Перечисление.ТипыАдресатов.Копия") Тогда
		ГруппаПоУмолчанию = ПредопределенноеЗначение("Перечисление.ТипыАдресатов.Копия");
	ИначеЕсли ТипАдреса = ПредопределенноеЗначение("Перечисление.ТипыАдресатов.СкрытаяКопия") Тогда
	...
	Если ЗначениеЗаполнено(Контакт) Тогда            
		...
		ВыбранныеАдресаты.Добавить(Новый Структура("Контакт", Контакт));
		ПараметрыОткрытия.Вставить("ВыбранныеАдресаты", ВыбранныеАдресаты);
	КонецЕсли;
...
Процедура ЗаполнитьПоШаблонуНаСервере(Результат)
	...
	КопироватьТолькоФайлыСИдентификаторомВложений = Ложь;
	КопироватьТолькоБезИдентификаторовВложений = Истина;
	
	ВставитьФайлыИсходногоПисьмаСервер(Результат, КопироватьТолькоФайлыСИдентификаторомВложений,
		КопироватьТолькоБезИдентификаторовВложений);
	...
	Для каждого Строка Из ТаблицаПолучателиПисьма Цикл
		ДобавитьУникальногоПолучателя(Перечисления.ТипыАдресатов.Кому, Строка.Адресат);
	КонецЦикла;
	Для каждого Строка Из ТаблицаПолучателиКопий Цикл
		ДобавитьУникальногоПолучателя(Перечисления.ТипыАдресатов.Копия, Строка.Адресат);
	КонецЦикла;
	Для каждого Строка Из ТаблицаПолучателиСкрытыхКопий Цикл
		ДобавитьУникальногоПолучателя(Перечисления.ТипыАдресатов.СкрытаяКопия, Строка.Адресат);
	КонецЦикла;
	Для каждого Строка Из ТаблицаПолучателиОтвета Цикл
		ДобавитьУникальногоПолучателя(Перечисления.ТипыАдресатов.ОбратныйАдрес, Строка.Адресат);
	КонецЦикла;
...
Процедура ВложенияВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	ПредставлениеВыделенногоФайла = "";
	Если ТекущиеДанные <> Неопределено Тогда
		ПредставлениеВыделенногоФайла = ТекущиеДанные.Представление;
	КонецЕсли;	
	...
	Если ТекущиеДанные <> Неопределено Тогда 
		...
		Если БылоСохранение И ЗначениеЗаполнено(ПредставлениеВыделенногоФайла) И 
			ПредставлениеВыделенногоФайла <> ТекущиеДанные.Представление Тогда
				
			ТекПозиция = Новый Структура("Представление", ПредставлениеВыделенногоФайла);
			Строки = Вложения.НайтиСтроки(ТекПозиция);
			Если Строки.Количество() > 0 Тогда
				ТекущиеДанные = Строки[0]; // это не то же самое, что на клиенте ТекущиеДанные, но там есть все поля что нужны
			КонецЕсли;
			
		КонецЕсли;	
		
		Если ТекущиеДанные.ЭтоВложенноеПисьмо Тогда
		...
		ИначеЕсли ЗначениеЗаполнено(ТекущиеДанные.Письмо) Тогда
			
			ВстроеннаяПочтаКлиент.ОткрытьПисьмо(ТекущиеДанные.Письмо);
			
		ИначеЕсли ВстроеннаяПочтаКлиентСервер.ЭтоФайлПриглашение(ТекущиеДанные.Представление, РасширенияICS) Тогда	
				
			ВстроеннаяПочтаКлиент.ОткрытьПриглашение(Объект.Ссылка, ТекущиеДанные.Ссылка, УникальныйИдентификатор);
			
		Иначе
			ОткрытьВложение(ТекущиеДанные.Ссылка, ОткрыватьФайлыИсходящегоПисьмаНаРедактирование);
		КонецЕсли;		
...
Процедура ВложенияНачалоПеретаскивания(Элемент, ПараметрыПеретаскивания, Выполнение)
	...
	#Если Не ВебКлиент Тогда
		...
		КоличествоВыделенныхВложений = Элементы.Вложения.ВыделенныеСтроки.Количество();
...
&НаСервере
Процедура ДобавитьФайлСтрока(ФайлИнфо, ИгнорироватьЗанятость = Ложь,
	ТребуетсяЗаполнитьВизуализацию = Ложь,
	ПриложитьПодписи = Ложь)
	
	ВложенияСтрока = Вложения.Добавить();
	ВложенияСтрока.ПорядковыйНомерВложения = Вложения.Количество() - 1;
	ЗаполнитьЗначенияСвойств(ВложенияСтрока, ФайлИнфо);
	
	Если ЗначениеЗаполнено(ВложенияСтрока.Ссылка) Тогда
		ВложенияСтрока.Расположение = "СсылкаНаФайл";
	Иначе
		ВложенияСтрока.Расположение = "ВременноеХранилище";
	КонецЕсли;	
	...
	Если ФайлИнфо.Свойство("Расширение") И СтрНайти(ВложенияСтрока.Представление, "." + ФайлИнфо.Расширение) = 0 Тогда
		ВложенияСтрока.Представление = ВложенияСтрока.Представление + "." + ФайлИнфо.Расширение; 
	КонецЕсли;	
		
	ВложенияСтрока.ТребуетсяЗаполнитьВизуализацию = ТребуетсяЗаполнитьВизуализацию;
	
	Если ФайлИнфо.ПодписанЭП И ПриложитьПодписи Тогда // добавим подписи
	
		ЭПФайлаИнфо = ПолучитьИнформациюОбЭПФайла(ФайлИнфо.Ссылка);
		Для каждого ЭПИнфо Из ЭПФайлаИнфо Цикл
			Наименование = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'ЭП: %1 - %2'"),
				ФайлИнфо.Представление,
				ЭПИнфо.Наименование);
...
&НаСервере
Процедура ДобавитьПодпись(Наименование, Подпись, Размер)
	...
	ВложенияСтрока = Вложения.Добавить();
	ВложенияСтрока.ПорядковыйНомерВложения = Вложения.Количество() - 1;
	ВложенияСтрока.ИмяФайла = ИмяФайла;
	ВложенияСтрока.Представление = ИмяФайла;
	ВложенияСтрока.Размер = Размер;
	ВложенияСтрока.РазмерПредставление = РаботаСоСтроками.ПолучитьРазмерСтрокой(Размер);
	ВложенияСтрока.ИндексКартинки = ФайловыеФункцииКлиентСервер.ПолучитьИндексПиктограммыФайла(
		РасширениеДляФайловПодписи); 
	 
	ВложенияСтрока.Расположение = "ВременноеХранилище";
	ВложенияСтрока.Адрес = ПоместитьВоВременноеХранилище(Подпись, УникальныйИдентификатор);
...
&НаСервере
Процедура ДобавитьВложениеФайлы(МассивФайлов, ИгнорироватьЗанятость = Ложь, ДокументПодписан = Ложь,
	ПрежнийВладелецФайлов = Неопределено)  
	
	ИспользованныеМЧД = Новый Соответствие;
	
	НастройкиШтампаЭП = ФайловыеФункцииПовтИсп.ПолучитьОбщиеНастройкиШтампаЭП();
	
	ПриложитьПодписи = ОтправлятьПодписиЭППоПочте = Перечисления.ДействияПриОтправкеПоПочтеЭП.Спрашивать
	...
	Для каждого ФайлИнфо Из ФайлыИнфо Цикл
		...
		ЕстьРегШтамп = Ложь;
		Если ЗначениеЗаполнено(ПрежнийВладелецФайлов) И ДелопроизводствоКлиентСервер.ЭтоДокумент(ПрежнийВладелецФайлов) Тогда
		...
		Если (ФайлИнфо.ПодписанЭП Или ДокументПодписан Или ЕстьРегШтамп) И ПриложитьПодписи Тогда
			...
			Если ЗначениеЗаполнено(СсылкаВизуализации) Тогда
				...
				ДобавитьФайлСтрока(ФайлИнфо, ИгнорироватьЗанятость, , ПриложитьПодписи);	
				
				РеквВизуал = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(СсылкаВизуализации, "ТекущаяВерсияРасширение, ИндексКартинки");
				ФайлИнфо.Ссылка = СсылкаВизуализации;
				ФайлИнфо.Представление = ФайлИнфо.Представление + НСтр("ru = ' (со штампом ЭП)'");
				ФайлИнфо.Расширение = РеквВизуал.ТекущаяВерсияРасширение;
				ФайлИнфо.ИндексКартинки = РеквВизуал.ИндексКартинки;
				Если НастройкиШтампаЭП.ИтоговыйФорматФайлаСоШтампомЭП = Перечисления.ИтоговыйФорматФайлаСоШтампомЭП.PDFA Тогда
					ФайлИнфо.ИндексКартинки = ФайловыеФункцииСлужебныйКлиентСервер.ПолучитьИндексПиктограммыФайла("pdf");
				КонецЕсли;
				ДобавитьФайлСтрока(ФайлИнфо, ИгнорироватьЗанятость);
			...
			ИначеЕсли ТребуетсяЗаполнитьВизуализацию Тогда
				...
				Иначе
				КонецЕсли;
				ДобавитьФайлСтрока(ФайлИнфо, ИгнорироватьЗанятость, ТребуетсяЗаполнитьВизуализацию);
			...
			Если ФайлИнфо.ПодписанЭП Тогда
				...
				Для Каждого ФайлДоверенности Из ФайлыДоверенностей Цикл
					ФайлИнфо.Вставить("Адрес", "");
					ФайлИнфо.Адрес = ФайлДоверенности.Адрес;  
					ФайлИнфо.Ссылка = Неопределено;
					ФайлИнфо.Размер = ФайлДоверенности.Размер;  
					ФайлИнфо.РазмерПредставление = РаботаСоСтроками.ПолучитьРазмерСтрокой(ФайлИнфо.Размер);
					
					ФайлИнфо.Представление = ФайлДоверенности.Представление;
					ФайлИнфо.ИмяФайла = ФайлДоверенности.Представление;
					ФайлИнфо.Расширение = "zip";
					ФайлИнфо.ИндексКартинки = ФайловыеФункцииСлужебныйКлиентСервер.ПолучитьИндексПиктограммыФайла(ФайлИнфо.Расширение);
					ДобавитьФайлСтрока(ФайлИнфо, ИгнорироватьЗанятость);   
				КонецЦикла;
...
Процедура ПолучателиПредставлениеОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	...
	Если ТипЗнч(ВыбранноеЗначение) = Тип("Структура") Тогда
		...
		Если ЗначениеЗаполнено(ВыбранноеЗначение.Адресат) 
			...
			Если АдресатыГруппы.Количество() <> 0 Тогда
			...
			Иначе // очистим строку	   
				
				Сообщить(НСтр("ru = 'В выбранной группе нет сотрудников с заполненным email.'"));
	...
	Если СтрокаДанных <> Неопределено Тогда   
		...
		ВыбранноеЗначение = СтрокаДанных.Адресат;
		
	ИначеЕсли ТипЗнч(ВыбранноеЗначение) = Тип("СправочникСсылка.АдресатыПочтовыхСообщений") Тогда
		...
		ПараметрыОтбора.Вставить("Адресат", Адресат);
		...
		Иначе	
			НоваяСтрока = Получатели.Добавить();       
			НоваяСтрока.ТипАдреса = ПредопределенноеЗначение("Перечисление.ТипыАдресатов.Кому");
		КонецЕсли;

		НоваяСтрока.Адресат = Адресат;
		НоваяСтрока.Контакт = Неопределено;
		НоваяСтрока.Представление = Строка(Адресат);
		
		НоваяСтрока.Адрес = ОбщегоНазначенияДокументооборотВызовСервера.ЗначениеРеквизитаОбъекта(Адресат, "Адрес");
		НоваяСтрока.Внешний = Истина;
		НоваяСтрока.ВидМаршрутизации = ПредопределенноеЗначение("Перечисление.ВидыМаршрутизацииПисем.Внешняя");
...
Процедура ПолучателиПредставлениеАвтоПодбор(Элемент, Текст, ДанныеВыбора, Параметры, 
	...
	Параметры.Отбор.Вставить("УчетнаяЗапись", Объект.УчетнаяЗапись); 
	...
	Если ПустаяСтрока(Текст) Тогда
		...
		Возврат; // тут не нужен фоновый подбор
	...
	СтрокаДанныхПредставление = СтрокаДанных.Представление;   
	Параметры.Отбор.Вставить("СтрокаДанныхПредставление", СтрокаДанныхПредставление); 
	
	ЭтоВебКлиент = Ложь;
	#Если ВебКлиент Тогда
		ЭтоВебКлиент = Истина;
	#КонецЕсли
	Параметры.Отбор.Вставить("ЭтоВебКлиент", ЭтоВебКлиент); 

	Параметры.Отбор.Вставить("ТекущийПользователь", ТекущийПользователь); 
...
Процедура ПолучателиПредставлениеПриИзменении(Элемент)
	...
	Если Элементы.Получатели.ТекущиеДанные <> Неопределено 
		И (Элементы.Получатели.ТекущиеДанные.Представление = ""
		Или (Не ЗначениеЗаполнено(Элементы.Получатели.ТекущиеДанные.Адресат) И Элемент.ТекстРедактирования = "") ) Тогда
...
Процедура ПолучателиПриНачалеРедактирования(Элемент, НоваяСтрока, Копирование)
	...
	ПредыдущаяСтрокаТипАдреса = ПредопределенноеЗначение("Перечисление.ТипыАдресатов.Кому");
	...
	Если Не ЗначениеЗаполнено(СтрокаДанных.ТипАдреса) Тогда
		Если КоличествоАдресатовДоДобавления = 1 
			...
			СтрокаДанных.ТипАдреса = ПредопределенноеЗначение("Перечисление.ТипыАдресатов.Кому");
		Иначе
...
Процедура ОтобразитьСвязиПисьма()
	...
	Если ЗначениеЗаполнено(Объект.Ссылка) Тогда
		...
		ПересылкаПисьма = СвязиОбъектов.ПолучитьСвязанныйОбъект(
			Объект.Ссылка,
			Справочники.ТипыСвязей.ПересылкаПисьма);
			
		ПеренаправлениеПисьма = СвязиОбъектов.ПолучитьСвязанныйОбъект(
			Объект.Ссылка,
			Справочники.ТипыСвязей.ПеренаправлениеПисьма);
			
		ПолученОтвет.ЗагрузитьЗначения(
			СвязиОбъектов.ПолучитьСвязанныеОбъекты(
...
&НаКлиенте
Процедура ТекстЦитирование(Команда)
	
	Если Элементы.ТекстПисьмаHTML.Документ.queryCommandValue("FormatBlock") = "blockquote" Тогда
		ВыполнитьHTMLКоманду("outdent");
	Иначе
		ВыполнитьHTMLКоманду("formatBlock", "<blockquote>");
	КонецЕсли;
...
#Если Не ВебКлиент Тогда 
	...
	ПутьФайла = ПолучитьИмяВременногоФайла("png");
	КартинкаИзБуфераОбмена.Записать(ПутьФайла);   
...
Функция НайтиУзелПоНомеру(ТекущийУзел, НомерУзла, Счетчик, НайденныйУзел)
	...
	Для пер = 0 По Всего - 1 Цикл
		
		Узел = ТекущийУзел.childNodes[пер];
...
Процедура ВосстановитьПоложениеHTML()
	...
	Если Объект.ТипТекста = ПредопределенноеЗначение("Перечисление.ТипыТекстовПочтовыхСообщений.HTML")
		...
		Если ТекущийНомерУзлаНачалаВыделения = -1
			И ТекущийНомерУзлаКонцаВыделения = -1 Тогда
			Возврат;
		КонецЕсли;	

		Если ТекущийНомерУзлаНачалаВыделения = Неопределено Или
			ТекущийНомерУзлаКонцаВыделения = Неопределено Или
			ТекущееСмещениеНачалаВыделения = Неопределено Или
			ТекущееСмещениеКонцаВыделения = Неопределено Тогда
			Возврат;
		КонецЕсли;	
		
		НайденныйУзелНачалаВыделения = Неопределено;
		НайденныйУзелКонцаВыделения = Неопределено;
		
		Попытка
			
			Счетчик = 0; 
			Результат1 = НайтиУзелПоНомеру(HTMLДокумент, ТекущийНомерУзлаНачалаВыделения, Счетчик, НайденныйУзелНачалаВыделения);
			Счетчик = 0; 
			Результат2 = НайтиУзелПоНомеру(HTMLДокумент, ТекущийНомерУзлаКонцаВыделения, Счетчик, НайденныйУзелКонцаВыделения);
			Если Результат1 И Результат2 Тогда
				
				Range = HTMLДокумент.createRange();
				Range.setStart(НайденныйУзелНачалаВыделения, ТекущееСмещениеНачалаВыделения);
				Range.setEnd(НайденныйУзелКонцаВыделения, ТекущееСмещениеКонцаВыделения);
				...
				НайденныйУзелНачалаВыделения.parentNode.scrollIntoView();
...
&НаКлиенте
Асинх Процедура ДобавитьКартинкуИзБуфераКакФайл(Команда)  
...
#Иначе  // веб клиент
	...
	ДобавитьВложениеКартинка(КартинкаИзБуфераОбмена);
...
&НаСервере
Процедура ДобавитьВложениеКартинка(КартинкаИзБуфераОбмена)
	
	ДвДанные = КартинкаИзБуфераОбмена.ПолучитьДвоичныеДанные();
	
	ВложенияСтрока = Вложения.Добавить();
	
	ВложенияСтрока.ПорядковыйНомерВложения = Вложения.Количество() - 1;
	ВложенияСтрока.Ссылка = Неопределено;
	ВложенияСтрока.Адрес = ПоместитьВоВременноеХранилище(ДвДанные, УникальныйИдентификатор);
	ВложенияСтрока.Представление = "Картинка.png";
	ВложенияСтрока.ИмяФайла = "Картинка.png";
	
	ИндексКартинки = ФайловыеФункцииКлиентСервер.ПолучитьИндексПиктограммыФайла("png");
	ВложенияСтрока.ИндексКартинки = ИндексКартинки;
	
	ВложенияСтрока.Размер = ДвДанные.Размер();
	ВложенияСтрока.РазмерПредставление = РаботаСоСтроками.ПолучитьРазмерСтрокой(ВложенияСтрока.Размер);
	ВложенияСтрока.ПометкаУдаления = Ложь;
	ВложенияСтрока.Редактирует = Неопределено;
	ВложенияСтрока.РедактируетТекущийПользователь = Ложь;
	...
	Если Объект.ТипТекста = ПредопределенноеЗначение("Перечисление.ТипыТекстовПочтовыхСообщений.HTML") Тогда
		...
		Если Не ВыполняетсяЗапись И Не ЗаписьПисьмаПриВыбореФайла Тогда
			...
			Если HTMLДокумент <> Неопределено Тогда
				...
				ТекстПисьма = ОбработкаСтрокиXML.УдалитьНедопустимыеСимволыXML(ТекстПисьма);
...
Процедура ЗапомнитьПозициюHTML()
	...
	Попытка
		...
		ОсновнаяОбластьВыделения = Выделение.getRangeAt(0);
		
		УзелНачалаВыделения = ОсновнаяОбластьВыделения.startContainer;
		Счетчик = 0;
		ТекущийНомерУзлаНачалаВыделения = -1;
		НайтиНомерУзла(HTMLДокумент, УзелНачалаВыделения, Счетчик, ТекущийНомерУзлаНачалаВыделения);
		ТекущееСмещениеНачалаВыделения = ОсновнаяОбластьВыделения.startOffset;
		
		УзелКонцаВыделения = ОсновнаяОбластьВыделения.endContainer;
		Счетчик = 0;
		ТекущийНомерУзлаКонцаВыделения = -1;
		НайтиНомерУзла(HTMLДокумент, УзелКонцаВыделения, Счетчик, ТекущийНомерУзлаКонцаВыделения);
		ТекущееСмещениеКонцаВыделения = ОсновнаяОбластьВыделения.endOffset;
...
Процедура ВыполнитьЗаписьHTML()
	...
	Если ФайловыеФункцииСлужебныйКлиент.Расширение1СПредприятияПодключено() Тогда
...
Процедура ТекстПисьмаHTMLПриНажатии(Элемент, ДанныеСобытия, СтандартнаяОбработка)
	...
	Если ПисьмоОтправлено Тогда
		СтандартнаяОбработка = Ложь;
		ВстроеннаяПочтаКлиент.ОткрытьСсылку(ДанныеСобытия.Href, ДанныеСобытия.Element, Объект.Ссылка, Элемент.Документ, ЭтаФорма);
...
&НаКлиенте
Асинх Процедура ВставитьТекстИзБуфера(Команда)
	...
	Если ОбщегоНазначенияКлиентСервер.ЭтоWindowsКлиент() Тогда
		...
		HTMLДокумент = Элементы.ТекстПисьмаHTML.Документ; 
		Если Не HTMLДокумент.queryCommandEnabled("paste") Тогда
			ВставитьКартинкуИзБуфера(Команда);
			Возврат;
		КонецЕсли;
	...
	Если Объект.ТипТекста = ПредопределенноеЗначение("Перечисление.ТипыТекстовПочтовыхСообщений.HTML")
		...
		ШаблонТекста = Ждать СредстваБуфераОбмена.ПолучитьДанныеАсинх(СтандартныйФорматДанныхБуфераОбмена.HTML);

		Если ШаблонТекста = Неопределено Тогда
			
			ШаблонТекста = Ждать СредстваБуфераОбмена.ПолучитьДанныеАсинх(СтандартныйФорматДанныхБуфераОбмена.Текст);
			
			Если ШаблонТекста = Неопределено Тогда    
				
				ПоказатьПредупреждение(
					, НСтр(
						"ru='Буфер обмена не содержит текста'"
						, "ru"
					)
				);
...
Процедура ПроверитьОрфографию(Команда)
	...
	#Если Не ВебКлиент И Не МобильныйКлиент Тогда
		...
		Иначе	  
			...
			ОрфографияКлиент.ПроверитьОрфографиюТекст(ОписаниеОповещения, ТекстПисьма);
		КонецЕсли;	
...
Процедура ЗавершениеПроверитьОрфографиюТекст(Результат, Параметры) Экспорт
	...
	Если Результат.ТекстИзменен Тогда
		ТекстПисьма = Результат.ПроверяемыйТекст;
		Модифицированность = Истина;
	КонецЕсли;	
...
#Если Не МобильныйКлиент И Не ВебКлиент Тогда
	...
	ПараметрыПроверкиОрфографии.Вставить("ОбъектServiceManager", ОбъектServiceManager);
	ПараметрыПроверкиОрфографии.Вставить("OOEmptyProps", OOEmptyProps);
	ПараметрыПроверкиОрфографии.Вставить("OOlocale", OOlocale);
	ПараметрыПроверкиОрфографии.Вставить("OOSpellChecker", OOSpellChecker);
	
	ПараметрыПроверкиОрфографии.Вставить("СпособПроверкиОрфографии", СпособПроверкиОрфографии);
	...
	OOСловаИсключение = ОрфографияКлиент.ЗагрузитьИсключенияOpenOffice(ПараметрыПроверкиОрфографии);
	ПараметрыПроверкиОрфографии.Вставить("OOСловаИсключение", OOСловаИсключение);
...
Функция ОрфографияОбработатьСлово(Слово, СтрокаИзТекста)
	...
	НаличиеСлова = ПараметрыПроверкиОрфографии.OOСловаИсключение.Получить(Слово);
	Если НаличиеСлова <> Неопределено Тогда
		СловоНужноПроверитьWord = Ложь;
		ПараметрыПроверкиОрфографии.СловПропущено = ПараметрыПроверкиОрфографии.СловПропущено + 1;
	КонецЕсли;	
	...
	Если СловоНужноПроверитьWord Тогда
		...
		ИначеЕсли ПараметрыПроверкиОрфографии.СпособПроверкиОрфографии 
			...
			Альтернативы = ПараметрыПроверкиОрфографии.OOSpellChecker.spell(
				Слово, ПараметрыПроверкиОрфографии.OOlocale, ПараметрыПроверкиОрфографии.OOEmptyProps );
			
			Если Альтернативы <> Неопределено Тогда // т.е. есть ошибки в слове
...
Процедура ЗавершитьПроверкуОрфографии()
	...
	Если ПараметрыПроверкиОрфографии.СпособПроверкиОрфографии = ПредопределенноеЗначение("Перечисление.СпособыПроверкиОрфографии.MicrosoftOfficeWord") Тогда
	
		ПараметрыПроверкиОрфографии.Word.Quit();
		ПараметрыПроверкиОрфографии.Word = Неопределено;

	ИначеЕсли ПараметрыПроверкиОрфографии.СпособПроверкиОрфографии = ПредопределенноеЗначение("Перечисление.СпособыПроверкиОрфографии.OpenOfficeOrgWriter") Тогда
		
		ПараметрыПроверкиОрфографии.ОбъектServiceManager = Неопределено;
		ПараметрыПроверкиОрфографии.OOSpellChecker = Неопределено;
			
		ПараметрыПроверкиОрфографии.OOEmptyProps = Неопределено;
		ПараметрыПроверкиОрфографии.OOlocale = Неопределено;
...
Процедура ЗавершениеПроверкиОрфографииВБлокеТекста(КодВозврата, Параметр) Экспорт
	...
	Если Кнопка = "ДобавитьИсключение" Тогда
		...
		ПараметрыПроверкиОрфографии.Исключения.Вставить(ПараметрыПроверкиОрфографии.Слово, 1);
		ОрфографияКлиент.ДобавитьСловоВИсключенияWord(
			ПараметрыПроверкиОрфографии, 
...
&НаКлиенте
Процедура ЕстьДокументыСтрокаНажатие(Элемент, СтандартнаяОбработка)
	
	СтандартнаяОбработка = Ложь;
	
	Если ЗначениеЗаполнено(ЕстьДокументыСсылка) Тогда
		ПоказатьЗначение(, ЕстьДокументыСсылка);
	Иначе
		...
		ОткрытьФорму("ОбщаяФорма.СвязиОбъекта",
			Новый Структура("ОсновнойОбъект, ТолькоВажные, ТолькоСвязиВОтветНа, ТолькоПисьмоОснование", 
				Объект.Ссылка, Ложь, Ложь, Истина),,
			Новый УникальныйИдентификатор);
...
&НаКлиенте
Процедура ДекорацияКартинкаПоУмолчаниюНажатие(Элемент)
	...
	Если ТипТекста = ПредопределенноеЗначение("Перечисление.ТипыТекстовПочтовыхСообщений.HTML") Тогда
		...
		Если ШрифтОтображенияУстановлен Тогда
			РаботаС_HTML.ПрименитьНастройкиОтображениеПисьма(
				ТекстПисьма, ШрифтОтображения, Ложь, Истина,,"Исходящее");
		Иначе
			РаботаС_HTML.ПрименитьНастройкиОтображениеПисьма(
				ТекстПисьма, ШрифтОтображения, ОтображатьДругиеШрифтыВПисьмах, Истина,,"Исходящее");
		КонецЕсли;
...
#Если Не ВебКлиент Тогда
	...
	Если ТипЗнч(Значение) = Тип("Массив") Тогда
		...
		Для каждого ПеретаскиваемыйЭлемент Из Значение Цикл
			
			Если ТипЗнч(ПеретаскиваемыйЭлемент) = Тип("СсылкаНаФайл") Тогда
				Файл = ПеретаскиваемыйЭлемент.Файл;
				МассивФайловНаДиске.Добавить(ПолучитьИнформациюОФайлеНаДиске(Файл));
			КонецЕсли; 
...
&НаСервере
Процедура ЗаполнитьЕстьДокументыСтрока(Письмо)  
	
	ДокументыПоПисьму = ВстроеннаяПочтаСервер.ДокументыНаОсновании(Письмо);
	
	Если ЗначениеЗаполнено(ДокументыПоПисьму) Тогда            
		
		Элементы.ЕстьДокументыСтрока.Видимость = Истина;
		
		Если ТипЗнч(ДокументыПоПисьму) <> Тип("Строка") Тогда  
			
			ЕстьДокументыСсылка = ДокументыПоПисьму;
			ЕстьДокументыСтрока = Строка(ЕстьДокументыСсылка);
			
		Иначе		           
			
			ЕстьДокументыСтрока = Неопределено;		
			ЕстьДокументыСтрока = СтроковыеФункцииКлиентСервер.СтрокаСЧисломДляЛюбогоЯзыка(
...
Функция ПисьмоСодержитЗанятыеФайлы(Письмо, Пользователь = Неопределено)
	...
	Запрос = Новый Запрос(
		...
		|	И (НЕ (Файлы.Редактирует В (&МассивРедактирует)))");
	
	СотрудникиТекущегоПользователя = СотрудникиВызовСервера.ПользовательИЕгоСотрудники();
	МассивРедактирует = ОбщегоНазначения.СкопироватьРекурсивно(СотрудникиТекущегоПользователя);
		
	Запрос.УстановитьПараметр("Письмо", Письмо);
	Запрос.УстановитьПараметр("МассивРедактирует", МассивРедактирует);
...
&НаКлиенте
Процедура ДобавитьФайлИзБазы(Команда)
	
	ИмяФормыВыбора = "Справочник.Файлы.Форма.ФормаВыбораФайлаВПапках";

	ОписаниеОповещенияОЗакрытии = Новый ОписаниеОповещения(
		"ДобавитьФайлИзБазыПродолжение",
		ЭтотОбъект);
	
	РезультатОткрытия = ОткрытьФорму(
		ИмяФормыВыбора, ,,,,,ОписаниеОповещенияОЗакрытии, РежимОткрытияОкнаФормы.БлокироватьВесьИнтерфейс);
	
КонецПроцедуры

&НаКлиенте
Процедура ДобавитьФайлИзБазыПродолжение(РезультатОткрытия, Параметры) Экспорт
	...
	ФайлИнфо = ОбщегоНазначенияДокументооборотВызовСервера.ЗначенияРеквизитовОбъекта(РезультатОткрытия, 
		"Наименование, ИндексКартинки, ТекущаяВерсияРазмер, ТекущаяВерсияРасширение");

	НовСтр = Вложения.Добавить();
	НовСтр.Ссылка = РезультатОткрытия;
	НовСтр.Расположение = "СсылкаНаФайл";
	
	НовСтр.Представление = 
		ФайловыеФункцииКлиентСервер.ПолучитьИмяСРасширением(ФайлИнфо.Наименование, 
			ФайлИнфо.ТекущаяВерсияРасширение);
	
	НовСтр.ИндексКартинки = ФайлИнфо.ИндексКартинки;
	НовСтр.Размер = ФайлИнфо.ТекущаяВерсияРазмер;
	НовСтр.РазмерПредставление = РаботаСоСтроками.ПолучитьРазмерСтрокой(НовСтр.Размер); 
...
Функция ОпределитьНомерСпособаАдресации(ТипАдреса)
	...
	Если ТипАдреса = ПредопределенноеЗначение("Перечисление.ТипыАдресатов.Копия") Тогда
		Возврат 2;
	КонецЕсли;
	
	Если ТипАдреса = ПредопределенноеЗначение("Перечисление.ТипыАдресатов.СкрытаяКопия") Тогда
		Возврат 3;
	КонецЕсли;

	Если ТипАдреса = ПредопределенноеЗначение("Перечисление.ТипыАдресатов.ОбратныйАдрес") Тогда
		Возврат 4;
	КонецЕсли;
...
Процедура ДобавитьПользователейВПолучатели(ПользователиДляДобавления)
	...
	Для Каждого ПользовательДляДобавленияКлючИЗначение Из ПользователиДляДобавления Цикл
		...
		Для Каждого НайденнаяСтрока Из НайденныеСтроки Цикл
			Если НайденнаяСтрока.ТипАдреса = Перечисления.ТипыАдресатов.Копия
...
// Возвращает размер в Кбайтах
// Возвращаемое значение:
//  Число - суммарный размер в Кбайтах всех вложений, включая картинки в теле HTML письма
&НаКлиенте
Функция ПолучитьСуммарныйРазмерВложений()
	...
	Для каждого ВложенияСтрока Из ВложенияКартинки Цикл
		Если Не ВложенияСтрока.ПометкаУдаления Тогда
			РазмерПисьмаБайт = РазмерПисьмаБайт + ВложенияСтрока.Размер;
		КонецЕсли;
	КонецЦикла;
	
	РазмерПисьмаКБайт = РазмерПисьмаБайт / 1024;
	
	Возврат РазмерПисьмаКБайт;
...
Процедура РаспознаваниеРечи_ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Элементы.ГруппаРаспознаваниеРечиТекст.Видимость = ЭтотОбъект["РаспознаваниеРечи_РаспознаваниеДоступно"];
	Элементы.ГруппаРаспознаваниеРечиHTML.Видимость = ЭтотОбъект["РаспознаваниеРечи_РаспознаваниеДоступно"];
	Элементы.НайтиАдресатовТекст.Видимость = ЭтотОбъект["РаспознаваниеРечи_РаспознаваниеДоступно"];
...
Процедура РаспознаваниеРечи_НачатьРаспознавание()
	...
	Контекст.Вставить("Элемент", ТекущийЭлемент);
	Контекст.Вставить("ИмяРеквизита", "ТекстПисьма");
...
Процедура РаспознаваниеРечи_ИзменитьСостояниеФормы()
	...
	Если ЭтотОбъект["РаспознаваниеРечи_ВыполняетсяРаспознавание"] Тогда
		...
		Элементы.РаспознаваниеРечиHTML.Картинка = БиблиотекаКартинок.КрасныйМикрофон;
		...
		Если ЭтотОбъект["РаспознаваниеРечи_РаспознаваниеФразыЗавершено"] Тогда
			...
			Элементы.РаспознаваниеРечиHTML.ЦветФона = Новый Цвет(255, 204, 0);
		Иначе
			Элементы.РаспознаваниеРечиТекст.ЦветФона = Новый Цвет(225, 154, 0);
			Элементы.РаспознаваниеРечиHTML.ЦветФона = Новый Цвет(225, 154, 0);
		КонецЕсли;
		
	Иначе
		
		Элементы.РаспознаваниеРечиТекст.Картинка = БиблиотекаКартинок.ЧерныйМикрофон;
		Элементы.РаспознаваниеРечиHTML.Картинка = БиблиотекаКартинок.ЧерныйМикрофон;
		
		Элементы.РаспознаваниеРечиТекст.ЦветФона = Новый Цвет;
		Элементы.РаспознаваниеРечиHTML.ЦветФона = Новый Цвет;
		
	КонецЕсли;
	
	ЭтоПростойТекст = (Объект.ТипТекста = ПредопределенноеЗначение("Перечисление.ТипыТекстовПочтовыхСообщений.ПростойТекст"));
	
	Элементы.ГруппаРаспознаваниеРечиТекст.Видимость = ЭтотОбъект["РаспознаваниеРечи_РаспознаваниеДоступно"] И РаботаСРечьюБМОКлиент.ПоддерживаетсяПотоковоеРаспознавание() И ЭтоПростойТекст;
	Элементы.ГруппаРаспознаваниеРечиHTML.Видимость  = ЭтотОбъект["РаспознаваниеРечи_РаспознаваниеДоступно"] И РаботаСРечьюБМОКлиент.ПоддерживаетсяПотоковоеРаспознавание() И Не ЭтоПростойТекст;
	
	Элементы.НайтиАдресатовТекст.Видимость = ЭтотОбъект["РаспознаваниеРечи_РаспознаваниеДоступно"] И РаботаСРечьюБМОКлиент.ПоддерживаетсяПотоковоеРаспознавание();
	
	Элементы.ТекстПисьма.Доступность     = ЭтотОбъект["РаспознаваниеРечи_РаспознаваниеФразыЗавершено"];
	Элементы.ТекстПисьмаHTML.Доступность = ЭтотОбъект["РаспознаваниеРечи_РаспознаваниеФразыЗавершено"];
	
	// Полноценная блокировка поля ввода html письма в момент распознавания
	ДокументHTML = Элементы.ТекстПисьмаHTML.Документ;
	Если ДокументHTML <> Неопределено Тогда
		ТелоHTML = Элементы.ТекстПисьмаHTML.Документ.defaultView;
		Если ЭтотОбъект["РаспознаваниеРечи_РаспознаваниеФразыЗавершено"] Тогда
#Если ВебКлиент Тогда
			ТелоHTML.document.body.setAttribute("contenteditable", "true");
			ТелоHTML.document.body.removeAttribute("style");
			ТелоHTML.document.body.removeAttribute("ondragstart");
#Иначе
			ТелоHTML.onselect = Неопределено;
#КонецЕсли
		Иначе
#Если ВебКлиент Тогда
			ТелоHTML.document.body.removeAttribute("contenteditable");
			ТелоHTML.document.body.setAttribute("style", "user-select: none");
			ТелоHTML.document.body.setAttribute("ondragstart", "return false");
#Иначе
			ТелоHTML.onselect = "return false";
#КонецЕсли
		КонецЕсли
	КонецЕсли;
...
Процедура НайтиАдресатов(Команда)
	...
	ОткрытьФорму("ОбщаяФорма.НайтиАдресатов", , ЭтотОбъект, , , ,
		Новый ОписаниеОповещения("ПослеПолученияРезультата", ЭтотОбъект)
	);
...
Процедура ПослеПолученияРезультата(Результат, Контекст) Экспорт
	...
	Если Результат.Свойство("Текст") Тогда
		...
		Если Объект.ТипТекста = ПредопределенноеЗначение("Перечисление.ТипыТекстовПочтовыхСообщений.ПростойТекст") Тогда
			ПоляВводаКлиент.ВставитьТекстПоКурсору(ЭтотОбъект, Элементы.ТекстПисьма, "ТекстПисьма", Результат.Текст);
		Иначе
			ПоляВводаКлиент.ВставитьТекстПоКурсору(ЭтотОбъект, Элементы.ТекстПисьмаHTML, "ТекстПисьма", Результат.Текст);
		КонецЕсли;
	
	КонецЕсли;
	
	Если Результат.Свойство("Адресаты") Тогда
		
		МассивРезультата = Новый Массив;
		Для Каждого Адресат Из Результат.Адресаты Цикл
			СтруктураАдресат = Новый Структура;
			СтруктураАдресат.Вставить("ФИО", Адресат.ИмяПолучателя);
			СтруктураАдресат.Вставить("Адрес", "");
			СтруктураАдресат.Вставить("Представление", "");
			СтруктураАдресат.Вставить("ТипПолучателя", Адресат.ТипАдреса);    
			СтруктураАдресат.Вставить("ВидМаршрутизации", Неопределено);
			СтруктураАдресат.Вставить("Внешний", Ложь);
			
			МассивРезультата.Добавить(СтруктураАдресат);
		КонецЦикла;
		...
		Если МассивРезультата.Количество() <> 0 Тогда
			...
			РаботаСРечьюДокументооборотВызовСервера.ЗаполнитьАдресатовПоФИО(
				МассивРезультата, ТекстОшибок); 
	...
	Если МК_Клиент.ДоступностьФормы(ЭтотОбъект)
		...
		Если ТипЗнч(ДанныеСканов) = Тип("Массив") Тогда
			Если ДанныеСканов.Количество() = 1 Тогда
				...
				ДобавитьСканСМобильногоВоВложения(ДанныеСкана);
			Иначе
				Порядок = 1;
				Для Каждого ДанныеСкана Из ДанныеСканов Цикл
					ДобавитьСканСМобильногоВоВложения(ДанныеСкана, Порядок);
					Порядок = Порядок + 1;
				КонецЦикла;
			КонецЕсли;
		Иначе
...
&НаКлиенте
Процедура ДобавитьСканСМобильногоВоВложения(ДанныеСкана, Порядок = 0)

	АдресСкана = ПоместитьВоВременноеХранилище(ДанныеСкана, УникальныйИдентификатор);
	ИмяСкана = МК_Клиент.СформироватьИмяФайла("jpg", Порядок);
	ЗавершениеДобавитьФайлыСДиска(Истина, АдресСкана, ИмяСкана, Неопределено);
