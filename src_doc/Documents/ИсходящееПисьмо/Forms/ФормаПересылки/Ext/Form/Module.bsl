...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Для Каждого СтрокаСписка Из ПересылаемыеПисьма Цикл
		...
		Если Не ЗначениеЗаполнено(ПересылаемоеПисьмо.УчетнаяЗапись) 
			Или ПересылаемоеПисьмо.УчетнаяЗапись.ОтветственныеЗаОбработкуПисем.Найти(ТекущийПользователь, "Сотрудник") = Неопределено
			Или ПересылаемоеПисьмо.УчетнаяЗапись.ОтветственныеЗаОбработкуПисем.Найти(ТекущийСотрудник, "Сотрудник") = Неопределено Тогда
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	Если ИмяСобытия = "ВыбраныПолучатели" Тогда
		...
		ДобавитьВыбранныхАдресатов(Параметр);
	КонецЕсли;
...
&НаКлиенте
Процедура МК_ДобавитьАдресатов(Команда)
...
Процедура ВыбратьИзАдреснойКниги()
	...
	Если СтрокаДанных.ТипАдреса = ПредопределенноеЗначение("Перечисление.ТипыАдресатов.Кому") Тогда
		ГруппаПоУмолчанию = ПредопределенноеЗначение("Перечисление.ТипыАдресатов.Кому");
	ИначеЕсли СтрокаДанных.ТипАдреса = ПредопределенноеЗначение("Перечисление.ТипыАдресатов.Копия") Тогда
...
Процедура ЗавершениеВыбратьИзАдреснойКниги(Результат, Параметры) Экспорт
	...
	Иначе
		...
		Для Индекс = 0 По Результат.Количество() - 1 Цикл
			...
			ИначеЕсли СтрокаДанных.ТипАдреса = ПредопределенноеЗначение("Перечисление.ТипыАдресатов.СкрытаяКопия") Тогда
				НоваяСтрока.ТипАдреса = ПредопределенноеЗначение("Перечисление.ТипыАдресатов.СкрытаяКопия");
			КонецЕсли;
...
Процедура ПолучателиПослеУдаления(Элемент)
	...
	Если ЭтоМобильныйКлиент Тогда
		МК_ОбновитьЗаголовокПолучателей();
	КонецЕсли;
...
Процедура ПолучателиПриНачалеРедактирования(Элемент, НоваяСтрока, Копирование)
	...
	Если Не ЗначениеЗаполнено(СтрокаДанных.ТипАдреса) Тогда
		...
		Иначе
			СтрокаДанных.ТипАдреса = ПредопределенноеЗначение("Перечисление.ТипыАдресатов.Копия");
		КонецЕсли;
	КонецЕсли;
...
Процедура ЗаполнитьСписокВыбора()
	...
	Запрос.Текст = 
		...
		|	ЛичныеАдресаты.Сотрудник КАК Сотрудник
		|ИЗ
...
Процедура ПереслатьНаСервере(ОтправленныеПисьма, СписокОшибок) 
	...
	Для Каждого СтрокаСписка Из ПересылаемыеПисьма Цикл
		...
		Попытка
			...
			ТекстХранилище = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ОтправляемоеПисьмо.Ссылка, "ТекстХранилище");
			РегистрыСведений.ТекстыПисем.ЗаписатьТекстПисьма(ОтправляемоеПисьмо.Ссылка, 
				ОтправляемоеПисьмо.УчетнаяЗапись, ТекстХранилище);
...
Процедура МК_НастроитьЭлементыФормы()
	...
	Элементы.ПолучателиТипАдреса.Ширина = 6;
	Элементы.МК_ГруппаПолучателиЗаголовок.Видимость = Истина;
	Элементы.ПолучателиКонтекстноеМенюДобавитьАдресатов.Видимость = Ложь;
	Элементы.ГруппаСтраницыФотографии.Видимость = Ложь;
	Элементы.ПолучателиКонтекстноеМенюИзменить.Видимость = Ложь;
	...
	МК_КлиентСервер.ОформитьАкцентнуюКнопку(Элементы.Переслать, МК_ЭлементыСтиля);
	
КонецПроцедуры

&НаКлиенте
Процедура ДобавитьВыбранныхАдресатов(Адресаты)
...
&НаКлиенте
Процедура МК_ОбновитьЗаголовокПолучателей()
	
	КоличествоПолучателей = Получатели.Количество();
	
	Элементы.ПолучателиНадпись.Заголовок = СтрШаблон(НСтр("ru = 'Получатели (%1)'"),
