...
Процедура ПриЗаписи(Отказ)
	...
	Если НеЗаписыватьОперациюСПисьмом = Ложь Тогда
		...
		Иначе
			...
			Если (Папка <> ПредыдущаяПапка И ЗначениеЗаполнено(Папка) И ЗначениеЗаполнено(ПредыдущаяПапка))
...
Функция СформироватьСтруктуруРеквизитовПисьма(Знач ДанныеЗаполнения) Экспорт
	...
	Результат.Вставить("Автор", Сотрудники.ОсновнойСотрудник());
	...
	Если ЗначениеЗаполнено(УчетнаяЗаписьДляОтправки) Тогда
		...
		Результат.Вставить("ОтправительКонтакт", 
			Сотрудники.ОсновнойСотрудникПользователя(
			ДанныеАдресата.Контакт));
	...
	Если ДелопроизводствоКлиентСервер.ЭтоДокументПредприятия(ДанныеЗаполнения) Тогда
	...
	Если ТипЗнч(ДанныеЗаполнения) = Тип("ДокументСсылка.Задача") Тогда
		ЗаполнитьСтруктуруРеквизитовПоЗадаче(Результат, ДанныеЗаполнения);
	КонецЕсли;
	
	Если ТипЗнч(ДанныеЗаполнения) = Тип("ДокументСсылка.ДействиеЗадачи") Тогда
		ЗаполнитьСтруктуруРеквизитовПоДействиюЗадачи(Результат, ДанныеЗаполнения);
	КонецЕсли;
...
Процедура ЗаполнитьСтруктуруРеквизитовПоПисьму(СтруктураРеквизитов, Знач Письмо, Знач Команда)
	...
	ПеренестиОбратныеАдресаВПолучатели = Ложь;
	...
	Попытка
		...
		Если ПисьмоИнфо.УчетнаяЗапись.ОтветственныеЗаОбработкуПисем.Найти(
			СтруктураРеквизитов.Автор, "Сотрудник") <> Неопределено
			
			Или ПисьмоИнфо.УчетнаяЗапись.ОтветственныеЗаОбработкуПисем.Найти(
			Сотрудники.ОсновнойСотрудникПользователя(СтруктураРеквизитов.Автор),
			"Сотрудник") <> Неопределено Тогда
	...
	Если ПеренестиОбратныеАдресаВПолучатели Тогда
...
Процедура ЗаполнитьСтруктуруРеквизитовПоВнутреннемуДокументу(СтруктураРеквизитов, Знач Документ)
	...
	Если РеквизитыДокумента.ВидДокументаВестиУчетСторон = Истина Тогда 
		...
		Для Каждого КонтрагентыСтрока Из Документ.Стороны Цикл 
			...
			Если ЗначениеЗаполнено(КонтрагентыСтрока.Сторона) Тогда
				
				МассивАдресов = ПолучитьАдресаЭлектроннойПочты(КонтрагентыСтрока.Сторона);
				
				Если МассивАдресов.Количество() <> 0 Тогда
					Для Каждого АдресатАдрес Из МассивАдресов Цикл
						АдресатПредставление = Строка(КонтрагентыСтрока.Сторона);
						Адресат = ВстроеннаяПочтаСерверПовтИсп.ПолучитьПочтовогоАдресата(
							АдресатАдрес, АдресатПредставление);
	...
	Если РеквизитыДокумента.ВидДокументаИспользоватьВажность = Истина Тогда 
		Если РеквизитыДокумента.Важность = Перечисления.ВариантыВажностиОбъектов.Высокая Тогда
			СтруктураРеквизитов.Вставить("Важность", Перечисления.ВажностьПисем.Высокая);
		ИначеЕсли РеквизитыДокумента.Важность = Перечисления.ВариантыВажностиОбъектов.Низкая Тогда
			СтруктураРеквизитов.Вставить("Важность", Перечисления.ВажностьПисем.Низкая);
		КонецЕсли; 
	КонецЕсли;
...
Процедура ЗаполнитьСтруктуруРеквизитовПоПроектнойЗадаче(СтруктураРеквизитов, Знач ПроектнаяЗадача)
	...
	Для каждого ИсполнительСтрока Из ПроектнаяЗадача.Исполнители Цикл
		...
		Или ТипЗнч(Исполнитель) = Тип("СправочникСсылка.Сотрудники")) Тогда
...
Процедура ЗаполнитьСтруктуруРеквизитовПоМероприятию(СтруктураРеквизитов, Знач Мероприятие)
	...
	РеквизитыМероприятия = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(
		Мероприятие, "Наименование, Проект");
	...
	Для каждого УчастникМероприятия Из УчастникиМероприятия Цикл
		...
		Если ЗначениеЗаполнено(УчастникМероприятия)
			И ТипЗнч(УчастникМероприятия) = Тип("СправочникСсылка.Пользователи") Тогда
			
			МассивАдресов = ПолучитьАдресаЭлектроннойПочты(УчастникМероприятия);
			...
			Для Каждого Адрес Из МассивАдресов Цикл
				...
				Представление = Строка(УчастникМероприятия);
...
Процедура ЗаполнитьСтруктуруРеквизитовПоЗадаче(СтруктураРеквизитов, Знач Задача)
	
	СтруктураРеквизитов.Вставить("Тема", Строка(Задача));
	СтруктураРеквизитов.Вставить("Проект", ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Задача, "Проект"));
	
КонецПроцедуры

Процедура ЗаполнитьСтруктуруРеквизитовПоДействиюЗадачи(СтруктураРеквизитов, Знач ДействиеЗадачи)
	
	Задача = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДействиеЗадачи, "Задача");
	
	СтруктураРеквизитов.Вставить("Тема", Строка(ДействиеЗадачи));
	СтруктураРеквизитов.Вставить("Проект", ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Задача, "Проект"));
...
Функция ПолучитьАдресаЭлектроннойПочты(Знач Контрагент)
	
	Если ТипЗнч(Контрагент) = Тип("СправочникСсылка.ПолныеРоли") Тогда
		Контрагент = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Контрагент, "Владелец");
	КонецЕсли;	
