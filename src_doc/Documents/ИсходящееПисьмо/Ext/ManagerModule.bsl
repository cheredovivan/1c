...
Функция ПолучитьСписокПочтовыхАдресовКонтрагента(Контрагент) Экспорт
	
	Результат = Новый СписокЗначений;
	
	ОбъектКИ = Контрагент;
	
	РеквКонтрагента = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Контрагент, "ФизЛицо, ЮрФизЛицо");
	Если РеквКонтрагента.ЮрФизЛицо = Перечисления.ЮрФизЛицо.ФизЛицо Тогда
		Если ЗначениеЗаполнено(РеквКонтрагента.ФизЛицо) Тогда 
			ОбъектКИ = РеквКонтрагента.ФизЛицо;
		КонецЕсли;
	КонецЕсли;	
	...
	Если ТипЗнч(ОбъектКИ) = Тип("СправочникСсылка.ФизическиеЛица") Тогда
		Запрос.Текст = СтрЗаменить(Запрос.Текст, 
			"Справочник.Контрагенты",
			"Справочник.ФизическиеЛица");
	КонецЕсли;	
	
	Запрос.УстановитьПараметр("Ссылка", ОбъектКИ);    
...
Функция ПолучитьHTMLПредставлениеСодержанияПисьма(РеквизитыПисьма, ДляСохраненияПисьма = Ложь) Экспорт
	...
	РаботаС_HTML.ПрименитьНастройкиОтображениеПисьма(ТекстHTML,,,,,"Исходящее");
...
Процедура ОбработкаПолученияФормы(ВидФормы, Параметры, ВыбраннаяФорма, ДополнительнаяИнформация, СтандартнаяОбработка)
	...
	Если ВидФормы = "ФормаОбъекта" Тогда
		...
		Если ОбщегоНазначенияКлиентСервер.ЭтоМобильныйКлиент() Тогда
			ВыбраннаяФорма = "МК_ФормаДокумента";
		Иначе
			ВыбраннаяФорма = "ФормаДокумента"
		КонецЕсли;
	КонецЕсли;
