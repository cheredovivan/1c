...
Процедура НастроитьВидимостьИНаименованиеДействийПоСостояниюЭДО()
	...
	ОбменСКонтрагентамиДОСобытия.ЭлектронныйДокументИсходящийЭДОПриНастройкеВидимостиДействий(ЭтотОбъект);
...
Процедура ВывестиПредставлениеДокументовУчета()
	...
	ИначеЕсли КоличествоДокументов = 1 Тогда
		...
		// Документооборот
		Если СтрНайти(ПредставлениеДокументов, "<Объект не найден>") <> 0 Тогда
			ПредставлениеДокументов = ОбменСКонтрагентамиДОСервер.ТекстНадписиНетДоступаКОбъектуУчета();
		КонецЕсли;
		ОтображатьПодбор = Ложь;
		// Документооборот
...
Функция ЗагрузкаИзФайлаНаСервереУспешна(ВыбранныйФайл, ТаблицаОшибок, ЗагрузитьПринудительно = Ложь)
	...
	ТаблицаОшибок = ФорматыЭДО.ЗагружаемыйФайлСоответствуетЗагруженномуФайлу(
