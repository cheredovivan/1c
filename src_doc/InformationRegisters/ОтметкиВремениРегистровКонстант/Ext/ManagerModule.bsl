...
//
Процедура ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию_3_0_19_16(Параметры) Экспорт
	
	МетаданныеОбъекта = Метаданные.РегистрыСведений.ОтметкиВремениРегистровКонстант;
	ПолноеИмяОбъекта = МетаданныеОбъекта.ПолноеИмя();
	
	ПараметрыВыборки = Параметры.ПараметрыВыборки;
	ПараметрыВыборки.СпособВыборки = ОбновлениеИнформационнойБазы.СпособВыборкиИзмеренияНезависимогоРегистраСведений();
	ПараметрыВыборки.ПолныеИменаРегистров = ПолноеИмяОбъекта;
	ПараметрыВыборки.ПоляУпорядочиванияПриРаботеПользователей.Добавить("Объект");
	ПараметрыВыборки.ПоляУпорядочиванияПриОбработкеДанных.Добавить("Объект");
	ПараметрыВыборки.ИмяИзмеренияДляОтбора = "Объект";
	...
	Запрос = Новый Запрос(
		...
		|	ДанныеТаблицы.Объект КАК Объект,
		...
		|			ПО Таблица.Объект = ТаблицаИзменений.Объект
		|	ГДЕ
		|		ТаблицаИзменений.Объект ЕСТЬ NULL
		|	
		|	ОБЪЕДИНИТЬ ВСЕ
		|	
		|	ВЫБРАТЬ
		...
		|			ПО Таблица.Объект = ТаблицаИзменений.Объект
		|	ГДЕ
		|		ТаблицаИзменений.Объект ЕСТЬ NULL) КАК Таблица
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
	...
	Пока Истина Цикл
		...
		ЗаписьЖурналаРегистрации(
			...
			СтрШаблон(Нстр("ru = 'Обработчик обновления: %1.
				...
				"ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию_3_0_19_16",
				Граница,
...
//	Параметры - см. ОбновлениеИнформационнойБазы.ОсновныеПараметрыОтметкиКОбработке.
//
Процедура ОбработатьДанныеДляПереходаНаНовуюВерсию_3_0_19_16(Параметры) Экспорт
	...
	Запрос.Текст =
		"ВЫБРАТЬ РАЗЛИЧНЫЕ
		|	ДанныеДляОбновления.Объект КАК Объект
		|ИЗ
	...
	Пока Выборка.Следующий() Цикл
		...
		Запрос.Текст =
			"ВЫБРАТЬ
			|	&ОбрабатываемыйТип КАК Объект
			|ПОМЕСТИТЬ ДанныеДляОбновленияОбрабатываемыйТип";
		Запрос.УстановитьПараметр("ОбрабатываемыйТип", Выборка.Объект);
		...
		Пока ЕстьДанныеДляОбработки Цикл
			...
			Запрос.Текст =
				...
				|		ПО ДанныеРегистра.Объект = ДанныеДляОбновленияОбрабатываемыйТип.Объект
				|
				|СГРУППИРОВАТЬ ПО
				...
				|	ДублирующиесяКлючи.Объект КАК Объект
				|ИЗ
			...
			Если ЕстьДанныеДляОбработки = Истина Тогда
				...
				Попытка
					...
					Запрос.Текст =
						...
						|			И ДанныеРегистра.Объект = ДублирующиесяКлючи.Объект
						|;
						|
						|////////////////////////////////////////////////////////////////////////////////
						|ВЫБРАТЬ
						...
						|			И ДанныеРегистра.Объект = АктуальныеОтметкиВремени.Объект
						...
						|			И ДанныеРегистра.Объект = АктуальныеЗаписи.Объект
			...
			Иначе
				...
				Запрос.Текст =
					"ВЫБРАТЬ
					|	ОбработанныеДанные.Объект КАК Объект
					|ИЗ
