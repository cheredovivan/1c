...
Процедура ВидыБизнесСобытийВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	Если Поле = Элементы.ВидыБизнесСобытийУсловие Тогда
		...
		ОписаниеОповещения = Новый ОписаниеОповещения("ВыборАлгоритмаПроверки", ЭтотОбъект);
			
		ОткрытьФорму("Справочник.АлгоритмыПроверки.ФормаВыбора", , Элементы.ВидыБизнесСобытийУсловие,,,,
...
&НаКлиенте
Процедура ВыборАлгоритмаПроверки(Результат, Параметры) Экспорт  
...
Процедура НастроитьВидимостьОрганизации()
	...
	Если Не ТипОбъекта = Тип("СправочникСсылка.ВидыДокументов") Тогда
...
Процедура НастроитьВидимостьУсловия()
	...
	Если ТипыОбъектовСУсловиями.НайтиПоЗначению(Запись.КомуНазначен) = Неопределено
		И Не ТипОбъекта = Тип("СправочникСсылка.ВидыДокументов") Тогда
		
		Запись.Условие = ПредопределенноеЗначение("Справочник.АлгоритмыПроверки.ПустаяСсылка");
...
Процедура ЗаполнитьСписокСобытий()
	...
	Если ТипЗнч(Запись.КомуНазначен) = Тип("СправочникСсылка.ВидыДокументов") Тогда
		Элементы.ВидИнтерактивногоСобытия.СписокВыбора.Добавить(Перечисления.ВидыИнтерактивныхДействий.ЗакрытиеКарточкиТолькоЧтоСозданногоДокумента);
		Элементы.ВидИнтерактивногоСобытия.СписокВыбора.Добавить(Перечисления.ВидыИнтерактивныхДействий.ИнтерактивнаяРегистрацияДокумента);
	...
	Пока Выборка.Следующий() Цикл
		...
		Если Не Выборка.ЭтоГруппа Тогда
			...
			Если ТипЗнч(Запись.КомуНазначен) = Тип("СправочникСсылка.ВидыДокументов") Тогда
				...
				Если Выборка.Ссылка = Справочники.ВидыБизнесСобытий.СозданиеДокумента Тогда
					НужноДобавить = Истина;
				ИначеЕсли Выборка.Ссылка = Справочники.ВидыБизнесСобытий.ИзменениеДокумента Тогда
					НужноДобавить = Истина;
				ИначеЕсли Выборка.Ссылка = Справочники.ВидыБизнесСобытий.РегистрацияДокумента Тогда
					НужноДобавить = Истина;
				ИначеЕсли Выборка.Ссылка = Справочники.ВидыБизнесСобытий.ПеререгистрацияДокумента Тогда
					НужноДобавить = Истина;
				КонецЕсли;	
...
Функция ПроверитьСовместимостьШаблонаИВидаДокумента(Шаблон, КомуНазначен)
	...
	Если ТипЗнч(КомуНазначен) = Тип("СправочникСсылка.ВидыДокументов") Тогда
		Возврат ПроцессМожетСоздаватьсяПоПредмету(МетаданныеПроцесса, Метаданные.Справочники.ДокументыПредприятия);
