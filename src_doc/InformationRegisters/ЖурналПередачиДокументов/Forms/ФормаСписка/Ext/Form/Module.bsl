...
Процедура ПоПользователюПриИзменении(Элемент)
	...
	Если ПоПользователю = Неопределено Тогда
		ПоПользователю = СотрудникиПустаяСсылка;	
	КонецЕсли;
...
Процедура ПоПользователюНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ОткрытьФорму("ОбщаяФорма.ВыборПользователяКонтактноеЛицо", 
		Новый Структура("ТекущаяСтрока", ПоПользователю), Элемент);
...
Процедура НастроитьФормуДляМобильногоПриНеобходимости()
	...
	Элементы.Показывать.РастягиватьПоГоризонтали = Истина;
	Элементы.ПоДокументу.РастягиватьПоГоризонтали = Истина;
	Элементы.ПоПользователю.РастягиватьПоГоризонтали = Истина;
	
	
	Элементы.ГруппаКоманднаяПанель.Видимость = Ложь;
	Элементы.ОтметитьВозвратМК.Видимость = Истина;
