...
Процедура ОбновитьДанныеЗаписиКалендаря()
	...
	Если ЗначениеЗаполнено(ЗаписьКалендаря.Предмет) Тогда
		...
		ИначеЕсли ТипЗнч(Предмет) = Тип("СправочникСсылка.ДокументыПредприятия") Тогда
			ПредставлениеПредмета = НСтр("ru = 'документу'");
...
Процедура МК_НастроитьЭлементыФормы()
	...
	ГоризонтальныйИнтервал = ИнтервалМеждуЭлементамиФормы.Нет;
	ВертикальныйИнтервал = ИнтервалМеждуЭлементамиФормы.Нет;
	
	Элементы.ЗаписьКалендаряОписание.Ширина = 0;
	Элементы.ЗаписьКалендаряОписание.Высота = 2;
	Элементы.ЗаписьКалендаряОписание.АвтоМаксимальнаяВысота = Ложь;
	Элементы.ЗаписьКалендаряОписание.РастягиватьПоВертикали = Истина;
	Элементы.ЗаписьКалендаряОписание.РастягиватьПоГоризонтали = Истина;
	
	Элементы.ГруппаКомандыИСрок.Видимость = Ложь;
	
	Элементы.МК_ГруппаКомандыИСрок.Видимость = Истина;
	
	ЭлементыСтиля = МК.ЭлементыСтиля();
	
	Элементы.КнопкаОтложить.ВертикальноеПоложениеВГруппе = ВертикальноеПоложениеЭлемента.Низ;

	ПараметрыДопКнопки = МК_КлиентСервер.НовыйПараметрыЭлемента();
	ПараметрыДопКнопки.ОтображениеКнопки = ОтображениеКнопки.Текст;

	Элементы.СрокПовторногоОповещения.РежимВыбораИзСписка = Истина;
	Элементы.СрокПовторногоОповещения.КнопкаВыпадающегоСписка = Ложь;
	
	МК_КлиентСервер.ОформитьДополнительнуюКнопку(ЭлементыСтиля,
		Элементы.КнопкаПрекратить, 
		ПараметрыДопКнопки);
	
	Элементы.Переместить(Элементы.КнопкаПрекратить,
		Элементы.МК_ГруппаКомандыИСрок,
		Элементы.МК_ГруппаОтложить);

	// Акцентная кнопка "Отложить"
	МК.ПреобразоватьКнопкуВАкцентную(ЭтотОбъект,
		ЭлементыСтиля,
		Элементы.КнопкаОтложить,
		Элементы.МК_ГруппаОтложить);

	Элементы.Переместить(Элементы.ГруппаСрокМК, Элементы.МК_ГруппаОтложить);
