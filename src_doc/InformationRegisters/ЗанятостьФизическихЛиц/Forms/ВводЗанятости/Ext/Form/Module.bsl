...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ТипЗнч(Параметры.Основание) = Тип("Структура") Тогда
		...
		Если Параметры.Основание.Свойство("ДатаНачала")
			И ЗначениеЗаполнено(Параметры.Основание.ДатаНачала) Тогда
			
			ДатаНачала = Параметры.Основание.ДатаНачала;
			
		КонецЕсли;
		
		Если Параметры.Основание.Свойство("ДатаОкончания")
			И ЗначениеЗаполнено(Параметры.Основание.ДатаОкончания) Тогда
...
Процедура Пометить(Команда)
	...
	Если Не Отказ Тогда
		ПометитьСервер();
		Оповестить("Запись_ЗанятостьФизЛица");
		Закрыть();
	КонецЕсли;
...
Процедура ПометитьСервер()
	...
	РаботаСРабочимКалендаремСервер.ИзменитьДоступностьВремени(
		ПараметрыСеанса.ФизическоеЛицоПользователя, ДатаНачала, ДатаОкончания, Занят);
