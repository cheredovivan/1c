...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ФизЛицо = ПараметрыСеанса.ФизическоеЛицоПользователя;
	СотрудникФизЛица = Сотрудники.ОсновнойСотрудникФизЛица(ФизЛицо);
	
	УстановитьПривилегированныйРежим(Истина);
	НастройкаДоступногоВремени = РегистрыСведений.ДоступноеВремяФизическихЛиц.СоздатьНаборЗаписей();
	НастройкаДоступногоВремени.Отбор.ФизическоеЛицо.Установить(ФизЛицо);
	НастройкаДоступногоВремени.Прочитать();
	НастройкаДоступногоВремени = НастройкаДоступногоВремени.Выгрузить();
	
	// Если настроек нет, то читаем сведения из устаревшего регистра.
	Если НастройкаДоступногоВремени.Количество() = 0 И ЗначениеЗаполнено(СотрудникФизЛица) Тогда
		Запрос = Новый Запрос;
		Запрос.Текст =
			"ВЫБРАТЬ
			...
			|ГДЕ
			|	ДоступноеВремяСотрудников.Сотрудник = &Сотрудник";
		Запрос.УстановитьПараметр("Сотрудник", СотрудникФизЛица);
		НастройкаДоступногоВремени = Запрос.Выполнить().Выгрузить();
	КонецЕсли;
	
	УстановитьПривилегированныйРежим(Ложь);
	
	ДоступностьФизЛица = РаботаСРабочимКалендаремСервер.ПолучитьНастройкуДоступностиФизЛица(ФизЛицо);
	
	Если ДоступностьФизЛица <> Перечисления.ДоступностьФизическихЛиц.ДоступенПоРасписанию 
		И НастройкаДоступногоВремени.Количество() = 0 Тогда
		
		Если ПолучитьФункциональнуюОпцию("ИспользоватьГрафикиРаботы")
			И ЗначениеЗаполнено(СотрудникФизЛица) Тогда
					
			ГрафикРаботы = ГрафикиРаботы.ГрафикРаботыСотрудника(СотрудникФизЛица);
			...
			Для Каждого ЭлементРабочееВремя Из ГрафикРаботы.РабочееВремя Цикл
				...
				Для ДеньНедели = 1 По 5 Цикл
					...
					ЭлементНастройкиДоступногоВремени.ФизическоеЛицо = ФизЛицо;
	...
	НастройкаДоступногоВремени.Сортировать("ВремяНачала");
	ЗаполнитьРеквизитыФормыИзНастройкиДоступногоВремени(НастройкаДоступногоВремени);
...
&НаКлиенте
Процедура ДоступностьФизЛицаПриИзменении(Элемент)
...
Процедура Готово(Команда)
	...
	Оповестить("Запись_ДоступноеВремяФизЛица");
...
Процедура УстановитьДоступностьЭлементов()
	...
	ДоступенПоРасписанию = (ДоступностьФизЛица 
		= ПредопределенноеЗначение("Перечисление.ДоступностьФизическихЛиц.ДоступенПоРасписанию"));
...
Процедура ЗаписатьНастройкиДоступногоВремениИзРеквизитовФормы()
	...
	Попытка
		...
		РаботаСРабочимКалендаремСервер.УстановитьНастройкуДоступностиФизЛица(
			ФизЛицо, ДоступностьФизЛица);
