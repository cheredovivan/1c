...
Процедура ПередЗаписью(Отказ, Замещение)
	...
	// Для записи набора записей требуем чтобы был установлен отбор по физ. лицу
	ФизическоеЛицо = Отбор.Найти("ФизическоеЛицо").Значение;
	Если Не ЗначениеЗаполнено(ФизическоеЛицо) Тогда
		ВызватьИсключение НСтр("ru = 'Запрещено записывать набор регистра сведений ""Доступное время физических лиц"" без установки отбора по физическому лицу.'");
	КонецЕсли;
...
Функция ЗаписиСовпадают(Запись1, Запись2)
	...
	Возврат Запись1.ДеньНедели = Запись2.ДеньНедели
		...
		И Запись1.ФизическоеЛицо = Запись2.ФизическоеЛицо;
	...
	Пока ТребуетсяВнестиКоррективы Цикл
		...
		Для Каждого Запись Из МассивЗаписей Цикл
			...
			Для Каждого ВнесеннаяЗапись Из ЭтотОбъект Цикл
				...
				Если Запись.ФизическоеЛицо = ВнесеннаяЗапись.ФизическоеЛицо
