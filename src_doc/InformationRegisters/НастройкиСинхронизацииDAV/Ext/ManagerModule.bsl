...
//
// Параметры:
//  Узел - ПланОбменаСсылка.СинхронизацияКалендарей - Узел физ. лица, для которого требуется доступ.
//
// Возвращаемое значение:
//  Структура - Ключи соответствуют ресурсам регистра НастройкиСинхронизацииDAV.
//   * Сервер            - Строка - Сервер.
...
Функция ДанныеАвторизации(Узел) Экспорт
	...
	Запрос = Новый Запрос(
	"ВЫБРАТЬ ПЕРВЫЕ 1
	|	НастройкиСинхронизацииDAV.Узел.ФизическоеЛицо КАК ФизЛицо,
...
//
// Параметры:
//  Узел   - ПланОбменаСсылка.СинхронизацияКалендарей - Узел физ. лица, для которого записываются данные.
...
//
// Параметры:
//  Узел   - ПланОбменаСсылка.СинхронизацияКалендарей - Узел физ. лица, для которого удаляются данные.
//
