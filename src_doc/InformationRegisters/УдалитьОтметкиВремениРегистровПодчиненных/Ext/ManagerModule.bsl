...
Функция ПереходНаНовуюВерсиюКоличествоОбъектов() Экспорт
	...
	Запрос = Новый Запрос(
		...
		|		Таблица.Объект
		|	ИЗ
		|		РегистрСведений.УдалитьОтметкиВремениРегистровПодчиненных КАК Таблица
		|	СГРУППИРОВАТЬ ПО
		|		Таблица.Ключ,
	...
	Пока Выборка.Следующий() Цикл
		...
		Исключение
			...
			ТекстСообщения = СтрШаблон(
				НСтр("ru = 'Не удалось удалить данные РС УдалитьОтметкиВремениРегистровПодчиненных
							|по причине:
							|%1'",
			...
			ЗаписьЖурналаРегистрации(
				...
				Метаданные.РегистрыСведений.УдалитьОтметкиВремениРегистровПодчиненных, ,
				ТекстСообщения);
		КонецПопытки;
	КонецЦикла;
