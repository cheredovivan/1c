...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Список.Параметры.УстановитьЗначениеПараметра("Объект", Параметры.Документ);
...
Функция УдалитьЗаписи(ТекущаяСтрока)
	...
	ТекущийОбъект = ТекущаяСтрока.Объект;
	ТекущийТипСвязи = ТекущаяСтрока.ТипСвязи;
	
	Если ЗначениеЗаполнено(ТекущаяСтрока.СвязанныйОбъект) Тогда 
		ТекущийСвязанныйОбъект = ТекущаяСтрока.СвязанныйОбъект;
	ИначеЕсли ЗначениеЗаполнено(ТекущаяСтрока.СвязаннаяСтрока) Тогда 
		ТекущийСвязанныйОбъект = ТекущаяСтрока.СвязаннаяСтрока;
	КонецЕсли;
	
	// удаление связи
	СвязиОбъектов.УдалитьСвязь(ТекущийОбъект, ТекущийСвязанныйОбъект, ТекущийТипСвязи);
	РеквизитыВладельцаИзменены = СвязиОбъектов.УстановитьРеквизитыПриУдаленииСвязи(ТекущийОбъект,
...
Процедура СписокПередНачаломДобавления(Элемент, Отказ, Копирование, Родитель, Группа)
	...
	Если Копирование Тогда 
		...
		Открытьформу("РегистрСведений.СвязиОбъектов.ФормаЗаписи", ПараметрыФормы, Элементы.Список);
	Иначе 
		ОчиститьСообщения();
...
Процедура СписокПередУдалениемПродолжение(Результат, Параметры)Экспорт 
	...
	Если Результат = КодВозвратаДиалога.Да Тогда 
		...
		ТекущийДокумент = ТекущиеДанные.Объект;
...
Процедура СписокПеретаскивание(Элемент, ПараметрыПеретаскивания, СтандартнаяОбработка, Строка, Поле)
	...
	Если ЗначениеЗаполнено(СвязанныйДокумент)
		...
		ПараметрыФормы = Новый Структура;
		ПараметрыФормы.Вставить("Документ", Параметры.Документ);
		ПараметрыФормы.Вставить("СвязанныйОбъект", СвязанныйДокумент);
...
Процедура ПерейтиКСсылке(Команда)
	...
	Если ТипЗнч(ТекущиеДанные.СвязанныйОбъект) = Тип("Строка") Тогда 
		ПерейтиПоНавигационнойСсылке(ТекущиеДанные.СвязанныйОбъект);
	Иначе
...
Процедура МК_НастроитьЭлементыФормы()
	...
	Элементы.ГруппаСписки.ЦветФона = ЦветаСтиля.МК_ЦветФонаГруппы;
	Элементы.ГруппаСписки.ВертикальныйИнтервал = ИнтервалМеждуЭлементамиФормы.Половинный;
	
	Элементы.Список.Шапка = Ложь;
	Элементы.Переместить(Элементы.Список, Элементы.МК_ГруппаСписка);
	Элементы.Переместить(Элементы.СвязанныйОбъект, Элементы.МК_СписокГруппаПерваяСтрока);
	Элементы.Переместить(Элементы.ТипСвязи, Элементы.МК_СписокГруппаПерваяСтрока);
	Элементы.Переместить(Элементы.Комментарий, Элементы.МК_СписокГруппаВтораяСтрока);
	
	Элементы.ТипыСвязей.ИспользованиеТекущейСтроки =
