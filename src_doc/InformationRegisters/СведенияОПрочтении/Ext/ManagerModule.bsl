...
// Убирает признак Прочтено для объекта у всех пользователей, кроме текущего.
Процедура УбратьСведенияОПрочтенииВсемКромеАвтора(Объект) Экспорт
	...
	// ставим "прочтен" текущем пользователю.
	РаботаСПрочтениями.УстановитьСвойствоПрочтен(Объект, Истина);
...
//   Значение - Булево
// 
Функция ПометкиПрочтения(Пользователь, Объекты) Экспорт
	
	ПометкиПрочтения = Новый Соответствие;
	Для Каждого Объект Из Объекты Цикл
		ПометкиПрочтения[Объект] = Ложь;
	КонецЦикла;
	
	Если Не ЗначениеЗаполнено(Пользователь) Или Объекты.Количество() = 0 Тогда
		Возврат ПометкиПрочтения;
	КонецЕсли;
	
	Запрос = Новый Запрос(
		"ВЫБРАТЬ РАЗРЕШЕННЫЕ
		|	СведенияОПрочтении.Объект КАК Объект,
		|	СведенияОПрочтении.Прочтен КАК Прочтен
		|ИЗ
		|	РегистрСведений.СведенияОПрочтении КАК СведенияОПрочтении
		|ГДЕ
		|	СведенияОПрочтении.Пользователь = &Пользователь
		|	И СведенияОПрочтении.Объект В (&Объекты)");
	Запрос.УстановитьПараметр("Пользователь", Пользователь);
	Запрос.УстановитьПараметр("Объекты", Объекты);
	РезультатЗапроса = Запрос.Выполнить();
	ВыборкаДетальныеЗаписи = РезультатЗапроса.Выбрать();
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		ПометкиПрочтения[ВыборкаДетальныеЗаписи.Объект] = ВыборкаДетальныеЗаписи.Прочтен;
	КонецЦикла;
