...
Функция ИсторияСостоянийПоДокументу(Документ) Экспорт
	...
	Запрос.УстановитьПараметр("Документ", Документ);
	// Чтобы напрямую не задавать тип предмета МЭДО, т.к. он зависит от встраивания в конфигурацию:
	Если ЗначениеЗаполнено(Документ) Тогда
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "&НеПустойДокумент", "ИСТИНА");
	Иначе
		// Для пустого документа (например, создание нового) - будет всегда пустая таблица.
		// Это нужно для того, чтобы уведомления от отказе с не заполненным документом (когда не распознан входящий XML)
		// не присоединялись ко всем новым созданным документам в форме:
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "&НеПустойДокумент", "ЛОЖЬ");
	КонецЕсли;
