...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СостояниеВыполненияЗадачи = ОбработкаОчередиЗаданийСервер.СостояниеВыполненияЗадачи(Объект.Ссылка);
	...
	Если Объект.Важность = Перечисления.ВариантыВажностиОбъектов.Высокая Тогда
		ВажностьСтрокой = НСтр("ru = 'Высокая'");
	ИначеЕсли Объект.Важность = Перечисления.ВариантыВажностиОбъектов.Обычная Тогда
		ВажностьСтрокой = НСтр("ru = 'Обычная'");
	ИначеЕсли Объект.Важность = Перечисления.ВариантыВажностиОбъектов.Низкая Тогда
	...
	ЗадачаДелегированаТекущемуПользователю = Ложь;
	
	ТекущийИсполнительСотрудник = 
	...
	Для Каждого СотрудникПользователя Из СотрудникиПользователя Цикл
		...
		Если Не РегистрыСведений.ИсполнителиРолейИДелегаты.ИсполнительЯвляетсяДелегатом(
			ТекущийИсполнительСотрудник, СотрудникПользователя, 
			ЗамещающиеИПомощники.ИменаОбластейЗамещенияПоЗадачам()) Тогда
			
			Продолжить;
		КонецЕсли;
		
		ЗадачаДелегированаТекущемуПользователю = Истина;
		Прервать;
