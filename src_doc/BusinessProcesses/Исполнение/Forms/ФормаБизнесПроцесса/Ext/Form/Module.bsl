...
Процедура УстановитьДоступность() Экспорт
	...
	Если Объект.Исполнители.Количество() > 1 Тогда 
		Элементы.НазначитьОтветственным.Доступность = Элементы.Исполнители.ИзменятьСоставСтрок;
	КонецЕсли;
...
Процедура НастроитьФормуДляМобильногоПриНеобходимости()
	...
	Элементы.ПорядокИсполнения.Ширина = 0;
	Элементы.ИсполнителиСрокИсполненияПредставление.Ширина = 0;
	
	Элементы.КоманднаяПанельСпискаИсполнители.Видимость = Ложь;
	Элементы.Исполнители.ПоложениеКоманднойПанели = ПоложениеКоманднойПанелиЭлементаФормы.Авто;
	
	// Переместим команды ТЧ Исполнители в ком. панель
	Элементы.Переместить(Элементы.Подобрать,
		Элементы.Исполнители.КоманднаяПанель, Элементы.ИсполнителиКонтекстноеМенюВводОписанияЗадачи);
	...
	Элементы.Переместить(Элементы.НазначитьОтветственным,
		Элементы.Исполнители.КоманднаяПанель, Элементы.ИсполнителиКонтекстноеМенюВводОписанияЗадачи);
	
	Элементы.Переместить(Элементы.ИсполнителиКонтекстноеМенюОсобыеСрокиИОписаниеДляИсполнителей,
		Элементы.КоманднаяПанельСпискаИсполнители);
	
	Элементы.ИсполнителиКонтекстноеМенюВывестиСписок.Видимость = Ложь;
	Элементы.ИсполнителиОписание.МногострочныйРежим = Истина;
	Элементы.ИсполнителиКонтекстноеМенюВводОписанияЗадачи.Видимость = Ложь;
		
	Элементы.Проверяющий.ГоризонтальноеПоложение = ГоризонтальноеПоложениеЭлемента.Право;
	Элементы.Проверяющий.ПоложениеЗаголовка = ПоложениеЗаголовкаЭлементаФормы.Верх;
