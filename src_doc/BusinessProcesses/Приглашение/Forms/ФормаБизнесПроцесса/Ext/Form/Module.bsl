...
Процедура ПередЗаписью(Отказ, ПараметрыЗаписи)
	...
	Если Не ПараметрыЗаписи.Свойство("ЗанятостьИсполнителейПроверена")
		...
		Если Не РаботаСРабочимКалендаремКлиент.ПроверитьДоступностьФизЛицСотрудников(
				МассивИсполнителей, Объект.ДатаНачалаМероприятия,
		...
		Если УправлениеМероприятиями.ЭтоМероприятие(Предмет, Мероприятие) 
			...
			Попытка
				...
				Если Объект.ДатаНачалаМероприятия <> Мероприятие.ДатаНачала 
					...
					Если Не ДокументооборотПраваДоступа.ПраваПользователяПоОбъекту(Мероприятие).Изменение Тогда
				...
				Для Каждого Строка Из Объект.Исполнители Цикл
					...
					СотрудникИсполнителя = 
						Сотрудники.ОсновнойСотрудникПользователя(Строка.Исполнитель);
					
					СтруктураПоиска = Новый Структура;
					СтруктураПоиска.Вставить("Исполнитель", СотрудникИсполнителя);
					...
					Иначе
						...
						НоваяЗапись.Исполнитель = СотрудникИсполнителя;
...
Процедура ИсполнителиПриАктивизацииСтроки(Элемент)
	...
	#Если МобильныйКлиент Тогда
		...
		Элементы.ИсполнителиПредставлениеЧасовогоПояса.Видимость =
			(ЗначениеЗаполнено(ТекущиеДанные.ПредставлениеЧасовогоПояса));
	#КонецЕсли
...
Процедура МК_НастроитьЭлементыФормы()
	...
	Элементы.ГруппаМестоВремя.Группировка = ГруппировкаПодчиненныхЭлементовФормы.ГоризонтальнаяЕслиВозможно;
	...
	Элементы.ИсполнителиКонтекстноеМенюУстановитьЯвкаОбязательнаДляВсех.Видимость = Ложь;
	Элементы.ИсполнителиКонтекстноеМенюСнятьЯвкаОбязательнаДляВсех.Видимость = Ложь;
