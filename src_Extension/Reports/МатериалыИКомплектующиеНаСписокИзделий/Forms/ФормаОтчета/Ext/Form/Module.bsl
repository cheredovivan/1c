...
Процедура ПриОткрытии(Отказ)
	...
	// Также непосредственно формируем, если пользователь добавил изделия вручную (они не сериализуются в фоновое задание).
	Непосредственно = НастройкиОтчета.Внешний Или НастройкиОтчета.Безопасный Или Отчет.Изделия.Количество() > 0;
...
Процедура ПрименитьНастройкиИПереформироватьОтчет(Результат, ПараметрыВыполнения) Экспорт
	...
	Если Не Результат.Свойство("ПараметрыИзменены") Тогда
...
Процедура Сформировать()
	...
	// Если пользователь добавил изделия вручную - формируем непосредственно (не в фоне),
	// т.к. табличная часть не сериализуется в фоновое задание
	ФормироватьНепосредственно = НастройкиОтчета.Внешний Или НастройкиОтчета.Безопасный Или Отчет.Изделия.Количество() > 0;
	Результат = РезультатФормированияОтчета(ФормированиеПриОткрытии, ФормироватьНепосредственно, Ложь);
...
Процедура СформироватьПослеПредупреждения(Ответ, Контекст) Экспорт
	...
	ИначеЕсли ТипЗнч(Ответ) = Тип("Структура") Тогда
		...
		Если Ответ.Значение = "Продолжить" Тогда
			...
			ФормироватьНепосредственно = НастройкиОтчета.Внешний Или НастройкиОтчета.Безопасный Или Отчет.Изделия.Количество() > 0;
			Результат = РезультатФормированияОтчета(ФормированиеПриОткрытии, ФормироватьНепосредственно, Истина);
		Иначе
			Возврат;
		КонецЕсли;
	Иначе
...
Процедура ПрименитьЗначениеУниверсальногоПоиска(ВариантыПоиска)
	...
	Если Не ЭтоПользовательскийПоиск Тогда 
		...
		РезультатНастройки.Вставить("ПараметрыИзменены", Ложь);
		ПодключитьОбработчикОжидания("ОбновитьЭлементыФормыНастроекОтложенно", 0.1, Истина);
	КонецЕсли;
...
Процедура ОбновитьЭлементыФормыНастроекНаСервере(ПараметрыОбновления = Неопределено)
	// Добавление полей, требуемых новой версией БСП
	Если ТипЗнч(ПараметрыОбновления) = Тип("Структура") Тогда
		Если Не ПараметрыОбновления.Свойство("ПараметрыИзменены") Тогда
			ПараметрыОбновления.Вставить("ПараметрыИзменены", Ложь);
		КонецЕсли;
	КонецЕсли;
	...
	//ОтчетыСервер.ВосстановитьЗначенияОтборов(ЭтотОбъект); // Метод удалён в новой версии БСП
...
Процедура ПроверитьПараметрыЗагрузки(ПараметрыЗагрузки)
	...
	Если Не ПараметрыЗагрузки.Свойство("КомпоновщикНастроекКД") Тогда
		ПараметрыЗагрузки.Вставить("КомпоновщикНастроекКД", Отчет.КомпоновщикНастроек);
	КонецЕсли;
	
	Если Не ПараметрыЗагрузки.Свойство("СброситьПользовательскиеНастройки") Тогда
		ПараметрыЗагрузки.Вставить("СброситьПользовательскиеНастройки", Ложь);
	КонецЕсли;
	
	Если Не ПараметрыЗагрузки.Свойство("СброситьНастройкиВарианта") Тогда
		ПараметрыЗагрузки.Вставить("СброситьНастройкиВарианта", Ложь);
	КонецЕсли;
	
	Если Не ПараметрыЗагрузки.Свойство("УсловияОтборов") Тогда
		ПараметрыЗагрузки.Вставить("УсловияОтборов", Неопределено);
	КонецЕсли;
	
	Если Не ПараметрыЗагрузки.Свойство("ПараметрыИзменены") Тогда
		ПараметрыЗагрузки.Вставить("ПараметрыИзменены", Ложь);
	КонецЕсли;
...
&НаСервере
Процедура ИзделияНоменклатураПриИзмененииНаСервере(знач Номенклатура,Спецификация)
	Запрос = Новый Запрос;
	Запрос.Текст = 
		"ВЫБРАТЬ
...
&НаКлиенте
Процедура ИзделияНоменклатураПриИзменении(Элемент)
	ТД = Элементы.Изделия.ТекущиеДанные;
	Номенклатура = ТД.Номенклатура;
	Если ТД.Количество=0 тогда
		ТД.Количество = 1000;
	КонецЕсли;
	Спецификация = ТД.Спецификация;
	ИзделияНоменклатураПриИзмененииНаСервере(Номенклатура,Спецификация);
	ТД.Спецификация = Спецификация;
КонецПроцедуры
