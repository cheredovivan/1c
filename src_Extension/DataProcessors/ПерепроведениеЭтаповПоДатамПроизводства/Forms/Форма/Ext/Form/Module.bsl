...
Процедура ПерепровестиДокументыНаСервере()
	...
	Для Каждого СтрокаТаблицы Из Объект.НайденныеДокументы Цикл
		...
		Попытка
			...
			// Перепроводим документ (попытка проведения покажет конкретную ошибку, если документ не заполнен)
			ДокументОбъект.Записать(РежимЗаписиДокумента.Проведение, РежимПроведения);
			
			// Обновляем статус (развёртка будет во второй фазе)
	...
	// === ФАЗА 2: Развёртка полуфабрикатов для всех успешно проведённых документов ===
	// Теперь все базовые движения в регистрах актуальны, можно делать развёртку.
	// Порядок обработки важен: от ранних этапов к поздним (таблица уже отсортирована).
	...
	Для Каждого СтрокаТаблицы Из Объект.НайденныеДокументы Цикл
		...
		Попытка
			...
			Расш1_ОбщийМодуль1.РазвернутьПолуфабрикатыДокумента(ДокументОбъект);
...
Функция ПопытатьсяИсправитьОшибкуИПровестиПовторно(ДокументОбъект, ТекстОшибки, РежимПроведения)
	...
	Если СтрНачинаетсяС(ТекстОшибки, "Количество расхода материала") 
		...
		Исключение
			// Если не удалось исправить, возвращаем Ложь
			Возврат Ложь;
		КонецПопытки;
	КонецЕсли;
	...
	Если СтрНайти(ТекстОшибки, "Не заполнена колонка ""Выполнено""") > 0 
		...
		Попытка
			// Извлекаем номер строки из текста ошибки
			// Формат: "Не заполнена колонка "Выполнено" в строке 3 списка "Трудозатраты""
			НомерСтроки = ИзвлечьНомерСтрокиИзОшибки(ТекстОшибки);
			
			Если НомерСтроки > 0 Тогда
				// Удаляем указанную строку из ТЧ Трудозатраты
				// Номер строки в ошибке - это порядковый номер (начиная с 1), а в табличной части - индекс (начиная с 0)
	...
	Если ИсправлениеВыполнено Тогда
		Попытка
			...
			Возврат Истина;
		Исключение
			// Если повторное проведение не удалось, возвращаем Ложь
			Возврат Ложь;
		КонецПопытки;
	КонецЕсли;
...
Процедура ПерезаполнитьРасходПоОбеспечению(ДокументОбъект)
	...
	// Формируем структуру для вызова функции перезаполнения
	...
	// Перезаполняем ТЧ Расход по ТЧ Обеспечение
	// Режим 0 - полное перезаполнение (расход предварительно очищается)
	Документы.ЭтапПроизводства2_2.ЗаполнитьРасходМатериаловИРаботПоДаннымОбеспечения(СтруктураОбъекта, 0);
	
	// Загружаем результат обратно в объект документа
...
Функция ИзвлечьНомерСтрокиИзОшибки(ТекстОшибки)
	...
	// Ищем паттерн "в строке X списка"
...
&НаСервере
Процедура ВыполнитьДополнительнуюРазверткуПолуфабрикатов()
	
	// Дополнительный цикл развёртки для обработки ситуаций,
	// когда источник полуфабриката был развёрнут позже потребителя.
	// Повторяем до тех пор, пока есть неразвёрнутые полуфабрикаты.
	
	МаксИтераций = 100; // защита от бесконечного цикла
	НомерИтерации = 0;
	...
	Пока НомерИтерации < МаксИтераций Цикл
		...
		Для Каждого СтрокаТаблицы Из Объект.НайденныеДокументы Цикл
			Если НЕ СтрНачинаетсяС(СтрокаТаблицы.Результат, "Успешно") Тогда
				Продолжить; // пропускаем документы с ошибками
			КонецЕсли;
			
			// Проверяем, есть ли неразвёрнутые полуфабрикаты
			Кандидат = Расш1_ОбщийМодуль1.НайтиКандидатаПолуфабриката(СтрокаТаблицы.СсылкаНаЭтап);
			Если Кандидат <> Неопределено Тогда
				// Выполняем развёртку
				ДокументОбъект = СтрокаТаблицы.СсылкаНаЭтап.ПолучитьОбъект();
				Расш1_ОбщийМодуль1.РазвернутьПолуфабрикатыДокумента(ДокументОбъект);
				БылаРазвертка = Истина;
			КонецЕсли;
		КонецЦикла;
		
		Если НЕ БылаРазвертка Тогда
			Прервать; // все полуфабрикаты развёрнуты
		КонецЕсли;
	КонецЦикла;
...
Функция ИзвлечьЧислаИзНомера(НомерДокумента)
	...
	// Номер вида "ПОУП-1570.7.1" → части: 1570, 7, 1
